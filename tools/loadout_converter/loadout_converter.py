...
print(f"Converted {len(loadouts)} gear listings.\n")

# Update the database.
print("Updating database entries...")
for id in loadouts:
	gear = loadouts[id]
	cursor.execute(f"UPDATE `characters` SET `gear` = '{gear}' WHERE `id` = '{id}';")
db.commit()

stopwatch = time.time()

print(f"Database updated in {round(stopwatch - startwatch)} seconds!")
