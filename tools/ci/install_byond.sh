...
then
  ...
  mkdir -p "$HOME/BYOND"
  cd "$HOME/BYOND"
  curl "https://cmss13-devs.github.io/byond-build-mirror/download/build/${BYOND_MAJOR}/${BYOND_MAJOR}.${BYOND_MINOR}_byond_linux.zip" -H "User-Agent: ss220club/Paradise-SS220 CI" -o byond.zip
  unzip byond.zip
  rm byond.zip
  cd byond
  make here
