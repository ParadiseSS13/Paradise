...
def check_515_proc_syntax(lines):
    for idx, line in enumerate(lines):
        if CHECK_515_PROC_MARKER_RE.search(line):
            return Failure(idx, "Outdated proc reference use detected in code. Please use proc reference helpers.")
...
def check_global_vars(lines):
    for idx, line in enumerate(lines):
        if GLOBAL_VARS_RE.match(line):
            return Failure(idx, "Unmanaged global var use detected in code, please use the helpers.")
...
def check_toplevel_vardecls(lines):
    for idx, line in enumerate(lines):
        if match := TOPLEVEL_VARDECLS_RE.match(line):
            return Failure(idx, f"Top-level var {match.group(0)} found, please move to type declaration.")
...
def check_proc_args_with_var_prefix(lines):
    for idx, line in enumerate(lines):
        if PROC_ARGS_WITH_VAR_PREFIX_RE.match(line):
            return Failure(idx, "Changed files contains a proc argument starting with 'var'.")
...
def check_for_nanotrasen_camel_case(lines):
    for idx, line in enumerate(lines):
        if NANOTRASEN_CAMEL_CASE.search(line):
            return Failure(idx, "Nanotrasen should not be spelled in the camel case form.")
...
def check_to_chats_have_a_user_arguement(lines):
    for idx, line in enumerate(lines):
        if TO_CHAT_WITH_NO_USER_ARG_RE.search(line):
            return Failure(idx, "Changed files contains a to_chat() procedure without a user argument.")
...
def check_conditional_spacing(lines):
    for idx, line in enumerate(lines):
        if CONDITIONAL_LEADING_SPACE.search(line):
            return Failure(idx, "Found a conditional statement matching the format \"if (thing)\" (irregular spacing), please use \"if(thing)\" instead.")
        ...
        if CONDITIONAL_DOUBLE_PARENTHESIS.search(line):
            return Failure(idx, "Found a conditional statement matching the format \"if((thing))\" (unnecessary outer parentheses), please use \"if(thing)\" instead.")
...
def check_global_list_empty(lines):
    for idx, line in enumerate(lines):
        if GLOBAL_LIST_EMPTY.search(line):
            return Failure(idx, "Found a GLOBAL_LIST_INIT(_, list()), please use GLOBAL_LIST_EMPTY(_) instead.")
...
def check_tgui_ui_new_argument(lines):
    for idx, line in enumerate(lines):
        if "\tui = new" in line and not TGUI_UI_NEW.search(line):
            return Failure(idx, "Invalid argument within constructor, please make sure window sizing is in corresponding JavaScript file.")
