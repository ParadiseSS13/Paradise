...
/* Full of placeholders while building

	if(istype(W, "card" && istype(equipment_system.lock_system, "card")
		if(equipment_system.lock_system.mode)
			if(W.getaccess() == equipment_system.lock_system.accesslist)
				locksecondseat()
		else
			if(W.getname() == equipment_system.lock_system.owner
				locksecondseat()
		return
*/
