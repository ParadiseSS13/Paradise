...
!(function (e) {
	...
	function a(e) {
		(this.tokens = []),
			...
			this.options.pedantic
				...
				: this.options.gfm && (this.options.tables ? (this.rules = _.tables) : (this.rules = _.gfm));
	}
	(_._label = /(?!\s*\])(?:\\[\[\]]|[^\[\]])+/),
		(_._title = /(?:"(?:\\"?|[^"\\])*"|'[^'\n]*(?:\n[^'\n]+)*\n?'|\([^()]*\))/),
		(_.def = i(_.def).replace('label', _._label).replace('title', _._title).getRegex()),
		(_.bullet = /(?:[*+-]|\d{1,9}\.)/),
		(_.item = /^( *)(bull) ?[^\n]*(?:\n(?!\1bull ?)[^\n]*)*/),
		...
		(_.paragraph = i(_.paragraph)
			...
			.replace('tag', _._tag)
			.getRegex()),
		(_.blockquote = i(_.blockquote).replace('paragraph', _.paragraph).getRegex()),
		(_.normal = d({}, _)),
		(_.gfm = d({}, _.normal, {
			fences: /^ {0,3}(`{3,}|~{3,})([^`\n]*)\n(?:|([\s\S]*?)\n)(?: {0,3}\1[~`]* *(?:\n+|$)|$)/,
			paragraph: /^/,
		...
		(_.gfm.paragraph = i(_.paragraph)
			.replace(
				'(?!',
				'(?!' + _.gfm.fences.source.replace('\\1', '\\2') + '|' + _.list.source.replace('\\1', '\\3') + '|'
			)
			.getRegex()),
		(_.tables = d({}, _.gfm, {
			nptable: /^ *([^|\n ].*\|.*)\n *([-:]+ *\|[-| :]*)(?:\n((?:.*[^>\n ].*(?:\n|$))*)\n*|$)/,
			...
			for (e = e.replace(/^ +$/gm, ''); e; )
				if (
						...
						/^ *-+: *$/.test(o.align[c])
							...
							: /^ *:-+: *$/.test(o.align[c])
								...
								: /^ *:-+ *$/.test(o.align[c])
									? (o.align[c] = 'left')
									: (o.align[c] = null);
					for (c = 0; c < o.cells.length; c++) o.cells[c] = w(o.cells[c], o.header.length);
					this.tokens.push(o);
					...
					for (
						...
						x++, this.tokens.push({ type: 'blockquote_start' }), (b = b.replace(/^ *> ?/gm, ''));
					for (this.token(b, t), c = 0; c < x; c++) this.tokens.push({ type: 'blockquote_end' });
					...
					for (
						...
						(u = (o = s[c]).length),
							...
							r && (a.loose = !0),
							(m = void 0),
							(k = /^\[[ xX]\] /.test(o)) && ((m = ' ' !== o[1]), (o = o.replace(/^\[[ xX]\] +/, ''))),
							(p = { type: 'list_item_start', task: k, checked: m, loose: r }),
							h.push(p),
							this.tokens.push(p),
							this.token(o, !1),
					...
					for (c = 0; c < o.cells.length; c++)
						o.cells[c] = w(o.cells[c].replace(/^ *\| *| *\| *$/g, ''), o.header.length);
					this.tokens.push(o);
	...
	(n._punctuation = '!"#$%&\'()*+,\\-./:;<=>?@\\[^_{|}~'),
		...
		(n.tag = i(n.tag).replace('comment', _._comment).replace('attribute', n._attribute).getRegex()),
		(n._label = /(?:\[[^\[\]]*\]|\\[\[\]]?|`[^`]*`|`(?!`)|[^\[\]\\`])*?/),
			...
			for (var t, n, r, s, i, l, o = ''; e; )
				if ((i = this.rules.escape.exec(e))) (e = e.substring(i[0].length)), (o += u(i[1]));
					...
					(e = e.substring(i[0].length)),
						...
						(o += this.outputLink(i, { href: p.escapes(r), title: p.escapes(s) })),
						(this.inLink = !1);
		...
		(h.prototype.parse = function (e) {
			(this.inline = new p(e.links, this.options)),
				(this.inlineText = new p(e.links, d({}, this.options, { renderer: new s() }))),
		...
		(h.prototype.tok = function () {
			switch (this.token.type) {
				...
				case 'blockquote_start':
					for (var l = 1; this.peek() && 'blockquote_start' === this.peek().type; ) this.next(), l++;
					for (i = ''; 'blockquote_end' !== this.next().type; ) i += this.tok();
					for (; this.peek() && 'blockquote_end' === this.peek().type; ) this.next();
					for (e = 0; e < l; e++) i = this.renderer.blockquote(i);
					return i;
				case 'list_start':
					i = '';
					for (var o = this.token.ordered, a = this.token.start; 'list_end' !== this.next().type; ) i += this.tok();
				...
				case 'list_item_start':
					...
					var h = this.token.loose,
						...
						u = this.token.task;
					for (this.token.task && (i += this.renderer.checkbox(p)); 'list_item_end' !== this.next().type; )
						i += h || 'text' !== this.token.type ? this.tok() : this.parseText();
