...
/*
	...
	var
		image
			I = image(icon = 'disguise.dmi', icon_state = "disguise", loc = src)
	I.override = 1
...
atom
	proc
		add_alt_appearance(key, img, list/displayTo = list())
			if(!key || !img)
				return
...
atom
	proc
		remove_alt_appearance(key)
			if(alternate_appearances)
				if(alternate_appearances[key])
					var
						datum
							alternate_appearance
								AA = alternate_appearances[key]
					qdel(AA)
...
atom
	proc
		display_alt_appearance(key, list/displayTo)
			if(!alternate_appearances || !key)
				return
			var
				datum
...
atom
	proc
		hide_alt_appearance(key, list/hideFrom)
			if(!alternate_appearances || !key)
				return
			var
				datum
