proc
	getAssignedBlock(var/name,var/list/blocksLeft, var/activity_bounds=DNA_DEFAULT_BOUNDS, var/good=0)
		if(blocksLeft.len==0)
			warning("[name]: No more blocks left to assign!")
			return 0
		var
			assigned = pick(blocksLeft)
				...
				var
					mob
						living
							carbon
								human
									M = new /mob/living/carbon/human(A.loc)
				M.real_name = "Corpse"
				M.death()
				qdel(A)
				continue
