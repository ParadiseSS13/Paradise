			...
			changeling
				hivemind_upload
					sting_action(var/mob/user)
						...
						var
							chosen_name = input("Select a DNA to channel: ", "Channel DNA", null) as null|anything in names
						if(!chosen_name)
							return
						...
						var
							datum
								dna
									chosen_dna = changeling.GetDNA(chosen_name)
						if(!chosen_dna)
							return
...
obj
	effect
		proc_holder
			changeling
				hivemind_download
					can_sting(var/mob/living/carbon/user)
						if(!..())
							return
						var
							datum
			...
			changeling
				hivemind_download
					sting_action(var/mob/user)
						...
						var
							S = input("Select a DNA absorb from the air: ", "Absorb DNA", null) as null|anything in names
						if(!S)	return
						var
							datum
								dna
