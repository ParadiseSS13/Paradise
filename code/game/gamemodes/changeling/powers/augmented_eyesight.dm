...
obj
	effect
		proc_holder
			changeling
				augmented_eyesight
					sting_action(mob/living/carbon/human/user)
						if(!istype(user))
							return
								...
								item
									organ
										internal
											cyberimp
												eyes
													O = user.get_organ_slot("eye_ling")
						if(O)
							O.remove(user)
							qdel(O)
										...
										obj
											item
												organ
													internal
														eyes
															E = owner.get_int_organ(/obj/item/organ/internal/eyes)
			...
			internal
				cyberimp
					eyes
						thermals
							ling
								insert(mob/living/carbon/M, special = 0)
									..()
									if(ishuman(owner))
										var
											mob
			...
			internal
				cyberimp
					eyes
						thermals
							ling
								remove(mob/living/carbon/M, special = 0)
									if(ishuman(owner))
										var
											mob
												living
