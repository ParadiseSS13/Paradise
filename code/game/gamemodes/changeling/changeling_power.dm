		...
		proc_holder
			changeling
				...
				var
					helptext = "" // Details
				var
					chemical_cost = 0 // negative chemical cost is for passive abilities (chemical glands)
				var
					dna_cost = -1 //cost of the sting in dna points. 0 = auto-purchase, -1 = cannot be purchased
				var
					req_dna = 0  //amount of dna needed to use this ability. Changelings always have atleast 1
				var
					req_human = 0 //if you need to be human to use this ability
				var
					req_stat = CONSCIOUS // CONSCIOUS, UNCONSCIOUS or DEAD
				var
					genetic_damage = 0 // genetic damage caused by using the sting. Nothing to do with cloneloss.
				var
					max_genetic_damage = 100 // hard counter for spamming abilities. Not used/balanced much yet.
...
obj
	effect
		proc_holder
			changeling
				proc
					try_to_sting(var/mob/user, var/mob/target)
...
obj
	effect
		proc_holder
			changeling
				proc
					sting_action(var/mob/user, var/mob/target)
						return 0
...
obj
	effect
		proc_holder
			changeling
				proc
					sting_feedback(var/mob/user, var/mob/target)
						return 0
...
obj
	effect
		proc_holder
			changeling
				proc
					take_chemical_cost(var/datum/changeling/changeling)
...
obj
	effect
		proc_holder
			changeling
				proc
					can_sting(var/mob/user, var/mob/target)
...
obj
	effect
		proc_holder
			changeling
				proc
					transfer_changeling_powers(var/mob/living/carbon/user, var/mob/living/carbon/target)
