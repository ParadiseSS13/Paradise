var
	list
		possible_changeling_IDs = list("Alpha","Beta","Gamma","Delta","Epsilon","Zeta","Eta","Theta","Iota","Kappa","Lambda","Mu","Nu","Xi","Omicron","Pi","Rho","Sigma","Tau","Upsilon","Phi","Chi","Psi","Omega")
...
datum
	game_mode
		proc
			forge_changeling_objectives(var/datum/mind/changeling)
							...
							var
								mob
									living
										carbon
											human
												H = identity_theft.target.current
...
datum
	game_mode
		proc
			greet_changeling(var/datum/mind/changeling, var/you_are=1)
				if(you_are)
...
datum
	game_mode
		proc
			remove_changeling(datum/mind/changeling_mind)
...
datum
	game_mode
		proc
			update_change_icons_added(datum/mind/changeling)
				var
					datum
						atom_hud
							antag
								linghud = huds[ANTAG_HUD_CHANGELING]
...
datum
	game_mode
		proc
			update_change_icons_removed(datum/mind/changeling)
				var
					datum
						atom_hud
							antag
...
datum
	game_mode
		proc
			grant_changeling_powers(mob/living/carbon/changeling_mob)
	...
	game_mode
		proc
			auto_declare_completion_changeling()
				if(changelings.len)
					var
						text = "<FONT size = 3><B>The changelings were:</B></FONT>"
					for(var/datum/mind/changeling in changelings)
						var
							changelingwin = 1
...
datum
	changeling //stores changeling powers, changeling recharge thingie, changeling absorbed DNA and changeling ID (for changeling hivemind)
		var
			list
				absorbed_dna = list()
		var
		...
		var
			list
				protected_dna = list() //DNA that is not lost when capacity is otherwise full.
		var
			dna_max = 4 //How many extra DNA strands the changeling can store for transformation.
		var
			absorbedcount = 1 //Would require at least 1 sample to take on the form of a human
		var
			chem_charges = 20
		var
...
datum
	changeling
		proc
			can_absorb_dna(var/mob/living/carbon/user, var/mob/living/carbon/target)
