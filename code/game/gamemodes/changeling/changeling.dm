		...
		if(!(locate(/datum/objective/escape) in changeling.objectives))
			...
			if(identity_theft.target && identity_theft.target.current)
				...
				if(can_absorb_species(H.species)) // For species that can't be absorbed - should default to an escape objective
					identity_theft.explanation_text = "Escape on the shuttle or an escape pod with the identity of [identity_theft.target_real_name], the [identity_theft.target.assigned_role] while wearing [identity_theft.p_their()] identification card."
