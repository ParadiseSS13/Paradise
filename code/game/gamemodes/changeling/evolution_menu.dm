	...
	effect
		proc_holder
			changeling
				evolution_menu
					proc
						create_menu(var/datum/changeling/changeling)
							var
								dat
							...
							for(var/obj/effect/proc_holder/changeling/cling_power in sting_paths)
								...
								var
									ownsthis = changeling.has_sting(cling_power)
			...
			changeling
				evolution_menu
					Topic(href, href_list)
						...
						var
							dat = create_menu(usr.mind.changeling)
...
datum
	changeling
		proc
			purchasePower(var/mob/living/carbon/user, var/sting_name)
		...
		make_changeling()
			...
			var
				mob
					living
						carbon
							C = src		//only carbons have dna now, so we have to typecaste
...
mob
	proc
		changeling_update_languages(var/updated_languages)
...
mob
	proc
		remove_changeling_powers(var/keep_free_powers=0)
			if(ishuman(src))
...
datum
	changeling
		proc
			has_sting(obj/effect/proc_holder/changeling/power)
