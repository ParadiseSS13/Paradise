	...
	game_mode
		...
		var
			list
				datum
					mind
						shadowling_thralls = list()
		var
			list
				shadow_objectives = list()
		var
			required_thralls = 15 //How many thralls are needed (hardcoded for now)
		var
			shadowling_ascended = 0 //If at least one shadowling has ascended
		var
			shadowling_dead = 0 //is shadowling kill
		var
			...
			pre_setup()
				...
				var
					list
						datum
							mind
								possible_shadowlings = get_players_for_role(ROLE_SHADOWLING)

				if(!possible_shadowlings.len)
					return 0

				var
					shadowlings = max(3, round(num_players()/14))

				while(shadowlings)
					var
						datum
							mind
								shadow = pick(possible_shadowlings)
					shadows += shadow
...
datum
	game_mode
		proc
			greet_shadow(var/datum/mind/shadow)
...
datum
	game_mode
		proc
			process_shadow_objectives(var/datum/mind/shadow_mind)
				var
					objective = "enthrall" //may be devour later, but for now it seems murderbone-y
...
datum
	game_mode
		proc
			finalize_shadowling(var/datum/mind/shadow_mind)
				var
					mob
						living
							carbon
...
datum
	game_mode
		proc
			add_thrall(datum/mind/new_thrall_mind)
...
datum
	game_mode
		proc
			remove_thrall(datum/mind/thrall_mind, var/kill = 0)
...
datum
	game_mode
		shadowling
			check_finished()
				var
					shadows_alive = 0 //and then shadowling was kill
...
datum
	game_mode
		proc
			remove_shadowling(datum/mind/ling_mind)
...
datum
	game_mode
		shadowling
			proc
				check_shadow_victory()
					var
						success = 0 //Did they win?
...
datum
	game_mode
		proc
			auto_declare_completion_shadowling()
				var
					text = ""
				if(shadows.len)
...
datum
	species
		shadow
			ling
				lesser //Empowered thralls. Obvious, but powerful
...
datum
	game_mode
		proc
			update_shadow_icons_added(datum/mind/shadow_mind)
				var
					datum
						atom_hud
							antag
								shadow_hud = huds[ANTAG_HUD_SHADOW]
...
datum
	game_mode
		proc
			update_shadow_icons_removed(datum/mind/shadow_mind) //This should never actually occur, but it's here anyway.
				var
					datum
						atom_hud
							antag
