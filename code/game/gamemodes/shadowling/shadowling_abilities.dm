...
obj
	effect
		proc_holder
			spell
				proc
					shadowling_check(var/mob/living/carbon/human/H)
...
obj
	effect
		proc_holder
			spell
				targeted
					glare //Stuns and mutes a human target for 10 seconds
						name = "Glare"
			...
			spell
				aoe_turf
					veil
						...
						range = 5
						var
							blacklisted_lights = list(/obj/item/device/flashlight/flare, /obj/item/device/flashlight/slime)
		...
		proc_holder
			spell
				aoe_turf
					veil
						proc
							extinguishItem(var/obj/item/I) //WARNING NOT SUFFICIENT TO EXTINGUISH AN ITEM HELD BY A MOB
										...
										datum
											data
												pda
													utility
														flashlight
															FL = P.find_program(/datum/data/pda/utility/flashlight)
									if(FL && FL.fon)
										FL.start()
		...
		proc_holder
			spell
				aoe_turf
					veil
						proc
							extinguishMob(var/mob/living/H)
...
obj
	effect
		proc_holder
			spell
				targeted
					enthrall //Turns a target into the shadowling's slave. This overrides all previous loyalties
						name = "Enthrall"
...
obj
	effect
		proc_holder
			spell
				targeted
					shadowling_regenarmor //Resets a shadowling's species to normal, removes genetic defects, and re-equips their armor
						name = "Rapid Re-Hatch"
...
obj
	effect
		proc_holder
			spell
				targeted
					collective_mind //Lets a shadowling bring together their thralls' strength, granting new abilities and a headcount
	...
	effect
		proc_holder
			spell
				targeted
					blindness_smoke
						cast(list/targets) //Extremely hacky
							if(!shadowling_check(usr))
								charge_counter = charge_max
								return
							for(var/mob/living/user in targets)
											...
											weapon
												reagent_containers
													glass
														beaker
															large
																B = new /obj/item/weapon/reagent_containers/glass/beaker/large(user.loc)
								...
								var
									datum
										effect
											system
												chem_smoke_spread
													S = new /datum/effect/system/chem_smoke_spread
								S.attach(B)
								if(S)
...
datum
	reagent
		shadowling_blindness_smoke //Blinds non-shadowlings, heals shadowlings/thralls
										...
										var
											datum
												effect
													system
														spark_spread
															sp = new /datum/effect/system/spark_spread
										sp.set_up(5, 1, S)
										sp.start()
										S.Weaken(6)
	...
	effect
		proc_holder
			spell
				aoe_turf
					drainLife
						cast(list/targets, var/mob/living/carbon/human/U = usr)
							if(!shadowling_check(usr))
								charge_counter = charge_max
								return
							targetsDrained = 0
					...
					reviveThrall
						cast(list/targets)
							...
							for(var/mob/living/carbon/human/thrallToRevive in targets)
								var
									choice = alert(usr,"Empower a living thrall or revive a dead one?",,"Empower","Revive","Cancel")
								switch(choice)
									if("Empower")
							...
							for(var/mob/living/carbon/human/target in targets)
								...
								if(shuttle_master.emergency.mode == SHUTTLE_CALL)
									var
										more_minutes = 9000
									var
										timer = shuttle_master.emergency.timeLeft()
									timer += more_minutes
				...
				targeted
					shadowlingAscendantTransmit
						cast(list/targets)
							for(var/mob/living/user in targets)
								var
									text = stripped_input(user, "What do you want to say to everything on and near [station_name()]?.", "Transmit to World", "")
								if(!text)
									return
