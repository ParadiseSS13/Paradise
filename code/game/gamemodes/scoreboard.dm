				...
				scoreboard()
						...
						for(var/mob/living/player in mob_list)
							if(player.client)
								if(player.stat != DEAD)
									var
										turf
											location = get_turf(player.loc)
									var
										area
											escape_zone = locate(/area/shuttle/escape)
					...
					if(shuttle_master.emergency.mode >= SHUTTLE_ENDGAME)
						for(var/mob/living/carbon/human/E in mob_list)
							...
							var
								turf
									location = get_turf(E.loc)
							var
								area
									escape_zone = shuttle_master.emergency.areaInstance
					...
					var
						deathpoints = score_deadcrew * 25 //done
					var
						researchpoints = score_researchdone * 30
					var
						eventpoints = score_eventsendured * 50
					var
						escapoints = score_escapees * 25 //done
					var
						harvests = score_stuffharvested * 5 //done
					var
						shipping = score_stuffshipped * 5
					var
						mining = score_oremined * 2 //done
					var
						meals = score_meals * 5 //done, but this only counts cooked meals, not drinks served
					var
						power = score_powerloss * 20
					var
						messpoints
					if(score_mess != 0)
						messpoints = score_mess //done
					var
						plaguepoints = score_disease * 30
...
datum
	controller
		gameticker
			proc
				get_score_container_worth(atom/C, level=0)
					if(level >= 5)
						...
						for(var/obj/item/weapon/card/id/id in C.contents)
							var
								datum
									money_account
										A = get_money_account(id.associated_account_number)
							// has an account?
							if(A)
								. += A.money
...
mob
	proc
		scorestats()
			var
				dat = "<b>Round Statistics and Score</b><br><hr>"
			if(ticker && ticker.mode)
