...
datum
	game_mode
		traitor
			...
			var
				const
					traitor_scaling_coeff = 5.0 //how much does the amount of players get divided by to determine traitors
...
datum
	game_mode
		proc
			forge_traitor_objectives(datum/mind/traitor)
				if(istype(traitor.current, /mob/living/silicon))
					...
					var
						martyr_compatibility = 1 //You can't succeed in stealing if you're dead.
...
datum
	game_mode
		proc
			greet_traitor(var/datum/mind/traitor)
...
datum
	game_mode
		proc
			finalize_traitor(var/datum/mind/traitor)
				if(istype(traitor.current, /mob/living/silicon))
...
datum
	game_mode
		proc
			add_law_zero(mob/living/silicon/ai/killer)
				var
	...
	game_mode
		proc
			auto_declare_completion_traitor()
				if(traitors.len)
					var
						text = "<FONT size = 2><B>The traitors were:</B></FONT>"
					for(var/datum/mind/traitor in traitors)
						var
							traitorwin = 1
...
datum
	game_mode
		proc
			equip_traitor(mob/living/carbon/human/traitor_mob, var/safety = 0)
				...
				var
					obj
						item
							R = locate(/obj/item/device/pda) in traitor_mob.contents //Hide the uplink in a PDA if available, otherwise radio
				if(!R)
				...
				if(!R)
					...
					if(istype(R, /obj/item/device/radio))
						...
						var
							pda_pass = "[rand(100,999)] [pick("Alpha","Bravo","Delta","Omega")]"
...
datum
	game_mode
		proc
			update_traitor_icons_added(datum/mind/traitor_mind)
				var
					datum
						atom_hud
							antag
...
datum
	game_mode
		proc
			update_traitor_icons_removed(datum/mind/traitor_mind)
				var
					datum
						atom_hud
							antag
...
datum
	game_mode
		proc
			remove_traitor_mind(datum/mind/traitor_mind, datum/mind/head)
				//var/list/removal
				var
					ref = "\ref[head]"
				if(ref in implanter)
...
datum
	game_mode
		proc
			assign_exchange_role(var/datum/mind/owner)
				//set faction
				var
					faction = "red"
