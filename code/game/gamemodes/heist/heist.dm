...
var
	global
		list
			obj
				cortical_stacks = list() //Stacks for 'leave nobody behind' objective. Clumsy, rewrite sometime.
...
datum
	game_mode/
		...
		var
			list
				raid_objectives = list()     //Raid objectives
	...
	game_mode
		heist
			...
			var
				list
					obj
						cortical_stacks = list() //Stacks for 'leave nobody behind' objective.
			var
				win_button_triggered = 0
	...
	game_mode
		heist
			can_start()
				...
				var
					list
						candidates = get_players_for_role(ROLE_RAIDER)
				var
					raider_num = 0
...
datum
	game_mode
		proc
			create_vox(var/datum/mind/newraider)
	...
	game_mode
		proc
			forge_vox_objectives()
				...
				var
					list
						goals = list("kidnap","loot","salvage")
...
datum
	game_mode
		proc
			greet_vox(var/datum/mind/raider)
			...
			auto_declare_completion_heist()
				if(raiders.len)
					...
					if(ticker && istype(ticker.mode,/datum/game_mode/heist))
						check_return = 1
					var
						text = "<FONT size = 2><B>The Vox raiders were:</B></FONT>"
