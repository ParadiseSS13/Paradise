...
datum
	game_mode
		proc
			set_antag_hud(mob/M, new_icon_state)
				if(!istype(M))
					CRASH("set_antag_hud(): [M] ([M.type]) is not a mob!")
				var
					image
						holder = M.hud_list[SPECIALROLE_HUD]
				if(holder)
...
datum
	game_mode
		proc
			set_nations_hud(mob/M, new_icon_state)
				if(!istype(M))
					CRASH("set_antag_hud(): [M] ([M.type]) is not a mob!")
				var
					image
						holder = M.hud_list[NATIONS_HUD]
				if(holder)
					holder.icon_state = new_icon_state
				if(M.mind || new_icon_state) //in mindless mobs, only null is acceptable, otherwise we're antagging a mindless mob, meaning we should runtime
					M.mind.antag_hud_icon_state = new_icon_state
...
datum
	mind
		proc
			transfer_antag_huds(var/datum/atom_hud/antag/newhud)
				leave_all_huds()
...
datum
	mindslaves
		proc
			add_serv_hud(datum/mind/serv_mind,var/icon)
...
datum
	mindslaves
		proc
			leave_serv_hud(datum/mind/free_mind)
