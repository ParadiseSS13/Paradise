...
proc
	GenerateTheft(var/job,var/datum/mind/traitor)
		var
			list
				datum
					objective
		...
		for(var/o in subtypesof(/datum/objective/steal))
			var
				datum
					objective
						target = new o(null,job)
			objectives += target
			objectives[target] = target.weight
		return objectives

proc
	GenerateAssassinate(var/job,var/datum/mind/traitor)
		var
			list
				datum
					objective
			...
			if((target != traitor) && istype(target.current, /mob/living/carbon/human))
				if(target && target.current)
					var
						datum
							objective
								target_obj = new /datum/objective/assassinate(null,job,target)
					missions += target_obj
					missions[target_obj] = target_obj.weight
		return missions

proc
	GenerateFrame(var/job,var/datum/mind/traitor)
		var
			list
				datum
					objective
			...
			if((target != traitor) && istype(target.current, /mob/living/carbon/human))
				if(target && target.current)
					var
						datum
							objective
								target_obj = new /datum/objective/frame(null,job,target)
					missions += target_obj
					missions[target_obj] = target_obj.weight
		return missions

proc
	GenerateProtection(var/job,var/datum/mind/traitor)
		var
			list
				datum
					objective
			...
			if((target != traitor) && istype(target.current, /mob/living/carbon/human))
				if(target && target.current)
					var
						datum
							objective
								target_obj = new /datum/objective/protection(null,job,target)
					missions += target_obj
					missions[target_obj] = target_obj.weight
		return missions
...
proc
	SelectObjectives(var/job,var/datum/mind/traitor,var/hijack = 0)
		var
			list
				chosenobjectives = list()
		var
			list
				theftobjectives = GenerateTheft(job,traitor)		//Separated all the objective types so they can be picked independantly of each other.
		var
			list
				killobjectives = GenerateAssassinate(job,traitor)
		var
			list
				frameobjectives = GenerateFrame(job,traitor)
		var
			list
				protectobjectives = GenerateProtection(job,traitor)
		var
			total_weight
		var
			conflict
		...
		var
			steal_weight = THEFT_PROBABILITY
		var
			frame_weight = FRAME_PROBABILITY
		var
			kill_weight = KILL_PROBABILITY
		var
			protect_weight = PROTECT_PROBABILITY
		var
			target_weight = 50
...
/////////////////////////////////////////////////////////////
	...
	var
		savefile
			info = new("data/player_saves/[copytext(traitor.key, 1, 2)]/[traitor.key]/traitor.sav")
	var
		list
			infos
	info >> infos
	if(istype(infos))
		var
			total_attempts = infos["Total"]
		var
			total_overall_success = infos["Success"]
		var
			success_ratio = total_overall_success/total_attempts
		var
			steal_success = infos["Steal"]
		var
			kill_success = infos["Kill"]
		var
			frame_success = infos["Frame"]
		var
			protect_success = infos["Protect"]

		var
			list
				ordered_success = list(steal_success, kill_success, frame_success, protect_success)

		var
			difficulty = pick(LENIENT, LENIENT, NORMAL, NORMAL, NORMAL, HARD, HARD, IMPOSSIBLE)
		//Highest to lowest in terms of success rate, and resulting weight for later computation
		var
			success_weights = list(1, 1, 1, 1)
		switch(difficulty)
			if(LENIENT)
		...
		var
			total_weights = kill_weight + protect_weight + frame_weight + steal_weight
	...
	var
		steal_range = steal_weight
	var
		frame_range = frame_weight + steal_range
	var
		kill_range = kill_weight + frame_range
	//Protect is whatever is left over.
	...
	objective
		...
		var
			datum
				mind
					target
		var
			explanation_text = "text not set"
		var
			job
		var
			points = INFINITY //If this isn't set to something else, the objective is bugged and should be ignored
		var
			weight = INFINITY
		...
		frame
			...
			check_completion()
				...
				var
					turf
						location = get_turf(target.current.loc)
				if(!location)
					return 0
			...
			get_points()
				if(target)
					var
						difficulty = GetRank(target.assigned_role) + 1
					switch(GetRank(job))
						if(4)
		...
		hijack
			...
			check_completion()
				...
				var
					turf
						location = get_turf(owner.current.loc)
			...
			lucy
				...
				check_completion()
					var
						target_amount = 10
					var
						found_amount = 0.0//Always starts as zero.
			...
			*/
			...
			reagent
				...
				New(var/text,var/joba)
					...
					var
						list
							items = list("Sulphuric acid", "Polytrinic acid", "Space Lube", "Unstable mutagen",\
		...
		block
			...
			check_completion()
				...
				var
					area
						shuttle = locate(/area/shuttle/escape/centcom)
				var
	...
	objective
		silence
			...
			check_completion()
				...
				var
					area
						pod1 =    locate(/area/shuttle/escape_pod1/centcom)
				var
					area
						pod2 =    locate(/area/shuttle/escape_pod2/centcom)
				var
					area
						pod3 =    locate(/area/shuttle/escape_pod3/centcom)
				var
					area
						pod4 =    locate(/area/shuttle/escape_pod5/centcom)
