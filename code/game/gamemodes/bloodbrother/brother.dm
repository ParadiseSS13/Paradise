...
/datum/antagonist/brother/on_gain()
    ...
    objective += team.objectives
    owner.special_role = special role
    finalize_brother()
    return ..()
...
/datum/antagonist/brother/on_removal()
    SSticker.mode.brothers -= owner
    if(owner.current)
        to_chat(owner.current,"<span class='userdanger'> You are no longer the [special role]!</span>")
...
/datum/antagonist/brother/remove_innate_effects(mob/living/mob_override)
    var/mod/living/M = mob_override || owner.current
...
/datum/antagonist/brother/proc/get_brother_names()
    ...
    var/brother_text = ""
    for(var/i = 1 to brother.len)
        var/datum/mind/M = brother[i]
        brother_text += M.name
        if(i == brother.len - 1)
            brother_text += " and "
...
/datum/team/brother_team/proc/update_name()
    ...
    for(var/datum/mind/M in members)
        ...
        last_names += split_name[split_name.lan]
...
/datum/team/brother_team/proc/add_objective(datum/objective/O, needs_target = FALSE)
	O.team = src
	if(needs_target)
		O.find_target() //need come back to this 
