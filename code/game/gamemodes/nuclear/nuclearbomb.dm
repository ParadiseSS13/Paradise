						...
						weapon
							disk
								nuclear
									auth = null
			var
				removal_stage = 0 // 0 is no removal, 1 is covers removed, 2 is covers open, 3 is sealant open, 4 is unwrenched, 5 is removed from bolts.
			var
				lastentered
			var
				is_syndicate = 0
						...
						if(ticker.mode && ticker.mode.name == "nuclear emergency")
							var
								obj
									docking_port
										mobile
											syndie_shuttle = shuttle_master.getShuttle("syndicate")
							if(syndie_shuttle)
								...
								obj
									item
										weapon
											disk
												nuclear
													NEWDISK = new(pick(blobstart))
							transfer_fingerprints_to(NEWDISK)
							var
								turf
									diskturf = get_turf(src)
