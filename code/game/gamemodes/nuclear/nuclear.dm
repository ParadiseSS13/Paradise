...
/datum/game_mode/proc/greet_syndicate(datum/mind/syndicate, you_are=1)
	...
	if(you_are)
		to_chat(syndicate.current, span_notice("You are a [syndicate_name()] agent!"))
	var/obj_count = 1
	for(var/datum/objective/objective in syndicate.objectives)
		to_chat(syndicate.current, "<B>Objective #[obj_count]</B>: [objective.explanation_text]")
		obj_count++
	to_chat(syndicate.current, span_motd("For more information, check the wiki page: ([GLOB.configuration.url.wiki_url]/index.php/Nuclear_Agent)"))
	return
