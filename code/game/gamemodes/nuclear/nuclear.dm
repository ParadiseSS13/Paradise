...
datum
	game_mode
		nuclear
			...
			var
				const
					agents_possible = 5 //If we ever need more syndicate agents.

			var
				nukes_left = 1 // Call 3714-PRAY right now and order more nukes! Limited offer!
			var
				nuke_off_station = 0 //Used for tracking if the syndies actually haul the nuke to the station
			var
				syndies_didnt_escape = 0 //Used for tracking if the syndies got the shuttle off of the z-level
	...
	game_mode
		nuclear
			can_start()//This could be better, will likely have to recode it later
				...
				var
					list
						possible_syndicates = get_players_for_role(ROLE_OPERATIVE)
				var
					agent_number = 0
				...
				while(agent_number > 0)
					var
						datum
							mind
								new_syndicate = pick(possible_syndicates)
...
datum
	game_mode
		proc
			update_synd_icons_added(datum/mind/synd_mind)
				var
					datum
						atom_hud
							antag
								opshud = huds[ANTAG_HUD_OPS]
...
datum
	game_mode
		proc
			update_synd_icons_removed(datum/mind/synd_mind)
				var
					datum
						atom_hud
							antag
			...
			post_setup()
				...
				var
					obj
						effect
							landmark
								uplinklocker = locate("landmark*Syndicate-Uplink")	//i will be rewriting this shortly
				var
					obj
						effect
							landmark
								nuke_spawn = locate("landmark*Nuclear-Bomb")
				...
				for(var/datum/mind/synd_mind in syndicates)
					...
					if(!leader_selected)
						...
						var
							list
								foundIDs = synd_mind.current.search_contents_for(/obj/item/weapon/card/id)
						if(foundIDs.len)
...
datum
	game_mode
		proc
			create_syndicate(var/datum/mind/synd_mind) // So we don't have inferior species as ops - randomize a human
				var
					mob
						living
							carbon
...
datum
	game_mode
		proc
			prepare_syndicate_leader(var/datum/mind/synd_mind, var/nuke_code)
				var
					leader_title = pick("Czar", "Boss", "Commander", "Chief", "Kingpin", "Director", "Overlord")
				/*
				spawn(1)
				...
				var
					obj
						item
							device
								nuclear_challenge
									challenge = new /obj/item/device/nuclear_challenge
...
datum
	game_mode
		proc
			forge_syndicate_objectives(var/datum/mind/syndicate)
				var
					datum
						objective
							nuclear
...
datum
	game_mode
		proc
			greet_syndicate(var/datum/mind/syndicate, var/you_are=1)
				if(you_are)
...
datum
	game_mode
		proc
			equip_syndicate(mob/living/carbon/human/synd_mob)
				var
				...
				var
					obj
						item
							device
								radio
									R = new /obj/item/device/radio/headset/syndicate/alt(synd_mob)
					...
					obj
						item
							device
								radio
									uplink
										U = new /obj/item/device/radio/uplink(synd_mob)
					...
					obj
						item
							weapon
								implant
									explosive
										E = new/obj/item/weapon/implant/explosive(synd_mob)
				E.implant(synd_mob)
...
proc
	nukelastname(var/mob/M as mob) //--All praise goes to NEO|Phyte, all blame goes to DH, and it was Cindi-Kate's idea. Also praise Urist for copypasta ho.
		var
			randomname = pick(last_names)
		var
			newname = sanitize(copytext(input(M,"You are the nuke operative [pick("Czar", "Boss", "Commander", "Chief", "Kingpin", "Director", "Overlord")]. Please choose a last name for your family.", "Name change",randomname),1,MAX_NAME_LEN))
		...
		nuclear
			set_scoreboard_gvars()
				...
				for(var/obj/machinery/nuclearbomb/nuke in world)
					...
					var
						list
							thousand_penalty = list(/area/syndicate_station, /area/wizard_station, /area/solar, /area)
					var
						list
							fiftythousand_penalty = list(/area/security/main, /area/security/brig, /area/security/armoury, /area/security/checkpoint2)
					...
					var
						killpoints = score_opkilled * 250
					var
						arrestpoints = score_arrested * 1000
