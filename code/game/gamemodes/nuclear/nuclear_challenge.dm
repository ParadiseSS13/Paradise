		...
		device
			nuclear_challenge
				attack_self(mob/living/user)
					...
					declaring_war = 1
					var
						are_you_sure = alert(user, "Consult your team carefully before you declare war on [station_name()]]. Are you sure you want to alert the enemy crew?", "Declare war?", "Yes", "No")
					if(are_you_sure == "No")
					...
					var
						war_declaration = "[user.real_name] has declared his intent to utterly destroy [station_name()] with a nuclear device, and dares the crew to try and stop them."
						...
						obj
							item
								device
									radio
										uplink
											U = new /obj/item/device/radio/uplink(get_turf(user))
