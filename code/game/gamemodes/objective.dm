...
var
	list
		potential_theft_objectives=subtypesof(/datum/theft_objective) \
...
datum
	objective
		var
			datum
				mind
					owner = null			//Who owns the objective.
		var
			explanation_text = "Nothing"	//What that person is supposed to do.
		var
			datum
				mind
					target = null		//If they are focused on a particular person.
		var
			target_amount = 0				//If they are focused on a particular number. Steal objectives have their own counter.
		var
			completed = 0					//currently only used for custom objectives.
		var
			martyr_compatible = 0			//If the objective is compatible with martyr objective, i.e. if you can still do it while dead.
...
//Selects someone with a specific special role if role is != null. Or just anyone with a special role
	proc
		find_target_with_special_role(role)
			var
				list
					possible_targets = list()
			for(var/datum/mind/possible_target in ticker.minds)
...
datum
	objective
		protect
			mindslave //subytpe for mindslave implants
	...
	objective
		hijackclone
			...
			check_completion()
				...
				var
					area
						A = shuttle_master.emergency.areaInstance
	...
	objective
		block
			...
			check_completion()
				...
				var
					list
						protected_mobs = list(/mob/living/silicon/ai, /mob/living/silicon/pai, /mob/living/silicon/robot)
...
datum
	objective
		escape
			...
			var
				escape_areas = list(/area/shuttle/escape,
					area
						shuttle
							escape_pod1
								centcom,
			...
			check_completion()
				...
				var
					turf
						location = get_turf(owner.current)
				if(!location)
					return 0
	...
	objective
		escape
			escape_with_identity
				var
					target_real_name // Has to be stored because the target's real_name can change over the course of the round

				find_target()
					var
						list
							possible_targets = list() //Copypasta because NO_SCAN races, yay for snowflakes.
					for(var/datum/mind/possible_target in ticker.minds)
						if(possible_target != owner && ishuman(possible_target.current) && (possible_target.current.stat != DEAD))
							var
								mob
		...
		steal
			...
			find_target(var/special_only=0)
				...
				while(!steal_target && loop > 0)
					loop--
					var
						thefttype = pick(potential_theft_objectives)
					var
						datum
							theft_objective
								O = new thefttype
			...
			proc
				select_target()
					var
						list
							possible_items_all = potential_theft_objectives+"custom"
					var
						new_target = input("Select target:", "Objective target", null) as null|anything in possible_items_all
	...
	objective
		steal
			exchange
				...
				proc
					set_faction(var/faction,var/otheragent)
						target = otheragent
						var
							datum
								theft_objective
...
datum
	objective
		absorb
			proc
				gen_amount_goal(var/lowbound = 4, var/highbound = 6)
	...
	objective
		destroy
			...
			find_target()
				var
					list
						possible_targets = active_ais(1)
				var
					mob
						living
							silicon
								ai
									target_ai = pick(possible_targets)
...
datum
	objective
		blood
			proc
				gen_amount_goal(low = 150, high = 400)
		...
		heist
			kidnap
				choose_target()
					var
						list
							roles = list("Chief Engineer","Research Director","Roboticist","Chemist","Station Engineer")
					var
						list
							possible_targets = list()
					var
					...
					if(target && target.current)
						...
						var
							area
								shuttle
									vox
										A = locate() //stupid fucking hardcoding
						var
							area
								vox_station
									B = locate() //but necessary
...
datum
	objective
		trade // Yes, I know there's no check_completion. The objectives exist only to tell the traders what to get.
datum
	objective
		trade
			proc
