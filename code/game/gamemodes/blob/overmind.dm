		...
		blob
			...
			var
				obj
					effect
						blob
							core
								blob_core = null // The blob overmind's core
			var
				blob_points = 0
			var
				max_blob_points = 100
			...
			var
				datum
					reagent
						blob
							blob_reagent_datum = new/datum/reagent/blob()
			var
				list
					blob_mobs = list()
			var
...
mob
	camera
		blob
			New()
				var
					new_name = "[initial(name)] ([rand(1, 999)])"
				name = new_name
				real_name = new_name
			...
			proc
				blob_talk(message)
					...
					var
						verb = "states,"
					var
						rendered = "<font color=\"#EE4000\"><i><span class='game say'>Blob Telepathy, <span class='name'>[name]([blob_reagent_datum.name])</span> <span class='message'>[verb] \"[message]\"</span></span></i></font>"
...
mob
	camera
		blob
			Move(var/NewLoc, var/Dir = 0)
				var
					obj
						effect
							blob
								B = locate() in range("3x3", NewLoc)
				if(B)
					loc = NewLoc
				else
					return 0
