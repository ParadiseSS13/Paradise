...
/datum/game_mode/blob/proc/stage(var/stage)
	switch(stage)
		if(0)
			...
			declared = 1
		if(1)
			GLOB.event_announcement.Announce("Подтверждена вспышка биологической угрозы пятого уровня на борту [station_name()]. Весь персонал обязан локализовать вспышку.", "ВНИМАНИЕ: БИОЛОГИЧЕСКАЯ УГРОЗА.", 'sound/AI/outbreak5.ogg')
		if(2)
			send_intercept(2)
