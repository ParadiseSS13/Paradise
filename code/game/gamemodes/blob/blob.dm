	...
	game_mode
		blob
			pre_setup()
				...
				var
					list
						possible_blobs = get_players_for_role(ROLE_BLOB)
...
datum
	game_mode
		blob
			proc
				blobize(var/mob/living/carbon/human/blob)
					var
						datum
							mind
								blobmind = blob.mind
...
datum
	game_mode
		blob
			proc
				make_blobs(var/count)
					var
						list
							candidates = get_blob_candidates()
					var
						mob
							living
								carbon
...
datum
	game_mode
		blob
			proc
				greet_blob(var/datum/mind/blob)
...
datum
	game_mode
		blob
			proc
				burst_blob(var/datum/mind/blob, var/warned=0)
					var
						client
							blob_client = null
					var
								...
								var
									obj
										effect
											blob
												core
													core = new(location, 200, blob_client, blob_point_rate)
		...
		blob
			post_setup()
				...
				spawn(0)
					...
					var
						wait_time = rand(waittime_l, waittime_h)
