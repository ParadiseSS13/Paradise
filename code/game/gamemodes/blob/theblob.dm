	...
	effect
		blob
			fire_act(datum/gas_mixture/air, exposed_temperature, exposed_volume)
				..()
				var
					damage = Clamp(0.01 * exposed_temperature, 0, 4)
...
obj
	effect
		blob
			proc
				Pulse(var/pulse = 0, var/origin_dir = 0, var/a_color)//Todo: Fix spaceblob expand
...
obj
	effect
		blob
			proc
				expand(var/turf/T = null, var/prob = 1, var/a_color)
					...
					var
						obj
							effect
								blob
									normal
										B = new /obj/effect/blob/normal(src.loc, min(src.health, 30))
					B.color = a_color
					B.density = 1
