			...
			verb
				jump_to_node()
					...
					if(blob_nodes.len)
						...
						var
							node_name = input(src, "Choose a node to jump to.", "Node Jump") in nodes
						var
							obj
								effect
									blob
							...
							living
								simple_animal
									hostile
										blob
											blobbernaut
												blobber = new /mob/living/simple_animal/hostile/blob/blobbernaut (get_turf(B))
					if(blobber)
						qdel(B)
					...
					var
						obj
							effect
								blob
									node
										B = locate(/obj/effect/blob/node) in T
					if(!B)
		...
		blob
			proc
				rally_spores(var/turf/T)
					...
					var
						list
							surrounding_turfs = block(locate(T.x - 1, T.y - 1, T.z), locate(T.x + 1, T.y + 1, T.z))
					...
					var
						obj
							effect
								blob
									node
										N = locate(/obj/effect/blob) in blob_nodes
					if(!N)
						to_chat(src, "<span class='warning'>A node is required to birth your offspring...</span>")
						return
		...
		blob
			verb
				blob_broadcast()
					...
					var
						speak_text = input(usr, "What would you like to say with your minions?", "Blob Broadcast", null) as text
				...
				chemical_reroll()
					...
					var
						datum
							reagent
								blob
									B = pick((subtypesof(/datum/reagent/blob) - blob_reagent_datum.type))
