			...
			if(href_list["hsb"])
				switch(href_list["hsb"])
						...
						var
							obj
								item
									stack
										sheet
											hsb = new/obj/item/stack/sheet/metal
						hsb.amount = 50
						hsb.loc = usr.loc
					if("hsbglass")
						var
							obj
								item
									stack
										sheet
											hsb = new/obj/item/stack/sheet/glass
						hsb.amount = 50
						hsb.loc = usr.loc
					if("hsbairlock")
						var
							obj
								machinery
									door
										hsb = new/obj/machinery/door/airlock
					...
					if("hsbcanister")
						var
							list
								hsbcanisters = subtypesof(/obj/machinery/portable_atmospherics/canister/)
						var
							hsbcanister = input(usr, "Choose a canister to spawn.", "Sandbox:") in hsbcanisters + "Cancel"
						...
						var
							obj
								item
									weapon
										storage
											hsb = new/obj/item/weapon/storage/toolbox/mechanical
							...
							obj
								item
									weapon
										storage
											firstaid
												hsb = new/obj/item/weapon/storage/firstaid/regular
						hsb.loc = usr.loc
					if("hsbobj")
						...
						var
							hsbitem = input(usr, "Choose an object to spawn.", "Sandbox:") in selectable + "Cancel"
						if(hsbitem != "Cancel")
							new hsbitem(usr.loc)
