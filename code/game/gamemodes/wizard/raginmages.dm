		...
		wizard
			raginmages
				...
				var
					time_checked = 0
				var
					players_per_mage = 6 // If the admin wants to tweak things or something
				but_wait_theres_more = 1
				var
					delay_per_mage = 4200 // Every 7 minutes by default
				var
					time_till_chaos = 18000 // Half-hour in
		...
		wizard
			raginmages
				check_finished()
					...
					// Accidental pun!
					var
						wizard_cap = (max_mages || (num_players_started() / players_per_mage))
...
datum
	game_mode
		wizard
			raginmages
				proc
					makeBody(var/mob/dead/observer/G)
