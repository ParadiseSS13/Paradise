...
datum
	game_mode
		wizard
			can_start()//This could be better, will likely have to recode it later
				if(!..())
					return 0
				var
					list
						datum
							mind
								possible_wizards = get_players_for_role(ROLE_WIZARD)
...
datum
	game_mode
		proc
			update_wiz_icons_added(datum/mind/wiz_mind)
				var
					datum
						atom_hud
							antag
								wizhud = huds[ANTAG_HUD_WIZ]
...
datum
	game_mode
		proc
			update_wiz_icons_removed(datum/mind/wiz_mind)
				var
					datum
						atom_hud
							antag
...
datum
	game_mode
		proc
			forge_wizard_objectives(var/datum/mind/wizard)
				switch(rand(1,100))
					if(1 to 30)
						var
							datum
...
datum
	game_mode
		proc
			name_wizard(mob/living/carbon/human/wizard_mob)
				//Allows the wizard to choose a custom name or go with a random one. Spawn 0 so it does not lag the round starting.
				var
					wizard_name_first = pick(wizard_first)
				var
					wizard_name_second = pick(wizard_second)
				var
					randomname = "[wizard_name_first] [wizard_name_second]"
				spawn(0)
					var
						newname = sanitize(copytext(input(wizard_mob, "You are the Space Wizard. Would you like to change your name to something else?", "Name change", randomname) as null|text,1,MAX_NAME_LEN))
...
datum
	game_mode
		proc
			greet_wizard(var/datum/mind/wizard, var/you_are=1)
				if(you_are)
...
datum
	game_mode
		proc
			equip_wizard(mob/living/carbon/human/wizard_mob)
				if(!istype(wizard_mob))
					return
...
datum
	game_mode
		wizard
			declare_completion(var/ragin = 0)
				if(finished && !ragin)
...
datum
	mind
		proc
			remove_spell(var/obj/effect/proc_holder/spell/spell) //To remove a specific spell from a mind
				if(!spell) return
