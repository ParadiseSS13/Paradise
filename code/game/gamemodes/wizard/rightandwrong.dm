...
mob
	proc
		rightandwrong(var/summon_type) //0 = Summon Guns, 1 = Summon Magic
			var
				list
					gunslist = list("taser","egun","laser","revolver","detective","c20r","nuclear","deagle","gyrojet","pulse","suppressed","cannon","doublebarrel","shotgun","combatshotgun","bulldog","mateba","sabr","crossbow","saw","car","boltaction","arg","uzi","turret","pulsecarbine","decloner","mindflayer","hyperkinetic","advplasmacutter","wormhole","wt550","grenadelauncher","medibeam")
			var
				list
					magiclist = list("fireball","smoke","blind","mindswap","forcewall","knock","horsemask","charge", "summonitem", "wandnothing", "wanddeath", "wandresurrection", "wandpolymorph", "wandteleport", "wanddoor", "wandfireball", "staffhealing", "armor", "scrying", "staffdoor", "special","voodoo","special")
			var
				list
			...
			for(var/mob/living/carbon/human/H in player_list)
				...
				if(H.mind)
					if(H.mind.special_role == "Wizard" || H.mind.special_role == "apprentice")
						continue
				var
					randomizeguns = pick(gunslist)
				var
					randomizemagic = pick(magiclist)
				var
					randomizemagicspecial = pick(magicspeciallist)
				if(!summon_type)
