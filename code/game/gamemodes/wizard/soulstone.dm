			...
			transfer_soul(var/choice as text, var/target, var/mob/U as mob).
				switch(choice)
									...
									if(C.contents.len)
										...
										var
											atom
												movable
													overlay
														animation = new /atom/movable/overlay( T.loc )
										...
										var
											mob
												living
													simple_animal
														shade
															S = new /mob/living/simple_animal/shade( T.loc )
					...
					if("CONSTRUCT")
						...
						if(A)
							var
								construct_class = alert(U, "Please choose which type of construct you wish to create.",,"Juggernaut","Wraith","Artificer")
		...
		var
			mob
				living
					simple_animal
						construct
							newstruct = new ctype(get_turf(target))
