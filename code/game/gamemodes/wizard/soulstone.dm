...
/obj/item/soulstone/proc/can_use(mob/living/user)
	...
	if(iscultist(user) || iswizard(user) || HAS_TRAIT(user, TRAIT_HOLY) && purified || usability == TRUE)
		return TRUE
...
/obj/item/soulstone/attackby(obj/item/O, mob/user)
	if(istype(O, /obj/item/storage/bible) && !iscultist(user) && HAS_TRAIT(user, TRAIT_HOLY))
		if(purified)
			return
