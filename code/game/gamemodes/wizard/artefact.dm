				...
				Topic(href, href_list)
					...
					if(loc == H || (in_range(src, H) && istype(loc, /turf)))
						...
						if(href_list["school"])
							...
							var
								list
									candidates = pollCandidates("Do you want to play as the wizard apprentice of [H.real_name]?", ROLE_WIZARD, 1)
							if(candidates.len)
								var
									mob
										C = pick(candidates)
								...
								var
									mob
										living
											carbon
												human
													M = new/mob/living/carbon/human(H.loc)
								M.key = C.key
								...
								var
									newname = sanitize(copytext(input(M, "You are the wizard's apprentice. Would you like to change your name to something else?", "Name change", randomname) as null|text,1,MAX_NAME_LEN))
								...
								var
									datum
										objective
											protect
												new_objective = new /datum/objective/protect
...
obj
	effect
		rend
			...
			anchored = 1.0
			var
				spawn_path = /mob/living/simple_animal/cow //defaulty cows to prevent unintentional narsies
			var
				spawn_amt_left = 20
...
obj
	effect
		rend
			New(loc, var/spawn_type, var/spawn_amt, var/desc)
				..()
		...
		weapon
			multisword
				...
				var
					cooldown = 0
				var
					cooldown_between_uses = 400 //time in deciseconds between uses--default of 40 seconds.
				var
					assigned = "unassigned"
				var
					evil = TRUE
				var
					probability_evil = 30 //what's the probability this sword will be evil when activated?
				var
					duplicate_self = 0 //Do we want the species randomized along with equipment should the user be duplicated in their entirety?
				var
					sword_type = /obj/item/weapon/multisword //type of sword to equip.
...
obj
	item
		weapon
			multisword
				attack(mob/living/M as mob, mob/living/user as mob)  //to prevent accidental friendly fire or out and out grief.
				...
				attack_self(mob/user)
					...
					if(cooldown < world.time)
						...
						if(faction_check == 0)
							...
							var
								list
									candidates = pollCandidates("Do you want to play as the wizard apprentice of [user.real_name]?", ROLE_WIZARD, 1, 100)
							if(candidates.len)
								var
									mob
										C = pick(candidates)
...
obj
	item
		weapon
			multisword
				proc
					spawn_copy(var/client/C, var/turf/T, mob/user)
						var
							mob
								living
									carbon
										human
											M = new/mob/living/carbon/human(T)
						if(duplicate_self)
						...
						if(duplicate_self)
							...
							if(prob(50))
								var
									list
										all_species = list("Human","Unathi","Skrell","Tajaran","Kidan","Golem","Diona","Machine","Slime People","Grey","Vulpkanin")
...
obj
	item
		weapon
			multisword
				proc
					equip_copy(var/mob/living/carbon/human/M)
						...
						if(duplicate_self)
							...
							var
								randomize = pick("mobster","roman","wizard","cyborg","syndicate","assistant", "animu", "cultist", "highlander", "clown", "killer", "pirate", "soviet", "officer", "gladiator")
							...
							switch(randomize)
								...
								if("roman")
									var
										hat = pick(/obj/item/clothing/head/helmet/roman, /obj/item/clothing/head/helmet/roman/legionaire)
							...
							obj
								item
									weapon
										card
											id
												W = new /obj/item/weapon/card/id
						if(duplicate_self)
							var
								duplicated_access = usr.get_item_by_slot(slot_wear_id)
									...
									obj
										item
											weapon
												card
													id
														duplicated_id = duplicated_access
...
obj
	item
		weapon
			multisword
				pike //If We are to be used and spent, let it be for a noble purpose.
					name = "phantom pike"
...
obj
	item
		device
			necromantic_stone
				proc
					equip_skeleton(mob/living/carbon/human/H as mob)
						for(var/obj/item/I in H)
							H.unEquip(I)
						var
							randomSpooky = "roman"//defualt
...
obj
	item
		device
			necromantic_stone
				proc
					spawnheresy(mob/living/carbon/human/H as mob)
						H.set_species("Human")
						if(H.gender == MALE)
						...
						var
							list
								anime_hair =list("Odango", "Kusanagi Hair", "Pigtails", "Hime Cut", "Floorlength Braid", "Ombre", "Twincurls", "Twincurls 2")
						H.change_hair(pick(anime_hair))

						var
							list
								anime_hair_colours = list(list(216, 192, 120),
						list(140,170,74),list(0,0,0))

						var
							list
								chosen_colour = pick(anime_hair_colours)
			...
			attack_self(mob/user as mob)
				...
				if(target && cooldown < world.time)
					switch(user.zone_sel.selecting)
						if("mouth")
							var
								wgw =  sanitize(input(user, "What would you like the victim to say", "Voodoo", null)  as text)
							target.say(wgw)
						...
						if("r_leg","l_leg")
							...
							var
								turf
									T = get_step(target,pick(cardinal))
							target.Move(T)
						if("r_arm","l_arm")
...
obj
	item
		voodoo
			proc
				GiveHint(mob/victim,force=0)
					if(prob(50) || force)
						var
							way = dir2text(get_dir(victim,get_turf(src)))
