...
/obj/item/weapon/multisword/proc/equip_copy(var/mob/living/carbon/human/M)
	...
	if(duplicate_self)
		...
		M.put_in_hands_or_del(sword) //Don't duplicate what's equipped to hands, or else duplicate swords could be generated...or weird cases of factionless swords.
	else
...
/obj/item/device/necromantic_stone/proc/equip_skeleton(mob/living/carbon/human/H as mob)
	...
	switch(randomSpooky)
		...
		if("yand")//mine is an evil laugh
			...
			H.put_in_hands_or_del(new /obj/item/weapon/katana(H))
			H.put_in_hands_or_del(new /obj/item/weapon/shield/riot/roman(H))
			H.equip_to_slot_or_del(new /obj/item/weapon/twohanded/spear(H), slot_back)
		if("clown")
