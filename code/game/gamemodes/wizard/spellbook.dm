datum
	spellbook_entry
		...
		var
			category = "Offensive"
		var
			log_name = "XX" //What it shows up as in logs
		var
			cost = 2
		var
			refundable = 1
		var
			surplus = -1 // -1 for infinite, not used by anything atm
		var
			obj
				effect
					proc_holder
						spell
							S = null //Since spellbooks can be used by only one person anyway we can track the actual spell
		var
			buy_word = "Learn"
		var
			limit //used to prevent a spellbook_entry from being bought more than X times with one wizard spellbook

datum
	spellbook_entry
		proc
			IsAvailible() // For config prefs / gamemode restrictions - these are round applied
				return 1
datum
	spellbook_entry
		proc
			CanBuy(var/mob/living/carbon/human/user,var/obj/item/weapon/spellbook/book) // Specific circumstances
...
datum
	spellbook_entry
		proc
			Buy(var/mob/living/carbon/human/user,var/obj/item/weapon/spellbook/book) //return 1 on success
				if(!S)
					S = new spell_type()
...
datum
	spellbook_entry
		proc
			CanRefund(var/mob/living/carbon/human/user,var/obj/item/weapon/spellbook/book)
				if(!refundable)
					return 0
				if(!S)
					S = new spell_type()
...
datum
	spellbook_entry
		proc
			Refund(var/mob/living/carbon/human/user,var/obj/item/weapon/spellbook/book) //return point value or -1 for failure
				var
					area
						wizard_station
							A = locate()
...
datum
	spellbook_entry
		item
			Buy(var/mob/living/carbon/human/user,var/obj/item/weapon/spellbook/book)
...
datum
	spellbook_entry
		summon
			CanBuy(var/mob/living/carbon/human/user,var/obj/item/weapon/spellbook/book)
				return ..() && !active
		...
		weapon
			spellbook
				proc
					Initialize()
						var
							entry_types = subtypesof(/datum/spellbook_entry) - /datum/spellbook_entry/item - /datum/spellbook_entry/summon
...
obj
	item
		weapon
			spellbook
				proc
					GetCategoryHeader(var/category)
						var
							dat = ""
						switch(category)
							if("Offensive")
	...
	item
		weapon
			spellbook
				oneuse
					var
						spell = /obj/effect/proc_holder/spell/targeted/projectile/magic_missile //just a placeholder to avoid runtimes if someone spawned the generic
					var
						spellname = "sandbox"
					var
						used = 0
			...
			spellbook
				oneuse
					mindswap
						...
						var
							mob
								stored_swap = null //Used in used book recoils to store an identity for mindswaps
							...
							var
								obj
									structure
										closet
											statue
												S = new /obj/structure/closet/statue(user.loc, user)
							S.timer = 30
							user.drop_item()
	...
	item
		weapon
			spellbook
				oneuse
					horsemask
						recoil(mob/living/carbon/user as mob)
									...
									obj
										item
											clothing
												mask
													horsehead
														magichead = new /obj/item/clothing/mask/horsehead
