...
datum
	faction
		...
		var
			desc		// small paragraph explaining the traitor faction

		var
			list
				restricted_species = list() // only members of these species can be recruited.
		var
			list
				members = list() 	// a list of mind datums that belong to this faction
		var
			max_op = 0		// the maximum number of members a faction can have (0 for no max)
...
datum
	faction
		syndicate
			...
			var
				list
					alliances = list() // these alliances work together
			var
				list
					equipment = list() // associative list of equipment available for this faction and its prices
			var
				friendly_identification	// 0 to 2, the level of identification of fellow operatives or allied factions
											// 0 - no identification clues
											// 1 - faction gives key words and phrases
											// 2 - faction reveals complete identity/job of other agents
			var
				operative_notes // some notes to pass onto each operative

			var
				uplink_contents			// the contents of the uplink

			proc
				assign_objectives(var/datum/mind/traitor)
					..()
