...
//			world << D.name
	...
	proc/add_faction(datum/mind/M, faction, list/faction_list)
		if(!can_be_recruited(M, faction))
			M.current << "<span class='warning'>Recruitment canceled; your role has already changed.</span>"
			head_mutineer.current << "<span class='warning'>Could not recruit [M]. Their role has changed.</span>"
			return
...
/datum/game_mode/mutiny/pre_setup()
	...
	if(!loyalist_candidates || loyalist_candidates.len == 0)
		world << "<span class='warning'>Mutiny mode aborted: no valid candidates for head loyalist.</span>"
		return 0

	var/list/mutineer_candidates = get_head_mutineer_candidates()
	if(!mutineer_candidates || mutineer_candidates.len == 0)
		world << "<span class='warning'>Mutiny mode aborted: no valid candidates for head mutineer.</span>"
		return 0
...
/mob/living/carbon/human/proc/recruit_loyalist()
	...
	if(!candidates.len)
		src << "<span class='warning'>You aren't close enough to anybody that can be recruited.</span>"
		return

	if(world.time < mode.recruit_loyalist_cooldown)
		src << "<span class='warning'>Wait [MUTINY_RECRUITMENT_COOLDOWN] seconds before recruiting again.</span>"
		return
	...
	if (M)
		...
		message_admins("<span class='warning'>[key_name_admin(src)] attempted to recruit [M] as a loyalist.</span>")
		...
		if(choice == "Yes")
			...
			M << "<span class='warning'>You declined to join the loyalists.</span>"
			mode.head_loyalist.current << "<span class='warning'><b>[M] declined to support the loyalists.</b></span>"
...
/mob/living/carbon/human/proc/recruit_mutineer()
	...
	if (M)
		...
		message_admins("<span class='warning'>[key_name_admin(src)] attempted to recruit [M] as a mutineer.</span>")

		var/choice = alert(M, "Asked by [src]: Will you help me stop Directive X?", "Mutineer recruitment", "Yes", "No")
		if(choice == "Yes")
			mode.add_mutineer(M.mind)
		else if(choice == "No")
			M << "<span class='warning'>You declined to join the mutineers.</span>"
			mode.head_mutineer.current << "<span class='warning'><b>[M] declined to support the mutineers.</b></span>"
