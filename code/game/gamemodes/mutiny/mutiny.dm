	...
	game_mode
		mutiny
			...
			var
				obj
					machinery
						emergency_authentication_device
							ead
			var
				datum
					mind
			...
			var
				datum
					mind
						head_mutineer
			var
				recruit_loyalist_cooldown = 0
			var
				recruit_mutineer_cooldown = 0
			var
				list
					loyalists = list()
			var
...
return candidates
				...
				datum
					data
						pda
							app
								messenger
									pdam = pda.find_program(/datum/data/pda/app/messenger)
			if(pdam)
	...
	proc
		equip_head(datum/mind/head, faction, proc/recruitment_verb)
			var
				mob
					living
						carbon
	...
	proc
		add_loyalist(datum/mind/M)
			add_faction(M, "loyalist", loyalists)

	proc
		add_mutineer(datum/mind/M)
			add_faction(M, "mutineer", mutineers)

	proc
		add_faction(datum/mind/M, faction, list/faction_list)
	...
	proc
		was_bloodbath()
			var
				list
					remaining_loyalists = loyalists - body_count
			if(!remaining_loyalists.len)
				return 1

			var
				list
					remaining_mutineers = mutineers - body_count
			...
			var
				obj
					machinery
						door
							airlock
								vault = locate(/obj/machinery/door/airlock/vault)
			vault.unlock(1)
				...
				obj
					machinery
						computer
							telecomms
								server
									S = locate(/obj/machinery/computer/telecomms/server)
			if(!S) return
	...
	proc
		round_outcome()
			...
			var
				directives_completed = current_directive.directives_complete()
			var
				ead_activated = ead.activated
	...
	proc
		update_icon(datum/mind/M)
			...
			if(M in loyalists)
				var
					I = image('icons/mob/mob.dmi', loc=M.current, icon_state = "loyalist")
				head_loyalist.current.client.images += I

			if(M in mutineers)
				var
					I = image('icons/mob/mob.dmi', loc=M.current, icon_state = "mutineer")
	...
	game_mode
		mutiny
			pre_setup()
				var
					list
						loyalist_candidates = get_head_loyalist_candidates()
				...
				var
					list
						mutineer_candidates = get_head_mutineer_candidates()
				...
				var
					list
						directive_candidates = get_directive_candidates()
				...
				proc
					recruit_loyalist()
						...
						var
							mob
								living
									carbon
										human
											M = input("Select a person to recruit", "Loyalist recruitment", null) as mob in candidates
						...
						if(M)
							...
							var
								choice = alert(M, "Asked by [src]: Will you help me complete Directive X?", "Loyalist recruitment", "Yes", "No")
							if(choice == "Yes")
				...
				proc
					recruit_mutineer()
						...
						var
							mob
								living
									carbon
										human
											M = input("Select a person to recruit", "Mutineer recruitment", null) as mob in candidates
						...
						if(M)
							...
							var
								choice = alert(M, "Asked by [src]: Will you help me stop Directive X?", "Mutineer recruitment", "Yes", "No")
							if(choice == "Yes")
