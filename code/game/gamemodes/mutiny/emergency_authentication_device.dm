				...
				launch_shuttle()
					spawn(rand(5 SECONDS, 45 SECONDS))
						if(shuttle_master.requestEvac(usr, "Directive X"))
							spawn(20 SECONDS)
								var
									text = "[station_name()], we have confirmed your completion of Directive X. An evacuation shuttle is en route to receive your crew for debriefing."
...
obj
	machinery
		emergency_authentication_device
			attackby(obj/item/weapon/O, mob/user, params)
				if(activated)
					to_chat(user, "\blue \The [src] is already active!")
					return
