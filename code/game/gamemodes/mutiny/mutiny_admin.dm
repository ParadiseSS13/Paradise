datum
	game_mode
		mutiny
			proc
				check_antagonists_ui(datum/admins/admins)
						var
							turf
								captains_key_loc = captains_key ? captains_key.get_loc_turf() : "Lost or Destroyed"
						var
							turf
								secondary_key_loc = secondary_key ? secondary_key.get_loc_turf() : "Lost or Destroyed"
						var
							remaining_objectives = current_directive.get_remaining_orders()
						var
							txt = {"
									<h5>Context:</h5>
									<p>
						...
						var
							list
								loyal_crew = loyalists - head_loyalist
						if(loyal_crew.len)
						...
						var
							list
								mutiny_crew = mutineers - head_mutineer
						if(mutiny_crew.len)
...
datum
	game_mode
		mutiny
			check_antagonists_topic(href, href_list[])
					switch(href_list["choice"])
							...
							if("reassign_head_loyalist")
									var
										mob
											M = get_reassignment_candidate("Loyalist")
									if(M)
							...
							if("reassign_head_mutineer")
									var
										mob
											M = get_reassignment_candidate("Mutineer")
									if(M)
...
datum
	game_mode
		mutiny
			proc
				get_reassignment_candidate(faction)
						var
							list
								targets[0]
