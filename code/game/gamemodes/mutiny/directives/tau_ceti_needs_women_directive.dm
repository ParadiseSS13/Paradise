...
hook
	reassign_employee
		proc
			command_reassignments(obj/item/weapon/card/id/id_card)
				var
					datum
						directive
							tau_ceti_needs_women
								D = get_directive("tau_ceti_needs_women")
				if(!D) return 1
...
hook
	terminate_employee
		proc
			gender_target_termination_directive(obj/item/weapon/card/id)
				var
					datum
						directive
							tau_ceti_needs_women
