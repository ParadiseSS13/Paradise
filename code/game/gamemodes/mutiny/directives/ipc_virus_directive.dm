datum
	directive
		ipc_virus
			...
			var
				list
					brains_to_enslave = list()
			var
				list
					cyborgs_to_make = list()
			var
				list
					ids_to_terminate = list()
...
hook
	debrain
		proc
			debrain_directive(var/obj/item/organ/internal/brain/B)
				var
					datum
						directive
							ipc_virus
								D = get_directive("ipc_virus")
				if(!D) return 1
...
hook
	borgify
		proc
			borgify_directive(mob/living/silicon/robot/cyborg)
				var
					datum
						directive
							ipc_virus
...
hook
	terminate_employee
		proc
			ipc_termination(obj/item/weapon/card/id)
				var
					datum
						directive
							ipc_virus
