		...
		vox_heist
				...
				proc
					get_weight(mob/M)
							...
							var
								relation = M.client.prefs.nanotrasen_relation
							if(relation == "Opposed")
									return 8
	...
	directive
		vox_heist
			initialize()
					var
						list
							vox_candidates = get_vox_candidates()
					...
					var
						list
							sympathizer_candidates = get_sympathizer_candidates()
					var
						list
							sympathizer_names = list()
					var
						sympathizer_count = min(rand(2,4), sympathizer_candidates.len)
					for(var/i=0, i < sympathizer_count, i++)
							...
							var
								mob
									candidate = pickweight(sympathizer_candidates)
...
hook
	death
		proc
			vox_or_sympathizer_killed(mob/living/carbon/human/deceased, gibbed)
					var
						datum
							directive
								vox_heist
									D = get_directive("vox_heist")
					if(!D) return 1
