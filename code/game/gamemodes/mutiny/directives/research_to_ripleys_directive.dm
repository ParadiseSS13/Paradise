...
datum
	directive
		research_to_ripleys
			...
			proc
				count_researchers_reassigned()
					var
						researchers_reassigned = 0
...
hook
	reassign_employee
		proc
			research_reassignments(obj/item/weapon/card/id/id_card)
				var
					datum
						directive
							research_to_ripleys
								D = get_directive("research_to_ripleys")
				if(!D) return 1
...
hook
	sell_crate
		proc
			deliver_materials(obj/structure/closet/crate/sold, area/shuttle)
				var
					datum
						directive
							research_to_ripleys
