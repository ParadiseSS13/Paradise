...
proc
	runerandom() //randomizes word meaning
		var
			list
				runewords=list("ire","ego","nahlizet","certum","veri","jatkaa","mgar","balaq", "karazet", "geeri") ///"orkan" and "allaq" removed.
		for(var/word in engwords)
			var
				runeword = pick(runewords)
...
obj
	effect
		rune
			var
				list
					effect_dictionary = list(	"teleport"=/obj/effect/rune/proc/teleportRune,
	...
	item
		weapon
			tome
				...
				var
					list
						words = list("ire" = "ire", "ego" = "ego", "nahlizet" = "nahlizet", "certum" = "certum", "veri" = "veri", "jatkaa" = "jatkaa", "balaq" = "balaq", "mgar" = "mgar", "karazet" = "karazet", "geeri" = "geeri")
				...
				Topic(href,href_list[])
					if(src.loc == usr)
						var
							number = text2num(href_list["number"])
	...
	attack(mob/living/M as mob, mob/living/user as mob)
		...
		if(iscultist(M))
			if(M.reagents && M.reagents.has_reagent("holywater")) //allows cultists to be rescued from the clutches of ordained religion
				...
				var
					holy2unholy = M.reagents.get_reagent_amount("holywater")
...
obj
	item
		weapon
			tome
				imbued //admin tome, spawns working runes without waiting
					w_class = 2.0
					var
						cultistsonly = 1
					attack_self(mob/user as mob)
						...
						if(user)
							...
							var
								list
									runes = list("teleport", "itemport", "tome", "armor", "convert", "tear in reality", "emp", "drain", "seer", "raise", "obscure", "reveal", "astral journey", "manifest", "imbue talisman", "sacrifice", "wall", "freedom", "cultsummon", "deafen", "blind", "bloodboil", "communicate", "stun")
							r = input("Choose a rune to scribe", "Rune Scribing") in runes //not cancellable.
							var
								obj
									effect
										rune
											R = new /obj/effect/rune
							...
							switch(r)
								if("teleport")
									var
										list
											words = list("ire", "ego", "nahlizet", "certum", "veri", "jatkaa", "balaq", "mgar", "karazet", "geeri")
									var
										beacon
									if(usr)
