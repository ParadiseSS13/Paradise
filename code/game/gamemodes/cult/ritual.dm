	...
	if(ispath(rune_to_scribe, /obj/effect/rune/narsie) || ispath(rune_to_scribe, /obj/effect/rune/slaughter))//may need to change this - Fethas
		if(finale_runes_ok(user,rune_to_scribe))
			...
			if(!(A in summon_spots))  // Check again to make sure they didn't move
				...
				return
			command_announcement.Announce("Figments from an eldritch god are being summoned by [user] into [A.map_name] from an unknown dimension. Disrupt the ritual at all costs!","Central Command Higher Dimensional Affairs", 'sound/AI/spanomalies.ogg')
