...
/datum/game_mode/proc/check_cult_size()
	...
	if((cult_players >= rise_number) && !cult_risen)
		...
		GLOB.command_announcement.Announce("На вашей станции зафиксирована межпространственная активность, связанная с культом [SSticker.cultdat ? SSticker.cultdat.entity_name : "Нар'Си"]. Данные свидетельствуют о том, что около [ascend_percent * 100]% экипажа станции были порабощены культом. Сотрудники службы безопасности имеют право свободно применять летальную силу против культистов. Сотрудники, не относящиеся к службе безопасности, должны быть готовы защищать себя и свои рабочие места. Самооборона позволяет сотрудникам, не относящимся к службе безопасности, использовать летальную силу в качестве крайней меры. Сотрудники, не относящиеся к службе безопасности, должны обезопасить свои рабочие места, охота на культистов недопустима. Мертвые члены экипажа должны быть оживлены и деконвертированы, как только ситуация будет взята под контроль.", "Отдел Центрального Командования по делам межпространственных аномалий.", 'sound/AI/commandreport.ogg')
