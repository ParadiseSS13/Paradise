...
/proc/is_convertable_to_cult(datum/mind/mind, datum/team/cult/specific_cult)
	if(!mind)
		return FALSE
	if(!mind.current)
		return FALSE
	if(specific_cult && specific_cult.is_sacrifice_target(mind.current))
		return FALSE
...
/datum/game_mode/proc/remove_cultist(datum/mind/cult_mind, silent)
