...
datum
	game_mode
		cult
			...
			var
				eldergod = 1 //for the summon god objective

			var
				acolytes_needed = 4 //for the survive objective - base number of acolytes, increased by 1 for every 10 players
			var
				const
					min_cultists_to_start = 3
			var
				const
					max_cultists_to_start = 4
			var
				acolytes_survived = 0
	...
	game_mode
		cult
			pre_setup()
				...
				var
					list
						cultists_possible = get_players_for_role(ROLE_CULTIST)
				...
				for(var/cultists_number = 1 to max_cultists_to_start)
					...
					var
						datum
							mind
								cultist = pick(cultists_possible)
		...
		cult
			post_setup()
				...
				if("sacrifice" in objectives)
					var
						list
							possible_targets = get_unconvertables()
...
datum
	game_mode
		cult
			proc
				memorize_cult_objectives(var/datum/mind/cult_mind)
...
datum
	game_mode
		proc
			equip_cultist(mob/living/carbon/human/mob)
				if(!istype(mob))
					return
				...
				var
					where = mob.equip_in_one_of_slots(T, slots)
				if(!where)
...
datum
	game_mode
		cult
			grant_runeword(mob/living/carbon/human/cult_mob, var/word)
				if(!word)
					if(startwords.len > 0)
		...
		proc
			grant_runeword(mob/living/carbon/human/cult_mob, var/word)
				...
				if(!word)
					word=pick(allwords)
				var
					wordexp = "[cultwords[word]] is [word]..."
...
datum
	game_mode
		proc
			add_cult_viewpoint(var/mob/target)
				for(var/obj/cult_viewpoint/viewpoint in target)
					return
				var
					obj
						cult_viewpoint
							viewpoint = new(target)
...
datum
	game_mode
		proc
			add_cultist(datum/mind/cult_mind) //BASE
				if(!istype(cult_mind))
					return 0
...
datum
	game_mode
		cult
			add_cultist(datum/mind/cult_mind) //INHERIT
				if(!..(cult_mind))
					return
				memorize_cult_objectives(cult_mind)
...
datum
	game_mode
		proc
			remove_cultist(datum/mind/cult_mind, show_message = 1)
				if(cult_mind in cult)
					...
					var
						obj
							viewpoint = getCultViewpoint(cult_mind.current)
					qdel(viewpoint)
...
datum
	game_mode
		proc
			add_cult_icon_to_spirit(mob/spirit/currentSpirit)
...
datum
	game_mode
		proc
			remove_cult_icon_from_spirit(mob/spirit/currentSpirit)
				if(!istype(currentSpirit))
					return FALSE
				if(currentSpirit.client)
					var
...
datum
	game_mode
		proc
			update_cult_icons_added(datum/mind/cult_mind)
...
datum
	game_mode
		proc
			update_cult_icons_removed(datum/mind/cult_mind)
...
datum
	game_mode
		proc
			auto_declare_completion_cult()
				if( cult.len || (ticker && istype(ticker.mode,/datum/game_mode/cult)) )
					var
						text = "<FONT size = 2><B>The cultists were:</B></FONT>"
