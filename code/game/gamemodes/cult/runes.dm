...
/obj/effect/rune/raise_dead/invoke(list/invokers)
	...
	if(!mob_to_revive.client || mob_to_revive.client.is_afk())
		...
		if(length(candidates) && !QDELETED(mob_to_revive))
			...
			to_chat(mob_to_revive.mind, span_biggerdanger("Your physical form has been taken over by another soul due to your inactivity! Ahelp if you wish to regain your form."))
...
/obj/effect/rune/summon/invoke(list/invokers)
	...
	if(!iscultist(cultist_to_summon))
		to_chat(user, span_cultitalic("[cultist_to_summon] is not a follower of the [SSticker.cultdat.entity_title3]!"))
		fail_invoke()
