...
/obj/effect/rune/manifest/proc/summon_ghosts(mob/living/user, turf/T)
	...
	for(var/mob/dead/observer/O in T)
		if(O.client && !iscultist(O) && !jobban_isbanned(O, ROLE_CULTIST) && !O.has_enabled_antagHUD && !QDELETED(src) && !QDELETED(O) && !HAS_TRAIT_FROM(O.mind.current, SCRYING, CULT_TRAIT))
			ghosts_on_rune += O
