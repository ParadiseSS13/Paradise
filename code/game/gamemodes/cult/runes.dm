		...
		rune
			proc
				emp(var/U,var/range_red) //range_red - var which determines by which number to reduce the default emp range, U is the source loc, needed because of talisman emps which are held in hand at the moment of using and that apparently messes things up -- Urist
					...
					var
						rune = src // detaching the proc - in theory
		...
		rune
			proc
				communicate()
					...
					var
						input = stripped_input(usr, "Please choose a message to tell to the other acolytes.", "Voice of Blood", "")
					if(!input)
						if(istype(src))
							fizzle()
							return 0
					...
					var
						F = investigate_subject2file("cult")
					if(!F)
						return 0
					...
					var
						obj
							cult_viewpoint
								vp = getCultViewpoint(usr)
					if(!vp)
						return 0

					var
						displayName = vp.get_display_name()
					var
						cultName = vp.get_cult_name()
					...
					if(users.len>=3)
						var
							mob
								living
									carbon
										cultist = input("Choose the one who you want to free", "Followers of Geometer") as null|anything in (cultists - users)
						if(!cultist)
							return fizzle()
					...
					if(users.len>=3)
						var
							mob
								living
									carbon
										cultist = input("Choose the one who you want to summon", "Followers of Geometer") as null|anything in (cultists - user)
						if(!cultist)
							return fizzle()
						if(cultist == user) //just to be sure.
							return
...
*/
	var
		culcount = 0 //also, wording for it is old wording for obscure rune, which is now hide-see-blood.
