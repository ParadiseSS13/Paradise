...
mob
	living
		silicon
			ai
				proc
					overload_machine(obj/machinery/M as obj in world)
...
mob
	living
		silicon
			ai
				proc
					override_machine(obj/machinery/M as obj in world)
			...
			place_cyborg_transformer
				New()
					for(var/i=0;i<3;i++)
						var
							image
								I = image("icon"='icons/turf/overlays.dmi')
						turfOverlays += I
					..()
			...
			ai
				proc
					place_transformer()
						...
						var
							sure = alert(src, "Are you sure you want to place the machine here?", "Are you sure?", "Yes", "No")
						if(sure == "Yes")
							if(!canPlaceTransformer())
								return
							var
						...
						if(turfs.len == 3)
							for(var/n=1;n<4,n++)
								...
								var
									datum
										camerachunk
											C = cameranet.getCameraChunk(T.x, T.y, T.z)
								if(!C.visibleTurfs[T])
				...
				proc
					reactivate_cameras()
						...
						if(stat || malf_cooldown)
							return
						var
							fixedcams = 0 //Tells the AI how many cams it fixed. Stats are fun.

						for(var/datum/AI_Module/small/reactivate_cameras/camera in current_modules)
							for(var/obj/machinery/camera/C in cameranet.cameras)
								var
									initial_range = initial(C.view_range) //To prevent calling the proc twice
								if(camera.uses > 0)
									if(!C.status)
...
datum
	module_picker
		proc
			remove_verbs(var/mob/living/silicon/ai/A)
		...
		proc
			use(user as mob)
				...
				var
					datum
						browser
							popup = new(user, "modpicker", "Malf Module Menu")
			...
			for(var/datum/AI_Module/AM in possible_modules)
				if(href_list[AM.mod_pick_name])
					...
					var
						datum
							AI_Module
								already_AM = locate(AM.type) in A.current_modules
					if(already_AM)
						if(!AM.one_time)
