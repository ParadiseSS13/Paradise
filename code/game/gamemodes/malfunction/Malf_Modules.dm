...
/obj/machinery/doomsday_device/Destroy()
	...
	if(SSshuttle.emergency.mode == SHUTTLE_STRANDED)
		...
		GLOB.priority_announcement.Announce("Враждебная угроза устранена. У вас есть 3 минуты, чтобы подняться на борт эвакуационного шаттла.", "Приоритетное оповещение.", 'sound/AI/shuttledock.ogg')
	return ..()
...
/obj/machinery/doomsday_device/process()
	...
	if(!T || !is_station_level(T.z))
		GLOB.minor_announcement.Announce("УСТРОЙСТВО СУДНОГО ДНЯ ВНЕ ЗОНЫ ДЕЙСТВИЯ СТАНЦИИ, ОСТАНОВКА.", "ОШИБКА ОШИБКА $0ШБК$!R41.%%!!(%$^^__+ @#F0E4", 'sound/misc/notice1.ogg')
		SSshuttle.emergencyNoEscape = 0
		if(SSshuttle.emergency.mode == SHUTTLE_STRANDED)
			SSshuttle.emergency.mode = SHUTTLE_DOCKED
			SSshuttle.emergency.timer = world.time
	...
	if(sec_left <= 0)
		...
		if(!(sec_left % 60) && !announced)
			...
			GLOB.minor_announcement.Announce(message, "ОШИБКА ОШИБКА $0ШБК$!R41.%%!!(%$^^__+ @#F0E4", 'sound/misc/notice1.ogg')
			announced = 10
