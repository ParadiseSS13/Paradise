...
datum
	game_mode
		malfunction
			...
			var
				AI_win_timeleft = 1500 //started at 1500, in case I change this for testing round end.
			var
			...
			var
				apcs = 0 //Adding dis to track how many APCs the AI hacks. --NeoFite
			var
				list
					datum
						mind
...
datum
	game_mode
		malfunction
			get_players_for_role(var/role = ROLE_MALF)
				var
					roletext = get_roletext(role)
...
datum
	game_mode
		proc
			greet_malf(var/datum/mind/malf)
...
datum
	game_mode
		proc
			greet_malf_robot(var/datum/mind/robot)
		...
		proc
			check_ai_loc()
				for(var/datum/mind/AI_mind in malf_ai)
					var
						turf
							ai_location = get_turf(AI_mind.current)
							...
							var
								mob
									living
										silicon
											ai
												A = AI_mind.current // the current mob the mind owns
							if(A.stat != DEAD)
...
datum
	game_mode
		malfunction
			declare_completion()
				var
					malf_dead = is_malf_ai_dead()
				var
					crew_evacuated = (shuttle_master.emergency.mode >= SHUTTLE_ESCAPE)
...
datum
	game_mode
		proc
			auto_declare_completion_malfunction()
				if( malf_ai.len || istype(ticker.mode,/datum/game_mode/malfunction) )
					var
						text = "<FONT size = 2><B>The malfunctioning AI were:</B></FONT>"
					var
						module_text_temp = "<br><b>Purchased modules:</b><br>" //Added at the end
