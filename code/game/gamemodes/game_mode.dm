...
datum
	game_mode
		...
		var
			list
				restricted_jobs = list()	// Jobs it doesn't make sense to be.  I.E chaplain or AI cultist
		var
			list
				protected_jobs = list()	// Jobs that can't be traitors
		var
			list
				protected_species = list() // Species that can't be traitors
		var
			required_players = 0
		var
			required_enemies = 0
...
datum
	game_mode
		proc
			announce() //to be calles when round starts
					...
					if(M.mind)
						...
						for(var/datum/job_objective/objective in M.mind.job_objectives)
							...
							if(objective.per_unit && objective.units_compensated<objective.units_completed)
								var
									newunits = objective.units_completed - objective.units_compensated
...
datum
	game_mode
		proc
			check_finished() //to be called by ticker
		...
		proc
			declare_completion()
				...
				var
					list
						area
							escape_locations = list(/area/shuttle/escape, /area/shuttle/escape_pod1/centcom, /area/shuttle/escape_pod2/centcom, /area/shuttle/escape_pod3/centcom, /area/shuttle/escape_pod5/centcom)
...
datum
	game_mode
		proc
			check_win() //universal trigger to be called at mob death, nuke explosion, etc. To be called from everywhere.
				return 0

datum
	game_mode
		proc
			get_players_for_role(var/role, override_jobbans=0)
				var
					list
						players = list()
				var
...
datum
	game_mode
		proc
			check_player_role_pref(var/role, var/mob/player)
...
proc
	display_roundstart_logout_report()
		var
			msg = "<span class='notice'>Roundstart logout report</span>\n\n"
...
proc
	show_generic_antag_text(var/datum/mind/player)
		if(player.current)
...
proc
	show_objectives(var/datum/mind/player)
...
datum
	game_mode
		proc
			replace_jobbaned_player(mob/living/M, role_type)
				var
					list
						mob
							dead
								observer
									candidates = pollCandidates("Do you want to play as a [role_type]?", role_type, 0, 100)
				var
					mob
						dead
							observer
...
datum
	game_mode
		proc
			printplayer(datum/mind/ply, fleecheck)
				var
...
datum
	game_mode
		proc
			printobjectives(datum/mind/ply)
				var
					text = ""
				var
					count = 1
