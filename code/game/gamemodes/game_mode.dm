...
/datum/game_mode/proc/post_setup()
	...
	feedback_report("round_start","[time2text(world.realtime)]")
	if(ticker && ticker.mode)
		feedback_report("game_mode","[ticker.mode]")
//	if(revdata)
//		feedback_report("revision","[revdata.revision]")
	feedback_report("server_ip","[world.internet_address]:[world.port]")
...
/datum/game_mode/proc/declare_completion()
	...
	if(clients > 0)
		feedback_report("round_end_clients",clients)
	if(ghosts > 0)
		feedback_report("round_end_ghosts",ghosts)
	if(surviving_humans > 0)
		feedback_report("survived_human",surviving_humans)
	if(surviving_total > 0)
		feedback_report("survived_total",surviving_total)
	if(escaped_humans > 0)
		feedback_report("escaped_human",escaped_humans)
	if(escaped_total > 0)
		feedback_report("escaped_total",escaped_total)
	if(escaped_on_shuttle > 0)
		feedback_report("escaped_on_shuttle",escaped_on_shuttle)
	if(escaped_on_pod_1 > 0)
		feedback_report("escaped_on_pod_1",escaped_on_pod_1)
	if(escaped_on_pod_2 > 0)
		feedback_report("escaped_on_pod_2",escaped_on_pod_2)
	if(escaped_on_pod_3 > 0)
		feedback_report("escaped_on_pod_3",escaped_on_pod_3)
	if(escaped_on_pod_5 > 0)
		feedback_report("escaped_on_pod_5",escaped_on_pod_5)
