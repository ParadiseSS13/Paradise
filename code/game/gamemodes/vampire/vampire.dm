	...
	if(vampires.len)
		...
		for(var/datum/mind/vampire in vampires)
			...
			if(vampire.objectives.len)//If the traitor had no objectives, don't need to process this.
				...
				for(var/datum/objective/objective in vampire.objectives)
					if(objective.check_completion())
						...
						feedback_report("traitor_objective","[objective.type]|SUCCESS")
					else
						text += "<br><B>Objective #[count]</B>: [objective.explanation_text] <font color='red'>Fail.</font>"
						feedback_report("traitor_objective","[objective.type]|FAIL")
						traitorwin = 0
					count++
			...
			if(traitorwin)
				...
				feedback_report("traitor_success","SUCCESS")
			else
				text += "<br><font color='red'><B>The [special_role_text] has failed!</B></font>"
				feedback_report("traitor_success","FAIL")
		to_chat(world, text)
	return 1
