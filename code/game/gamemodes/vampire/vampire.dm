	...
	game_mode
		...
		var
			list
				datum
					mind
						vampire_enthralled = list() //those controlled by a vampire
		var
			list
				vampire_thralls = list() //vammpires controlling somebody
	...
	game_mode
		proc
			auto_declare_completion_vampire()
				if(vampires.len)
					var
						text = "<FONT size = 2><B>The vampires were:</B></FONT>"
					for(var/datum/mind/vampire in vampires)
						var
							traitorwin = 1
		...
		proc
			auto_declare_completion_enthralled()
				if(vampire_enthralled.len)
					var
						text = "<FONT size = 2><B>The Enthralled were:</B></FONT>"
...
datum
	game_mode
		proc
			forge_vampire_objectives(var/datum/mind/vampire)
...
datum
	game_mode
		proc
			grant_vampire_powers(mob/living/carbon/vampire_mob)
...
datum
	game_mode
		proc
			greet_vampire(var/datum/mind/vampire, var/you_are=1)
				var
					dat
				if(you_are)
...
datum
	vampire
		var
			bloodtotal = 0 // CHANGE TO ZERO WHEN PLAYTESTING HAPPENS
		var
			bloodusable = 0 // CHANGE TO ZERO WHEN PLAYTESTING HAPPENS
		var
			mob
				living
					owner = null
		var
			gender = FEMALE
		var
			iscloaking = 0 // handles the vampire cloak toggle
		var
			list
				powers = list() // list of available powers and passives
		var
			mob
				living
					carbon
		...
		var
			nullified = 0 //Nullrod makes them useless for a short while.
		var
			list
				upgrade_tiers = list(
					obj
...
datum
	vampire
		proc
			handle_bloodsucking(mob/living/carbon/human/H)
				draining = H
				var
					blood = 0
				var
					old_bloodtotal = 0 //used to see if we increased our blood total
				var
					old_bloodusable = 0 //used to see if we increased our blood usable
...
datum
	vampire
		proc
			check_vampire_upgrade(announce = 1)
				var
					list
						old_powers = powers.Copy()

				for(var/ptype in upgrade_tiers)
					var
						level = upgrade_tiers[ptype]
					if(bloodtotal >= level)
						add_ability(ptype)
...
datum
	vampire
		proc
			announce_new_power(list/old_powers)
				for(var/p in powers)
					if(!(p in old_powers))
...
datum
	game_mode
		proc
			update_vampire_icons_added(datum/mind/vampire_mind)
				var
					datum
						atom_hud
							antag
								vamp_hud = huds[ANTAG_HUD_VAMPIRE]
...
datum
	game_mode
		proc
			update_vampire_icons_removed(datum/mind/vampire_mind)
				var
					datum
						atom_hud
							antag
								vampire_hud = huds[ANTAG_HUD_VAMPIRE]
...
datum
	game_mode
		proc
			remove_vampire_mind(datum/mind/vampire_mind, datum/mind/head)
				//var/list/removal
				if(!istype(head))
		...
		proc
			check_sun()
				...
				for(var/i = 1 to 20)
					...
					var
						turf
							T = locate(round(ax, 0.5), round(ay, 0.5), owner.z)
			...
			handle_vampire_cloak()
				...
				var
					atom
						movable
							lighting_overlay
								L = locate(/atom/movable/lighting_overlay) in T
				var
					light_available
				if(L)
...
datum
	vampire
		proc
			vamp_burn(severe_burn)
				var
					burn_chance = severe_burn ? 35 : 8
