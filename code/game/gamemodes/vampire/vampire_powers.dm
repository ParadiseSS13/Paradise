...
obj
	effect
		proc_holder
			spell
				vampire
					cast_check(skipcharge = 0, mob/living/user = usr)
						if(!user.mind)
							return 0
						if(!ishuman(user))
						...
						var
							datum
								vampire
									vampire = user.mind.vampire

						if(!vampire)
							return 0

						var
							fullpower = vampire.get_ability(/datum/vampire_passive/full)
	...
	effect
		proc_holder
			spell
				vampire
					proc
						affects(mob/target, mob/user = usr)
...
proc
	isvampirethrall(mob/living/M as mob)
			...
			spell
				vampire
					targetted
						enthrall
							proc
								can_enthrall(mob/living/user, mob/living/carbon/C)
									var
										enthrall_safe = 0
			...
			spell
				vampire
					targetted
						enthrall
							proc
								handle_enthrall(mob/living/user, mob/living/carbon/human/H as mob)
									if(!istype(H))
										return 0
									var
										ref = "\ref[user.mind]"
					...
					bats
						choose_targets(mob/user = usr)
							...
							for(var/direction in alldirs) //looking for bat spawns
								...
								var
									turf
										T = get_step(usr, direction) //getting a loc in that direction
					...
					shadowstep
						choose_targets(mob/user = usr)
							...
							for(var/turf/T in range(user, outer_tele_radius))
								...
								var
									lightingcount = L.get_clamped_lum(0.5) * 10
							...
							spawn(0)
								...
								var
									atom
										movable
											overlay
												animation = new /atom/movable/overlay(get_turf(usr))
