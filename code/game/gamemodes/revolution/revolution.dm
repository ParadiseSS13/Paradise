...
/datum/game_mode/proc/remove_revolutionary(datum/mind/rev_mind , beingborged)
	...
	if((rev_mind in revolutionaries) || remove_head)
		...
		if(beingborged)
			...
			message_admins("[key_name_admin(rev_mind.current)] <A HREF='?_src_=holder;adminmoreinfo=\ref[rev_mind.current]'>?</A> [ADMIN_FLW(rev_mind.current,"FLW")] has been borged while being a [remove_head ? "leader" : " member"] of the revolution.")
