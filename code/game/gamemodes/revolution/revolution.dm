			...
			pre_setup()
				...
				for(var/i=1 to max_headrevs)
					...
					var
						datum
							mind
								lenin = pick(possible_revolutionaries)
			...
			post_setup()
				...
				var
					list
						sec = get_living_sec()
				var
					weighted_score = min(max(round(heads.len - ((8 - sec.len) / 3)),1),max_headrevs)

				while(weighted_score < head_revolutionaries.len) //das vi danya
					var
						datum
							mind
								trotsky = pick(head_revolutionaries)
...
datum
	game_mode
		proc
			forge_revolutionary_objectives(datum/mind/rev_mind)
				var
					list
						heads = get_living_heads()
				for(var/datum/mind/head_mind in heads)
...
datum
	game_mode
		proc
			greet_revolutionary(datum/mind/rev_mind, you_are=1)
				var
					obj_count = 1
...
datum
	game_mode
		proc
			equip_revolutionary(mob/living/carbon/human/mob)
				if(!istype(mob))
					return
				...
				var
					where2 = mob.equip_in_one_of_slots(C, slots)
...
datum
	game_mode
		revolution
			proc
				mark_for_death(datum/mind/rev_mind, datum/mind/head_mind)
					var
						datum
							objective
								mutiny
...
datum
	game_mode
		proc
			add_revolutionary(datum/mind/rev_mind)
				...
				var
					mob
						living
							carbon
								human
									H = rev_mind.current//Check to see if the potential rev is implanted
				if(isloyal(H))
					return 0
...
datum
	game_mode
		proc
			remove_revolutionary(datum/mind/rev_mind , beingborged)
				var
					remove_head = 0
...
datum
	game_mode
		proc
			update_rev_icons_added(datum/mind/rev_mind)
				var
					datum
						atom_hud
							antag
								revhud = huds[ANTAG_HUD_REV]
...
datum
	game_mode
		proc
			update_rev_icons_removed(datum/mind/rev_mind)
				var
					datum
						atom_hud
							antag
	...
	game_mode
		revolution
			set_scoreboard_gvars()
				...
				var
					killpoints = score_opkilled * 500
				var
					comdeadpts = score_deadcommand * 500
