			...
			can_start()
				...
				var
					list
						datum
							mind
								possible_borers = get_players_for_role(ROLE_BORER)
				...
				while(possible_borers.len >= 2)
					var
						datum
							mind
								borer = pick(possible_borers)
					possible_borers.Remove(borer)

					var
						datum
							mind
								first_host = pick(possible_borers)
			...
			post_setup()
				...
				for(var/datum/mind/borer in borers)
					...
					var
						mob
							living
								simple_animal
									borer
										M = new(spawn_loc,1) // loc, by_gamemode=0
					var
						mob
							original = borer.current
					borer.transfer_to(M)
					//M.clearHUD()
					...
					var
						datum
							mind
								first_host = assigned_hosts[borer.key]
...
datum
	game_mode
		proc
			greet_borer(var/datum/mind/borer, var/you_are=1)
				if(you_are)
...
datum
	game_mode
		proc
			forge_borer_objectives(var/datum/mind/borer, var/datum/mind/first_host)
				var
					datum
						objective
							survive
