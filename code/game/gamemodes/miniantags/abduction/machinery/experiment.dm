...
obj
	machinery
		abductor
			experiment
				proc
					dissection_icon(mob/living/carbon/human/H)
						var
							icon
								I = icon(H.stand_icon)
						...
						var
							icon
								splat = icon(H.species.damage_overlays, "30")
				...
				proc
					experimentUI(mob/user)
						...
						if(occupant)
							var
								icon
									H = icon(dissection_icon(occupant), dir = SOUTH)
							if(H)
						...
						var
							datum
								browser
									popup = new(user, "experiment", "Probing Console", 300, 300)
				...
				proc
					Experiment(mob/occupant,type)
							...
							obj
								item
									organ
										internal
											gland
												GlandTest = locate() in H.internal_organs
						if(!GlandTest)
						...
						if(H.mind != null && H.ckey != null)
							...
							var
								objtype = pick(subtypesof(/datum/objective/abductee/))
							var
								datum
									objective
										abductee
...
obj
	machinery
		abductor
			experiment
				proc
					SendBack(mob/living/carbon/human/H)
						H.Sleeping(8)
