...
obj
	machinery
		abductor
			proc
				IsAgent(mob/living/carbon/human/H)
			...
			console
				attack_hand(mob/user)
					...
					var
						datum
							browser
								popup = new(user, "computer", "Abductor Console", 400, 500)
			...
			console
				proc
					SelectDisguise(remote=0)
						...
						var
							entry_name = input( "Choose Disguise", "Disguise") in entries
						var
							datum
								icon_snapshot
...
obj
	machinery
		abductor
			console
				proc
					SetDroppoint(turf/location,user)
						if(!istype(location))
...
obj
	machinery
		abductor
			console
				proc
					Link_Abduction_Equipment() // these must all be explicitly `in machines` or they will not properly link.
...
obj
	machinery
		abductor
			console
				proc
					AddSnapshot(mob/living/carbon/human/target)
						var
							datum
								icon_snapshot
									entry = new
...
obj
	machinery
		abductor
			console
				attackby(obj/O, mob/user, params)
