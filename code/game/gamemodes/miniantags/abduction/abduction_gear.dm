							...
							var
								datum
									icon_snapshot
										disguise
							var
								stealth_armor = list(melee = 15, bullet = 15, laser = 15, energy = 15, bomb = 15, bio = 15, rad = 15)
							var
								combat_armor = list(melee = 50, bullet = 50, laser = 50, energy = 50, bomb = 50, bio = 50, rad = 50)
							species_fit = null
							sprite_sheets = null
			...
			suit
				armor
					abductor
						vest
							proc
								SetDisguise(datum/icon_snapshot/entry)
									disguise = entry
	...
	item
		device
			abductor
				gizmo
					proc
						scan(atom/target, mob/living/user)
	...
	item
		device
			abductor
				gizmo
					proc
						mark(atom/target, mob/living/user)
							if(marked == target)
	...
	item
		device
			abductor
				gizmo
					proc
						prepare(atom/target, mob/living/user)
	...
	item
		device
			abductor
				silencer
					proc
						radio_off(atom/target, mob/living/user)
	...
	item
		device
			abductor
				silencer
					proc
						radio_off_mob(mob/living/carbon/human/M)
							var
								list
...
obj
	item
		weapon
			implant
				abductor
					implant(var/mob/source, var/mob/user)
						if(..())
							var
								obj
									machinery
...
obj
	item
		weapon
			abductor_baton
				proc
					StunAttack(mob/living/L,mob/living/user)
						user.lastattacked = L
						L.lastattacker = user
...
obj
	item
		weapon
			abductor_baton
				proc
					SleepAttack(mob/living/L,mob/living/user)
						if(L.stunned)
...
obj
	item
		weapon
			abductor_baton
				proc
					CuffAttack(mob/living/L,mob/living/user)
						if(!iscarbon(L))
							return
						var
							mob
...
obj
	item
		weapon
			abductor_baton
				proc
					ProbeAttack(mob/living/L,mob/living/user)
								...
								obj
									item
										organ
											internal
												gland
													temp = locate() in H.internal_organs
							if(temp)
