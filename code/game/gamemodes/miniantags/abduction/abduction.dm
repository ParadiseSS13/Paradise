	...
	game_mode
		abduction
			pre_setup()
				...
				var
					possible_teams = max(1,round(possible_abductors.len / 2))
...
datum
	game_mode
		abduction
			proc
				make_abductor_team(team_number,preset_agent=null,preset_scientist=null)
					//Team Name
...
datum
	abductor //stores abductor's team and whether they're a scientist or agent; since species datums are global, we have to use this, instead.
		var
			scientist = 0
		var
			agent = 0
...
datum
	game_mode
		abduction
			proc
				greet_agent(datum/mind/abductor,team_number)
...
datum
	game_mode
		abduction
			proc
				greet_scientist(datum/mind/abductor,team_number)
					abductor.objectives += team_objectives[team_number]
					var
						team_name = team_names[team_number]
...
datum
	game_mode
		abduction
			proc
				equip_common(mob/living/carbon/human/agent,team_number)
					var
					...
					var
						obj
							item
								device
									radio
										R = new /obj/item/device/radio/headset/syndicate/alt(agent)
...
datum
	game_mode
		abduction
			proc
				equip_agent(mob/living/carbon/human/agent,team_number)
					if(!team_number)
					...
					var
						obj
							machinery
								abductor
									console
										console = get_team_console(team_number)
					var
						obj
							item
								clothing
									suit
										armor
											abductor
												vest
													V = new /obj/item/clothing/suit/armor/abductor/vest(agent)
					if(console!=null)
						console.vest = V
						V.flags |= NODROP
...
datum
	game_mode
		abduction
			proc
				equip_scientist(var/mob/living/carbon/human/scientist,var/team_number)
					if(!team_number)
						...
						obj
							item
								device
									abductor
										gizmo
											G = new /obj/item/device/abductor/gizmo(scientist)
					if(console!=null)
						console.gizmo = G
						G.console = console
						...
						obj
							item
								weapon
									implant
										abductor
											beamplant = new /obj/item/weapon/implant/abductor(scientist)
						...
						var
							obj
								machinery
									abductor
										console
											con = get_team_console(team_number)
						var
							datum
								objective
									objective = team_objectives[team_number]
...
datum
	game_mode
		proc
			update_abductor_icons_added(datum/mind/alien_mind)
				var
					datum
						atom_hud
							antag
								hud = huds[ANTAG_HUD_ABDUCTOR]
...
datum
	game_mode
		proc
			update_abductor_icons_removed(datum/mind/alien_mind)
				var
					datum
						atom_hud
							antag
	...
	objective
		abductee
			steal
				New()
					var
						target = pick(list("pets","lights","monkeys","fruits","shoes","bars of soap"))
		...
		abductee
			capture
				New()
					var
						list
							jobs = job_master.occupations.Copy()
