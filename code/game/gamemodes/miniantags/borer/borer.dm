	...
	living
		simple_animal
			borer
				...
				var
					talk_inside_host = 0 				// So that borers don't accidentally give themselves away on a botched message
				var
					used_dominate
				var
					chemicals = 10                      // Chemicals used for reproduction and chemical injection.
				var
					max_chems = 250						// How many chemicals that can be stored in total
				var
					mob
						living
							carbon
								human
									host        // Human host for the brain worm.
				var
					truename                            // Name used for brainworm-speak.
				var
					mob
						living
							captive_brain
								host_brain // Used for swapping control of the body back and forth.
				var
					controlling                         // Used in human death check.
				var
					docile = 0                          // Sugar can stop borers from acting.
				var
					list
						borer_injection_chems = list("mannitol","salglu_solution","methamphetamine", "hydrocodone", "spaceacillin", "mitocholide", "charcoal", "salbutamol", "capulettium_plus")
			...
			borer
				verb
					Communicate()
						...
						var
							input = stripped_input(src, "Please enter a message to tell your host.", "Borer", "")
						if(!input) return


						var
							say_string = (docile) ? "slurs" :"states"
						if(host)
				...
				var
					mob
						living
							simple_animal
								borer
									B = src.has_brain_worms()
				if(!B)
					return

				var
					input = stripped_input(src, "Please enter a message to tell the borer.", "Message", "")
				if(!input) return
					...
					mob
						living
							captive_brain
								CB = B.host_brain
				var
					input = stripped_input(src, "Please enter a message to tell the trapped mind.", "Message", "")
				if(!input) return
					...
					dominate_victim()
						...
						var
							mob
								living
									carbon
										M = input(src,"Who do you wish to dominate?") in null|choices
					...
					bond_brain()
						...
						spawn(300+(host.getBrainLoss()*5))
							...
							if(!host || !src || controlling)
								...
								// host -> brain
								var
									h2b_id = host.computer_id
								var
									h2b_ip= host.lastKnownIP
								...
								// self -> host
								var
									s2h_id = src.computer_id
								var
									s2h_ip= src.lastKnownIP
			...
			borer
				verb
					secrete_chemicals()
						...
						var
							list
								nice_name_chem_list = list()
						for(var/rgnt in borer_injection_chems)
							var
								datum
									reagent
										R2 = chemical_reagents_list[rgnt]
							nice_name_chem_list[R2.name] = rgnt
						var
							chem_name = input("Select a chemical to secrete.", "Chemicals") as null|anything in nice_name_chem_list
						var
							chem = nice_name_chem_list[chem_name]

						if(!chem || chemicals < chem_cost || !host || controlling || !src || stat) //Sanity check.
							return

						var
							chem_amount = host.reagents.get_reagent_amount(chem)
						var
							datum
								reagent
				...
				proc
					detatch()
						...
						if(host_brain)
							...
							// host -> self
							var
								h2s_id = host.computer_id
							var
								h2s_ip= host.lastKnownIP
							host.computer_id = null
							host.lastKnownIP = null
							...
							// brain -> host
							var
								b2h_id = host_brain.computer_id
							var
								b2h_ip= host_brain.lastKnownIP
						...
						var
							mob
								living
									carbon
										human
											M = input(src,"Who do you wish to infest?") in null|choices
...
mob
	living
		simple_animal
			borer
				proc
					perform_infestation(var/mob/living/carbon/M)
						src.host = M
						src.forceMove(M)
				...
				proc
					question(var/client/C)
						spawn(0)
							if(!C)	return
							var
								response = alert(C, "A cortical borer needs a player. Are you interested?", "Cortical borer request", "Yes", "No", "Never for this round")
							if(!C || ckey)
								return
							if(response == "Yes")
...
mob
	living
		simple_animal
			borer
				proc
					transfer_personality(var/client/candidate)
			...
			borer
				say(var/message)
					var
						datum
							language
								dialect = parse_language(message)
					if(!dialect)
