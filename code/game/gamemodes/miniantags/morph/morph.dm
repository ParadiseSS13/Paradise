		...
		simple_animal
			hostile
				morph
					...
					var
						playstyle_string = "<b><font size=3 color='red'>You are a morph,</font> an abomination of science created primarily with changeling cells. \
	...
	living
		simple_animal
			hostile
				morph
					proc
						allowed(atom/movable/A) // make it into property/proc ? not sure if worth it
...
mob
	living
		simple_animal
			hostile
				morph
					update_action_buttons() //So all eaten objects are not counted every life
						return
