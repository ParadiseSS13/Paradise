			...
			hostile
				guardian
					...
					var
						cooldown = 0
					var
						damage_transfer = 1 //how much damage from each attack we transfer to the owner
					var
						light_on = 0
					var
						luminosity_on = 3
					var
						mob
							living
								summoner
					var
						range = 10 //how far from the user the spirit can be
					var
						playstyle_string = "You are a standard Guardian. You shouldn't exist!"
					var
						magic_fluff_string = " You draw the Coder, symbolizing bugs and errors. This shouldn't happen! Submit a bug report!"
					var
						tech_fluff_string = "BOOT SEQUENCE COMPLETE. ERROR MODULE LOADED. THIS SHOULDN'T HAPPEN. Submit a bug report!"
					var
						bio_fluff_string = "Your scarabs fail to mutate. This shouldn't happen! Submit a bug report!"
					var
						admin_fluff_string = "URK URF!"//the wheels on the bus...
					var
						adminseal = FALSE
...
mob
	living
		simple_animal
			hostile
				guardian
					Life() //Dies if the summoner dies
						..()
						if(summoner)
...
mob
	living
		simple_animal
			hostile
				guardian
					Move() //Returns to summoner if they move out of range
						..()
						if(summoner)
							if(get_dist(get_turf(summoner),get_turf(src)) <= range)
								return
...
mob
	living
		simple_animal
			hostile
				guardian
					adjustHealth(amount) //The spirit is invincible, but passes on damage to the summoner
						var
							damage = amount * damage_transfer
						if(summoner)
							if(loc == summoner)
								return
	...
	living
		proc
			guardian_comm()
				...
				var
					input = stripped_input(src, "Please enter a message to tell your guardian.", "Message", "")
				if(!input) return
						...
						var
							list
								mob
									dead
										observer
											candidates = pollCandidates("Do you want to play as [G.real_name]?", ROLE_GUARDIAN, 0, 100)
						var
							mob
								dead
									observer
		...
		simple_animal
			hostile
				guardian
					fire
						proc
							collision_ignite(AM as mob|obj)
					...
					punch
						verb
							Battlecry()
								...
								var
									input = stripped_input(src,"What do you want your battlecry to be? Max length of 5 characters.", ,"", 6)
								if(input)
									battlecry = input
						...
						verb
							Snare()
								...
								if(src.snares.len <6)
									var
										turf
											snare_loc = get_turf(src.loc)
									var
										obj
											item
												effect
													snare
														S = new /obj/item/effect/snare(snare_loc)
									S.spawner = src
					...
					ranged
						verb
							DisarmSnare()
								...
								var
									picked_snare = input(src, "Pick which trap to disarm", "Disarm Trap") as null|anything in src.snares
								if(picked_snare)
									...
									var
										obj
											item
												weapon
													guardian_bomb
														B = new /obj/item/weapon/guardian_bomb(get_turf(A))
	...
	item
		weapon
			guardiancreator
				...
				var
					use_message = "You shuffle the deck..."
				var
					used_message = "All the cards seem to be blank now."
				var
					failure_message = "..And draw a card! It's...blank? Maybe you should try again later."
				var
					ling_failure = "The deck refuses to respond to a souless creature such as you."
				var
					list
						possible_guardians = list("Chaos", "Standard", "Ranged", "Support", "Explosive")
				var
					random = TRUE
					...
					var
						list
							mob
								dead
									observer
										candidates = pollCandidates("Do you want to play as the [mob_name] of [user.real_name]?", ROLE_GUARDIAN, 0, 100)
					var
						mob
							dead
								observer
...
obj
	item
		weapon
			guardiancreator
				proc
					spawn_guardian(var/mob/living/user, var/key)
						var
						...
						var
							picked_name
						var
							picked_color = pick("#FFFFFF","#000000","#808080","#A52A2A","#FF0000","#8B0000","#DC143C","#FFA500","#FFFF00","#008000","#00FF00","#006400","#00FFFF","#0000FF","#000080","#008080","#800080","#4B0082")
