	...
	item
		unactivated_swarmer
			attack_ghost(mob/user as mob)
				var
					be_swarmer = alert("Become a swarmer? (Warning, You can no longer be cloned!)",,"Yes","No")
				if(be_swarmer == "No")
					return
				if(qdeleted(src))
					...
					mob
						living
							simple_animal
								hostile
									swarmer
										S = new /mob/living/simple_animal/hostile/swarmer(get_turf(loc))
				S.key = user.key
				qdel(src)
		...
		simple_animal
			hostile
				swarmer
					...
					AIStatus = AI_OFF
					var
						resources = 0 //Resource points, generated by consuming metal/glass
...
atom
	proc
		swarmer_act(mob/living/simple_animal/hostile/swarmer/S)
			S.DisIntegrate(src)
...
obj
	item
		weapon
			gun
				swarmer_act()//Stops you from eating the entire armory
					return

turf
	simulated
		floor
			swarmer_act()//ex_act() on turf calls it on its contents, this is to prevent attacking mobs by DisIntegrate()'ing the floor
				return
...
obj
	machinery
		particle_accelerator
			swarmer_act(mob/living/simple_animal/hostile/swarmer/S) // Since the console is still parented to this
				to_chat(S, "<span class='warning'>Disrupting the power grid would bring no benefit to us. Aborting.</span>")
...
obj
	machinery
		vending
			swarmer_act(mob/living/simple_animal/hostile/swarmer/S)//It's more visually interesting than dismantling the machine
				S.DisIntegrate(src)
...
obj
	item
		stack
			cable_coil
				swarmer_act(mob/living/simple_animal/hostile/swarmer/S)//Wiring would be too effective as a resource
	...
	living
		simple_animal
			hostile
				swarmer
					proc
						Fabricate(var/atom/fabrication_object,var/fabrication_cost = 0)
							if(!isturf(loc))
								...
								var
									obj
										effect
											swarmer
												integrate
													I = new /obj/effect/swarmer/integrate(get_turf(target))
	...
	living
		simple_animal
			hostile
				swarmer
					proc
						DisIntegrate(var/atom/movable/target)
	...
	living
		simple_animal
			hostile
				swarmer
					proc
						DisperseTarget(var/mob/living/target)
							if(target != src)
								...
								if(do_mob(src, target, 30))
									...
									for(cycle=0,cycle<100,cycle++)
										var
											random_location = locate(rand(37,202),rand(75,192),ZLEVEL_STATION)//Drunk dial a turf in the general ballpark of the station
																			...
																			weapon
																				restraints
																					handcuffs
																						cable
																							zipties
																								Z = new /obj/item/weapon/restraints/handcuffs/cable/zipties(src)
	...
	living
		simple_animal
			hostile
				swarmer
					proc
						DismantleMachine(var/obj/machinery/target)
							do_attack_animation(target)
							...
							var
								obj
									effect
										swarmer
											dismantle
												D = new /obj/effect/swarmer/dismantle(get_turf(target))
										...
										obj
											item
												stack
													sheet
														metal
															M = new /obj/item/stack/sheet/metal(target.loc)
									M.amount = 5
									...
									var
										obj
											effect
												swarmer
													disintegration
														N = new /obj/effect/swarmer/disintegration(get_turf(target))
...
obj
	effect
		swarmer //Default swarmer effect object visual feedback
			name = "swarmer ui"
			desc = null
			gender = NEUTER
			icon = 'icons/mob/swarmer.dmi'
...
obj
	effect
		swarmer
			destructible //Default destroyable object for swarmer constructions
				light_range = 1
				mouse_opacity = 1
				var
					health = 30
...
obj
	effect
		swarmer
			destructible
				attackby(obj/item/weapon/I, mob/living/user, params)
			...
			hostile
				swarmer
					proc
						ContactSwarmers()
							var
								message = input(src, "Announce to other swarmers", "Swarmer contact")
							if(message)
