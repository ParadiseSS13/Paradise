...
mob
	living
		simple_animal
			revenant
				ClickOn(var/atom/A, var/params) //Copypaste from ghost code - revenants can't interact with the world directly.
...
mob
	living
		simple_animal
			revenant
				proc
					Harvest(mob/living/carbon/human/target)
						if(!castcheck(0))
							return
						if(draining)
	...
	effect
		proc_holder
			spell
				targeted
					revenant_transmit
						cast(list/targets, mob/living/simple_animal/revenant/user = usr)
							for(var/mob/living/M in targets)
								spawn(0)
									var
										msg = stripped_input(usr, "What do you wish to tell [M]?", null, "")
									if(!msg)
			...
			spell
				aoe_turf
					revenant
						...
						var
							stun = 20 //How long it stuns the revenant in deciseconds
						var
							locked = 1 //If it's locked and needs to be unlocked before use
						var
							unlock_amount = 100 //How much essence it costs to unlock
						var
							cast_amount = 50 //How much essence it costs to use
	...
	effect
		proc_holder
			spell
				aoe_turf
					revenant
						can_cast(mob/living/simple_animal/revenant/user = usr)
							if(user.inhibited)
								return 0
		...
		proc_holder
			spell
				aoe_turf
					revenant
						proc
							attempt_cast(mob/living/simple_animal/revenant/user = usr)
								if(locked)
														...
														var
															datum
																effect
																	system
																		spark_spread
																			z = new /datum/effect/system/spark_spread/
														z.set_up(4, 0, M)
														z.start()
