	...
	living
		simple_animal
			revenant
				...
				var
					essence = 75 //The resource of revenants. Max health is equal to three times this amount
				var
					essence_regen_cap = 75 //The regeneration cap of essence (go figure); regenerates every Life() tick up to this amount.
				var
					essence_regenerating = 1 //If the revenant regenerates essence or not; 1 for yes, 0 for no
				var
					essence_regen_amount = 5 //How much essence regenerates
				var
					essence_accumulated = 0 //How much essence the revenant has stolen
				var
					revealed = 0 //If the revenant can take damage from normal sources.
				var
					unreveal_time = 0 //How long the revenant is revealed for, is about 2 seconds times this var.
				var
					unstun_time = 0 //How long the revenant is stunned for, is about 2 seconds times this var.
				var
					inhibited = 0 //If the revenant's abilities are blocked by a chaplain's power.
				var
					essence_drained = 0 //How much essence the revenant has drained.
				var
					draining = 0 //If the revenant is draining someone.
				var
					list
						drained_mobs = list() //Cannot harvest the same mob twice
				var
					perfectsouls = 0 //How many perfect, regen-cap increasing souls the revenant has.
				var
					image
						ghostimage = null //Visible to ghost with darkness off
		...
		simple_animal
			revenant
				say(message)
					...
					var
						rendered = "<span class='revennotice'><b>[src]</b> says, \"[message]\"</span>"
								...
								weapon
									ectoplasm
										revenant
											R = new (get_turf(src))
					var
						reforming_essence = essence_regen_cap //retain the gained essence capacity
...
mob
	living
		simple_animal
			revenant
				proc
					change_essence_amount(essence_amt, silent = 0, source = null)
						if(!src)
							return
	...
	objective
		revenantFluff
			New()
				var
					list
						explanationTexts = list("Assist and exacerbate existing threats at critical moments.", \
			...
			ectoplasm
				revenant
					...
					var
						reforming = 1
					var
						essence = 75 //the maximum essence of the reforming revenant
					var
						inert = 0
					var
						client
						...
						reform()
							...
							spawn()
								if(!key_of_revenant)
									...
									var
										list
											candidates = pollCandidates("Do you want to play as a revenant?", ROLE_REVENANT, 1)
									if(!candidates.len)
										qdel(R)
								...
								var
									datum
										mind
											player_mind = new /datum/mind(key_of_revenant)
