...
mob
	living
		proc
			phaseout(var/obj/effect/decal/cleanable/B)
					...
					var
						obj
							effect
								dummy
									slaughter
										holder = new /obj/effect/dummy/slaughter(mobloc)
					var
						atom
							movable
								overlay
...
mob
	living
		proc
			phasein(var/obj/effect/decal/cleanable/B)
				...
				var
					atom
						movable
							overlay
								animation = new /atom/movable/overlay( B.loc )
				animation.name = "odd blood"
				animation.density = 0
				animation.anchored = 1
				animation.icon = 'icons/mob/mob.dmi'
				...
				if(prob(25) && istype(src, /mob/living/simple_animal/slaughter))
					var
						list
							voice = list('sound/hallucinations/behind_you1.ogg','sound/hallucinations/im_here1.ogg','sound/hallucinations/turn_around1.ogg','sound/hallucinations/i_see_you1.ogg')
...
obj
	effect
		dummy
			slaughter //Can't use the wizard one, blocked by jaunt/slow
				name = "odd blood"
