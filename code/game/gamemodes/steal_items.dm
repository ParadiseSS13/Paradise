...
datum
	theft_objective
		proc
			check_completion(var/datum/mind/owner)
				if(!owner.current)
					return 0
...
datum
	theft_objective
		ai
			check_special_completion(var/obj/item/device/aicard/C)
				if(..())
...
datum
	theft_objective
		slime_extract
			check_special_completion(var/obj/item/slime_extract/E)
				if(..())
					if(E.Uses > 0)
						return 1
				return 0
...
datum
	theft_objective
		number
			proc
				getAmountStolen(var/obj/item/I)
					return I:amount
