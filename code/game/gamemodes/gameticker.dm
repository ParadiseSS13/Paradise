...
datum
	controller
		gameticker
			...
			var
				hide_mode = 0 // leave here at 0 ! setup() will take care of it when needed for Secret mode -walter0o
			var
				datum
					game_mode
						mode = null
			...
			var
				login_music // music played in pregame lobby

			var
				list
					datum
						mind
							minds = list()//The people in the game. Used for objective tracking.
			...
			var
				random_players = 0 	// if set to nonzero, ALL players who latejoin or declare-ready join will have random appearances/genders

			var
				list
					syndicate_coalition = list() // list of traitor-compatible factions
			var
				list
					factions = list()			  // list of all factions
			var
				list
					availablefactions = list()	  // list of factions with openings

			var
				pregame_timeleft = 0

			var
				delay_end = 0	//if set to nonzero, the round will not restart on it's own

			var
				triai = 0//Global holder for Triumvirate
			var
				initialtpass = 0 //holder for inital autotransfer vote timer

			var
				round_end_announced = 0 // Spam Prevention. Announce round end only once.
					...
					if((master_mode=="random") || (master_mode=="secret"))
						...
						if(secret_force_mode != "secret")
							var
								datum
									game_mode
										M = config.pick_mode(secret_force_mode)
							if(M.can_start())
						...
						if(holiday_master.holidays)
							...
							for(var/holidayname in holiday_master.holidays)
								var
									datum
										holiday
											holiday = holiday_master.holidays[holidayname]
						...
						var
							list
								area
									dynamic
										source
											available_source_candidates = subtypesof(/area/dynamic/source)
						var
							list
								area
									dynamic
										destination
											available_destination_candidates = subtypesof(/area/dynamic/destination)
						...
						for(var/area/dynamic/destination/current_destination_candidate in available_destination_candidates)
							var
								area
									dynamic
										destination
											current_destination = locate(current_destination_candidate)
							...
							var
								area
									dynamic
										source
											selected_source = pick(candidate_source_areas)
...
datum
	controller
		gameticker
			...
			proc
				station_explosion_cinematic(var/station_missed=0, var/override = null)
			...
			proc
				process()
					...
					var
						game_finished = shuttle_master.emergency.mode >= SHUTTLE_ENDGAME || mode.station_was_nuked
				...
				declare_completion()
					...
					for(var/mob/living/silicon/ai/aiPlayer in mob_list)
						...
						if(aiPlayer.connected_robots.len)
							var
								robolist = "<b>The AI's loyal minions were:</b> "
