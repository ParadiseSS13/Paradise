...
obj
	machinery
		porta_turret
			...
			var
				raised = 0			//if the turret cover is "open" and the turret is raised
			var
				raising= 0			//if the turret is currently opening or closing its cover
			var
				health = 80			//the turret's health
			var
				locked = 1			//if the turret's behaviour control access is locked
			var
				controllock = 0		//if the turret responds to control panels

			var
				installation = /obj/item/weapon/gun/energy/gun/turret		//the type of weapon installed
			var
				gun_charge = 0		//the charge of the gun inserted
			var
				projectile = null	//holder for bullettype
			var
				eprojectile = null	//holder for the shot when emagged
			var
				reqpower = 500		//holder for power needed
			var
				iconholder = null	//holder for the icon_state. 1 for orange sprite, null for blue.
			var
				egun = null			//holder to handle certain guns switching bullettypes

			var
				last_fired = 0		//1: if the turret is cooling down from a shot, 0: turret is ready to fire
			var
			...
			var
				check_weapons = 0	//checks if it can shoot people that have a weapon they aren't authorized to have
			var
				check_access = 1	//if this is active, the turret shoots everything that does not meet the access requirements
			var
				check_anomalies = 1	//checks if it can shoot at unidentified lifeforms (ie xenos)
			var
				check_synth	 = 0 	//if active, will shoot at anything not an AI or cyborg
			var
				ailock = 0 			// AI cannot use this

			var
				attacked = 0		//if set to 1, the turret gets pissed off and shoots at people nearby (unless they have sec access!)

			var
				enabled = 1				//determines if the turret is on
			var
				lethal = 0			//whether in lethal or stun mode
			var
				disabled = 0

			var
				shot_sound 			//what sound should play when the turret fires
			var
				eshot_sound			//what sound should play when the emagged turret fires
			...
			var
				wrenching = 0
			var
				last_target //last target fired at, prevents turrets from erratically firing at all valid targets in range

			var
				screen = 0 // Screen 0: main control, screen 1: access levels
			var
				one_access = 0 // Determines if access control is set to req_one_access or req_access
						...
						obj
							item
								weapon
									gun
										energy
											E= new installation	//All energy-based weapons are applicable
					var
						obj
							item
								ammo_casing
		...
		porta_turret
			Topic(href, href_list, var/nowindow = 0)
				...
				if(href_list["command"] && href_list["value"])
					var
						value = text2num(href_list["value"])
...
obj
	machinery
		porta_turret
			proc
				toggle_access(var/access)
					var
						required = text2num(access)
...
obj
	machinery
		porta_turret
			proc
				die()	//called when the turret dies, ie, health <= 0
					health = 0
	...
	machinery
		porta_turret
			process()
				...
				var
					list
						targets = list()			//list of primary targets
				var
					list
						secondarytargets = list()	//targets that are least important
...
obj
	machinery
		porta_turret
			proc
				assess_and_assign(var/mob/living/L, var/list/targets, var/list/secondarytargets)
...
obj
	machinery
		porta_turret
			proc
				assess_living(var/mob/living/L)
					if(!istype(L))
...
obj
	machinery
		porta_turret
			proc
				tryToShootAt(var/list/mob/living/targets)
		...
		porta_turret
			proc
				popUp()	//pops the turret up
					...
					var
						atom
							flick_holder = new /atom/movable/porta_turret_cover(loc)
...
obj
	machinery
		porta_turret
			on_assess_perp(mob/living/carbon/human/perp)
...
obj
	machinery
		porta_turret
			proc
				set_raised_raising(var/raised, var/raising)
					src.raised = raised
					src.raising = raising
...
obj
	machinery
		porta_turret
			proc
				setState(var/datum/turret_checks/TC)
					if(controllock)
						return
...
obj
	machinery
		porta_turret_construct
			name = "turret frame"
			icon = 'icons/obj/turrets.dmi'
			icon_state = "turret_frame"
			density=1
			var
				target_type = /obj/machinery/porta_turret	// The type we intend to build
			var
				build_step = 0			//the current step in the building process
			var
				finish_name="turret"	//the name applied to the product turret
			var
				installation = null		//the gun type installed
			var
				gun_charge = 0			//the gun charge of the gun type installed
		...
		porta_turret_construct
			attackby(obj/item/I, mob/user)
								...
								obj
									item
										weapon
											gun
												energy
													E = I //typecasts the item to an energy gun
							if(!user.unEquip(I))
				...
				if(istype(I, /obj/item/weapon/pen))	//you can rename turrets like bots!
					var
						t = input(user, "Enter new turret name", name, finish_name) as text
