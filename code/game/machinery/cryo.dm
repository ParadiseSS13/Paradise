			...
			unary
				cryo_cell
					...
					var
						next_trans = 0
					var
						current_heat_capacity = 50
					var
						efficiency

					var
						running_bob_animation = 0 // This is used to prevent threads from building up if update_icons is called multiple times
	...
	machinery
		atmospherics
			unary
				cryo_cell
					proc
						handle_update_icon() //making another proc to avoid spam in update_icon
							...
							if(occupant)
								var
									image
										pickle = image(occupant.icon, occupant.icon_state)
								...
								if(src.on && !running_bob_animation) //no bobbing if off
									var
										up = 0 //used to see if we are going up or down, 1 is down, 2 is up
							...
							if(occupant)
								...
								if(occupant.bodytemperature < T0C)
									...
									if(occupant.bodytemperature < 225)
										...
										var
											heal_brute = occupant.getBruteLoss() ? min(efficiency, 20*(efficiency**2) / occupant.getBruteLoss()) : 0
										var
											heal_fire = occupant.getFireLoss() ? min(efficiency, 20*(efficiency**2) / occupant.getFireLoss()) : 0
				...
				cryo_cell
					proc
						heat_gas_contents()
							...
							var
								combined_heat_capacity = current_heat_capacity + air_heat_capacity
							if(combined_heat_capacity > 0)
								var
									combined_energy = T20C*current_heat_capacity + air_heat_capacity*air_contents.temperature
...
datum
	data
		function
			proc
				r_input(href, href_list, mob/user as mob)
					return
