		...
		dye_generator
			attack_hand(mob/user as mob)
				...
				if(stat & (BROKEN|NOPOWER))
					return
				var
					temp = input(usr, "Choose a dye color", "Dye Color") as color
				dye_color = temp
		...
		hair_dye_bottle
			proc
				update_dye_overlay()
					...
					var
						image
							I = new('icons/obj/items.dmi', "hairdyebottle-overlay")
					I.color = dye_color
					overlays += I
		...
		hair_dye_bottle
			attack(mob/living/carbon/M as mob, mob/user as mob)
				...
				if(ishuman(M))
					...
					var
						what_to_dye = input(user, "Choose an area to apply the dye","Dye Application") in dye_list
					...
					if(do_after(user, 50, target = H))
						switch(what_to_dye)
							if("hair")
								var
									r_hair = hex2num(copytext(dye_color, 2, 4))
								var
									g_hair = hex2num(copytext(dye_color, 4, 6))
								var
									b_hair = hex2num(copytext(dye_color, 6, 8))
								if(H.change_hair_color(r_hair, g_hair, b_hair))
									H.update_dna()
							if("facial hair")
								var
									r_facial = hex2num(copytext(dye_color, 2, 4))
								var
									g_facial = hex2num(copytext(dye_color, 4, 6))
								var
									b_facial = hex2num(copytext(dye_color, 6, 8))
								if(H.change_facial_hair_color(r_facial, g_facial, b_facial))
									H.update_dna()
							if("body")
								var
									r_skin = hex2num(copytext(dye_color, 2, 4))
								var
									g_skin = hex2num(copytext(dye_color, 4, 6))
								var
									b_skin = hex2num(copytext(dye_color, 6, 8))
