		...
		biogenerator
			interact(mob/user as mob)
				...
				var
					datum
						browser
							popup = new(user, "biogen", name, 350, 520)
			...
			proc
				activate()
					...
					for(var/obj/item/weapon/reagent_containers/food/snacks/grown/I in contents)
						S += 5
						var
							reagent_amount = I.reagents.get_reagent_amount("nutriment") + I.reagents.get_reagent_amount("plantmatter")
...
obj
	machinery
		biogenerator
			proc
				check_cost(var/cost, var/checkonly)
					if(cost > points)
						menustat = "nopoints"
						return 1
					else if(checkonly)
...
obj
	machinery
		biogenerator
			proc
				check_container_volume(var/reagent_amount)
