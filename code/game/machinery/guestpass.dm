			...
			card
				id
					guest
						...
						var
							temp_access = list() //to prevent agent cards stealing access as permanent
						var
							expiration_time = 0
						var
				...
				Topic(href, href_list)
					...
					if(href_list["choice"])
						switch(href_list["choice"])
							if("giv_name")
								var
									nam = strip_html_simple(input("Person pass is issued to", "Name", giv_name) as text|null)
								if(nam)
									giv_name = nam
							if("reason")
								var
									reas = strip_html_simple(input("Reason why pass is issued", "Reason", reason) as text|null)
								if(reas)
									reason = reas
							if("duration")
								var
									dur = input("Duration (in minutes) during which pass is valid (up to 30 minutes).", "Duration") as num|null
								if(dur)
									if(dur > 0 && dur <= 30)
										duration = dur
									else
					...
					if(href_list["action"])
						switch(href_list["action"])
							...
							if("print")
								var
									dat = "<h3>Activity log of guest pass terminal #[uid]</h3><br>"
								for(var/entry in internal_log)
									dat += "[entry]<br><hr>"
...
if("issue")
				if(giver)
					var
						number = add_zero("[rand(0,9999)]", 4)
					var
						entry = "\[[worldtime2text()]\] Pass #[number] issued by [giver.registered_name] ([giver.assignment]) to [giv_name]. Reason: [reason]. Grants access to following areas: "
