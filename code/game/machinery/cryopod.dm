...
/obj/machinery/cryopod/proc/go_out()
	...
	icon_state = base_icon_state
	name = initial(name)
	log_admin("[key_name(E)] exited a stasis pod.")
	if(isAntag(occupant))
		message_admins("[key_name_admin(E)] antag exited a stasis pod after " + round((world.time - time_entered )/600, 2) + " minutes inside (<A href='byond://?_src_=holder;adminplayerobservecoodjump=1;X=[x];Y=[y];Z=[z]'>JMP</a>)" )
