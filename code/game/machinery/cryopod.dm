		...
		if(istype(O,/datum/objective/mutiny) && O.target == occupant.mind)
			...
			if(O.target && istype(O.target,/datum/mind))
				if(O.owner && O.owner.current)
					var/datum/antagonist/cult/C = O.owner.has_antag_datum(/datum/antagonist/cult)
					var/list/p_s_t = C.get_possible_sac_targets()
					if(p_s_t.len)
						O.target = pick(p_s_t)
						O.update_explanation_text()
						for(var/datum/antagonist/cult/H in GLOB.antagonists)
							if(H.owner.current)
								to_chat(H.owner.current, "<span class='danger'>[SSticker.cultdat.entity_name]</span> murmurs, <span class='cultlarge'>[occupant] is beyond your reach. Sacrifice [O.target] instead...</span></span>")
								H.owner.current << 'sound/ambience/alarm4.ogg'
