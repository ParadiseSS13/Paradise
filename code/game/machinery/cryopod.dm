	...
	machinery
		computer
			cryopod
				...
				var
					storage_type = "crewmembers"
				var
					storage_name = "Cryogenic Oversight Control"
				var
					allow_items = 1
				...
				Topic(href, href_list)
					...
					if(href_list["view"])
						...
						var
							obj
								item
									I = input(usr, "Please choose which object to retrieve.","Object recovery",null) as null|anything in frozen_items
						if(!I)
							return
...
obj
	structure
		cryofeed
			...
			var
				orient_right = null //Flips the sprite.
...
obj
	machinery
		cryopod
			...
			var
				on_store_message = "has entered long-term storage."
			var
				on_store_name = "Cryogenic Oversight"
			var
			...
			var
				mob
					living
						occupant = null       // Person waiting to be despawned.
			var
				orient_right = null       // Flips the sprite.
			var
				time_till_despawn = 9000 // 15 minutes-ish safe period before being despawned.
			var
				time_entered = 0          // Used to keep track of the safe period.
			var
				obj
					item
						device
...
obj
	machinery
		cryopod
			proc
				find_control_computer(urgent=0)
			...
			proc
				despawn_occupant()
					...
					if(occupant.mind && occupant.mind.assigned_role)
						...
						var
							job = occupant.mind.assigned_role
		...
		cryopod
			attackby(var/obj/item/weapon/G as obj, var/mob/user as mob, params)
				...
				if(istype(G, /obj/item/weapon/grab))
					...
					var
						willing = null //We don't want to allow people to be forced into despawning.
					var
						mob
							living
								M = G:affecting
