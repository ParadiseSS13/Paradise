...
/obj/machinery/cryopod/proc/despawn_occupant()
	...
	if(!silent)
		...
		if(length(ailist))
			...
			if(announce_rank)
				announcer.say(";[occupant.real_name] ([announce_rank]) [on_store_message]", ignore_languages = TRUE, is_ai_auto = TRUE)
			else
				announcer.say(";[occupant.real_name] [on_store_message]", ignore_languages = TRUE, is_ai_auto = TRUE)
		else
			if(announce_rank)
