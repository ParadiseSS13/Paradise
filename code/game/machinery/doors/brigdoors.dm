...
/obj/machinery/door_timer
	...
	var/id = null
	/// when world.timeofday reaches it - release the prisoner
	var/releasetime = 0
	...
	/// Used to set releasetime upon starting the timer
	var/timetoset = 0
	var/obj/item/radio/Radio
	/// has a report already be printed?
	var/printed = FALSE
...
/obj/machinery/door_timer/proc/timer_end()
	...
	// Reset vars
	occupant = initial(occupant)
	crimes = initial(crimes)
	time = initial(time)
	timetoset = initial(timetoset)
	officer = initial(officer)
	releasetime = initial(releasetime)
	printed = initial(printed)
	if(prisoner)
...
/obj/machinery/door_timer/ui_act(action, params)
	...
	switch(action)
		...
		if("prisoner_notes")
			var/new_charge = tgui_input_text(usr, "Prisoner Notes:", name, prisoner_notes, encode = FALSE)
			if(isnull(new_charge))
				return
			prisoner_notes = new_charge
		...
		if("start")
			if(!prisoner_name || !crimes || !prisoner_time)
				return FALSE
			timeset(prisoner_time * 60)
			occupant = prisoner_name
			crimes = params["final_crimes"]
			prisoner_name = null
			prisoner_notes = null
			prisoner_time = null
			timing = TRUE
		...
		if("restart_timer")
			if(timing)
				...
				if(!reset_reason)
					...
					return FALSE
				releasetime = world.timeofday + timetoset
				var/reset_text = isobserver(usr) ? "for: [reset_reason]." : "by [usr.name] for: [reset_reason]."
				Radio.autosay("Prisoner [occupant] had their timer reset [reset_text]", name, "Security")
				notify_prisoner("Your brig timer has been reset for: '[reset_reason]'.")
				var/datum/data/record/R = find_security_record("name", occupant)
				if(istype(R))
					R.fields["comments"] += "Autogenerated by [name] on [GLOB.current_date_string] [station_time_timestamp()] - Timer reset [reset_text]"
			else
				. = FALSE

		if("add_time")
			var/added_time_reason = tgui_input_text(usr, "Reason for added time:", "Added Time Reason")
			if(!added_time_reason)
			...
			var/datum/data/record/R = find_security_record("name", occupant)
			if(istype(R))
				R.fields["comments"] += "Autogenerated by [name] on [GLOB.current_date_string] [station_time_timestamp()] - Timer extended [added_time_text]"
			else
				. = FALSE
