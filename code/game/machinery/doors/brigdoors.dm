...
/obj/machinery/door_timer/proc/print_report()
	if(cell_occupant == CELL_NONE || crimes == CELL_NONE)
		return 0
	...
	for(var/obj/machinery/computer/prisoner/C in GLOB.prisoncomputer_list)
		...
		P.name = "[id] log - [cell_occupant] [station_time_timestamp()]"
	...
	var/datum/data/record/G = find_record("name", cell_occupant, GLOB.data_core.general)
	...
	var/datum/data/record/R = find_security_record("name", cell_occupant)

	var/timetext = seconds_to_time(timetoset / 10)
	var/announcetext = "Detainee [cell_occupant] ([prisoner_drank]) has been incarcerated for [timetext] for the crime of: '[crimes]'. \
...
/obj/machinery/door_timer/proc/notify_prisoner(notifytext)
	for(var/mob/living/carbon/human/H in range(4, get_turf(src)))
		if(cell_occupant == H.name)
			to_chat(H, "[src] beeps, \"[notifytext]\"")
			return
	atom_say("[src] beeps, \"[cell_occupant]: [notifytext]\"")
...
/obj/machinery/door_timer/ui_data(mob/user)
	var/list/data = list()
	data["cell_id"] = name
	data["cell_occupant"] = cell_occupant
	data["crimes"] = crimes
	...
	switch(action)
		...
		if("restart_timer")
			if(timing)
				...
				Radio.autosay("Prisoner [cell_occupant] had their timer reset [resettext]", name, "Security", list(z))
