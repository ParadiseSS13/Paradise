...
obj
	machinery
		door_timer
			...
			var
				releasetime = 0		// when world.timeofday reaches it - release the prisoner
			var
				timing = 0    		// boolean, true/1 timer is on, false/0 means it's not timing
			var
				...
				list
					obj
						machinery
							targets = list()
			var
				timetoset = 0		// Used to set releasetime upon starting the timer
			var
				obj
					item
						device
	...
	machinery
		door_timer
			proc
				timeleft()
					var
						time = releasetime - world.timeofday
	...
	machinery
		door_timer
			attack_hand(var/mob/user as mob)
				...
				var
					second = round(timeleft() % 60)
				var
					minute = round((timeleft() - second) / 60)

				// Used for 'set timer'
				var
					setsecond = round((timetoset / 10) % 60)
				var
					setminute = round(((timetoset / 10) - setsecond) / 60)
			...
			update_icon()
				...
				if(src.timing)
					...
					var
						timeleft = timeleft()
					var
						disp2 = "[add_zero(num2text((timeleft / 60) % 60),2)]~[add_zero(num2text(timeleft % 60), 2)]"
...
obj
	machinery
		door_timer
			proc
				texticon(var/tn, var/px = 0, var/py = 0)
					var
						image
							I = image('icons/obj/status_display.dmi', "blank")
					var
						len = lentext(tn)

					for(var/d = 1 to len)
						var
							char = copytext(tn, len-d+1, len-d+2)
						if(char == " ")
							continue
						var
							image
								ID = image('icons/obj/status_display.dmi', icon_state=char)
						ID.pixel_x = -(d-1)*5 + px
						ID.pixel_y = py
						I.overlays += ID
					return I
