		...
		door
			window
				...
				var
					base_state = "left"
				var
					health = 150.0 //If you change this, consider changing ../door/window/brigdoor/ health at the bottom of this .dm file
				visible = 0.0
				flags = ON_BORDER
				opacity = 0
				var
...
obj
	machinery
		door
			window
				Bumped(atom/movable/AM as mob|obj)
...
obj
	machinery
		door
			window
				CanPass(atom/movable/mover, turf/target, height=0)
...
obj
	machinery
		door
			window
				CanAStarPass(var/obj/item/weapon/card/id/ID, var/to_dir)
...
obj
	machinery
		door
			window
				proc
					attack_generic(mob/user as mob, damage = 0)
						if(src.operating)
							return
...
obj
	machinery
		door
			window
				attack_animal(mob/living/user as mob)
					if(!isanimal(user))
						return
					var
						mob
...
obj
	machinery
		door
			window
				attack_slime(mob/living/carbon/slime/user as mob)
					if(!user.is_adult)
						return
					attack_generic(user, 25)
...
obj
	machinery
		door
			window
				attackby(obj/item/weapon/I as obj, mob/living/user as mob, params)
								...
								if(src.p_open && !src.density && !src.operating && src.loc)
									var
										obj
											structure
												windoor_assembly
													WA = new /obj/structure/windoor_assembly(src.loc)
									switch(base_state)
										if("left")
											WA.facing = "l"
										if("right")
