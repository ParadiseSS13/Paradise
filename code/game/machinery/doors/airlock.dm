	...
	machinery
		door
			airlock
				...
				var
					electrified_until = 0	// World time when the door is no longer electrified. -1 if it is permanently electrified until someone fixes it.
				var
					main_power_lost_until = 0	 //World time when main power is restored.
				var
					backup_power_lost_until = -1 //World time when backup power is restored.
				var
				...
				var
					locked = 0
				var
					lights = 1 // bolt lights show by default
				var
					datum
						wires
							airlock
						...
						item
							weapon
								airlock_electronics
									electronics = null
				var
					hasShocked = 0 //Prevents multiple shocks from happening
				var
					frozen = 0 //special condition for airlocks that are frozen shut, this will look weird on not normal airlocks because of a lack of special overlays.
				autoclose = 1
				explosion_block = 1
				var
				...
				var
					doorDeni = 'sound/machines/DeniedBeep.ogg' // i'm thinkin' Deni's
				var
...
obj
	machinery
		door
			airlock
				plasma
					BlockSuperconductivity() //we don't stop the heat~
						return 0
...
obj
	machinery
		door
			airlock
				alien
					bumpopen(mob/living/user as mob)
...
obj
	machinery
		door
			airlock
				alien
					attackby(C as obj, mob/user as mob, params)
...
obj
	machinery
		door
			airlock
				bumpopen(mob/living/user as mob) //Airlocks now zap you when you 'bump' them open when they're electrified. --NeoFite
					if(!issilicon(usr))
...
obj
	machinery
		door
			airlock
				bumpopen(mob/living/simple_animal/user as mob)
					..(user)
...
obj
	machinery
		door
			airlock
				proc
					electrify(var/duration, var/feedback = 0)
						if(electrified_timer)
							deltimer(electrified_timer)
							electrified_timer = null
		...
		door
			airlock
				Topic(href, href_list, var/nowindow = 0)
					...
					var
						activate = text2num(href_list["activate"])
...
obj
	machinery
		door
			airlock
				close(var/forced=0, var/override = 0)
					...
					var
						obj
							structure
								window
									killthis = (locate(/obj/structure/window) in get_turf(src))
					if(killthis)
...
obj
	machinery
		door
			airlock
				CanAStarPass(var/obj/item/weapon/card/id/ID)
		...
		door
			airlock
				emp_act(var/severity)
					if(prob(40/severity))
						var
							duration = world.time + SecondsToTicks(30 / severity)
...
obj
	machinery
		door
			airlock
				power_change() //putting this is obj/machinery/door itself makes non-airlock doors turn invisible for some reason
					..()
					if(stat & NOPOWER)
