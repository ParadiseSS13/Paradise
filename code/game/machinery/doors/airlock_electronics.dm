/obj/item/airlock_electronics
	...
	/// Which direction has unrestricted access (e.g. medbay doors from the inside)
...
/obj/item/airlock_electronics/tgui_interact(mob/user, ui_key, datum/tgui/ui, force_open, datum/tgui/master_ui, datum/tgui_state/state)
...
/obj/item/airlock_electronics/tgui_data(mob/user)
	...
	data["unrestricted_dir"] = unres_direction
	return data
...
/obj/item/airlock_electronics/tgui_act(action, list/params, datum/tgui/ui, datum/tgui_state/state)
	...
	switch(action)
		if("unrestricted_access")
			...
			if(direction == unres_direction)
				unres_direction = null // Deselecting
				return
			unres_direction = direction
