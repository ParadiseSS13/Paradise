	...
	machinery
		gibber
			...
			var
				mob
					living
						occupant // Mob who has been put inside
			var
				locked = 0 //Used to prevent mobs from breaking the feedin anim

			var
				gib_throw_dir = WEST // Direction to spit meat and gibs in. Defaults to west.
			var
...
obj
	machinery
		gibber
			RefreshParts() //If you want to make the machine upgradable, this is where you would change any vars basd on its stock parts.
				return
...
obj
	machinery
		gibber
			attackby(obj/item/P as obj, mob/user as mob, params)
...
obj
	machinery
		gibber
			proc
				move_into_gibber(var/mob/user,var/mob/living/victim)
					if(occupant)
		...
		gibber
			proc
				feedinTopanim()
					...
					var
						image
							gibberoverlay = new //used to simulate 3D effects
					...
					var
						atom
							movable
								holder = new //holder for occupant image
					...
					var
						atom
							movable
								holder2 = new //holder for gibber overlay, used to simulate 3D effect
					holder2.name = null
...
obj
	machinery
		gibber
			proc
				startgibbing(var/mob/user, var/UserOverride=0)
					...
					var
						slab_name = occupant.name
					var
						slab_count = 3
					var
						slab_type = /obj/item/weapon/reagent_containers/food/snacks/meat/human //gibber can only gib humans on paracode, no need to check meat type
					var
						slab_nutrition = src.occupant.nutrition / 15
...
obj
	machinery
		gibber
			autogibber
				proc
					force_move_into_gibber(var/mob/living/carbon/human/victim)
...
obj
	machinery
		gibber
			autogibber
				proc
					ejectclothes(var/mob/living/carbon/human/H)
		...
		gibber
			autogibber
				proc
					cleanbay()
						var
							spats = 0 //keeps track of how many items get spit out. Don't show a message if none are found.
						for(var/obj/O in src)
							if(istype(O))
								O.loc = src.loc
								O.throw_at(get_edge_target_turf(src,gib_throw_dir),rand(1,5),15)
