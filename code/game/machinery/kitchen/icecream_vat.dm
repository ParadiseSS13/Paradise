					...
					if(istype(O, /obj/item/weapon/reagent_containers/food/snacks/icecream))
						...
						if(!I.ice_creamed)
							if(ingredients[ICECREAM_VANILLA] > 0)
								var
									flavour_name = get_icecream_flavour_string(dispense_flavour)
...
obj
	machinery
		icecream_vat
			proc
				make(var/mob/user, var/make_type)
					switch(make_type)
						if(CONE_WAFFLE)
							if(ingredients[INGR_FLOUR] > 0 && ingredients[INGR_SUGAR] > 0)
								var
									amount = max( min(ingredients[INGR_FLOUR], ingredients[INGR_SUGAR]), 5)
						...
						if(CONE_CHOC)
							if(ingredients[FLAVOUR_CHOCOLATE] > 0 && ingredients[CONE_WAFFLE] > 0)
								var
									amount = min(ingredients[CONE_WAFFLE], ingredients[FLAVOUR_CHOCOLATE])
								ingredients[CONE_WAFFLE] -= amount
						...
						if(ICECREAM_VANILLA)
							if(ingredients[INGR_ICE] > 0 && ingredients[INGR_MILK] > 0)
								var
									amount = min(ingredients[INGR_ICE], ingredients[INGR_MILK])
	...
	machinery
		icecream_vat
			Topic(href, href_list)
				...
				if(href_list["cone"])
					var
						dispense_cone = text2num(href_list["cone"])
					if(ingredients[dispense_cone] <= ingredients.len)
						var
							cone_name = get_icecream_flavour_string(dispense_cone)
			...
			reagent_containers
				food
					snacks
						icecream
							proc
								add_ice_cream(var/flavour)
									var
										flavour_name = get_icecream_flavour_string(flavour)
