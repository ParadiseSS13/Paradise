...
obj
	machinery
		kitchen_machine
			...
			var
				dirty = 0 // = {0..100} Does it need cleaning?
			var
				broken = 0 // ={0,1,2} How broken is it???
			var
				efficiency = 0
			var
				list
					cook_verbs = list("Cooking")
			//Recipe & Item vars
			var
				recipe_type		//Make sure to set this on the machine definition, or else you're gonna runtime on New()
			var
				list
					datum
						recipe
							available_recipes // List of the recipes you can use
			var
				list
					acceptable_items // List of the items you can put in
			var
				list
					acceptable_reagents // List of the reagents you can put in
			var
				max_n_of_items = 0
			//Icon states
			var
			...
			proc
				cook()
					...
					if(!recipe)
						...
						var
							halftime = round(recipe.time/10/2)
						if(!wzhzhzh(halftime))
							abort()
							return
						if(!wzhzhzh(halftime))
			...
			proc
				fail()
					...
					for(var/obj/O in contents-ffuu)
						...
						if(O.reagents)
							var
								id = O.reagents.get_master_reagent_id()
							if(id)
