	...
	machinery
		portable_atmospherics
			canister
				...
				var
					list
						canister_color //variable that stores colours
				var
					list
						decals //list that stores the decals
				...
				var
					temperature_resistance = 1000 + T0C
				volume = 1000
				use_power = 0
				interact_offline = 1
				var
			...
			canister
				proc
					init_data_vars()
						...
						var
							list
								L = canister_icon_container.possibledecals
						for(i=1;i<=L.len;i++)
							var
								list
									LL = L[i]
...
obj
	machinery
		portable_atmospherics
			canister
				proc
					is_a_color(var/inputVar, var/checkColor = "all")
			...
			canister
				process()
					...
					if(valve_open)
						...
						var
							pressure_delta = min(release_pressure - env_pressure, (air_contents.return_pressure() - env_pressure)/2)
				...
				attackby(var/obj/item/weapon/W as obj, var/mob/user as mob, params)
								...
								obj
									item
										stack
											sheet
												metal
													M = new /obj/item/stack/sheet/metal(src.loc)
							M.amount = 3
							qdel(src)
						return
					...
					if(istype(user, /mob/living/silicon/robot) && istype(W, /obj/item/weapon/tank/jetpack))
						var
							datum
								gas_mixture
									thejetpack = W:air_contents
						var
							env_pressure = thejetpack.return_pressure()
						var
							pressure_delta = min(10*ONE_ATMOSPHERE - env_pressure, (air_contents.return_pressure() - env_pressure)/2)
						//Can not have a pressure delta that would cause environment pressure > tank pressure
						var
							transfer_moles = 0
						if((air_contents.temperature > 0) && (pressure_delta > 0))
			...
			canister
				Topic(href, href_list)
						...
						if(valve_open)
							...
							if(holding)
								...
								var
									datum
										gas
											sleeping_agent = locate(/datum/gas/sleeping_agent) in air_contents.trace_gases
					...
					if(href_list["rename"])
						if(can_label)
							var
								T = sanitize(copytext(input("Choose canister label", "Name", name) as text|null,1,MAX_NAME_LEN))
						...
						New()
							...
							var
								datum
									gas
										sleeping_agent
											trace_gas = air_contents.trace_gases[1]
