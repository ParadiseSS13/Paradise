...
/obj/machinery/portable_atmospherics/canister
	...
	var/list/item_color //variable that stores colours
...
/obj/machinery/portable_atmospherics/canister/proc/init_data_vars()
	...
	for(var/C in colorcontainer)
		...
		if(!(item_color[C]) || (item_color[C] == "none"))
...
/obj/machinery/portable_atmospherics/canister/proc/check_change()
	...
	if(list2params(oldcolor) != list2params(item_color))
		update_flag |= 64
		oldcolor = item_color.Copy()
...
*/
	...
	if (src.destroyed)
		src.overlays = 0
		src.icon_state = text("[]-1", src.item_color["prim"])//yes, I KNOW the colours don't reflect when the can's borked, whatever.

	if(icon_state != src.item_color["prim"])
		icon_state = src.item_color["prim"]
	...
	for(var/C in item_color)
		if(C == "prim") continue
		if(item_color[C] == "none") continue
		overlays.Add(item_color[C])
