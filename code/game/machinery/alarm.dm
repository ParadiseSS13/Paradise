...
datum
	tlv
		proc
			get_danger_level(curval as num)
	...
	machinery
		alarm
				...
				datum
					wires
						alarm
							wires = null
			var
				wiresexposed = 0 // If it's been screwdrivered open.
			var
				aidisabled = 0
			var
				AAlarmwires = 31
			...
			var
				danger_level = 0
			var
				alarmActivated = 0 // Manually activated (independent from danger level)
...
obj
	machinery
		alarm
			proc
				apply_preset(var/no_cycle_after=0)
					// Propogate settings.
	...
	machinery
		alarm
			process()
				...
				var
					GET_PP = R_IDEAL_GAS_EQUATION*environment.temperature/environment.volume
				...
				cur_tlv = TLV["oxygen"]
				var
					oxygen_dangerlevel = cur_tlv.get_danger_level(environment.oxygen*GET_PP)

				cur_tlv = TLV["nitrogen"]
				var
					nitrogen_dangerlevel = cur_tlv.get_danger_level(environment.nitrogen*GET_PP)

				cur_tlv = TLV["carbon dioxide"]
				var
					co2_dangerlevel = cur_tlv.get_danger_level(environment.carbon_dioxide*GET_PP)

				cur_tlv = TLV["plasma"]
				var
					plasma_dangerlevel = cur_tlv.get_danger_level(environment.toxins*GET_PP)
				...
				for(var/datum/gas/G in environment.trace_gases)
					other_moles+=G.moles
				var
					other_dangerlevel = cur_tlv.get_danger_level(other_moles*GET_PP)
...
obj
	machinery
		alarm
			proc
				handle_heating_cooling(var/datum/gas_mixture/environment, var/datum/tlv/cur_tlv, var/turf/simulated/location)
					cur_tlv = TLV["temperature"]
					...
					if(environment.temperature < target_temperature - 2 || environment.temperature > target_temperature + 2 || regulating_temperature)
						...
						if(!cur_tlv.get_danger_level(target_temperature))
							...
							var
								energy_used = max( abs( heat_capacity*(gas.temperature - target_temperature) ), MAX_ENERGY_CHANGE)
			...
			proc
				refresh_all()
					for(var/id_tag in alarm_area.air_vent_names)
						var
							list
								I = alarm_area.air_vent_info[id_tag]
					...
					for(var/id_tag in alarm_area.air_scrub_names)
						var
							list
								I = alarm_area.air_scrub_info[id_tag]
						if(I && I["timestamp"]+AALARM_REPORT_TIMEOUT/2 > world.time)
							continue
						send_signal(id_tag, list("status") )
...
obj
	machinery
		alarm
			proc
				send_signal(var/target, var/list/command)//sends signal 'command' to 'target'. Returns 0 if no radio connection, 1 otherwise
					if(!radio_connection)
						return 0
...
obj
	machinery
		alarm
			proc
				apply_danger_level(var/new_danger_level)
			...
			proc
				ui_air_status()
					...
					var
						datum
							gas_mixture
								environment = location.return_air()
					var
						total = environment.oxygen + environment.nitrogen + environment.carbon_dioxide + environment.toxins
					if(total==0)
						return null
					...
					var
						oxygen_dangerlevel = cur_tlv.get_danger_level(environment.oxygen*GET_PP)
					var
						oxygen_percent = round(environment.oxygen / total * 100, 2)

					cur_tlv = TLV["nitrogen"]
					var
						nitrogen_dangerlevel = cur_tlv.get_danger_level(environment.nitrogen*GET_PP)
					var
						nitrogen_percent = round(environment.nitrogen / total * 100, 2)

					cur_tlv = TLV["carbon dioxide"]
					var
						co2_dangerlevel = cur_tlv.get_danger_level(environment.carbon_dioxide*GET_PP)
					var
						co2_percent = round(environment.carbon_dioxide / total * 100, 2)

					cur_tlv = TLV["plasma"]
					var
						plasma_dangerlevel = cur_tlv.get_danger_level(environment.toxins*GET_PP)
					var
						plasma_percent = round(environment.toxins / total * 100, 2)
...
obj
	machinery
		alarm
			proc
				get_nano_data(mob/user, href_list)
					var
						data[0]
					...
					if(alarm_area.air_vent_names.len)
						for(var/id_tag in alarm_area.air_vent_names)
							var
								vent_info[0]
							var
								long_name = alarm_area.air_vent_names[id_tag]
							var
								list
									vent_data = alarm_area.air_vent_info[id_tag]
							if(!vent_data)
								continue
					...
					if(alarm_area.air_scrub_names.len)
						for(var/id_tag in alarm_area.air_scrub_names)
							var
								long_name = alarm_area.air_scrub_names[id_tag]
							var
								list
									scrubber_data = alarm_area.air_scrub_info[id_tag]
							if(!scrubber_data)
								continue
...
obj
	machinery
		alarm
			proc
				get_nano_data_console(mob/user)
					var
						data[0]
					data["name"] = sanitize(name)
					data["ref"] = "\ref[src]"
...
obj
	machinery
		alarm
			ui_interact(mob/user, ui_key = "main", datum/nanoui/ui = null, force_open = 1, var/master_ui = null, var/datum/topic_state/state = default_state)
				var
					list
						href = state.href_list(user)
				var
					remote_connection = 0
				var
					remote_access = 0
...
obj
	machinery
		alarm
			proc
				is_authenticated(var/mob/user, href_list)
...
obj
	machinery
		alarm
			proc
				is_locked(mob/user as mob, href_list)
...
obj
	machinery
		alarm
			CanUseTopic(var/mob/user, var/datum/topic_state/state, var/href_list = list())
				if(buildstage != 2)
					return STATUS_CLOSE
...
obj
	machinery
		alarm
			Topic(href, href_list, var/nowindow = 0, var/datum/topic_state/state)
				if(..(href, href_list, nowindow, state))
					return 1

				var
					state_href = state.href_list(usr)

				if(href_list["rcon"])
					var
						attempted_rcon_setting = text2num(href_list["rcon"])
					...
					switch(href_list["command"])
						if( "power",
							...
							if(href_list["val"])
								...
								var
									newval = input("Enter new value") as num|null
								if(isnull(newval))
									return
						...
						if("set_threshold")
							var
								env = href_list["env"]
							var
								varname = href_list["var"]
							var
								datum
									tlv
										tlv = TLV[env]
							var
								newval = input("Enter [varname] for [env]", "Alarm triggers", tlv.vars[varname]) as num|null
					...
					var
						datum
							tlv
								selected = TLV["temperature"]
					var
						max_temperature = selected.max1 >= 0 ? min(selected.max1, MAX_TEMPERATURE) : max(selected.max1, MAX_TEMPERATURE)
					var
						min_temperature = max(selected.min1, MIN_TEMPERATURE)
					var
						max_temperature_c = max_temperature - T0C
					var
						min_temperature_c = min_temperature - T0C
					var
						input_temperature = input("What temperature would you like the system to maintain? (Capped between [min_temperature_c]C and [max_temperature_c]C)", "Thermostat Controls") as num|null
					...
					var
						obj
							item
								stack
									cable_coil
										new_coil = new /obj/item/stack/cable_coil()
					new_coil.amount = 5
