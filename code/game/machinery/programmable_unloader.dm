...
obj
	machinery
		programmable
			...
			var
				debug = 0	// When set, this WILL spam people around the machine.
				...
				list
					profiles = list(new/datum/cargoprofile/cargo(),new/datum/cargoprofile/boxes(),new/datum/cargoprofile/supplies(),
											new
												datum
													cargoprofile
														exotics(),new/datum/cargoprofile/tools(),new/datum/cargoprofile/weapons(),
											new
												datum
													cargoprofile
														pressure(),new/datum/cargoprofile/chemical(),
											new
												datum
													cargoprofile
														food(),new/datum/cargoprofile/clothing(),new/datum/cargoprofile/trash())
			var
				list
					overrides = list(new/datum/cargoprofile/cargo/unload(),new/datum/cargoprofile/in_stacker())
			var
				list
					emag_overrides = list(new/datum/cargoprofile/people(),new/datum/cargoprofile/unary/shredder(),new/datum/cargoprofile/unary/trainer())
			var
				list
					types = list()
			...
			use_power = 1
			var
				sleep = 0 // When set, the machine will skip the next however-many updates (to avoid spam)
			var
				open = 0
			var
				circuit_removed = 0
		...
		programmable
			process()
				...
				for(var/mob/M in input.contents)
					for(var/datum/cargoprofile/p in overrides + default)
						if(p.enabled && p.mobcheck && p.contains(M))
							var
								done = p.inlet_reaction(M,input,workmax - work)
							if(done)
								work += done
								if(src.debug)
				...
				for(var/obj/A in input.contents)// I fully expect this to cause unreasonable lag
					...
					var
						done = 0 // work done        - testing only
					var
						aname    // target item name - testing only
			...
			circuitboard
				programmable
					...
					var
						datum
							cargoprofile
								default = new/datum/cargoprofile()
					var
						list
							profiles = list()
					var
						...
						resetlists()
							profiles = list(new/datum/cargoprofile/cargo(),new/datum/cargoprofile/boxes(),new/datum/cargoprofile/supplies(),
												new
													datum
														cargoprofile
															exotics(),new/datum/cargoprofile/tools(),new/datum/cargoprofile/weapons(),new/datum/cargoprofile/finished(),
												new
													datum
														cargoprofile
															pressure(),new/datum/cargoprofile/pressure/full(),new/datum/cargoprofile/pressure/empty(),
												new
													datum
														cargoprofile
															chemical(),new/datum/cargoprofile/organics(),new/datum/cargoprofile/food(),
												new
													datum
														cargoprofile
															clothing(),new/datum/cargoprofile/trash())
							overrides = list(new/datum/cargoprofile/cargo/unload(),new/datum/cargoprofile/in_stacker(),
												new
													datum
														cargoprofile
															unary
																stacker(),new/datum/cargoprofile/unary/trainer())
					...
					proc
						format(var/datum/cargoprofile/P,var/level)
					...
					Topic(href, href_list)
						...
						var
							id = href_list["id"]
						var
							level = text2num(href_list["level"])
						switch(href_list["operation"])
							if("promote")
								if(level == 0)
