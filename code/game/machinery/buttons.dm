	...
	machinery
		driver_button
			...
			var
				frequency = 0
			var
				logic_id_tag = "default"					//Defines the ID tag to send logic signals to, so you don't have to unlink from doors and stuff
			var
				logic_connect = 0							//Set this to allow the button to send out logic signals when pressed in addition to normal stuff

obj
	machinery
		driver_button
			New(turf/loc, var/w_dir=null)
				..()
				switch(w_dir)
					if(NORTH)
						pixel_y = 25
...
obj
	machinery
		driver_button
			attackby(obj/item/weapon/W, mob/user as mob, params)
