...
obj
	machinery
		recharger
			attackby(obj/item/weapon/G as obj, mob/user as mob, params)
...
obj
	machinery
		recharger
			update_icon()	//we have an update_icon() in addition to the stuff in process to make it feel a tiny bit snappier.
				if(charging)
					icon_state = icon_state_charging
				else
