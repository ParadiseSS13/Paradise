...
var
	datum
		feed_network
			news_network = new /datum/feed_network     //The global news-network, which is coincidentally a global list.
...
var
	list
		obj
			machinery
				newscaster
					allCasters = list() //Global list that will contain reference to all newscasters in existence.
...
obj
	machinery
		newscaster
			...
			var
				isbroken = 0  //1 if someone banged it with something heavy
			var
				ispowered = 1 //starts powered, changes with power_change()
			//var/list/datum/feed_channel/channel_list = list() //This list will contain the names of the feed channels. Each name will refer to a data region where the messages of the feed channels are stored.
			//OBSOLETE: We're now using a global news network
			var
				screen = 0                  //Or maybe I'll make it into a list within a list afterwards... whichever I prefer, go fuck yourselves :3
			...
			var
				securityCaster = 0
					...
					// 1 = the opposite
			var
				unit_no = 0 //Each newscaster has a unit number
			...
			var
				alert = 0
					...
					// 1 = there has
			var
				scanned_user = "Unknown" //Will contain the name of the person who currently uses the newscaster
			var
				msg = "";                //Feed message
			var
				obj
					item
						weapon
							photo
								photo = null
			var
				channel_name = ""; //the feed channel which will be receiving the feed, or being created
			var
				c_locked=0;        //Will our new channel be locked to public submissions?
			var
				hitstaken = 0      //Death at 3 hits from an item with force>=15
			var
				datum
					feed_channel
...
obj
	machinery
		newscaster
			attack_hand(mob/user as mob)            //########### THE MAIN BEEF IS HERE! And in the proc below this...############
				...
				if(istype(user, /mob/living/carbon/human) || istype(user,/mob/living/silicon) )
					...
					switch(screen)
						...
						if(8)
							var
								total_num=length(news_network.network_channels)
							var
		...
		newscaster
			Topic(href, href_list)
				...
				if(href_list["set_channel_name"])
					...
					var
						input_param = text2num(href_list["submit_wanted"])
					...
					var
						datum
							feed_channel
								FC = locate(href_list["censor_channel_author"])
					...
					var
						datum
							feed_message
								MSG = locate(href_list["censor_channel_story_author"])
					...
					var
						datum
							feed_message
								MSG = locate(href_list["censor_channel_story_body"])
					if(MSG.is_admin_message)
						alert("This channel was created by a Nanotrasen Officer. You cannot censor it.","Ok")
						return
					if(MSG.img != null)
					...
					var
						datum
							feed_channel
								FC = locate(href_list["pick_d_notice"])
					...
					var
						datum
							feed_channel
								FC = locate(href_list["toggle_d_notice"])
					if(FC.is_admin_channel)
					...
					var
						datum
							feed_channel
								FC = locate(href_list["show_channel"])
					src.viewing_channel = FC
					src.screen = 9
					src.updateUsrDialog()
					...
					var
						datum
							feed_channel
								FC = locate(href_list["pick_censor_channel"])
					src.viewing_channel = FC
					src.screen = 12
					src.updateUsrDialog()
						...
						switch(screen)
							...
							if(1) // X channel pages inbetween.
								...
								var
									datum
										feed_channel
											C = src.news_content[src.curr_page]
				...
				attackby(obj/item/weapon/W as obj, mob/user as mob, params)
					if(istype(W, /obj/item/weapon/pen))
						if(src.scribble_page == src.curr_page)
							...
							var
								s = strip_html( input(user, "Write something", "Newspaper", "") )
...
obj
	machinery
		newscaster
			proc
				scan_user(mob/living/user as mob)
					...
					var
						obj
							item
								weapon
									newspaper
										NEWSPAPER = new /obj/item/weapon/newspaper
					for(var/datum/feed_channel/FC in news_network.network_channels)
...
obj
	machinery
		newscaster
			proc
				newsAlert(var/news_call)   //This isn't Agouri's work, for it is ugly and vile.
					var
						turf
							T = get_turf(src)                      //Who the fuck uses spawn(600) anyway, jesus christ
					if(news_call)
