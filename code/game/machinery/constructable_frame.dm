...
obj
	machinery
		constructable_frame //Made into a seperate type to make future revisions easier.
			...
			var
				list
					req_component_names = null // user-friendly names of components
			var
				state = 1
			...
			proc
				update_req_desc()
					...
					for(var/i = 1 to req_components.len)
						var
							tname = req_components[i]
						var
							amt = req_components[tname]
						if(amt == 0)
							continue
						var
							use_and = i == req_components.len
								...
								var
									obj
										item
											stack
												cable_coil
													A = new /obj/item/stack/cable_coil(src.loc,5)
								A.amount = 5
							...
							if(istype(P, /obj/item/weapon/screwdriver))
								...
								if(component_check)
									...
									var
										obj
											machinery
												new_machine = new src.circuit.build_path(src.loc)
													...
													item
														stack
															cable_coil
																CP = P
											var
												camt = min(CP.amount, req_components[I])
											var
												obj
													item
														stack
															cable_coil
																CC = new /obj/item/stack/cable_coil(src)
											CC.amount = camt
											CC.update_icon()
											CP.use(camt)
											components += CC
		...
		weapon
			circuitboard
				vendor
					...
					var
						list
							names_paths = list("Booze-O-Mat" = /obj/machinery/vending/boozeomat,
	...
	item
		weapon
			circuitboard
				vendor
					proc
						set_type(typepath, mob/user)
							var
								new_name = "Booze-O-Mat Vendor"
							if(!typepath)
				...
				logic_gate
					New()
						...
						if(build_path == /obj/machinery/logic_gate)			//If we spawn the base type board (determined by the base type machine as the build path), become a random gate board
							var
								new_path = names_paths[pick(names_paths)]
							set_type(new_path)
