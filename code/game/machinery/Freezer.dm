					...
					freezer
						attackby(obj/item/I, mob/user, params)
							...
							if(istype(I, /obj/item/weapon/wrench))
								...
								var
									list
										choices = list("West" = WEST, "East" = EAST, "South" = SOUTH, "North" = NORTH)
								var
									selected = input(user,"Select a direction for the connector.", "Connector Direction") in choices
				...
				cold_sink
					freezer
						Topic(href, href_list)
							...
							if(href_list["temp"])
								var
									amount = text2num(href_list["temp"])
								if(amount > 0)
								...
								obj
									item
										weapon
											circuitboard
												thermomachine
													H = new /obj/item/weapon/circuitboard/thermomachine(null)
