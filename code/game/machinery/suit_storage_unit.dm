			...
			var
				obj
					item
						clothing
							mask
								MASK = null  //All the stuff that's gonna be stored insiiiiiiiiiiiiiiiiiiide, nyoro~n
			var
				MASK_TYPE = null //Erro's idea on standarising SSUs whle keeping creation of other SSU types easy: Make a child SSU, name it something then set the TYPE vars to your desired suit output. New() should take it from there by itself.
			var
				isopen = 0
			var
				islocked = 0
...
obj
	machinery
		suit_storage_unit
			Topic(href, href_list) //I fucking HATE this proc
				if(..())
					return 1
...
obj
	machinery
		suit_storage_unit
			proc
				togglesafeties(mob/user as mob)
//	var/protected = 0
//	var/mob/living/carbon/human/H = user
...
obj
	machinery
		suit_storage_unit
			proc
				dispense_helmet(mob/user as mob)
					if(!src.HELMET)
				...
				start_UV(mob/user as mob)
					...
					for(i=0,i<4,i++)
						...
						if(src.OCCUPANT)
							if(src.issuperUV)
								var
									burndamage = rand(35,45)
								OCCUPANT.take_organ_damage(0,burndamage)
								OCCUPANT.emote("scream")
							else
								var
									burndamage = rand(10,15)
								OCCUPANT.take_organ_damage(0,burndamage)
								OCCUPANT.emote("scream")
...
obj
	machinery
		suit_storage_unit
			proc
				eject_occupant(mob/user as mob)
					if(src.islocked)
						return
...
obj
	machinery
		suit_cycler
			...
			var
				safeties = 1        // The cycler won't start with a living thing inside it unless safeties are off.
			var
				irradiating = 0     // If this is > 0, the cycler is decontaminating whatever is inside it.
			var
				radiation_level = 2 // 1 is removing germs, 2 is removing blood, 3 is removing plasma.
			var
				model_text = ""     // Some flavour text for the topic box.
			var
			...
			var
				const
					WIRE_EXTEND = 1 // Safeties
			var
				const
					WIRE_SCANID = 2 // Locked status
			var
				const
					WIRE_SHOCK = 3  // What it says on the tin.
			...
			var
				list
					species = list("Human","Skrell","Unathi","Tajaran")
...
/*	if(panel_open)
		...
		for(var/wiredesc in vendwires)
			var
				is_uncut = src.wires & APCWireColorToFlag[vendwires[wiredesc]]
			dat += "[wiredesc] wire: "
			if(!is_uncut)
		...
		suit_cycler
			Topic(href, href_list)
				if(href_list["eject_suit"])
					...
					var
						choice = input("Please select the target department paintjob.","Suit cycler",null) as null|anything in departments
					if(choice) target_department = choice
				else if(href_list["select_species"])
					var
						choice = input("Please select the target species configuration.","Suit cycler",null) as null|anything in species
...
/*	else if((href_list["cutwire"]) && (src.panel_open))
		var
			twire = text2num(href_list["cutwire"])
		...
		var
			twire = text2num(href_list["pulsewire"])
		...
		suit_cycler
			proc
				isWireColorCut(var/wireColor)
					var
						wireFlag = APCWireColorToFlag[wireColor]
			...
			proc
				cut(var/wireColor)
					var
						wireFlag = APCWireColorToFlag[wireColor]
					var
						wireIndex = APCWireColorToIndex[wireColor]
					src.wires &= ~wireFlag
					switch(wireIndex)
			...
			proc
				mend(var/wireColor)
					var
						wireFlag = APCWireColorToFlag[wireColor]
					var
						wireIndex = APCWireColorToIndex[wireColor] //not used in this function
					src.wires |= wireFlag
					switch(wireIndex)
						if(WIRE_SHOCK)
							src.electrified = 0
