...
obj
	item
		pipe
			New(loc, pipe_type, dir, obj/machinery/atmospherics/make_from)
				..()
				if(make_from)
...
obj
	item
		pipe
			proc
				update(var/obj/machinery/atmospherics/make_from)
					var
						list
							nlist = list( \
								"pipe", \
...
obj
	item
		pipe
			proc
				get_pdir() //endpoints for regular pipes
...
obj
	item
		pipe
			proc
				get_hdir() //endpoints for h/e pipes
