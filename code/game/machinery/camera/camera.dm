obj
	machinery
		camera
			...
			anchored = 1
			var
				start_active = 0 //If it ignores the random chance to start broken on round start
			var
				invuln = null
			var
				obj
			...
			var
				busy = 0
			var
				emped = 0  //Number of consecutive EMP's on this camera
...
obj
	machinery
		camera
			attack_alien(mob/living/carbon/alien/humanoid/user as mob)
				if(!istype(user))
					return
...
obj
	machinery
		camera
			attackby(W as obj, mob/living/user as mob, params)
				var
					msg = "<span class='notice'>You attach [W] into the assembly inner circuits.</span>"
				var
					msg2 = "<span class='notice'>The camera already has that upgrade!</span>"
				...
				datum
					data
						pda
							app
								notekeeper
									N = P.find_program(/datum/data/pda/app/notekeeper)
			if(N)
				itemname = P.name
				info = N.notehtml
...
obj
	machinery
		camera
			proc
				toggle_cam(mob/user, displaymessage = 1)
					status = !status
					if(can_use())
						cameranet.addCamera(src)
					else
...
obj
	machinery
		camera
			proc
				triggerCameraAlarm(var/duration = 0)
					alarm_on = 1
...
obj
	machinery
		camera
			proc
				weld(var/obj/item/weapon/weldingtool/WT, var/mob/user)
					if(busy)
						return 0
...
obj
	machinery
		camera
			portable
				process() //Updates whenever the camera is moved.
