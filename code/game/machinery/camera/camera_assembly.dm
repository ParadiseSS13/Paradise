			...
			camera_assembly
				...
				var
					list
						obj
							item
								possible_upgrades = list(/obj/item/device/assembly/prox_sensor, /obj/item/stack/sheet/mineral/plasma, /obj/item/device/analyzer)
				var
					list
						upgrades = list()
				var
					...
					switch(state)
						...
						if(3)
							...
							if(isscrewdriver(W))
								...
								var
									input = strip_html(input(usr, "Which networks would you like to connect this camera to? Seperate networks with a comma. No Spaces!\nFor example: SS13,Security,Secret ", "Set Network", "SS13"))
								if(!input)
								...
								var
									area
										camera_area = get_area(src)
								var
									temptag = "[sanitize(camera_area.name)] ([rand(1, 999)])"
								...
								for(var/i = 5; i >= 0; i -= 1)
									...
									if(i != 0)
										var
											confirm = alert(user, "Is this what you want? Chances Remaining: [i]", "Confirmation", "Yes", "No")
										if(confirm == "Yes")
											break
								return
...
obj
	item
		weapon
			camera_assembly
				proc
					weld(var/obj/item/weapon/weldingtool/WT, var/mob/living/user)
