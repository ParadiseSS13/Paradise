				...
				proc
					get_camera_list()
						...
						for(var/obj/machinery/camera/C in L)
							var
								list
									tempnetwork = C.network & src.network
							if(tempnetwork.len)
...
mob
	living
		silicon
			ai
				proc
					ai_camera_list(var/camera in get_camera_list())
...
mob
	living
		silicon
			ai
				proc
					ai_goto_location(loc in sorted_stored_locations())
						set category = "AI Commands"
...
mob
	living
		silicon
			ai
				proc
					ai_remove_location(loc in sorted_stored_locations())
						set category = "AI Commands"
			...
			ai
				proc
					trackable_mobs()
						...
						var
							list
								targets = sortList(track.humans) + sortList(track.others)
...
mob
	living
		silicon
			ai
				proc
					ai_camera_track(target_name in trackable_mobs())
						set category = "AI Commands"
...
mob
	living
		silicon
			ai
				proc
					ai_cancel_tracking(var/forced = 0)
						if(!cameraFollow)
							return
...
mob
	living
		silicon
			ai
				proc
					ai_actual_track(mob/living/target)
						if(!istype(target))
							return
						var
							mob
