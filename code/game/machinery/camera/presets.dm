...
obj
	machinery
		camera
			autoname
				var
					number = 0 //camera number in area
	...
	machinery
		camera
			proc
				isEmpProof()
					var
						O = locate(/obj/item/stack/sheet/mineral/plasma) in assembly.upgrades
					return O
	...
	machinery
		camera
			proc
				isXRay()
					var
						O = locate(/obj/item/device/analyzer) in assembly.upgrades
					return O
	...
	machinery
		camera
			proc
				isMotion()
					var
						O = locate(/obj/item/device/assembly/prox_sensor) in assembly.upgrades
					return O
