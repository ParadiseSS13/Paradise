...
obj
	machinery
		computer
			security
				proc
					drawmap(var/mob/user as mob)

						var
							icx = round(world.maxx/16) + 1
						var
							icy = round(world.maxy/16) + 1

						var
							xoff = round( (icx*16-world.maxx)-2)
						var
							yoff = round( (icy*16-world.maxy)-2)
	...
	for(var/wx = 1 ; wx <= world.maxx; wx++)
		...
		for(var/wy = 1; wy <= world.maxy; wy++)
			...
			if(!colour2 && !T.density)
				...
				var
					t1 = turf_total / MOLES_CELLSTANDARD * 150
			...
			var
				ix = round((wx*2+xoff)/32)
			var
				iy = round((wy*2+yoff)/32)
...
//			to_chat(world, "trying [ix],[iy] : [ix+icx*iy]")
			var
				icon
					I = imap[1+(ix + icx*iy)*2]
			var
				icon
					I2 = imap[2+(ix + icx*iy)*2]
				...
				switch("[T.type]")
					...
					if("/turf/simulated/floor", "/turf/simulated/floor/engine")
						...
						var
							turf_total = environment.total_moles()
						var
							t1 = turf_total / MOLES_CELLSTANDARD * 175
