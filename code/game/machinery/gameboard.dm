				...
				var
					datum
						asset
							simple
								chess
									assets = get_asset_datum(/datum/asset/simple/chess)
				assets.send(user)

				var
					datum
						browser
							popup = new(user, "SpessChess", name, 500, 800, src)
...
obj
	machinery
		gameboard
			proc
				close_game() //yes, shamelessly copied over from arcade_base
					in_use = 0
