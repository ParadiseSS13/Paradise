			...
			var
				mob
					living
						occupant
			var
				heal_level = 90 //The clone is released once its health reaches this level.
			var
				locked = 0
			var
				obj
					machinery
						computer
							cloning
								connected = null //So we remember the connected clone machine.
			var
				mess = 0 //Need to clean out it if it's full of exploded clone.
			var
				attempting = 0 //One clone attempt at a time thanks
			var
				eject_wait = 0 //Don't eject them as soon as they are created fuckkk
			var
				biomass = 0
			var
				speed_coeff
				...
				data
					monkey
						...
						New()
							...
							var
								list
									new_SE=list(0x098,0x3E8,0x403,0x44C,0x39F,0x4B0,0x59D,0x514,0x5FC,0x578,0x5DC,0x640,0x6A4)
		...
		clonepod
			attack_hand(mob/user as mob)
				...
				if((!isnull(occupant)) && (occupant.stat != 2))
					var
						completion = (100 * ((occupant.health + 100) / (heal_level + 100)))
...
obj
	machinery
		clonepod
			proc
				growclone(var/datum/dna2/record/R)
