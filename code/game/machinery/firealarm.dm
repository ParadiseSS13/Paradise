											...
											item
												weapon
													firealarm_electronics
														circuit = new /obj/item/weapon/firealarm_electronics()
...
obj
	machinery
		firealarm
			process()//Note: this processing was mostly phased out due to other code, and only runs when needed
...
/*
	...
	if(ishuman(user) || issilicon(user) || isobserver(user))
		...
		var
			second = round(src.time) % 60
		var
			minute = (round(src.time) - second) / 60
		var
