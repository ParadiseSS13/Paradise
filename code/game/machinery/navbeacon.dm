...
obj
	machinery
		navbeacon
			...
			var
				open = 0		// true if cover is open
			var
				locked = 1		// true if controls are locked
			var
				location = ""	// location response text
			var
				list
					codes		// assoc. list of transponder codes
			var
		...
		navbeacon
			proc
				set_codes()
					...
					var
						list
							entries = splittext(codes_txt, ";")	// entries are separated by semicolons

					for(var/e in entries)
						var
							index = findtext(e, "=")		// format is "key=value"
						if(index)
							var
								key = copytext(e, 1, index)
							var
								val = copytext(e, index+1)
							codes[key] = val
						else
							codes[e] = "1"
...
var
	datum
		browser
			popup = new(user, "navbeacon", "Navigation Beacon", 300, 400)
				popup.set_content(t)
				popup.open()
				return
			...
			Topic(href, href_list)
				...
				if(open && !locked)
					...
					if(href_list["locedit"])
						var
							newloc = copytext(sanitize(input("Enter New Location", "Navigation Beacon", location) as text|null),1,MAX_MESSAGE_LEN)
						if(newloc)
							location = newloc
							updateDialog()
						...
						var
							newkey = stripped_input(usr, "Enter Transponder Code Key", "Navigation Beacon", codekey)
						if(!newkey)
							return

						var
							codeval = codes[codekey]
						var
							newval = stripped_input(usr, "Enter Transponder Code Value", "Navigation Beacon", codeval)
						if(!newval)
							newval = codekey
							return
						...
						var
							newkey = stripped_input(usr, "Enter New Transponder Code Key", "Navigation Beacon")
						if(!newkey)
							return

						var
							newval = stripped_input(usr, "Enter New Transponder Code Value", "Navigation Beacon")
						if(!newval)
							newval = "1"
							return
