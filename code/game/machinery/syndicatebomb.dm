	...
	machinery
		syndicatebomb
			...
			var
				timer = 120
			var
				open_panel = 0 	//are the wires exposed?
			var
				active = 0		//is the bomb counting down?
			var
				defused = 0		//is the bomb capable of exploding?
			var
				obj
					item
						weapon
		...
		syndicatebomb
			process()
				if(active && !defused && (timer > 0)) 	//Tick Tock
					var
						volume = (timer <= 20 ? 40 : 10) // Tick louder when the bomb is closer to being detonated.
...
obj
	machinery
		syndicatebomb
			proc
				settings(var/mob/user)
					var
						newtime = input(user, "Please set the timer.", "Timer", "[timer]") as num
...
obj
	item
		weapon
			bombcore
				ex_act(severity) //Little boom can chain a big boom
					src.detonate()
...
obj
	item
		weapon
			bombcore
				badmin
					defuse() //because we wouldn't want them being harvested by players
						var
							obj
								machinery
									syndicatebomb
		...
		weapon
			bombcore
				badmin
					summon/
						var
							summon_path = /obj/item/weapon/reagent_containers/food/snacks/cookie
						var
							amt_summon = 1
