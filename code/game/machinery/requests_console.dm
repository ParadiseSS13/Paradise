...
var
	req_console_assistance = list()
var
	req_console_supplies = list()
var
	req_console_information = list()
var
	list
		obj
			machinery
...
obj
	machinery
		requests_console
			...
			var
				department = "Unknown" //The list of all departments on the station (Determined from this variable on each unit) Set this to the same thing if you want several consoles in one department
			var
				list
					message_log = list() //List of all messages
			var
				departmentType = 0 		//Bitflag. Zero is reply-only. Map currently uses raw numbers instead of defines.
			var
			...
			var
				screen = RCS_MAINMENU
			var
				silent = 0 // set to 1 for it not to beep all the time
//	var/hackState = 0
		// 0 = not hacked
		// 1 = hacked
var
	announcementConsole = 0
		...
		var
			open = 0 // 1 if open
		var
			announceAuth = 0 //Will be set to 1 when you authenticate yourself for announcements
		var
			msgVerified = "" //Will contain the name of the person who varified it
		var
			msgStamped = "" //If a message is stamped, this will contain the stamp name
		var
			message = "";
		var
			recipient = ""; //the department which will be receiving the message
		var
			priority = -1 ; //Priority of the message being sent
		light_range = 0
		var
			datum
				announcement
		...
		requests_console
			Topic(href, href_list)
				...
				if(reject_bad_text(href_list["write"]))
					...
					var
						new_message = sanitize(input("Write your message:", "Awaiting Input", ""))
					if(new_message)
						message = new_message
						screen = RCS_MESSAUTH
				...
				if(href_list["setScreen"])
					var
						tempScreen = text2num(href_list["setScreen"])
...
obj
	machinery
		requests_console
			proc
				createMessage(source, title, message, priority)
					var
						linkedSender
