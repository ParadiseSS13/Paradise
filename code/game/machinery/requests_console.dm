...
/obj/machinery/requests_console/ui_act(action, list/params)
	...
	switch(action)
		...
		if("printLabel")
			...
			shipping_log.Add(list(list("Shipping Label printed for [ship_tag_name]", "[msgVerified]")))
			reset_message(TRUE)
...
/obj/machinery/requests_console/proc/createMessage(source, title, message, priority)
	...
	switch(priority)
		if(RQ_HIGHPRIORITY) // High
			message_log.Add(list(list("High Priority - From: [linkedSender]") + message))
		else // Normal
			message_log.Add(list(list("From: [linkedSender]") + message))
	set_light(2)
