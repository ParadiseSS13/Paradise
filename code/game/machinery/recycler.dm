...
obj
	machinery
		recycler
			...
			density = 1
			var
				safety_mode = 0 // Temporality stops the machine if it detects a mob
			var
				grinding = 0
			var
				icon_name = "grinder-o"
	...
	machinery
		recycler
			update_icon()
				..()
				var
					is_powered = !(stat & (BROKEN|NOPOWER))
				if(safety_mode)
					is_powered = 0
		...
		recycler
			proc
				recycle(obj/item/I, sound = 1)
					...
					var
						material_amount = materials.get_item_material_amount(I)
					if(!material_amount)
						qdel(I)
						return
