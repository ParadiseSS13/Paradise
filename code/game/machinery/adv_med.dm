		...
		bodyscanner
			attackby(var/obj/item/weapon/G as obj, var/mob/user as mob)
				...
				if(istype(G, /obj/item/weapon/grab))
					...
					var
						mob
							M = TYPECAST_YOUR_SHIT.affecting
					if(M.abiotic())
...
obj
	machinery
		bodyscanner
			MouseDrop_T(mob/living/carbon/human/O, mob/user as mob)
				if(!istype(O))
					return 0 //not a mob
	...
	machinery
		body_scanconsole
			...
			var
				printing_text = null
			var
				known_implants = list(/obj/item/weapon/implant/chem, /obj/item/weapon/implant/death_alarm, /obj/item/weapon/implant/loyalty, /obj/item/weapon/implant/tracking)
			...
			proc
				findscanner()
					...
					for(dir in list(NORTH,EAST,SOUTH,WEST))
						...
						var
							temp = locate(/obj/machinery/bodyscanner, get_step(src, dir))
						if(!isnull(temp))
							bodyscannernew = temp
				...
				generate_printing_text()
					...
					if(connected)
						...
						if(istype(occupant)) //is there REALLY someone in there?
							...
							if(occupant.vessel)
								var
									blood_volume = round(occupant.vessel.get_reagent_amount("blood"))
								var
									blood_percent =  blood_volume / 560
								blood_percent *= 100
