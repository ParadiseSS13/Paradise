...
/obj/machinery/body_scanconsole/ui_data(mob/user, datum/topic_state/state)
	...
	if(connected.occupant)
		...
		occupantData["unconscious"] = H.AmountUnconscious()
		occupantData["paralysisSeconds"] = round(H.AmountUnconscious() / 4)
...
/obj/machinery/body_scanconsole/proc/generate_printing_text()
	...
	if(connected)
		...
		if(istype(occupant)) //is there REALLY someone in there?
			...
			dat += "Unconscious Summary %: [occupant.AmountUnconscious()] ([round(occupant.AmountUnconscious() / 4)] seconds left!)<br>"
