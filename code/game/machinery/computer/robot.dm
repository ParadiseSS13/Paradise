...
obj
	machinery
		computer
			robotics
				proc
					is_authenticated(var/mob/user as mob)
				...
				Topic(href, href_list)
					...
					if(href_list["detonate"])
						var
							mob
								living
									silicon
										robot
											target = get_cyborg_by_name(href_list["detonate"])
						...
						if((isrobot(user) && user != target) || !is_authenticated(user))
							...
							return
						var
							choice = input("Really detonate [target.name]?") in list ("Yes", "No")
						if(choice != "Yes")
							return
						if(!target || !istype(target))
							return
					...
					// Locks or unlocks the cyborg
						...
						var
							mob
								living
									silicon
										robot
											target = get_cyborg_by_name(href_list["lockdown"])
						if(!target || !istype(target))
							return
						...
						var
							choice = input("Really [target.lockcharge ? "unlock" : "lockdown"] [target.name] ?") in list ("Yes", "No")
						if(choice != "Yes")
							return
					...
					// Remotely hacks the cyborg. Only antag AIs can do this and only to linked cyborgs.
						...
						var
							mob
								living
									silicon
										robot
											target = get_cyborg_by_name(href_list["hack"])
						if(!target || !istype(target))
							return
						...
						var
							choice = input("Really hack [target.name]? This cannot be undone.") in list("Yes", "No")
						if(choice != "Yes")
							return
