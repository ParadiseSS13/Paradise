	...
	machinery
		computer
			card
				...
				var
					max_relative_positions = 30 //30%: Seems reasonable, limit of 6 @ 20 players
...
obj
	machinery
		computer
			card
				proc
					format_card_skins(list/card_skins)
						var
							list
								formatted = list()
...
obj
	machinery
		computer
			card
				attackby(obj/item/weapon/card/id/id_card, mob/user, params)
					if(!istype(id_card))
						return ..()
...
obj
	machinery
		computer
			card
				proc
					can_open_job(datum/job/job)
						if(job)
							if(!job_blacklisted(job))
								if((job.total_positions <= player_list.len * (max_relative_positions / 100)))
									var
										delta = (world.time / 10) - time_last_changed_position
		...
		computer
			card
				ui_interact(mob/user, ui_key="main", var/datum/nanoui/ui = null, var/force_open = 1)
					...
					var
						time_to_wait = round(change_position_cooldown - ((world.time / 10) - time_last_changed_position), 1)
					var
						mins = round(time_to_wait / 60)
					var
						seconds = time_to_wait - (60*mins)
					...
					switch(href_list["choice"])
						...
						if("access")
							if(href_list["allowed"])
								if(is_authenticated(usr))
									var
										access_type = text2num(href_list["access_target"])
									var
										access_allowed = text2num(href_list["allowed"])
						...
						if("assign")
							if(is_authenticated(usr) && modify)
								...
								if(t1 == "Custom")
									var
										temp_t = sanitize(copytext(input("Enter a custom job assignment.","Assignment"),1,MAX_MESSAGE_LEN))
						...
						if("reg")
							if(is_authenticated(usr))
								...
								if((modify == t2 && (in_range(src, usr) || (istype(usr, /mob/living/silicon))) && istype(loc, /turf)))
									var
										temp_name = reject_bad_name(href_list["reg"])
									if(temp_name)
						...
						if("account")
							if(is_authenticated(usr))
								...
								if((modify == t2 && (in_range(src, usr) || (istype(usr, /mob/living/silicon))) && istype(loc, /turf)))
									var
										account_num = text2num(href_list["account"])
						...
						if("make_job_available")
							...
							if(is_authenticated(usr))
								...
								var
									datum
										job
											j = job_master.GetJob(edit_job_target)
								if(!j)
									return 0
								if(can_open_job(j) != 1)
									return 0
