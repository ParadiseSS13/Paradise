...
var
	global
		list
			priority_air_alarms = list()
var
	global
		list
			minor_air_alarms = list()
		...
		computer
			atmos_alert
				update_icon()
					var
						list
							alarms = atmosphere_alarm.major_alarms()
					if(alarms.len)
						icon_screen = "alert:2"
					else
				...
				Topic(href, href_list)
					...
					if(href_list["clear_alarm"])
						var
							datum
								alarm
									alarm = locate(href_list["clear_alarm"]) in atmosphere_alarm.alarms
						if(alarm)
							for(var/datum/alarm_source/alarm_source in alarm.sources)
								var
									obj
										machinery
											alarm
												air_alarm = alarm_source.source
								if(istype(air_alarm))
									var
										list
											new_ref = list("atmos_reset" = 1)
