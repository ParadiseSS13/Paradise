					...
					if(href_list["switchTo"])
						...
						var
							obj
								machinery
									camera
										C = locate(href_list["switchTo"]) in cameranet.cameras
						if(!C)
							return 1
						...
						var
							net = href_list["activate"]	// Network to be enabled or disabled.
						var
							active = href_list["active"] // Is the network currently active.
...
obj
	machinery
		computer
			security
				proc
					can_access_camera(var/obj/machinery/camera/C)
						var
							list
								shared_networks = src.network & C.network
...
obj
	machinery
		computer
			security
				proc
					switch_to_camera(var/mob/user, var/obj/machinery/camera/C)
...
obj
	machinery
		computer
			security
				proc
					set_current(var/obj/machinery/camera/C)
						if(current == C)
							return
...
obj
	machinery
		computer
			security
				proc
					jump_on_click(var/mob/user,var/A)
						if(user.machine != src)
							return
						var
							obj
