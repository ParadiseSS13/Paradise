		...
		computer
			communications
				...
				var
					aicurrmsg = 0
				var
					menu_state = COMM_SCREEN_MAIN
				var
					ai_menu_state = COMM_SCREEN_MAIN
				var
					message_cooldown = 0
				var
...
obj
	machinery
		computer
			communications
				proc
					is_authenticated(var/mob/user, var/message = 1)
						if(authenticated == 2)
							return 2
							...
							obj
								item
									weapon
										card
											id
												I = (card && card.GetID())||M.wear_id||M.wear_pda
					...
					switch(href_list["operation"])
								...
								obj
									item
										weapon
											card
												id
													I = (card && card.GetID())||L.wear_id||L.wear_pda
							if(istype(I, /obj/item/device/pda))
								var
									obj
										item
						...
						if("announce")
							if(is_authenticated(usr) == 2)
								if(message_cooldown)
									...
									return
								var
									input = input(usr, "Please write a message to announce to the station crew.", "Priority Announcement")
						...
						if("cancelshuttle")
							if(isAI(usr) || isrobot(usr))
								...
								return 1
							var
								response = alert("Are you sure you wish to recall the shuttle?", "Confirm", "Yes", "No")
							if(response == "Yes")
						...
						if("delmessage")
							...
							var
								response = alert("Are you sure you wish to delete this message?", "Confirm", "Yes", "No")
							if(response == "Yes")
								if(src.currmsg)
									var
										id = getCurrentMessage()
									var
										title = src.messagetitle[id]
									var
										text  = src.messagetext[id]
						...
						if("nukerequest")
							if(is_authenticated(usr) == 2)
								if(centcomm_message_cooldown)
									...
									return
								var
									input = stripped_input(usr, "Please enter the reason for requesting the nuclear self-destruct codes. Misuse of the nuclear request system will not be tolerated under any circumstances.  Transmission does not guarantee a response.", "Self Destruct Code Request.","") as text|null
						...
						if("MessageCentcomm")
							if(is_authenticated(usr) == 2)
								if(centcomm_message_cooldown)
									...
									return
								var
									input = stripped_input(usr, "Please choose a message to transmit to Centcomm via quantum entanglement.  Please be aware that this process is very expensive, and abuse will lead to... termination.  Transmission does not guarantee a response.", "To abort, send an empty message.", "") as text|null
								if(!input || ..() || !(is_authenticated(usr) == 2))
									nanomanager.update_uis(src)
									return
						...
						if("MessageSyndicate")
							if((is_authenticated(usr) == 2) && (src.emagged))
								if(centcomm_message_cooldown)
									...
									return
								var
									input = stripped_input(usr, "Please choose a message to transmit to \[ABNORMAL ROUTING CORDINATES\] via quantum entanglement.  Please be aware that this process is very expensive, and abuse will lead to... termination. Transmission does not guarantee a response.", "To abort, send an empty message.", "") as text|null
								if(!input || ..() || !(is_authenticated(usr) == 2))
									nanomanager.update_uis(src)
									return
			...
			communications
				ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null)
					...
					if(shuttle_master.emergency.mode == SHUTTLE_CALL)
						var
							timeleft = shuttle_master.emergency.timeLeft()
...
obj
	machinery
		computer
			communications
				proc
					setCurrentMessage(var/mob/user,var/value)
...
obj
	machinery
		computer
			communications
				proc
					getCurrentMessage(var/mob/user)
						if(isAI(user) || isrobot(user))
							return aicurrmsg
						else
							return currmsg
...
obj
	machinery
		computer
			communications
				proc
					setMenuState(var/mob/user,var/value)
						if(isAI(user) || isrobot(user))
							ai_menu_state=value
						else
							menu_state=value
...
proc
	call_shuttle_proc(var/mob/user, var/reason)
