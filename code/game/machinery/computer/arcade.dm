...
/obj/machinery/computer/arcade/battle/proc/arcade_action()
	if((enemy_mp <= 0) || (enemy_hp <= 0))
		if(!gameover)
			...
			if(emagged)
				feedback_report("arcade_win_emagged")
				...
				feedback_report("arcade_win_normal")
		...
		if(player_mp <= 0)
			...
			if(emagged)
				feedback_report("arcade_loss_mana_emagged")
				usr.gib()
			else
				feedback_report("arcade_loss_mana_normal")
	...
	if((player_mp <= 0) || (player_hp <= 0))
		...
		playsound(src.loc, 'sound/arcade/Lose.ogg', 20, 1, extrarange = -6, falloff = 10)
		if(emagged)
			feedback_report("arcade_loss_hp_emagged")
			usr.gib()
		else
			feedback_report("arcade_loss_hp_normal")
