		...
		computer
			arcade
				New()
					..()
					var
						choice = pick(subtypesof(/obj/machinery/computer/arcade))
					new choice(loc)
					qdel(src)
		...
		computer
			arcade
				battle
					...
					var
						enemy_name = "Space Villian"
					var
						temp = "Winners Don't Use Spacedrugs" //Temporary message, for attack messages, etc
					var
				...
				battle
					attack_hand(mob/user as mob)
						...
						var
							datum
								browser
									popup = new(user, "arcade", "Space Villian 2000")
		...
		computer
			arcade
				orion_trail
					...
					var
						busy = 0 //prevent clickspam that allowed people to ~speedrun~ the game.
					var
						engine = 0
					var
						hull = 0
					...
					var
						list
							events = list(ORION_TRAIL_RAIDERS		= 3,
															ORION_TRAIL_FLUX			= 1,
				...
				orion_trail
					attack_hand(mob/user)
						...
						var
							datum
								browser
									popup = new(user, "arcade", "The Orion Trail",400,700)
						popup.set_content(dat)
						popup.set_title_image(user.browse_rsc_icon(icon, icon_state))
						popup.open()
						return
				...
				orion_trail
					Topic(href, href_list)
						...
						if(href_list["continue"]) //Continue your travels
							...
							if(emagged)
								...
								switch(event)
									...
									if(ORION_TRAIL_ILLNESS)
										var
											severity = rand(1,3) //pray to RNGesus. PRAY, PIGS
										if(severity == 1)
							...
							if(prob(75))
								...
								if(emagged) //has to be here because otherwise it doesn't work
									...
									var
										S = new /obj/singularity/academy(usr.loc)
							...
							var
								sheriff = remove_crewmember() //I shot the sheriff
						...
						//they also call event() again, to regen the eventdata, which is kind of odd but necessary
							...
							var
								success = min(15 * alive,100) //default crew (4) have a 60% chance
							spaceport_raided = 1
														...
														simple_animal
															hostile
																syndicate
																	ranged
																		orion
																			O = new/mob/living/simple_animal/hostile/syndicate/ranged/orion(get_turf(src))
											O.target = usr
							...
							switch(event)
								...
								if(ORION_TRAIL_LING_ATTACK)
									if(lings_aboard <= 0) //shouldn't trigger, but hey.
										...
										var
											chance2attack = alive*20
										if(prob(chance2attack))
											var
												chancetokill = 30*lings_aboard-(5*alive) //eg: 30*2-(10) = 50%, 2 lings, 2 crew is 50% chance
											if(prob(chancetokill))
												var
													deadguy = remove_crewmember()
	...
	machinery
		computer
			arcade
				orion_trail
					proc
						remove_crewmember(var/specific = "", var/dont_remove = "")
							var
								list
									safe2remove = settlers
							var
								removed = ""
							if(dont_remove)
					...
					proc
						win()
							...
							if(emagged)
								...
								var
									score = alive + round(food/2) + round(fuel/5) + engine + hull + electronics - lings_aboard
								prizevend(score)
							emagged = 0
							name = "The Orion Trail"
							desc = "Learn how our ancestors got to Orion, and have fun in the process!"
...
obj
	item
		weapon
			orion_ship
				attack_self(mob/user) //Minibomb-level explosion. Should probably be more because of how hard it is to survive the machine! Also, just over a 5-second fuse
					if(active)
						return
