...
/obj/machinery/computer/cloning/ui_data(mob/user, ui_key = "main", datum/topic_state/state = default_state)
	var/data[0]
	data["menu"] = src.menu
	data["scanner"] = sanitize_local("[src.scanner]")
	...
	if(pods.len)
		...
		for(var/obj/machinery/clonepod/pod in pods)
			...
			tempods.Add(list(list("pod" = "\ref[pod]", "name" = sanitize_local(capitalize(pod.name)), "biomass" = pod.biomass)))
			data["pods"] = tempods
	...
	for(var/datum/dna2/record/R in records)
		...
		temprecords.Add(list(list("record" = "\ref[R]", "realname" = sanitize_local(tempRealName))))
	...
	if(src.menu == 3)
		if(src.active_record)
			...
			data["realname"] = sanitize_local(src.active_record.dna.real_name)
