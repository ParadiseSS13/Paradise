			...
			cloning
				...
				var
					obj
						machinery
							dna_scannernew
								scanner = null //Linked scanner. For scanning.
				var
					list
						pods = list() //Linked cloning pods.
				var
					temp = ""
				var
					scantemp = "Scanner ready."
				var
					menu = 1 //Which menu screen to display
				var
					list
						records = list()
				var
					...
					obj
						item
							weapon
								disk
									data
										diskette = null //Mostly so the geneticist can steal everything.
				var
					loading = 0 // Nice loading text
				var
					autoprocess = 0
				var
					obj
					...
					//No locking an open scanner.
						...
						var
							obj
								machinery
									clonepod
										selected = locate(href_list["selectpod"])
						...
						if(istype(C))
							...
							if(!pods.len)
								...
								if(!selected_pod)
									...
									var
										mob
											selected = find_dead_player("[C.ckey]")
									if(!selected)
										return
										selected << 'sound/machines/chime.ogg' //probably not the best sound but I think it's reasonable

									var
										answer = alert(selected,"Do you want to return to life?","Cloning","Yes","No")
...
obj
	machinery
		computer
			cloning
				proc
					scan_mob(mob/living/carbon/human/subject as mob, var/scan_brain = 0)
						if(stat & NOPOWER)
							return
								...
								obj
									item
										organ
											internal
												brain
													Brn = subject.get_int_organ(/obj/item/organ/internal/brain)
							if(istype(Brn))
								var
									datum
										species
											S = all_species[Brn.dna.species] // stepladder code wooooo
								if(S.flags & NO_SCAN)
						...
						if(scan_brain)
							var
								obj
									item
										organ
											B = subject.get_int_organ(/obj/item/organ/internal/brain)
							B.dna.check_integrity()
							R.dna=B.dna.Clone()
							var
								datum
									species
										S = all_species[R.dna.species]
							if(S.flags & NO_SCAN)
							...
							obj
								item
									weapon
										implant
											health
												imp = locate(/obj/item/weapon/implant/health, subject)
						if(isnull(imp))
