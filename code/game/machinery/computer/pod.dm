				...
				attack_hand(var/mob/user as mob)
					...
					for(var/ident_tag in id_tags)
						...
						if(ident_tag in synced)
							...
							var
								second = times[ident_tag] % 60
							var
								minute = (times[ident_tag] - second) / 60
							var
								maxsecond = maxtimes[ident_tag] % 60
							var
								maxminute = (maxtimes[ident_tag] - maxsecond) / 60
			...
			pod
				Topic(href, href_list)
					if(..())
						...
						if(href_list["add"])
							var
								new_id_tag = input("Enter a new id_tag", "Mass Driver Controls", "id_tag")
										...
										computer
											pod
												deathsquad
													D = src
							var
								input = input("Enter the destination Z-Level. The mechs will arrive from the East. Leave 0 if you don't want to set a specific ZLevel", "Mass Driver Controls", 0) in choices
						...
						if(href_list["rename"])
							var
								new_title = input("Enter a new title", "[name]", "[name]")
							if(new_title)
								name = new_title
						updateUsrDialog()
					return
							...
							if(L.name == "Marauder Exit")
								var
									obj
										effect
											portal
												P = new(L.loc, pick(spawn_marauder))
		...
		deathsquad_tele
			Bumped(var/atom/movable/AM)
				...
				spawn()
					...
					var
						atom
							target = get_edge_target_turf(AM, AM.dir)
