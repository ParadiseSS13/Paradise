	...
	machinery
		computer
			message_monitor
				...
				var
					rebootmsg = "<span class='warning'>%$&(£: Critical %$$@ Error // !RestArting! <lOadiNg backUp iNput ouTput> - ?pLeaSe wAit!</span>"
				//Computer properties
				var
					screen = 0 		// 0 = Main menu, 1 = Message Logs, 2 = Hacked screen, 3 = Custom Message, 4 = chat room selection, 5 = chat room logs
				var
					hacking = 0		// Is it being hacked into by the AI/Cyborg
				var
					emag = 0		// When it is emagged.
				var
					message = "<span class='notice'>System bootup complete. Please select an option.</span>"	// The message that shows on the main menu.
				var
					auth = 0 // Are they authenticated?
				var
					optioncount = 8
...
obj
	machinery
		computer
			message_monitor
				attackby(obj/item/weapon/O as obj, mob/living/user as mob, params)
								...
								obj
									item
										weapon
											paper
												monitorkey
													MK = new/obj/item/weapon/paper/monitorkey
							MK.loc = src.loc
					...
					switch(screen)
						...
						if(4)
							...
							/* 	data_rc_msg
								X												 - 5%
								var
									rec_dpt = "Unspecified" //name of the person - 15%
								var
									send_dpt = "Unspecified" //name of the sender- 15%
								var
									message = "Blank" //transferred message		 - 300px
								var
									stamp = "Unstamped"							 - 15%
								var
									id_auth = "Unauthenticated"					 - 15%
								var
									priority = "Normal"							 - 10%
							*/
							dat += "<center><A href='?src=\ref[src];back=1'>Back</a> - <A href='?src=\ref[src];refresh=1'>Refresh</center><hr>"
					...
					if((usr.contents.Find(src) || (in_range(src, usr) && istype(src.loc, /turf))) || (istype(usr, /mob/living/silicon)))
						...
						if(href_list["auth"])
							if(auth)
								...
								var
									dkey = trim(input(usr, "Please enter the decryption key.") as text|null)
								if(dkey && dkey != "")
							...
							if(!linkedServer || (src.linkedServer.stat & (NOPOWER|BROKEN)))
								...
								if(auth)
									...
									if(dkey && dkey != "")
										if(src.linkedServer.decryptkey == dkey)
											var
												newkey = trim(input(usr,"Please enter the new key (3 - 16 characters max):"))
											if(length(newkey) <= 3)
											...
											datum
												data
													pda
														app
															messenger
																recipient_messenger = customrecepient.find_program(/datum/data/pda/app/messenger)
