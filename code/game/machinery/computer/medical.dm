		...
		switch(screen)
			if(MED_DATA_R_LIST)
				if(!isnull(SSrecords.general))
					...
					for(var/datum/data/record/R in sortRecord(SSrecords.general))
			...
			if(MED_DATA_RECORD)
				...
				if(istype(active2, /datum/data/record) && SSrecords.medical.Find(active2))
					var/list/fields = list()
...
/obj/machinery/computer/med_data/Topic(href, href_list)
	...
	if(!SSrecords.general.Find(active1))
		active1 = null
	if(!SSrecords.medical.Find(active2))
		active2 = null
