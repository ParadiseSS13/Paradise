						...
						if("MessageHonkplanet")
							if(src.authenticated==1)
								if(message_cooldown)
									...
									return
								var
									input = stripped_input(usr, "Please choose a message to transmit to your HONKbrothers on the homeworld. Transmission does not guarantee a response.", "To abort, send an empty message.", "")
		...
		computer
			HONKputer
				attack_hand(var/mob/user as mob)
					...
					var
						dat = "<head><title>HONKputer Interface</title></head><body>"
							...
							var
								obj
									structure
										computerframe
											HONKputer
												A = new /obj/structure/computerframe/HONKputer( src.loc )
							var
								obj
									item
										weapon
