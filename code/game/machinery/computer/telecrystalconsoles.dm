...
var
	list
		possible_uplinker_IDs = list("Alfa","Bravo","Charlie","Delta","Echo","Foxtrot","Zero", "Niner")
				...
				uplinker
					attack_hand(mob/user as mob)
						...
						var
							datum
								browser
									popup = new(user, "computer", "Telecrystal Upload/Recieve Station", 700, 500)
	...
	machinery
		computer
			telecrystals
				boss
					proc
						getDangerous()//This scales the TC assigned with the round population.
							..()
							var
								danger
	...
	machinery
		computer
			telecrystals
				boss
					proc
						scaleTC(var/amt)//Its own proc, since it'll probably need a lot of tweaks for balance, use a fancier algorhithm, etc.
				...
				boss
					attack_hand(var/mob/user as mob)
						...
						var
							datum
								browser
									popup = new(user, "computer", "Team Telecrystal Management Console", 700, 500)
						popup.set_content(dat)
						popup.set_title_image(user.browse_rsc_icon(src.icon, src.icon_state))
						popup.open()
						return
