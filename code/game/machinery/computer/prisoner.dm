				...
				Topic(href, href_list)
					...
					if(href_list["id"])
						switch(href_list["id"])
							...
							if("3")
								var
									num = round(input(usr, "Choose prisoner's goal:", "Input an Integer", null) as num|null)
								if(num >= 0)
					...
					if(href_list["inject1"])
						...
						var
							warning = sanitize(copytext(input(usr,"Message:","Enter your message here!",""),1,MAX_MESSAGE_LEN))
						if(!warning) return
						var
							obj
								item
