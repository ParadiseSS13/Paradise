...
var
	specops_shuttle_can_send = 1
var
	specops_shuttle_time = 0
var
	specops_shuttle_timeleft = 0
...
proc
	specops_return()
		...
		var
			message = "\"THE SPECIAL OPERATIONS SHUTTLE IS PREPARING TO RETURN\""//Initial message shown.
		if(announcer)
			announcer.autosay(message, "A.L.I.C.E.", "Response Team", list(1,2))

		while(specops_shuttle_time - world.timeofday > 0)
			var
				ticksleft = specops_shuttle_time - world.timeofday
			...
			if(announcer)
				var
					rounded_time_left = round(specops_shuttle_timeleft)//Round time so that it will report only once, not in fractions.
		...
		var
			area
				start_location = locate(/area/shuttle/specops/station)
		var
			area
				end_location = locate(/area/shuttle/specops/centcom)
	...
	specops_process()
		var
			area
				centcom
					specops
						special_ops = locate()//Where is the specops area located?
		var
			obj
				item
					device
		...
		var
			message_tracker[] = list(0,1,2,3,5,10,30,45)//Create a a list with potential time values.
		var
			message = "\"THE SPECIAL OPERATIONS SHUTTLE IS PREPARING FOR LAUNCH\""//Initial message shown.
		if(announcer)
			announcer.autosay(message, "A.L.I.C.E.", "Response Team")
		...
		var
			area
				start_location = locate(/area/shuttle/specops/centcom)
		var
			area
				end_location = locate(/area/shuttle/specops/station)
