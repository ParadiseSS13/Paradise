		...
		autolathe
			interact(mob/user)
				...
				var
					datum
						browser
							popup = new(user, "autolathe", name, 800, 500)
	...
	machinery
		autolathe
			attackby(obj/item/O, mob/user, params)
				...
				var
					material_amount = materials.get_item_material_amount(O)
				if(!material_amount)
				...
				busy = 1
				var
					inserted = materials.insert_item(O)
				if(inserted)
		...
		autolathe
			Topic(href, href_list)
				...
				if(href_list["make"])
					...
					var
						max_multiplier = min(design_last_ordered.maxstack, design_last_ordered.materials[MAT_METAL] ?round(materials.amount(MAT_METAL)/design_last_ordered.materials[MAT_METAL]):INFINITY,design_last_ordered.materials[MAT_GLASS]?round(materials.amount(MAT_GLASS)/design_last_ordered.materials[MAT_GLASS]):INFINITY)
					var
						is_stack = ispath(design_last_ordered.build_path, /obj/item/stack)
				...
				if(href_list["remove_from_queue"])
					var
						index = text2num(href_list["remove_from_queue"])
					if(isnum(index) && IsInRange(index,1,queue.len))
						remove_from_queue(index)
				if(href_list["queue_move"] && href_list["index"])
					var
						index = text2num(href_list["index"])
					var
						new_index = index + text2num(href_list["queue_move"])
...
obj
	machinery
		autolathe
			proc
				get_coeff(var/datum/design/D)
					var
						coeff = (ispath(D.build_path,/obj/item/stack) ? 1 : 2 ** prod_coeff)//stacks are unaffected by production coefficient
					return coeff
...
obj
	machinery
		autolathe
			proc
				build_item(var/datum/design/D, var/multiplier)
					desc = initial(desc)+"\nIt's building \a [initial(D.name)]."
					var
						is_stack = ispath(D.build_path, /obj/item/stack)
					var
						coeff = get_coeff(D)
					var
						metal_cost = D.materials[MAT_METAL]
					var
						glass_cost = D.materials[MAT_GLASS]
					var
						power = max(2000, (metal_cost+glass_cost)*multiplier/5)
					if(can_build(D,multiplier))
						...
						if(is_stack)
							var
								list
									materials_used = list(MAT_METAL=metal_cost*multiplier, MAT_GLASS=glass_cost*multiplier)
						...
						if(is_stack)
							var
								obj
									item
										stack
											S = new D.build_path(BuildTurf)
							S.amount = multiplier
						else
							var
								obj
									item
										new_item = new D.build_path(BuildTurf)
...
obj
	machinery
		autolathe
			proc
				can_build(var/datum/design/D,var/multiplier=1,var/custom_metal,var/custom_glass)
					var
						coeff = get_coeff(D)

					var
						metal_amount = materials.amount(MAT_METAL)
					if(custom_metal)
						metal_amount = custom_metal
					var
						glass_amount = materials.amount(MAT_GLASS)
					if(custom_glass)
...
obj
	machinery
		autolathe
			proc
				get_design_cost_as_list(var/datum/design/D,var/multiplier=1)
					var
						list
							OutputList = list(0,0)
					var
					...
					var
						datum
							design
								D = being_built[1]
					var
						multiplier = being_built[2]
					var
						is_stack = (multiplier>1)
					var
						output = "PROCESSING: [initial(D.name)][is_stack?" (x[multiplier])":null]"
					return output
	...
	machinery
		autolathe
			proc
				get_queue()
					var
						temp_metal = materials.amount(MAT_METAL)
					var
						temp_glass = materials.amount(MAT_GLASS)
					var
						output = "<td valign='top' style='width: 300px'>"
					...
					if(!istype(queue) || !queue.len)
						...
						for(var/list/L in queue)
							...
							var
								list
									LL = get_design_cost_as_list(D,multiplier)
							var
								is_stack = (multiplier>1)
...
obj
	machinery
		autolathe
			proc
				category_win(mob/user,var/selected_category)
					var
						dat = "<table style='width:100%'><tr><td valign='top' style='margin-right: 300px'>"
					...
					for(var/datum/design/D in files.known_designs)
						...
						if(ispath(D.build_path, /obj/item/stack))
							var
								max_multiplier = min(D.maxstack, D.materials[MAT_METAL] ?round(materials.amount(MAT_METAL)/D.materials[MAT_METAL]):INFINITY,D.materials[MAT_GLASS]?round(materials.amount(MAT_GLASS)/D.materials[MAT_GLASS]):INFINITY)
...
obj
	machinery
		autolathe
			proc
				get_design_cost(var/datum/design/D)
					var
						coeff = get_coeff(D)
					var
						dat
