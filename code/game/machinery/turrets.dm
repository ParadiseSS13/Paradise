	...
	machinery
		turret
			...
			var
				lastfired = 0
			var
				shot_delay = 30 //3 seconds between shots
			var
				datum
					effect
						system
...
obj
	machinery
		turret
			proc
				setState(var/enabled, var/lethal)
					src.enabled = enabled
					src.lasers = lethal
					src.power_change()
...
obj
	machinery
		turret
			proc
				check_target(var/atom/movable/T as mob|obj)
...
obj
	machinery
		gun_turret //related to turrets but work way differentely because of being mounted on a moving ship.
			name = "machine gun turret"
			desc = "Syndicate heavy defense turret. It really packs a bunch."
			density = 1
			anchored = 1
			var
				state = 0 //Like stat on mobs, 0 is alive, 1 is damaged, 2 is dead
			var
				faction = "syndicate"
			var
				atom
					cur_target = null
			var
				scan_range = 9 //You will never see them coming
			var
				health = 200 //Because it lacks a cover, and is mostly to keep people from touching the syndie shuttle.
			var
				bullet_type = /obj/item/projectile/bullet
			var
				firing_sound = 'sound/weapons/Gunshot3.ogg'
			var
				base_icon = "syndieturret"
			icon = 'icons/obj/turrets.dmi'
...
obj
	machinery
		gun_turret
			emp_act() //Can't emp an mechanical turret.
				return
		...
		gun_turret
			proc
				take_damage(damage)
					health -= damage
					var
						orig_health = initial(health)
					var
						health_percent = (health/orig_health) * 100
