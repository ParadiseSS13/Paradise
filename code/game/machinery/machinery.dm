...
obj
	machinery
		...
		var
			power_channel = EQUIP //EQUIP,ENVIRON or LIGHT
		var
			list
				component_parts = null //list of all the parts used to build it, if made from certain kinds of frames.
		var
			uid
		var
			manual = 0
		...
		var
			area
				myArea
		var
			interact_offline = 0 // Can the machine be interacted with while de-powered.
		var
			use_log = list()
		var
			list
				settagwhitelist = list()//WHITELIST OF VARIABLES THAT THE set_tag HREF CAN MODIFY, DON'T PUT SHIT YOU DON'T NEED ON HERE, AND IF YOU'RE GONNA USE set_tag (format_tag() proc), ADD TO THIS LIST.
...
obj
	machinery
		process() // If you dont use process or power why are you here
			return PROCESS_KILL
...
obj
	machinery
		proc
			update_use_power(var/new_use_power)
...
obj
	machinery
		proc
			handle_multitool_topic(var/href, var/list/href_list, var/mob/user)
				...
				if(P && istype(P))
					...
					if("set_tag" in href_list)
						...
						if(!(href_list["set_tag"] in vars))
							...
							return 1
						var
							current_tag = src.vars[href_list["set_tag"]]
						var
							newid = copytext(reject_bad_text(input(usr, "Specify the new value", src, current_tag) as null|text),1,MAX_MESSAGE_LEN)
						if(newid)
					...
					var
						ret = multitool_topic(usr,href_list,P.buffer)
					if(ret == MT_ERROR)
						return 1
					if(ret & MT_UPDATE)
						update_mt_menu=1
...
obj
	machinery
		proc
			operable(var/additional_flags = 0)
				return !inoperable(additional_flags)

obj
	machinery
		proc
			inoperable(var/additional_flags = 0)
...
obj
	machinery
		proc
			dropContents()//putting for swarmers, occupent code commented out, someone can use later.
				var
					turf
						T = get_turf(src)
...
obj
	machinery
		proc
			RefreshParts() //Placeholder proc for machines that are built using frames.
				return
				return 0
...
obj
	machinery
		proc
			default_deconstruction_crowbar(var/obj/item/weapon/crowbar/C, var/ignore_panel = 0)
					...
					var
						obj
							machinery
								constructable_frame
									machine_frame
										M = new /obj/machinery/constructable_frame/machine_frame(src.loc)
					M.state = 2
					M.icon_state = "box_1"
...
obj
	machinery
		proc
			default_deconstruction_screwdriver(var/mob/user, var/icon_state_open, var/icon_state_closed, var/obj/item/weapon/screwdriver/S)
				if(istype(S))
...
obj
	machinery
		proc
			default_change_direction_wrench(var/mob/user, var/obj/item/weapon/wrench/W)
...
obj
	machinery
		proc
			exchange_parts(mob/user, obj/item/weapon/storage/part_replacer/W)
				var
					shouldplaysound = 0
						...
						var
							obj
								item
									weapon
										circuitboard
											CB = locate(/obj/item/weapon/circuitboard) in component_parts
						var
							P
				...
				var
					obj
						machinery
							constructable_frame
								machine_frame
									M = new /obj/machinery/constructable_frame/machine_frame(loc)
				M.state = 2
				M.icon_state = "box_1"
...
obj
	machinery
		proc
			assess_perp(mob/living/carbon/human/perp, var/check_access, var/auth_weapons, var/check_records, var/check_arrest)
				var
					threatcount = 0	//the integer returned
				...
				if(check_records || check_arrest)
					...
					var
						datum
							data
								record
									R = find_security_record("name", perpname)
					if(check_records && !R)
						threatcount += 4
