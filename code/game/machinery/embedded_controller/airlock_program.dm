			...
			embedded_program
				airlock
					...
					var
						tag_interior_sensor
					var
						tag_airlock_mech_sensor
					var
						tag_shuttle_mech_sensor
...
datum
	computer
		file
			embedded_program
				airlock
					New(var/obj/machinery/embedded_controller/M)
						..(M)
						...
						switch(state)
							if(STATE_PREPARE)
								if(check_doors_secured())
									var
										chamber_pressure = memory["chamber_sensor_pressure"]
									var
										target_pressure = memory["target_pressure"]
			...
			embedded_program
				airlock
					proc
						check_doors_secured()
							var
								ext_closed = check_exterior_door_secured()
							var
	...
	computer
		file
			embedded_program
				airlock
					proc
						signalDoor(var/tag, var/command)
							var
								datum
									signal
										signal = new
	...
	computer
		file
			embedded_program
				airlock
					proc
						signalPump(var/tag, var/power, var/direction, var/pressure)
							var
								datum
									signal
										signal = new
	...
	computer
		file
			embedded_program
				airlock
					proc
						signal_mech_sensor(var/command, var/sensor)
							var
								datum
									signal
										signal = new
	...
	computer
		file
			embedded_program
				airlock
					proc
						toggleDoor(var/list/doorStatus, var/doorTag, var/secure, var/command)
							var
								doorCommand = null
