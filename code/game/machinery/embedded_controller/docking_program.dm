	...
	computer
		file
			embedded_program
				docking
					var
						tag_target				//the tag of the docking controller that we are trying to dock with
					var
						dock_state = STATE_UNDOCKED
					var
						control_mode = MODE_NONE
					var
						response_sent = 0		//so we don't spam confirmation messages
					var
						resend_counter = 0		//for periodically resending confirmation messages in case they are missed

					var
						override_enabled = 0	//when enabled, do not open/close doors or cycle airlocks and wait for the player to do it manually
					var
						received_confirm = 0	//for undocking, whether the server has recieved a confirmation from the client
			...
			embedded_program
				docking
					receive_signal(datum/signal/signal, receive_method, receive_param)
						var
							receive_tag = signal.data["tag"]		//for docking signals, this is the sender id
						var
							command = signal.data["command"]
						var
							recipient = signal.data["recipient"]	//the intended recipient of the docking signal
	...
	computer
		file
			embedded_program
				docking
					proc
						send_docking_command(var/recipient, var/command)
							var
								datum
									signal
										signal = new
