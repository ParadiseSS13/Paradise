...
/obj/machinery/wish_granter/attack_hand(mob/living/carbon/user)
	...
	if(charges <= 0)
		...
		to_chat(user, "<span class='notice'>The Wish Granter listens and materializes an experimetal tool!</span>")
		if(prob(50))
			new /obj/item/gun/energy/kinetic_accelerator/experimental(get_turf(src))
		else
			new /obj/item/twohanded/kinetic_crusher/experimental(get_turf(src))
