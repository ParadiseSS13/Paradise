	...
	if(on)
		if(cell && cell.charge > 0)
			...
			if(istype(L))
				...
				env.synchronize(CALLBACK(src, TYPE_PROC_REF(/obj/machinery/space_heater, process_sync), env))
		else
			on = FALSE
			update_icon()

/obj/machinery/space_heater/proc/process_sync(datum/gas_mixture/env)
