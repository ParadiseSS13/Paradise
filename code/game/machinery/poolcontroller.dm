obj
	machinery
		poolcontroller
			...
			var
				list
					linkedturfs = list() //List contains all of the linked pool turfs to this controller, assignment happens on New()
			var
				linked_area = null
			var
				temperature = "normal" //The temperature of the pool, starts off on normal, which has no effects.
			var
				temperaturecolor = "" //used for nanoUI fancyness
			var
				srange = 5 //The range of the search for pool turfs, change this for bigger or smaller pools.
			var
				list
					linkedmist = list() //Used to keep track of created mist
			var
				deep_water = 0		//set to 1 to drown even standing people


obj
	machinery
		poolcontroller
			New() //This proc automatically happens on world start
				if(!linked_area)
...
obj
	machinery
		poolcontroller
			emag_act(user as mob) //Emag_act, this is called when it is hit with a cryptographic sequencer.
...
obj
	machinery
		poolcontroller
			attackby(obj/item/P as obj, mob/user as mob, params) //Proc is called when a user hits the pool controller with something.
								...
								var
									mob
										living
											carbon
												human
													drownee = M //Typecast them as human.
...
obj
	machinery
		poolcontroller
			proc
				handleDrowning(var/mob/living/carbon/human/drownee)
					if(!drownee)
						return
...
obj
	machinery
		poolcontroller
			proc
				miston() //Spawn /obj/effect/mist (from the shower) on all linked pool tiles
					if(linkedmist.len)
						return

					for(var/turf/simulated/floor/beach/water/W in linkedturfs)
						var
							M = new /obj/effect/mist(W)
						linkedmist += M
...
obj
	machinery
		poolcontroller
			proc
				mistoff() //Delete all /obj/effect/mist from all linked pool tiles.
