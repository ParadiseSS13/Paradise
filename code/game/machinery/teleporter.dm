	...
	machinery
		computer
			teleporter
				...
				var
					turf
						target //Used for one-time-use teleport cards (such as clown planet coordinates.)
...
obj
	machinery
		computer
			teleporter
				attackby(I as obj, mob/living/user as mob, params)
				...
				proc
					set_target(mob/user)
						if(regime_set == "Teleporter")
							...
							var
								desc = input("Please select a location to lock in.", "Locking Computer") in L
							target = L[desc]
							...
							var
								list
									S = power_station.linked_stations
							if(!S.len)
							...
							for(var/obj/machinery/teleport/station/R in S)
								...
								L[tmpname] = R
							var
								desc = input("Please select a station to lock in.", "Locking Computer") in L
							target = L[desc]
							if(target)
								var
									obj
						...
						machinery
							teleport
								station
									power_station
				var
					calibrated //Calibration prevents mutation
				var
					admin_usage = 0 // if 1, works on z2. If 0, doesn't. Used for admin room teleport.
					...
					if(power_station && power_station.engaged && !panel_open)
						...
						if(istype(M, /mob/living/silicon/ai) || istype(M, /obj/structure/AIcore))
							...
							if(istype(M, /mob/living/silicon/ai))
								...
								var
									list
										TPError = list("<span class='warning'>Firmware instructions dictate you must remain on your assigned station!</span>",
...
obj
	machinery
		teleport
			hub
				proc
					teleport(atom/movable/M as mob|obj, turf/T)
						var
							obj
								machinery
									computer
										teleporter
											com = power_station.teleporter_console
						if(!com)
							return
						if(!com.target)
...
obj
	machinery
		teleport
			station
				attackby(var/obj/item/weapon/W, mob/user, params)
