				...
				var
					mob
						living
							silicon
								ai
									master//Which AI, if any, is controlling the object? Only one AI may control a hologram at any time.
				var
					last_request = 0 //to prevent request spam. ~Carn
				var
					holo_range = 5 // Change to change how far the AI can move away from the holopad before deactivating.
...
obj
	machinery
		hologram
			holopad
				attack_hand(var/mob/living/carbon/human/user) //Carn: Hologram requests.
					if(!istype(user))
						return
...
obj
	machinery
		hologram
			holopad
				proc
					activate_holo(mob/living/silicon/ai/user)
...
obj
	machinery
		hologram
			holopad
				hear_message(mob/living/M, text)
					if(M&&hologram&&master)//Master is mostly a safety in case lag hits or something.
						var
							name_used = M.GetVoice()
						var
							rendered = "<i><span class='game say'>Holopad received, <span class='name'>[name_used]</span> [text]</span></i>"
						master.show_message(rendered, 2)
					return
...
obj
	machinery
		hologram
			holopad
				proc
					create_holo(mob/living/silicon/ai/A, turf/T = loc)
						...
						if(master && !master.stat && master.client && master.eyeobj)//If there is an AI attached, it's not incapacitated, it has a client, and the client eye is centered on the projector.
							if(!(stat & NOPOWER))//If the  machine has power.
								if((HOLOPAD_MODE == 0 && (get_dist(master.eyeobj, src) <= holo_range)))
									...
									var
										area
											holo_area = get_area(src)
									var
										area
											eye_area = get_area(master.eyeobj)
		...
		hologram
			...
			var
				obj
					effect
						overlay
							hologram//The projection itself. If there is one, the instrument is on, off otherwise.
		...
		hologram_test()
			...
			var
				obj
					effect
						overlay
							hologram = new(loc)//Spawn a blank effect at the location.
			var
				icon
					flat_icon = icon(getFlatIcon(src,0))//Need to make sure it's a new icon so the old one is not reused.
			flat_icon.ColorTone(rgb(125,180,225))//Let's make it bluish.
			flat_icon.ChangeOpacity(0.5)//Make it half transparent.
			var
				input = input("Select what icon state to use in effect.",,"")
			if(input)
				var
					icon
						alpha_mask = new('icons/effects/effects.dmi', "[input]")
