	...
	machinery
		shieldgen
				...
				var
					active = 0
				var
					malfunction = 0 //Malfunction causes parts of the shield to slowly dissapate
				var
					list
						deployed_shields = list()
				var
					is_open = 0 //Whether or not the wires are exposed
				var
					locked = 0
		...
		shieldwallgen
			proc
				power()
					...
					var
						surplus = max(PN.avail-PN.load, 0)
					var
						shieldload = min(rand(50,200), surplus)
			...
			proc
				setup_field(var/NSEW = 0)
					...
					for(var/dist = 0, dist < steps, dist += 1) // creates each field tile
						var
							field_dir = get_dir(T2,get_step(T2, NSEW))
						T = get_step(T2, NSEW)
						T2 = T
						var
							obj
								machinery
									shieldwall
										CF = new/obj/machinery/shieldwall/(src, G) //(ref to this gen, ref to connected gen)
						CF.loc = T
						CF.dir = field_dir
...
obj
	machinery
		shieldwall
			New(var/obj/machinery/shieldwallgen/A, var/obj/machinery/shieldwallgen/B)
				..()
				src.gen_primary = A
				src.gen_secondary = B
				if(A && B)
