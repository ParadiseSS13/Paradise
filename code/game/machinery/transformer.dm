obj
	machinery
		transformer
			...
			var
				cooldown_duration = 600 // 1 minute
			var
				cooldown = 0
			var
...
obj
	machinery
		transformer
			proc
				do_transform(var/mob/living/carbon/human/H)
					...
					var
						mob
							living
								silicon
									robot
										R = H.Robotize(1) // Delete the items or they'll all pile up in a single tile and lag
...
obj
	machinery
		transformer
			proc
				do_transform_mime(var/obj/item/I)
					if(stat & (BROKEN|NOPOWER))
						return
					if(cooldown == 1)
						return
					...
					var
						icon
							newicon = new(I.icon, I.icon_state)
					newicon.GrayScale()
					I.icon = newicon
...
obj
	machinery
		transformer
			xray
				proc
					irradiate(var/mob/living/carbon/human/H)
						if(stat & (BROKEN|NOPOWER))
							return
