...
/obj/machinery/fabricator/New()
	...
	component_parts += new /obj/item/weapon/circuitboard/fabricator(null)
	...
	wires = new(src)
	files = new /datum/research/fabricator(src)
	matching_designs = list()

/obj/machinery/fabricator/upgraded/New()
	..()
	component_parts = list()
	component_parts += new /obj/item/weapon/circuitboard/fabricator(null)
...
/obj/machinery/fabricator/attackby(obj/item/O, mob/user, params)
	if (busy)
		user << "<span class=\"alert\">The fabricator is busy. Please wait for completion of previous operation.</span>"
		return 1

	if(default_deconstruction_screwdriver(user, "fabricator_t", "fabricator", O))
		updateUsrDialog()
		return
...
/obj/machinery/fabricator/attack_paw(mob/user)
...
/obj/machinery/fabricator/Topic(href, href_list)
	...
	if(href_list["make"])
		...
		if((queue.len+1)<queue_max_len)
			...
			usr << "<span class='warning'>The fabricator queue is full!</span>"
		if (!busy)
			busy = 1
			process_queue()
			busy = 0
...
/obj/machinery/fabricator/proc/get_coeff(var/datum/design/D)
	var/coeff = (ispath(D.build_path,/obj/item/stack) ? 1 : 2 ** prod_coeff)//stacks are unaffected by production coefficient
	return coeff

/obj/machinery/fabricator/proc/build_item(var/datum/design/D, var/multiplier)
...
/obj/machinery/fabricator/proc/get_design_cost_as_list(var/datum/design/D,var/multiplier=1)
	var/list/OutputList = list(0,0,0)
	var/coeff = get_coeff(D)
...
/obj/machinery/fabricator/proc/get_processing_line()
...
/obj/machinery/fabricator/proc/add_to_queue(D,var/multiplier)
	if(!istype(queue))
		queue = list()
	if(D)
...
/obj/machinery/fabricator/proc/remove_from_queue(index)
	if(!isnum(index) || !istype(queue) || (index<1 || index>queue.len))
		return 0
	queue.Cut(index,++index)
	return 1

/obj/machinery/fabricator/proc/process_queue()
...
/obj/machinery/fabricator/proc/search_win(mob/user)
	var/dat = "<table style='width:100%'><tr><td valign='top' style='margin-right: 300px'>"
	dat += "<div class='statusDisplay'>"
...
/obj/machinery/fabricator/proc/get_design_cost(var/datum/design/D)
	var/coeff = get_coeff(D)
	var/dat
	if(D.materials["$plastic"])
...
/obj/machinery/fabricator/proc/shock(mob/user, prb)
...
/obj/machinery/fabricator/proc/adjust_hacked(var/hack)
	hacked = hack
