...
datum
	data
		vending_product
			var
				product_name = "generic" // Display name for the product
			var
				product_path = null
			var
				amount = 0  // Amount held in the vending machine
			var
				max_amount = 0
			var
				price = 0  // Price to buy one
			var
				display_color = null  // Display color for vending machine listing
			var
				category = CAT_NORMAL  // CAT_HIDDEN for contraband, CAT_COIN for premium

datum
	data
		vending_product
			New(var/path, var/name = null, var/amount = 1, var/price = 0, var/color = null, var/category = CAT_NORMAL)
				..()
...
obj
	machinery
		vending
			...
			idle_power_usage = 10
			var
				vend_power_usage = 150

			// Vending-related
			var
				active = 1 //No sales pitches if off!
			var
				vend_ready = 1 //Are we ready to vend?? Is it time??
			var
				vend_delay = 10 //How long does it take to vend?
			var
				categories = CAT_NORMAL // Bitmask of cats we're currently showing
			var
				datum
					data
						vending_product
							currently_vending = null // What we're requesting payment for right now
			var
				status_message = "" // Status screen messages like "insufficient funds", displayed in NanoUI
			var
				status_error = 0 // Set to 1 if status_message is an error

			// To be filled out at compile time
			var
				list
					products	= list()	// For each, use the following pattern:
			var
				list
					contraband	= list()	// list(/type/path = amount,/type/path2 = amount2)
			var
				list
					premium 	= list()	// No specified amount = only one in stock
			var
				list
					prices     = list()	// Prices for each item, list(/type/path = price), items not in the list don't have a price.
			...
			var
				list
					slogan_list = list()
			var
				vend_reply				//Thank you for shopping!
			var
				shut_up = 0				//Stop spouting those godawful pitches!
			var
				last_reply = 0
			var
				last_slogan = 0			//When did we last pitch?
			var
				slogan_delay = 6000		//How long until we can pitch again?
			...
			var
				obj
					item
						weapon
							vending_refill
								refill_canister = null    //The type of refill canisters used by this machine.

			// Things that can go wrong
			emagged = 0			//Ignores if somebody doesn't have card access to that machine.
			var
				seconds_electrified = 0	//Shock customers like an airlock.
			var
				shoot_inventory = 0		//Fire items at customers! We're broken!
			var
				shoot_speed = 3			//How hard are we firing the items?
			var
				shoot_chance = 2		//How often are we firing the items?
						...
						for(var/entry in current_list[1])
							var
								datum
									data
										vending_product
											product = new/datum/data/vending_product(entry)
...
obj
	machinery
		vending
			proc
				refill_inventory(obj/item/weapon/vending_refill/refill, list/machine, mob/user)
					var
						total = 0
					...
					if(to_restock <= refill.charges)
						...
						var
							tmp_charges = refill.charges
						for(var/datum/data/vending_product/machine_content in machine)
							var
								restock = Ceiling(((machine_content.max_amount - machine_content.amount)/to_restock)*tmp_charges)
...
obj
	machinery
		vending
			proc
				pay_with_cash(var/obj/item/weapon/spacecash/cashmoney, mob/user)
					...
					var
						left = cashmoney.get_total() - currently_vending.price
...
obj
	machinery
		vending
			proc
				pay_with_card(var/obj/item/weapon/card/id/I)
...
obj
	machinery
		vending
			proc
				pay_with_account(var/datum/money_account/customer_account)
					if(!customer_account)
					...
					if(currently_vending.price > customer_account.money)
						...
						var
							paid = customer_account.charge(currently_vending.price,
...
obj
	machinery
		vending
			proc
				credit_purchase(var/target as text)
			...
			Topic(href, href_list)
				...
				if((usr.contents.Find(src) || (in_range(src, usr) && istype(src.loc, /turf))))
					if((href_list["vend"]) && (src.vend_ready) && (!currently_vending))
						...
						var
							key = text2num(href_list["vend"])
						var
							datum
								data
									vending_product
										R = product_records[key]
...
obj
	machinery
		vending
			proc
				vend(datum/data/vending_product/R, mob/user)
					if((!allowed(usr)) && !emagged && scan_id)	//For SECURE VENDING MACHINES YEAH
						to_chat(usr, "<span class='warning'>Access denied.</span>")//Unless emagged of course
...
obj
	machinery
		vending
			proc
				stock(var/datum/data/vending_product/R, var/mob/user)
					if(src.panel_open)
...
obj
	machinery
		vending/[vendors name here]   // --vending machine template   :)
			name = ""
			desc = ""
			icon = ''
			icon_state = ""
...
obj
	machinery
		vending
			atmospherics //Commenting this out until someone ponies up some actual working, broken, and unpowered sprites - Quarxink
			...
			assist
				products = list(	/obj/item/device/assembly/prox_sensor = 5,/obj/item/device/assembly/igniter = 3,/obj/item/device/assembly/signaler = 4,
									obj
										item
											weapon
												wirecutters = 1, /obj/item/weapon/cartridge/signal = 4)
									...
									item
										weapon
											reagent_containers
												food
													drinks
														chocolate = 10, /obj/item/weapon/reagent_containers/food/drinks/chicken_soup = 10,/obj/item/weapon/reagent_containers/food/drinks/weightloss = 10)
										...
										item
											weapon
												reagent_containers
													food
														drinks
															chicken_soup = 30,/obj/item/weapon/reagent_containers/food/drinks/weightloss = 50)
									...
									item
										weapon
											reagent_containers
												food
													snacks
														sosjerky = 6,/obj/item/weapon/reagent_containers/food/snacks/no_raisin = 6,/obj/item/weapon/reagent_containers/food/snacks/pistachios =6,
								obj
									item
										weapon
											reagent_containers
												food
													snacks
														spacetwinkie = 6,/obj/item/weapon/reagent_containers/food/snacks/cheesiehonkers = 6,/obj/item/weapon/reagent_containers/food/snacks/tastybread = 6)
									...
									item
										weapon
											reagent_containers
												food
													snacks
														chips =25,/obj/item/weapon/reagent_containers/food/snacks/sosjerky = 30,/obj/item/weapon/reagent_containers/food/snacks/no_raisin = 20,
								obj
									item
										weapon
											reagent_containers
												food
													snacks
														pistachios = 35, /obj/item/weapon/reagent_containers/food/snacks/spacetwinkie = 30,/obj/item/weapon/reagent_containers/food/snacks/cheesiehonkers = 25,/obj/item/weapon/reagent_containers/food/snacks/tastybread = 30)
										...
										weapon
											reagent_containers
												food
													snacks
														chinese
															rice = 5, /obj/item/weapon/reagent_containers/food/snacks/wingfangchu = 5)
										...
										weapon
											reagent_containers
												food
													snacks
														chinese
															rice = 50, /obj/item/weapon/reagent_containers/food/snacks/wingfangchu = 50)
										...
										weapon
											reagent_containers
												food
													drinks
														cans
															dr_gibb = 10,/obj/item/weapon/reagent_containers/food/drinks/cans/starkist = 10,
								obj
									item
										weapon
											reagent_containers
												food
													drinks
														cans
															space_up = 10,/obj/item/weapon/reagent_containers/food/drinks/cans/grape_juice = 10)
										...
										weapon
											reagent_containers
												food
													drinks
														cans
															dr_gibb = 20,/obj/item/weapon/reagent_containers/food/drinks/cans/starkist = 20,
								obj
									item
										weapon
											reagent_containers
												food
													drinks
														cans
															space_up = 20,/obj/item/weapon/reagent_containers/food/drinks/cans/grape_juice = 20)
				...
				products = list(/obj/item/device/pda =10,/obj/item/weapon/cartridge/medical = 10,/obj/item/weapon/cartridge/chemistry = 10,
								obj
									item
										weapon
											cartridge
												engineering = 10,/obj/item/weapon/cartridge/atmos = 10,/obj/item/weapon/cartridge/janitor = 10,
								obj
									item
										weapon
											cartridge
												signal
													toxins = 10,/obj/item/weapon/cartridge/signal = 10)
				...
				prices = list(/obj/item/device/pda =300,/obj/item/weapon/cartridge/medical = 200,/obj/item/weapon/cartridge/chemistry = 150,/obj/item/weapon/cartridge/engineering = 100,
								obj
									item
										weapon
											cartridge
												atmos = 75,/obj/item/weapon/cartridge/janitor = 100,/obj/item/weapon/cartridge/signal/toxins = 150,
								obj
									item
										weapon
											cartridge
										...
										weapon
											gun
												projectile
													automatic
														pistol
															m1911 = 2,/obj/item/weapon/gun/projectile/automatic/proto = 2,
								obj
									item
										weapon
											gun
												projectile
													shotgun
														automatic
															combat = 2,/obj/item/weapon/gun/projectile/automatic/gyropistol = 1,
								obj
									item
										weapon
											gun
												projectile
													shotgun = 2,/obj/item/weapon/gun/projectile/automatic/ar = 2)
									...
									item
										weapon
											reagent_containers
												glass
													bottle
														toxin = 4,/obj/item/weapon/reagent_containers/syringe/antiviral = 6,/obj/item/weapon/reagent_containers/syringe/insulin = 4,
								obj
									item
										weapon
											reagent_containers
												syringe = 12,/obj/item/device/healthanalyzer = 5,/obj/item/device/healthupgrade = 5,/obj/item/weapon/reagent_containers/glass/beaker = 4,
								obj
									item
										weapon
											reagent_containers
												dropper = 2,/obj/item/stack/medical/advanced/bruise_pack = 3, /obj/item/stack/medical/advanced/ointment = 3,
								obj
									item
										stack
											medical
												bruise_pack = 3,/obj/item/stack/medical/splint = 2, /obj/item/device/sensor_device = 2)
			...
			plasmaresearch
				...
				products = list(/obj/item/clothing/under/rank/scientist = 6,/obj/item/clothing/suit/bio_suit = 6,/obj/item/clothing/head/bio_hood = 6,
								obj
									item
										device
											transfer_valve = 6,/obj/item/device/assembly/timer = 6,/obj/item/device/assembly/signaler = 6,
								obj
									item
										device
											assembly
												prox_sensor = 6,/obj/item/device/assembly/igniter = 6)
				...
				products = list(/obj/item/weapon/reagent_containers/hypospray/autoinjector = 5,/obj/item/weapon/reagent_containers/syringe/charcoal = 3,/obj/item/stack/medical/bruise_pack = 3,
								obj
									item
										stack
											medical
												ointment =3,/obj/item/device/healthanalyzer = 3)
										...
										weapon
											reagent_containers
												food
													snacks
														donut
															normal = 12,/obj/item/weapon/storage/box/evidence = 6,/obj/item/device/flashlight/seclite = 4)
				...
				products = list(/obj/item/weapon/reagent_containers/glass/fertilizer/ez = 35,/obj/item/weapon/reagent_containers/glass/fertilizer/l4z = 25,/obj/item/weapon/reagent_containers/glass/fertilizer/rh = 15,/obj/item/weapon/plantspray/pests = 20,
								obj
									item
										weapon
											reagent_containers
												syringe = 5,/obj/item/weapon/storage/bag/plants = 5)
			...
			hydroseeds
				...
				products = list(/obj/item/seeds/aloeseed = 3,/obj/item/seeds/ambrosiavulgarisseed = 3,/obj/item/seeds/appleseed = 3,/obj/item/seeds/bananaseed = 3,/obj/item/seeds/berryseed = 3,
									obj
										item
											seeds
												cabbageseed = 3,/obj/item/seeds/carrotseed = 3,/obj/item/seeds/cherryseed = 3,/obj/item/seeds/chantermycelium = 3,
									obj
										item
											seeds
												chiliseed = 3,/obj/item/seeds/cocoapodseed = 3,/obj/item/seeds/coffeeaseed = 3,/obj/item/seeds/comfreyseed =3,/obj/item/seeds/cornseed = 3,
									obj
										item
											seeds
												eggplantseed = 3,/obj/item/seeds/grapeseed = 3,/obj/item/seeds/grassseed = 3,/obj/item/seeds/lemonseed = 3,
									obj
										item
											seeds
												limeseed = 3,/obj/item/seeds/orangeseed = 3,/obj/item/seeds/peanutseed = 3,/obj/item/seeds/potatoseed = 3,/obj/item/seeds/poppyseed = 3,
									obj
										item
											seeds
												pumpkinseed = 3,/obj/item/seeds/replicapod = 3,/obj/item/seeds/riceseed = 3,/obj/item/seeds/soyaseed = 3,/obj/item/seeds/sunflowerseed = 3,
									obj
										item
											seeds
												teaasperaseed = 3,/obj/item/seeds/tobaccoseed = 3,/obj/item/seeds/tomatoseed = 3,
									obj
										item
											seeds
												towermycelium = 3,/obj/item/seeds/watermelonseed = 3,/obj/item/seeds/wheatseed = 3,/obj/item/seeds/whitebeetseed = 3)
				contraband = list(/obj/item/seeds/amanitamycelium = 2,/obj/item/seeds/glowshroom = 2,/obj/item/seeds/libertymycelium = 2,/obj/item/seeds/nettleseed = 2,
									obj
										item
											seeds
												plumpmycelium = 2,/obj/item/seeds/random = 2, /obj/item/seeds/reishimycelium = 2, /obj/item/seeds/brownmold = 2)
						...
						for(var/entry in current_list[1])
							...
							var
								datum
									data
										vending_product
											product = new/datum/data/vending_product(entry, name)
								...
								obj
									item
										clothing
											head
												helmet
													gladiator = 1,/obj/item/clothing/under/gimmick/rank/captain/suit = 1,/obj/item/clothing/head/flatcap = 1,
								obj
									item
										clothing
											suit
												storage
													labcoat
														mad = 1,/obj/item/clothing/glasses/gglasses = 1,/obj/item/clothing/shoes/jackboots = 1,
								obj
									item
										clothing
											under
												schoolgirl = 1,/obj/item/clothing/head/kitty = 1,/obj/item/clothing/under/blackskirt = 1,/obj/item/clothing/head/beret = 1,
								obj
									item
										clothing
											accessory
												waistcoat = 1,/obj/item/clothing/under/suit_jacket = 1,/obj/item/clothing/head/that =1,/obj/item/clothing/under/kilt = 1,/obj/item/clothing/head/beret = 1,/obj/item/clothing/accessory/waistcoat = 1,
								obj
									item
										clothing
											glasses
												monocle =1,/obj/item/clothing/head/bowlerhat = 1,/obj/item/weapon/cane = 1,/obj/item/clothing/under/sl_suit = 1,
								obj
									item
										clothing
											mask
												fakemoustache = 1,/obj/item/clothing/suit/bio_suit/plaguedoctorsuit = 1,/obj/item/clothing/head/plaguedoctorhat = 1,/obj/item/clothing/mask/gas/plaguedoctor = 1,
								obj
									item
										clothing
											suit
												apron = 1,/obj/item/clothing/under/waiter = 1,/obj/item/clothing/suit/jacket/miljacket = 1,
								obj
									item
										clothing
											suit
												jacket
													miljacket
														white = 1, /obj/item/clothing/suit/jacket/miljacket/desert = 1, /obj/item/clothing/suit/jacket/miljacket/navy = 1,
								obj
									item
										clothing
											under
												pirate = 1,/obj/item/clothing/suit/pirate_brown = 1,/obj/item/clothing/suit/pirate_black =1,/obj/item/clothing/under/pirate_rags =1,/obj/item/clothing/head/pirate = 1,/obj/item/clothing/head/bandana = 1,
								obj
									item
										clothing
											head
												bandana = 1,/obj/item/clothing/under/soviet = 1,/obj/item/clothing/head/ushanka = 1,/obj/item/clothing/suit/imperium_monk = 1,
								obj
									item
										clothing
											mask
												gas
													cyborg = 1,/obj/item/clothing/suit/holidaypriest = 1,/obj/item/clothing/head/wizard/marisa/fake = 1,
								obj
									item
										clothing
											suit
												wizrobe
													marisa
														fake = 1,/obj/item/clothing/under/sundress = 1,/obj/item/clothing/head/witchwig = 1,/obj/item/weapon/twohanded/staff/broom = 1,
								obj
									item
										clothing
											suit
												wizrobe
													fake = 1,/obj/item/clothing/head/wizard/fake = 1,/obj/item/weapon/twohanded/staff = 3,/obj/item/clothing/mask/gas/sexyclown = 1,
								obj
									item
										clothing
											under
												sexyclown = 1,/obj/item/clothing/mask/gas/sexymime = 1,/obj/item/clothing/under/sexymime = 1,/obj/item/clothing/suit/apron/overalls = 1,
								obj
									item
										clothing
											head
												rabbitears =1, /obj/item/clothing/head/sombrero = 1, /obj/item/clothing/suit/poncho = 1,
								obj
									item
										clothing
											suit
												poncho
													green = 1, /obj/item/clothing/suit/poncho/red = 1, /obj/item/clothing/accessory/blue = 1, /obj/item/clothing/accessory/red = 1, /obj/item/clothing/accessory/black = 1, /obj/item/clothing/accessory/horrible = 1,
								obj
									item
										clothing
											under
												maid = 1, /obj/item/clothing/under/janimaid = 1, /obj/item/clothing/under/pants/camo = 1, /obj/item/clothing/mask/bandana = 1, /obj/item/clothing/mask/bandana/black = 1)
				...
				products = list(/obj/item/weapon/storage/bag/tray = 8,/obj/item/weapon/kitchen/utensil/fork = 6,
								obj
									item
										weapon
											kitchen
												knife = 3,/obj/item/weapon/kitchen/rollingpin = 2,
								obj
									item
										weapon
											reagent_containers
												food
													drinks
														drinkingglass = 8, /obj/item/clothing/suit/chef/classic = 2,
								obj
									item
										weapon
											reagent_containers
								...
								obj
									item
										weapon
											kitchen
												mould
													bear = 1, /obj/item/weapon/kitchen/mould/worm = 1,
								obj
									item
										weapon
											kitchen
												mould
													bean = 1, /obj/item/weapon/kitchen/mould/ball = 1,
								obj
									item
										weapon
											kitchen
												mould
													cane = 1, /obj/item/weapon/kitchen/mould/cash = 1,
								obj
									item
										weapon
											kitchen
												mould
													coin = 1, /obj/item/weapon/kitchen/mould/loli = 1)
			...
			tool
				...
				products = list(/obj/item/stack/cable_coil/random = 10,/obj/item/weapon/crowbar = 5,/obj/item/weapon/weldingtool = 3,/obj/item/weapon/wirecutters = 5,
								obj
									item
										weapon
											wrench = 5,/obj/item/device/analyzer = 5,/obj/item/device/t_scanner = 5,/obj/item/weapon/screwdriver = 5)
								...
								obj
									item
										weapon
											storage
												belt
													utility = 4,/obj/item/clothing/glasses/meson = 4,/obj/item/clothing/gloves/color/yellow = 4, /obj/item/weapon/screwdriver = 12,
								obj
									item
										weapon
											crowbar = 12,/obj/item/weapon/wirecutters = 12,/obj/item/device/multitool = 12,/obj/item/weapon/wrench = 12,/obj/item/device/t_scanner = 12,
								obj
									item
										stack
											cable_coil
												heavyduty = 8, /obj/item/weapon/stock_parts/cell = 8, /obj/item/weapon/weldingtool = 8,/obj/item/clothing/head/welding = 8,
								obj
									item
										weapon
											light
												tube = 10,/obj/item/clothing/suit/fire = 4, /obj/item/weapon/stock_parts/scanning_module = 5,/obj/item/weapon/stock_parts/micro_laser = 5,
								obj
									item
										weapon
											stock_parts
												matter_bin = 5,/obj/item/weapon/stock_parts/manipulator = 5,/obj/item/weapon/stock_parts/console_screen = 5)
								...
								obj
									item
										weapon
											stock_parts
												cell
													high = 12, /obj/item/device/assembly/prox_sensor = 3,/obj/item/device/assembly/signaler = 3,/obj/item/device/healthanalyzer = 3,
								obj
									item
										weapon
											scalpel = 2,/obj/item/weapon/circular_saw = 2,/obj/item/weapon/tank/anesthetic = 2,/obj/item/clothing/mask/breath/medical = 5,
								obj
									item
										weapon
											screwdriver = 5,/obj/item/weapon/crowbar = 5)
								...
								obj
									item
										clothing
											under
												color
													white = 10, /obj/item/clothing/under/color/yellow = 10,/obj/item/clothing/under/color/lightblue = 10,/obj/item/clothing/under/color/aqua = 10,/obj/item/clothing/under/color/purple = 10,/obj/item/clothing/under/color/lightgreen = 10,
								obj
									item
										clothing
											under
												color
													lightblue = 10,/obj/item/clothing/under/color/lightbrown = 10,/obj/item/clothing/under/color/brown = 10,/obj/item/clothing/under/color/yellowgreen = 10,/obj/item/clothing/under/color/darkblue = 10,/obj/item/clothing/under/color/lightred = 10, /obj/item/clothing/under/color/darkred = 10)
									...
									item
										weapon
											reagent_containers
												food
													snacks
														syndicake = 6, /obj/item/weapon/reagent_containers/food/snacks/cheesiehonkers = 6)
				refill_canister = /obj/item/weapon/vending_refill/snack
				...
				obj
					item
						clothing
							under
								suit_jacket
									navy=2,/obj/item/clothing/under/kilt=1,/obj/item/clothing/under/overalls=1,
				obj
					item
						clothing
							under
								suit_jacket
									really_black=2,/obj/item/clothing/under/pants/jeans=3,/obj/item/clothing/under/pants/classicjeans=2,
				obj
					item
						clothing
							under
								pants
									camo = 1,/obj/item/clothing/under/pants/blackjeans=2,/obj/item/clothing/under/pants/khaki=2,
				obj
					item
						clothing
							under
								pants
									white=2,/obj/item/clothing/under/pants/red=1,/obj/item/clothing/under/pants/black=2,
				obj
					item
						clothing
							under
								pants
									tan=2,/obj/item/clothing/under/pants/blue=1,/obj/item/clothing/under/pants/track=1,/obj/item/clothing/suit/jacket/miljacket = 1,
				obj
					item
						clothing
							accessory
								scarf
									red=1,/obj/item/clothing/accessory/scarf/green=1,/obj/item/clothing/accessory/scarf/darkblue=1,
				obj
					item
						clothing
							accessory
								scarf
									purple=1,/obj/item/clothing/accessory/scarf/yellow=1,/obj/item/clothing/accessory/scarf/orange=1,
				obj
					item
						clothing
							accessory
								scarf
									lightblue=1,/obj/item/clothing/accessory/scarf/white=1,/obj/item/clothing/accessory/scarf/black=1,
				obj
					item
						clothing
							accessory
								scarf
									zebra=1,/obj/item/clothing/accessory/scarf/christmas=1,/obj/item/clothing/accessory/stripedredscarf=1,
				obj
					item
						clothing
							accessory
								stripedbluescarf=1,/obj/item/clothing/accessory/stripedgreenscarf=1,/obj/item/clothing/accessory/waistcoat=1,
				obj
					item
						clothing
							under
								sundress=2,/obj/item/clothing/under/stripeddress = 1, /obj/item/clothing/under/sailordress = 1, /obj/item/clothing/under/redeveninggown = 1, /obj/item/clothing/under/blacktango=1,/obj/item/clothing/suit/jacket=3,
				obj
					item
						clothing
							glasses
								regular=2,/obj/item/clothing/head/sombrero=1,/obj/item/clothing/suit/poncho=1,
				obj
					item
						clothing
							suit
								ianshirt=1,/obj/item/clothing/shoes/laceup=2,/obj/item/clothing/shoes/black=4,
				obj
					item
						clothing
							shoes
								sandal=1, /obj/item/clothing/gloves/fingerless=2,
				obj
					item
						weapon
							storage
								belt
									fannypack=1, /obj/item/weapon/storage/belt/fannypack/blue=1, /obj/item/weapon/storage/belt/fannypack/red=1)
			...
			artvend
				...
				obj
					item
						weapon
							storage
								photo_album = 2,/obj/item/stack/wrapping_paper = 4,/obj/item/stack/tape_roll = 5,/obj/item/stack/packageWrap = 4,
				obj
					item
						weapon
							storage
								fancy
									crayons = 4,/obj/item/weapon/hand_labeler = 4,/obj/item/weapon/paper = 10,
				obj
					item
						weapon
							c_tube = 10,/obj/item/weapon/pen = 5,/obj/item/weapon/pen/blue = 5,
				obj
					item
						weapon
							pen
			...
			crittercare
				...
				products = list(/obj/item/clothing/accessory/petcollar = 5, /obj/item/weapon/storage/firstaid/aquatic_kit/full =5, /obj/item/fish_eggs/goldfish = 5,
								obj
									item
										fish_eggs
											clownfish = 5, /obj/item/fish_eggs/shark = 5, /obj/item/fish_eggs/feederfish = 10,
								obj
									item
										fish_eggs
											salmon = 5, /obj/item/fish_eggs/catfish = 5, /obj/item/fish_eggs/glofish = 5,
								obj
									item
										fish_eggs
											electric_eel = 5, /obj/item/fish_eggs/shrimp = 10, /obj/item/toy/pet_rock = 5,
								)
				prices = list(/obj/item/clothing/accessory/petcollar = 50, /obj/item/weapon/storage/firstaid/aquatic_kit/full = 60, /obj/item/fish_eggs/goldfish = 10,
								obj
									item
										fish_eggs
											clownfish = 10, /obj/item/fish_eggs/shark = 10, /obj/item/fish_eggs/feederfish = 5,
								obj
									item
										fish_eggs
											salmon = 10, /obj/item/fish_eggs/catfish = 10, /obj/item/fish_eggs/glofish = 10,
								obj
									item
										fish_eggs
											electric_eel = 10, /obj/item/fish_eggs/shrimp = 5, /obj/item/toy/pet_rock = 100,
								)
