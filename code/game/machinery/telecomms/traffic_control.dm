		...
		computer
			telecomms
				traffic
					...
					var
						allservers = 0          // Are all servers being edited at the same time?
					var
						list
							servers = list()	// the servers located by the computer
					var
						mob
							editingcode
					var
				...
				traffic
					interact(mob/user)
						...
						var
							datum
								browser
									popup = new(user, "traffic_control", "Telecommunication Traffic Control", 700, 500)
						//codemirror
				...
				traffic
					Topic(href, href_list)
							...
							if("Compile")
								...
								if(SelectedServer)
									...
									spawn(0)
										...
										var
											list
												compileerrors = Server.compile(user) // then compile the code!
								...
								if(allservers)
									spawn(0)
										...
										for(var/obj/machinery/telecomms/server/Server in servers)
											...
											var
												list
													compileerrors = Server.compile(user)
											if(!telecomms_check(user))
												return
											if(compileerrors.len)
												compiling_errors = "<b>Compile Errors</b><br>"
						...
						if(href_list["network"])
							...
							var
								newnet = input(user, "Which network do you want to view?", "Comm Monitor", network) as null|text
								...
								if(src.stat & BROKEN)
									...
									var
										obj
											structure
												computerframe
													A = new /obj/structure/computerframe( src.loc )
										...
										obj
											item
												weapon
													circuitboard
														comm_traffic
															M = new /obj/item/weapon/circuitboard/comm_traffic( A )
