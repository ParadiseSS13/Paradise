							...
							var
								obj
									item
										stack
											cable_coil
												A = new /obj/item/stack/cable_coil( user.loc )
							A.amount = 5
...
obj
	machinery
		telecomms
			proc
				formatInput(var/label,var/varname, var/input)
					var
						value = vars[varname]
					if(!value || value=="")
						value="-----"
	...
	machinery
		telecomms
			processor
				Options_Menu()
					var
						dat = "<br>Processing Mode: <A href='?src=\ref[src];process=1'>[process_mode ? "UNCOMPRESS" : "COMPRESS"]</a>"
					return dat
*/
			...
			bus
				Options_Topic(href, href_list)
					...
					if(href_list["change_freq"])
						...
						var
							newfreq = input(usr, "Specify a new frequency for new signals to change to. Enter null to turn off frequency changing. Decimals assigned automatically.", src, network) as null|num
						if(canAccess(usr))
							if(newfreq)
				...
				if(href_list["input"])
					switch(href_list["input"])
						...
						*/
						...
						if("id")
							var
								newid = copytext(reject_bad_text(input(usr, "Specify the new ID for this machine", src, id) as null|text),1,MAX_MESSAGE_LEN)
						...
						if("network")
							var
								newnet = input(usr, "Specify the new network for this machine. This will break all current links.", src, network) as null|text
						...
						if("freq")
							var
								newfreq = input(usr, "Specify a new frequency to filter (GHz). Decimals assigned automatically.", src, network) as null|num
					...
					if(text2num(href_list["unlink"]) <= length(links))
						var
							obj
								machinery
									telecomms
										T = links[text2num(href_list["unlink"])]
