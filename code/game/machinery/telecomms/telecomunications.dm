...
obj
	machinery
		telecomms
			var
				list
					links = list() // list of machines this machine is linked to
			var
			...
			var
				list
					autolinkers = list() // list of text/number values to link with
			var
			...
			var
				toggled = 1 	// Is it toggled on
			var
				on = 1
			var
				long_range_link = 0	// Can you link it across Z levels or on the otherside of the map? (Relay & Hub)
			var
				circuitboard = null // type pointing to a circuitboard type
			var
				hide = 0				// Is it a hidden machine?
			var
				listening_level = 0	// 0 = auto set in New() - this is the z level that the machine is listening to.
			Mtoollink = 1
...
obj
	machinery
		telecomms
			proc
				relay_information(datum/signal/signal, filter, copysig, amount = 20)
...
obj
	machinery
		telecomms
			proc
				relay_direct_information(datum/signal/signal, obj/machinery/telecomms/machine)
...
obj
	machinery
		telecomms
			proc
				is_freq_listening(datum/signal/signal)
...
obj
	machinery
		telecomms
			proc
				add_link(var/obj/machinery/telecomms/T)
					var
						turf
							position = get_turf(src)
					var
						turf
							T_position = get_turf(T)
				...
				receive_signal(datum/signal/signal)
					...
					if(signal.transmission_method == 2)
						...
						if(is_freq_listening(signal)) // detect subspace signals
							...
							var
								can_send = relay_information(signal, "/obj/machinery/telecomms/hub") // ideally relay the copied information to relays
							if(!can_send)
...
obj
	machinery
		telecomms
			receiver
				proc
					check_receive_level(datum/signal/signal)
...
obj
	machinery
		telecomms
			relay
				proc
					can_receive(datum/signal/signal)
						if(!can(signal))
							return 0
						return receiving
				...
				receive_information(datum/signal/signal, obj/machinery/telecomms/machine_from)
					...
					if(is_freq_listening(signal))
						...
						if(!istype(machine_from, /obj/machinery/telecomms/processor) && machine_from != src) // Signal must be ready (stupid assuming machine), let's send it
							...
							var
								send_to_processor = relay_information(signal, "/obj/machinery/telecomms/processor")
						...
						var
							list
								try_send = list("/obj/machinery/telecomms/server", "/obj/machinery/telecomms/hub", "/obj/machinery/telecomms/broadcaster")
						var
							i = 0
						for(var/send in try_send)
							...
							i++
							var
								can_send = relay_information(signal, send)
							if(can_send)
								break
	...
	machinery
		telecomms
			processor
				...
				var
					process_mode = 1 // 1 = Uncompress Signals, 0 = Compress Signals
		...
		telecomms
			server
				...
				var
					datum
						TCS_Compiler
							Compiler	// the compiler that compiles and runs the code
				var
					...
					if(signal.data["message"])
						...
						if(is_freq_listening(signal))
							...
							if(signal.data["type"] != 4)
								...
								// Give the log a name
								var
									identifier = num2text( rand(-1000,1000) + world.time )
							...
							var
								can_send = relay_information(signal, "/obj/machinery/telecomms/hub")
							if(!can_send)
...
obj
	machinery
		telecomms
			server
				proc
					add_entry(var/content, var/input)
						var
							datum
								comm_log_entry
									log = new
...
obj
	machinery
		telecomms
			server
				proc
					admin_log(var/mob/mob)
						var
							msg="[key_name(mob)] has compiled a script to server [src]:"
						diary << msg
						diary << rawcode
...
datum
	comm_log_entry
		var
			parameters = list() // carbon-copy to signal.data[]
		var
			name = "data packet (#)"
		var
			garbage_collector = 1 // if set to 0, will not be garbage collected
		var
			input_type = "Speech File"
