...
var
	list
		recentmessages = list() // global list of recent messages broadcasted : used to circumvent massive radio spam
var
	message_delay = 0 // To make sure restarting the recentmessages list is kept in sync
	...
	machinery
		telecomms
			allinone
				...
				machinetype = 6
				var
					intercept = 0 // if nonzero, broadcasts all messages to syndicate channel
...
proc
	Broadcast_Message(var/datum/radio_frequency/connection, var/mob/M,
							var
								vmask, var/vmessage, var/obj/item/device/radio/radio,
							var
								message, var/name, var/job, var/realname, var/vname,
							var
								data, var/compression, var/list/level, var/freq, var/verbage = "says", var/datum/language/speaking = null,
							var
								atom
									follow_target = null)
		...
		// --- Broadcast to antag radios! ---
			...
			for(var/antag_freq in ANTAG_FREQS)
				var
					datum
						radio_frequency
							antag_connection = radio_controller.return_frequency(antag_freq)
		...
		var
			list
				heard_masked 	= list() // masked name or no real name
		var
			list
				heard_normal 	= list() // normal message

		// Did not understand the message:
		var
			list
				heard_voice 	= list() // voice message	(ie "chimpers")
		var
			list
				heard_garbled	= list() // garbled message (ie "f*c* **u, **i*er!")
		var
			list
				heard_gibberish= list() // completely screwed over message (ie "F%! (O*# *#!<>&**%!")
		...
		var
			freq_text = get_frequency_name(display_freq)
				...
				var
					part_a = "<span class='[frequency_span_class(display_freq)]'>[bicon(radio)]<b>\[[freq_text]\][part_b_extra]</b> <span class='name'>" // goes in the actual output

				// --- Some more pre-message formatting ---
				var
					part_b = "</span> <span class='message'>" // Tweaked for security headsets -- TLE
				var
					part_c = "</span></span>"
				...
				var
					part_blackbox_b = "</span><b> \[[freq_text]\]</b> <span class='message'>" // Tweaked for security headsets -- TLE
				var
					blackbox_msg = "[part_a][name][part_blackbox_b][quotedmsg][part_c]"
...
proc
	Broadcast_SimpleMessage(var/source, var/frequency, var/text, var/data, var/mob/M, var/compression, var/level)
		...
		// --- Broadcast to antag radios! ---
			...
			for(var/freq in ANTAG_FREQS)
				var
					datum
						radio_frequency
							antag_connection = radio_controller.return_frequency(freq)
				for(var/obj/item/device/radio/R in antag_connection.devices["[RADIO_CHAT]"])
					var
						turf
							position = get_turf(R)
		...
		if(length(heard_normal) || length(heard_garbled) || length(heard_gibberish))
				...
				/* --- Some miscellaneous variables to format the string output --- */
		var
			part_a = "<span class='[frequency_span_class(display_freq)]'><span class='name'>" // goes in the actual output
				var
					freq_text = get_frequency_name(display_freq)
				...
				var
					part_b = "</span><b> [bicon(radio)]\[[freq_text]\][part_b_extra]</b> <span class='message'>" // Tweaked for security headsets -- TLE
				var
...
atom
	proc
		telecomms_process(var/do_sleep = 1)
