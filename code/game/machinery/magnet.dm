...
obj
	machinery
		magnetic_module
			...
			var
				freq = 1449		// radio frequency
			var
				electricity_level = 1 // intensity of the magnetic pull
			var
				magnetic_field = 1 // the range of magnetic attraction
			var
				code = 0 // frequency code, they should be different unless you have a group of magnets working together or something
			var
				turf
					center // the center of magnetic attraction
			var
				on = 0
			var
				pulling = 0
			...
			var
				center_y = 0
			var
				max_dist = 20 // absolute value of center_x,y cannot exceed this integer
			...
			proc
				magnetic_process() // proc that actually does the pulling
					if(pulling) return
					while(on)
...
obj
	machinery
		magnetic_controller
			...
			var
				autolink = 0 // if set to 1, can't probe for other magnets!

			var
				pathpos = 1 // position in the path
			var
				path = "NULL" // text path of the magnet
			var
				speed = 1 // lowest = 1, highest = 10
			var
				list
					rpath = list() // real path of the magnet, used in iterator

			var
				moving = 0 // 1 if scheduled to loop
			var
				looping = 0 // 1 if looping
			...
			Topic(href, href_list)
				...
				if(href_list["operation"])
					switch(href_list["operation"])
						...
						if("setpath")
							var
								newpath = sanitize(copytext(input(usr, "Please define a new path!",,path) as text|null,1,MAX_MESSAGE_LEN))
							if(newpath && newpath != "")
			...
			proc
				MagnetMove()
					...
					while(moving && rpath.len >= 1)
						...
						var
							nextmove = uppertext(rpath[pathpos]) // makes it un-case-sensitive
			...
			proc
				filter_path()
					...
					var
						maximum_character = min( 50, length(path) ) // chooses the maximum length of the iterator. 50 max length

					for(var/i=1, i<=maximum_character, i++) // iterates through all characters in path

						var
							nextchar = copytext(path, i, i+1) // find next character
