			...
			proc
				findsleeper()
					...
					for(dir in list(NORTH,EAST,SOUTH,WEST))
						...
						var
							temp = locate(/obj/machinery/sleeper, get_step(src, dir))
						if(!isnull(temp))
							sleepernew = temp
...
obj
	machinery
		sleeper
			attack_animal(var/mob/living/simple_animal/M)//Stop putting hostile mobs in things guise
			...
			ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1)
				...
				for(var/re in connected.injection_chems)
					var
						datum
							reagent
								temp = chemical_reagents_list[re]
					if(temp)
						var
							reagent_amount = 0
						var
							pretty_amount
						var
							injectable = occupant ? 1 : 0
						var
							overdosing = 0
						var
							caution = 0 // To make things clear that you're coming close to an overdose
...
obj
	machinery
		sleeper
			...
			dir = 8
			var
				orient = "LEFT" // "RIGHT" changes the dir suffix to "-r"
			var
				mob
					living
						carbon
							human
								occupant = null
			var
				possible_chems = list(list("epinephrine", "ether", "salbutamol", "styptic_powder"),
			...
			var
				emergency_chems = list("epinephrine") // Desnowflaking
			var
				amounts = list(5, 10)
			var
				obj
			...
			var
				max_chem
			var
				initial_bin_rating = 1
			var
				min_health = -25
			var
				injection_chems = list()
...
obj
	machinery
		sleeper
			proc
				inject_chemical(mob/living/user as mob, chemical, amount)
