...
obj
	mecha
		...
		force = 5
		var
			initial_icon = null //Mech type for resetting icon. Only used for reskinning kits (see custom items)
		var
			can_move = 1
		var
			mob
				living
					carbon
						occupant = null
		var
			step_in = 10 //make a step in step_in/10 sec.
		var
			dir_in = 2//What direction will the mech face when entered/powered on? Defaults to South.
		var
			step_energy_drain = 10
		var
			health = 300 //health is health
		var
			deflect_chance = 10 //chance to deflect the incoming projectiles, hits, or lesser the effect of ex_act.
		//the values in this list show how much damage will pass through, not how much will be absorbed.
		var
			list
				damage_absorption = list("brute"=0.8,"fire"=1.2,"bullet"=0.9,"laser"=1,"energy"=1,"bomb"=1)
		var
			obj
				item
					weapon
		...
		var
			maint_access = 1
		var
			dna	//dna-locking the mech
		var
			list
				proc_res = list() //stores proc owners, like proc_res["functionname"] = owner reference
		var
			datum
				effect
					system
		...
		var
			use_internal_tank = 0
		var
			internal_tank_valve = ONE_ATMOSPHERE
		var
			obj
				machinery
		...
		var
			internal_damage_threshold = 50 //health percentage below which internal damage is possible
		var
			internal_damage = 0 //contains bitflags

		var
			list
				operation_req_access = list()//required access level for mecha operation
		var
			list
				internals_req_access = list(access_engine,access_robotics)//required access level to open cell compartment

		var
			datum
				global_iterator
					pr_int_temp_processor //normalizes internal air mixture temperature
		var
			datum
				global_iterator
					pr_give_air //moves air from tank to cabin
		var
			datum
				global_iterator
					pr_internal_damage //processes internal damage
		...
		var
			stepsound = 'sound/mecha/mechturn.ogg'
			...
			var
				datum
					atom_hud
						data
							diagnostic
								diag_hud = huds[DATA_HUD_DIAGNOSTIC]
...
obj
	mecha
		proc
			add_cell(var/obj/item/weapon/stock_parts/cell/C=null)
				if(C)
					C.forceMove(src)
					cell = C
					return
...
obj
	mecha
		proc
			enter_after(delay as num, var/mob/user as mob, var/numticks = 5)
				var
					delayfraction = delay/numticks
...
obj
	mecha
		proc
			drop_item()//Derpfix, but may be useful in future for engineering exosuits.
				return
...
obj
	mecha
		proc
			click_action(atom/target, mob/user, params)
				...
				if(src == target)
					return
				var
					dir_to_target = get_dir(src,target)
...
obj
	mecha
		proc
			mech_toxin_damage(mob/living/target)
			...
			var
				mob
					living
						L = obstacle
			var
				hit_sound = list('sound/weapons/genhit1.ogg','sound/weapons/genhit2.ogg','sound/weapons/genhit3.ogg')
			if(L.flags & GODMODE)
				return
...
obj
	mecha
		proc
			check_for_internal_damage(var/list/possible_int_damage,var/ignore_threshold=null)
				...
				if(prob(20))
					if(ignore_threshold || src.health*100/initial(src.health)<src.internal_damage_threshold)
						...
						var
							int_dam_flag = safepick(possible_int_damage)
						if(int_dam_flag)
...
obj
	mecha
		proc
			hasInternalDamage(int_dam_flag=null)
...
obj
	mecha
		proc
			clearInternalDamage(int_dam_flag)
				internal_damage &= ~int_dam_flag
				switch(int_dam_flag)
...
obj
	mecha
		proc
			take_damage(amount, type="brute")
				if(amount)
					var
						damage = absorbDamage(amount,type)
					health -= damage
					update_health()
	...
	mecha
		attack_animal(mob/living/simple_animal/user as mob)
			...
			if(user.melee_damage_upper == 0)
				...
				if(!prob(src.deflect_chance))
					var
						damage = rand(user.melee_damage_lower, user.melee_damage_upper)
...
obj
	mecha
		hitby(atom/movable/A as mob|obj) //wrapper
			..()
...
obj
	mecha
		bullet_act(var/obj/item/projectile/Proj) //wrapper
...
obj
	mecha
		proc
			dynbulletdamage(var/obj/item/projectile/Proj)
				if(prob(src.deflect_chance))
...
obj
	mecha
		proc
			dynattackby(obj/item/weapon/W as obj, mob/living/user as mob, params)
				...
				var
					obj
						item
							mecha_parts
								mecha_tracking
									B = locate(/obj/item/mecha_parts/mecha_tracking) in src
...
obj
	mecha
		transfer_ai(var/interaction, mob/user, var/mob/living/silicon/ai/AI, var/obj/item/device/aicard/card)
			if(!..())
				return
...
obj
	mecha
		proc
			ai_enter_mech(var/mob/living/silicon/ai/AI, var/interaction)
					...
					obj
						machinery
							atmospherics
								unary
									portables_connector
										possible_port = locate(/obj/machinery/atmospherics/unary/portables_connector/) in loc
				if(possible_port)
...
obj
	mecha
		proc
			mmi_move_inside(var/obj/item/device/mmi/mmi_as_oc as obj,mob/user as mob)
				...
				/*
					if(ishuman(occupant) && (return_pressure() > HAZARD_HIGH_PRESSURE))
						...
						var
							datum
								gas_mixture
									environment = get_turf_air()
						if(environment)
							var
								env_pressure = environment.return_pressure()
							var
								pressure_delta = (cabin.return_pressure() - env_pressure)
					...
					//Actually transfer the gas
								var
									datum
										gas_mixture
											removed = cabin.air_contents.remove(transfer_moles)
...
obj
	mecha
		proc
			operation_allowed(mob/living/carbon/human/H)
				if(!ishuman(H))
					return 0
...
obj
	mecha
		proc
			internals_access_allowed(mob/living/carbon/human/H)
...
obj
	mecha
		check_access(obj/item/weapon/card/id/I, list/access_list)
		...
		proc
			get_stats_part()
				var
					integrity = health/initial(health)*100
				var
					cell_charge = get_charge()
				var
					tank_pressure = internal_tank ? round(internal_tank.return_pressure(),0.01) : "None"
				var
					tank_temperature = internal_tank ? internal_tank.return_temperature() : "Unknown"
				var
					cabin_pressure = round(return_pressure(),0.01)
				var
					output = {"[report_internal_damage()]
...
obj
	mecha
		proc
			get_equipment_menu() //outputs mecha html equipment menu
				var
					output
				if(equipment.len)
					output += {"<div class='wr'>
...
obj
	mecha
		proc
			get_equipment_list() //outputs mecha equipment list in html
				if(!equipment.len)
					return
				var
...
obj
	mecha
		proc
			get_log_html()
				var
					output = "<html><head><title>[src.name] Log</title></head><body style='font: 13px 'Courier', monospace;'>"
...
obj
	mecha
		proc
			output_access_dialog(obj/item/weapon/card/id/id_card, mob/user)
...
obj
	mecha
		proc
			output_maintenance_dialog(obj/item/weapon/card/id/id_card,mob/user)
				if(!id_card || !user) return
				var
					output = {"<html>
										<head>
...
obj
	mecha
		proc
			occupant_message(message as text)
				if(message)
...
obj
	mecha
		proc
			log_message(message as text,red=null)
				log.len++
...
obj
	mecha
		proc
			log_append_to_last(message as text,red=null)
				var
					list
						last_entry = src.log[src.log.len]
	...
	mecha
		Topic(href, href_list)
			...
			var
				datum
					topic_input
						filter = new /datum/topic_input(href,href_list)
			...
			if(href_list["rfreq"])
				if(usr != src.occupant)	return
				var
					new_frequency = (radio.frequency + filter.getNum("rfreq"))
			...
			if(href_list["change_name"])
				if(usr != src.occupant)	return
				var
					newname = strip_html_simple(input(occupant,"Choose new exosuit name","Rename exosuit",initial(name)) as text, MAX_NAME_LEN)
			...
			if(href_list["set_internal_tank_valve"] && state >=1)
				...
				if(user)
					var
						new_pressure = input(user,"Input new output pressure","Pressure setting",internal_tank_valve) as num
					if(new_pressure)
...
obj
	mecha
		proc
			dyngetcharge()//returns null if no powercell, else returns cell.charge
				if(!src.cell) return
				diag_hud_set_mechcell()
...
datum
	global_iterator
		mecha_preserve_temp  //normalizing cabin air temperature to 20 degrees celsium
			delay = 20

			process(var/obj/mecha/mecha)
				if(mecha.cabin_air && mecha.cabin_air.return_volume() > 0)
					var
						delta = mecha.cabin_air.temperature - T20C
			...
			process(var/obj/mecha/mecha)
				if(mecha.internal_tank)
					var
						datum
							gas_mixture
								tank_air = mecha.internal_tank.return_air()
					var
						datum
							gas_mixture
								cabin_air = mecha.cabin_air

					var
						release_pressure = mecha.internal_tank_valve
					var
						cabin_pressure = cabin_air.return_pressure()
					var
						pressure_delta = min(release_pressure - cabin_pressure, (tank_air.return_pressure() - cabin_pressure)/2)
					var
						transfer_moles = 0
					if(pressure_delta > 0) //cabin pressure lower than release pressure
						...
						if(pressure_delta > 0) //if location pressure is lower than cabin pressure
							...
							var
								datum
									gas_mixture
										removed = cabin_air.remove(transfer_moles)
							if(t_air)
								t_air.merge(removed)
...
datum
	global_iterator
		mecha_internal_damage // processing internal damage
				...
				if(mecha.hasInternalDamage(MECHA_INT_TANK_BREACH)) //remove some air from internal tank
					if(mecha.internal_tank)
						...
						var
							datum
								gas_mixture
									leaked_gas = int_tank_air.remove_ratio(0.10)
...
obj
	mecha
		speech_bubble(var/bubble_state = "",var/bubble_loc = src, var/list/bubble_recipients = list())
