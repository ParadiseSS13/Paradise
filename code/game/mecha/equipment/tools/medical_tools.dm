			...
			mecha_equipment
				tool
					sleeper
										...
										item
											mecha_parts
												mecha_equipment
													tool
														syringe_gun
															SG = locate(/obj/item/mecha_parts/mecha_equipment/tool/syringe_gun) in chassis
						...
						proc
							inject_reagent(var/datum/reagent/R,var/obj/item/mecha_parts/mecha_equipment/tool/syringe_gun/SG)
								if(!R || !occupant || !SG || !(SG in chassis.equipment))
									return 0
								var
									to_inject = min(R.volume, inject_amount)
			...
			mecha_equipment
				tool
					cable_layer
						...
						Topic(href,href_list)
							...
							if(href_list["cut"])
								if(cable && cable.amount)
									var
										m = round(input(chassis.occupant,"Please specify the length of cable to cut","Cut cable",min(cable.amount,30)) as num, 1)
						...
						proc
							load_cable(var/obj/item/stack/cable_coil/CC)
								if(istype(CC) && CC.amount)
									var
										cur_amount = cable? cable.amount : 0
									var
										to_load = max(max_cable - cur_amount,0)
									if(to_load)
						...
						proc
							dismantleFloor(var/turf/new_turf)
						...
						proc
							layCable(var/turf/new_turf)
								if(equip_ready || !istype(new_turf) || !dismantleFloor(new_turf))
									return reset()
								var
									fdirn = turn(chassis.dir,180)
				...
				tool
					syringe_gun
						...
						var
							max_syringes = 10
						var
							max_volume = 75 //max reagent volume
						var
							synth_speed = 5 //[num] reagent units per cycle
						energy_drain = 10
						var
							mode = 0 //0 - fire syringe, 1 - analyze reagents.
						var
							datum
								global_iterator
									mech_synth
						...
						Topic(href,href_list)
							...
							if(filter.get("purge_reagent"))
								var
									reagent = filter.get("purge_reagent")
								if(reagent)
						...
						proc
							load_syringe(obj/item/weapon/reagent_containers/syringe/S)
		...
		mech_synth
			...
			process(var/obj/item/mecha_parts/mecha_equipment/tool/syringe_gun/S)
				if(!S.chassis)
					return stop()
				var
					energy_drain = S.energy_drain*10
				...
				if(anyprob(S.reliability))
					S.critfail()
				var
					amount = S.synth_speed / S.processed_reagents.len
