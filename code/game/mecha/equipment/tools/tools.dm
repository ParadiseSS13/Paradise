					...
					drill
						...
						action(atom/target)
							...
							var
								T = chassis.loc
							var
								C = target.loc	//why are these backwards? we may never know -Pete
							if(do_after_cooldown(target))
								if(T == chassis.loc && src == chassis.selected)
										...
										if(locate(/obj/item/mecha_parts/mecha_equipment/tool/hydraulic_clamp) in chassis.equipment)
											var
												obj
													structure
														ore_box
															ore_box = locate(/obj/structure/ore_box) in chassis:cargo
											if(ore_box)
							...
							if(do_after_cooldown(target))
								if(istype(target, /obj/structure/reagent_dispensers/watertank) && get_dist(chassis,target) <= 1)
									...
									if(src.reagents.total_volume > 0)
										...
										var
											direction = get_dir(chassis,target)
										var
											turf
												T = get_turf(target)
										var
												...
												var
													obj
														effect
															effect
																water
																	W = new /obj/effect/effect/water(get_turf(chassis))
												if(!W)
													return
												var
													turf
			...
			mecha_equipment
				tool
					rcd
						...
						range = MELEE|RANGED
						var
							mode = 0 //0 - deconstruct, 1 - wall or floor, 2 - airlock.
						var
							canRwall = 0
											...
											var
												obj
													machinery
														door
															airlock
																T = new /obj/machinery/door/airlock(target)
											T.autoclose = 1
											playsound(target, 'sound/items/Deconstruct.ogg', 50, 1)
					...
					action(atom/target)
						...
						var
							obj
								effect
									portal
										P = new /obj/effect/portal(get_turf(target), target_turf)
...
obj
	item
		mecha_parts
			mecha_equipment
				anticcw_armor_booster //what is that noise? A BAWWW from TK mutants.
					...
					proc
						dynattackby(obj/item/weapon/W as obj, mob/user as mob, params)
		...
		mecha_parts
			mecha_equipment
				repair_droid
					...
					var
						list
							repairable_damage = list(MECHA_INT_TEMP_CONTROL,MECHA_INT_TANK_BREACH)
		...
		mecha_parts
			mecha_equipment
				tesla_energy_relay
					...
					var
						list
							use_channels = list(EQUIP,ENVIRON,LIGHT)
		...
		mecha_energy_relay
			...
			process(var/obj/item/mecha_parts/mecha_equipment/tesla_energy_relay/ER)
				if(!ER.chassis || ER.chassis.hasInternalDamage(MECHA_INT_SHORT_CIRCUIT))
					...
					return
				var
					cur_charge = ER.chassis.get_charge()
				...
				if(cur_charge<ER.chassis.cell.maxcharge)
					...
					if(A)
						...
						if(pow_chan)
							var
								delta = min(12, ER.chassis.cell.maxcharge-cur_charge)
			...
			mecha_equipment
				generator
					...
					var
						coeff = 100
					var
						fuel_type = MAT_PLASMA
					var
						max_fuel = 150000
					var
						fuel_name = "plasma" // Our fuel name as a string
					var
						fuel_amount = 0
					var
						fuel_per_cycle_idle = 10
					var
						fuel_per_cycle_active = 100
					var
						power_per_cycle = 30
					reliability = 1000
						...
						load_fuel(var/obj/item/I)
							if(istype(I) && (fuel_type in I.materials))
								if(istype(I, /obj/item/stack/sheet))
											...
											item
												stack
													sheet
														P = I
									var
										to_load = max(max_fuel - P.amount*P.perunit,0)
									if(to_load)
										var
											units = min(max(round(to_load / P.perunit),1),P.amount)
										if(units)
											var
												added_fuel = units * P.perunit
									...
									var
										to_load = max(min(I.materials[fuel_type], max_fuel - fuel_amount),0)
									if(to_load == 0)
										return 0
									fuel_amount += to_load
									qdel(I)
		...
		mecha_generator
			...
			process(var/obj/item/mecha_parts/mecha_equipment/generator/EG)
				...
				if(anyprob(EG.reliability))
					...
					return 0
				var
					cur_charge = EG.chassis.get_charge()
				if(isnull(cur_charge))
					...
					return 0
				var
					use_fuel = EG.fuel_per_cycle_idle
	...
	item
		mecha_parts
			mecha_equipment
				tool
					mimercd
						can_attach(obj/mecha/combat/recitence/M as obj)
							if(..())
								if(istype(M))
									return 1
							return 0
