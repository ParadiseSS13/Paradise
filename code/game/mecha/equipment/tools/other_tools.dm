	...
	if(chassis.internal_damage & MECHA_INT_SHORT_CIRCUIT)
		...
		for(var/int_dam_flag in repairable_damage)
			if(chassis.internal_damage & int_dam_flag)
				...
				repaired = 1
				break
	if(health_boost<0 || chassis.obj_integrity < initial(chassis.obj_integrity))
		chassis.obj_integrity += min(health_boost, initial(chassis.obj_integrity)-chassis.obj_integrity)
		repaired = 1
	if(repaired)
