...
/obj/item/mecha_parts/mecha_equipment/wormhole_generator/action(atom/target)
	...
	P.icon_state = "anom"
	P.name = "wormhole"
	message_admins("[key_name_admin(chassis.occupant, chassis.occupant.client)][ADMIN_QUE(chassis.occupant)] [ADMIN_FLW(chassis.occupant)] used a Wormhole Generator in ([loc.x],[loc.y],[loc.z] - <A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[loc.x];Y=[loc.y];Z=[loc.z]'>JMP</a>)",0,1)
