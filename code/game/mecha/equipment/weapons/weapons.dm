			...
			mecha_equipment
				weapon
					...
					var
						variance = 0
					var
						randomspread = 0 //use random spread for machineguns, instead of shotgun scatter
					var
						projectile_delay = 0
...
obj
	item
		mecha_parts
			mecha_equipment
				weapon
					can_attach(var/obj/mecha/combat/M as obj)
						if(..())
							if(istype(M))
								if(size > M.maxsize)
									return 0
						...
						proc
							rearm()
								if(projectiles < initial(projectiles))
									var
										projectiles_to_add = initial(projectiles) - projectiles
			...
			mecha_equipment
				weapon
					ballistic
						missile_rack
							flashbang
								clusterbang//Because I am a heartless bastard -Sieve
					...
					ballistic
						missile_rack
							flashbang
								clusterbang
									limited
										get_equip_info()//Limited version of the clusterbang launcher that can't reload
