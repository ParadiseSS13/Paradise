...
obj
	item
		mecha_parts
			mecha_equipment
				Destroy()//missiles detonating, teleporter creating singularity?
					if(chassis)
...
obj
	item
		mecha_parts
			mecha_equipment
				proc
					is_ranged()//add a distance restricted equipment. Why not?
						return range&RANGED
