...
turf
	simulated
		floor
			mech_bay_recharge_floor
				Entered(var/obj/mecha/mecha, atom/OL, ignoreRest = 0)
					. = ..()
					if(istype(mecha))
...
obj
	machinery
		mech_bay_recharge_port
			proc
				start_charge(var/obj/mecha/recharging_mecha)
			...
			process(var/obj/machinery/mech_bay_recharge_port/port, var/obj/mecha/mecha)
				...
				if(mecha && mecha in port.recharge_floor)
					if(!mecha.cell)
						return
					var
						delta = min(max_charge, mecha.cell.maxcharge - mecha.cell.charge)
					if(delta>0)
