					...
					var
						obj
							item
								mecha_parts
									mecha_equipment
										ME = new /obj/item/mecha_parts/mecha_equipment/tool/rcd
					ME.attach(src)
...
obj
	mecha
		combat
			phazon
				click_action(atom/target,mob/user)
					if(phasing)
				...
				verb
					switch_damtype()
						...
						if(usr!=src.occupant)
							return
						var
							new_damtype = alert(src.occupant,"Arm Tool Selection",null,"Fists","Torch","Toxic Injector")
						switch(new_damtype)
							if("Fists")
								damtype = "brute"
