...
/datum/action/innate/mecha/mech_toggle_floorbuffer/Activate()
	if(!chassis.floorbuffer)
		chassis.floorbuffer = TRUE
		chassis.step_in += chassis.buffer_delay
	else
		chassis.floorbuffer = FALSE
		chassis.step_in -= chassis.buffer_delay
	to_chat(usr, "<span class='notice'>The floor buffer is now [chassis.floorbuffer ? "active" : "deactivated"].</span>")
