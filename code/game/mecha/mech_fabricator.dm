					...
					for(var/datum/design/D in files.known_designs)
						if(D.build_type & MECHFAB)
							if(!(set_name in D.category))
								continue
							var
								resources_available = check_resources(D)
...
obj
	machinery
		mecha_part_fabricator
			proc
				output_part_info(datum/design/D)
					var
						output = "[initial(D.name)] (Cost: [output_part_cost(D)]) [get_construction_time_w_coeff(D)/10] seconds"
					return output
...
obj
	machinery
		mecha_part_fabricator
			proc
				output_part_cost(datum/design/D)
					var
						i = 0
					var
						output
			...
			proc
				output_available_resources()
					...
					for(var/resource in resources)
						var
							amount = min(res_max_amount, resources[resource])
...
obj
	machinery
		mecha_part_fabricator
			proc
				remove_resources(datum/design/D)
...
obj
	machinery
		mecha_part_fabricator
			proc
				check_resources(datum/design/D)
								...
								item
									weapon
										storage
											lockbox
												large
													L = new /obj/item/weapon/storage/lockbox/large(get_step(src,SOUTH)) //(Don't use capitals in paths, or single letters.
						I.loc = L
...
obj
	machinery
		mecha_part_fabricator
			proc
				get_resource_cost_w_coeff(datum/design/D, resource, roundto = 1)
...
obj
	machinery
		mecha_part_fabricator
			proc
				get_construction_time_w_coeff(datum/design/D, roundto = 1) //aran
		...
		mecha_part_fabricator
			Topic(href, href_list)
				...
				if(href_list["remove_mat"] && href_list["material"])
					var
						amount = text2num(href_list["remove_mat"])
					var
					...
					var
						removed = remove_material(material,amount)
					if(removed == -1)
...
obj
	machinery
		mecha_part_fabricator
			proc
				remove_material(var/mat_string, var/amount)
					...
					var
						total_amount = round(resources[mat_string]/MINERAL_MATERIAL_AMOUNT)
						...
						var
							obj
								item
									stack
										sheet
											res = new type(get_turf(src),min(amount,total_amount))
...
obj
	machinery
		mecha_part_fabricator
			attackby(obj/W as obj, mob/user as mob, params)
				...
				if(istype(W, /obj/item/stack))
					...
					if(resources[material] < res_max_amount)
						...
						var
							transfer_amount = min(stack.amount, round((res_max_amount - resources[material])/MINERAL_MATERIAL_AMOUNT,1))
