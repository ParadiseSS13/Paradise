...
var
	responseteam_age = 21 // Minimum account age to play as an ERT member
var
	datum
		response_team
			active_team = null
		...
		observer
			verb
				JoinResponseTeam()
					...
					var
						player_age_check = check_client_age(usr.client, responseteam_age)
							...
							var
								mob
									living
										carbon
											human
												new_commando = C.create_response_team(L.loc)
							qdel(L)
...
proc
	trigger_armed_response_team(var/datum/response_team/response_team_type)
...
/*
	var
		area
			security
				nuke_storage
					nukeloc = locate() //To find the nuke in the vault
	var
		obj
			machinery
				nuclearbomb
					nuke = locate() in nukeloc
	if(!nuke)
...
client
	proc
		create_response_team(obj/spawn_location)
			var
				mob
					living
						carbon
			...
			var
				new_gender = alert(usr, "Please select your gender.", "Character Generation", "Male", "Female")
			if(new_gender)
				if(new_gender == "Male")
...
datum
	response_team
		proc
			equip_officer(var/officer_type, var/mob/living/carbon/human/M)
