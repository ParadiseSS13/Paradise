...
/mob/living/carbon/human/clean_blood(radiation_clean = FALSE, clean_hands = TRUE, clean_mask = TRUE, clean_feet = TRUE)
	if(!(wear_suit && wear_suit.flags_inv & HIDEJUMPSUIT))
		w_uniform?.clean_blood(radiation_clean)
	...
	if(!(head && head.flags_inv & HIDEMASK))
		lip_style = null
		update_body()
	if(!(wear_mask && wear_mask.flags_inv & HIDEEYES))
		glasses?.clean_blood(radiation_clean)
	if(!(wear_mask && wear_mask.flags_inv & HIDEEARS))
		l_ear?.clean_blood(radiation_clean)
	if(!(wear_mask && wear_mask.flags_inv & HIDEEARS))
		r_ear?.clean_blood(radiation_clean)
	belt?.clean_blood(radiation_clean)
	neck?.clean_blood(radiation_clean)
	. = ..()
