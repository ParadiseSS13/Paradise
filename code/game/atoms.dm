atom
	...
	var
		throwpass = 0
	var
		germ_level = GERM_LEVEL_AMBIENT // The higher the germ level, the more germ on the atom.
	var
		simulated = 1 //filter for actions - used by lighting overlays
	var
		atom_say_verb = "says"
	...
	var
		allow_spin = 1 //Set this to 1 for a _target_ that is being thrown at; if an atom has this set to 1 then atoms thrown AT it will not spin; currently used for the singularity. -Fox
			...
			for(var/centcom_shuttle in list("emergency", "pod1", "pod2", "pod3", "pod4", "ferry"))
				var
					obj
						docking_port
							mobile
								M = shuttle_master.getShuttle(centcom_shuttle)
				...
				var
					datum
						alternate_appearance
							AA = alternate_appearances[aakey]
				qdel(AA)
...
atom
	proc
		bullet_act(var/obj/item/projectile/Proj, def_zone)
			Proj.on_hit(src, 0, def_zone)
			return 0

atom
	proc
		in_contents_of(container)//can take class or object instance as argument
			if(ispath(container))
...
atom
	proc
		search_contents_for(path,list/filter_path=null)
			var
				list
					found = list()
...
atom
	proc
		examine(mob/user, var/distance = -1, var/infix = "", var/suffix = "")
...
atom
	proc
		add_hiddenprint(mob/living/M as mob)
			if(isnull(M)) return
...
atom
	proc
		add_fingerprint(mob/living/M as mob, ignoregloves = 0)
			if(isnull(M)) return
			if(isnull(M.key)) return
			if(ishuman(M))
				//Add the list if it does not exist.
...
atom
	proc
		transfer_fingerprints_to(var/atom/A)
...
atom
	proc
		add_blood(mob/living/carbon/human/M as mob)
...
atom
	proc
		add_blood_list(mob/living/carbon/M)
...
atom
	proc
		add_blood_floor(mob/living/carbon/M)
...
atom
	proc
		add_vomit_floor(mob/living/carbon/M as mob, var/toxvomit = 0)
