...
atom
	var
		list
			canSmoothWith=list() // TYPE PATHS I CAN SMOOTH WITH~~~~~

// MOVED INTO UTILITY FUNCTION FOR LESS DUPLICATED CODE.
atom
	proc
		findSmoothingNeighbors()
...
atom
	proc
		isSmoothableNeighbor(var/atom/A)
...
atom
	proc
		relativewall_neighbours(var/sko=0) //SKO: Skip Optimizations
