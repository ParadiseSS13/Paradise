...
/datum/dna/proc/write_head_attributes(obj/item/organ/external/head/head_organ)
	...
	//Hair Gradient
	var/gradient = GetUIValueRange(DNA_UI_GRADIENT_STYLE,GLOB.hair_gradients_list.len)
	if((gradient > 0) && (gradient <= GLOB.hair_gradients_list.len))
		head_organ.h_grad_style = GLOB.hair_gradients_list[gradient]
		head_organ.h_grad_offset_x = GetUIValueRange(DNA_UI_GRADIENT_X, 32) - 16
		head_organ.h_grad_offset_y = GetUIValueRange(DNA_UI_GRADIENT_Y, 32) - 16

	head_organ.h_grad_colour = rgb(head_organ.dna.GetUIValueRange(DNA_UI_GRADIENT_R, 255), head_organ.dna.GetUIValueRange(DNA_UI_GRADIENT_G, 255), head_organ.dna.GetUIValueRange(DNA_UI_GRADIENT_B, 255))
	//Facial Hair
...
/datum/dna/proc/head_traits_to_dna(mob/living/carbon/human/character, obj/item/organ/external/head/head_organ)
	...
	SetUIValueRange(DNA_UI_GRADIENT_R,		color2R(head_organ.h_grad_colour),		255,	 1)
	SetUIValueRange(DNA_UI_GRADIENT_G,		color2G(head_organ.h_grad_colour),		255,	 1)
	SetUIValueRange(DNA_UI_GRADIENT_B,		color2B(head_organ.h_grad_colour),		255,	 1)

	SetUIValueRange(DNA_UI_GRADIENT_X,		head_organ.h_grad_offset_x + 16,		32,	 1)
	SetUIValueRange(DNA_UI_GRADIENT_Y,		head_organ.h_grad_offset_y + 16,		32,	 1)
	...
	SetUIValueRange(DNA_UI_GRADIENT_STYLE,	gradient,		GLOB.hair_gradients_list.len,	 1)
