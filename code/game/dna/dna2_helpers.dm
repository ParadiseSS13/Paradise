...
proc
	GetDNABounds(var/block)
		var
			list
				BOUNDS=dna_activity_bounds[block]
		if(!istype(BOUNDS))
...
mob
	proc
		UpdateAppearance(var/list/UI=null)
			if(istype(src, /mob/living/carbon/human))
				...
				//Hair
				var
					hair = dna.GetUIValueRange(DNA_UI_HAIR_STYLE,hair_styles_list.len)
				if((0 < hair) && (hair <= hair_styles_list.len))
					head_organ.h_style = hair_styles_list[hair]

				//Facial Hair
				var
					beard = dna.GetUIValueRange(DNA_UI_BEARD_STYLE,facial_hair_styles_list.len)
				if((0 < beard) && (beard <= facial_hair_styles_list.len))
					head_organ.f_style = facial_hair_styles_list[beard]

				//Head Accessories
				var
					headacc = dna.GetUIValueRange(DNA_UI_HACC_STYLE,head_accessory_styles_list.len)
				if((0 < headacc) && (headacc <= head_accessory_styles_list.len))
					head_organ.ha_style = head_accessory_styles_list[headacc]

				//Markings
				var
					marks = dna.GetUIValueRange(DNA_UI_MARK_STYLE,marking_styles_list.len)
