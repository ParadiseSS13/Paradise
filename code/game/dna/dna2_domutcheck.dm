...
proc
	genemutcheck(var/mob/living/M, var/block, var/connected=null, var/flags=0)
		...
		var
			datum
				dna
					gene
						gene = assigned_gene_blocks[block]
		domutation(gene, M, connected, flags)


proc
	domutation(var/datum/dna/gene/gene, var/mob/living/M, var/connected=null, var/flags=0)
		if(!gene || !istype(gene))
			return 0
		...
		// Current state
		var
			gene_active = (gene.flags & GENE_ALWAYS_ACTIVATE)
		if(!gene_active)
			gene_active = M.dna.GetSEState(gene.block)

		var
			defaultgenes // Do not mutate inherent species abilities
		if(ishuman(M))
			var
				mob
					living
		...
		// Prior state
		var
			gene_prior_status = (gene.type in M.active_genes)
		var
			changed = gene_active != gene_prior_status || (gene.flags & GENE_ALWAYS_ACTIVATE)
