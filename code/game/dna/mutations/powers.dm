	...
	if(ishuman(the_item))
		...
		if(!do_mob(user,H,EAT_MOB_DELAY))
			...
			if(!limb || !H)
				return
			if(H.loc != oldloc)
				to_chat(user, span_danger("\The [limb] moved away from your mouth!"))
				return
		...
		user.visible_message(span_danger("[user] eats \the [the_item]."))
		playsound(user.loc, 'sound/items/eatfood.ogg', 50, 0)
		qdel(the_item)
		doHeal(user)
...
/obj/effect/proc_holder/spell/mindscan/cast(list/targets, mob/user = usr)
	...
	for(var/mob/living/target in targets)
		...
		target.show_message(span_abductor("<A href='?src=[UID()];target=[target.UID()];user=[user.UID()]'>[message]</a>"))
