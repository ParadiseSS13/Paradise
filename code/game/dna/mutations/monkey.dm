...
/datum/mutation/monkey/activate(mob/living/carbon/human/H)
	...
	H.regenerate_icons()
	ADD_TRAIT(H, TRAIT_IMMOBILIZED, "transforming")
	ADD_TRAIT(H, TRAIT_HANDS_BLOCKED, "transforming")
	H.icon = null
	H.invisibility = 101
	...
	if(!has_primitive_form) //If the pre-change mob in question has no primitive set, this is going to be messy.
		H.gib()
		return
	REMOVE_TRAIT(H, TRAIT_IMMOBILIZED, "transforming")
	REMOVE_TRAIT(H, TRAIT_HANDS_BLOCKED, "transforming")
