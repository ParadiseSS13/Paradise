...
var
	global
		list
			dna_activity_bounds[DNA_SE_LENGTH]
var
	global
		list
			assigned_gene_blocks[DNA_SE_LENGTH]

// Used to determine what each block means (admin hax and species stuff on /vg/, mostly)
var
	global
		list
			assigned_blocks[DNA_SE_LENGTH]
...
datum
	dna
		...
		var
			uni_identity="" // Encoded UI
		var
			struc_enzymes="" // Encoded SE
		var
			unique_enzymes="" // MD5 of player name

		// Original Encoded SE, for use with Ryetalin
		var
			struc_enzymes_original="" // Encoded SE
		var
			list
				SE_original[DNA_SE_LENGTH]
		...
		// From old dna.
		var
			b_type = "A+"  // Should probably change to an integer => string map but I'm lazy.
		var
...
datum
	dna
		proc
			ResetUIFrom(var/mob/living/carbon/human/character)
				// INITIALIZE!
				ResetUI(1)
				// Hair
				...
				if(!H.h_style)
					H.h_style = "Skinhead"
				var
					hair = hair_styles_list.Find(H.h_style)

				// Facial Hair
				if(!H.f_style)
					H.f_style = "Shaved"
				var
					beard	= facial_hair_styles_list.Find(H.f_style)

				// Head Accessory
				if(!H.ha_style)
					H.ha_style = "None"
				var
					headacc	= head_accessory_styles_list.Find(H.ha_style)

				/*// Body Accessory
				if(!character.body_accessory)
					character.body_accessory = null
				var
					bodyacc	= character.body_accessory*/

				// Markings
				if(!character.m_style)
					character.m_style = "None"
				var
					marks	= marking_styles_list.Find(character.m_style)
...
datum
	dna
		proc
			SetUIValue(var/block,var/value,var/defer=0)
				if(block<=0) return
				ASSERT(value>0)
				ASSERT(value<=4095)
				UI[block]=value
...
datum
	dna
		proc
			SetUIValueRange(var/block,var/value,var/maxvalue,var/defer=0)
				if(block<=0)
					return
				if(value == 0)
					value = 1
...
datum
	dna
		proc
			GetUIValueRange(var/block,var/maxvalue)
				if(block<=0) return 0
				var
					value = GetUIValue(block)
...
datum
	dna
		proc
			SetUIState(var/block,var/on,var/defer=0)
				if(block<=0) return
				var
					val
				if(on)
...
datum
	dna
		proc
			SetUIBlock(var/block,var/value,var/defer=0)
				if(block<=0) return
...
datum
	dna
		proc
			GetUISubBlock(var/block,var/subBlock)
...
datum
	dna
		proc
			SetUISubBlock(var/block,var/subBlock, var/newSubBlock, var/defer=0)
				if(block<=0) return
				var
					oldBlock=GetUIBlock(block)
				var
					newBlock=""
...
datum
	dna
		proc
			SetSEValue(var/block,var/value,var/defer=0)
...
datum
	dna
		proc
			SetSEValueRange(var/block,var/value,var/maxvalue)
				if(block<=0) return
				ASSERT(maxvalue<=4095)
				var
					range = round(4095 / maxvalue)
				if(value)
...
datum
	dna
		proc
			GetSEValueRange(var/block,var/maxvalue)
				if(block<=0) return 0
				var
					value = GetSEValue(block)
				return round(1 +(value / 4096)*maxvalue)
...
datum
	dna
		proc
			GetSEState(var/block)
				if(block<=0) return 0
				var
					list
						BOUNDS=GetDNABounds(block)
				var
					value=GetSEValue(block)
...
datum
	dna
		proc
			SetSEState(var/block,var/on,var/defer=0)
				if(block<=0) return
				var
					list
						BOUNDS=GetDNABounds(block)
...
datum
	dna
		proc
			SetSEBlock(var/block,var/value,var/defer=0)
				if(block<=0) return
				var
					nval=hex2num(value)
...
datum
	dna
		proc
			GetSESubBlock(var/block,var/subBlock)
...
datum
	dna
		proc
			SetSESubBlock(var/block,var/subBlock, var/newSubBlock, var/defer=0)
				if(block<=0) return
				var
					oldBlock=GetSEBlock(block)
				var
					newBlock=""
				for(var/i=1, i<=length(oldBlock), i++)
					if(i==subBlock)
...
datum
	dna
		proc
			check_integrity(var/mob/living/carbon/human/character)
				if(character)
...
datum
	dna
		proc
			ready_dna(mob/living/carbon/human/character, flatten_SE = 1)
