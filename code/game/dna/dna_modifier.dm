		...
		record
			proc
				GetData()
					var
						list
							ser=list("data" = null, "owner" = null, "label" = null, "type" = null, "ue" = 0)
					if(dna)
				...
				copy()
					var
						datum
							dna2
								record
									newrecord = new /datum/dna2/record
...
obj
	machinery
		dna_scannernew
			attackby(var/obj/item/weapon/item as obj, var/mob/user as mob, params)
		...
		dna_scannernew
			proc
				radiation_check()
					...
					var
						radiation_protection = occupant.run_armor_check(null, "rad", "Your clothes feel warm.", "Your clothes feel warm.")
	...
	machinery
		computer
			scan_consolenew
				...
				var
					selected_ui_block = 1.0
				var
					selected_ui_subblock = 1.0
				var
					selected_se_block = 1.0
				var
					selected_se_subblock = 1.0
				var
					selected_ui_target = 1
				var
					selected_ui_target_hex = 1
				var
					radiation_duration = 2.0
				var
				...
				var
					irradiating = 0
				var
					injector_ready = 0	//Quick fix for issue 286 (screwdriver the screen twice to restore injector)	-Pete
				var
					obj
						machinery
							dna_scannernew
							...
							weapon
								disk
									data
										disk = null
				var
					selected_menu_key = null
				anchored = 1
				use_power = 1
				idle_power_usage = 10
				active_power_usage = 400
				var
					waiting_for_user_input=0 // Fix for #274 (Mash create block injector without answering dialog to make unlimited injectors) - N3X
...
obj
	machinery
		computer
			scan_consolenew
				proc
					all_dna_blocks(var/list/buffer)
						var
							list
								arr = list()
...
obj
	machinery
		computer
			scan_consolenew
				proc
					setInjectorBlock(var/obj/item/weapon/dnainjector/I, var/blk, var/datum/dna2/record/buffer)
						var
							pos = findtext(blk,":")
						if(!pos) return 0
						var
							id = text2num(copytext(blk,1,pos))
						if(!id) return 0
						I.block = id
						I.buf = buffer
						return 1
...
obj
	machinery
		computer
			scan_consolenew
				process() //not really used right now
			...
			scan_consolenew
				Topic(href, href_list)
					...
					if(href_list["pulseRadiation"])
						...
						var
							lock_state = src.connected.locked
						...
						var
							radiation = (((src.radiation_intensity*3)+src.radiation_duration*3) / connected.damage_coeff)
					...
					if(href_list["selectUIBlock"] && href_list["selectUISubblock"]) // This chunk of code updates selected block / sub-block based on click
						var
							select_block = text2num(href_list["selectUIBlock"])
						var
							select_subblock = text2num(href_list["selectUISubblock"])
					...
					if(href_list["pulseUIRadiation"])
						var
							block = src.connected.occupant.dna.GetUISubBlock(src.selected_ui_block,src.selected_ui_subblock)
						...
						if(prob((80 + (src.radiation_duration / 2))))
							var
								radiation = (src.radiation_intensity+src.radiation_duration)
							src.connected.occupant.apply_effect(radiation,IRRADIATE,0)
							...
							var
								radiation = ((src.radiation_intensity*2)+src.radiation_duration)
							src.connected.occupant.apply_effect(radiation,IRRADIATE,0)
							if(src.connected.radiation_check())
								return 1
					...
					if(href_list["injectRejuvenators"])
						if(!connected.occupant)
							return 0
						var
							inject_amount = round(text2num(href_list["injectRejuvenators"]), 5) // round to nearest 5
					...
					if(href_list["selectSEBlock"] && href_list["selectSESubblock"]) // This chunk of code updates selected block / sub-block based on click (se stands for strutural enzymes)
						var
							select_block = text2num(href_list["selectSEBlock"])
						var
							select_subblock = text2num(href_list["selectSESubblock"])
					...
					if(href_list["pulseSERadiation"])
						...
						if(src.connected.occupant)
							if(prob((80 + ((src.radiation_duration / 2) + (connected.precision_coeff ** 3)))))
								var
									radiation = ((src.radiation_intensity+src.radiation_duration) / connected.damage_coeff)
								src.connected.occupant.apply_effect(radiation,IRRADIATE,0)
								...
								var
									real_SE_block=selected_se_block
								...
								var
									radiation = (((src.radiation_intensity*2)+src.radiation_duration) / connected.damage_coeff)
								src.connected.occupant.apply_effect(radiation,IRRADIATE,0)
					...
					if(href_list["bufferOption"])
						...
						var
							bufferId = text2num(href_list["bufferId"])
						...
						if(bufferOption == "changeLabel")
							var
								datum
									dna2
										record
											buf = src.buffers[bufferId]
							var
								text = sanitize(input(usr, "New Label:", "Edit Label", buf.name) as text|null)
							buf.name = text
						...
						if(bufferOption == "transfer")
							...
							var
								radiation = (rand(20,50) / connected.damage_coeff)
							src.connected.occupant.apply_effect(radiation,IRRADIATE,0)
						...
						if(bufferOption == "createInjector")
							if(injector_ready && !waiting_for_user_input)
								...
								var
									obj
										item
											weapon
												dnainjector
													I = new /obj/item/weapon/dnainjector
								var
									datum
										dna2
											record
								...
								if(href_list["createBlockInjector"])
									...
									var
										blk = input(usr,"Select Block","Block") as null|anything in all_dna_blocks(selectedbuf)
