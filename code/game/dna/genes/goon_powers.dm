					...
					cryokinesis
						cast(list/targets)
							...
							new
								obj
									effect
										self_deleting(C.loc, icon('icons/effects/genetics.dmi', "cryokinesis"))
								...
								var
									obj
										item
											organ
												external
													limb = H.get_organ(user.zone_sel.selecting)
								if(!istype(limb))
					...
					empath
						cast(list/targets)
							...
							for(var/mob/living/carbon/M in targets)
								...
								var
									list
										randomthoughts = list("what to have for lunch","the future","the past","money",
								...
								var
									thoughts = "thinking about [pick(randomthoughts)]"
