				...
				targeted
					morph
						cast(list/targets)
							...
							var
								new_gender = alert(usr, "Please select gender.", "Character Generation", "Male", "Female")
							if(new_gender)
								if(new_gender == "Male")
							...
							var
								new_eyes = input("Please select eye color.", "Character Generation", rgb(M.r_eyes,M.g_eyes,M.b_eyes)) as null|color
							if(new_eyes)
							...
							var
								list
									valid_hairstyles = M.generate_valid_hairstyles()
							var
								new_style = input("Please select hair style", "Character Generation", head_organ.h_style) as null|anything in valid_hairstyles
							...
							var
								new_hair = input("Please select hair color.", "Character Generation", rgb(head_organ.r_hair, head_organ.g_hair, head_organ.b_hair)) as null|color
							if(new_hair)
							...
							var
								list
									valid_facial_hairstyles = M.generate_valid_facial_hairstyles()
							...
							var
								new_facial = input("Please select facial hair color.", "Character Generation", rgb(head_organ.r_facial, head_organ.g_facial, head_organ.b_facial)) as null|color
							if(new_facial)
							...
							if(head_organ.species.bodyflags & HAS_HEAD_ACCESSORY)
								var
									list
										valid_head_accessories = M.generate_valid_head_accessories()
								var
									new_head_accessory = input("Please select head accessory style", "Character Generation", head_organ.ha_style) as null|anything in valid_head_accessories
								if(new_head_accessory)
									head_organ.ha_style = new_head_accessory

								var
									new_head_accessory_colour = input("Please select head accessory colour.", "Character Generation", rgb(head_organ.r_headacc, head_organ.g_headacc, head_organ.b_headacc)) as null|color
							...
							if(M.species.bodyflags & HAS_MARKINGS)
								var
									list
										valid_markings = M.generate_valid_markings()
								var
									new_marking = input("Please select marking style", "Character Generation", M.m_style) as null|anything in valid_markings
								if(new_marking)
									M.m_style = new_marking

								var
									new_marking_colour = input("Please select marking colour.", "Character Generation", rgb(M.r_markings, M.g_markings, M.b_markings)) as null|color
							...
							if(M.species.tail && M.species.bodyflags & HAS_TAIL)
								var
									list
										valid_body_accessories = M.generate_valid_body_accessories()
								if(valid_body_accessories.len > 1) //By default valid_body_accessories will always have at the very least a 'none' entry populating the list, even if the user's species is not present in any of the list items.
									var
										new_body_accessory = input("Please select body accessory style", "Character Generation", M.body_accessory) as null|anything in valid_body_accessories
							...
							if(M.species.bodyflags & HAS_SKIN_TONE)
								var
									new_tone = input("Please select skin tone level: 1-220 (1=albino, 35=caucasian, 150=black, 220='very' black)", "Character Generation", M.s_tone) as null|text
								if(!new_tone)
									new_tone = 35
							...
							if(M.species.bodyflags & HAS_ICON_SKIN_TONE)
								var
									prompt = "Please select skin tone: 1-[M.species.icon_skin_tones.len] ("
								...
								var
									new_tone = input(prompt, "Character Generation", M.s_tone) as null|text
								if(!new_tone)
									new_tone = 0
							...
							if(M.species.bodyflags & HAS_SKIN_COLOR)
								var
									new_body_colour = input("Please select body colour.", "Character Generation", rgb(M.r_skin, M.g_skin, M.b_skin)) as null|color
								if(new_body_colour)
				...
				targeted
					remotetalk
						cast(list/targets)
							if(!ishuman(usr))	return
							var
								say = input("What do you wish to say") as text|null
							if(!say)
								return
							say = strip_html(say)
