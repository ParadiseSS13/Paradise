...
turf
	simulated
		floor
			plating
				metalfoam
					attackby(var/obj/item/C, mob/user, params)
						if(..())
							return 1
						if(istype(C) && C.force)
							...
							var
								smash_prob = max(0, C.force*17 - metal*25) // A crowbar will have a 60% chance of a breakthrough on alum, 35% on iron
							if(prob(smash_prob))
