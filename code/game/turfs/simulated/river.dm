...
/turf/proc/Spread(probability = 30, prob_loss = 25, whitelisted_area)
	...
	for(var/F in RANGE_TURFS(1, src) - src)
		...
		if(!T || (T.density && !ismineralturf(T)) || istype(T, /turf/simulated/floor/indestructible) || istype(T, /turf/simulated/wall/indestructible) || (whitelisted_area && !istype(new_area, whitelisted_area)) || (T.flags & NO_LAVA_GEN) )
			continue
