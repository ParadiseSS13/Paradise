...
/turf/simulated/floor/is_safe()
	...
	if(Z.oxygen() < 16 || Z.toxins() >= 0.05 || Z.carbon_dioxide() >= 10 || Z.sleeping_agent() >= 1 || Z.hydrogen() >= 0.01 || Z.water_vapor() >= 25 || (Z.temperature() <= 270) || (Z.temperature() >= 360) || (pressure <= 20) || (pressure >= 550))
		return FALSE
	return TRUE
