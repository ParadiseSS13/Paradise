	...
	simulated
		wall
			...
			var
				damage = 0
			var
				damage_cap = 100 //Wall will break down to girders if damage reaches this point
			...
			var
				max_temperature = 1800 //K, walls will take damage if they're next to a fire hotter than this
			...
			var
				engraving, engraving_quality //engraving on the wall
		...
		wall
			proc
				update_icon()
					...
					var
						overlay = round(damage / damage_cap * damage_overlays.len) + 1
	...
	simulated
		wall
			proc
				generate_overlays()
					var
						alpha_inc = 256 / damage_overlays.len

					for(var/i = 1; i <= damage_overlays.len; i++)
						var
							image
								img = image(icon = 'icons/turf/walls.dmi', icon_state = "overlay_damage")
...
turf
	simulated
		wall
			adjacent_fire_act(turf/simulated/floor/adj_turf, datum/gas_mixture/adj_air, adj_temp, adj_volume)
...
turf
	simulated
		wall
			proc
				dismantle_wall(devastated=0, explode=0)
					if(istype(src,/turf/simulated/wall/r_wall))
						...
						if(!devastated)
							...
							if(mineral == "metal")
								...
								var
									M = text2path("/obj/item/stack/sheet/mineral/[mineral]")
								new M( src )
								new M( src )
						else
							if(mineral == "metal")
			...
			proc
				rot()
					if(!rotting)
						...
						var
							number_rots = rand(2,3)
						for(var/i=0, i<number_rots, i++)
							var
								obj
									effect
										overlay
											O = new/obj/effect/overlay( src )
							O.name = "Wallrot"
							O.desc = "Ick..."
