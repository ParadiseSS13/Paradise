...
turf
	space
		Entered(atom/movable/A as mob|obj, atom/OL, ignoreRest = 0)
			..()
			...
			if(destination_z && A && (src in A.locs))
				...
				if(isliving(A))
					...
					if(L.pulling)
						var
							turf
								T = get_step(L.loc,turn(A.dir, 180))
						L.pulling.loc = T
...
turf
	space
		proc
			Sandbox_Spacemove(atom/movable/A as mob|obj)
				var
					cur_x
				var
					cur_y
				...
				if(src.x <= 1)
					...
					var
						list
							cur_pos = src.get_global_map_pos()
					if(!cur_pos) return
					cur_x = cur_pos["x"]
					cur_y = cur_pos["y"]
