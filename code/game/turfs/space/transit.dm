turf
	space
		transit
			var
				pushdirection // push things that get caught in the transit tile this direction
...
turf
	space
		transit
			Entered(atom/movable/AM, atom/OldLoc, ignoreRest = 0)
				if(!AM)
					return
				...
				var
					max = world.maxx-TRANSITIONEDGE
				var
					min = 1+TRANSITIONEDGE

				var
					_z = rand(ZLEVEL_CENTCOMM+1,MAX_Z)	//select a random space zlevel
