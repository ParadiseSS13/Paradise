...
proc
	explosion(turf/epicenter, devastation_range, heavy_impact_range, light_impact_range, flash_range, adminlog = 1, ignorecap = 0, flame_range = 0 ,silent = 0)
		...
		spawn(0)
			...
			var
				max_range = max(devastation_range, heavy_impact_range, light_impact_range, flame_range)
			var
				list
					...
					if(M && M.client)
						...
						if(M_turf && M_turf.z == epicenter.z)
							...
							// You hear a far explosion if you're outside the blast radius. Small bombs shouldn't be heard all over the station.
								...
								var
									far_volume = Clamp(far_dist, 30, 50) // Volume is based on explosion size and dist
								far_volume += (dist <= far_dist * 0.5 ? 50 : 0) // add 50 volume if the mob is pretty close to the explosion
								M.playsound_local(epicenter, 'sound/effects/explosionfar.ogg', far_volume, 1, frequency, falloff = 5)

				var
					close = range(world.view+round(devastation_range,1), epicenter)
				...
				var
					datum
						effect
							system
								explosion
									E = new/datum/effect/system/explosion()
				E.set_up(epicenter)
				E.start()
			...
			var
				list
					affected_turfs = spiral_range_turfs(max_range, epicenter)
			...
			if(throw_dist > 0)
				...
				for(var/obj/item/I in T)
					spawn(0) //Simultaneously not one at a time
						if(I && !I.anchored)
							var
								throw_mult = 0.5 + (0.5 * rand()) // Between 0.5 and 1.0
							var
								throw_range = round((throw_dist + 1) * throw_mult) // Roughly 50% to 100% of throw_dist
							if(throw_range > 0)
								var
									turf
...
client
	proc
		check_bomb_impacts()
			...
			var
				newmode = alert("Use reactionary explosions?","Check Bomb Impact", "Yes", "No")
			var
				turf
			...
			var
				list
					choices = list("Small Bomb","Medium Bomb","Big Bomb","Custom Bomb")
			var
				choice = input("Bomb Size?") in choices
			switch(choice)
				if(null)
					return 0
				if("Small Bomb")
