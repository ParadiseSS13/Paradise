...
/proc/explosion(turf/epicenter, devastation_range, heavy_impact_range, light_impact_range, flash_range, adminlog = 1, ignorecap = 0, flame_range = 0 ,silent = 0, smoke = 1, cause = null, breach = TRUE)
	...
	var/newbombcap = 1.5 // este factor multiplica al cap de bombas de los admin. editar al gusto
	...
	if (MAX_EX_LIGHT_RANGE <= limnewbombcap) // esto define al nuevo cap de bombas (si tiene sentido)
		MAX_EX_DEVESTATION_RANGE1 = MAX_EX_DEVESTATION_RANGE * newbombcap
		MAX_EX_HEAVY_RANGE1 = MAX_EX_HEAVY_RANGE * newbombcap
