							...
							var
								obj
									effect
										spider
											spiderling
												S = new /obj/effect/spider/spiderling(src.loc)
							S.faction = faction
					...
					if(travelling_in_vent)
						...
						if(get_dist(src, entry_vent) <= 1)
							...
							spawn(rand(20,60))
								loc = exit_vent
								var
									travel_time = round(get_dist(loc, exit_vent.loc) / 2)
								spawn(travel_time)
						...
						if(amount_grown >= 100)
							...
							if(player_spiders && !selecting_player)
								...
								spawn()
									var
										list
											candidates = pollCandidates("Do you want to play as a spider?", ROLE_ALIEN, 1)
