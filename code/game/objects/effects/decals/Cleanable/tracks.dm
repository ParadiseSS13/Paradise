...
/obj/effect/decal/cleanable/blood/footprints/Crossed(atom/movable/O, oldloc)
	...
	if(ishuman(O))
		...
		if(S && S.bloody_shoes[blood_state] && S.blood_color == basecolor)
			...
			S.bloody_shoes["blood_alpha"] = base_alpha
			if(!S.blood_DNA)
				S.blood_DNA = list()
			...
			H.bloody_feet["blood_alpha"] = base_alpha
			if(!H.feet_blood_DNA)
...
/proc/createFootprintsFrom(atom/movable/A, dir, turf/T)
	...
	if(ishuman(A))
		...
		FP.base_alpha = H.bloody_feet["blood_alpha"]
		if(H.blood_DNA)
		...
		FP.base_alpha = S.bloody_shoes["blood_alpha"]
		if(S.blood_DNA)
