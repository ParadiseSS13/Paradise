...
datum
	effect
		proc
			fadeOut(var/atom/A, var/frames = 16)
...
var
	datum
		effect
			system
				steam_spread
					steam = new /datum/effect/system/steam_spread() -- creates new system
							...
							var
								obj
									effect
										effect
											steam
												steam = new /obj/effect/effect/steam(src.location)
							var
								direction
							if(src.cardinals)
							...
							var
								obj
									effect
										effect
											sparks
												sparks = new /obj/effect/effect/sparks(src.location)
							src.total_sparks++
							var
								direction
							if(src.cardinals)
							...
							var
								obj
									effect
										effect
											harmless_smoke
												smoke = new /obj/effect/effect/harmless_smoke(src.location)
							src.total_smoke++
							var
...
obj
	effect
		effect
			bad_smoke
				Crossed(mob/living/carbon/M as mob )
					..()
							...
							var
								obj
									effect
										effect
											bad_smoke
												smoke = new /obj/effect/effect/bad_smoke(src.location)
							src.total_smoke++
							var
								direction = src.direction
							if(!direction)
			...
			chem_smoke_spread
				...
				set_up(var/datum/reagents/carry = null, n = 5, c = 0, loca, direct, silent = 0)
					...
					if(!silent)
						...
						var
							where = "[A.name] | [location.x], [location.y]"
						var
							whereLink = "<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[location.x];Y=[location.y];Z=[location.z]'>[where]</a>"

						if(carry && carry.my_atom)
							if(carry.my_atom.fingerprintslast)
								var
									mob
										M = get_mob_by_key(carry.my_atom.fingerprintslast)
								var
									more = ""
								if(M)
									more = " "
					...
					var
						obj
							effect
								effect
									chem_smoke
										smokeholder = new /obj/effect/effect/chem_smoke(src.location)
							...
							var
								obj
									effect
										effect
											chem_smoke
												smoke = new /obj/effect/effect/chem_smoke(src.location)
							src.total_smoke++
							var
								direction = src.direction
							if(!direction)
							...
							var
								obj
									effect
										effect
											sleep_smoke
												smoke = new /obj/effect/effect/sleep_smoke(src.location)
							src.total_smoke++
							var
								direction = src.direction
							if(!direction)
...
obj
	effect
		effect
			mustard_gas
				Crossed(mob/living/carbon/human/R as mob )
					..()
							...
							var
								obj
									effect
										effect
											mustard_gas
												smoke = new /obj/effect/effect/mustard_gas(src.location)
							src.total_smoke++
							var
								direction = src.direction
							if(!direction)
...
datum
	effect
		system
			ion_trail_follow
				start() //Whoever is responsible for this abomination of code should become an hero
					if(!src.on)
						src.on = 1
						src.processing = 1
					if(src.processing)
								...
								var
									obj
										effect
											effect
												ion_trails
													I = new /obj/effect/effect/ion_trails(src.oldposition)
		...
		system
			ion_trail_follow
				space_trail
					var
						turf
							oldloc // secondary ion trail loc
					var
						turf
							currloc
										...
										var
											obj
												effect
													effect
														ion_trails
															II = new /obj/effect/effect/ion_trails(src.oldloc)
										//src.oldposition = T
										I.dir = src.holder.dir
								...
								var
									obj
										effect
											effect
												steam
													I = new /obj/effect/effect/steam(src.oldposition)
								src.number++
					...
					if(!metal && reagents)
						...
						for(var/atom/A in oview(1, src))
							...
							if(reagents.total_volume)
								var
									fraction = 5 / reagents.total_volume
						...
						if(M.slip("foam", 5, 2))
							if(reagents)
								for(var/reagent_id in reagents.reagent_list)
									var
										amount = M.reagents.get_reagent_amount(reagent_id)
									if(amount < 25)
	...
	effect
		system
			foam_spread
				...
				var
					list
						carried_reagents	// the IDs of reagents present when the foam was mixed
				var
				...
				var
					list
						banned_reagents = list("smoke_powder", "fluorosurfactant", "stimulants")
							...
							if(carried_reagents)
								for(var/id in carried_reagents)
									...
									var
										datum
											reagent
												reagent_volume = carried_reagents[id]
