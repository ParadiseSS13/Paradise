...
/obj/effect/acid/alien/process(obj/item/W as obj)
	. = ..()
	if(.)
		if(prob(45))
			playsound(loc, 'sound/items/welder.ogg', 100, 1)
		...
		if(target_strength <= 0)
			W.visible_message("<span class='warning'>[target] collapses under its own weight into a puddle of goop and undigested debris!</span>")
			W.acid_melt()
			qdel(src)
		else
