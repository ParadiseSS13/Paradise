	...
	effect
		anomaly
			New()
				...
				var
					new_frequency = sanitize_frequency(rand(PUBLIC_LOW_FREQ, PUBLIC_HIGH_FREQ))
				...
				proc
					gravShock(var/mob/A)
						if(boing && isliving(A) && !A.stat)
							...
							var
								atom
									target = get_edge_target_turf(A, get_dir(src, get_step_away(A, src)))
...
obj
	effect
		anomaly
			bhole
				proc
					grav(var/r, var/ex_act_force, var/pull_chance, var/turf_removal_chance)
						for(var/t = -r, t < r, t++)
...
obj
	effect
		anomaly
			bhole
				proc
					affect_coord(var/x, var/y, var/ex_act_force, var/pull_chance, var/turf_removal_chance)
