...
/obj/item/mail_scanner
	name = "mail scanner"
	desc = "An Orion Express-brand mail scanner, this device allows you to log mail deliveries in exchange for financial compensation."
	force = 0
	throwforce = 0
...
/obj/item/mail_scanner/afterattack__legacy__attackchain(atom/A, mob/user)
	...
	if(isliving(A))
		...
		GLOB.station_money_database.credit_account(SSeconomy.cargo_account, MAIL_DELIVERY_BONUS, "Mail Delivery Compensation", "Orion Express Delivery Services", supress_log = FALSE)
