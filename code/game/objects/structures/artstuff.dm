...
client
	var
		viewingCanvas = 0 //If this is 1, as soon as client /TRIES/ to move the view resets.
...
var
	global
		list
			globalBlankCanvases[AMT_OF_CANVASES]
	...
	item
		weapon
			canvas
				...
				icon_state = "11x11"
				var
					whichGlobalBackup = 1 //List index
				...
				proc
					getGlobalBackup()
						...
						if(globalBlankCanvases[whichGlobalBackup])
							...
							var
								icon
									I = icon(initial(icon),initial(icon_state))
		...
		weapon
			canvas
				attackby(var/obj/item/I, var/mob/user, params)
					...
					var
						pixX = text2num(click_params["icon-x"])
					var
						pixY = text2num(click_params["icon-y"])
					...
					var
						icon
							masterpiece = icon(icon,icon_state)
								...
								item
									toy
										crayon
											C = I
						var
							pix = masterpiece.GetPixel(pixX, pixY)
