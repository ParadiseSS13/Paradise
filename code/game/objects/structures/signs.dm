...
obj
	structure
		sign
			attackby(obj/item/tool as obj, mob/user as mob)	//deconstruction
...
obj
	item
		sign
			attackby(obj/item/tool as obj, mob/user as mob)	//construction
				if(istype(tool, /obj/item/weapon/screwdriver) && isturf(user.loc))
					var
						direction = input("In which direction?", "Select direction.") in list("North", "East", "South", "West", "Cancel")
...
obj
	structure
		sign
			science			//These 3 have multiple types, just var-edit the icon_state to whatever one you want on the map
