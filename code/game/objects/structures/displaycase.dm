...
var
	global
		list
			captain_display_cases = list()

hook
	captain_spawned
		proc
			displaycase(mob/living/carbon/human/captain)
				if(!captain_display_cases.len)
					return 1
				var
					fingerprint = captain.get_full_print()
						...
						device
							assembly
								prox_sensor
									sensor = null
			var
				state = DISPLAYCASE_FRAME_CIRCUIT
								...
								obj
									item
										stack
											sheet
												glass
													G = new /obj/item/stack/sheet/glass(T)
							G.amount = 5
							qdel(src)
		...
		displaycase
			update_icon()
				...
				if(occupant)
					var
						icon
							occupant_icon=getFlatIcon(occupant)
