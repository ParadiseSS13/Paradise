								...
								if(!E.teleporting)
									...
									for(var/obj/machinery/telepad_cargo/R in world)
										if(R.stage == 0)
											...
											L[tmpname] = R
									var
										desc = input("Please select a telepad.", "RCS") in L
									E.pad = L[desc]
...
obj
	structure
		closet
			crate
				proc
					notifyRecipient(var/destination)
						var
							msg = "[capitalize(name)] has arrived at [destination]."
		...
		closet
			crate
				secure
					...
					var
						redlight = "securecrater"
					var
						greenlight = "securecrateg"
					var
						sparks = "securecratesparks"
					var
						emag = "securecrateemag"
					var
						broken = 0
					var
						locked = 1
