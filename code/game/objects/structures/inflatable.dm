			...
			attack_self(mob/user)
				...
				var
					obj
						structure
							inflatable
								R = new /obj/structure/inflatable(user.loc)
...
obj
	structure
		inflatable
			proc
				attack_generic(mob/user as mob, damage = 0)	//used by attack_alien, attack_animal, and attack_slime
					health -= damage
					if(health <= 0)
						...
						var
							obj
								item
									inflatable
										torn
											R = new /obj/item/inflatable/torn(loc)
						src.transfer_fingerprints_to(R)
						qdel(src)
					else
					...
					var
						obj
							structure
								inflatable
									door
										R = new /obj/structure/inflatable/door(user.loc)
					src.transfer_fingerprints_to(R)
					R.add_fingerprint(user)
					qdel(src)
...
obj
	structure
		inflatable
			door //Based on mineral door code
				name = "inflatable door"
				density = 1
				anchored = 1
				opacity = 0
							...
							obj
								item
									inflatable
										door
											torn
												R = new /obj/item/inflatable/door/torn(loc)
						src.transfer_fingerprints_to(R)
						qdel(src)
					else
						visible_message("[src] slowly deflates.")
							...
							var
								obj
									item
										inflatable
											door
												R = new /obj/item/inflatable/door(loc)
							src.transfer_fingerprints_to(R)
							qdel(src)
					air_update_turf(1)
