...
obj
	structure
		safe
			...
			density = 1
			var
				open = 0		//is the safe open?
			var
				tumbler_1_pos	//the tumbler position- from 0 to 72
			var
				tumbler_1_open	//the tumbler position to open at- 0 to 72
			var
				tumbler_2_pos
			var
				tumbler_2_open
			var
				dial = 0		//where is the dial pointing?
			var
				space = 0		//the combined w_class of everything in the safe
			var
				maxspace = 24	//the maximum combined w_class of stuff in the safe
...
obj
	structure
		safe
			proc
				check_unlocked(mob/user, canhear)
					if(user && canhear)
			...
			Topic(href, href_list)
				...
				if(href_list["retrieve"])
					...
					var
						obj
							item
								P = locate(href_list["retrieve"]) in src
					if(open)
