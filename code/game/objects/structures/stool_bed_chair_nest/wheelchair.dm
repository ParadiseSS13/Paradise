				...
				chair
					wheelchair
						handle_rotation()
							...
							var
								image
									O = image(icon = 'icons/obj/objects.dmi', icon_state = "w_overlay", layer = FLY_LAYER, dir = src.dir)
							overlays += O
							if(buckled_mob)
								buckled_mob.dir = dir
								...
								var
									mob
										living
											thedriver = user
								var
									mob_delay = thedriver.movement_delay()
								if(mob_delay > 0)
									...
									var
										obj
											item
												organ
													external
														l_hand = driver.get_organ("l_hand")
									var
										obj
											item
												organ
													external
														r_hand = driver.get_organ("r_hand")
					...
					wheelchair
						bike
							handle_rotation()
								...
								var
									image
										O = image(icon = 'icons/vehicles/motorcycle.dmi', icon_state = "motorcycle_overlay_4d", layer = FLY_LAYER, dir = src.dir)
								overlays += O
								if(buckled_mob)
									buckled_mob.dir = dir
