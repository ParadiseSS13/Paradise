...
obj
	structure
		toilet
			...
			anchored = 1
			var
				open = 0			//if the lid is up
			var
				cistern = 0			//if the cistern bit is open
			var
				w_items = 0			//the combined w_class of all the items in the cistern
			var
				mob
					living
						swirlie = null	//the mob being given a swirlie
				...
				obj
					effect
						mist
							mymist = null
			var
				ismist = 0				//needs a var so we can make it linger~
			var
				watertemp = "normal"	//freezing, normal, or boiling
			var
				mobpresent = 0		//true if there is a mob on the shower's loc, this is to ease process()
...
obj
	machinery
		shower
			update_icon()	//this is terribly unreadable, but basically it makes the shower mist up
...
obj
	machinery
		shower
			proc
				wash(atom/movable/O as obj|mob)
					if(!on) return
...
obj
	structure
		sink
			...
			anchored = 1
			var
				busy = 0 	//Something's being washed at the moment
		...
		sink
			attack_hand(mob/user as mob)
				...
				if(busy)
					...
					return
				var
					selected_area = parse_zone(user.zone_sel.selecting)
				var
					washing_face = 0
