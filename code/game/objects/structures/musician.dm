	...
	song
		...
		var
			list
				lines = new()
		var
			tempo = 5					// delay between notes

		var
			playing = 0					// if we're playing
		var
			help = 0					// if help is open
		var
			repeat = 0					// number of times remaining to repeat
		var
			max_repeat = 10				// maximum times we can repeat

		var
			instrumentDir = "piano"		// the folder with the sounds
		var
			instrumentExt = "ogg"		// the file extension
		var
			obj
				instrumentObj = null	// the associated obj playing the sound
...
datum
	song
		proc
			playnote(var/note, var/acc as text, var/oct)
				...
				// now generate name
				var
					soundfile = "sound/[instrumentDir]/[ascii2text(note+64)][acc][oct].[instrumentExt]"
...
//					to_chat(world, "Parse: [copytext(note,1,2)]")
					var
						cur_note = text2ascii(note) - 96
					if(cur_note < 1 || cur_note > 7)
						continue
					for(var/i=2 to lentext(note))
						var
							ni = copytext(note,i,i+1)
						if(!text2num(ni))
							if(ni == "#" || ni == "b" || ni == "n")
								cur_acc[cur_note] = ni
							else if(ni == "s")
		...
		Topic(href, href_list)
			...
			if(href_list["newsong"])
				...
				do
					...
					if(lentext(t) >= 3072)
						var
							cont = input(usr, "Your message is too long! Would you like to continue editing it?", "", "yes") in list("yes", "no")
						if(cont == "no")
							break
			...
			if(href_list["repeat"]) //Changing this from a toggle to a number of repeats to avoid infinite loops.
				...
				var
					num = round(text2num(href_list["insertline"]))
				if(num < 1 || num > lines.len + 1)
					return

				var
					newline = html_encode(input("Enter your line: ", instrumentObj.name) as text|null)
				...
				var
					num = round(text2num(href_list["deleteline"]))
				...
				var
					num = round(text2num(href_list["modifyline"]))
				var
					content = html_encode(input("Enter your line: ", instrumentObj.name, lines[num]) as text|null)
