	...
	structure
		transit_tube
			...
			var
				global
					list
						tube_dir_list = list(NORTH, SOUTH, EAST, WEST, NORTHEAST, NORTHWEST, SOUTHEAST, SOUTHWEST)
			...
			proc
				get_exit(in_dir)
					var
						near_dir = 0
					var
						in_dir_cw = turn(in_dir, -45)
					var
						in_dir_ccw = turn(in_dir, 45)
			...
			proc
				init_dirs_automatic()
					...
					for(var/direction in tube_dir_list)
						var
							location = get_step(loc, direction)
...
obj
	structure
		transit_tube
			proc
				select_automatic_dirs(connected)
...
obj
	structure
		transit_tube
			proc
				select_automatic_icon_state(directions)
...
obj
	structure
		transit_tube
			proc
				generate_automatic_corners(directions)
					for(var/direction in directions)
...
obj
	structure
		transit_tube
			proc
				create_automatic_decorative_corner(location, direction)
					var
						state = "D-[dir2text_short(direction)]"
			...
			proc
				parse_dirs(text)
					...
					for(var/text_part in split_text)
						var
							direction = text2dir_extended(text_part)
