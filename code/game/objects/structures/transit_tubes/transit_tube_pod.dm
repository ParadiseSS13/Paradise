...
obj
	structure
		transit_tube_pod
			proc
				follow_tube(var/reverse_launch)
					if(moving)
						return
		...
		transit_tube_pod
			proc
				mix_air()
					...
					var
						int_pressure = air_contents.return_pressure()
					var
					...
					//  SS13.
					var
						transfer_in = max(0.1, 0.5 * (env_pressure - int_pressure) / total_pressure)
					var
						transfer_out = max(0.1, 0.3 * (int_pressure - env_pressure) / total_pressure)

					var
						datum
							gas_mixture
								from_env = loc.remove_air(environment.total_moles() * transfer_in)
					var
						datum
							gas_mixture
								from_int = air_contents.remove(air_contents.total_moles() * transfer_out)
