		...
		if(prob(50) && anchored && !broken)
			...
			if(O.throwforce != 0)//don't want to let people spam tesla bolts, this way it will break after time
				...
				if(C)
					...
					basic_tesla_zap(src, 3, C.get_queued_available_power() * 0.01, ZAP_MOB_DAMAGE | ZAP_OBJ_DAMAGE | ZAP_MOB_STUN | ZAP_ALLOW_DUPLICATES) //Zap for 1/100 of the amount of power. At a million watts in the grid, it will be as powerful as a tesla revolver shot.
