	...
	if(damage * 2 >= obj_integrity && shardtype && !mob_hurt)
		...
		for(var/obj/structure/grille/G in T.contents)
			...
			if(SC)
				...
				basic_tesla_zap(G, 3, SC.get_queued_available_power() * 0.05, ZAP_MOB_DAMAGE | ZAP_OBJ_DAMAGE | ZAP_MOB_STUN | ZAP_ALLOW_DUPLICATES) //Zap for 1/20 of the amount of power, because I am evil.
