obj
	structure
		sign
			barsign // All Signs are 64 by 32 pixels, they take two tiles
				name = "Bar Sign"
...
obj
	structure
		sign
			barsign
				proc
					set_sign(var/datum/barsign/sign)
						if(!istype(sign))
							return
		...
		sign
			barsign
				proc
					pick_sign()
						var
							picked_name = input("Available Signage", "Bar Sign") as null|anything in barsigns
						if(!picked_name)
							return
