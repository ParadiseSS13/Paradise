...
/obj/machinery/barsign/Initialize(mapload, direction, building=FALSE)
	. = ..()
	...
	if(building)
		playsound(src, 'sound/items/deconstruct.ogg', 10, TRUE)
...
/obj/machinery/barsign/update_icon_state()
	if((stat & BROKEN) && !istype(current_sign, /datum/barsign/hiddensigns/signbroken))
		...
		return
	if((stat & NOPOWER) && !istype(current_sign, /datum/barsign/hiddensigns/signoff))
		set_sign(new /datum/barsign/hiddensigns/signoff)
		return
	if((stat & EMPED) && !istype(current_sign, /datum/barsign/hiddensigns/empbarsign))
...
/obj/machinery/barsign/update_overlays()
	...
	if(power_state == IDLE_POWER_USE || stat & (BROKEN|NOPOWER) || !current_sign)
		return
...
/obj/machinery/barsign/proc/turn_off()
	if((stat & (BROKEN|EMPED)) || !is_on() || build_stage < BARSIGN_COMPLETE)
		return FALSE
	save_sign()
	set_sign(new /datum/barsign/hiddensigns/signoff)
...
/obj/machinery/barsign/proc/turn_on()
	if((stat & (BROKEN|NOPOWER)) || build_stage < BARSIGN_COMPLETE)
		return FALSE
	if(panel_open)
		return FALSE
	if(prev_sign)
...
/obj/machinery/barsign/wrench_act(mob/living/user, obj/item/I)
	...
	if(!(flags & NODECONSTRUCT))
		...
		new /obj/item/mounted/frame/barsign(get_turf(user))
		I.play_tool_sound(user, I.tool_volume)
		qdel(src)
...
/obj/machinery/barsign/deconstruct(disassembled)
...
/obj/item/mounted/frame/barsign
	name = "bar sign frame"
	desc = "A fold-out bar sign frame.  Just attach it to a wall and the device will automatically extend into a full-sized frame."
	icon = 'icons/obj/items.dmi'
	lefthand_file = 'icons/mob/inhands/items_lefthand.dmi'
	righthand_file = 'icons/mob/inhands/items_righthand.dmi'
	icon_state = "barsign_frame_item"
	item_state = "rack_parts"
	w_class = WEIGHT_CLASS_HUGE

	metal_sheets_refunded = 4

/obj/item/mounted/frame/barsign/do_build(turf/on_wall, mob/user)
	new /obj/machinery/barsign(get_turf(src), get_dir(user, on_wall), TRUE)
	to_chat(user, "<span class='notice'>You place the device on the wall and it folds out into a frame!</span>")
	qdel(src)

/obj/item/mounted/frame/barsign/wrench_act(mob/living/user, obj/item/I)
	. = TRUE
	if(I.tool_behaviour != TOOL_WRENCH)
		return
	if(!I.use_tool(src, user, 0, volume = 0))
...
/obj/item/mounted/frame/barsign/try_build(turf/on_wall, mob/user, proximity_flag) //checks
	if(!on_wall || !user)
		return
	...
	// Signs are two tiles, so make sure they are both clear
	if((gotwallitem(user_turf, get_dir(on_wall,user))) || gotwallitem(get_step(user_turf, EAST), get_dir(on_wall,user)))
...
/obj/machinery/barsign/free_access
	req_access = list()
