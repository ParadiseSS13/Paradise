					...
					for(var/direction in list(turn(dir,90), turn(dir,-90)) )
						var
							obj
								structure
									table
										T = locate(/obj/structure/table,get_step(src,direction))
						if(T && T.flipped)
							type++
							if(type == 1)
...
obj
	structure
		table
			proc
				check_cover(obj/item/projectile/P, turf/from)
					var
						turf
							cover = flipped ? get_turf(src) : get_step(loc, get_dir(from, loc))
			...
			attackby(obj/item/W as obj, mob/user as mob, params)
				...
				if(!(W.flags & ABSTRACT))
					if(user.drop_item())
						...
						var
							list
								click_params = params2list(params)
...
obj
	structure
		table
			proc
				straight_table_check(var/direction)
					var
						obj
							structure
								table
		...
		table
			proc
				flip(var/direction)
					...
					var
						list
							targets = list(get_step(src,dir),get_step(src,turn(dir, 45)),get_step(src,turn(dir, -45)))
						...
						if(locate(/obj/structure/table,get_step(src,D)))
							var
								obj
									structure
										table
											T = locate(/obj/structure/table,get_step(src,D))
							T.flip(direction)
					update_icon()
...
obj
	structure
		table
			woodentable
				poker //No specialties, Just a mapping object.
					name = "gambling table"
...
obj
	structure
		table
			glass
				proc
					collapse() //glass table collapse is called twice in this code, more efficent to just have a proc
