					...
					item
						weapon
							airlock_electronics
								electronics = null
			var
				airlock_type = "" //the type path of the airlock once completed
			var
				glass_type = "/glass"
			var
				glass = 0 // 0 = glass can be installed. -1 = glass can't be installed. 1 = glass is already installed. Text = mineral plating is installed instead.
			var
				created_name = null
		...
		door_assembly
			attackby(obj/item/W as obj, mob/user as mob, params)
				if(istype(W, /obj/item/weapon/pen))
					var
						t = copytext(stripped_input(user, "Enter the name for the door.", src.name, src.created_name),1,MAX_NAME_LEN)
					if(!t)	return
					...
					if(WT.remove_fuel(0, user))
						...
						if(istext(glass))
							...
							if(do_after(user, 40, target = src))
								...
								var
									M = text2path("/obj/item/stack/sheet/mineral/[glass]")
								new M(src.loc, 2)
								glass = 0
						else if(glass == 1)
