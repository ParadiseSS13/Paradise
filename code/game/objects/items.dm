obj
	item
		...
		var
			discrete = 0 // used in item_attack.dm to make an item not show an attack message to viewers
		var
			no_embed = 0 // For use in item_attack.dm
		var
			image
				blood_overlay = null //this saves our blood splatter overlay, which will be processed not to go over the edges of the sprite
		var
			blood_overlay_color = null
		var
			item_state = null
		var
		...
		var
			w_class = 3.0
		var
			slot_flags = 0		//This is used to determine on which slots an item can fit.
...
//	causeerrorheresoifixthis
	...
	var
		heat_protection = 0 //flags which determine which body parts are protected from heat. Use the HEAD, UPPER_TORSO, LOWER_TORSO, etc. flags. See setup.dm
	var
		cold_protection = 0 //flags which determine which body parts are protected from cold. Use the HEAD, UPPER_TORSO, LOWER_TORSO, etc. flags. See setup.dm
	var
	...
	var
		action_button_name //It is also the text which gets displayed on the action button. If not set it defaults to 'Use [name]'. If it's not set, there'll be no button.
	var
		action_button_custom_type = null
	var
		datum
			action
				item_action
	...
	var
		item_color = null
	var
		body_parts_covered = 0 //see setup.dm for appropriate bit flags
	//var/heat_transfer_coefficient = 1 //0 prevents all transfers, 1 is invisible
	var
		gas_transfer_coefficient = 1 // for leaking gas from turf to mask and vice-versa (for masks right now, but at some point, i'd like to include space helmets)
	var
		permeability_coefficient = 1 // for chemicals/diseases
	var
		siemens_coefficient = 1 // for electrical admittance/conductance (electrocution checks and shit)
	var
		slowdown = 0 // How much clothing is slowing you down. Negative values speeds you up
	var
		armor = list(melee = 0, bullet = 0, laser = 0, energy = 0, bomb = 0, bio = 0, rad = 0)
	var
		armour_penetration = 0 //percentage of armour effectiveness to remove
	var
		list
			allowed = null //suit storage stuff.
	var
		obj
			item
				device
					uplink
						hidden
							hidden_uplink = null // All items can have an uplink hidden inside, just remember to add the triggers.

	var
		needs_permit = 0			//Used by security bots to determine if this item is safe for public use.

	var
		strip_delay = DEFAULT_ITEM_STRIP_DELAY
	var
		put_on_delay = DEFAULT_ITEM_PUTON_DELAY
	var
		breakouttime = 0

	var
		flags_size = 0 //flag, primarily used for clothing to determine if a fatty can wear something or not.
	...
	*/
	var
		list
			sprite_sheets = null
	var
		icon_override = null  //Used to override hardcoded clothing dmis in human clothing proc.
	var
		sprite_sheets_obj = null //Used to override hardcoded clothing inventory object dmis in human clothing proc.
	var
		list
			species_fit = null //This object has a different appearance when worn by these species
		...
		examine(mob/user, var/distance = -1)
			...
			if(user.research_scanner) //Mob has a research scanner active.
				...
				if(origin_tech)
					...
					var
						list
							techlvls = params2list(origin_tech)
...
obj
	item
		proc
			talk_into(mob/M as mob, var/text, var/channel=null)
				return
...
obj
	item
		proc
			on_exit_storage(obj/item/weapon/storage/S as obj)
				return
...
obj
	item
		proc
			on_enter_storage(obj/item/weapon/storage/S as obj)
				return
...
obj
	item
		proc
			IsReflect(var/def_zone) //This proc determines if and at what% an object will reflect energy projectiles if it's in l_hand,r_hand or wear_suit
				return 0
...
obj
	item
		proc
			eyestab(mob/living/carbon/M as mob, mob/living/carbon/user as mob)
						...
						obj
							item
								organ
									internal
										eyes
											eyes = H.get_int_organ(/obj/item/organ/internal/eyes)
...
obj
	item
		proc
			remove_item_from_storage(atom/newLoc) //please use this if you're going to snowflake an item out of a obj/item/weapon/storage
				if(!newLoc)
					return 0
