...
/obj/item/proc/update_slot_icon()
	...
	if(flags & SLOT_HUD_OCLOTHING)
		owner.update_inv_wear_suit()
	if(flags & SLOT_HUD_ICLOTHING)
		owner.update_inv_w_uniform()
	if(flags & SLOT_HUD_GLOVES)
		owner.update_inv_gloves()
	if(flags & SLOT_HUD_EYES)
		owner.update_inv_glasses()
	if(flags & SLOT_HUD_BOTH_EARS)
		owner.update_inv_ears()
	if(flags & SLOT_HUD_MASK)
		owner.update_inv_wear_mask()
	if(flags & SLOT_HUD_HEAD)
		owner.update_inv_head()
	if(flags & SLOT_HUD_FEET)
		owner.update_inv_shoes()
	if(flags & SLOT_HUD_ID)
		owner.update_inv_wear_id()
	if(flags & SLOT_HUD_BELT)
		owner.update_inv_belt()
	if(flags & SLOT_HUD_BACK)
		owner.update_inv_back()
	if(flags & SLOT_HUD_PDA)
