	...
	item
		target
			bullet_act(var/obj/item/projectile/Proj)
				var
					p_x = Proj.p_x + pick(0,0,0,0,0,-1,1) // really ugly way of coding "sometimes offset Proj.p_x!"
				var
					p_y = Proj.p_y + pick(0,0,0,0,0,-1,1)
				var
					decaltype = 1 // 1 - scorch, 2 - bullet
					...
					if(Proj.damage >= 10 && bulletholes.len <= 35) // maximum of 35 bullet holes
						if(decaltype == 2) // bullet
							if(prob(Proj.damage+30)) // bullets make holes more commonly!
								new
									datum
										bullethole(src, bmark.pixel_x, bmark.pixel_y) // create new bullet hole
						else // Lasers!
