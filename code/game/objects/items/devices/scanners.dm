		...
		device
			healthanalyzer
				attack(mob/living/M as mob, mob/living/user as mob)
					...
					var
						fake_oxy = max(rand(1,40), M.getOxyLoss(), (300 - (M.getToxLoss() + M.getFireLoss() + M.getBruteLoss())))
					var
						OX = M.getOxyLoss() > 50 	? 	"<b>[M.getOxyLoss()]</b>" 		: M.getOxyLoss()
					var
						TX = M.getToxLoss() > 50 	? 	"<b>[M.getToxLoss()]</b>" 		: M.getToxLoss()
					var
						BU = M.getFireLoss() > 50 	? 	"<b>[M.getFireLoss()]</b>" 		: M.getFireLoss()
					var
						BR = M.getBruteLoss() > 50 	? 	"<b>[M.getBruteLoss()]</b>" 	: M.getBruteLoss()
					...
					if(ishuman(M))
						...
						if(H.vessel)
							var
								blood_volume = round(M:vessel.get_reagent_amount("blood"))
							var
				...
				afterattack(obj/O, mob/user as mob)
					...
					if(!isnull(O.reagents))
						...
						if(O.reagents.reagent_list.len > 0)
							var
								one_percent = O.reagents.total_volume / 100
