		...
		device
			floor_painter
				...
				var
					static
						list
							allowed_states = list("arrival", "arrivalcorner", "bar", "barber", "blackcorner", "blue", "bluecorner", 
...
obj
	item
		device
			floor_painter
				afterattack(var/atom/A, var/mob/user, proximity, params)
					if(!proximity)
						return
			...
			floor_painter
				interact(mob/user as mob)
					...
					var
						datum
							browser
								popup = new(user, "floor_painter", name, 225, 300)
			...
			floor_painter
				Topic(href, href_list)
					...
					if(href_list["choose_state"])
						var
							state = input("Please select a style", "[src]") as null|anything in allowed_states
						if(state)
							floor_state = state
							floor_dir = SOUTH // Reset dir, because some icon_states might not have that dir.
					if(href_list["choose_dir"])
						var
							seldir = input("Please select a direction", "[src]") as null|anything in list("north", "south", "east", "west", "northeast", "northwest", "southeast", "southwest")
						if(seldir)
							floor_dir = text2dir(seldir)
					if(href_list["cycleleft"])
						var
							index = allowed_states.Find(floor_state)
						index--
						if(index < 1)
