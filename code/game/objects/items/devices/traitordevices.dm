...
obj
	item
		device
			batterer
				attack_self(mob/living/carbon/user, flag = 0, emp = 0)
					if(!user)
						return
					if(times_used >= max_uses)
	...
	item
		device
			rad_laser
				...
				var
					intensity = 5 // how much damage the radiation does
				var
					wavelength = 10 // time it takes for the radiation to kick in, in seconds
				var
					used = 0 // is it cooling down?
			...
			rad_laser
				attack(mob/living/M, mob/living/user)
					if(!used)
						...
						var
							cooldown = round(max(100,(((intensity*8)-(wavelength/2))+(intensity*2))*10))
						used = 1
						icon_state = "health1"
		...
		device
			rad_laser
				interact(mob/user)
					...
					var
						cooldown = round(max(10,((intensity*8)-(wavelength/2))+(intensity*2)))
					var
						dat = {"
					...
					var
						datum
							browser
								popup = new(user, "radlaser", "Radioactive Microlaser Interface", 400, 240)
		...
		device
			rad_laser
				Topic(href, href_list)
					...
					if(href_list["radint"])
						var
							amount = text2num(href_list["radint"])
						amount += intensity
						intensity = max(1,(min(10,amount)))

					else if(href_list["radwav"])
						var
							amount = text2num(href_list["radwav"])
						amount += wavelength
