		...
		device
			taperecorder
				hear_talk(mob/living/M as mob, msg)
					if(mytape && recording)
						var
							ending = copytext(msg, length(msg))
				...
				verb
					record()
						...
						if(mytape.used_capacity < mytape.max_capacity)
							...
							var
								used = mytape.used_capacity	//to stop runtimes when you eject the tape
							var
								max = mytape.max_capacity
							for(used, used < max)
								if(recording == 0)
									break
			...
			tape
				attackby(obj/item/I, mob/user)
					if(ruined && istype(I, /obj/item/weapon/screwdriver))
						...
						var
							title = stripped_input(usr,"What do you want to name the tape?", "Tape Renaming", name, MAX_NAME_LEN)
