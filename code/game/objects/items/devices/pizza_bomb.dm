		...
		device
			pizza_bomb
				...
				var
					disarmed = 0
				var
					wires = list("orange", "green", "blue", "yellow", "aqua", "purple")
				var
					correct_wire
				var
					armer //Used for admin purposes
			...
			pizza_bomb
				attackby(var/obj/item/I, var/mob/user, params)
					if(istype(I, /obj/item/weapon/wirecutters) && primed)
						...
						var
							chosen_wire = input(user, "OH GOD OH GOD", "WHAT WIRE?!") in wires
						if(!in_range(src, usr) || issilicon(usr) || !usr.canmove || usr.restrained())
							return
							...
							var
								obj
									item
										stack
											cable_coil
												C = new /obj/item/stack/cable_coil(src.loc)
							C.amount = 3
