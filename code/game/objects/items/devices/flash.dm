	...
	item
		device
			flash
				...
				var
					broken = 0     //Is the flash burnt out?
				var
					last_used = 0 //last world.time it was used.
				var
					battery_panel = 0 //whether the flash can be modified with a cell or not
				var
					overcharged = 0   //if overcharged the flash will set people on fire then immediately burn out (does so even if it doesn't blind them).
...
obj
	item
		device
			flash
				proc
					try_use_flash(var/mob/user = null)
						flash_recharge(user)
...
obj
	item
		device
			flash
				proc
					flash_carbon(var/mob/living/carbon/M, var/mob/user = null, var/power = 5, targeted = 1)
...
obj
	item
		device
			flash
				proc
					terrible_conversion_proc(mob/M, mob/user)
	...
	item
		device
			flash
				cyborg
					proc
						cyborg_flash_animation(var/mob/living/user)
							var
								atom
									movable
										overlay
											animation = new(user.loc)
