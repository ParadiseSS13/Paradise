...
obj
	item
		device
			uplink
				var
					welcome 			// Welcoming menu message
				var
					uses 				// Numbers of crystals
				var
					list
						ItemsCategory	// List of categories with lists of items
				var
					list
						ItemsReference	// List of references with an associated item
				var
					list
						nanoui_items	// List of items for NanoUI use
				var
					nanoui_menu = 0		// The current menu we are in
				var
					list
						nanoui_data = new // Additional data for NanoUI use

				var
					purchase_log = ""
				var
					uplink_owner = null//text-only
				var
					used_TC = 0
...
obj
	item
		device
			uplink
				proc
					generate_items(mob/user as mob)
						var
							datum
								nano_item_lists
									IL = generate_item_lists(user)
...
obj
	item
		device
			uplink
				proc
					generate_item_lists(mob/user as mob)
						if(!job)
							job = user.mind.assigned_role
				...
				Topic(href, href_list)
					...
					if(href_list["buy_item"] == "random")
						...
						var
							datum
								uplink_item
									UI = ItemsReference[href_list["buy_item"]]
...
obj
	item
		device
			uplink
				proc
					buy(var/datum/uplink_item/UI, var/reference)
						if(!UI)
							return
						UI.buy(src,usr)
	...
	item
		device
			uplink
				hidden
					proc
						check_trigger(mob/user as mob, var/value, var/target)
							if(value == target)
								trigger(user)
								return 1
							return 0
...
obj
	item
		proc
			active_uplink_check(mob/user as mob)
