...
obj
	item
		device
			camera_bug
				interact(var/mob/user = usr)
					var
						datum
							browser
								popup = new(user, "camerabug","Camera Bug",nref=src)
						...
						switch(track_mode)
							...
							if(BUGMODE_TRACK)
								if(tracking)
									...
									if(last_found)
										var
											time_diff = round((world.time - last_seen) / 150)
										var
											obj
												machinery
													camera
														C = bugged_cameras[last_found]
										var
											outstring
										if(C)
						...
						if(current && current.can_use())
							...
							for(var/obj/singularity/S in seen) // god help you if you see more than one
								if(S.name in names)
									...
									dat += "[S.name]"
								var
									stage = round(S.current_size / 2)+1
								dat += " (Stage [stage])"
