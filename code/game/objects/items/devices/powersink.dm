	...
	item
		device
			powersink
				...
				var
					drain_rate = 1600000		// amount of power to drain per tick
				var
					apc_drain_rate = 5000 		// Max. amount drained from single APC. In Watts.
				var
					dissipation_rate = 20000	// Passive dissipation of drained power. In Watts.
				var
					power_drained = 0 			// Amount of power drained.
				var
					max_power = 1e10			// Detonation point.
				var
					mode = 0					// 0 = off, 1=clamped (off), 2=operating
				var
					drained_this_tick = 0		// This is unfortunately necessary to ensure we process powersinks BEFORE other machinery such as APCs.
				var
					admins_warned = 0			// stop spam, only warn the admins once that we are about to go boom
						...
						for(var/obj/machinery/power/terminal/T in PN.nodes)
							...
							if(istype(T.master, /obj/machinery/power/apc))
								...
								if(A.operating && A.cell)
									var
										cur_charge = A.cell.charge / CELLRATE
									var
										drain_val = min(apc_drain_rate, cur_charge)
