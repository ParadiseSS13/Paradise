...
/obj/item/analyzer/examine(mob/user)
	. = ..()
	. += "<span class='notice'>To scan an environment, activate it or use it on your location.</span>"
...
/proc/atmos_scan(mob/user, atom/target, silent=FALSE, print=TRUE)
	...
	if(!silent && isliving(user))
		user.visible_message("<span class='notice'>[user] uses the analyzer on [bicon(icon)] [target].</span>", "<span class='notice'>You use the analyzer on [bicon(icon)] [target].</span>")
	...
	for(var/datum/gas_mixture/air as anything in airs)
		...
		if(total_moles > 0)
			...
			if(air.oxygen)
				message += "<span class='notice'>Oxygen: [round(air.oxygen, 0.01)] mol ([round(air.oxygen / total_moles*100, 0.01)] %)</span>"
			if(air.carbon_dioxide)
				message += "<span class='notice'>Carbon Dioxide: [round(air.carbon_dioxide, 0.01)] mol ([round(air.carbon_dioxide / total_moles*100, 0.01)] %)</span>"
			if(air.nitrogen)
				message += "<span class='notice'>Nitrogen: [round(air.nitrogen, 0.01)] mol ([round(air.nitrogen / total_moles*100, 0.01)] %)</span>"
			if(air.toxins)
				message += "<span class='notice'>Plasma: [round(air.toxins, 0.01)] mol ([round(air.toxins / total_moles*100, 0.01)] %)</span>"
			if(air.sleeping_agent)
				message += "<span class='notice'>Nitrous Oxide: [round(air.sleeping_agent, 0.01)] mol ([round(air.sleeping_agent / total_moles*100, 0.01)] %)</span>"
			if(air.agent_b)
				message += "<span class='notice'>Agent B: [round(air.agent_b, 0.01)] mol ([round(air.agent_b / total_moles*100, 0.01)] %)</span>"
			...
			message += airs.len > 1 ? "<span class='notice'>This node is empty!</span>" : "<span class='notice'>[target] is empty!</span>"
			message += "<span class='notice'>Volume: [volume] L</span>" // don't want to change the order volume appears in, suck it
