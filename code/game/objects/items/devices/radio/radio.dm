		...
		device
			radio
				...
				var
					last_transmission
				var
					frequency = PUB_FREQ //common chat
				var
				...
				var
					list
						channels = list() //see communications.dm for full list. First channes is a "default" for :h
				var
					subspace_transmission = 0
				var
					syndie = 0//Holder to see if it's a syndicate encrpyed radio

				var
					is_special = 0 //For electropacks mostly, skips Topic() checks
				...
				var
					atom
						follow_target // Custom follow target for autosay-using bots
...
obj
	item
		device
			radio
				proc
					list_secure_channels(var/mob/user)
						var
							dat[0]

						for(var/ch_name in channels)
							var
								chan_stat = channels[ch_name]
							var
								listening = !!(chan_stat & FREQ_LISTENING) != 0
...
obj
	item
		device
			radio
				proc
					list_internal_channels(var/mob/user)
						var
							dat[0]
...
obj
	item
		device
			radio
				proc
					has_channel_access(var/mob/user, var/freq)
						if(!user)
							return 0
...
mob
	proc
		has_internal_radio_channel_access(var/mob/user, var/list/req_one_accesses)
			var
				obj
					item
						weapon
			...
			radio
				Topic(href, href_list)
					...
					if(href_list["track"])
						var
							mob
								target = locate(href_list["track"])
						var
							mob
								living
									silicon
										ai
											A = locate(href_list["track2"])
						if(A && target)
...
obj
	item
		device
			radio
				proc
					autosay(var/message, var/from, var/channel, var/zlevel = config.contact_levels, var/role = "Unknown") //BS12 EDIT
						var
							datum
								radio_frequency
									connection = null
...
connection = secure_radio_connections[channel]
	...
	var
		mob
			living
				automatedannouncer
					A = new /mob/living/automatedannouncer(src)
	A.name = from
	A.role = role
	A.message = message
...
obj
	item
		device
			radio
				proc
					handle_message_mode(mob/living/M as mob, message, message_mode)
				...
				talk_into(mob/living/M as mob, message, channel, var/verb = "says", var/datum/language/speaking = null)
					...
					*/
					...
					var
						datum
							radio_frequency
								connection = handle_message_mode(M, message, channel)
					if(!istype(connection))
						return 0
					if(!connection)
						return 0
					...
					var
						displayname = M.name	// grab the display name (name you get when you hover over someone's icon)
					var
						real_name = M.real_name // mob's real name
					var
						mobkey = "none" // player key associated with mob
					var
						voicemask = 0 // the speaker is wearing a voice mask
					if(M.client)
...
obj
	item
		device
			radio
				hear_talk(mob/M as mob, msg, var/verb = "says", var/datum/language/speaking = null)
...
obj
	item
		device
			radio
				proc
					accept_rad(obj/item/device/radio/R as obj, message)
				...
				proc
					receive_range(freq, level)
						...
						if(!freq) //recieved on main frequency
							...
							var
								accept = (freq==frequency && listening)
							if(!accept)
								for(var/ch_name in channels)
									var
										datum
											radio_frequency
												RF = secure_radio_connections[ch_name]
			...
			radio
				proc
					send_hear(freq, level)
						...
						var
							range = receive_range(freq, level)
						if(range > -1)
					...
					var
						mob
							living
								silicon
									robot
										myborg = null // Cyborg which owns this radio. Used for power checks
					var
						obj
							item
								device
									encryptionkey
										keyslot = null//Borg radios can handle a single encryption key
					var
						shut_up = 1
	...
	item
		device
			radio
				borg
					syndicate
						CanUseTopic(mob/user, datum/topic_state/state)
							. = ..()
			...
			radio
				borg
					Topic(href, href_list)
						...
						if(href_list["mode"])
							var
								enable_subspace_transmission = text2num(href_list["mode"])
						...
						if(href_list["shutup"]) // Toggle loudspeaker mode, AKA everyone around you hearing your radio.
							var
								do_shut_up = text2num(href_list["shutup"])
