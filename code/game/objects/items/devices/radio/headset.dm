...
obj
	item
		device
			radio
				headset
					examine(mob/user, var/distance = -1)
...
obj
	item
		device
			radio
				headset
					handle_message_mode(mob/living/M as mob, message, channel)
						if(channel == "special")
							if(translate_binary)
								var
									datum
										language
											binary = all_languages["Robot Talk"]
								binary.broadcast(M, message)
							if(translate_hive)
								var
									datum
										language
											hivemind = all_languages["Hivemind"]
...
obj
	item
		device
			radio
				headset
					receive_range(freq, level, aiOverride = 0)
						if(aiOverride)
							return ..(freq, level)
						if(ishuman(loc))
							var
	...
	item
		device
			radio
				headset
					syndicate
						alt //undisguised bowman with flash protection
	...
	item
		device
			radio
				headset
					heads
						ai_integrated //No need to care about icons, it should be hidden inside the AI anyway.
							...
							var
								myAi = null    // Atlantis: Reference back to the AI which has this radio.
							var
								disabledAi = 0 // Atlantis: Used to manually disable AI's integrated radio via intellicard menu.
	...
	item
		device
			radio
				headset
					proc
						recalculateChannels(var/setDescription = 0)
							src.channels = list()
						...
						setupRadioDescription()
							...
							for(var/i = 1 to channels.len)
								var
									channel = channels[i]
								var
									key = get_radio_key_from_channel(channel)
