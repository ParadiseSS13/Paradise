...
/obj/item/autopsy_scanner/attackby(obj/item/P, mob/user)
	...
	var/chems = tgui_input_text(user, "Insert any chemical traces", multiline = TRUE, title = "Coroner's Report", submit_on_enter = FALSE)
	var/notes = tgui_input_text(user, "Insert any relevant notes", multiline = TRUE, title = "Coroner's Report", submit_on_enter = FALSE)
