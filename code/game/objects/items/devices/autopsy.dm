...
datum
	autopsy_data_scanner
		...
		var
			list
				organs_scanned = list() // this maps a number of scanned organs to
...
obj
	item
		weapon
			autopsy_scanner
				proc
					add_data(var/obj/item/organ/external/O)
				...
				verb
					print_data()
						...
						for(var/wdata_idx in wdata)
							...
							var
								list
									weapon_chances = list() // maps weapon names to a score
							var
								age = 0
					...
					var
						obj
							item
								organ
									external
										S = M.get_organ(user.zone_sel.selecting)
					if(!S)
