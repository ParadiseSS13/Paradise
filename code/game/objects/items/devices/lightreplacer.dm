				...
				attackby(obj/item/W, mob/user, params)
					if(istype(W, /obj/item/stack/sheet/glass))
						...
						if(G.amount - decrement >= 0 && uses < max_uses)
							var
								remaining = max(G.amount - decrement, 0)
...
obj
	item
		device
			lightreplacer
				proc
					ReplaceLight(var/obj/machinery/light/target, var/mob/living/U)
									...
									var
										obj
											item
												weapon
													light
														L1 = new target.light_type(target.loc)
...
obj
	item
		device
			lightreplacer
				proc
					janicart_insert(mob/user, obj/structure/janitorialcart/J)
