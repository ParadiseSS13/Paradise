	...
	item
		device
			paicard
				...
				var
					request_cooldown = 5 // five seconds
				var
					last_request
				var
					obj
			...
			paicard
				Topic(href, href_list)
					...
					if(href_list["request"])
						var
							delta = (world.time / 10) - last_request
						if(request_cooldown > delta)
							var
								cooldown_time = round(request_cooldown - ((world.time / 10) - last_request), 1)
					...
					if(href_list["wipe"])
						var
							confirm = input("Are you CERTAIN you wish to delete the current personality? This action cannot be undone.", "Personality Wipe") in list("Yes", "No")
						if(confirm == "Yes")
					...
					if(href_list["setlaws"])
						var
							newlaws = sanitize(copytext(input("Enter any additional directives you would like your pAI personality to follow. Note that these directives will not override the personality's allegiance to its imprinted master. Conflicting directives will be ignored.", "pAI Directive Configuration", pai.pai_laws) as message,1,MAX_MESSAGE_LEN))
						if(newlaws)
...
obj
	item
		device
			paicard
				proc
					setPersonality(mob/living/silicon/pai/personality)
						src.pai = personality
