...
obj
	item
		mounted
			afterattack(var/atom/A, mob/user, proximity_flag)
				var
					found_type = 0
...
obj
	item
		mounted
			proc
				try_build(turf/on_wall, mob/user, proximity_flag) //checks
					if(!on_wall || !user)
						return
...
obj
	item
		mounted
			proc
				do_build(turf/on_wall, mob/user) //the buildy bit after we pass the checks
					return
