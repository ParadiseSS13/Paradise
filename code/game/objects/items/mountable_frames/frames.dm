	...
	item
		mounted
			frame
				...
				var
					list
						mount_reqs = list() //can contain simfloor, nospace. Used in try_build to see if conditions are needed, then met
							...
							obj
								item
									stack
										sheet
											metal
												M = new /obj/item/stack/sheet/metal(get_turf(src))
...
obj
	item
		mounted
			frame
				try_build(turf/on_wall, mob/user)
					if(..()) //if we pass the parent tests
						var
							turf
								turf_loc = get_turf(user)
