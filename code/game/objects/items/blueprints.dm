	...
	item
		areaeditor
			attack_self(mob/user as mob)
				...
				var
					text = "<BODY><HTML><head><title>[src]</title></head> \
			...
			permit
				attack_self(mob/user as mob)
					...
					var
						datum
							browser
								popup = new(user, "blueprints", "[src]", 700, 500)
					popup.set_content(.)
					popup.open()
...
obj
	item
		areaeditor
			proc
				get_area_type(var/area/A = get_area())
					...
					var
						list
							turf
								turfs = res
					var
						str = trim(stripped_input(usr,"New area name:", "Blueprint Editing", "", MAX_NAME_LEN))
...
obj
	item
		areaeditor
			proc
				move_turfs_to_area(var/list/turf/turfs, var/area/A)
				...
				edit_area()
					var
						area
							A = get_area()
					var
						prevname = "[sanitize(A.name)]"
					var
						str = trim(stripped_input(usr,"New area name:", "Blueprint Editing", prevname, MAX_NAME_LEN))
...
obj
	item
		areaeditor
			proc
				set_area_machinery_title(var/area/A,var/title,var/oldtitle)
...
obj
	item
		areaeditor
			proc
				check_tile_is_border(var/turf/T2,var/dir)
			...
			proc
				detect_room(var/turf/first)
					...
					while(pending.len)
						...
						var
							turf
								T = pending[1] //why byond havent list::pop()?
						pending -= T
