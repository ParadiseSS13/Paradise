		...
		robot_parts
			...
			var
				list
					part = null
			var
				sabotaged = 0 //Emagging limbs can have repercussions when installed as prosthetics.
			var
				model_info
			dir = SOUTH
						...
						var
							obj
								item
									weapon
										ed209_assembly
											B = new /obj/item/weapon/ed209_assembly
						B.loc = get_turf(src)
						...
						if(check_completion())
							...
							var
								mob
									living
										silicon
											robot
												O = new /mob/living/silicon/robot(get_turf(loc), unfinished = 1)
							if(!O)	return
							...
							var
								datum
									job_objective
										make_cyborg
											task = user.mind.findJobTask(/datum/job_objective/make_cyborg)
							if(istype(task))
								task.unit_completed()
		...
		robot_parts
			robot_suit
				proc
					Interact(mob/user)
								var
									t1 = text("Designation: <A href='?src=\ref[];Name=1'>[(created_name ? "[created_name]" : "Default Cyborg")]</a><br>\n",src)
								...
								var
									datum
										browser
											popup = new(user, "robotdebug", "Cyborg Boot Debug", 310, 220)
		...
		robot_parts
			robot_suit
				Topic(href, href_list)
					...
					if(href_list["Name"])
						var
							new_name = reject_bad_name(input(usr, "Enter new designation. Set to blank to reset to default.", "Cyborg Debug", src.created_name),1)
