...
obj
	item
		weapon
			dnainjector
				proc
					GetState(var/selblock=0)
						var
							real_block=GetRealBlock(selblock)
...
obj
	item
		weapon
			dnainjector
				proc
					SetState(var/on, var/selblock=0)
						var
							real_block=GetRealBlock(selblock)
						if(buf.types&DNA2_BUF_SE)
...
obj
	item
		weapon
			dnainjector
				proc
					SetValue(var/val,var/selblock=0)
						var
							real_block=GetRealBlock(selblock)
						if(buf.types&DNA2_BUF_SE)
...
obj
	item
		weapon
			dnainjector
				proc
					inject(mob/living/M as mob, mob/user as mob)
						if(used)
							return
