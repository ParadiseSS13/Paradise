...
/obj/item/grenade/attack_self(mob/user as mob)
	if(!active)
		if(clown_check(user))
			...
			message_admins("[key_name_admin(usr)] has primed a [name] for detonation at <A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[bombturf.x];Y=[bombturf.y];Z=[bombturf.z]'>[A.name] (JMP)</a>", user.areaMaster)
