...
obj
	item
		weapon
			spacecash
				New(var/new_loc,var/new_amount=1)
					loc = new_loc
...
obj
	item
		weapon
			spacecash
				proc
					collect_from(var/obj/item/weapon/spacecash/cash)
						if(cash.worth == src.worth)
							var
								taking = min(100-src.amount,cash.amount)
							cash.amount -= taking
							src.amount += taking
							if(cash.amount <= 0)
								qdel(cash)
								...
								item
									weapon
										spacecash
											cash = A
						var
							collected = src.collect_from(cash)
						if(collected)
							update_icon()
							to_chat(user, "<span class='notice'>You add [collected] credit [amount > 1 ? "chips":"chip"] to your stack of cash.</span>")
...
proc
	dispense_cash(var/amount, var/loc)
		for(var/cashtype in moneytypes)
			var
				slice = moneytypes[cashtype]
			var
				dispense_count = Floor(amount/slice)
			amount = amount % slice
			while(dispense_count>0)
				var
					dispense_this_time = min(dispense_count,100)
