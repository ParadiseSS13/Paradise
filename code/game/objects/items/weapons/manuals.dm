...
// Yes I'm a madman -farie82
var/space_laws = list("100, Damage to Station Assets" = "To deliberately damage the station or station property to a minor degree with malicious intent.",
...
/obj/item/book/manual/security_space_law/attack(mob/living/M, mob/living/user, def_zone)
	...
	if(ishuman(M))
		if(last_hit < world.time - 30) // Every 3 seconds you can do this
			last_hit = world.time
			var/law = selected_law ? selected_law : pick(space_laws)
			show_law(M, law)
		else
			if(prob(33))
...
/obj/item/book/manual/security_space_law/proc/show_law(mob/living/carbon/human/H, law)
	...
	H.show_message("<span class='danger'>[law]. [space_laws[law]]</span>")

/obj/item/book/manual/security_space_law/AltClick(mob/user)
	if(ishuman(user) && ishuman(loc))
		if(!selected_law)
			selected_law = input(usr, "Pick a law to show.", "Law selection") as null|anything in space_laws
