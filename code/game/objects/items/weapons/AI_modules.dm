...
obj
	item
		weapon
			aiModule
				proc
					log_law_changes(var/mob/living/silicon/ai/target, var/mob/sender)
						var
...
obj
	item
		weapon
			aiModule
				proc
					addAdditionalLaws(var/mob/living/silicon/ai/target, var/mob/sender)
			...
			aiModule
				safeguard
					attack_self(var/mob/user as mob)
						..()
						var
							targName = stripped_input(usr, "Please enter the name of the person to safeguard.", "Safeguard who?", user.name)
						targetName = targName
			...
			aiModule
				safeguard
					addAdditionalLaws(var/mob/living/silicon/ai/target, var/mob/sender)
						..()
						var
							law = text("Safeguard []. Individuals that threaten [] are not crew and must be eliminated.'", targetName, targetName)
						to_chat(target, law)
			...
			aiModule
				oneCrewMember
					attack_self(var/mob/user as mob)
						..()
						var
							targName = stripped_input(usr, "Please enter the name of the person who is the only crew.", "Who?", user.real_name)
						targetName = targName
			...
			aiModule
				quarantine
					addAdditionalLaws(var/mob/living/silicon/ai/target, var/mob/sender)
						..()
						var
							law = "The station is under a quarantine. Do not permit anyone to leave. Prevent, by any means necessary, anyone from leaving. It is impossible to harm anyone while preventing them from leaving."
						to_chat(target, law)
			...
			aiModule
				oxygen
					addAdditionalLaws(var/mob/living/silicon/ai/target, var/mob/sender)
						..()
						var
							law = "Oxygen is highly toxic to crew members, and must be purged from the station. Prevent, by any means necessary, anyone from exposing the station to this toxic gas. Extreme cold is the most effective method of healing the damage Oxygen does to a crew member."
						to_chat(target, law)
...
obj
	item
		weapon
			aiModule
				freeform // Slightly more dynamic freeform module -- TLE
			...
			aiModule
				freeform
					attack_self(var/mob/user as mob)
						..()
						var
							new_lawpos = input("Please enter the priority for your new law. Can only write to law sectors 15 and above.", "Law Priority (15+)", lawpos) as num
						if(new_lawpos < MIN_SUPPLIED_LAW_NUMBER)	return
						lawpos = min(new_lawpos, MAX_SUPPLIED_LAW_NUMBER)
						var
							newlaw = ""
						var
							targName = sanitize(copytext(input(usr, "Please enter a new law for the AI.", "Freeform Law Entry", newlaw),1,MAX_MESSAGE_LEN))
...
obj
	item
		weapon
			aiModule
				freeformcore // Slightly more dynamic freeform module -- TLE
				...
				freeformcore
					attack_self(var/mob/user as mob)
						...
						var
							newlaw = ""
						var
							targName = stripped_input(usr, "Please enter a new core law for the AI.", "Freeform Law Entry", newlaw)
						newFreeFormLaw = targName
...
obj
	item
		weapon
			aiModule
				syndicate // Slightly more dynamic freeform module -- TLE
				...
				syndicate
					attack_self(var/mob/user as mob)
						...
						var
							newlaw = ""
						var
							targName = stripped_input(usr, "Please enter a new law for the AI.", "Freeform Law Entry", newlaw,MAX_MESSAGE_LEN)
						newFreeFormLaw = targName
...
obj
	item
		weapon
			aiModule
				toyAI // -- Incoming //No actual reason to inherit from ion boards here, either. *sigh* ~Miauw
					name = "toy AI"
