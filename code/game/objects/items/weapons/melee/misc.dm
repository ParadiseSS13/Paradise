			...
			if(istype(H))
				...
				if(M.stat == DEAD)	//si el mob esta muerto
					...
					if(M == user)	//si intentas hacertelo a ti mismo
						to_chat(user, "<span class='notice'>You can't do this to yourself.</span>")
					else
						...
						if(do_after(user, time, 0, target = user))
							...
							if(prob(15))
								M.dna.SetSEState(CHAVBLOCK, 1)
								genemutcheck(M, CHAVBLOCK, null, MUTCHK_FORCED)
								M.adjustBrainLoss(rand(2,8))
							else if(prob(20))
								M.dna.SetSEState(TWITCHBLOCK, 1)
								genemutcheck(M, TWITCHBLOCK, null, MUTCHK_FORCED)
								M.adjustBrainLoss(rand(1,5))
							else if(prob(15))
								M.dna.SetSEState(MUTEBLOCK, 1)
								genemutcheck(M, MUTEBLOCK, null, MUTCHK_FORCED)
								M.adjustBrainLoss(rand(2,8))
							else if(prob(8))
								M.dna.SetSEState(DEAFBLOCK, 1)
								genemutcheck(M, DEAFBLOCK, null, MUTCHK_FORCED)
								M.adjustBrainLoss(rand(1,5))
							else if(prob(7))
								M.dna.SetSEState(SWEDEBLOCK, 1)
								genemutcheck(M, SWEDEBLOCK, null, MUTCHK_FORCED)
								M.adjustBrainLoss(rand(2,8))
							else if(prob(3))
								M.dna.SetSEState(EPILEPSYBLOCK, 1)
								genemutcheck(M, EPILEPSYBLOCK, null, MUTCHK_FORCED)
								M.adjustBrainLoss(rand(1,5))
							else if(prob(1))
								M.dna.SetSEState(TELEBLOCK, 1)
								genemutcheck(M, TELEBLOCK, null, MUTCHK_FORCED)
								M.adjustBrainLoss(rand(2,8))
							else
								M.dna.SetSEState(NERVOUSBLOCK, 1)
								genemutcheck(M, NERVOUSBLOCK, null, MUTCHK_FORCED)
								M.adjustBrainLoss(rand(1,5))
			else
