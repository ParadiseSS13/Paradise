			...
			paint
				anycolor
					...
					attack_self(mob/user as mob)
						var
							t1 = input(user, "Please select a color:", "Locking Computer", null) in list( "red", "blue", "green", "yellow", "black", "white")
			...
			paint
				afterattack(turf/target, mob/user as mob, proximity)
					...
					if(!istype(target) || istype(target, /turf/space))
						return
					var
						ind = "[initial(target.icon)][color]"
					if(!cached_icons[ind])
						var
							icon
								overlay = new/icon(initial(target.icon))
