	...
	item
		weapon
			nullrod
				...
				var
					list
						fluff_transformations = list() //does it have any special transformations only accessible to it? Should only be subtypes of /obj/item/weapon/nullrod
...
obj
	item
		weapon
			nullrod
				attack(mob/M, mob/living/carbon/user)
					..()
					if(M.mind)
						if(M.mind.vampire)
							if(ishuman(M))
...
obj
	item
		weapon
			nullrod
				proc
					reskin_holy_weapon(mob/M)
						var
							list
								holy_weapons_list = typesof(/obj/item/weapon/nullrod) - typesof(/obj/item/weapon/nullrod/fluff)
						...
						var
							choice = input(M,"What theme would you like for your holy weapon?","Holy Weapon Theme") as null|anything in display_names
					...
					talking
						attack_self(mob/living/user)
							...
							var
								list
									mob
										dead
											observer
												candidates = pollCandidates("Do you want to play as the spirit of [user.real_name]'s blade?", ROLE_PAI, 0, 100)
							var
								mob
									dead
										observer
							...
							if(candidates.len)
								...
								var
									input = stripped_input(S,"What are you named?", ,"", MAX_NAME_LEN)
...
obj
	item
		weapon
			nullrod
				whip
					afterattack(atom/movable/AM, mob/user, proximity)
						if(!proximity)
							return
						if(istype(AM, /mob/living/carbon/human))
							...
							if(is_shadow(H))
								var
									phrase = pick("Die monster! You don't belong in this world!!!", "You steal men's souls and make them your slaves!!!", "Your words are as empty as your soul!!!", "Mankind ill needs a savior such as you!!!")
								user.say("[phrase]")
...
obj
	item
		weapon
			nullrod
				claymore
					bostaff //May as well make it a "claymore" and inherit the blocking
						name = "monk's staff"
