...
/obj/item/grenade/plastic/afterattack(atom/movable/AM, mob/user, flag)
	...
	if(do_after(user, 50 * toolspeed, target = AM))
		...
		if(notify_admins)
			message_admins("[ADMIN_LOOKUPFLW(user)] planted [src.name] on [target.name] at ([target.x],[target.y],[target.z] - <a href='?_src_=holder;adminplayerobservecoodjump=1;X=[target.x];Y=[target.y];Z=[target.z]'>JMP</a>) with [det_time] second fuse", 0, 1)
