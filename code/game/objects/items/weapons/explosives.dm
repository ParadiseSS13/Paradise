...
/obj/item/grenade/plastic/afterattack(atom/movable/AM, mob/user, flag)
	...
	if(do_after(user, 50 * toolspeed, target = AM))
		...
		message_admins("[key_name_admin(user)][ADMIN_QUE(user)] [ADMIN_FLW(user)] planted [src.name] on [target.name] at ([target.x],[target.y],[target.z] - <A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[target.x];Y=[target.y];Z=[target.z]'>JMP</a>) with [det_time] second fuse",0,1)
...
/obj/item/grenade/plastic/suicide_act(mob/user)
	message_admins("[key_name_admin(user)][ADMIN_QUE(user)] [ADMIN_FLW(user)] suicided with [src.name] at ([user.x],[user.y],[user.z] - <A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[user.x];Y=[user.y];Z=[user.z]'>JMP</a>)",0,1)
