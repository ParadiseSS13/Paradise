		...
		weapon
			tank
				examine(mob/user)
					...
					var
						celsius_temperature = src.air_contents.temperature-T0C
					var
						descriptive
			...
			tank
				proc
					remove_air_volume(volume_to_return)
						...
						var
							tank_pressure = air_contents.return_pressure()
						if(tank_pressure < distribute_pressure)
							distribute_pressure = tank_pressure

						var
							moles_needed = distribute_pressure*volume_to_return/(R_IDEAL_GAS_EQUATION*air_contents.temperature)
...
//		to_chat(world, "\blue[x],[y] tank is leaking: [pressure] kPa, integrity [integrity]")
		if(integrity <= 0)
			...
			var
				datum
					gas_mixture
						leaked_gas = air_contents.remove_ratio(0.25)
