	...
	item
		weapon
			defibrillator
				...
				var
					on = 0 //if the paddles are equipped (1) or on the defib (0)
				var
					safety = 1 //if you can zap people with the defibs on harm mode
				var
					powered = 0 //if there's a cell in the defib with enough power for a revive, blocks paddles from reviving otherwise
				var
					obj
						item
							weapon
								...
								stock_parts
									cell
										high
											bcell = null
				var
					combat = 0 //can we revive through space suits?

obj
	item
		weapon
			defibrillator
				New() //starts without a cell for rnd
					..()
				...
				proc
					update_charge()
						if(powered) //so it doesn't show charge if it's unpowered
							if(bcell)
								var
									ratio = bcell.charge / bcell.maxcharge
	...
	item
		weapon
			twohanded
				shockpaddles
					proc
						check_defib_exists(mainunit, var/mob/living/carbon/human/M, var/obj/O)
				...
				shockpaddles
					attack(mob/M, mob/user)
						var
							tobehealed
						var
							threshold = -config.health_threshold_dead
						var
							mob
								living
									carbon
							...
							if(user.zone_sel && user.zone_sel.selecting == "chest")
								...
								if(do_after(user, 30, target = M)) //beginning to place the paddles on patient's chest to allow some time for people to move away to stop the process
									...
									if(ghost && !ghost.client)
										...
										ghost = null
									var
										tplus = world.time - H.timeofdeath
									var
										tlimit = 1800 //past this much time the patient is unrecoverable (in deciseconds)
									var
										tloss = 600 //brain damage starts setting in on the patient after some time left rotting
									var
										total_burn	= 0
									var
										total_brute	= 0
