...
/obj/item/defibrillator/proc/update_overlays()
	...
	if(!on)
		overlays += "[initial_icon]-paddles"
	if(powered)
		overlays += "[initial_icon]-powered"
	if(!bcell)
		overlays += "[initial_icon]-nocell"
	if(!safety)
		overlays += "[initial_icon]-emagged"
...
/obj/item/defibrillator/proc/toggle_paddles()
	...
	if(on)
		...
		if(!usr.put_in_hands(paddles))
			on = FALSE
			to_chat(usr, "<span class='warning'>You need a free hand to hold the paddles!</span>")
			update_icon()
			return
	else
...
/obj/item/twohanded/shockpaddles/New(mainunit)
	..()
	if(req_defib && check_defib_exists(mainunit, src))
		defib = mainunit
		forceMove(defib)
		update_icon()
...
/obj/item/twohanded/shockpaddles/proc/try_revive(mob/living/carbon/human/H, mob/living/user, tplus)
