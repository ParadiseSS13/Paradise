	...
	item
		weapon
			pneumatic_cannon
				...
				var
					maxWeightClass = 20 //The max weight of items that can fit into the cannon
				var
					loadedWeightClass = 0 //The weight of items currently in the cannon
				var
					obj
						item
							weapon
								tank
									tank = null //The gas tank that is drawn from to fire things
				var
					gasPerThrow = 3 //How much gas is drawn from a tank's pressure to fire
				var
					list
						loadedItems = list() //The items loaded into the cannon that will be fired out
				var
					pressureSetting = 1 //How powerful the cannon is - higher pressure = more gas but more powerful throws
...
obj
	item
		weapon
			pneumatic_cannon
				afterattack(atom/target, mob/living/carbon/human/user, flag, params)
...
obj
	item
		weapon
			pneumatic_cannon
				proc
					Fire(var/mob/living/carbon/human/user, var/atom/target)
...
datum
	table_recipe
		improvised_pneumatic_cannon //Pretty easy to obtain but
			name = "Pneumatic Cannon"
...
obj
	item
		weapon
			pneumatic_cannon
				proc
					updateTank(obj/item/weapon/tank/thetank, removing = 0, mob/living/carbon/human/user)
						if(removing)
							if(!src.tank)
								return
