...
obj
	item
		weapon
			wirecutters
				attack(mob/living/carbon/human/C as mob, mob/user as mob)
	...
	item
		weapon
			weldingtool
				...
				var
					welding = 0 	//Whether or not the welding tool is off(0), on(1) or currently welding(2)
				var
					status = 1 		//Whether the welder is secured or unsecured (able to attach rods to it to make a flamethrower)
				var
					max_fuel = 20 	//The max amount of fuel the welder can hold
				var
					change_icons = 1
				var
					can_off_process = 0
				var
					light_intensity = 2 //how powerful the emitted light is when used.
...
obj
	item
		weapon
			weldingtool
				proc
					remove_fuel(var/amount = 1, var/mob/living/M = null)
...
obj
	item
		weapon
			weldingtool
				proc
					flamethrower_screwdriver(obj/item/I, mob/user)
						if(welding)
...
obj
	item
		weapon
			weldingtool
				proc
					flamethrower_rods(obj/item/I, mob/user)
						if(!status)
							var
								obj
									item
								...
								var
									obj
										item
											weapon
												flamethrower
													F = new /obj/item/weapon/flamethrower(user.loc)
