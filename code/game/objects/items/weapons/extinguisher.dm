	...
	item
		weapon
			extinguisher
				...
				var
					power = 5 //Maximum distance launched water will travel
				var
					precision = 0 //By default, turfs picked from a spray are random, set to 1 to make it always have at least one water effect per row
				var
					cooling_power = 2 //Sets the cooling_temperature of the water reagent datum inside of the extinguisher when it is refilled
...
obj
	item
		weapon
			extinguisher
				proc
					AttemptRefill(atom/target, mob/user)
									...
									structure
										reagent_dispensers
											watertank
												W = target
							var
								transferred = W.reagents.trans_to(src, max_water)
							if(transferred > 0)
			...
			extinguisher
				afterattack(atom/target, mob/user , flag)
					...
					if(target.loc == user)//No more spraying yourself when putting your extinguisher away
						return
					var
						Refill = AttemptRefill(target, user)
					if(Refill)
						return
					if(!safety)
							...
							spawn(0)
								...
								var
									obj
										B = usr.buckled
								var
									movementdirection = turn(direction,180)
								if(C)	C.propelled = 4
						...
						var
							turf
								T1 = get_step(T,turn(direction, 90))
						var
							turf
								T2 = get_step(T,turn(direction, -90))
						var
							list
						...
						if(precision)
							var
								turf
									T3 = get_step(T1, turn(direction, 90))
							var
								turf
									T4 = get_step(T2,turn(direction, -90))
								...
								var
									obj
										effect
											effect
												water
													W = new /obj/effect/effect/water( get_turf(src) )
								var
									turf
