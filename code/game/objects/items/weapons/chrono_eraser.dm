	...
	item
		weapon
			chrono_eraser
				...
				var
					list
						erased_minds = list() //a collection of minds from the dead
	...
	item
		weapon
			gun
				energy
					chrono_gun
						New(var/obj/item/weapon/chrono_eraser/T)
							. = ..()
							if(istype(T))
								TED = T
		...
		weapon
			gun
				energy
					chrono_gun
						proc
							field_connect(var/obj/effect/chrono_field/F)
								var
									mob
										living
											user = src.loc
		...
		weapon
			gun
				energy
					chrono_gun
						proc
							field_disconnect(var/obj/effect/chrono_field/F)
								if(F && field == F)
									var
										mob
											living
		...
		weapon
			gun
				energy
					chrono_gun
						proc
							field_check(var/obj/effect/chrono_field/F)
								if(F)
									if(field == F)
										var
											turf
												currentpos = get_turf(src)
										var
											mob
												living
													user = src.loc
...
obj
	effect
		chrono_field
			New(loc, var/mob/living/target, var/obj/item/weapon/gun/energy/chrono_gun/G)
				if(target && isliving(target) && G)
					...
					for(var/i=1, i<=CHRONO_FRAME_COUNT, i++)
						var
							icon
								removing_frame = icon('icons/obj/chronos.dmi', "erasing", SOUTH, i)
						var
							icon
								mob_icon = icon(mob_snapshot)
...
obj
	effect
		chrono_field
			update_icon()
				var
					ttk_frame = 1 - (tickstokill / initial(tickstokill))
...
obj
	effect
		chrono_field
			return_air() //we always have nominal air and temperature
				var
					datum
						gas_mixture
							GM = new
