...
/obj/item/rollingpaper/afterattack(atom/target, mob/user, proximity)
	...
	if(istype(target, /obj/item/food/snacks/grown))
		...
		if(plant.dry)
			...
			var/obj/item/clothing/mask/cigarette/rollie/custom/custom_rollie  = new /obj/item/clothing/mask/cigarette/rollie/custom(user.loc)
			custom_rollie.chem_volume = target.reagents.total_volume
			custom_rollie.smoke_time = target.reagents.total_volume * REAGENT_TIME_RATIO
			target.reagents.trans_to(custom_rollie, custom_rollie.chem_volume)
			user.put_in_active_hand(custom_rollie)
			to_chat(user, "<span class='notice'>You roll the [target.name] into a rolling paper.</span>")
			custom_rollie.desc = "Dried [target.name] rolled up in a thin piece of paper."
			qdel(target)
			qdel(src)
		else
			to_chat(user, "<span class='warning'>You need to dry this first!</span>")
