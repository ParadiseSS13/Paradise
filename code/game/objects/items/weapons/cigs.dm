...
obj
	item
		clothing
			mask
				cigarette
					attack(var/mob/living/M, var/mob/living/user, def_zone)
				...
				cigarette
					afterattack(obj/item/weapon/reagent_containers/glass/glass, mob/user as mob, proximity)
						...
						if(istype(glass))	//you can dip cigarettes into beakers
							var
								transfered = glass.reagents.trans_to(src, chem_volume)
				...
				cigarette
					joint
						New()
							...
							var
								list
									jointnames = list("joint","doobie","spliff","blunt")
	...
	item
		clothing
			mask
				cigarette
					pipe
						attack_self(mob/user as mob) //Refills the pipe. Can be changed to an attackby later, if loose tobacco is added to vendors or something.
							if(lit)
