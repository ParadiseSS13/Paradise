...
/obj/item/powerfist/Destroy()
	QDEL_NULL(tank)
	return ..()

/obj/item/powerfist/examine(mob/user)
	. = ..()
...
/obj/item/powerfist/attackby__legacy__attackchain(obj/item/W, mob/user, params)
...
/obj/item/powerfist/wrench_act(mob/user, obj/item/I)
	. = TRUE
...
/obj/item/powerfist/screwdriver_act(mob/user, obj/item/I)
	if(!tank)
		return
	. = TRUE
	if(!I.use_tool(src, user, 0, volume = I.tool_volume))
...
/obj/item/powerfist/proc/updateTank(obj/item/tank/thetank, removing = 0, mob/living/carbon/human/user)
	if(removing)
		if(!tank)
...
/obj/item/powerfist/attack__legacy__attackchain(mob/living/target, mob/living/user)
...
/obj/item/powerfist/proc/use_air()
	if(!tank)
		return FALSE
