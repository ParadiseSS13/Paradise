...
/obj/item/storage
	...
	// Will this spawn as an empty box
	var/empty = FALSE
...
/obj/item/storage/MouseDrop(obj/over_object)
	...
	if((istype(over_object, /obj/structure/table) || isfloorturf(over_object)) && length(contents) \
		&& loc == M && !M.stat && !M.restrained() && over_object.Adjacent(M) && !istype(src, /obj/item/storage/lockbox)) // Worlds longest `if()`
		...
		if(isfloorturf(over_object))
			...
			if(alert(M, "Empty [src] onto [T]?", "Confirm", "Yes", "No") != "Yes")
				return
			if(!(M && over_object && length(contents) && loc == M && !M.stat && !M.restrained() && get_turf(M) == T))
