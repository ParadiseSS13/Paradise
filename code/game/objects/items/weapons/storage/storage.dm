	...
	item
		weapon
			storage
				...
				w_class = 3.0
				var
					silent = 0 // No message on putting items in
				var
					list
						can_hold = new/list() //List of objects which this item can store (if set, it can't store anything else)
				var
					list
						cant_hold = new/list() //List of objects which this item can't store (in effect only if can_hold isn't set)
				var
				...
				var
					collection_mode = 1;  //0 = pick one at a time, 1 = pick all on tile
				var
					foldable = null	// BubbleWrap - if set, can be folded (when empty) into a sheet of cardboard
				var
					use_sound = "rustle"	//sound played when used. null for no sound.
...
obj
	item
		weapon
			storage
				proc
					standard_orient_objs(var/rows, var/cols, var/list/obj/item/display_contents)
						var
							cx = 4
						var
							cy = 2+rows
			...
			storage
				proc
					orient2hud(mob/user as mob)
						...
						var
							adjusted_contents = contents.len
