	...
	if(istype(W, /obj/item/storage/backpack/holding))
		...
		if(response == "Yes")
			...
			if(do_after(user, 30, target=src))
				...
				user.visible_message(span_warning("[user] erupts in evil laughter as [user.p_they()] put[user.p_s()] the Bag of Holding into another Bag of Holding!"), span_warning("You can't help but laugh wildly as you put the Bag of Holding into another Bag of Holding, complete darkness surrounding you."),span_warning(" You hear the sound of scientific evil brewing! "))
				qdel(W)
