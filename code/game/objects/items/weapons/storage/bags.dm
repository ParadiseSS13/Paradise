	...
	item
		weapon
			storage
				bag
					ore
						holding //miners, your messiah has arrived
		...
		if(display_contents_with_number)
			...
			for(var/obj/item/stack/sheet/I in contents)
				...
				var
					datum
						numbered_display
							D = new/datum/numbered_display(I)
				D.number = I.amount
				...
				var
					obj
						item
							stack
								sheet
									N = new S.type(location)
				var
					stacksize = min(S.amount,N.max_amount)
				N.amount = stacksize
				S.amount -= stacksize
			if(!S.amount)
	...
	item
		weapon
			storage
				bag
					tray
						handle_item_insertion(obj/item/I, prevent_warning = 0)
							overlays += image("icon" = I.icon, "icon_state" = I.icon_state, "layer" = -1)
							..()
