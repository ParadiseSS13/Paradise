	...
	item
		weapon
			restraints
				legcuffs
					beartrap
						attackby(var/obj/item/I, mob/user as mob) //Let's get explosive.
			...
			legcuffs
				bolas
					...
					var
						dispenser = 0
					var
						throw_sound = 'sound/weapons/whip.ogg'
					var
						trip_prob = 60
					var
						thrown_from
				...
				bolas
					throw_at(var/atom/A, throw_range, throw_speed)
						...
						var
							atom
								movable
									adjtarget = new /atom/movable
						var
							xadjust = 0
						var
							yadjust = 0
						var
							scaler = 0 //used to changed the normalised vector to the proper size
	...
	item
		weapon
			legcuffs
				bolas
					proc
						throw_failed() //called when the throw doesn't entangle
