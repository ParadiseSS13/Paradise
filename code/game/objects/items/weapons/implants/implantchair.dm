...
/obj/machinery/implantchair/attack_hand(mob/user)
	...
	dat += "<B>Bio-chips:</B> [src.implant_list.len ? "[implant_list.len]" : "<A href='?src=[UID()];replenish=1'>Replenish</A>"]<BR>"
	if(src.occupant)
		dat += "[src.ready ? "<A href='?src=[UID()];implant=1'>Bio-chip</A>" : "Recharging"]<BR>"
	user.set_machine(src)
