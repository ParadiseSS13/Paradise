...
/obj/item/implant/mindshield/implant(mob/target)
	if(..())
		if(target.mind in SSticker.mode.head_revolutionaries || is_shadow_or_thrall(target))
			target.visible_message("<span class='warning'>[target] parece resistirse al implante!</span>", "<span class='warning'>You feel the corporate tendrils of Nanotrasen try to invade your mind!</span>")
			removed(target, 1)
			qdel(src)
			return -1
		...
		if(target.mind in SSticker.mode.cult)
			to_chat(target, "<span class='warning'>Sientes que los zarcillos corporativos de Nanotrasen intentan invadir tu mente!</span>")
		else
			to_chat(target, "<span class='notice'>Tu mente se siente endurecida - mas resistente al lavado de cerebro.</span>")
		return 1
	return 0
