...
#define MICROCHIP_EMOTE_TRIGGER_INTENTIONAL (1<<0)
/// If used, an implant will trigger when an emote is forced/unintentionally used.
#define MICROCHIP_EMOTE_TRIGGER_UNINTENTIONAL (1<<1)
/// If used, an implant will always trigger when the user makes an emote.
#define MICROCHIP_EMOTE_TRIGGER_ALWAYS (MICROCHIP_EMOTE_TRIGGER_UNINTENTIONAL | MICROCHIP_EMOTE_TRIGGER_INTENTIONAL)
/// If used, an implant will trigger on the user's first death.
#define MICROCHIP_TRIGGER_DEATH_ONCE (1<<2)
/// If used, an implant will trigger any time a user dies.
#define MICROCHIP_TRIGGER_DEATH_ANY (1<<3)
/// If used, an implant will NOT trigger on death when a user is gibbed.
#define MICROCHIP_TRIGGER_NOT_WHEN_GIBBED (1<<4)
...
#define MICROCHIP_ACTIVATED_PASSIVE 0
/// The implant is activated manually by a trigger
#define MICROCHIP_ACTIVATED_ACTIVE 1
...
/obj/item/implant
	...
	var/activated = MICROCHIP_ACTIVATED_ACTIVE
...
/obj/item/implant/proc/set_trigger(mob/user, emote_key, on_implant = FALSE, silent = TRUE)
	...
	if(!(emote_key in user.usable_emote_keys(trigger_causes & MICROCHIP_EMOTE_TRIGGER_INTENTIONAL)))
		if(!silent)
			to_chat(user, "<span class='warning'> You can't trigger [src] with that emote! Try *help to see emotes you can use.</span>")
		return FALSE

	if(!(emote_key in user.usable_emote_keys(trigger_causes & MICROCHIP_EMOTE_TRIGGER_UNINTENTIONAL)))
		CRASH("User was given an microchip for an unintentional emote that they can't use.")
...
/obj/item/implant/proc/on_emote(mob/living/user, datum/emote/fired_emote, key, emote_type, message, intentional)
	...
	if(!(intentional && (trigger_causes & MICROCHIP_EMOTE_TRIGGER_INTENTIONAL)) && !(!intentional && (trigger_causes & MICROCHIP_EMOTE_TRIGGER_UNINTENTIONAL)))
		return
...
/obj/item/implant/proc/on_death(mob/source, gibbed)
	...
	if(gibbed && (trigger_causes & MICROCHIP_TRIGGER_NOT_WHEN_GIBBED))
		return

	// This should help avoid infinite recursion for things like dust that call death()
	if(has_triggered_on_death && (trigger_causes & MICROCHIP_TRIGGER_DEATH_ONCE))
		return

	has_triggered_on_death = TRUE

	add_attack_logs(source, source, "had their [src] microchip triggered on [gibbed ? "gib" : "death"].")
...
/obj/item/implant/proc/implant(mob/source, mob/user)
	...
	implanted = TRUE
	if(trigger_emotes)
		if(!(trigger_causes & MICROCHIP_EMOTE_TRIGGER_INTENTIONAL | MICROCHIP_EMOTE_TRIGGER_UNINTENTIONAL))
			CRASH("Microchip [src] has trigger emotes defined but no trigger cause with which to use them!")
		if(!activated && (trigger_causes & MICROCHIP_EMOTE_TRIGGER_INTENTIONAL))
			CRASH("Microchip [src] has intentional emote triggers on a passive microchip")
	...
	if(activated)
		for(var/X in actions)
			...
			A.Grant(source)
	if(trigger_causes & (MICROCHIP_TRIGGER_DEATH_ONCE | MICROCHIP_TRIGGER_DEATH_ANY))
