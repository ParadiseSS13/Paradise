...
obj
	item
		weapon
			twohanded
				proc
					unwield(mob/living/carbon/user)
						if(!wielded || !user) return
						wielded = 0
						force = force_unwielded
						var
							sf = findtext(name," (Wielded)")
						if(sf)
							...
							obj
								item
									weapon
										twohanded
											offhand
												O = new(user) ////Let's reserve his other hand~
						O.name = "[name] - offhand"
...
obj
	item
		weapon
			twohanded
				offhand
					IsShield()//if the actual twohanded weapon is a shield, we count as a shield too!
						var
							mob
								user = loc
...
obj
	item
		weapon
			twohanded
				required
					attack_hand(mob/user)//Can't even pick it up without both hands empty
						var
							obj
								item
									weapon
...
obj
	item
		weapon
			twohanded
				fireaxe
					update_icon()  //Currently only here to fuck with the on-mob icons.
...
obj
	item
		weapon
			twohanded
				fireaxe
					afterattack(atom/A as mob|obj|turf|area, mob/user as mob, proximity)
						if(!proximity) return
						..()
...
obj
	item
		weapon
			twohanded
				dualsaber
					wield(mob/living/carbon/M) //Specific wield () hulk checks due to reflection chance for balance issues and switches hitsounds.
						...
						if(istype(W, /obj/item/weapon/twohanded/spear))
							...
							var
								obj
									structure
										headspear
											HS = new /obj/structure/headspear(user.loc)
							var
								matrix
									M = matrix()
							src.transform = M
							...
							var
								image
									IM = image(src.icon,src.icon_state)
...
obj
	item
		weapon
			twohanded
				chainsaw
					wield() //you can't disarm an active chainsaw, you crazy person.
						..()
						flags |= NODROP
	...
	item
		weapon
			twohanded
				singularityhammer
					proc
						vortex(var/turf/pull as turf, mob/wielder as mob)
	...
	item
		weapon
			twohanded
				mjollnir
					proc
						shock(mob/living/target as mob)
							var
								datum
									effect
										system
							...
							var
								atom
									throw_target = get_edge_target_turf(target, get_dir(src, get_step_away(target, src)))
						...
						if(charged == 5)
							...
							if(istype(A, /mob/living/))
								...
								if(Z.health >= 1)
									...
									var
										atom
											throw_target = get_edge_target_turf(Z, get_dir(src, get_step_away(Z, src)))
