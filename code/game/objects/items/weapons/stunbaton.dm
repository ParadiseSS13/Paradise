...
/obj/item/melee/baton/mob_can_equip(mob/user, slot, disable_warning = TRUE, bypass_equip_delay_self = FALSE)
	if(turned_on && (slot == SLOT_HUD_BELT || slot == SLOT_HUD_SUIT_STORE))
		...
		return FALSE
	return ..(user, slot, disable_warning = TRUE, bypass_equip_delay_self) // call parent but disable warning
