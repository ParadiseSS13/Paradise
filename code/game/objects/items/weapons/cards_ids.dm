...
/obj/item/card/id/syndicate/proc/change_photo()
	...
	if(!newphoto)
		return
	photo = newphoto
	to_chat(registered_human, "<span class='notice'>Photo changed. Select another occupation and take a new photo if you wish to appear with different clothes.</span>")
...
/obj/item/card/id/syndicate/proc/change_age()
	...
	if(ishuman(registered_human))
		...
		default = H.age
	var/new_age = tgui_input_number(registered_human,"What age would you like to be written on this card?","Agent Card Age", default)
	if(!Adjacent(registered_human))
		return
	age = new_age
...
/obj/item/card/id/syndicate/proc/change_blood_type()
	...
	var/new_blood_type = tgui_input_text(registered_human,"What blood type would you like to be written on this card?","Agent Card Blood Type",default)
	if(!Adjacent(registered_human))
		return
...
/obj/item/card/id/syndicate/proc/change_dna_hash()
	...
	var/new_dna_hash = tgui_input_text(registered_human,"What DNA hash would you like to be written on this card?","Agent Card DNA Hash",default)
	if(!Adjacent(registered_human))
		return
...
/obj/item/card/id/syndicate/proc/change_fingerprints()
	...
	var/new_fingerprint_hash = tgui_input_text(registered_human,"What fingerprint hash would you like to be written on this card?","Agent Card Fingerprint Hash",default)
	if(!Adjacent(registered_human))
		return
