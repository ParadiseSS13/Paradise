		...
		weapon
			card
				id
					...
					slot_flags = SLOT_ID
					var
						untrackable // Can not be tracked by AI's
					...
					var
						assignment = null	//can be alt title or the actual job
					var
						rank = null			//actual job
					var
						dorm = 0			// determines if this ID has claimed a dorm already
							...
							item
								weapon
									card
										id
											guest
												guest_pass = null // Guest pass attached to the ID
	...
	item
		weapon
			card
				id
					proc
						show(mob/user as mob)
							var
								datum
									asset
										assets = get_asset_datum(/datum/asset/simple/paper)
							assets.send(user)

							var
								datum
									browser
										popup = new(user, "idcard", name, 600, 400)
	...
	item
		weapon
			card
				id
					proc
						SetOwnerInfo(var/mob/living/carbon/human/H)
							if(!H || !H.dna)
								return
			...
			card
				id
					proc
						RebuildHTML()
							var
								photo_front = "'data:image/png;base64,[icon2base64(icon(photo, dir = SOUTH))]'"
							var
								photo_side = "'data:image/png;base64,[icon2base64(icon(photo, dir = WEST))]'"
			...
			card
				id
					syndicate
						...
						var
							list
								initial_access = list(access_maint_tunnels, access_syndicate, access_external_airlocks)
	...
	item
		weapon
			card
				id
					syndicate
						afterattack(var/obj/item/weapon/O as obj, mob/user as mob, proximity)
							if(!proximity)
								return
		...
		weapon
			card
				id
					syndicate
						proc
							fake_id_photo(var/mob/living/carbon/human/H)//get_id_photo wouldn't work correctly
								if(!istype(H))
									return
								var
									storedDir = H.dir //don't want to lose track of this
								...
								switch(alert(user,"Would you like to display \the [src] or edit it?","Choose","Show","Edit"))
									...
									if("Edit")
										switch(input(user,"What would you like to edit on \the [src]?") in list("Name","Photo","Appearance","Sex","Age","Occupation","Money Account","Blood Type","DNA Hash","Fingerprint Hash","Reset Card"))
											if("Name")
												var
													new_name = reject_bad_name(input(user,"What name would you like to put on this card?","Agent Card Name", ishuman(user) ? user.real_name : user.name))
												if(!Adjacent(user))
													return
											...
											if("Appearance")
												...
												var
													choice = input(user, "Select the appearance for this card.", "Agent Card Appearance") in appearances
												if(!Adjacent(user))
													return
												if(!choice)
													return
											...
											if("Sex")
												var
													new_sex = sanitize(stripped_input(user,"What sex would you like to put on this card?","Agent Card Sex", ishuman(user) ? capitalize(user.gender) : "Male", MAX_MESSAGE_LEN))
												if(!Adjacent(user))
													return
												src.sex = new_sex
											...
											if("Age")
												var
													new_age = sanitize(stripped_input(user,"What age would you like to put on this card?","Agent Card Age","21", MAX_MESSAGE_LEN))
												if(!Adjacent(user))
													return
												src.age = new_age
											...
											if("Money Account")
												var
													new_account = input(user,"What money account would you like to link to this card?","Agent Card Account",12345) as num
												if(!Adjacent(user))
													return
											...
											if("Blood Type")
												...
												var
													new_blood_type = sanitize(input(user,"What blood type would you like to be written on this card?","Agent Card Blood Type",default) as text)
												if(!Adjacent(user))
													return
											...
											if("DNA Hash")
												...
												var
													new_dna_hash = sanitize(input(user,"What DNA hash would you like to be written on this card?","Agent Card DNA Hash",default) as text)
												if(!Adjacent(user))
													return
											...
											if("Fingerprint Hash")
												...
												var
													new_fingerprint_hash = sanitize(input(user,"What fingerprint hash would you like to be written on this card?","Agent Card Fingerprint Hash",default) as text)
												if(!Adjacent(user))
													return
	...
	item
		weapon
			id_decal
				...
				icon_state = "batterer"
				var
					decal_name = "identification card"
				var
					decal_desc = "A card used to provide ID and determine access across the station."
				var
					decal_icon_state = "id"
				var
					decal_item_state = "card-id"
				var
					override_name = 0
