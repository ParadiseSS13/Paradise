...
/obj/item/scissors/safety/attack(mob/living/carbon/M as mob, mob/user as mob)
	...
	if(ishuman(M))
		...
		if(!is_cutting)
			is_cutting = 1
			user.visible_message(span_notice("[user] starts cutting [M]'s hair!"), span_notice("You start cutting [M]'s hair!"))
			playsound(loc, 'sound/goonstation/misc/scissor.ogg', 100, 1)
			if(do_after(user, 50 * toolspeed, target = H))
				playsound(loc, 'sound/weapons/bladeslice.ogg', 50, 1, -1)
				user.visible_message(span_danger("[user] abruptly stops cutting [M]'s hair and slices [M.p_their()] throat!"), span_danger("You stop cutting [M]'s hair and slice [M.p_their()] throat!")) //Just a little off the top.
