...
obj
	item
		stack
			tape_roll
				attack(mob/living/carbon/human/M as mob, mob/living/user as mob)
					if(M.wear_mask)
								...
								obj
									item
										clothing
											mask
												muzzle
													G = new /obj/item/clothing/mask/muzzle/tapegag
...
obj
	item
		stack
			tape_roll
				proc
					stick(var/obj/item/weapon/W, mob/user)
...
obj
	item
		weapon
			ducttape
				afterattack(var/A, mob/user, flag, params)
					...
					if(params)
						var
							list
								mouse_control = params2list(params)
