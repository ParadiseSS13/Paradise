		...
		weapon
			locator
				Topic(href, href_list)
					...
					var
						turf
							current_location = get_turf(usr)//What turf is the user on?
		...
		weapon
			hand_tele
				attack_self(mob/user as mob)
					var
						turf
							current_location = get_turf(user)//What turf is the user on?
					...
					var
						t1 = input(user, "Please select a teleporter to lock in on.", "Hand Teleporter") as null|anything in L
					...
					var
						obj
							effect
								portal
									P = new /obj/effect/portal(get_turf(src), T, src)
					try_move_adjacent(P)
					active_portals++
