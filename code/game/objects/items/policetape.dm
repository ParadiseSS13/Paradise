...
obj
	item
		taperoll
			New() //just using tape is not enough to guarantee the overlays are genned
				..()
				if(!hazard_overlays)
					hazard_overlays = list()
					hazard_overlays["[NORTH]"]	= new/image('icons/effects/warning_stripes.dmi', icon_state = "N")
...
obj
	item
		taperoll
			afterattack(var/atom/A, mob/user as mob, proximity)
				if(!proximity)
					return
				...
				if(istype(A, /turf/simulated/floor) ||istype(A, /turf/unsimulated/floor))
					var
						turf
							F = A
					var
						direction = user.loc == F ? user.dir : turn(user.dir, 180)
					var
						icon
							hazard_overlay = hazard_overlays["[direction]"]
...
obj
	item
		tape
			proc
				breaktape(obj/item/weapon/W as obj, mob/user as mob)
