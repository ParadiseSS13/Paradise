...
obj
	item
		toy
			balloon
				afterattack(atom/A as mob|obj, mob/user as mob, proximity)
					if(!proximity) return
	...
	item
		weapon
			twohanded
				dualsaber
					toy
						IsReflect()//Stops Toy Dualsabers from reflecting energy projectiles
							return 0
	...
	item
		toy
			cards
				...
				var
					list
						card_attack_verb = list("attacked")
...
obj
	item
		toy
			cards
				proc
					apply_card_vars(obj/item/toy/cards/newobj, obj/item/toy/cards/sourceobj) // Applies variables for supporting multiple types of card deck
						if(!istype(sourceobj))
							return
							...
							obj
								item
									toy
										cards
											singlecard
												H = new/obj/item/toy/cards/singlecard(user.loc)
						choice = cards[1]
						H.cardname = choice
						H.parentdeck = src
						var
...
obj
	item
		toy
			cards
				deck
					attackby(obj/item/toy/cards/singlecard/C, mob/living/user, params)
						..()
						if(istype(C))
							if(C.parentdeck == src)
								if(!user.unEquip(C))
...
obj
	item
		toy
			cards
				deck
					attackby(obj/item/toy/cards/cardhand/C, mob/living/user, params)
						..()
						if(istype(C))
							if(C.parentdeck == src)
								if(!user.unEquip(C))
				...
				cardhand
					interact(mob/user)
						...
						var
							datum
								browser
									popup = new(user, "cardhand", "Hand of Cards", 400, 240)
									...
									obj
										item
											toy
												cards
													singlecard
														C = new/obj/item/toy/cards/singlecard(cardUser.loc)
								currenthand -= choice
										...
										obj
											item
												toy
													cards
														singlecard
															N = new/obj/item/toy/cards/singlecard(src.loc)
...
obj
	item
		toy
			cards
				cardhand
					apply_card_vars(obj/item/toy/cards/newobj,obj/item/toy/cards/sourceobj)
						..()
									...
									obj
										item
											toy
												cards
													cardhand
														H = new/obj/item/toy/cards/cardhand(user.loc)
...
obj
	item
		toy
			cards
				singlecard
					apply_card_vars(obj/item/toy/cards/singlecard/newobj,obj/item/toy/cards/sourceobj)
						..()
						newobj.deckstyle = sourceobj.deckstyle
	...
	item
		toy
			plushie
				...
				icon = 'icons/obj/toy.dmi'
				var
					poof_sound = 'sound/weapons/thudswoosh.ogg'
	...
	item
		toy
			plushie
				attack_self(mob/user as mob)
					var
						cuddle_verb = pick("hugs","cuddles","snugs")
		...
		toy
			owl
				attack_self(mob/user)
					if(!cooldown) //for the sanity of everyone
						var
							message = pick("You won't get away this time, Griffin!", "Stop right there, criminal!", "Hoot! Hoot!", "I am the night!")
		...
		toy
			griffin
				attack_self(mob/user)
					if(!cooldown) //for the sanity of everyone
						var
							message = pick("You can't stop me, Owl!", "My plan is flawless! The vault is mine!", "Caaaawwww!", "You will never catch me!")
						to_chat(user, "<span class='notice'>You pull the string on the [src].</span>")
...
obj
	item
		toy
			minigibber
				attackby(var/obj/O, var/mob/user, params)
			...
			toy_xeno
				attack_self(mob/user)
					if(cooldown <= world.time)
						...
						var
							list
								possible_sounds = list('sound/voice/hiss1.ogg', 'sound/voice/hiss2.ogg', 'sound/voice/hiss3.ogg', 'sound/voice/hiss4.ogg')
	...
	item
		toy
			eight_ball
				...
				var
					use_action = "shakes the ball"
				var
					cooldown = 0
				var
					list
						possible_answers = list("Definitely", "All signs point to yes.", "Most likely.", "Yes.", "Ask again later.", "Better not tell you now.", "Future Unclear.", "Maybe.", "Doubtful.", "No.", "Don't count on it.", "Never.")
