				...
				glass
					attackby(obj/item/W, mob/user, params)
						...
						if(istype(W,/obj/item/stack/cable_coil))
							...
							src = null
							var
								replace = (user.get_inactive_hand()==G)
							G.use(1)
							if(!G && !RG && replace)
	...
	item
		stack
			sheet
				glass
					proc
						construct_window(mob/user as mob)
								...
								if("Fishbowl")
									...
									var
										obj
											machinery
												fishtank
													F = new /obj/machinery/fishtank/bowl(user.loc, 0)
									F.air_update_turf(1)
									src.use(1)
								if("Fish Tank")
									if(!src)	return 1
									...
									var
										obj
											machinery
												fishtank
													F = new /obj/machinery/fishtank/tank(user.loc, 0)
									F.air_update_turf(1)
									src.use(3)
								if("Wall Aquarium")
									if(!src)	return 1
									...
									var
										obj
											machinery
												fishtank
													F = new /obj/machinery/fishtank/wall(user.loc, 0)
									F.air_update_turf(1)
									src.use(4)
							return 0
					...
					proc
						construct_window(mob/user as mob)
							...
							if(!user.IsAdvancedToolUser())
								...
								return 0
							var
								title = "Reinforced plasma-glass alloy"
							title += " ([src.amount] sheet\s left)"
							switch(alert(title, "Would you like full tile glass or one direction?", "One Direction", "Full Window", "Cancel", null))
								if("One Direction")
									if(!src)  return 1
