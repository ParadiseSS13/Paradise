...
obj
	item
		stack
			proc
				list_recipes(mob/user as mob, recipes_sublist)
					if(!recipes)
						return
					if(!src || amount<=0)
					...
					var
						t1 = text("<HTML><HEAD><title>Constructions from []</title></HEAD><body><TT>Amount Left: []<br>", src, src.amount)
							...
							var
								datum
									stack_recipe
										R = E
							var
								max_multiplier = round(src.amount / R.req_amount)
							var
								title as text
							var
								can_build = 1
				...
				if(href_list["make"])
					...
					if(href_list["sublist"])
						var
							datum
								stack_recipe_list
									srl = recipes_list[text2num(href_list["sublist"])]
						recipes_list = srl.recipes
					var
						datum
							stack_recipe
								R = recipes_list[text2num(href_list["make"])]
					var
		...
		stack
			attackby(obj/item/W as obj, mob/user as mob, params)
				...
				if(istype(W, src.type))
					...
					if(user.get_inactive_hand()==src)
						var
							desired = input("How much would you like to transfer from this stack?", "How much?", 1) as null|num
						if(!desired)
							return
...
obj
	item
		stack
			proc
				copy_evidences(obj/item/stack/from as obj)
