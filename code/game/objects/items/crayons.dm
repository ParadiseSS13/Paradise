	...
	item
		toy
			crayon
				...
				var
					list
						graffiti = list("body","amyjon","face","matt","revolution","engie","guy","end","dwarf","uboa","up","down","left","right","heart","borgsrogue","voxpox","shitcurity","catbeast","hieroglyphs1","hieroglyphs2","hieroglyphs3","security","syndicate1","syndicate2","nanotrasen","lie","valid")
				var
					list
				...
				var
					list
						validSurfaces = list(/turf/simulated/floor)
...
obj
	item
		toy
			crayon
				proc
					update_window(mob/living/user as mob)
						...
						var
							datum
								browser
									popup = new(user, "crayon", name, 300, 500)
...
obj
	item
		toy
			crayon
				afterattack(atom/target, mob/user as mob, proximity)
					if(!proximity) return
		...
		toy
			crayon
				attack(mob/M as mob, mob/user as mob)
					var
						huffable = istype(src,/obj/item/toy/crayon/spraycan)
					if(M == user)
			...
			crayon
				rainbow
					Topic(href,href_list[])
						...
						if(href_list["color"])
							var
								temp = input(usr, "Please select colour.", "Crayon colour") as color
							if((usr.restrained() || usr.stat || usr.get_active_hand() != src))
								return
							colour = temp
							update_window(usr)
		...
		toy
			crayon
				spraycan
					attack_self(mob/living/user as mob)
						var
							choice = input(user,"Spraycan options") in list("Toggle Cap","Change Drawing","Change Color")
						switch(choice)
							if("Toggle Cap")
			...
			crayon
				spraycan
					update_icon()
						...
						var
							image
								I = image('icons/obj/crayons.dmi',icon_state = "[capped ? "spraycan_cap_colors" : "spraycan_colors"]")
						I.color = colour
						overlays += I
