...
/obj/item/roulette
	...
	var/spinning = FALSE
	/// List of custom options the player inserted into the roulette
...
/obj/item/roulette/AltClick()
	if(spinning)
		return
	if(!length(options))
		atom_say("Please insert the options before spinning it.")
		return
...
/obj/item/roulette/attack_hand(mob/user)
	...
	var/list/radials = list("Option 1" = image(icon = 'icons/obj/stationobjs.dmi', icon_state = "roulette"),
							"Option 2" = image(icon = 'icons/obj/stationobjs.dmi', icon_state = "roulette"),
							"Option 3" = image(icon = 'icons/obj/stationobjs.dmi', icon_state = "roulette"),
							"Option 4" = image(icon = 'icons/obj/stationobjs.dmi', icon_state = "roulette"),
							"Option 5" = image(icon = 'icons/obj/stationobjs.dmi', icon_state = "roulette"),
							"Option 6" = image(icon = 'icons/obj/stationobjs.dmi', icon_state = "roulette")
							)
	var/choice = show_radial_menu(user, src, radials, require_near = TRUE)
	switch(choice)
		if("Option 1")
