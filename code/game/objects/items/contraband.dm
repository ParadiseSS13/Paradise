...
/obj/item/reagent_containers/food/pill/random_drugs/New()
	...
	while(primaries > 0)
		primaries--
		reagents.add_reagent(text2path(pick_list("chemistry_tools.json", "CYBERPUNK_drug_primaries")), 6)
	while(adulterants > 0)
		adulterants--
		reagents.add_reagent(text2path(pick_list("chemistry_tools.json", "CYBERPUNK_drug_adulterants")), 3)
