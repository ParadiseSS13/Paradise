...
var
	global
		member_ooc_colour = "#035417"
var
	global
		mentor_ooc_colour = "#0099cc"
var
	global
		moderator_ooc_colour = "#184880"
var
	global
		admin_ooc_colour = "#b82e00"
				...
				if(C.prefs.toggles & CHAT_OOC)
					...
					if(prefs.unlock_content)
						if(prefs.toggles & MEMBER_PUBLIC)
							var
								icon
									byond = icon('icons/member_content.dmi', "blag")
...
client
	proc
		colorooc()
			...
			var
				new_ooccolor = input(src, "Please select your OOC color.", "OOC color", prefs.ooccolor) as color|null
			if(new_ooccolor)
...
client
	verb
		looc(msg as text)
			...
			var
				mob
					source = mob.get_looc_source()
			var
				list
					heard = get_mobs_in_view(7, source)
