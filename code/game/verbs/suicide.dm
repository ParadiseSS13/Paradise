...
mob
	living
		carbon
			human
				proc
					do_suicide(damagetype, byitem)
						var
							threshold = (config.health_threshold_crit + config.health_threshold_dead) / 2
						var
							dmgamt = maxHealth - threshold
			...
			pai
				verb
					suicide()
						...
						var
							answer = input("REALLY kill yourself? This action can't be undone.", "Suicide", "No") in list ("Yes", "No")
						if(answer == "Yes")
							if(canmove || resting)
								close_up()
							var
