atom
	movable
		...
		var
			throw_range = 7
		var
			no_spin_thrown = 0 //set this to 1 if you don't want an item that you throw to spin, no matter what. -Fox
		var
			moved_recently = 0
		var
			mob
...
atom
	movable
		proc
			setLoc(var/T, var/teleported=0)
				loc = T

atom
	movable
		Move(atom/newloc, direct = 0)
...
atom
	movable
		Bump(var/atom/A as mob|obj|turf|area, sendBump)
			if(src.throwing)
				src.throw_impact(A)
...
atom
	movable
		proc
			throw_impact(atom/hit_atom, var/speed)
...
atom
	movable
		proc
			newtonian_move(direction) //Only moves the object if it's under no gravity
...
atom
	movable
		proc
			throw_at(atom/target, range, speed, thrower, no_spin)
...
atom
	movable
		proc
			water_act(var/volume, var/temperature, var/source) //amount of water acting : temperature of water in kelvin : object that called it (for shennagins)
				return 1

atom
	movable
		proc
			handle_buckled_mob_movement(newloc,direct)
...
atom
	movable
		CanPass(atom/movable/mover, turf/target, height=1.5)
