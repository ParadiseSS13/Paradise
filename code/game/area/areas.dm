...
area
	proc
		atmosalert(danger_level, var/alarm_source)
			if(danger_level == 0)
...
area
	proc
		powered(var/chan)		// return true if the area has power to given channel
...
area
	proc
		use_battery_power(var/amount, var/chan)
			switch(chan)
				if(EQUIP)
					used_equip += amount
				if(LIGHT)
...
area
	proc
		gravitychange(var/gravitystate = 0, var/area/A)
...
area
	proc
		thunk(var/mob/living/carbon/human/M)
