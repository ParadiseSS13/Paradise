...
area
	CATEGORY
		OR
			DESCRIPTOR
				NAME 	(you can make as many subdivisions as you want)
...
area
	...
	var
		radalert = 0
	var
		report_alerts = 1 // Should atmos alerts notify the AI/computers
	level = null
	name = "Space"
...
//	var/list/lights				// list of all lights on this area
	var
		list
			all_doors = list()		//Added by Strumpetplaya - Alarm Change - Contains a list of doors adjacent to this area
	var
		...
		proc
			process_teleport_locs()
				for(var/area/AR in world)
					...
					var
						turf
							picked = safepick(get_area_turfs(AR.type))
...
hook
	startup
		proc
			process_ghost_teleport_locs()
				for(var/area/AR in world)
					if(ghostteleportlocs.Find(AR.name)) continue
					var
						list
							turfs = get_area_turfs(AR.type)
					if(turfs.len)
...
area
	shuttle
		escape_pod5 //Pod 4 was lost to meteors
...
area
	airtunnel1/      // referenced in airtunnel.dm:759

area
	dummy/           // Referenced in engine.dm:261
...
area
	assembly
		assembly_line //Derelict Assembly Line
		...
		spawn(10) //let objects set up first
			for(var/turf/turfToGrayscale in src)
				if(turfToGrayscale.icon)
					var
						icon
							newIcon = icon(turfToGrayscale.icon)
					newIcon.GrayScale()
				...
				for(var/obj/objectToGrayscale in turfToGrayscale) //1 level deep, means tables, apcs, locker, etc, but not locker contents
					if(objectToGrayscale.icon)
						var
							icon
								newIcon = icon(objectToGrayscale.icon)
						newIcon.GrayScale()
	...
	list
		the_station_areas = list (
			...
			area
				ai_monitored
					storage
						eva, //do not try to simplify to "/area/ai_monitored" --rastaf0
			area
				ai_monitored
					storage
						secure,
			...
			area
				turret_protected
					ai_upload, //do not try to simplify to "/area/turret_protected" --rastaf0
			area
				turret_protected
					ai_upload_foyer,
			area
