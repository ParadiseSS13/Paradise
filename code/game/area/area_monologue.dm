...
/area/shuttle/escape_pod1/centcom/get_monologue(var/voxxy)
	if(voxxy)
		return "Is was finally overs, yaya. Voxxy needed rest and triple citrus. The next shift would start soon..."
	else
...
/area/maintenance/get_monologue(var/voxxy)
	if(voxxy)
...
/area/maintenance/disposal/get_monologue(var/voxxy)
	if(voxxy)
...
/area/hallway/get_monologue(var/voxxy)
	if(voxxy)
		return "The dustlungs go about their bussiness while Voxxy watches. They never understoods what is all about. yaya."
	else
...
/area/hallway/secondary/exit/get_monologue(var/voxxy)
	if(voxxy)
...
/area/hallway/secondary/entry/get_monologue(var/voxxy)
	if(voxxy)
...
/area/bridge/get_monologue(var/voxxy)
	if(voxxy)
...
/area/crew_quarters/get_monologue(var/voxxy)
	if(voxxy)
...
/area/crew_quarters/bar/get_monologue(var/voxxy)
	if(voxxy)
...
/area/chapel/get_monologue(var/voxxy)
	if(voxxy)
		return "Voxxy was too busy to look at Chapel decorations. Were skrek as always, yaya. "
	else
...
/area/engine/get_monologue(var/voxxy)
	if(voxxy)
...
/area/medical/get_monologue(var/voxxy)
	if(voxxy)
...
/area/security/get_monologue(var/voxxy)
	if(voxxy)
...
/area/security/detectives_office/get_monologue(var/voxxy)
	if(voxxy)
		return "Is Voxxy office. Is safe places. Safe places for a shift like this, yaya."
	else
...
/area/quartermaster/get_monologue(var/voxxy)
	if(voxxy)
...
/area/hydroponics/get_monologue(var/voxxy)
	if(voxxy)
		return "Botany, yaya. Is drugs and crime, mostly. But maybe that's what station needs, ech..."
	else
