...
datum
	mind
		proc
			findJobTask(var/typepath, var/options = 0)
				var
					datum
						job_objective
							task = locate(typepath) in job_objectives
...
datum
	job_objective
		...
		var
			units_completed = 0
		var
			units_compensated = 0 // Shit paid for
		var
			units_requested = INFINITY
		var
			completion_payment = 0			// Credits paid to owner when completed

datum
	job_objective
		New(var/datum/mind/new_owner)
			owner = new_owner
...
datum
	job_objective
		proc
			get_description()
				var
					desc = "Placeholder Objective"
				return desc

datum
	job_objective
		proc
			unit_completed(var/count=1)
	...
	game_mode
		proc
			declare_job_completion()
				var
					text = "<hr><b><u>Job Completion</u></b>"
