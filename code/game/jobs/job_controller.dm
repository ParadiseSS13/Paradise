...
datum
	controller
		occupations
			...
			proc
				SetupOccupations(var/list/faction = list("Station"))
					occupations = list()
					var
						list
							all_jobs = subtypesof(/datum/job)
					if(!all_jobs.len)
			...
			proc
				GetPlayerAltTitle(mob/new_player/player, rank)
					return player.client.prefs.GetPlayerAltTitle(GetJob(rank))

			proc
				AssignRole(var/mob/new_player/player, var/rank, var/latejoin = 0)
			...
			proc
				FreeRole(var/rank)	//making additional slot on the fly
					var
						datum
							job
								job = GetJob(rank)
			...
			proc
				FindOccupationCandidates(datum/job/job, level, flag)
			...
			proc
				GiveRandomJob(var/mob/new_player/player)
					...
					for(var/level = 1 to 3)
						for(var/command_position in command_positions)
							...
							var
								mob
									new_player
										candidate = pickweight(weightedCandidates)
...
/** Proc DivideOccupations
	...
	**/
	proc
		DivideOccupations()
			...
			var
				datum
					job
						civ = new /datum/job/civilian()
			var
				list
					civilian_candidates = FindOccupationCandidates(civ, 3)
	...
	proc
		EquipRank(var/mob/living/carbon/human/H, var/rank, var/joined_late = 0)
			if(!H)	return null
			var
				datum
					job
			...
			if(job)
				...
				var
					list
						custom_equip_slots = list() //If more than one item takes the same slot, all after the first one spawn in storage.
				var
					list
						custom_equip_leftovers = list()
			...
			var
				datum
					money_account
						M = create_account(H.real_name, rand(50,500)*10, null)
			if(H.mind)
				var
					remembered_info = ""
			...
			if(H.mind && job.head_position)
				...
				var
					datum
						money_account
							department_account = department_accounts[job.department]
			...
			if(H.mind)
				...
				for(var/thing in spawn_in_storage)
					...
					var
						metadata = H.client.prefs.gear[G.display_name]
					var
						item = G.spawn_item(null, metadata)

					var
						atom
							placed_in = H.equip_or_collect(item)
					if(placed_in)
			...
			if(H.disabilities & DISABILITY_FLAG_NEARSIGHTED)
				var
					equipped = H.equip_to_slot_or_del(new /obj/item/clothing/glasses/regular(H), slot_glasses)
				if(equipped != 1)
					var
						obj
							item
	...
	proc
		spawnId(var/mob/living/carbon/human/H, rank, title)
			if(!H)	return 0
			var
				obj
					item
				...
				var
					obj
						item
							device
								pda
									pda = locate(/obj/item/device/pda,H)
	...
	proc
		LoadJobs(jobsfile) //ran during round setup, reads info from jobs.txt -- Urist
			if(!config.load_jobs_from_txt)
				return 0

			var
				list
					jobEntries = file2list(jobsfile)
