...
var
	const
		access_brig = 2 // Brig timers and permabrig
var
	const
		access_armory = 3
var
...
var
	const
		access_sec_doors = 63 // Security front doors
var
	const
		access_psychiatrist = 64 // Psychiatrist's office
var
	const
		access_xenoarch = 65
var
...
var
	const
		access_blueshield = 67
var
	const
		access_salvage_captain = 69 // Salvage ship captain's quarters
var
	const
		access_mechanic = 70
var
...
var
	const
		access_magistrate = 74
var
	const
		access_minisat = 75
var
	const
		access_mineral_storeroom = 76

var
	const
		access_weapons = 99 //Weapon authorization for secbots
...
var
	const
		access_cent_living = 102//Living quarters.
var
	const
		access_cent_medical = 103//Medical.
var
	const
		access_cent_security = 104//Security.
var
	const
		access_cent_storage = 105//Storage areas.
var
	const
		access_cent_shuttles = 106//Shuttle docks.
var
	const
		access_cent_telecomms = 107//Telecomms.
var
	const
		access_cent_teleporter = 108//Telecomms.
var
	const
		access_cent_specops = 109//Special Ops.
var
	const
		access_cent_specops_commander = 110//Special Ops Commander.
var
	const
		access_cent_blackops = 111//Black Ops.
var
	const
		access_cent_thunder = 112//Thunderdome.
var
	const
		access_cent_bridge = 113//Bridge.
var
	const
		access_cent_commander = 114//Commander's Office/ID computer.
...
var
	const
		access_syndicate_leader = 151//Nuke Op Leader Access
var
	const
		...
		generate_req_lists()
			...
			if(!req_access)
				...
				if(req_access_txt)
					var
						list
							req_access_str = splittext(req_access_txt, ";")
			...
			if(!req_one_access)
				...
				if(req_one_access_txt)
					var
						list
							req_one_access_str = splittext(req_one_access_txt,";")
...
proc
	has_access(var/list/req_access, var/list/req_one_access, var/list/accesses)
...
proc
	get_all_jobs()
		...
		var
			list
				all_datums = subtypesof(/datum/job)
...
proc
	GetIdCard(var/mob/living/carbon/human/H)
		if(H.wear_id)
			var
				id = H.wear_id.GetID()
			if(id)
...
proc
	get_all_job_icons() //For all existing HUD icons
		return joblist + list("Prisoner")

obj
	proc
		GetJobName() //Used in secHUD icon generation
			var
				obj
					item
						weapon
			...
			if(I)
				var
					job_icons = get_all_job_icons()
				var
					centcom = get_all_centcom_jobs()
