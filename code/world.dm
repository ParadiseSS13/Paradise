...
var
	world_topic_spam_protect_ip = "0.0.0.0"
var
	...
	Topic(T, addr, master, key)
		...
		var
			list
				input = params2list(T)
		var
			key_valid = (config.comms_password && input["key"] == config.comms_password) //no password means no comms, not any password
		...
		if("ping" in input)
			...
			*/
			...
			var
				message =	"<font color='red'>IRC-Admin PM from <b><a href='?irc_msg=1'>[C.holder ? "IRC-" + input["sender"] : "Administrator"]</a></b>: [input["msg"]]</font>"
			var
				amessage =  "<font color='blue'>IRC-Admin PM from <a href='?irc_msg=1'>IRC-[input["sender"]]</a> to <b>[key_name(C)]</b> : [input["msg"]]</font>"
...
world
	Reboot(var/reason, var/feedback_c, var/feedback_r, var/time)
...
world
	proc
		KickInactiveClients()
			var
				tmp
					sleep_check = 0 // buffer for checking elapsed ticks
			var
				tmp
					work_length = 2 // number of ticks to run before yielding cpu
			var
				tmp
					sleep_length = 5 // number of ticks to yield
			var
				waiting=1
...
world
	proc
		save_mode(var/the_mode)
			var
				F = file("data/mode.txt")
			fdel(F)
			F << the_mode
