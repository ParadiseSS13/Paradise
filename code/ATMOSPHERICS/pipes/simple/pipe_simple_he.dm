			...
			pipe
				simple
					heat_exchanging
						...
						buckle_lying = 1
						var
							icon_temperature = T20C //stop small changes in temperature causing icon refresh
						...
						process()
							...
							if(pipe_air.temperature && (icon_temperature > 500 || pipe_air.temperature > 500)) //glow starts at 500K
								if(abs(pipe_air.temperature - icon_temperature) > 10)
									...
									var
										h_r = heat2color_r(icon_temperature)
									var
										h_g = heat2color_g(icon_temperature)
									var
										h_b = heat2color_b(icon_temperature)

									if(icon_temperature < 2000)//scale glow until 2000K
										var
											scale = (icon_temperature - 500) / 1500
										h_r = 64 + (h_r - 64) * scale
										h_g = 64 + (h_g - 64) * scale
										h_b = 64 + (h_b - 64) * scale
