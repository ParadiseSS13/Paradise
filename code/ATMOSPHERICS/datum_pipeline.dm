...
var
	global
		list
			deferred_pipenet_rebuilds = list()
...
datum
	pipeline
		proc
			build_pipeline(obj/machinery/atmospherics/base)
				var
					volume = 0
				...
				var
					list
						possible_expansions = list(base)
				while(possible_expansions.len>0)
					for(var/obj/machinery/atmospherics/borderline in possible_expansions)

						var
							list
								result = borderline.pipeline_expansion(src)
...
datum
	pipeline
		proc
			addMachineryMember(obj/machinery/atmospherics/A)
				other_atmosmch |= A
				var
					datum
						gas_mixture
							G = A.returnPipenetAir(src)
				other_airs |= G

datum
	pipeline
		proc
			addMember(obj/machinery/atmospherics/A, obj/machinery/atmospherics/N)
...
obj
	machinery
		atmospherics
			proc
				addMember(obj/machinery/atmospherics/A)
					return
...
datum
	pipeline
		proc
			temperature_interact(turf/target, share_volume, thermal_conductivity)
				var
					total_heat_capacity = air.heat_capacity()
				var
					partial_heat_capacity = total_heat_capacity*(share_volume/air.volume)
				...
				if(istype(target, /turf/simulated))
					...
					if(modeled_location.blocks_air)
						...
						if((modeled_location.heat_capacity>0) && (partial_heat_capacity>0))
							var
								delta_temperature = air.temperature - modeled_location.temperature
					...
					if((target.heat_capacity>0) && (partial_heat_capacity>0))
						var
							delta_temperature = air.temperature - target.temperature
						...
						if(total_trace_gases.len)
							for(var/datum/gas/trace_gas in total_trace_gases)
								var
									datum
										gas
											corresponding = locate(trace_gas.type) in G.trace_gases
								if(!corresponding)
									corresponding = new trace_gas.type()
