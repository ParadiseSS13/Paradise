...
var
	global
		list
			pipe_colors = list("grey" = PIPE_COLOR_GREY, "red" = PIPE_COLOR_RED, "blue" = PIPE_COLOR_BLUE, "cyan" = PIPE_COLOR_CYAN, "green" = PIPE_COLOR_GREEN, "yellow" = PIPE_COLOR_YELLOW, "purple" = PIPE_COLOR_PURPLE)

proc
	pipe_color_lookup(var/color)
		for(var/C in pipe_colors)
			if(color == pipe_colors[C])
				return "[C]"

proc
	pipe_color_check(var/color)
		if(!color)
			return 1
		for(var/C in pipe_colors)
			if(color == pipe_colors[C])
...
datum
	pipe_icon_manager
		proc
			get_atmos_icon(var/device, var/dir, var/color, var/state)
				check_icons()
	...
	pipe_icon_manager
		proc
			gen_pipe_icons()
				...
				var
					icon
						pipe = new('icons/atmos/pipes.dmi')
	...
	pipe_icon_manager
		proc
			gen_manifold_icons()
				...
				var
					icon
						pipe = new('icons/atmos/manifold.dmi')
	...
	pipe_icon_manager
		proc
			gen_omni_icons()
				...
				var
					icon
						omni = new('icons/atmos/omni_devices.dmi')
	...
	pipe_icon_manager
		proc
			gen_underlay_icons()
				...
				var
					icon
						pipe = new('icons/atmos/pipe_underlays.dmi')
