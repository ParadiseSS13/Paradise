			...
			unary
				vent_scrubber
					...
					var
						on = 0
					var
						scrubbing = 1 //0 = siphoning, 1 = scrubbing
					var
						scrub_O2 = 0
					var
						scrub_N2 = 0
					...
					var
						volume_rate = 200
					var
						widenet = 0 //is this scrubber acting on the 3x3 area around it.
						...
						broadcast_status()
							...
							if(frequency == 1439)//We're on the frequency the air alarms and stuff use
								if(!initial_loc.air_scrub_names[id_tag])
									var
										new_name = "[initial_loc.name] Air Scrubber #[initial_loc.air_scrub_names.len+1]"
					...
					proc
						scrub(var/turf/simulated/tile)
							...
							if(scrubbing)
								...
								var
									transfer_moles = environment.total_moles()*(volume_rate/environment.volume)
