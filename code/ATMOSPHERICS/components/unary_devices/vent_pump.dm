		...
		atmospherics
			unary
				vent_pump
					...
					var
						external_pressure_bound = EXTERNAL_PRESSURE_BOUND
					var
						internal_pressure_bound = INTERNAL_PRESSURE_BOUND
					...
					var
						external_pressure_bound_default = EXTERNAL_PRESSURE_BOUND
					var
						internal_pressure_bound_default = INTERNAL_PRESSURE_BOUND
					var
						pressure_checks_default = PRESSURE_CHECKS

					var
						welded = 0 // Added for aliens -- TLE
						...
						broadcast_status()
							...
							if(frequency == 1439)//We're on the frequency the air alarms and stuff use
								if(!initial_loc.air_vent_names[id_tag])
									var
										new_name = "[initial_loc.name] Vent Pump #[initial_loc.air_vent_names.len+1]"
