	...
	machinery
		atmospherics
			omni
				mixer
					proc
						switch_mode(var/port = NORTH, var/mode = ATM_NONE)
	...
	machinery
		atmospherics
			omni
				mixer
					proc
						change_concentration(var/port = NORTH)
							tag_north_con = null
							tag_south_con = null
							tag_east_con = null
							tag_west_con = null
							...
							var
								new_con = (input(usr,"Enter a new concentration (0-[round(remain_con * 100, 0.5)])%","Concentration control", min(remain_con, old_con)*100) as num) / 100
