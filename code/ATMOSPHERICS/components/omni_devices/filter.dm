				...
				filter
					process()
						...
						var
							datum
								gas_mixture
									output_air = output.air	//BYOND doesn't like referencing "output.air.return_pressure()" so we need to make a direct reference
						var
							datum
								gas_mixture
									input_air = input.air		// it's completely happy with them if they're in a loop though i.e. "P.air.return_pressure()"... *shrug*

						var
							output_pressure = output_air.return_pressure()
						...
						if(input.transfer_moles > 0)
							var
								datum
									gas_mixture
										removed = input_air.remove(input.transfer_moles)
	...
	machinery
		atmospherics
			omni
				filter
					proc
						mode_send_switch(var/mode = ATM_NONE)
							switch(mode)
								if(ATM_O2)
									return "Oxygen"
								if(ATM_N2)
					...
					Topic(href, href_list)
						...
						if(configuring && !on)
							switch(href_list["command"])
								if("set_pressure")
									var
										new_pressure = input(usr,"Enter new output pressure (0-4500kPa)","Pressure control",target_pressure) as num
									target_pressure = between(0, new_pressure, 4500)
								if("switch_mode")
									switch_mode(dir_flag(href_list["dir"]), mode_return_switch(href_list["mode"]))
								if("switch_filter")
									var
										new_filter = input(usr,"Select filter mode:","Change filter",href_list["mode"]) in list("None", "Oxygen", "Nitrogen", "Carbon Dioxide", "Plasma", "Nitrous Oxide")
	...
	machinery
		atmospherics
			omni
				filter
					proc
						switch_filter(var/dir, var/mode)
	...
	machinery
		atmospherics
			omni
				filter
					proc
						handle_port_change(var/datum/omni_port/P)
							switch(P.mode)
								if(ATM_NONE)
