			...
			binary
				dp_vent_pump
					...
					var
						on = 0
					var
						pump_direction = 1 //0 = siphoning, 1 = releasing
					...
					var
						datum
							radio_frequency
								radio_connection
					var
						advcontrol = 0//does this device listen to the AAC
						...
						if(pump_direction) //input -> external
							...
							if(pressure_delta > 0)
								if(air1.temperature > 0)
									var
										transfer_moles = pressure_delta*environment.volume/(air1.temperature * R_IDEAL_GAS_EQUATION)
							...
							if(pressure_delta > 0)
								if(environment.temperature > 0)
									var
										transfer_moles = pressure_delta*air2.volume/(environment.temperature * R_IDEAL_GAS_EQUATION)
