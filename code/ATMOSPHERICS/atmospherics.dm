...
obj
	machinery
		atmospherics
			...
			var
				connect_types[] = list(1) //1=regular, 2=supply, 3=scrubber
			var
				connected_to = 1 //same as above, currently not used for anything
			var
				icon_connect_type = "" //"-supply" or "-scrubbers"
...
obj
	machinery
		atmospherics
			proc
				color_cache_name(var/obj/machinery/atmospherics/node)
...
obj
	machinery
		atmospherics
			proc
				add_underlay(var/turf/T, var/obj/machinery/atmospherics/node, var/direction, var/icon_connect_type)
					if(node)
...
obj
	machinery
		atmospherics
			proc
				check_connect_types(obj/machinery/atmospherics/atmos1, obj/machinery/atmospherics/atmos2)
					var
						i
					var
						list1[] = atmos1.connect_types
					var
						list2[] = atmos2.connect_types
...
obj
	machinery
		atmospherics
			proc
				check_connect_types_construction(obj/machinery/atmospherics/atmos1, obj/item/pipe/pipe2)
					var
						i
					var
						list1[] = atmos1.connect_types
					var
						list2[] = pipe2.connect_types
					for(i=1,i<=list1.len,i++)
						var
							j
						for(j=1,j<=list2.len,j++)
		...
		atmospherics
			proc
				unsafe_pressure_release(var/mob/user,var/pressures)
					...
					var
						fuck_you_dir = get_dir(src,user)
					var
						turf
							general_direction = get_edge_target_turf(user,fuck_you_dir)
			...
			proc
				findConnecting(var/direction)
					for(var/obj/machinery/atmospherics/target in get_step(src,direction))
						var
							can_connect = check_connect_types(target, src)
...
obj
	machinery
		atmospherics
			relaymove(mob/living/user, direction)
				...
				if(target_move)
					...
					var
						can_connect = check_connect_types(target_move, src)
...
obj
	machinery
		atmospherics
			proc
				universal_underlays(var/obj/machinery/atmospherics/node, var/direction)
					var
						turf
							T = get_turf(src)
					if(!istype(T)) return
...
obj
	machinery
		atmospherics
			proc
				add_underlay_adapter(var/turf/T, var/obj/machinery/atmospherics/node, var/direction, var/icon_connect_type) //modified from add_underlay, does not make exposed underlays
					if(node)
						if(T.intact && node.level == 1 && istype(node, /obj/machinery/atmospherics/pipe))
							underlays += icon_manager.get_atmos_icon("underlay", direction, color_cache_name(node), "down" + icon_connect_type)
						else
