...
turf
	remove_air(amount as num)
		...
		var
			sum = oxygen + carbon_dioxide + nitrogen + toxins
		if(sum>0)
...
turf
	simulated
		...
		var
			atmos_overlay_type = "" //current active overlay
...
turf
	simulated
		proc
			copy_air_with_tile(turf/simulated/T)
...
turf
	simulated
		proc
			copy_air(datum/gas_mixture/copy)
				if(air && copy)
					air.copy_from(copy)
...
turf
	simulated
		proc
			mimic_temperature_solid(turf/model, conduction_coefficient)
				var
...
turf
	simulated
		proc
			share_temperature_mutual_solid(turf/simulated/sharer, conduction_coefficient)
				var
	...
	simulated
		proc
			process_cell()
				...
				var
					remove = 1 //set by non simulated turfs who are sharing with this turf
				...
				for(var/direction in cardinal)
					...
					/******************* GROUP HANDLING FINISH *********************************************************************/
						...
						if(!air.check_turf(enemy_tile, atmos_adjacent_turfs_amount))
							var
								difference = air.mimic(enemy_tile,atmos_adjacent_turfs_amount)
							if(difference)
								if(difference > 0)
	...
	simulated
		proc
			update_visuals()
				...
				var
					new_overlay_type = tile_graphic()
				if(new_overlay_type == atmos_overlay_type)
					return
				var
					atmos_overlay = get_atmos_overlay_by_name(atmos_overlay_type)
				if(atmos_overlay)
...
turf
	simulated
		proc
			get_atmos_overlay_by_name(var/name)
				switch(name)
					if("plasma")
						return plmaster
					if("sleeping_agent")
		...
		proc
			tile_graphic()
				...
				var
					datum
						gas
							sleeping_agent = locate(/datum/gas/sleeping_agent) in air.trace_gases
...
turf
	simulated
		proc
			share_air(var/turf/simulated/T)
...
turf
	proc
		consider_pressure_difference(var/turf/simulated/T, var/difference)
...
datum
	excited_group
		proc
			merge_groups(var/datum/excited_group/E)
...
turf
	simulated
		proc
			radiate_to_spess() //Radiate excess tile heat to space
				if(temperature > T0C) //Considering 0 degC as te break even point for radiation in and out
					var
						delta_temperature = (temperature_archived - 2.7) //hardcoded space temperature
					if((heat_capacity > 0) && (abs(delta_temperature) > MINIMUM_TEMPERATURE_DELTA_TO_CONSIDER))
