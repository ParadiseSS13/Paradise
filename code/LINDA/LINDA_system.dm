...
atom
	movable
		proc
			BlockSuperconductivity() // objects that block air and don't let superconductivity act. Only firelocks atm.
				return 0
...
turf
	proc
		GetAtmosAdjacentTurfs(alldir = 0)
			...
			for(var/direction in diagonals)
				...
				for(var/checkDirection in cardinal)
					...
					var
						turf
							simulated
								checkTurf = get_step(S, checkDirection)
...
atom
	movable
		proc
			atmos_spawn_air(var/text, var/amount) //because a lot of people loves to copy paste awful code lets just make a easy proc to spawn your plasma fires
				var
					turf
						simulated
							T = get_turf(src)
...
turf
	simulated
		proc
			atmos_spawn_air(var/flag, var/amount)
				if(!text || !amount || !air)
					return
