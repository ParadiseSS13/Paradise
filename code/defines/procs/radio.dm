...
proc
	get_frequency_name(var/display_freq)
		var
			freq_text
			...
			obj
				machinery
					message_server
						message_server = null
		var
			telecomms_reception = TELECOMMS_RECEPTION_NONE
		var
			message = ""
			...
			obj
				machinery
					message_server
						message_server = null
		var
			sender_reception = TELECOMMS_RECEPTION_NONE
		var
			list
				receiver_reception = new
...
proc
	check_signal(var/datum/signal/signal)
		return signal && signal.data["done"]

proc
	get_sender_reception(var/atom/sender, var/datum/signal/signal)
		return check_signal(signal) ? TELECOMMS_RECEPTION_SENDER : TELECOMMS_RECEPTION_NONE

proc
	get_receiver_reception(var/receiver, var/datum/signal/signal)
		if(receiver && check_signal(signal))
			var
				turf
					pos = get_turf(receiver)
			if(pos && (pos.z in signal.data["level"]))
				return TELECOMMS_RECEPTION_RECEIVER
		return TELECOMMS_RECEPTION_NONE

proc
	get_reception(var/atom/sender, var/receiver, var/message = "", var/do_sleep = 1)
		var
			datum
				reception
					reception = new
		...
		var
			datum
				signal
					signal = sender.telecomms_process(do_sleep)	// Be aware that this proc calls sleep, to simulate transmition delays
...
proc
	get_receptions(var/atom/sender, var/list/atom/receivers, var/do_sleep = 1)
		var
			datum
				receptions
					receptions = new
