...
Actual Adjacent procs :
	...
	turf
		proc
			reachableAdjacentTurfs : returns reachable turfs in cardinal directions (uses simulated_only)

	turf
		proc
			reachableAdjacentAtmosTurfs : returns turfs in cardinal directions reachable via atmos
...
PathNode
	var
		turf
			source //turf associated with the PathNode
	var
		PathNode
			prevNode //link to the parent PathNode
	var
		f		//A* Node weight (f = g + h)
	var
		g		//A* movement cost variable
	var
		h		//A* heuristic variable
	var
		nt		//count the number of Nodes traversed
...
proc
	HeapPathWeightCompare(PathNode/a, PathNode/b)
		return b.f - a.f

//wrapper that returns an empty list if A* failed to find a path
proc
	get_path_to(caller, end, dist, maxnodes, maxnodedepth = 30, mintargetdist, adjacent = /turf/proc/reachableAdjacentTurfs, id=null, turf/exclude=null, simulated_only = 1)
		var
			list
...
proc
	AStar(caller, end, dist, maxnodes, maxnodedepth = 30, mintargetdist, adjacent = /turf/proc/reachableAdjacentTurfs, id=null, turf/exclude=null, simulated_only = 1)
		...
		var
			Heap
				open = new /Heap(/proc/HeapPathWeightCompare) //the open list
		var
			list
				closed = new() //the closed list
		var
			list
				path = null //the returned path, if any
		var
			PathNode
				cur //current processed turf
		...
		while(!open.IsEmpty() && !path)
		{
			...
			var
				list
					L = call(cur.source,adjacent)(caller, id, simulated_only)
			for(var/turf/T in L)
				if(T == exclude || (T in closed))
					continue

				var
					newg = cur.g + call(cur.source,dist)(T)
...
turf
	proc
		LinkBlockedWithAccess(turf/T, caller, ID)
			var
				adir = get_dir(src, T)
			var
				rdir = get_dir(T, src)
