...
proc
	sql_report_death(mob/living/carbon/human/H)
		if(!config.sql_enabled)
			return
		if(!H)
			return
...
proc
	sql_report_cyborg_death(mob/living/silicon/robot/H)
		if(!config.sql_enabled)
			return
		if(!H)
			return
	...
	sql_commit_feedback()
		...
		var
			list
				datum
					feedback_variable
						content = blackbox.get_round_feedback()
		...
		if(!dbcon.IsConnected())
			...
			var
				DBQuery
					max_query = dbcon.NewQuery("SELECT MAX(roundid) AS max_round_id FROM [format_table_name("feedback")]")
			max_query.Execute()
			...
			for(var/datum/feedback_variable/item in content)
				...
				var
					DBQuery
						query = dbcon.NewQuery("INSERT INTO [format_table_name("feedback")] (id, roundid, time, variable, value) VALUES (null, [newroundid], Now(), '[variable]', '[value]')")
				if(!query.Execute())
					var
						err = query.ErrorMsg()
