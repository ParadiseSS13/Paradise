...
datum
	controller
		processScheduler
			proc
				addProcess(var/datum/controller/process/process)
...
datum
	controller
		processScheduler
			proc
				replaceProcess(var/datum/controller/process/oldProcess, var/datum/controller/process/newProcess)
...
datum
	controller
		processScheduler
			proc
				runProcess(var/datum/controller/process/process)
					spawn(0)
						process.process()
...
datum
	controller
		processScheduler
			proc
				processStarted(var/datum/controller/process/process)
...
datum
	controller
		processScheduler
			proc
				processFinished(var/datum/controller/process/process)
...
datum
	controller
		processScheduler
			proc
				setIdleProcessState(var/datum/controller/process/process)
...
datum
	controller
		processScheduler
			proc
				setQueuedProcessState(var/datum/controller/process/process)
					if(process in running)
						running -= process
					if(process in idle)
						idle -= process
...
datum
	controller
		processScheduler
			proc
				setRunningProcessState(var/datum/controller/process/process)
					if(process in queued)
						queued -= process
					if(process in idle)
						idle -= process
...
datum
	controller
		processScheduler
			proc
				recordStart(var/datum/controller/process/process, var/time = null)
					if(isnull(time))
						time = TimeOfGame
...
datum
	controller
		processScheduler
			proc
				recordEnd(var/datum/controller/process/process, var/time = null)
					if(isnull(time))
						time = TimeOfGame

					var
						lastRunTime = time - last_start[process]
...
datum
	controller
		processScheduler
			proc
				recordRunTime(var/datum/controller/process/process, time)
					...
					var
						list
							lastTwenty = last_twenty_run_times[process]
...
datum
	controller
		processScheduler
			proc
				averageRunTime(var/datum/controller/process/process)
					var
						lastTwenty = last_twenty_run_times[process]
...
datum
	controller
		processScheduler
			proc
				getProcessLastRunTime(var/datum/controller/process/process)
...
datum
	controller
		processScheduler
			proc
				getProcessHighestRunTime(var/datum/controller/process/process)
...
datum
	controller
		processScheduler
			proc
				hasProcess(var/processName as text)
...
datum
	controller
		processScheduler
			proc
				killProcess(var/processName as text)
...
datum
	controller
		processScheduler
			proc
				restartProcess(var/processName as text)
					if(hasProcess(processName))
						var
							datum
								controller
									process
										oldInstance = nameToProcessMap[processName]
						var
							datum
								controller
									process
										newInstance = new oldInstance.type(src)
...
datum
	controller
		processScheduler
			proc
				enableProcess(var/processName as text)
					if(hasProcess(processName))
						var
							datum
								controller
									process
										process = nameToProcessMap[processName]
						process.enable()
...
datum
	controller
		processScheduler
			proc
				disableProcess(var/processName as text)
					if(hasProcess(processName))
						var
							datum
								controller
		...
		processScheduler
			proc
				updateTimeAllowance()
					...
					var
						tmp
							error = cpuAverage - 100
					var
						tmp
							timeAllowanceDelta = SIMPLE_SIGN(error) * -0.5 * world.tick_lag * max(0, 0.001 * abs(error))
