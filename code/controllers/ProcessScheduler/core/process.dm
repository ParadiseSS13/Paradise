...
datum
	controller
		process
			...
			var
				tmp
					schedule_interval = PROCESS_DEFAULT_SCHEDULE_INTERVAL // run every 50 ticks
			...
			var
				tmp
					hang_warning_time = PROCESS_DEFAULT_HANG_WARNING_TIME
		...
		process
			proc
				handleHung()
					...
					var
						msg = "[name] process hung at tick #[ticks]. Process was unresponsive for [(TimeOfGame - run_start) / 10] seconds and was restarted. Last task: [last_task]. Last Object Type: [lastObjType]"
					log_debug(msg)
					message_admins(msg)
		...
		process
			proc
				kill()
					if(!killed)
						var
							msg = "[name] process was killed at tick #[ticks]."
						log_debug(msg)
						message_admins(msg)
						//finished()
...
datum
	controller
		process
			proc
				setLastTask(var/task, var/object)
					last_task = task
					last_object = object
...
datum
	controller
		process
			proc
				_copyStateFrom(var/datum/controller/process/target)
					main = target.main
					name = target.name
	...
	controller
		process
			proc
				statProcess()
					var
						averageRunTime = round(getAverageRunTime(), 0.1)/10
					var
						lastRunTime = round(getLastRunTime(), 0.1)/10
					var
						highestRunTime = round(getHighestRunTime(), 0.1)/10
...
datum
	controller
		process
			proc
				catchException(var/exception/e, var/thrower)
					...
					var
						etext = "[e]"
					var
						eid = "[e]" // Exception ID, for tracking repeated exceptions
					var
						ptext = "" // "processing..." text, for what was being processed (if known)
					if(istype(e))
