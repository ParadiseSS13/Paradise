...
var
	global
		datum
			controller
				game_controller
					master_controller //Set in world.New()
...
datum
	controller
		proc
			recover() // If we are replacing an existing controller (due to a crash) we attempt to preserve as much as we can.
...
datum
	controller
		game_controller
			var
				list
					shuttle_list	                    // For debugging and VV
