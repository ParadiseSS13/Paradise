...
var
	global
		list
			round_voters = list() //Keeps track of the individuals voting for a given round, for use in forcedrafting.
...
datum
	controller
		vote
			...
			proc
				get_result()
					...
					if(!config.vote_no_default && choices.len)
						var
							non_voters = (clients.len - total_votes)
						if(non_voters > 0)
							if(mode == "restart")
			...
			proc
				initiate_vote(var/vote_type, var/initiator_key)
					if(!mode)
						...
						switch(vote_type)
							...
							if("custom")
								...
								for(var/i=1,i<=10,i++)
									var
										option = capitalize(html_encode(input(usr,"Please enter an option or hit cancel to finish") as text|null))
			...
			proc
				interface(var/client/C)
					if(!C)	return
					var
						admin = check_rights(R_ADMIN,0)
					voting |= C
