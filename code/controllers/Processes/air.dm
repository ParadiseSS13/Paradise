...
datum
	controller
		process
			air_system
				proc
					remove_from_active(var/turf/simulated/T)
						if(istype(T))
							T.excited = 0
							active_turfs -= T
							if(T.excited_group)
...
datum
	controller
		process
			air_system
				proc
					add_to_active(var/turf/simulated/T, var/blockchanges = 1)
						if(istype(T) && T.air)
							T.excited = 1
							active_turfs |= T
...
datum
	controller
		process
			air_system
				proc
					setup_allturfs(var/turfs_in = world)
						for(var/turf/simulated/T in turfs_in)
							...
							if(!T.blocks_air)
								...
								for(var/direction in cardinal)
									...
									var
										turf
											enemy_tile = get_step(T, direction)
