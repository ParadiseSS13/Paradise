	...
	controller
		process
			npcpool
				...
				var
					list
						botPool_l = list() //list of all npcs using the pool
				var
					list
						botPool_l_non = list() //list of all non SNPC mobs using the pool
...
datum
	controller
		process
			npcpool
				copyStateFrom(var/datum/controller/process/npcpool/target)
				...
				doWork()
					...
					for(var/mob/living/carbon/human/interactive/check in botPool_l)
						if(!check)
							...
							continue
						var
							checkInRange = view(SNPC_MAX_RANGE_FIND, check)
