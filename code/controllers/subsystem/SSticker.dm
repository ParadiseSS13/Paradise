	...
	switch(current_state)
		...
		if(GAME_STATE_FINISHED)
			if(SSshuttle.emergency.mode >= SHUTTLE_ENDGAME && !mode.station_was_nuked)
				event_blackbox(outcome = CREW_TRANSFER)
			current_state = GAME_STATE_FINISHED
...
/datum/controller/subsystem/ticker/proc/event_blackbox(outcome = CREW_TRANSFER)
	for(var/I in SSevents.biohazards_this_round)
		switch(I)
			if(GREEN_SPIDER, PRINCE_SPIDER, WHITE_SPIDER, PRINCESS_SPIDER, QUEEN_SPIDER)
				...
				if(spiders >= 5 || (output == "Prince Terror" && spiders == 1)) //If a prince lives, record as win.
					switch(outcome)
						...
						if(CREW_TRANSFER)
							SSblackbox.record_feedback("tally", "Biohazard survives to nuclear round end", 1, output)
						if(FORCED_ROUND_END)
			...
			if("Xenomorphs")
				if(length(SSticker.mode.xenos) > 5)
					switch(outcome)
						...
						if(CREW_TRANSFER)
							SSblackbox.record_feedback("tally", "Biohazard survives to nuclear round end", 1, "Xenomorphs")
						if(FORCED_ROUND_END)
			...
			if("Blob")
				if(length(SSticker.mode.blob_overminds))
					switch(outcome)
						...
						if(CREW_TRANSFER)
							SSblackbox.record_feedback("tally", "Biohazard survives to nuclear round end", 1, "Blob")
						if(FORCED_ROUND_END)
