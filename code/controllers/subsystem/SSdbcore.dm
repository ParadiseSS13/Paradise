...
ADMIN_VERB(reestablish_db_connection, R_ADMIN, "Reestablish DB Connection", "Force a reconnection to the database.", VERB_CATEGORY_DEBUG)
	...
	if(SSdbcore.IsConnected())
		if(!check_rights_client(R_DEBUG, FALSE, user))
			to_chat(user, "<span class='warning'>The database is already connected! (Only those with +DEBUG can force a reconnection)</span>")
			return
