...
/datum/controller/subsystem/user_verbs/proc/process_pending_admins()
	...
	for(var/admin_ckey in pending_admins)
		associate_admin(GLOB.directory[admin_ckey])
...
/datum/controller/subsystem/user_verbs/proc/get_valid_verbs_for_admin(client/admin)
 ...
 */
/datum/controller/subsystem/user_verbs/proc/associate_admin(client/admin)
	if(IsAdminAdvancedProcCall())
		return

	if(!isnull(admins_pending_subsytem_init)) // if the list exists we are still initializing
		to_chat(admin, "<span class='big green'>Admin Verbs are still initializing. Please wait and you will be automatically assigned your verbs when it is complete.</span>")
		admins_pending_subsytem_init |= list(admin.ckey)
		return

	// refresh their verbs
	admin_visibility_flags[admin.ckey] ||= list()
	if(admin.holder.is_localhost_autoadmin)
		admin_visibility_flags[admin.ckey] |= list(VERB_VISIBILITY_FLAG_LOCALHOST)
	for(var/datum/user_verb/verb_singleton as anything in get_valid_verbs_for_admin(admin))
		verb_singleton.assign_to_client(admin)
	admin.init_verbs()

/**
 * Unassociates an admin from their admin verbs.
 ...
 */
/datum/controller/subsystem/user_verbs/proc/deassociate_admin(client/admin)
	if(IsAdminAdvancedProcCall())
		return

	UnregisterSignal(admin, COMSIG_MOB_CLIENT_LOGIN)
	for(var/verb_type in user_verbs_by_type)
		var/datum/user_verb/user_verb = user_verbs_by_type[verb_type]
		user_verb.unassign_from_client(admin)
