...
SUBSYSTEM_DEF(admin_verbs)
	...
	var/list/admin_verbs_by_type = list()
	/// A list of all admin verbs indexed by their visibility flag.
	var/list/admin_verbs_by_visibility_flag = list()
...
/datum/controller/subsystem/admin_verbs/proc/invoke_verb(client/admin, datum/admin_verb/verb_type, ...)
 ...
 */
/datum/controller/subsystem/admin_verbs/proc/deassociate_admin(client/admin)
	if(IsAdminAdvancedProcCall())
		return

	UnregisterSignal(admin, COMSIG_MOB_CLIENT_LOGIN)
	for(var/verb_type in admin_verbs_by_type)
		var/datum/admin_verb/admin_verb = admin_verbs_by_type[verb_type]
		admin_verb.unassign_from_client(admin)
