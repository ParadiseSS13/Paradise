...
/datum/controller/subsystem/redis/Initialize()
	...
	// Please format these as appname.function.in/out
	subscribe("aa07.debug.out")
	return ..()
		...
		switch(channel)
			if("aa07.debug.out")
				for(var/message in usable_data[channel])
					publish("aa07.debug.in", "Message received at BYOND at [time_stamp()] - '[message]'") // Send a message to the debug poker
