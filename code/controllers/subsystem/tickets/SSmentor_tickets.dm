...
/datum/controller/subsystem/tickets/mentor_tickets/Initialize()
	...
	close_messages = list("<font color='red' size='3'><b>- [ticket_name] Closed -</b></font>",
				...
				span_mentorhelp("Your [ticket_name] has now been closed."))
...
/datum/controller/subsystem/tickets/mentor_tickets/autoRespond(N)
	...
	to_chat_safe(returnClient(N), "[span_mentorhelp("[key_name_hidden(C)] is autoresponding with: ")][span_adminticketalt("[response_phrases[message_key]]")]") //for this we want the full value of whatever key this is to tell the player so we do response_phrases[message_key]
