...
/datum/controller/subsystem/tickets/proc/autoRespond(N)
	...
	switch(message_key)
		...
		if("Man Up")
			SSadmin_verbs.invoke_verb(C, /datum/admin_verb/man_up, returnClient(N))
...
/datum/controller/subsystem/tickets/Topic(href, href_list)
	...
	if(href_list["resolveall"])
		if(ticket_system_name == "Mentor Tickets")
			SSadmin_verbs.invoke_verb(usr, /datum/admin_verb/resolve_all_mentor_tickets)
		else
			SSadmin_verbs.invoke_verb(usr, /datum/admin_verb/resolve_all_admin_tickets)
