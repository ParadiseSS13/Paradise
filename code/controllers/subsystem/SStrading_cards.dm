...
/datum/controller/subsystem/trading_card_game/proc/styleKeywords()
	...
	for(var/keyword in keywords)
		...
		keywords[keyword] = "<span title='[tooltip_text]'>[keyword]</span>"
...
/datum/controller/subsystem/trading_card_game/proc/get_card(series, id)
	return cached_cards[series]?["ALL"][id]
...
/datum/card/proc/applyKeywords(list/changed_vars)
	for(var/varname in (changed_vars & vars))
		var/value = vars[varname]
		if(!istext(value))
			continue
		vars[varname] = SStrading_card_game.resolve_keywords(value)
