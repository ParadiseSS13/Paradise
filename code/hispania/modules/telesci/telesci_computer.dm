...
/obj/machinery/computer/telescience/proto/loaded
	crystals = 4//max is 6 //50 potency is enough 4cristlaes+1ranting = 5
		...
		spawn(round(proj_data.time) * 10) // in seconds
			...
			for(var/atom/movable/ROI in source)
				...
				if(!istype(ROI, /obj/structure/closet/bluespace))
					continue//if it's not a bluespace closet it won't teleport
				log_msg += "[ROI.name]"
				...
				. = TRUE//SO THE ERROR MESSAGE DOESN'T APPEAR
			if(dd_hassuffix(log_msg, ", "))
				log_msg = dd_limittext(log_msg, length(log_msg) - 2)
			else
				log_msg += "nothing"
...
/obj/machinery/teleci_reciver//basically an antenna, at the moment there will only be one, located in the z3
	name = "telescientific antenna"
	desc = "An antenna capable of perceiving bluespace interactions."
	icon = 'icons/obj/machines/research.dmi'//TODO:unique/only sprite (wasn't sure how to translate this part)
	icon_state = "tdoppler"
	density = TRUE
	anchored = TRUE
	use_power = NO_POWER_USE//I don't want to get too involved with this and I would like to be able to put it anywhere, in all aspects this is a structure.
	var/area/area
...
/obj/machinery/teleci_reciver/proc/revice_data(atom/target, obj/machinery/computer/telescience/source)
	...
	if(source.max_bluespace_tech < new_dist)
		return
	source.atom_say("A bluespace resonance triggered by [src] causes a forced recalibration")
	source.recalibrate()//this is an all or nothing, you test elsewhere and then point to the antenna
	...
	if(tmp_tech > source.bluespace_tech.level)
		...
		if(source.bluespace_tech.level == source.max_bluespace_tech && source.bluespace_tech.level < initial(source.max_bluespace_tech)+3)
			source.max_bluespace_tech++//for each time you reach the maximum it increases the maximum tech a little up to a certain limit
