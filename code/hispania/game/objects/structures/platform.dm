...
/obj/structure/platform/proc/CheckLayer() // To know if the icon needs to be above the mob or not
	if(dir == SOUTH)
...
/obj/structure/platform/CheckExit(atom/movable/O, turf/target)
	if(!anchored)
		CheckLayer()
	if(istype(O, /obj/structure/platform)) //To avoid having two platforms on the same tile
		return FALSE
...
/obj/structure/platform/CanPass(atom/movable/mover, turf/target)
	if(!anchored)
		CheckLayer()
	if(istype(mover, /obj/structure/platform)) // To avoid having two platforms on the same tile
		return FALSE
