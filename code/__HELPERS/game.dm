...
proc
	inLineOfSight(X1,Y1,X2,Y2,Z=1,PX1=16.5,PY1=16.5,PX2=16.5,PY2=16.5)
		...
		if(X1==X2)
			...
			var
				signX = SIMPLE_SIGN(X2-X1)
			var
				signY = SIMPLE_SIGN(Y2-Y1)
			if(X1<X2)
				b+=m
			while(X1!=X2 || Y1!=Y2)
				if(round(m*X1+b-Y1))
...
proc
	get_candidates(be_special_type, afk_bracket=3000, override_age=0, override_jobban=0)
		var
			roletext = get_roletext(be_special_type)
		var
			list
				candidates = list()
...
proc
	pollCandidates(var/Question, var/be_special_type, var/antag_age_check = 0, var/poll_time = 300)
		var
			roletext = be_special_type ? get_roletext(be_special_type) : null
		var
			list
				mob
					dead
