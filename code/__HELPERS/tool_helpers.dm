...
/proc/ispowertool(O)//used to check if a tool can force powered doors
	if(istype(O, /obj/item/tool/crowbar/power) || istype(O, /obj/item/mecha_parts/mecha_equipment/medical/rescue_jaw))
		return TRUE
	return FALSE
...
/proc/is_surgery_tool_by_behavior(obj/item/tool/W)
	if(!istype(W))
		return FALSE
