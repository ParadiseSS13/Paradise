...
proc
	animate_fade_grayscale(var/atom/A, var/time = 5)
...
proc
	animate_explode_pixel(var/atom/A)
		if(!istype(A))
			return
		var
			floatdegrees = rand(5, 20)
		var
			side = pick(-1, 1)
...
proc
	animate_float(var/atom/A, var/loopnum = -1, floatspeed = 20, random_side = 1)
		if(!istype(A))
			return
		var
			floatdegrees = rand(5, 20)
...
proc
	animate_levitate(var/atom/A, var/loopnum = -1, floatspeed = 20, random_side = 1)
		if(!istype(A))
			return
		var
			floatdegrees = rand(5, 20)
...
proc
	animate_ghostly_presence(var/atom/A, var/loopnum = -1, floatspeed = 20, random_side = 1)
		if(!istype(A))
			return
		var
			floatdegrees = rand(5, 20)
...
proc
	animate_fading_leap_up(var/atom/A)
		if(!istype(A))
			return
		var
			matrix
...
proc
	animate_fading_leap_down(var/atom/A)
		if(!istype(A))
			return
		var
			matrix
...
proc
	animate_shake(var/atom/A, var/amount = 5, var/x_severity = 2, var/y_severity = 2)
		...
		var
			x_severity_inverse = 0 - x_severity
		var
			y_severity_inverse = 0 - y_severity
...
proc
	animate_teleport(var/atom/A)
		if(!istype(A))
			return
		var
			matrix
				M = matrix(1, 3, MATRIX_SCALE)
...
proc
	animate_teleport_wiz(var/atom/A)
		if(!istype(A))
			return
		var
			matrix
				M = matrix(0, 4, MATRIX_SCALE)
...
proc
	animate_rainbow_glow_old(var/atom/A)
		if(!istype(A))
			return
...
proc
	animate_rainbow_glow(var/atom/A)
		if(!istype(A))
			return
		animate(A, color = "#FF0000", time = rand(5,10), loop = -1, easing = LINEAR_EASING)
...
proc
	animate_fade_to_color_fill(var/atom/A, var/the_color, var/time)
		if(!istype(A) || !the_color || !time)
			return
		animate(A, color = the_color, time = time, easing = LINEAR_EASING)

proc
	animate_flash_color_fill(var/atom/A, var/the_color, var/loops, var/time)
...
proc
	animate_flash_color_fill_inherit(var/atom/A, var/the_color, var/loops, var/time)
		if(!istype(A) || !the_color || !time || !loops)
			return
		var
			color_old = A.color
...
proc
	animate_wiggle_then_reset(var/atom/A, var/loops = 5, var/speed = 5, var/x_var = 3, var/y_var = 3)
...
proc
	animate_spin(var/atom/A, var/dir = "L", var/T = 1, var/looping = -1)
		if(!istype(A))
			return
...
proc
	animate_shockwave(var/atom/A)
		if(!istype(A))
			return
		var
			punchstr = rand(10, 20)
		var
			original_y = A.pixel_y
