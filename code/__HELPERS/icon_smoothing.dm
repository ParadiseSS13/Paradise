...
atom
	var
		list
			canSmoothWith = null // TYPE PATHS I CAN SMOOTH WITH~~~~~ If this is null and atom is smooth, it smooths only with itself
...
proc
	find_type_in_direction(atom/source, direction, range=1)
		var
			x_offset = 0
		var
			y_offset = 0
		...
		var
			turf
				target_turf = locate(source.x + x_offset, source.y + y_offset, source.z)
		if(!target_turf)
			return null
		if(source.canSmoothWith)
			...
			var
				atom
					A = locate(source.type) in target_turf
			return A && A.type == source.type ? A : null

//Icon smoothing helpers

proc
	smooth_icon_neighbors(atom/A)
...
proc
	smooth_zlevel(var/zlevel)
		var
			list
				away_turfs = block(locate(1, 1, zlevel), locate(world.maxx, world.maxy, zlevel))
