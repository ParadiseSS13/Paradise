...
proc
	format_table_name(table as text)
...
proc
	strip_html_simple(var/t,var/limit=MAX_MESSAGE_LEN)
		var
			list
				strip_chars = list("<",">")
...
proc
	strip_html(var/t,var/limit=MAX_MESSAGE_LEN)
		return copytext((sanitize(strip_html_simple(t))),1,limit)

//Runs byond's sanitization proc along-side strip_html_simple
//I believe strip_html_simple() is required to run first to prevent '<' from displaying as '&lt;' that html_encode() would cause
proc
	adminscrub(var/t,var/limit=MAX_MESSAGE_LEN)
...
proc
	stripped_input(var/mob/user, var/message = "", var/title = "", var/default = "", var/max_length=MAX_MESSAGE_LEN)
		var
			name = input(user, message, title, default)
		return strip_html_properly(name, max_length)

//Filters out undesirable characters from names
proc
	reject_bad_name(var/t_in, var/allow_numbers=0, var/max_length=MAX_NAME_LEN)
...
// See bygex.dm
proc
	replace_characters(var/t,var/list/repl_chars)
		for(var/char in repl_chars)
			t = replacetext(t, char, repl_chars[char])
		return t
...
//This is used for fingerprints
	...
	for(var/i = 1, i < lentext(text), i++)
		var
			a = copytext(text,i,i+1)
		var
			b = copytext(compare,i,i+1)
...
proc
	strip_html_properly(var/input, var/max_length = MAX_MESSAGE_LEN, allow_lines = 0)
		if(!input)
			return
		var
			opentag = 1 //These store the position of < and > respectively.
		var
			closetag = 1
		while(1)
...
proc
	trim_strip_html_properly(var/input, var/max_length = MAX_MESSAGE_LEN, allow_lines = 0)
					return trim(strip_html_properly(input, max_length, allow_lines))

//Used in preferences' SetFlavorText and human's set_flavor verb
//Previews a string of len or less length
proc
	TextPreview(var/string,var/len=40)
...
proc
	copytext_preserve_html(var/text, var/first, var/last)
...
proc
	sanitizeSafe(var/input, var/max_length = MAX_MESSAGE_LEN, var/encode = 1, var/trim = 1, var/extra = 1)
		return sanitize(replace_characters(input, list(">"=" ","<"=" ", "\""="'")), max_length, encode, trim, extra)


//Replaces \red \blue \green \b etc with span classes for to_chat
proc
	replace_text_macro(match, code, rest)
					var
						regex
							text_macro = new("(\\xFF.)(.*)$")
					switch(code)
