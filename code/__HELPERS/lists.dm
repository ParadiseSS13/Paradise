...
proc
	sortList(var/list/L)
		if(L.len < 2)
			return L
		var
			middle = L.len / 2 + 1 // Copy is first,second-1
...
proc
	mergeKeyedLists(var/list/L, var/list/R, var/key)
		var
			Li=1
		var
			Ri=1
...
proc
	insertion_sort_numeric_list_descending(var/list/L)
		...
		var
			list
				out = insertion_sort_numeric_list_ascending(L)
		//log_to_dd("	output: [out.len]")
		return reverselist(out)

proc
	dd_sortedObjectList(var/list/L, var/cache=list())
		if(L.len < 2)
			return L
		var
			middle = L.len / 2 + 1 // Copy is first,second-1
...
proc
	dd_mergeObjectList(var/list/L, var/list/R, var/list/cache)
		var
			Li=1
		var
			Ri=1
...
proc
	dd_insertObjectList(var/list/L, var/O)
		var
			min = 1
		var
			max = L.len
...
proc
	dd_sortedtextlist(list/incoming, case_sensitive = 0)
...
proc
	subtypesof(var/path) //Returns a list containing all subtypes of the given path, but not the given path itself.
		if(!path || !ispath(path))
