...
/obj/proc/atmosanalyzer_scan(datum/gas_mixture/air_contents, mob/user, obj/target = src)
	var/obj/icon = target
	user.visible_message("[user] has used the analyzer on [target].", span_notice("You use the analyzer on [target]."))
	var/pressure = air_contents.return_pressure()
	var/total_moles = air_contents.total_moles()

	user.show_message(span_notice("Results of analysis of [bicon(icon)] [target]."), 1)
	if(total_moles>0)
		...
		user.show_message(span_notice("Pressure: [round(pressure,0.1)] kPa"), 1)
		user.show_message(span_notice("Nitrogen: [round(n2_concentration*100)] % ([round(air_contents.nitrogen,0.01)] moles)"), 1)
		user.show_message(span_notice("Oxygen: [round(o2_concentration*100)] % ([round(air_contents.oxygen,0.01)] moles)"), 1)
		...
		user.show_message(span_notice("[target] is empty!"), 1)
	return
