...
/*
	...
	*/
	...
	/* Get the direction of startObj relative to endObj.
		...
		*/
proc
	getRelativeDirection(var/atom/movable/startObj, var/atom/movable/endObj)
	...
	invertHTML(HTMLstring)
		...
		if(!( istext(HTMLstring) ))
			...
			if(length(HTMLstring) != 7)
				...
				return
		var
			textr = copytext(HTMLstring, 2, 4)
		var
			textg = copytext(HTMLstring, 4, 6)
		var
			textb = copytext(HTMLstring, 6, 8)
		var
			r = hex2num(textr)
		var
			g = hex2num(textg)
...
proc
	Get_Angle(atom/movable/start,atom/movable/end)//For beams.
...
*/
	...
	var
		dirx = 0//Generic location finding variable.
	var
		diry = 0

	var
		xoffset = 0//Generic counter for offset location.
	var
		yoffset = 0

	var
		b1xerror = 0//Generic placing for point A in box. The lower left.
	var
		b1yerror = 0
	var
		b2xerror = 0//Generic placing for point B in box. The upper right.
	var
		b2yerror = 0
	...
	var
		turf
			destination=locate(location.x+dirx,location.y+diry,location.z)

	if(destination)//If there is a destination.
		if(errorx||errory)//If errorx or y were specified.
			var
				destination_list[] = list()//To add turfs to list.
			...
			var
				turf
					center = locate((destination.x+xoffset),(destination.y+yoffset),location.z)//So now, find the new center.
...
proc
	getline(atom/M,atom/N)//Ultra-Fast Bresenham Line-Drawing Algorithm
		...
		var
			py=M.y
		var
			line[] = list(locate(px,py,M.z))
		var
			dx=N.x-px	//x distance
		var
			dy=N.y-py
		var
			dxabs=abs(dx)//Absolute value of x distance
		var
			dyabs=abs(dy)
		var
			sdx=SIGN(dx)	//Sign of x distance (+ or -)
		var
			sdy=SIGN(dy)
		var
			x=dxabs>>1	//Counters for steps taken, setting to distance/2
		var
			y=dyabs>>1	//Bit-shifting makes me l33t.  It also makes getline() unnessecarrily fast.
		var
			j			//Generic integer for counting
...
proc
	sanitize_frequency(var/f, var/low = PUBLIC_LOW_FREQ, var/high = PUBLIC_HIGH_FREQ)
		f = round(f)
		f = max(low, f)
		f = min(high, f)
...
obj
	proc
		atmosanalyzer_scan(var/datum/gas_mixture/air_contents, mob/user, var/obj/target = src)
			var
				obj
					icon = target
			...
			if(total_moles>0)
				var
					o2_concentration = air_contents.oxygen/total_moles
				var
					n2_concentration = air_contents.nitrogen/total_moles
				var
					co2_concentration = air_contents.carbon_dioxide/total_moles
				var
					plasma_concentration = air_contents.toxins/total_moles
...
proc
	get_offset_target_turf(var/atom/A, var/dx, var/dy)
		var
			x = min(world.maxx, max(1, A.x + dx))
		var
			y = min(world.maxy, max(1, A.y + dy))
		return locate(x,y,A.z)
...
proc
	can_see(var/atom/source, var/atom/target, var/length=5) // I couldnt be arsed to do actual raycasting :I This is horribly inaccurate.
		var
			turf
...
proc
	get_step_towards2(var/atom/ref , var/atom/trg)
		var
			base_dir = get_dir(ref, get_step_towards(ref,trg))
		var
			turf
				temp = get_step_towards(ref,trg)

		if(is_blocked_turf(temp))
			var
				dir_alt1 = turn(base_dir, 90)
			var
				dir_alt2 = turn(base_dir, -90)
			var
				turf
					turf_last1 = temp
			var
...
area
	proc
		move_contents_to(var/area/A, var/turftoleave=null, var/direction = null)
			...
			var
				list
					turfs_trg = get_area_turfs(A.type)
					...
					for(var/turf/B in refined_trg)
						...
						if(C_src.x_pos == C_trg.x_pos && C_src.y_pos == C_trg.y_pos)
							...
							if(direction && findtext(X.icon_state, "swall_s"))
								...
								var
									turf
										nextturf = get_step(corner, direction)
...
proc
	DuplicateObject(obj/original, var/perfectcopy = 0 , var/sameloc = 0, var/atom/newloc = null)
		if(!original)
			return null
		...
		if(perfectcopy)
			if((O) && (original))
				var
					static
						list
							forbidden_vars = list("type","loc","locs","vars", "parent","parent_type", "verbs","ckey","key","power_supply","contents","reagents","stat","x","y","z","group")
...
area
	proc
		copy_contents_to(var/area/A , var/platingRequired = 0 )
...
proc
	can_puncture(obj/item/W as obj)		// For the record, WHAT THE HELL IS THIS METHOD OF DOING IT?
...
proc
	get_location_accessible(mob/M, location)
		var
			covered_locations	= 0	//based on body_parts_covered
		var
			face_covered		= 0	//based on flags_inv
		var
			eyesmouth_covered	= 0	//based on flags
		if(iscarbon(M))
			var
				mob
					living
...
atom
	proc
		GetTypeInAllContents(typepath)
			var
				list
					processing_list = list(src)
			var
				list
					processed = list()
...
proc
	get_random_colour(var/simple, var/lower, var/upper)
		var
			colour
		if(simple)
			...
			for(var/i=1;i<=3;i++)
				var
					temp_col = "[num2hex(rand(lower,upper))]"
...
proc
	get_random_chemical(var/is_plant = 0)
		var
			list
				blocked = blocked_chems.Copy()		//blocked_chems list is found in code/_globalvars/lists/reagents.dm
		if(is_plant)
			blocked.Add(plant_blocked_chems.Copy())	//plant_blocked_chems list is found in code/_globalvars/lists/reagents.dm
		var
			picked_chem = pick(chemical_reagents_list)
		if(blocked.Find(picked_chem))
			return get_random_chemical(is_plant)
		return picked_chem

proc
	get_distant_turf(var/turf/T,var/direction,var/distance)
...
proc
	dview(var/range = world.view, var/center, var/invis_flags = 0)
		if(!center)
			return
...
mob
	dview
		New() //For whatever reason, if this isn't called, then BYOND will throw a type mismatch runtime when attempting to add this to the mobs list. -Fox
...
atom
	movable
		proc
			orbit(atom/A, radius = 10, clockwise = FALSE, rotation_speed = 20, rotation_segments = 36, pre_rotation = TRUE, lockinorbit = FALSE)
				if(!istype(A))
					return
...
proc
	center_image(var/image/I, x_dimension = 0, y_dimension = 0)
		if(!I)
			return
		...
		var
			x_offset = -((x_dimension/world.icon_size)-1)*(world.icon_size*0.5)
		var
			y_offset = -((y_dimension/world.icon_size)-1)*(world.icon_size*0.5)
...
proc
	spiral_range_turfs(dist=0, center=usr, orange=0)
		if(!dist)
			if(!orange)
				return list(center)
			else
...
proc
	urange(dist=0, atom/center=usr, orange=0, areas=0)
		if(!dist)
			if(!orange)
				return list(center)
			else
...
proc
	screen_loc2turf(scr_loc, turf/origin)
		var
			tX = splittext(scr_loc, ",")
		var
			tY = splittext(tX[2], ":")
		var
			tZ = origin.z
...
proc
	get_closest_atom(type, list, source)
		var
			closest_atom
		var
			closest_distance
...
proc
	pick_closest_path(value)
		var
			list
				matches = get_fancy_list_of_types()
	...
	list
		TYPES_SHORTCUTS = list(
			...
			obj
				item
					weapon
						reagent_containers
							food
								drinks = "DRINK", //longest paths comes first
			obj
				item
					weapon
						reagent_containers
			...
			obj
				item
					weapon
						reagent_containers = "REAGENT_CONTAINERS",
			obj
				machinery
					atmospherics = "ATMOS",
			obj
				machinery
					portable_atmospherics = "PORT_ATMOS",
//	/obj/item/mecha_parts/mecha_equipment/weapon/ballistic/launcher/missile_rack = "MECHA_MISSILE_RACK",
	obj
		item
			mecha_parts
				mecha_equipment = "MECHA_EQUIP",
...
var
	global
		list
			g_fancy_list_of_types = null
proc
	get_fancy_list_of_types()
		if(isnull(g_fancy_list_of_types)) //init
			var
				list
					temp = sortList(subtypesof(/atom) - typesof(/area) - /atom/movable)
