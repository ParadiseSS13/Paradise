proc
	random_underwear(gender, species = "Human")
		var
			list
				pick_list = list()
		switch(gender)
...
proc
	random_undershirt(gender, species = "Human")
		var
			list
				pick_list = list()
		switch(gender)
...
proc
	random_socks(gender, species = "Human")
		var
			list
				pick_list = list()
		switch(gender)
...
proc
	pick_species_allowed_underwear(list/all_picks, species)
		var
			list
				valid_picks = list()
...
proc
	random_hair_style(var/gender, species = "Human")
		var
			h_style = "Bald"
...
proc
	random_facial_hair_style(var/gender, species = "Human")
		var
			f_style = "Shaved"
...
proc
	random_name(gender, species = "Human")
...
proc
	random_skin_tone(species = "Human")
...
proc
	skintone2racedescription(tone, species = "Human")
		if(species == "Human")
			switch(tone)
...
proc
	add_logs(mob/target, mob/user, what_done, var/object=null, var/addition=null, var/admin=1) //Victim : Attacker : what they did : what they did it with : extra notes
		var
			list
				ignore=list("shaked","CPRed","grabbed","punched")
		if(!user)
			return
		if(ismob(user))
...
proc
	do_mob(var/mob/user, var/mob/target, var/time = 30, var/uninterruptible = 0, progress = 1)
		if(!user || !target)
			return 0
		var
			user_loc = user.loc
...
proc
	do_after(mob/user, delay, needhand = 1, atom/target = null, progress = 1)
		if(!user)
			return 0
		var
			atom
		...
		var
			holdingnull = 1 //User's hand started out empty, check for an empty hand
		if(holding)
