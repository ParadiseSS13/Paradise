...
// Same thing as do_after but the target is only used for progress bar rendering instead of checking it's loc
/proc/do_after_visual(mob/user, delay, needhand = 1, atom/target = null)
	if(!user)
		return 0
	...
	var/datum/progressbar/progbar = new(user, delay, target)
