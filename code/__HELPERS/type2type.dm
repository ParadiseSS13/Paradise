...
proc
	rgb2hsl(red, green, blue)
		...
		var
			hue=0;var/saturation=0;var/lightness=0;
		lightness = (max + min)/2
		if(range != 0)
			...
			var
				dred = ((max-red)/(6*max)) + 0.5
			var
				dgreen = ((max-green)/(6*max)) + 0.5
			var
				dblue = ((max-blue)/(6*max)) + 0.5
...
proc
	num2septext(var/theNum, var/sigFig = 7,var/sep=",") // default sigFig (1,000,000)
		var
			finalNum = num2text(theNum, sigFig)

		// Start from the end, or from the decimal point
		var
			end = findtextEx(finalNum, ".") || length(finalNum) + 1
...
proc
	text2matrix(var/matrixtext)
		var
			list
				matrixtext_list = splittext(matrixtext, " ")
		var
			list
				matrix_list = list()
