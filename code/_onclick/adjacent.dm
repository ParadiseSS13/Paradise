...
atom
	proc
		Adjacent(var/atom/neighbor) // basic inheritance, unused
			return 0
...
*/
turf
	Adjacent(var/atom/neighbor, var/atom/target = null)
		var
			turf
				T0 = get_turf(neighbor)
		if(T0 == src)
			return 1
		...
		// Not orthagonal
		var
			in_dir = get_dir(neighbor,src) // eg. northwest (1+8)
		var
			d1 = in_dir&(in_dir-1)		// eg west		(1+8)&(8) = 8
		var
			d2 = in_dir - d1			// eg north		(1+8) - 8 = 1
...
obj
	item
		Adjacent(var/atom/neighbor, var/recurse = 1)
			if(neighbor == loc) return 1
...
turf
	proc
		ClickCross(var/target_dir, var/border_only, var/target_atom = null)
