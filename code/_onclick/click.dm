...
mob
	proc
		RangedAttack(var/atom/A, var/params)
			if(!mutations.len)
				return
			...
			var
				obj
					item
						projectile
							beam
								LE = new /obj/item/projectile/beam(loc)
		...
		click_catcher
			Click(location, control, params)
				...
				if(modifiers["middle"] && istype(usr, /mob/living/carbon))
					...
					var
						turf
							T = screen_loc2turf(modifiers["screen-loc"], get_turf(usr))
