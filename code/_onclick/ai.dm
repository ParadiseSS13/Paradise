...
atom
	proc
		AICtrlShiftClick(var/mob/user)  // Examines
			if(user.client)
				user.examinate(src)
			return
...
obj
	machinery
		door
			airlock
				AIAltShiftClick()  // Sets/Unsets Emergency Access Override
					if(density)
...
obj
	machinery
		door
			airlock
				AIShiftClick()  // Opens and closes doors!
					if(density)
...
atom
	proc
		AICtrlClick(var/mob/living/silicon/ai/user)
			if(user.holo)
				var
					obj
						machinery
...
obj
	machinery
		turretid
			AIAltClick() //toggles lethal on turrets
...
obj
	machinery
		door
			airlock
				AIMiddleClick() // Toggles door bolt lights.
					if(!src.lights)
