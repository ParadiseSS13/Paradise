...
obj
	item
		tk_grab
					...
					var
						obj
							item
								I = focus
					var
						resolved = target.attackby(I, user, params)
			...
			proc
				focus_object(var/obj/target, var/mob/living/user)
				...
				apply_focus_overlay()
					...
					var
						obj
							effect
								overlay
									O = new /obj/effect/overlay(locate(focus.x,focus.y,focus.z))
					O.name = "sparkles"
					O.anchored = 1
					O.density = 0
					O.layer = FLY_LAYER
...
/*Not quite done likely needs to use something thats not get_step_to
	proc
		check_path()
			var
				turf
					ref = get_turf(src.loc)
			var
				turf
					target = get_turf(focus.loc)
			if(!ref || !target)	return 0
			var
				distance = get_dist(ref, target)
