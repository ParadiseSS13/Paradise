...
obj
	screen
		...
		var
			obj
				master = null	//A reference to the object in the slot. Grabs or items, generally.
		var
			datum
				hud
					hud = null
	...
	screen
		act_intent
			Click(location, control, params)
				if(ishuman(usr))
					var
						_x = text2num(params2list(params)["icon-x"])
					var
						_y = text2num(params2list(params)["icon-y"])
							...
							if(no_mask)
								...
								var
									list
										tankcheck = null
								var
									breathes = "oxygen"    //default, we'll check later
								var
									list
										contents = list()
								var
	...
	screen
		zone_sel
			Click(location, control,params)
				var
					list
						PL = params2list(params)
				var
					icon_x = text2num(PL["icon-x"])
				var
					icon_y = text2num(PL["icon-y"])
				var
					old_selecting = selecting //We're only going to update_icon() if there's been a change
...
obj
	screen
		inventory
			var
				slot_id	//The indentifier for the slot. It has nothing to do with ID cards.
			layer = 19
		...
		inventory
			hand
				update_icon()
					...
					if(!handcuff_overlay)
						var
							state = (slot_id == slot_r_hand) ? "markus" : "gabrielle"
