...
datum
	action
		proc
			CheckRemoval(mob/living/user) // 1 if action is no longer valid for this mob and should be removed
				return 0
...
datum
	action
		proc
			Checks()// returns 1 if all checks pass
				if(!owner)
					return 0
	...
	screen
		movable
			action_button
				hide_toggle
					proc
						InitialiseIcon(var/mob/living/user)
							if(isalien(user))
								icon_state = "bg_alien"
							else
								icon_state = "bg_default"
			...
			action_button
				hide_toggle
					UpdateIcon()
						...
						var
							image
								img = image(icon,src,hidden?"show":"hide")
						overlays += img
						return
...
datum
	hud
		proc
			ButtonNumberToScreenCoords(var/number) // TODO : Make this zero-indexed for readabilty
				var
					row = round((number-1)/AB_MAX_COLUMNS)
				var
					col = ((number - 1)%(AB_MAX_COLUMNS)) + 1
				var
					coord_col = "+[col-1]"
				var
					coord_col_offset = 4+2*col
				var
					coord_row = "[-1 - row]"
				var
					coord_row_offset = 26
...
datum
	hud
		proc
			SetButtonCoords(var/obj/screen/button,var/number)
				var
					row = round((number-1)/AB_MAX_COLUMNS)
				var
					col = ((number - 1)%(AB_MAX_COLUMNS)) + 1
				var
					x_offset = 32*(col-1) + 4 + 2*col
				var
					y_offset = -32*(row+1) + 26
...
datum
	action
		item_action
			organ_action
				CheckRemoval(mob/living/carbon/user)
					if(!iscarbon(user))
						return 1
...
datum
	action
		scan_mode
			...
			var
				devices = 0 //How may enabled scanners the mob has
