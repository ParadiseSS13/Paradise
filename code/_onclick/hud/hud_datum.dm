...
/datum/hud/proc/show_hud(version = 0, mob/viewmob)
	...
	switch(display_hud_version)
		...
		if(HUD_STYLE_REDUCED)	//Reduced HUD
			...
			if(inv_slots[ITEM_SLOT_LEFT_HAND])
				screenmob.client.screen += inv_slots[ITEM_SLOT_LEFT_HAND]	//we want the hands to be visible
			if(inv_slots[ITEM_SLOT_RIGHT_HAND])
				screenmob.client.screen += inv_slots[ITEM_SLOT_RIGHT_HAND]	//we want the hands to be visible
			if(action_intent)
