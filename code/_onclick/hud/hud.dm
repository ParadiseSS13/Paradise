...
datum
	hud
		...
		var
			hud_version = 1			//Current displayed version of the HUD
		var
			inventory_shown = 1		//the inventory
		var
			show_intent_icons = 0
		var
			hotkey_ui_hidden = 0	//This is to hide the buttons that can be used via hotkeys. (hotkeybuttons list of buttons)
		...
		var
			list
				static_inventory = list()		//the screen objects which are static
		var
			list
				toggleable_inventory = list()	//the screen objects which can be hidden
		var
			list
				hotkeybuttons = list()			//the buttons that can be used via hotkeys
		var
			list
				infodisplay = list()			//the screen objects that display mob info (health, alien plasma, etc...)
		var
			list
				inv_slots[slots_amt]			// /obj/screen/inventory objects, ordered by their slot ID.
