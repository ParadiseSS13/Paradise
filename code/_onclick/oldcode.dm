atom
	DblClick(location, control, params) //TODO: DEFERRED: REWRITE
		if(!usr)	return
...
//	to_chat(world, "according to dblclick(), t5 is [t5]")
	...
	if(((t5 || (W && (W.flags & USEDELAY))) && !( istype(src, /obj/screen) )))
		...
		if((src.loc && (get_dist(src, usr) < 2 || src.loc == usr.loc)))
			...
			var
				direct = get_dir(usr, src)
			var
				obj
					item
						weapon
							dummy
								D = new /obj/item/weapon/dummy( usr.loc )
			var
				ok = 0
			if( (direct - 1) & direct)
		...
		if(!( usr.restrained() || (usr.lying && usr.buckled!=src) ))
			...
			if(W)
				...
				var
					ignoreAA = 0 //Ignore afterattack(). Surgery uses this.
				if(t5)
				...
				var
					obj
						item
							projectile
								beam
									A = new /obj/item/projectile/beam( usr.loc )
