	...
	if(!default_macro_sets) //If you ever remove legacy input mode, you can simplify this a lot
		default_macro_sets = list(
			"default" = list(
				...
				"Tab" = "\".winset \\\"mainwindow.macro=legacy input.focus=true\\\"\"", // Swaps us to legacy mode, forces input to the input bar, sets the input bar colour to salmon pink
				"Back" = "\".winset \\\"input.focus=true ? input.text=\\\"\"" // This makes it so backspace can remove default inputs
			),
			"legacy" = list(
				"Tab" = "\".winset \\\"mainwindow.macro=default map.focus=true\\\"\"", // Swaps us to rebind mode, moves input away from input bar, sets input bar to white
				"Back" = "\".winset \\\"input.focus=true ? input.text=\\\"\"" // This makes it so backspace can remove default inputs
			),
		)
