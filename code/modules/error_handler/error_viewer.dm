...
datum
	ErrorViewer
		proc
			browseTo(var/user, var/html)
				if(user)
					var
						datum
							browser
								popup = new(user, "error_viewer", "Runtime Viewer", 700, 500)
...
datum
	ErrorViewer
		proc
			buildHeader(var/datum/ErrorViewer/back_to, var/linear, var/refreshable)
...
datum
	ErrorViewer
		proc
			showTo(var/user, var/datum/ErrorViewer/back_to, var/linear)
...
datum
	ErrorViewer
		proc
			makeLink(var/linktext, var/datum/ErrorViewer/back_to, var/linear)
				var
					back_to_param = ""
				if(!linktext)
					linktext = name
...
datum
	ErrorViewer
		ErrorCache
			showTo(var/user, var/datum/ErrorViewer/back_to, var/linear)
				var
					html = buildHeader(null, linear, refreshable=1)
...
datum
	ErrorViewer
		ErrorCache
			proc
				logError(var/exception/e, var/list/desclines, var/skipCount, var/datum/e_src)
					if(!istype(e))
					...
					var
						datum
							ErrorViewer
								ErrorEntry
									error_entry = new(e, desclines, skipCount, e_src)
					...
					if(config && error_source.next_message_at <= world.time)
						var
							const
								viewtext = "\[view]" // Nesting these in other brackets went poorly
		...
		ErrorSource
			showTo(var/user, var/datum/ErrorViewer/back_to, var/linear)
				if(!istype(back_to))
					back_to = error_cache
				var
					html = buildHeader(back_to, refreshable=1)
				for(var/datum/ErrorViewer/ErrorEntry/error_entry in errors)
...
datum
	ErrorViewer
		ErrorEntry
			New(var/exception/e, var/list/desclines, var/skipCount, var/datum/e_src)
				if(!istype(e))
	...
	ErrorViewer
		ErrorEntry
			showTo(var/user, var/datum/ErrorViewer/back_to, var/linear)
				...
				var
					html = buildHeader(back_to, linear)
