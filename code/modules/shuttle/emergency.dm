...
obj
	machinery
		computer
			emergency_shuttle
				attackby(obj/item/weapon/card/W, mob/user, params)
					...
					if(istype(W, /obj/item/weapon/card/id)||istype(W, /obj/item/device/pda))
						...
						var
							choice = alert(user, text("Would you like to (un)authorize a shortened launch time? [] authorization\s are still needed. Use abort to cancel all authorizations.", src.auth_need - src.authorized.len), "Shuttle Launch", "Authorize", "Repeal", "Abort")
						if(shuttle_master.emergency.mode != SHUTTLE_DOCKED || user.get_active_hand() != W)
							return 0

						var
							seconds = shuttle_master.emergency.timeLeft()
						if(seconds <= 10)
							return 0
			...
			emergency_shuttle
				emag_act(mob/user)
					if(!emagged && shuttle_master.emergency.mode == SHUTTLE_DOCKED)
						var
							time = shuttle_master.emergency.timeLeft()
	...
	docking_port
		mobile
			emergency
				...
				var
					sound_played = 0 //If the launch sound has been sent to all players on the shuttle itself
...
obj
	docking_port
		mobile
			emergency
				request(obj/docking_port/stationary/S, coefficient=1, area/signalOrigin, reason, redAlert)
	...
	docking_port
		stationary
			random
				...
				height = 4
				var
					target_area = /area/mine/unexplored
		...
		stationary
			random
				initialize()
					...
					var
						list
							turfs = get_area_turfs(target_area)
					var
						turf
							T = pick(turfs)
					src.loc = T
