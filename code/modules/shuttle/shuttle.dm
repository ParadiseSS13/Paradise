...
/obj/docking_port/stationary/proc/launch_anim(var/list/L0)
	...
	for(var/turf/T in L0)
		...
		for(var/atom/A in atoms)
			...
			I.plane = FLOAT_PLANE
			I.transform = matrix(1, 0, (A.x - x)*world.icon_size, 0, 1, (A.y - y)*world.icon_size)
			I.underlays.Cut()
			overlay_list += I
	for(var/cdir in alldirs)
		...
		switch(shuttle_launch_anim_type)
			if("flyup")
				undock_anim(S)
				animate(pixel_y = world.icon_size*40, alpha = 0, time = 60, loop = 1, easing = QUAD_EASING | EASE_IN)
				spawn(70)
					qdel(S)
			if("flyright")
				undock_anim(S)
				animate(pixel_x = world.icon_size*40, alpha = 0, time = 60, loop = 1, easing = QUAD_EASING | EASE_IN)
				spawn(70)
					qdel(S)
			if("mining_home")
				animate(S, pixel_x = -world.icon_size, pixel_y = 0, transform = matrix(), time = 10, loop = 1, easing = QUAD_EASING | EASE_OUT)
				var/matrix/new_transform = matrix()
				new_transform.Turn(-90)
				animate(pixel_x = -world.icon_size*5, pixel_y = world.icon_size*3, transform = new_transform, time = 20, loop = 1, easing = QUAD_EASING | EASE_IN)
				animate(pixel_x = -world.icon_size*44, alpha = 0, time = 60, loop = 1)
				spawn(90)
					qdel(S)
			if("flydown_rotate_cw")
				...
				var/matrix/new_transform = matrix()
				new_transform.Turn(90)
				animate(pixel_y = -world.icon_size*11, alpha = 192, transform = new_transform, time = 15, loop = 1, easing = QUAD_EASING | EASE_IN)
				animate(pixel_y = -world.icon_size*41, alpha = 0, time = 25, loop = 1, easing = LINEAR_EASING)
				spawn(50)
					qdel(S)
			if("flyup_rotate_ccw")
				undock_anim(S)
			...
			if("flyleft_rotate_ccw")
				...
				var/matrix/new_transform = matrix()
				new_transform.Turn(-90)
				animate(pixel_x = -world.icon_size*11, alpha = 192, transform = new_transform, time = 15, loop = 1, easing = QUAD_EASING | EASE_IN)
				animate(pixel_x = -world.icon_size*41, alpha = 0, time = 25, loop = 1, easing = LINEAR_EASING)
				spawn(50)
					qdel(S)
			if("flyleft_rotate_cw")
				...
				spawn(50)
					qdel(S)
			if("backout")
				animate(S, pixel_x = -world.icon_size*13, pixel_y = 0, transform = matrix(), time = 60, loop = 1, easing = QUAD_EASING | EASE_OUT)
				var/matrix/new_transform = matrix()
				new_transform.Turn(-90)
				animate(pixel_y = world.icon_size*11, alpha = 192, transform = new_transform, time = 15, loop = 1, easing = QUAD_EASING | EASE_IN)
				animate(pixel_y = world.icon_size*41, alpha = 0, time = 25, loop = 1, easing = LINEAR_EASING)
...
/obj/docking_port/stationary/proc/undock_anim(var/obj/effect/shuttle_launch_anim/S)
	switch(dir)
		if(NORTH)
			animate(S, pixel_x = 0, pixel_y = world.icon_size, transform = matrix(), time = 10, loop = 1, easing = QUAD_EASING | EASE_OUT)
		if(SOUTH)
			animate(S, pixel_x = 0, pixel_y = -world.icon_size, transform = matrix(), time = 10, loop = 1, easing = QUAD_EASING | EASE_OUT)
		if(EAST)
			animate(S, pixel_x = world.icon_size, pixel_y = 0, transform = matrix(), time = 10, loop = 1, easing = QUAD_EASING | EASE_OUT)
