...
obj
	docking_port
		...
		var
			width = 0	//size of covered area, perpendicular to dir
		var
			height = 0	//size of covered area, paralell to dir
		var
			dwidth = 0	//position relative to covered area, perpendicular to dir
		var
			dheight = 0	//position relative to covered area, parallel to dir
...
obj
	docking_port
		proc
			return_ordered_turfs(_x, _y, _z, _dir, area/A)
				if(!_dir)
					_dir = dir
				if(!_x)
					_x = x
...
obj
	docking_port
		mobile
			...
			var
				mode = SHUTTLE_IDLE			//current shuttle mode (see global defines)
			var
				callTime = 50				//time spent in transit (deciseconds)
			var
				roundstart_move				//id of port to send shuttle to at roundstart
			var
				travelDir = 0				//direction the shuttle would travel in
			var
				rebuildable = 0				//can build new shuttle consoles for this one
...
obj
	docking_port
		mobile
			proc
				canDock(obj/docking_port/stationary/S)
					if(!istype(S))
						return 1
...
//			S1.dir = turn(NORTH, -travelDir)
	...
	var
		list
			L1 = return_ordered_turfs(S1.x, S1.y, S1.z, S1.dir)

	var
		rotation = dir2angle(S1.dir)-dir2angle(dir)
	if((rotation % 90) != 0)
					...
					var
						obj
							docking_port
								stationary
									transit
										T = shuttle_master.getDock("[id]_transit")
					if(T && !canDock(T))
						return T
...
obj
	docking_port
		mobile
			proc
				closePortDoors(obj/docking_port/stationary/S0)
					if(!istype(S0))
						return 1
...
obj
	docking_port
		mobile
			proc
				unlockPortDoors(obj/docking_port/stationary/S1)
					if(!istype(S1))
						return 0
...
obj
	machinery
		computer
			shuttle
				New(location, obj/item/weapon/circuitboard/shuttle/C)
					..()
					if(istype(C))
		...
		computer
			shuttle
				attack_hand(mob/user)
					...
					var
						list
							options = params2list(possible_destinations)
					var
						obj
							docking_port
								mobile
					...
					var
						datum
							browser
								popup = new(user, "computer", M ? M.name : "shuttle", 300, 200)
			...
			shuttle
				ferry
					request
						...
						var
							cooldown //prevents spamming admins
		...
		computer
			shuttle
				trade
					...
					docking_request = 1
					var
						possible_destinations_dock
					var
						possible_destinations_nodock
					var
						docking_request_message = "A trading ship has requested docking aboard the NSS Cyberiad for trading. This request can be accepted or denied using a communications console."
