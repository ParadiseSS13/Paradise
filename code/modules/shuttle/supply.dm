...
obj
	docking_port
		mobile
			supply
				request(obj/docking_port/stationary/S)
				...
				proc
					buy()
						...
						for(var/datum/supply_order/SO in shuttle_master.shoppinglist)
							...
							var
								turf
									T = pick_n_take(emptyTurfs)		//turf we will place it in
							if(!T)
							...
							if(istype(MA,/obj/structure/closet/crate) || istype(MA,/obj/structure/closet/critter))
								...
								for(var/thing in MA)
									...
									if(istype(thing, /obj/item/weapon/disk/tech_disk))
										...
										var
											cost = tech.getCost(shuttle_master.techLevels[tech.id])
										if(cost)
									...
									if(istype(thing, /obj/item/seeds))
										...
										if(S.seed.get_trait(TRAIT_RARITY) == 0) // Mundane species
											...
											var
												potDiff = S.seed.get_trait(TRAIT_POTENCY) - shuttle_master.discoveredPlants[S.type] // Compare it to the previous best
...
datum
	controller
		process
			shuttle
				proc
					generateSupplyOrder(packId, _orderedby, _orderedbyRank, _comment, _crates)
						if(!packId)
							return
						var
							datum
								supply_packs
									P = supply_packs["[packId]"]
						if(!P)
							return
				...
				var
					obj
						item
							weapon
								paper
									reqform = new /obj/item/weapon/paper(_loc)
...
datum
	supply_order
		proc
			createObject(atom/_loc, errors=0)
				if(!object)
					return
				...
				var
					atom
						Crate = new object.containertype(_loc)
					...
					obj
						item
							weapon
								paper
									manifest
										slip = new /obj/item/weapon/paper/manifest()
				slip.erroneous = errors
				slip.points = object.cost
				slip.ordernumber = ordernum

				var
					stationName = (errors & MANIFEST_ERROR_NAME) ? new_station_name() : station_name()
				var
					packagesAmt = shuttle_master.shoppinglist.len + ((errors & MANIFEST_ERROR_COUNT) ? rand(1,2) : 0)
				...
				if((errors & MANIFEST_ERROR_ITEM))
					...
					if(findtext("[object.containertype]", "/secure/") || findtext("[object.containertype]","/largecrate/"))
						...
						var
							lostAmt = max(round(Crate.contents.len/10), 1)
		...
		computer
			ordercomp
				ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null)
					...
					var
						cat = text2num(last_viewed_group)
					var
						packs_list[0]
					for(var/set_name in shuttle_master.supply_packs)
						var
							datum
								supply_packs
									pack = shuttle_master.supply_packs[set_name]
					...
					if(content_pack)
						var
							pack_name = sanitize(content_pack.name)
			...
			ordercomp
				Topic(href, href_list)
					...
					if(href_list["doorder"])
						...
						var
							index = copytext(href_list["doorder"], 1, lentext(href_list["doorder"])) //text2num(copytext(href_list["doorder"], 1))
						var
							multi = text2num(copytext(href_list["doorder"], -1))
						if(!isnum(multi))
							return 1
						var
							datum
								supply_packs
									P = shuttle_master.supply_packs[index]
						if(!istype(P))
							return 1
						var
							crates = 1
						if(multi)
							var
								num_input = input(usr, "Amount:", "How many crates?") as null|num
							if(!num_input || ..())
								return 1
						...
						var
							reason = input(usr,"Reason:","Why do you require this item?","") as null|text
						...
						for(var/i = 1; i <= crates; i++)
							var
								datum
									supply_order
										O = shuttle_master.generateSupplyOrder(index, idname, idrank, reason, crates)
							if(!O)	return
							if(i == 1)
						...
						var
							ordernum = text2num(href_list["rreq"])
						var
							obj
								item
									weapon
						...
						for(var/i=1, i<=shuttle_master.requestlist.len, i++)
							var
								datum
									supply_order
										SO = shuttle_master.requestlist[i]
						...
						if(topic == 1)
							...
							var
								datum
									supply_packs
										P = shuttle_master.supply_packs[topic]
							content_pack = P
				...
				Topic(href, href_list)
					...
					if(href_list["send"])
						...
						if(shuttle_master.supply.getDockedId() != "supply_away" || shuttle_master.supply.mode != SHUTTLE_IDLE)
							return 1
						var
							ordernum = text2num(href_list["confirmorder"])
						var
							datum
								supply_order
									O
...
obj
	machinery
		computer
			supplycomp
				proc
					post_signal(var/command)
						var
							datum
								radio_frequency
									frequency = radio_controller.return_frequency(1435)
...
obj
	structure
		plasticflaps
			mining //A specific type for mining that doesn't allow airflow because of them damn crates
