...
/obj/docking_port/mobile/arrivals/check()
	...
	if(damaged)
		if(!CheckTurfsPressure())
			damaged = FALSE
			if(console)
				console.atom_say("Repairs complete, launching soon.")
		return
...
//If this proc is high on the profiler add a cooldown to the stuff after this line
		...
		damaged = TRUE
		if(console)
			console.atom_say("Alert, hull breach detected!")
		global_announcer.autosay("The arrivals shuttle has been damaged. Docking for repairs...")
...
/obj/docking_port/mobile/arrivals/proc/CheckTurfsPressure()
	for(var/I in latejoin)
		var/turf/simulated/T = get_turf(I)
		if(T && istype(T))
...
/obj/docking_port/mobile/arrivals/proc/SendToStation()
	...
	if(mode == SHUTTLE_CALL && timeLeft(1) > dockTime)
		if(console)
			console.atom_say(damaged ? "Initiating emergency docking for repairs!" : "Now approaching: [station_name()].")
...
/obj/docking_port/mobile/arrivals/dock(obj/docking_port/stationary/S1, force=FALSE)
	...
	if(docked)	//about to launch
		if(!force_depart && PersonCheck())
			mode = SHUTTLE_IDLE
			if(console)
				console.atom_say("Launch cancelled, lifeform dectected on board.")
			return
		force_depart = FALSE
	. = ..()
	if(!. && !docked && !damaged)
		console.atom_say("Welcome to your new life, employees!")
...
/obj/docking_port/mobile/arrivals/proc/Launch(pickingup)
	if(mode != SHUTTLE_CALL)
		if(pickingup)
			force_depart = TRUE
		if(console)
			console.atom_say(pickingup ? "Departing immediately for new employee pickup." : "Shuttle departing.")
...
/obj/docking_port/mobile/arrivals/proc/QueueAnnounce(mob, rank, join_message)
	LAZYADD(queued_announces, CALLBACK(GLOBAL_PROC, .proc/AnnounceArrival, mob, rank, join_message))