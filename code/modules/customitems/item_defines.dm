			...
			fluff
				tattoo_gun // Generic tattoo gun, make subtypes for different folks
					...
					var
						used = 0
					var
						tattoo_name = "tiger stripe tattoo" // Tat name for visible messages
					var
						tattoo_icon = "Tiger Body" // body_accessory.dmi, new icons defined in sprite_accessories.dm
					var
						tattoo_r = 1 // RGB values for the body markings
					var
						tattoo_g = 1
					var
						tattoo_b = 1
				...
				tattoo_gun
					update_icon()
						...
						if(!used)
							var
								image
									ink = image(src.icon, src, "ink_overlay")
...
obj
	item
		weapon
			claymore
				fluff // MrBarrelrolll: Maximus Greenwood
...
obj
	item
		weapon
			crowbar
				fluff
					zelda_creedy_1 // Zomgponies: Griffin Rowley
	...
	item
		clothing
			glasses
				meson
					fluff
						book_berner_1 // Adrkiller59: Adam Cooper
								...
								mob
									living
										simple_animal
											pet
												corgi
													C = new /mob/living/simple_animal/pet/corgi(get_turf(user))
							var
								obj
									item
										clothing
											head
												det_hat
													D = new /obj/item/clothing/head/det_hat(C)
							D.flags |= NODROP
							C.inventory_head = D
							C.regenerate_icons()
	...
	item
		weapon
			storage
				toolbox
					fluff
						lunchbox //godoforeos: Jason Conrad
							name = "lunchpail"
...
obj
	item
		device
			guitar
				jello_guitar //Antcolon3: Dan Jello
...
obj
	item
		clothing
			head
				fluff
					heather_winceworth // Regens: Heather Winceworth
						name= "Heather's rose"
	...
	item
		clothing
			under
				psysuit
					fluff
						isaca_sirius_1 // Xilia: Isaca Sirius
							name = "Isaca's suit"
		...
		clothing
			suit
				armor
					vest
						fluff
							tactical //m3hillus: Medusa Schlofield
				...
				tattoo_gun
					elliot_cybernetic_tat
						attack_self(mob/user as mob)
							if(!used)
								var
									ink_color = input("Please select an ink color.", "Tattoo Ink Color", rgb(tattoo_r, tattoo_g, tattoo_b)) as color|null
