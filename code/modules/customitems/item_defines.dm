...
/obj/item/fluff/fei_gasmask_kit/afterattack(atom/target, mob/user, proximity)
	...
	if(istype(target, /obj/item/clothing/mask/gas) && !istype(target, /obj/item/clothing/mask/gas/welding))
		...
		M.desc = "It looks heavily modified, but otherwise functions as a gas mask. The words ‚ÄúProperty of Yon-Dale‚Äù can be seen on the inner band."
...
/obj/item/melee/classic_baton/telescopic/diosdado/attack(mob/target as mob, mob/living/user as mob)
	if(on)
		...
		if((CLUMSY in user.mutations) && prob(50))
			to_chat(user, "<span class ='danger'>Se expropiÛ a si mismo.</span>")
			user.Weaken(3 * force)
			if(ishuman(user))
				var/mob/living/carbon/human/H = user
