proc
	EquipCustomItems(mob/living/carbon/human/M)
		...
		var
			DBQuery
				query = dbcon.NewQuery("SELECT cuiPath, cuiPropAdjust, cuiJobMask, cuiDescription, cuiItemName FROM [format_table_name("customuseritems")] WHERE cuiCKey='[M.ckey]' AND (cuiRealName='[M.real_name]' OR cuiRealName='*')")
		query.Execute()

		while(query.NextRow())
			var
				path = text2path(query.item[1])
			var
			...
			if(jobmask != "*")
				var
					list
						allowed_jobs = splittext(jobmask,",")
...
proc
	HackProperties(var/mob/living/carbon/human/M,var/obj/item/I,var/script)
		var
			list
				statements = splittext(script,";")
		if(statements.len == 0)
			return
		for(var/statement in statements)
			var
				list
					assignmentChunks = splittext(statement,"=")
			var
				varname = assignmentChunks[1]
			var
				list
					typeChunks=splittext(script,":")
			var
