/proc/EquipCustomItems(mob/living/carbon/human/M)
	...
	while(query.NextRow())
		...
		var/ok = 0
		if(!path)
			log_debug("Incorrect database entry found in table 'customuseritems', cuiPath is null. cuiCKey='[M.ckey]' AND (cuiRealName='[sanitizeSQL(M.real_name)]' OR cuiRealName='*'")
			continue
		if(jobmask != "*")
