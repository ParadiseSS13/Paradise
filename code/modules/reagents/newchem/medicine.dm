...
datum
	reagent
		silver_sulfadiazine
			reaction_mob(var/mob/living/M as mob, var/method=TOUCH, var/volume, var/show_message = 1)
				if(iscarbon(M))
					if(method == TOUCH)
...
datum
	reagent
		synthflesh
			reaction_mob(var/mob/living/M, var/method=TOUCH, var/volume,var/show_message = 1)
				if(!M) M = holder.my_atom
				if(iscarbon(M))
					if(method == TOUCH)
...
datum
	reagent
		synthflesh
			reaction_turf(var/turf/T, var/volume) //let's make a mess!
				src = null
				if(volume >= 5)
							...
							obj
								item
									organ
										internal
											eyes
												E = H.get_int_organ(/obj/item/organ/internal/eyes)
						if(istype(E))
...
datum
	reagent
		strange_reagent
			reaction_mob(var/mob/living/M as mob, var/method=TOUCH, var/volume)
				if(isanimal(M))
					if(method == TOUCH)
						var
							mob
...
datum
	reagent
		mannitol
			on_mob_life(mob/living/M as mob)
				M.adjustBrainLoss(-3)
				..()
				return
	...
	reagent
		mutadone
			on_mob_life(var/mob/living/carbon/human/M as mob)
				M.jitteriness = 0
				var
					needs_update = M.mutations.len > 0 || M.disabilities > 0 || M.sdisabilities > 0
		...
		stimulants
			on_mob_life(var/mob/living/M as mob)
				...
				if(volume > 5)
					...
					M.SetSleeping(0)
					var
						status = CANSTUN | CANWEAKEN | CANPARALYSE
...
datum
	reagent
		stimulants
			reagent_deleted(var/mob/living/M as mob)
				if(!M) M = holder.my_atom
				M.status_flags |= CANSTUN | CANWEAKEN | CANPARALYSE
				..()
				return
