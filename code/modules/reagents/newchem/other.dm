...
var
	list
		random_color_list = list("#00aedb","#a200ff","#f47835","#d41243","#d11141","#00b159","#00aedb","#f37735","#ffc425","#008744","#0057e7","#d62d20","#ffa700")
...
datum
	reagent
		colorful_reagent
			reaction_mob(var/mob/living/simple_animal/M, var/method=TOUCH, var/volume)
							if(M && istype(M))
...
datum
	reagent
		hair_dye
			reaction_mob(var/mob/living/M, var/volume)
				if(M && ishuman(M))
					var
						mob
							living
			...
			reaction_mob(var/mob/living/M, var/volume)
				if(M && ishuman(M))
					...
					var
						datum
							sprite_accessory
								tmp_hair_style = hair_styles_list["Very Long Hair"]
					var
						datum
							sprite_accessory
								tmp_facial_hair_style = facial_hair_styles_list["Very Long Beard"]
						...
						var
							obj
								item
									clothing
										mask
											fakemoustache = new /obj/item/clothing/mask/fakemoustache
	...
	reagent
		love
			on_mob_life(var/mob/living/M as mob)
				...
				if(prob(8))
					var
						lovely_phrase = pick("appreciated", "loved", "pretty good", "really nice", "pretty happy with yourself, even though things haven't always gone as well as they could")
