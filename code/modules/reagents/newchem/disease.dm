								...
								obj
									item
										organ
											internal
												body_egg
													spider_eggs(M) //Yes, even Xenos can fall victim to the plague that is spider infestation.
				..()
							...
							obj
								item
									organ
										internal
											heart
												ate_heart = H.get_int_organ(/obj/item/organ/internal/heart)
						if(ate_heart)
							ate_heart.remove(H)
							qdel(ate_heart)
				..()
								...
								var
									obj
										effect
											blob
												core
													core = new(get_turf(C), 200, blob_client, blob_point_rate)
...
datum
	chemical_reaction
		virus_food_plasma_diphenhydramine
			name = "weakened virus plasma"
			id = "weakplasmavirusfood"
...
datum
	chemical_reaction
		virus_food_mutagen_salineglucose
			name = "sucrose agar"
			...
			on_reaction(datum/reagents/holder, created_volume)
				var
					datum
						reagent
							blood
								B = locate(/datum/reagent/blood) in holder.reagent_list
				if(B && B.data)
					var
						datum
							disease
								advance
									D = locate(/datum/disease/advance) in B.data["viruses"]
					if(D)
