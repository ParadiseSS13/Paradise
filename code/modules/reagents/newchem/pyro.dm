...
proc
	goonchem_vortex(var/turf/simulated/T, var/setting_type, var/range, var/pull_times)
...
datum
	reagent
		blackpowder
			reaction_turf(var/turf/T, var/volume) //oh shit
				src = null
				if(volume >= 5)
...
proc
	blackpowder_detonate(var/datum/reagents/holder, var/created_volume)
		var
			turf
				simulated
					T = get_turf(holder.my_atom)
		var
			ex_severe = round(created_volume / 100)
		var
			ex_heavy = round(created_volume / 42)
		var
			ex_light = round(created_volume / 20)
		var
			ex_flash = round(created_volume / 8)
...
datum
	chemical_reaction
		smoke
			...
			var
				forbidden_reagents = list("sugar", "phosphorus", "potassium", "stimulants") //Do not transfer this stuff through smoke.
...
datum
	reagent
		cryostylane
			on_mob_life(var/mob/living/M as mob) //TODO: code freezing into an ice cube
...
datum
	reagent
		firefighting_foam
			...
			color = "#A0A090"
			var
				cooling_temperature = 3 // more effective than water
...
datum
	reagent
		plasma_dust
			reaction_mob(mob/living/M, method=TOUCH, volume)//Splashing people with plasma dust is stronger than fuel!
				if(!istype(M, /mob/living))
					return
				if(method == TOUCH)
