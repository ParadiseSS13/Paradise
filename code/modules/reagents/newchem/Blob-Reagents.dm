...
datum
	reagent
		blob
			description = ""
			var
				message = "The blob strikes you" //message sent to any mob hit by the blob
			var
				message_living = null //extension to first mob sent to only living mobs i.e. silicons have no skin to be burnt

datum
	reagent
		blob
			reaction_mob(mob/living/M, method=TOUCH, volume, show_message, touch_protection)
...
datum
	reagent
		blob
			proc
				reagent_vortex(mob/living/M, setting_type, volume)
					var
						turf
							pull = get_turf(M)
					var
						range_power = Clamp(round(volume/5, 1), 1, 5)
					for(var/atom/movable/X in range(range_power,pull))
						if(istype(X, /obj/effect))
							continue
						if(!X.anchored)
							var
								distance = get_dist(X, pull)
							var
								moving_power = max(range_power - distance, 1)
							spawn(0)
								if(moving_power > 2) //if the vortex is powerful and we're close, we get thrown
									if(setting_type)
										var
											atom
												throw_target = get_edge_target_turf(X, get_dir(X, get_step_away(X, pull)))
										var
											throw_range = 5 - distance
...
datum
	reagent
		blob
			proc
				send_message(var/mob/living/M as mob)
					var
