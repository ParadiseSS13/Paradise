...
datum
	reagent
		histamine
			reaction_mob(var/mob/living/M as mob, var/method=TOUCH, var/volume) //dumping histamine on someone is VERY mean.
				if(iscarbon(M))
					if(method == TOUCH)
							...
							obj
								effect
									decal
										cleanable
											molten_item
												I = new/obj/effect/decal/cleanable/molten_item(O.loc)
		...
		lipolicide
			on_mob_life(var/mob/living/M as mob)
				...
				if(!M.nutrition)
					...
					if(prob(60))
						var
							fat_to_burn = max(round(M.nutrition/100,1), 5)
...
datum
	reagent
		glowing_slurry
			reaction_mob(var/mob/M, var/method=TOUCH, var/volume) //same as mutagen
				if(!..())	return
...
datum
	reagent
		ants
			reaction_mob(var/mob/living/M as mob, var/method=TOUCH, var/volume) //NOT THE ANTS
				if(iscarbon(M))
					if(method == TOUCH || method==INGEST)
