...
datum
	reagent
		...
		var
			addiction_stage = 1
		var
			last_addiction_dose = 0
		var
			overdosed = 0 // You fucked up and this is now triggering it's overdose effects, purge that shit quick.
		var
			current_cycle = 1
			...
			metabolize(var/mob/M)
				...
				for(var/A in reagent_list)
					...
					if(ishuman(M))
						...
						var
							species_handled = !(H.species.handle_reagents(H, R))
...
datum
	reagent
		proc
			addiction_act_stage5(var/mob/living/M as mob)
				if(prob(8))
					M.emote("twitch")
				if(prob(6))
...
var
	list
		chemical_mob_spawn_meancritters = list() // list of possible hostile mobs
var
	list
		chemical_mob_spawn_nicecritters = list() // and possible friendly mobs
datum
	chemical_reaction
		proc
			chemical_mob_spawn(datum/reagents/holder, amount_to_spawn, reaction_name, mob_faction = "chemicalsummon")
				if(holder && holder.my_atom)
					...
					var
						area
							my_area = get_area(T)
					var
						message = "A [reaction_name] reaction has occured in [my_area.name]. (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[T.x];Y=[T.y];Z=[T.z]'>JMP</A>)"
