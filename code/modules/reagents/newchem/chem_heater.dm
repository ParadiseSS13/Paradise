		...
		chem_heater
			Topic(href, href_list)
				...
				if(href_list["adjust_temperature"])
					var
						val = href_list["adjust_temperature"]
					if(isnum(val))
						desired_temp = Clamp(desired_temp+val, 0, 1000)
					else if(val == "input")
						var
							target = input("Please input the target temperature", name) as num
...
obj
	machinery
		chem_heater
			ui_interact(var/mob/user, ui_key = "main", var/datum/nanoui/ui = null)
