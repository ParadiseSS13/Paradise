							...
							if(beaker.reagents.reagent_list.len)
								var
									datum
										reagent
											blood
												BL = locate() in beaker.reagents.reagent_list
								if(BL)
...
obj
	machinery
		computer
			pandemic
				proc
					GetVirusTypeByIndex(index)
						var
							datum
								disease
									D = GetVirusByIndex(index)
						if(D)
			...
			pandemic
				Topic(href, href_list)
					...
					if(href_list["create_vaccine"])
									...
									item
										weapon
											reagent_containers
												glass
													bottle
														B = new/obj/item/weapon/reagent_containers/glass/bottle(src.loc)
							if(B)
								B.pixel_x = rand(-3, 3)
								B.pixel_y = rand(-3, 3)
								var
									path = GetResistancesByIndex(text2num(href_list["create_vaccine"]))
								var
									vaccine_type = path
								var
						...
						if(!wait)
							var
								type = GetVirusTypeByIndex(text2num(href_list["create_virus_culture"]))//the path is received as string - converting
							var
								datum
									disease
										D = null
							if(!ispath(type))
								...
								var
									datum
										disease
											advance
												A = archive_diseases[D.GetDiseaseID()]
								if(A)
									D = new A.type(0, A)
							else if(type)
							...
							if(!D)
								return
							var
								name = stripped_input(usr,"Name:","Name the culture",D.name,MAX_NAME_LEN)
							if(name == null || wait)
								return
							var
								obj
						...
						var
							new_name = stripped_input(usr, "Name the Disease", "New Name", "", MAX_NAME_LEN)
						if(!new_name)
							return
						if(..())
							return
						var
							id = GetVirusTypeByIndex(text2num(href_list["name_disease"]))
						if(archive_diseases[id])
							var
								datum
									disease
										advance
											A = archive_diseases[id]
