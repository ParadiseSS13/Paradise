			...
			remove_any(var/amount=1)
				...
				while(total_transfered != amount)
					...
					var
						datum
							reagent
								current_reagent = reagent_list[current_list_element]
			...
			trans_to(var/target, var/amount=1, var/multiplier=1, var/preserve_data=1)//if preserve_data=0, the reagents data will be lost. Usefull if you use data for some strange stuff and don't want it to be transferred.
				...
				for(var/datum/reagent/current_reagent in src.reagent_list)
					...
					if(current_reagent.id == "blood" && ishuman(target))
						...
						continue
					var
						current_reagent_transfer = current_reagent.volume * part
					if(preserve_data)
				...
				do
					...
					for(var/datum/reagent/R in reagent_list) // Usually a small list
						for(var/reaction in chemical_reactions_list[R.id]) // Was a big list but now it should be smaller since we filtered it with our reagent id
							...
							var
								matching_container = 0
							var
								matching_other = 0
							var
								list
									multipliers = new/list()
							var
								min_temp = C.min_temp			//Minimum temperature required for the reaction to occur (heat to/above this)
							var
								max_temp = C.max_temp			//Maximum temperature allowed for the reaction to occur (cool to/below this)
							...
							if(total_matching_reagents == total_required_reagents && total_matching_catalysts == total_required_catalysts && matching_container && matching_other && chem_temp <= max_temp && chem_temp >= min_temp)
								...
								var
									created_volume = C.result_amount*multiplier
								if(C.result)
								...
								var
									list
										seen = viewers(4, get_turf(my_atom))
...
datum
	reagents
		proc
			reaction(atom/A, method=TOUCH, volume_modifier = 1)
				switch(method)
					if(TOUCH)
						for(var/datum/reagent/R in reagent_list)
							if(ismob(A))
...
datum
	reagents
		proc
			add_reagent_list(list/list_reagents, list/data=null) // Like add_reagent but you can enter a list. Format it like this: list("toxin" = 10, "beer" = 15)
				for(var/r_id in list_reagents)
					var
						amt = list_reagents[r_id]
		...
		proc
			add_reagent(var/reagent, var/amount, var/list/data=null, var/reagtemp = 300)
				...
				var
					datum
						reagent
							D = chemical_reagents_list[reagent]
				if(D)
