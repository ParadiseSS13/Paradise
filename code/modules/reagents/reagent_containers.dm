			...
			reagent_containers
				verb
					set_APTFT() //set amount_per_transfer_from_this
						...
						var
							N = input("Amount per transfer from this:", "[src]", default) as null|anything in possible_transfer_amounts
						if(N)
				...
				New()
					...
					if(spawned_disease)
						var
							datum
								disease
									F = new spawned_disease(0)
						var
							list
								data = list("viruses" = list(F), "blood_colour" = "#A10808")
...
obj
	item
		weapon
			reagent_containers
				proc
					reagentlist(var/obj/item/weapon/reagent_containers/snack) //Attack logs for regents in pills
						var
							data
