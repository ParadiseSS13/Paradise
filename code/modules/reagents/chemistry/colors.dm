...
/proc/mix_color_from_reagents(const/list/reagent_list)
	...
	for(var/datum/reagent/reagent in reagent_list)
		if(reagent.type == /datum/reagent/blood && reagent.data && reagent.data["blood_color"])
			reagent_color = reagent.data["blood_color"]
		else if(reagent.type == /datum/reagent/slimejelly && reagent.data && reagent.data["colour"])
