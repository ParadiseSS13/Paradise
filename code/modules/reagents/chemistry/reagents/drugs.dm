...
/datum/reagent/crank/overdose_process(mob/living/M, severity)
	...
	if(severity == 1)
		if(effect <= 2)
			...
			update_flags |= M.adjustToxLoss(1, FALSE)
			update_flags |= M.Stun(4 SECONDS, FALSE)
		else if(effect <= 7)
			M.Jitter(30)
			M.emote("grumble")
	else if(severity == 2)
...
/datum/reagent/krokodil/overdose_process(mob/living/M, severity)
	...
	if(severity == 1)
		...
		if(effect <= 2)
			...
			update_flags |= M.Weaken(16 SECONDS)
			M.emote("faint")
		else if(effect <= 4)
			if(ishuman(M))
...
/datum/reagent/methamphetamine/overdose_process(mob/living/M, severity)
	...
	if(severity == 1)
		if(effect <= 2)
			...
			M.AdjustConfused(20)
			update_flags |= M.Weaken(8 SECONDS)
		else if(effect <= 4)
		...
		if(effect <= 2)
			...
			M.Jitter(10)
			update_flags |= M.Weaken(20 SECONDS)
		else if(effect <= 7)
			M.emote("laugh")
	return list(effect, update_flags)
...
/datum/reagent/fliptonium/overdose_process(mob/living/M, severity)
	...
	if(severity == 1)
		...
		if(effect <= 2)
			...
			M.visible_message("<span class='warning'>[M] falls to the floor and flails uncontrollably!</span>")
			M.Jitter(5)
			update_flags |= M.Weaken(10 SECONDS)
		else if(effect <= 7)
			M.emote("laugh")
	return list(effect, update_flags)
