...
/datum/reagent/crank/on_mob_life(mob/living/M)
	...
	if(prob(8))
		to_chat(M, span_notice("You feel great!"))
		M.reagents.add_reagent("methamphetamine", rand(1,2))
		M.emote(pick("laugh", "giggle"))
	if(prob(6))
		to_chat(M, span_notice("You feel warm."))
...
/datum/reagent/crank/overdose_process(mob/living/M, severity)
	...
	if(severity == 1)
		...
		if(effect <= 2)
			...
			M.visible_message(span_warning("[M] starts tweaking the hell out!"))
...
/datum/reagent/krokodil/on_mob_life(mob/living/M)
	...
	if(prob(10))
		...
		M.emote("smile")
	if(prob(5))
		to_chat(M, span_notice("You feel too chill!"))
	...
	if(severity == 1)
		...
		if(effect <= 2)
			...
			if(ishuman(M))
				...
				H.visible_message(span_warning("[M]'s skin is rotting away!"))
...
/datum/reagent/bath_salts/on_mob_life(mob/living/M)
	...
	if(ishuman(M))
		...
		if(check < 8 && head_organ.h_style != "Very Long Beard")
			...
			H.update_hair()
			H.update_fhair()
			H.visible_message(span_warning("[H] has a wild look in [H.p_their()] eyes!"))
	if(check < 60)
		M.SetParalysis(0)
		M.SetStunned(0)
		M.SetWeakened(0)
