...
/datum/reagent/nicotine/on_mob_life(mob/living/M)
	...
	if(prob(50))
		...
		update_flags |= M.AdjustImmobilized(-20, FALSE)
...
/datum/reagent/krokodil/overdose_process(mob/living/M, severity)
	...
	if(severity == 1)
		...
		if(effect <= 2)
			...
			update_flags |= M.Paralyze(160, FALSE)
			M.emote("faint")
		else if(effect <= 4)
			if(ishuman(M))
...
/datum/reagent/methamphetamine/on_mob_life(mob/living/M)
	...
	if(current_cycle >= 25)
		M.AdjustJitter(5)
	M.AdjustDrowsy(-10)
	update_flags |= M.AdjustUnconscious(-50, FALSE)
	update_flags |= M.AdjustStun(-50, FALSE)
	update_flags |= M.AdjustParalyzed(-50, FALSE)
	update_flags |= M.AdjustKnockdown(-50, FALSE)
	update_flags |= M.AdjustImmobilized(-50, FALSE)
...
/datum/reagent/methamphetamine/overdose_process(mob/living/M, severity)
	...
	if(severity == 1)
		if(effect <= 2)
			...
			M.AdjustConfused(20)
			update_flags |= M.Paralyze(80, FALSE)
		else if(effect <= 4)
		...
		if(effect <= 2)
			...
			M.Jitter(10)
			update_flags |= M.Paralyze(200, FALSE)
		else if(effect <= 7)
			M.emote("laugh")
	return list(effect, update_flags)
...
/datum/reagent/bath_salts/on_mob_life(mob/living/M)
	...
	if(check < 60)
		update_flags |= M.SetUnconscious(0, FALSE)
		update_flags |= M.SetStun(0, FALSE)
		update_flags |= M.SetKnockdown(0, FALSE)
		update_flags |= M.SetImmobilized(0, FALSE)
		update_flags |= M.SetParalyzed(0, FALSE)
	if(check < 30)
...
/datum/reagent/fliptonium/on_mob_life(mob/living/M)
	...
	M.AdjustDrowsy(-6)
	update_flags |= M.AdjustUnconscious(-30, FALSE)
	update_flags |= M.AdjustStun(-30, FALSE)
	update_flags |= M.AdjustParalyzed(-30, FALSE)
...
/datum/reagent/fliptonium/overdose_process(mob/living/M, severity)
	...
	if(severity == 1)
		if(effect <= 2)
			M.visible_message("<span class='warning'>[M] can't seem to control [M.p_their()] legs!</span>")
			M.AdjustConfused(33)
			update_flags |= M.Paralyze(40, FALSE)
		else if(effect <= 4)
			M.visible_message("<span class='warning'>[M]'s hands flip out and flail everywhere!</span>")
			M.drop_l_hand()
			M.drop_r_hand()
		...
		if(effect <= 2)
			...
			M.visible_message("<span class='warning'>[M] falls to the floor and flails uncontrollably!</span>")
			M.Jitter(5)
			update_flags |= M.Paralyze(100, FALSE)
		else if(effect <= 7)
			M.emote("laugh")
	return list(effect, update_flags)
