...
obj
	machinery
		chem_dispenser
			...
			var
				list
					dispensable_reagents = list("hydrogen", "lithium", "carbon", "nitrogen", "oxygen", "fluorine",
			"sodium", "aluminum", "silicon", "phosphorus", "sulfur", "chlorine", "potassium", "iron",
			"copper", "mercury", "plasma", "radium", "water", "ethanol", "sugar", "tungsten", "iodine", "bromine", "silver")
			var
				list
					hacked_reagents = list("toxin")
			var
				hack_message = "You disable the safety safeguards, enabling the \"Mad Scientist\" mode."
			var
				unhack_message = "You re-enable the safety safeguards, enabling the \"NT Standard\" mode."
			var
				list
					broken_requirements = list()
			var
				broken_on_spawn = 0
			var
				recharge_delay = 5
			var
				image
					icon_beaker = null //cached overlay
				...
				if(href_list["remove"])
					if(beaker)
						if(href_list["removeamount"])
							var
								amount = text2num(href_list["removeamount"])
...
obj
	machinery
		chem_dispenser
			attackby(var/obj/item/weapon/reagent_containers/B as obj, var/mob/user as mob, params)
				if(isrobot(user))
					return
...
obj
	machinery
		chem_dispenser
			attackby(var/obj/item/weapon/B as obj, var/mob/user as mob, params)
				..()
	...
	machinery
		chem_dispenser
			constructable
				...
				var
					list
						special_reagents = list(list("hydrogen", "oxygen", "silicon", "phosphorus", "sulfur", "carbon", "nitrogen", "tungsten", "water"),
	...
	machinery
		chem_master
			...
			var
				condi = 0
			var
				useramount = 30 // Last used amount
			var
				pillamount = 10
			var
				patchamount = 10
			var
				bottlesprite = "1" //yes, strings
			var
				pillsprite = "1"
			var
				client
				...
				if(beaker)
					...
					if(href_list["analyze"])
						...
						if(!condi)
							...
							var
								name = input(usr,"Name:","Name your pill!","[reagents.get_master_reagent_name()] ([amount_per_pill]u)") as text|null
							if(!name)
								return
										...
										item
											weapon
												reagent_containers
													food
														pill
															P = new/obj/item/weapon/reagent_containers/food/pill(src.loc)
							...
							var
								name = input(usr,"Name:","Name your bag!",reagents.get_master_reagent_name()) as text|null
							if(!name)
								return
							name = reject_bad_text(name)
							var
										...
										weapon
											reagent_containers
												food
													condiment
														pack
															P = new/obj/item/weapon/reagent_containers/food/condiment/pack(src.loc)
							if(!name) name = reagents.get_master_reagent_name()
							P.originalname = name
							P.name = "[name] pack"
						...
						if(!condi)
							...
							var
								amount_per_patch = reagents.total_volume/count
							if(amount_per_patch > 40) amount_per_patch = 40
							var
								name = input(usr,"Name:","Name your patch!","[reagents.get_master_reagent_name()] ([amount_per_patch]u)") as text|null
							if(!name)
								return
							name = reject_bad_text(name)
							var
								is_medical_patch = chemical_safety_check(reagents)
							while(count--)
								var
									obj
										item
											weapon
												reagent_containers
													food
														pill
															patch
																P = new/obj/item/weapon/reagent_containers/food/pill/patch(src.loc)
								if(!name) name = reagents.get_master_reagent_name()
								P.name = "[name] patch"
								P.pixel_x = rand(-7, 7) //random position
								P.pixel_y = rand(-7, 7)
						...
						if(!condi)
							var
								name = input(usr,"Name:","Name your bottle!",reagents.get_master_reagent_name()) as text|null
							if(!name)
								return
							name = reject_bad_text(name)
							var
...
obj
	machinery
		chem_master
			proc
				chemical_safety_check(datum/reagents/R)
					var
						all_safe = 1
...
obj
	machinery
		chem_master
			constructable
				attackby(obj/item/B, mob/user, params)
						...
						obj
							item
								stack
									sheet
										mineral
											plasma = list("plasma_dust" = 20),
						obj
							item
								stack
									sheet
										mineral
											uranium = list("uranium" = 20),
						obj
							item
								stack
									sheet
										mineral
											bananium = list("banana" = 20),
						obj
							item
								stack
									sheet
										mineral
											tranquillite = list("nothing" = 20),
						obj
							item
								stack
									sheet
										mineral
											silver = list("silver" = 20),
						obj
							item
								stack
									sheet
							...
							item
								weapon
									reagent_containers
										food
											snacks
												watermelonslice = list("watermelonjuice" = 0),
			)
...
obj
	machinery
		reagentgrinder
			interact(mob/user as mob) // The microwave Menu
				var
					is_chamber_empty = 0
				var
					is_beaker_ready = 0
...
obj
	machinery
		reagentgrinder
			proc
				is_allowed(var/obj/item/weapon/reagent_containers/O)
...
obj
	machinery
		reagentgrinder
			proc
				get_allowed_by_id(var/obj/item/weapon/grown/O)
					for(var/i in blend_items)
						if(istype(O, i))
...
obj
	machinery
		reagentgrinder
			proc
				get_allowed_snack_by_id(var/obj/item/weapon/reagent_containers/food/snacks/O)
					for(var/i in blend_items)
						if(istype(O, i))
							return blend_items[i]
...
obj
	machinery
		reagentgrinder
			proc
				get_allowed_juice_by_id(var/obj/item/weapon/reagent_containers/food/snacks/O)
...
obj
	machinery
		reagentgrinder
			proc
				get_allowed_snack_by_tag(var/obj/item/weapon/reagent_containers/food/snacks/grown/O)
...
obj
	machinery
		reagentgrinder
			proc
				get_allowed_juice_by_tag(var/obj/item/weapon/reagent_containers/food/snacks/grown/O)
...
obj
	machinery
		reagentgrinder
			proc
				get_grownweapon_amount(var/obj/item/weapon/grown/O)
					if(!istype(O))
						return 5
					else if(O.potency == -1)
						return 5
...
obj
	machinery
		reagentgrinder
			proc
				get_juice_amount(var/obj/item/weapon/reagent_containers/food/snacks/grown/O)
					if(!istype(O))
						return 5
					else if(O.potency == -1)
						return 5
				...
				juice()
					...
					for(var/obj/item/weapon/reagent_containers/food/snacks/O in holdingitems)
						...
						for(var/r_id in allowed)
							...
							var
								space = beaker.reagents.maximum_volume - beaker.reagents.total_volume
							var
