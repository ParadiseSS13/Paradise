datum
	reagent
		...
		var
			color = "#000000" // rgb: 0, 0, 0 (does not support alpha channels - yet!)
		var
			shock_reduction = 0
		var
			penetrates_skin = 0 //Whether or not a reagent penetrates the skin
		var
			can_grow_in_plants = 1	//Determines if the reagent can be grown in plants, 0 means it cannot be grown
			...
			reaction_mob(var/mob/M, var/method=TOUCH, var/volume) //Some reagents transfer on touch, others don't; dependent on if they penetrate the skin or not.
				...
				if(self.holder)		//for catching rare runtimes
							...
							var
								mob
									living
										L = M
							var
								block  = L.get_permeability_protection()
							var
								amount = round(self.volume * (1.0 - block), 0.1)
							if(L.reagents)
								if(amount >= 1)
					...
					if(method == INGEST) //Yes, even Xenos can get addicted to drugs.
						var
							can_become_addicted = M.reagents.reaction_check(M, self)
