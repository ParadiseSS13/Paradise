	...
	reagent
		water
			reaction_turf(var/turf/simulated/T, var/volume)
				...
				var
					hotspot = (locate(/obj/effect/hotspot) in T)
				if(hotspot && !istype(T, /turf/space))
					var
						datum
							gas_mixture
								lowertemp = T.remove_air( T:air:total_moles() )
...
datum
	reagent
		blood
			reaction_mob(mob/M, method=TOUCH, reac_volume)
...
datum
	reagent
		blood
			reaction_turf(turf/simulated/T, reac_volume)//splash the blood all over the place
				if(!istype(T))
					return
				if(reac_volume < 3)
					return
						...
						obj
							effect
								decal
									cleanable
										blood
											blood_prop = locate() in T //find some blood here
...
datum
	reagent
		fishwater
			toiletwater
				reaction_mob(var/mob/M, var/method=TOUCH, var/volume) //For shennanigans
					return
...
datum
	reagent
		liquidgibs
			reaction_turf(var/turf/T, var/volume) //yes i took it from synthflesh...
				src = null
				if(volume >= 5)
...
datum
	reagent
		drying_agent
			reaction_turf(turf/simulated/T, reac_volume)
				if(istype(T) && T.wet)
...
datum
	reagent
		drying_agent
			reaction_obj(obj/O, reac_volume)
