				...
				drugs
					...
					afterattack(obj/target, mob/user, proximity)
						...
						if(istype(target, /obj/structure/reagent_dispensers)) //A dispenser. Transfer FROM it TO us.
							...
							var
								trans = target.reagents.trans_to(src, target:amount_per_transfer_from_this)
							...
							if(target.reagents_to_log.len)
								...
								if(badshit.len)
									var
										hl = "<span class='danger'>([english_list(badshit)])</span>"
					...
					attackby(obj/item/weapon/W as obj, mob/user as mob, params)
						if(istype(W, /obj/item/weapon/pen) || istype(W, /obj/item/device/flashlight/pen))
							var
								tmp_label = sanitize(input(user, "Enter a label for [src.name]","Label",src.label_text))
					...
					baggie
						...
						update_icon()
							...
							if(reagents.total_volume)
								var
									image
										filling = image('icons/obj/reagentfillings.dmi', src, "[icon_state]2")
