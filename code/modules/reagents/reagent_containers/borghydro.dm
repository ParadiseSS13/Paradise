			...
			reagent_containers
				borghypo
					...
					var
						charge_tick = 0
					var
						recharge_time = 5 //Time it takes for shots to recharge (in seconds)
					var
						bypass_protection = 0 //If the hypospray can go through armor or thick material
					...
					var
						list
							reagent_ids = list("salglu_solution", "epinephrine", "spaceacillin", "charcoal")
					...
					proc
						add_reagent(var/reagent)
							...
							var
								datum
									reagents
										R = reagent_list[reagent_list.len]
						...
						if(R.total_volume && M.can_inject(user, 1, penetrate_thick = bypass_protection))
							...
							if(M.reagents)
								var
									datum
										reagent
											injected = chemical_reagents_list[reagent_ids[mode]]
								var
									contained = injected.name
								...
								var
									trans = R.trans_to(M, amount_per_transfer_from_this)
				...
				borghypo
					attack_self(mob/user as mob)
						...
						var
							datum
								reagent
									R = chemical_reagents_list[reagent_ids[mode]]
