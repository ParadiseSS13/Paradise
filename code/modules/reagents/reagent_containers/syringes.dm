		...
		if(SYRINGE_DRAW)
			...
			if(ismob(target))//Blood!
				if(istype(target, /mob/living/carbon/slime))
					user << "<span class='warning'>You are unable to locate any blood."
					return
				...
				if(istype(target, /mob/living/carbon))//maybe just add a blood reagent to all mobs. Then you can suck them dry...With hundreds of syringes. Jolly good idea.
					...
					if(!T.dna)
						usr << "<span class='warning'>You are unable to locate any blood. (To be specific, your target seems to be missing their DNA datum)</span>"
						return
					...
					if(target == user)
						...
						for(var/mob/O in viewers(world.view, user))
							O.show_message("<span class='danger'>[user] is trying to take a blood sample from [target]!</span>", 1)
		...
		if(SYRINGE_INJECT)
			...
			if(istype(target, /obj/item/clothing/mask/cigarette))
				...
				if(istype(C.loc, /obj/item/weapon/storage/fancy/cigarettes))
					user << "<span class='warning'>You cannot inject a cigarette while it's still in the pack.</span>"
					return
			...
			if(ismob(target) && target != user)
				...
				for(var/mob/O in viewers(world.view, user))
					if(time == 30)
						O.show_message("<span class='danger'>[user] is trying to inject [target]!</B></span>", 1)
					else
						O.show_message("<span class='danger'>[user] begins hunting for an injection port on [target]'s suit!</span>", 1)

				if(!do_mob(user, target, time)) return

				for(var/mob/O in viewers(world.view, user))
					O.show_message("<span class='warning'>[user] injects [target] with the syringe!</span>", 1)
				...
				if(istype(target,/mob/living))
					...
					M.attack_log += "\[[time_stamp()]\] <font color='orange'>Has been injected with [src.name] by [key_name(user)]. Reagents: [contained]</font>"
					user.attack_log += "\[[time_stamp()]\] <font color='red'>Used the [src.name] to inject [key_name(M)]. Reagents: [contained]</font>"
					if(M.ckey)
...
/obj/item/weapon/reagent_containers/syringe/proc/syringestab(mob/living/carbon/target as mob, mob/living/carbon/user as mob)
	...
	if(istype(target, /mob/living/carbon/human))
		...
		if (target != user && target.getarmor(target_zone, "melee") > 5 && prob(50))
			for(var/mob/O in viewers(world.view, user))
				O.show_message("<span class='danger'>[user] tries to stab [target] in \the [hit_area] with [src.name], but the attack is deflected by armor!</span>", 1)
			user.unEquip(src)
			qdel(src)
			return

		for(var/mob/O in viewers(world.view, user))
			O.show_message("<span class='danger'>[user] stabs [target] in \the [hit_area] with [src.name]!</span>", 1)
		...
		for(var/mob/O in viewers(world.view, user))
			O.show_message("<span class='warning'>[user] stabs [target] with [src.name]!</span>", 1)
		...
		if(SYRINGE_DRAW)
			...
			if(ismob(target))
				if(istype(target, /mob/living/carbon))//I Do not want it to suck 50 units out of people
					usr << "<span class='notice'>This needle isn't designed for drawing blood.</span>"
					return
			else //if not mob
				if(!target.reagents.total_volume)
					user << "<span class='warning'>[target] is empty.</span>"
				...
				user << "<span class='notice'>You fill the syringe with [trans] units of the solution.</notice>"
			if (reagents.total_volume >= reagents.maximum_volume)
				mode=!mode
				update_icon()
...
/obj/item/weapon/reagent_containers/ld50_syringe/lethal/New()
	..()
...
/obj/item/weapon/reagent_containers/syringe/robot/epinephrine/New()
	..()
...
/obj/item/weapon/reagent_containers/syringe/epinephrine/New()
	..()
	reagents.add_reagent("epinephrine", 15)
	mode = SYRINGE_INJECT
	update_icon()
...
/obj/item/weapon/reagent_containers/syringe/insulin/New()
	..()
...
/obj/item/weapon/reagent_containers/syringe/bioterror/New()
	..()
