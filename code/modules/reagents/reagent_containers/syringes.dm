		...
		if(SYRINGE_DRAW)
			...
			if(ismob(target))//Blood!
				...
				if(istype(target, /mob/living/carbon))//maybe just add a blood reagent to all mobs. Then you can suck them dry...With hundreds of syringes. Jolly good idea.
					...
					var
						amount = src.reagents.maximum_volume - src.reagents.total_volume
					if(amount == 0)
				...
				var
					trans = target.reagents.trans_to(src, amount_per_transfer_from_this) // transfer from, transfer to - who cares?
				...
				syringe
					update_icon()
						if(mode == SYRINGE_BROKEN)
							...
							return
						var
							rounded_vol = round(reagents.total_volume,5)
						overlays.Cut()
						if(ismob(loc))
							var
								injoverlay
	...
	item
		weapon
			reagent_containers
				syringe
					proc
						syringestab(mob/living/carbon/target as mob, mob/living/carbon/user as mob)
							...
							if(istype(target, /mob/living/carbon/human))
								...
								var
									target_zone = ran_zone(check_zone(user.zone_sel.selecting, target))
								var
									obj
										item
											organ
												external
													affecting = target:get_organ(target_zone)
							...
							src.reagents.reaction(target, INGEST)
							var
								syringestab_amount_transferred = rand(0, (reagents.total_volume - 5)) //nerfed by popular demand
		...
		weapon
			reagent_containers
				ld50_syringe
					...
					update_icon()
						var
							rounded_vol = round(reagents.total_volume,50)
						if(ismob(loc))
							var
								mode_t
							switch(mode)
