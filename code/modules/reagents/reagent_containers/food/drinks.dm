...
/obj/item/weapon/reagent_containers/food/drinks
	...
	attack(mob/M as mob, mob/user as mob, def_zone)
		...
		if(M == user)
			...
			log_attack("[key_name_admin(user)](<A HREF='?_src_=holder;adminmoreinfo=\ref[user]'>?</A>) ([admin_jump_link(user, "holder")]) fed [key_name_admin(M)] with [src.name] Reagents: [reagentlist(src)] (INTENT: [uppertext(user.a_intent)])")
			if(!iscarbon(user))
				M.LAssailant = null
			else
				M.LAssailant = user
