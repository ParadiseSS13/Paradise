					...
					condiment
						on_reagent_change()
							...
							if(reagents.reagent_list.len > 0)
								var
									main_reagent = reagents.get_master_reagent_id()
								if(main_reagent in possible_states)
									var
										list
											temp_list = possible_states[main_reagent]
		...
		weapon
			reagent_containers
				food
					condiment
						pack
							attack(mob/M as mob, mob/user as mob, def_zone) //Can't feed these to people directly.
								return
