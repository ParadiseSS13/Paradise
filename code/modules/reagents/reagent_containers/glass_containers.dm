...
/obj/item/weapon/reagent_containers/glass
	...
	afterattack(obj/target, mob/user, proximity)
		...
		if(ismob(target) && target.reagents && reagents.total_volume)
			...
			if(M.ckey)
				msg_admin_attack("[key_name_admin(user)](<A HREF='?_src_=holder;adminmoreinfo=\ref[user]'>?</A>) ([admin_jump_link(user, "holder")]) splashed [key_name_admin(M)] with [src.name]. Reagents: [contained] (INTENT: [uppertext(user.a_intent)])")
			if(!iscarbon(user))
				M.LAssailant = null
			else
				M.LAssailant = user
