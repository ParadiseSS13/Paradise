...
obj
	vehicle
		...
		buckle_lying = 0
		var
			keytype = null //item typepath, if non-null an item of this type is needed in your hands to drive this vehicle
		var
			next_vehicle_move = 0 //used for move delays
		var
			vehicle_move_delay = 2 //tick delay between movements, lower = faster, higher = slower
		var
			auto_door_open = TRUE
		var
			needs_gravity = 0//To allow non-space vehicles to move in no gravity or not, mostly for adminbus
		//Pixels
		var
			generic_pixel_x = 0 //All dirs show this pixel_x for the driver
		var
			generic_pixel_y = 0 //All dirs shwo this pixel_y for the driver
...
obj
	vehicle
		proc
			RunOver(var/mob/living/carbon/human/H)
				return		//write specifics for different vehicles


obj
	vehicle
		Process_Spacemove(direction)
			if(has_gravity(src))
				return 1
