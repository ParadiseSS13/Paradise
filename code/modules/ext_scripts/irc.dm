...
/hook/startup/proc/ircNotify()
	var/people_to_ping = ""
	var/datum/DBQuery/pull_notify = SSdbcore.NewQuery("SELECT discord_id FROM [format_table_name("discord")] WHERE notify = 1")
	...
	var/datum/DBQuery/reset_notify = SSdbcore.NewQuery("UPDATE [format_table_name("discord")] SET notify = 0 WHERE notify = 1")
