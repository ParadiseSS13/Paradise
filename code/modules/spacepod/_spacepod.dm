...
/obj/vehicle/sealed/spacepod
	...
	max_occupants = 1
	max_drivers = 1
	var/obj/item/storage/cargo_hold
...
/obj/vehicle/sealed/spacepod/update_icon()
	...
	if(LAZYLEN(equipment) && equipment[POD_EQUIPMENT_WEAPON])
		var/obj/item/spacepod_equipment/weaponry/weapon = equipment[POD_EQUIPMENT_WEAPON]
...
/obj/vehicle/sealed/spacepod/examine(mob/user)
	...
	if(LAZYLEN(equipment))
		...
		for(var/obj/item/spacepod_equipment/ME in equipment)
...
/obj/vehicle/sealed/spacepod/attack_hand(mob/user as mob)
	...
	if(!hatch_open && S.max_items > 0 && unlocked)
		S.show_to(user)
	else
...
/obj/vehicle/sealed/spacepod/attackby(obj/item/W as obj, mob/user as mob, params)
	...
	if(user.a_intent == INTENT_HARM)
		...
		if(istype(W, /obj/item/weldingtool))
			...
			if(obj_integrity < max_integrity)
				...
				if(do_after(user, 20 * W.toolspeed, target = src))
					if(!src || !WT.use_tool(src,user,3)) return
					repair_damage(10)
		...
		if(istype(W, /obj/item/spacepod_equipment))
			...
			return
		if(istype(W, /obj/item/spacepod_key) && istype(equipment[POD_EQUIPMENT_LOCK], /obj/item/spacepod_equipment/lock/keyed))
			var/obj/item/spacepod_key/key = W
			var/obj/item/spacepod_equipment/lock/keyed/LK = equipment[POD_EQUIPMENT_LOCK]
			if(key.id == LK.id)
				unlocked = !unlocked
				...
				return
		if(istype(equipment[POD_EQUIPMENT_LOCK], /obj/item/spacepod_equipment/lock/keyed) && istype(W, /obj/item/lock_buster))
...
/obj/vehicle/sealed/spacepod/driver_move(mob/user, direction)
	if(world.time < next_move)
		return FALSE
	var/moveship = TRUE
	var/obj/item/spacepod_equipment/thruster/TR = GetThruster()
	if(!TR)
	...
	if(cell && cell.charge >= TR.power_on_move && obj_integrity > 0 && empcounter == 0)
		...
		if(moveship)
			...
			var/scrape = FALSE
			for(var/turf/T in locs)
				for(var/obj/item/spacepod_equipment/cargo/C in GetAllEquipment())
...
/obj/vehicle/sealed/spacepod/process()
	...
	if(cell)
		...
		if(!has_power_been_lost && cell.charge < 1)
			...
			losepower = FALSE
		for(var/obj/item/spacepod_equipment/SE in equipment)
			if(SE.power_use)
...
/obj/vehicle/sealed/spacepod/proc/add_equipment(mob/user, obj/item/spacepod_equipment/E)
	if(!E || !(E.slot || E.size))
		return
	LAZYINITLIST(equipment)
	...
	E.forceMove(src)
	S.max_items += E.storage_mod["slots"]
	S.max_combined_w_class += E.storage_mod["w_class"]
...
/obj/vehicle/sealed/spacepod/proc/remove_equipment(mob/user)
	...
	if(istype(what_to_remove, /obj/item/stock_parts/cell))
		...
		cell = null
	else if(istype(equipment[what_to_remove], /obj/item/spacepod_equipment)) //this happens for "special slots" that don't contribute to total size, like weaponry
		var/obj/item/spacepod_equipment/SE = equipment[what_to_remove]
		if(!SE)
			return
		...
		if(cargo_hold)
			S.max_items -= SE.storage_mod["slots"]
			S.max_combined_w_class -= SE.storage_mod["w_class"]
	...
	else if(istype(what_to_remove, /obj/item/spacepod_equipment))
		var/obj/item/spacepod_equipment/SE = what_to_remove
		if(!SE)
			return
		user.visible_message("<span class='notice'>[user] removes \the [SE] from \the [src].</span>", "<span class='notice'>You remove \the [SE] from \the [src].</span>")
		SE.forceMove(user.loc)
...
/obj/vehicle/sealed/spacepod/proc/GetThruster()
	if(!istype(equipment[POD_EQUIPMENT_THRUSTER], /obj/item/spacepod_equipment/thruster))
		return FALSE
...
/obj/vehicle/sealed/spacepod/proc/calculate_equipment_size()
	. = 0
	for(var/A in equipment)
		var/obj/item/spacepod_equipment/SE = A
		if(SE.slot)
			continue
		. += SE.size
...
/obj/vehicle/sealed/spacepod/proc/unload(mob/user)
	if(user.incapacitated())
		return
	for(var/obj/item/spacepod_equipment/cargo/C in equipment)
		C.unload()
