...
var
	global
		list
			ghdels_profiled = list()

proc
	delete_profile(var/datum/D)
...
proc
	delete_profiler_report()
		var
			dat = "<html><head><title>Deletion Profiler Report</title></head>"
...
proc
	delete_profiler_sortedlist(var/list/L, var/header, var/anchorid)
		L = L.Copy()
		...
		var
			dat = "<table border='1' id='[anchorid]'><tr><th colspan='2'>[header]</th></tr>"
		for(var/t in L)
...
datum
	verb
		find_references(remove_from_queue = TRUE as num)
			set category = "Debug"
...
client
	verb
		purge_all_destroyed_objects()
			set category = "Debug"
			if(garbageCollector)
				while(garbageCollector.queue.len)
					var
						datum
							o = locate(garbageCollector.queue[1])
