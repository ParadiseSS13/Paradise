...
obj
	machinery
		fishtank
			...
			var
				tank_type = ""			// Type of aquarium, used for icon updating
			var
				water_capacity = 0		// Number of units the tank holds (varies with tank type)
			var
				water_level = 0			// Number of units currently in the tank (new tanks start empty)
			var
				light_switch = 0		// 0 = off, 1 = on (off by default)
			var
				filth_level = 0.0		// How dirty the tank is (max 10)
			var
				lid_switch = 0			// 0 = open, 1 = closed (open by default)
			var
				max_fish = 0			// How many fish the tank can support (varies with tank type, 1 fish per 50 units sounds reasonable)
			var
				food_level = 0			// Amount of fishfood floating in the tank (max 10)
			var
				fish_count = 0			// Number of fish in the tank
			var
				list
					fish_list = null	// Tracks the current types of fish in the tank
			var
				egg_count = 0			// How many fish eggs can be harvested from the tank (capped at the max_fish value)
			var
				list
					egg_list = null	// Tracks the current types of harvestable eggs in the tank

			var
				has_lid = 0				// 0 if the tank doesn't have a lid/light, 1 if it does
			var
				max_health = 0			// Can handle a couple hits
			var
				cur_health = 0			// Current health, starts at max_health
			var
				leaking = 0				// 0 if not leaking, 1 if minor leak, 2 if major leak (not leaking by default)
			var
				shard_count = 0			// Number of glass shards to salvage when broken (1 less than the number of sheets to build the tank)
...
obj
	machinery
		fishtank
			proc
				toggle_lid(var/mob/living/user)
...
obj
	machinery
		fishtank
			proc
				toggle_light(var/mob/living/user)
	...
	machinery
		fishtank
			update_icon()
				...
				var
					water_type = "_clean"							//Default to clean water
	...
	machinery
		fishtank
			proc
				select_egg_type()
					var
						fish = pick(fish_list)						//Select a fish from the fish in the tank
					...
					var
						obj
							item
								fish_eggs
									egg_path	= null			//Create empty variable to receive the egg_path
					...
					while(egg_count > 0)							//Loop until you've harvested all the eggs
						var
							obj
								item
									fish_eggs
										egg = pick(egg_list)	//Select an egg at random
...
obj
	machinery
		fishtank
			proc
				destroy(var/deconstruct = 0)
					var
						turf
							T = get_turf(src)										//Store the tank's turf for atmos updating after deletion of tank
					if(!deconstruct)												//Check if we are deconstructing or breaking the tank
						var
							shards_left = shard_count
						...
						var
							sheets = shard_count + 1								//Deconstructing it salvages all the glass used to build the tank
...
obj
	machinery
		fishtank
			attack_slime(mob/living/user as mob)
				var
					mob
						living
							carbon
...
obj
	machinery
		fishtank
			proc
				hit(var/damage, var/sound_effect = 1)
...
obj
	machinery
		fishtank
			proc
				attack_generic(mob/living/user as mob, damage = 0)	//used by attack_alien, attack_animal, and attack_slime
					user.changeNext_move(CLICK_CD_MELEE)
...
obj
	machinery
		fishtank
			attackby(var/obj/item/O, var/mob/user as mob)
