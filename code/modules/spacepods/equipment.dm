					...
					proc
						fire_weapons()
							...
							for(var/i = 0; i < shots_per; i++)
								...
								olddir = dir
								var
									proj_type = text2path(projectile_type)
								var
									obj
										item
											projectile
												projone = new proj_type(firstloc)
								var
									obj
										item
											projectile
												projtwo = new proj_type(secondloc)
				...
				list
					obj
						item
							device
								spacepod_equipment
									installed_modules = list() // holds an easy to access list of installed modules
				...
				obj
					item
						device
							spacepod_equipment
								cargo
									sec_cargo_system // secondary cargo system
			var
				obj
					item
						device
				...
				var
					obj
						spacepod
							my_atom
				var
					occupant_mod = 0	// so any module can modify occupancy
				var
					list
						storage_mod = list("slots" = 0, "w_class" = 0)		// so any module can modify storage slots
...
obj
	item
		device
			spacepod_equipment
				proc
					removed(var/mob/user) // So that you can unload cargo when you remove the module
						return
...
obj
	item
		device
			spacepod_equipment
				cargo
					removed(var/mob/user) // called when system removed
						. = ..()
						unload()
