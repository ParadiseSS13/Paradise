...
obj
	spacepod
		...
		var
			list
				mob
					pilot	//There is only ever one pilot and he gets all the privledge
		var
			list
				mob
					passengers = list() //passengers can't do anything and are variable in number
		var
			max_passengers = 0
		var
			obj
		...
		var
			battery_type = "/obj/item/weapon/stock_parts/cell/high"
		var
			obj
				item
					weapon
		...
		var
			health = 250
		var
			empcounter = 0 //Used for disabling movement when hit by an EMP
		...
		var
			list
				icon_light_color = list("pod_civ" = LIGHT_COLOR_WHITE, \
													"pod_mil" = "#BBF093", \
...
obj
	spacepod
		proc
			deal_damage(var/damage)
				var
					oldhealth = health
				health = max(0, health - damage)
				var
					percentage = (health / initial(health)) * 100
				occupant_sanity_check()
				if(passengers && oldhealth > health && percentage <= 25 && percentage > 0)
					var
						sound
							S = sound('sound/effects/engine_alert2.ogg')
					S.wait = 0 //No queue
				...
				if(passengers && oldhealth > health && !health)
					var
						sound
							S = sound('sound/effects/engine_alert1.ogg')
					S.wait = 0
					S.channel = 0
					S.volume = 50
					for(var/mob/M in passengers)
...
obj
	spacepod
		proc
			repair_damage(var/repair_amount)
				if(health)
...
obj
	spacepod
		proc
			add_equipment(mob/user, var/obj/item/device/spacepod_equipment/SPE, var/slot)
					...
					var
						obj
							item
								device
									spacepod_equipment
										system = equipment_system.vars[slot]
					system.my_atom = src
...
obj
	spacepod
		proc
			remove_equipment(mob/user, var/obj/item/device/spacepod_equipment/SPE, var/slot)
...
obj
	spacepod
		hear_talk
			hear_talk(mob/M, var/msg)
				cargo_hold.hear_talk(M, msg)
				..()

obj
	spacepod
		hear_message(mob/M, var/msg)
						...
						item
							device
								spacepod_equipment
									weaponry
										burst_taser
											T = new /obj/item/device/spacepod_equipment/weaponry/taser
						...
						item
							device
								spacepod_equipment
									misc
										tracker
											L = new /obj/item/device/spacepod_equipment/misc/tracker
						...
						item
							device
								spacepod_equipment
									sec_cargo
										chair
											C = new /obj/item/device/spacepod_equipment/sec_cargo/chair
						...
						item
							device
								spacepod_equipment
									lock
										keyed
											K = new /obj/item/device/spacepod_equipment/lock/keyed
...
obj
	spacepod
		proc
			moved_other_inside(var/mob/living/carbon/human/H as mob)
				occupant_sanity_check()
						...
						item
							device
								spacepod_equipment
									cargo
										ore
											C = equipment_system.cargo_system
				if(!C.storage)
	...
	spacepod
		proc
			enter_pod(mob/user)
				...
				var
					fukkendisk = user.GetTypeInAllContents(/obj/item/weapon/disk/nuclear)
...
obj
	spacepod
		proc
			occupant_sanity_check()  // going to have to adjust this later for cargo refactor
				if(passengers)
					if(passengers.len > max_passengers)
						for(var/i = passengers.len; i <= max_passengers; i--)
							var
								mob
									occupant = passengers[i - 1]
		...
		verb
			checkSeat()
				...
				if(do_after(user, 40, target = src))
					var
						obj
							badlist = list(internal_tank, cargo_hold, pilot, battery) + passengers + equipment_system.installed_modules
					var
						list
							true_contents = contents - badlist
					if(true_contents.len > 0)
						var
							obj
								I = pick(true_contents)
...
datum
	global_iterator
		pod_preserve_temp  //normalizing cabin air temperature to 20 degrees celsium
			delay = 20

			process(var/obj/spacepod/spacepod)
				if(spacepod.cabin_air && spacepod.cabin_air.return_volume() > 0)
					var
						delta = spacepod.cabin_air.temperature - T20C
			...
			process(var/obj/spacepod/spacepod)
				if(spacepod && spacepod.internal_tank)
					var
						datum
							gas_mixture
								tank_air = spacepod.internal_tank.return_air()
					var
						datum
							gas_mixture
								cabin_air = spacepod.cabin_air
...
obj
	spacepod
		proc
			handlerelaymove(mob/user, direction)
