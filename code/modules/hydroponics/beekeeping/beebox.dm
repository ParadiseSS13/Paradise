...
obj
	structure
		beebox
			...
			var
				list
					bees = list() //bees owned by the box, not those inside it
			var
				list
					honeycombs = list()
			var
			...
			process()
				if(queen_bee)
					...
					if(bees.len < get_max_bees())
						var
							freebee = FALSE //a freebee, geddit?, hahaha HAHAHAHA
			...
			attack_hand(mob/user)
				if(ishuman(user))
					if(!user.bee_friendly())
						...
						var
							option = input(user, "What Action do you wish to perform?", "Apiary") as null|anything in list("Remove a Honey Frame","Remove the Queen Bee")
