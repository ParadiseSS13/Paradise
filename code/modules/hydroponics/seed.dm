datum
	plantgene
		var
			genetype    // Label used when applying trait.
		var
			list
...
datum
	seed
		...
		var
			name						// Index for global list.
		var
			seed_name					// Plant name for seed packet.
		var
			seed_noun = "seeds"			// Descriptor for packet.
		var
			display_name				// Prettier name.
		var
			base_name					// Unchanging name for use with modified versions
		var
			roundstart					// If set, seed will not display variety number.
		var
			mysterious					// Only used for the random seed packets.
		var
			can_self_harvest = 0		// Mostly used for living mobs.
		var
			growth_stages = 0			// Number of stages the plant passes through before it is mature.
		var
			list
				traits = list()		// Initialized in New()
		var
			list
				mutants				// Possible predefined mutant varieties, if any.
		var
			list
				chems					// Chemicals that plant produces in products/injects into victim.
		var
			list
				consume_gasses			// The plant will absorb these gasses during its life.
		var
			list
				exude_gasses			// The plant will exude these gasses during its life.
		var
			kitchen_tag					// Used by the reagent grinder.
		var
			trash_type					// Garbage item produced when eaten.
		var
			splat_type = /obj/effect/decal/cleanable/fruit_smudge // Graffiti decal.
		var
			preset_product
		var
			final_form = 1
		var
			last_diverge_type = -1		// Used to check if we need to change our name prefix when we are mutated/modified/enhanced
		var
			modular_icon = 0			// Dictates if the product uses a modular sprite. 0 = preset, 1 = modular
		var
			preset_icon = "undef"		// Name of the iconstate in icon/obj/harvest.dmi to use for preset sprite
...
datum
	seed
		proc
			set_trait(var/trait,var/nval,var/ubound,var/lbound, var/degrade)
...
datum
	seed
		proc
			create_spores(var/turf/T, var/obj/item/thrown)
				if(!T)
					return
				if(!istype(T))
					T = get_turf(T)
				...
				if(chems.len)
					for(var/rid in chems)
						var
							injecting = min(5,max(1,get_trait(TRAIT_POTENCY)/3))
...
datum
	seed
		proc
			do_thorns(var/mob/living/carbon/human/target, var/obj/item/fruit, var/target_limb)
				...
				var
					obj
						item
							organ
								external
									affecting = target.get_organ(target_limb)
				var
					damage = 0
...
datum
	seed
		proc
			do_sting(var/mob/living/carbon/human/target, var/obj/item/fruit, var/target_limb)
				...
				if(chems && chems.len)
					...
					for(var/rid in chems)
						var
							injecting = min(5,max(1,get_trait(TRAIT_POTENCY)/5))
...
datum
	seed
		proc
			splatter(var/turf/T,var/obj/item/thrown)
				if(splat_type)
					var
						obj
							effect
								plant
									splat = new splat_type(T, src)
...
datum
	seed
		proc
			thrown_at(var/obj/item/thrown,var/atom/target, var/force_explode)

				var
					splatted
				var
					turf
						origin_turf = get_turf(target)
				...
				if(force_explode || get_trait(TRAIT_EXPLOSIVE))
					...
					var
						flood_dist = min(10,max(1,get_trait(TRAIT_POTENCY)/15))
					var
						list
							open_turfs = list()
					var
...
datum
	seed
		proc
			handle_environment(var/turf/current_turf, var/datum/gas_mixture/environment, var/light_supplied, var/obj/item/weapon/tank/holding, var/check_only)
				...
				if(!light_supplied)
					var
						atom
							movable
								lighting_overlay
									L = locate(/atom/movable/lighting_overlay) in current_turf
					if(L)
...
datum
	seed
		proc
			adjust_gas(var/datum/gas_mixture/environment, var/gas, var/amount = 0)
				if(!environment || !gas)	//no gas_mixture or gas defined to adjust
					return

				var
					transfer_moles = environment.total_moles()
				var
					datum
						gas_mixture
							temp_holding
...
datum
	seed
		proc
			apply_special_effect(var/mob/living/target,var/obj/item/thrown)
				...
				if(get_trait(TRAIT_TELEPORTING))
					...
					var
						outer_teleport_radius = get_trait(TRAIT_POTENCY)/5
					var
						inner_teleport_radius = get_trait(TRAIT_POTENCY)/15
						...
						var
							datum
								effect
									system
										spark_spread
											s = new /datum/effect/system/spark_spread
						s.set_up(3, 1, get_turf(target))
						s.start()
						var
							turf
								picked = get_turf(pick(turfs))                      // Just in case...
						new
							obj
								effect
									decal
										cleanable
											molten_item(get_turf(target)) // Leave a pile of goo behind for dramatic effect...
		...
		proc
			randomize()
				...
				if(prob(5))
					consume_gasses = list()
					var
						gas = pick("oxygen","nitrogen","toxins","carbon_dioxide")
				...
				var
					additional_chems = rand(0,5)

				if(additional_chems)
					for(var/x=1;x<=additional_chems;x++)
						var
							new_chem = get_random_chemical(1)
...
datum
	seed
		proc
			mutate(var/degree,var/turf/source_turf)
				...
				var
					total_mutations = rand(1,1+degree)
...
datum
	seed
		proc
			apply_gene(var/datum/plantgene/gene)
				...
				switch(gene.genetype)
					if(GENE_BIOCHEMISTRY)
						...
						var
							list
								gene_value = gene.values["[TRAIT_CHEMS]"]
						...
						var
							list
								new_gasses = gene.values["[TRAIT_EXUDE_GASSES]"]
					...
					if(GENE_DIET)
						var
							list
								new_gasses = gene.values["[TRAIT_CONSUME_GASSES]"]
...
datum
	seed
		proc
			harvest(var/mob/user,var/yield_mod,var/harvest_sample,var/force_amount)
...
datum
	seed
		proc
			update_name_prefixes(var/modified = 0)
