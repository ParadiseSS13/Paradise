							...
							proc
								update_desc()
									...
									if(plant_controller.product_descs["[seed.uid]"])
										...
										var
											descriptor_num = rand(2,4)
										var
											descriptor_count = descriptor_num
										desc = "A"
										while(descriptors.len && descriptor_num > 0)
											var
												chosen = pick(descriptors)
											descriptors -= chosen
							...
							update_icon()
								...
								var
									image
										plant_icon
								var
									icon_key = "fruit-[seed.get_trait(TRAIT_PRODUCT_ICON)]-[seed.get_trait(TRAIT_PRODUCT_COLOUR)]-[seed.get_trait(TRAIT_PLANT_COLOUR)]"
								if(plant_controller.plant_icon_cache[icon_key])
									...
									var
										image
											fruit_base = image('icons/obj/hydroponics_products.dmi',"[seed.get_trait(TRAIT_PRODUCT_ICON)]-product")
									...
									if("[seed.get_trait(TRAIT_PRODUCT_ICON)]-leaf" in icon_states('icons/obj/hydroponics_products.dmi'))
										var
											image
												fruit_leaves = image('icons/obj/hydroponics_products.dmi',"[seed.get_trait(TRAIT_PRODUCT_ICON)]-leaf")
													...
													item
														weapon
															stock_parts
																cell
																	potato
																		pocell = new /obj/item/weapon/stock_parts/cell/potato(get_turf(user))
														...
														item
															clothing
																mask
																	cigarette
																		joint
																			J = new /obj/item/clothing/mask/cigarette/joint(user.loc)
															...
															clothing
																mask
																	cigarette
																		joint
																			deus
																				J = new /obj/item/clothing/mask/cigarette/joint/deus(user.loc)
												J.chem_volume = src.reagents.total_volume
												src.reagents.trans_to(J, J.chem_volume)
												qdel(W)
												user.put_in_active_hand(J)
														...
														item
															clothing
																mask
																	cigarette
																		handroll
																			J = new /obj/item/clothing/mask/cigarette/handroll(user.loc)
												J.chem_volume = src.reagents.total_volume
												src.reagents.trans_to(J, J.chem_volume)
												qdel(W)
												user.put_in_active_hand(J)
		...
		weapon
			reagent_containers
				food
					snacks
						grown
							attack(var/mob/living/carbon/M, var/mob/user, var/def_zone)
								if(awakening)
												...
												living
													carbon
														human
															H = M
										var
											hit = H.attacked_by(src, user, def_zone)
										if(hit && hitsound)
								...
								if(!seed.final_form)	//This isn't even my final form! (sorry, it had to be done)
									switch(seed.kitchen_tag)
													...
													item
														stack
															medical
																bruise_pack
																	comfrey
																		poultice = new /obj/item/stack/medical/bruise_pack/comfrey(user.loc)
													...
													item
														stack
															medical
																ointment
																	aloe
																		poultice = new /obj/item/stack/medical/ointment/aloe(user.loc)
												...
												obj
													item
														weapon
															grown
																sunflower
																	SF = new /obj/item/weapon/grown/sunflower(user.loc)
											user.unEquip(src)
												...
												obj
													item
														weapon
															grown
																novaflower
																	NF = new /obj/item/weapon/grown/novaflower(user.loc)
											if(prob(10))
												...
												obj
													item
														weapon
															grown
																nettle
																	nettle = new /obj/item/weapon/grown/nettle(user.loc)
													...
													item
														weapon
															grown
																nettle
																	death
																		DN = new /obj/item/weapon/grown/nettle/death(user.loc)
														...
														mob
															living
																simple_animal
																	hostile
																		killertomato
																			K = new /mob/living/simple_animal/hostile/killertomato(get_turf(src.loc))
													var
														endurance_value = seed.get_trait(TRAIT_ENDURANCE)
													K.maxHealth += round(endurance_value / 3)
													K.melee_damage_lower += round(potency / 10)
													K.melee_damage_upper += round(potency / 10)
													var
														production_value = seed.get_trait(TRAIT_PRODUCTION)
										...
										if("cashpod")
											...
											var
												value = round(seed.get_trait(TRAIT_POTENCY))
											user.unEquip(src)
											switch(value)
												if(0)
