...
/obj/item/scythe/tele/attack_self(mob/user)
	...
	if(extend)
		...
		slot_flags = SLOT_HUD_BACK	//won't fit on belt, but can be worn on belt when extended
		...
		slot_flags = SLOT_HUD_BELT	//can be worn on belt again, but no longer makes sense to wear on the back
		w_class = WEIGHT_CLASS_SMALL
		force = 3
		attack_verb = list("hit", "poked")
		hitsound = "swing_hit"
