...
datum
	controller
		plants
			...
			var
				plants_per_tick = PLANTS_PER_TICK
			var
				plant_tick_time = PLANT_TICK_TIME
			var
				list
					product_descs = list()         // Stores generated fruit descs.
			var
				list
					plant_queue = list()           // All queued plants.
			var
				list
					seeds = list()                 // All seed data stored here.
			var
				list
					gene_tag_list = list()        	// List of gene tags for xenobotany fun
			var
				list
					plant_icon_cache = list()      // Stores images of growth, fruits and seeds.
			var
				list
					plant_sprites = list()         // List of all harvested product sprites.
			var
				list
					plant_product_sprites = list() // List of all growth sprites plus number of growth stages.
			...
			proc
				setup()
					...
					for(var/icostate in icon_states('icons/obj/hydroponics_growing.dmi'))
						var
							split = findtext(icostate,"-")
						if(!split)
							// invalid icon_state
							continue

						var
							ikey = copytext(icostate,(split+1))
						if(ikey == "dead")
							// don't count dead icons
							continue
						ikey = text2num(ikey)
						var
							base = copytext(icostate,1,split)
					...
					var
						list
							plant_traits = ALL_GENES
					while(plant_traits && plant_traits.len)
						var
							gene_tag = pick(plant_traits)
...
datum
	controller
		plants
			proc
				create_random_seed(var/survive_on_station)
					var
						datum
							seed
								seed = new()
					...
					spawn(0)
						...
						while(1)
							if(!processing)
								...
								if(plant_queue.len)
									var
										target_to_process = min(plant_queue.len,plants_per_tick)
...
datum
	controller
		plants
			proc
				add_plant(var/obj/effect/plant/plant)
					plant_queue |= plant
...
datum
	controller
		plants
			proc
				remove_plant(var/obj/effect/plant/plant)
					plant_queue -= plant
