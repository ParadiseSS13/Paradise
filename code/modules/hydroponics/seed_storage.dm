datum
	seed_pile
		...
		var
			datum
				seed
					seed_type // Keeps track of what our seed is
		var
			list
				obj
					item
						seeds
							seeds = list() // Tracks actual objects contained in the pile
		var
			ID

datum
	seed_pile
		New(var/obj/item/seeds/O, var/ID)
			name = O.name
			amount = 1
			seed_type = O.seed
			seeds += O
...
obj
	machinery
		seed_storage
			...
			var
				initialized = 0 // Map-placed ones break if seeds are loaded right at the start of the round, so we do it on the first interaction
			var
				list
					datum
						seed_pile
			...
			var
				list
					scanner = list() // What properties we can view

obj
	machinery
		seed_storage
			random // This is mostly for testing, but I guess admins could spawn it
			...
			interact(mob/user as mob)
				...
				if(!initialized)
					for(var/typepath in starting_seeds)
						var
							amount = starting_seeds[typepath]
				...
				var
					dat = "<center><h1>Seed storage contents</h1></center>"
				if(piles.len == 0)
...
obj
	machinery
		seed_storage
			proc
				add(var/obj/item/seeds/O as obj)
					if(istype(O.loc, /mob))
						var
							mob
								user = O.loc
