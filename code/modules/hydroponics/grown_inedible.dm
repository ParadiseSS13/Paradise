				...
				New(newloc,planttype)
					...
					if(planttype)
						...
						var
							datum
								seed
									S = plant_controller.seeds[plantname]
						if(!S || !S.chems)
							return
					...
					pickup(mob/living/user)
						...
						if(ishuman(user))
							...
							if(H.gloves)
								return 0
							var
								organ = ((user.hand ? "l_":"r_") + "arm")
							var
								obj
									item
										organ
											external
												affecting = H.get_organ(organ)
...
obj
	item
		weapon
			grown
				nettle
					afterattack(atom/A as mob|obj, mob/user,proximity)
						if(!proximity) return
						if(force > 0)
	...
	item
		weapon
			grown
				nettle
					death
						afterattack(mob/living/carbon/M, mob/user)
...
obj
	item
		weapon
			grown
				novaflower
					pickup(mob/living/carbon/human/user as mob)
						if(!user.gloves)
