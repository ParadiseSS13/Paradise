				...
				update_icon()
					...
					if(!isnull(seed))
						...
						if(dead)
							...
							var
								image
									dead_overlay = plant_controller.plant_icon_cache["[ikey]"]
							if(!dead_overlay)
							...
							if(age >= seed.get_trait(TRAIT_MATURATION))
								...
								var
									maturation = seed.get_trait(TRAIT_MATURATION)/seed.growth_stages
								if(maturation < 1)
									maturation = 1
								overlay_stage = maturation ? max(1,round(age/maturation)) : 1
							var
								ikey = "[seed.get_trait(TRAIT_PLANT_ICON)]-[overlay_stage]"
							var
								image
							...
							if(harvest && overlay_stage == seed.growth_stages)
								...
								var
									image
										harvest_overlay = plant_controller.plant_icon_cache["product-[ikey]-[seed.get_trait(TRAIT_PLANT_COLOUR)]"]
								if(!harvest_overlay)
