				...
				proc
					process_reagents()
						...
						for(var/datum/reagent/R in reagents.reagent_list)
							...
							var
								reagent_total = reagents.get_reagent_amount(R.id)
							...
							var
								health_value = 0
							var
								production_stat_value = 0
							var
								potency_stat_value = 0
							...
							if(seed && !dead)
								...
								if(seed.get_trait(TRAIT_IMMUTABLE) <= 0)
									...
									if(production_stat_value && seed.get_trait(TRAIT_PRODUCTION) > 2)
										...
										var
											new_production = seed.get_trait(TRAIT_PRODUCTION) + (reagent_total * production_stat_value)
									...
									if(potency_stat_value)
										if(!isnull(plant_controller.seeds[seed.name]))	//This is so we don't affect plants in other trays unintentionally
											...
											seed.update_name_prefixes(2)
										var
											new_potency = seed.get_trait(TRAIT_POTENCY) + (reagent_total * potency_stat_value)
...
obj
	machinery
		portable_atmospherics
			hydroponics
				proc
					handle_mutagens(var/amount, var/min_amount, var/step_amount)
