	...
	item
		device
			analyzer
				plant_analyzer
					proc
						print_report(var/mob/living/user)
							if(!last_data)
							...
							var
								obj
									item
										weapon
											paper
												P = new /obj/item/weapon/paper(get_turf(src))
			...
			analyzer
				plant_analyzer
					afterattack(obj/target, mob/user, flag)
						...
						//--FalseIncarnate
						var
							IS_TRAY = 0			//used to track if the target is a hydroponics tray or soil. 1 if true, otherwise 0
						var
							tray_age			//age of the plant in the tray
						var
							tray_weed_level		//weed level of tray
						var
							tray_pest_level		//pest level of tray
						var
							tray_toxins			//toxicity of the tray
						var
							tray_yield_mod		//yield modifier of the tray
						var
							tray_mutation_mod	//mutation modifier of the tray
						//--FalseIncarnate
				...
				tele
					attack_self(mob/user as mob)
						...
						if(blood_overlay && (blood_DNA.len >= 1)) //updates blood overlay, if any
							...
							var
								icon
									I = new /icon(src.icon, src.icon_state)
