...
/obj/item/seeds/kudzu/proc/plant(mob/user)
	...
	message_admins("Kudzu planted by [key_name_admin(user)][ADMIN_QUE(user)] [ADMIN_FLW(user)] at ([T.x],[T.y],[T.z] - <A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[T.x];Y=[T.y];Z=[T.z]'>(JMP)</a>)",0,1)
