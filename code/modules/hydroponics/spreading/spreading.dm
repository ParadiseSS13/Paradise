...
proc
	spacevine_infestation(var/potency_min=70, var/potency_max=100, var/maturation_min=5, var/maturation_max=15)
			...
			var
				list
					turf
						simulated
							floor
								turfs = list() //list of all the empty floor turfs in the hallway areas
...
obj
	effect
		plant
			New(var/newloc, var/datum/seed/newseed, var/obj/effect/plant/newparent)
				..()
		...
		plant
			update_icon()
				...
				if(growth_type == 0 && !floor)
					...
					src.transform = M
				var
					icon_colour = seed.get_trait(TRAIT_PLANT_COLOUR)
				if(icon_colour)
					color = icon_colour
			...
			proc
				calc_dir(turf/location = loc)
					...
					for(var/wallDir in cardinal)
						var
							turf
								newTurf = get_step(location,wallDir)
						if(newTurf.density)
							direction |= wallDir
