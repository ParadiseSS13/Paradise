		...
		plant
			proc
				get_cardinal_neighbors()
					var
						list
							cardinal_neighbors = list()
					for(var/check_dir in cardinal)
						var
							turf
								simulated
									T = get_step(get_turf(src), check_dir)
						if(istype(T))
			...
			process()
				...
				*/
				...
				if(is_mature() && neighbors.len && prob(spread_chance))
					...
					var
						max_spread = between(1, round(seed.get_trait(TRAIT_YIELD)*3/14), 3)
					...
					for(var/i in 1 to max_spread)
						if(prob(spread_chance))
							...
							var
								turf
									target_turf = pick(neighbors)
							var
								obj
									effect
										plant
											child = new(get_turf(src),seed,parent)
