			...
			disk
				botany
					attack_self(var/mob/user as mob)
						if(genes.len)
							var
								choice = alert(user, "Are you sure you want to wipe the disk?", "Xenobotany Data", "No", "Yes")
		...
		botany
			...
			var
				obj
					item
						seeds
							seed // Currently loaded seed packet.
			var
				obj
					item
						weapon
							disk
								botany
									loaded_disk //Currently loaded data disk.
		...
		botany
			extractor
				...
				var
					datum
						seed
							genetics // Currently scanned seed genetic structure.
				var
					degradation = 0     // Increments with each scan, stops allowing gene mods after a certain point.
				...
				Topic(href, href_list)
					...
					if(href_list["get_gene"])
						...
						var
							datum
								plantgene
									P = genetics.get_gene(href_list["get_gene"])
						if(!P) return
						loaded_disk.genes += P
