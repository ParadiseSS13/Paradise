...
/obj/machinery/atmospherics/supermatter_crystal/proc/countdown()
	...
	for(var/i in SUPERMATTER_COUNTDOWN_TIME to 0 step -10)
		...
		if(damage < explosion_point) // Cutting it a bit close there engineers
			radio.autosay("<span class='big'>[safe_alert] Failsafe has been disengaged.</span>", name, null, list(z))
			cut_overlay(causality_field, TRUE)
			final_countdown = FALSE
	...
	if(damage > warning_point) // while the core is still damaged and it's still worth noting its status
		if((REALTIMEOFDAY - lastwarning) / 10 >= WARNING_DELAY)
			...
			if(damage > emergency_point)
				radio.autosay("<span class='big'>[emergency_alert] Integrity: [get_integrity()]%</span>", name, null, list(z))
				...
				radio.autosay("<b>[warning_alert] Integrity: [get_integrity()]%</b>", name, "Engineering", list(z))
				lastwarning = REALTIMEOFDAY - (WARNING_DELAY * 5)

			else                                                 // Phew, we're safe
				radio.autosay("<b>[safe_alert] Integrity: [get_integrity()]%</b>", name, "Engineering", list(z))
				lastwarning = REALTIMEOFDAY

			if(power > POWER_PENALTY_THRESHOLD)
				radio.autosay("<b>Warning: Hyperstructure has reached dangerous power level.</b>", name, "Engineering", list(z))
				if(powerloss_inhibitor < 0.5)
					radio.autosay("<b>DANGER: CHARGE INERTIA CHAIN REACTION IN PROGRESS.</b>", name, "Engineering", list(z))

			if(combined_gas > MOLE_CRUNCH_THRESHOLD)
				radio.autosay("<b>Warning: Critical coolant mass reached.</b>", name, "Engineering", list(z))
		//Boom (Mind blown)
