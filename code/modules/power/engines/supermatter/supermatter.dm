...
/obj/machinery/atmospherics/supermatter_crystal/proc/process_atmos_safely(turf/T, datum/gas_mixture/env)
	...
	if(power > POWER_PENALTY_THRESHOLD || damage > damage_penalty_point || combined_gas > MOLE_CRUNCH_THRESHOLD) //If the power is above 5000, if the damage is above 550, or mole crushing
		...
		if((power + mole_crunch_bonus) > SEVERE_POWER_PENALTY_THRESHOLD && prob(5) || prob(1))
			supermatter_anomaly_gen(src, GRAVITATIONAL_ANOMALY, rand(5, 10))
		if(((power + mole_crunch_bonus) > SEVERE_POWER_PENALTY_THRESHOLD && prob(2)) || (prob(0.3) && (power + mole_crunch_bonus) > POWER_PENALTY_THRESHOLD))
