		...
		power
			apc
				...
				var
					cell_type = 2500
				var
					opened = 0 //0=closed, 1=opened, 2=cover removed
				var
					shorted = 0
				var
					lighting = 3
				...
				var
					malfhack = 0 //New var for my changes to AI malf. --NeoFite
				var
					mob
						living
							silicon
								ai
									malfai = null //See above --NeoFite
				var
					debug= 0
				var
					autoflag= 0		// 0 = off, 1= eqp and lights off, 2 = eqp off, 3 = all on.
//	luminosity = 1
	var
		has_electronics = 0 // 0 - none, 1 - plugged in, 2 - secured by screwdriver
	var
		overload = 1 //used for the Blackout malf module
	var
		beenhit = 0 // used for counting how many times it has been hit, used for Aliens at the moment
	var
		mob
			living
				silicon
	...
	var
		global
			list
				status_overlays_lock
	var
		global
			list
				status_overlays_charging
	var
		global
			list
				status_overlays_equipment
	var
		global
			list
				status_overlays_lighting
	var
		global
			list
				status_overlays_environ
	var
		indestructible = 0 // If set, prevents aliens from destroying it

	var
		report_power_alarm = 1

	var
		shock_proof = 0 //if set to 1, this APC will not arc bolts of electricity if it's overloaded.
		...
		power
			apc
				update_icon()
					...
					var
						update = check_updates() 		//returns 0 if no need to update icons.
...
obj
	machinery
		power
			apc
				proc
					can_use(var/mob/user, var/loud = 0) //used by attack_hand() and Topic()
...
obj
	machinery
		power
			apc
				proc
					is_authenticated(mob/user as mob)
...
obj
	machinery
		power
			apc
				Topic(href, href_list, var/usingUI = 1)
					if(..())
						return 1
...
obj
	machinery
		power
			apc
				proc
					malfoccupy(var/mob/living/silicon/ai/malf)
						if(!istype(malf))
							return
					...
					if(cell && !shorted)
						...
						if(src.attempt_charging())
							if(excess > 0)		// check to make sure we have enough to charge
								...
								var
									ch = min(excess*CELLRATE, cell.maxcharge*CHARGELEVEL)
...
obj
	machinery
		power
			apc
				proc
					overload_lighting(var/chance = 100)
