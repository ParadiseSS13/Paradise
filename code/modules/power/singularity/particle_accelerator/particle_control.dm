...
/obj/machinery/particle_accelerator/control_box/proc/add_strength(var/s)
	if(assembled)
		...
		if(strength > strength_upper_limit)
			...
			message_admins("PA Control Computer increased to [strength] by [key_name_admin(usr)] in ([x],[y],[z] - <A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[x];Y=[y];Z=[z]'>JMP</a>)", areaMaster)
...
/obj/machinery/particle_accelerator/control_box/proc/remove_strength(var/s)
	if(assembled)
		...
		if(strength < 0)
			...
			message_admins("PA Control Computer decreased to [strength] by [key_name_admin(usr)] in ([x],[y],[z] - <A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[x];Y=[y];Z=[z]'>JMP</a>)", areaMaster)
...
/obj/machinery/particle_accelerator/control_box/proc/toggle_power()
	...
	if(active)
		msg_admin_attack("PA Control Computer turned ON by [key_name_admin(usr)]", ATKLOG_FEW, areaMaster)
