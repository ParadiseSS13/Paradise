	...
	machinery
		field
			generator
				...
				var
					const
						num_power_levels = 6	// Total number of power level icon has
				var
					Varedit_start = 0
				var
					Varpower = 0
		...
		field
			generator
				update_icon()
					...
					var
						level = round(num_power_levels * power / field_generator_max_power)
...
obj
	machinery
		field
			generator
				proc
					draw_power(draw = 0, failsafe = 0, obj/machinery/field/generator/G = null, obj/machinery/field/generator/last = null)
						if(Varpower)
							return 1
				...
				proc
					setup_field(NSEW)
						...
						for(var/dist = 0, dist < steps, dist += 1) // creates each field tile
							var
								field_dir = get_dir(T,get_step(G.loc, NSEW))
							T = get_step(T, NSEW)
								...
								var
									obj
										machinery
											field
												containment
													CF = new/obj/machinery/field/containment()
								CF.set_master(src,G)
								fields += CF
								G.fields += CF
								CF.loc = T
