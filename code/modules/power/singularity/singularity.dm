...
obj
	singularity
		...
		var
			last_warning
		var
			consumedSupermatter = 0 //If the singularity has eaten a supermatter shard and can go to stage six
		allow_spin = 0
...
obj
	singularity
		Process_Spacemove() //The singularity stops drifting for no man!
			return 0
	...
	singularity
		proc
			admin_investigate_setup()
				...
				var
					count = locate(/obj/machinery/field/containment) in orange(30, src)
	...
	singularity
		proc
			expand(var/force_size = 0)
				var
					temp_allowed_size = src.allowed_size
				if(force_size)
...
obj
	singularity
		proc
			consume(var/atom/A)
				var
					gain = A.singularity_act(current_size)
				src.energy += gain
...
obj
	singularity
		proc
			move(var/force_move = 0)
				if(!move_self)
					return 0

				var
					movement_dir = pick(alldirs - last_failed_movement)
				...
				if((locate(/obj/machinery/field/containment) in T)||(locate(/obj/machinery/shieldwall) in T))
					...
					var
						obj
							machinery
								shieldwallgen
									S = locate(/obj/machinery/shieldwallgen) in T
					if(S && S.active)
						return 0
				return 1
