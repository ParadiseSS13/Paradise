			...
			large
				New()
					...
					if(A)
						var
							image
								alert_overlay = image('icons/effects/effects.dmi', "ghostalertsie")
...
obj
	singularity
		narsie
			Bump(atom/A)//you dare stand before a god?!
				godsmack(A)
				return
...
obj
	singularity
		narsie
			proc
				pickcultist() //Narsie rewards his cultists with being devoured first, then picks a ghost to follow. --NEO
					var
						list
							cultists = list()
					var
