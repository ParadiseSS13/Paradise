				...
				cell
					proc
						give(var/amount)
							...
							var
								power_used = min(maxcharge-charge,amount)
...
/*
	...
	* */
	if(charge==0)
		return
	var
		devastation_range = -1 //round(charge/11000)
	var
		heavy_impact_range = round(sqrt(charge)/60)
	var
		light_impact_range = round(sqrt(charge)/30)
	var
