datum
	powernet
		var
			list
				cables = list()	// all cables & junctions
		var
			list
				nodes = list()		// all connected machines

		var
			load = 0				// the current load on the powernet, increased by each machine at processing
		var
			newavail = 0			// what available power was gathered last tick, then becomes...
		var
			avail = 0				//...the current available power in the powernet
		var
			viewload = 0			// the load as it appears on the power console (gradually updated)
		var
			number = 0				// Unused //TODEL
		...
		var
			perapc_excess = 0
		var
			netexcess = 0			// excess power on the powernet (typically avail-load)

		var
			problem = 0	// If either of these is set to 1 there is some sort of issue at the powernet.
	...
	powernet
		proc
			draw_power(var/amount)
				var
					draw = between(0, amount, avail - load)
				load += draw
				return draw
...
datum
	powernet
		proc
			remove_cable(var/obj/structure/cable/C)
				cables -= C
				C.powernet = null
...
datum
	powernet
		proc
			add_cable(var/obj/structure/cable/C)
...
datum
	powernet
		proc
			remove_machine(var/obj/machinery/power/M)
				nodes -=M
				M.powernet = null
				if(is_empty())//the powernet is now empty...
					qdel(src)///... delete it - qdel
...
datum
	powernet
		proc
			add_machine(var/obj/machinery/power/M)
...
datum
	powernet
		proc
			trigger_warning(var/duration_ticks = 20)
