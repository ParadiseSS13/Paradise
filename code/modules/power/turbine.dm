			...
			compressor
				process()
					...
					var
						datum
							gas_mixture
								removed = inturf.remove_air(transfer_moles)
			...
			turbine
				process()
					...
					if(compressor.gas_contained.total_moles()>0)
						var
							oamount = min(compressor.gas_contained.total_moles(), (compressor.rpm+100)/35000*compressor.capacity)
						var
							datum
								gas_mixture
									removed = compressor.gas_contained.remove(oamount)
			...
			turbine_computer
				interact(mob/user)
					...
					var
						datum
							browser
								popup = new(user, "turbinecomputer", name)
