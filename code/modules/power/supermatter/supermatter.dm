		...
		power
			supermatter_shard
				...
				var
					damage_archived = 0
				var
					safe_alert = "Crystalline hyperstructure returning to safe operating levels."
				var
					warning_point = 50
				var
				...
				var
					lastwarning = 0				// Time in 1/10th of seconds since the last sent warning
				var
					power = 0

				var
					oxygen = 0					// Moving this up here for easier debugging.
				...
				process()
					...
					if(damage > warning_point) // while the core is still damaged and it's still worth noting its status
						if((world.timeofday - lastwarning) / 10 >= WARNING_DELAY)
							var
								stability = num2text(round((damage / explosion_point) * 100))
							...
							if(get_turf(src))
								...
								for(var/mob/living/mob in living_mob_list)
									...
									if(mob_pos && mob_pos.z == position.z)
										...
										var
											rads = DETONATION_RADS * sqrt( 1 / (get_dist(mob, src) + 1) )
					...
					var
						device_energy = power * REACTION_POWER_MODIFIER
							...
							obj
								item
									organ
										internal
											eyes
												eyes = l.get_int_organ(/obj/item/organ/internal/eyes)
						if(!istype(eyes))
							continue
					...
					for(var/mob/living/l in range(src, round((power / 100) ** 0.25)))
						var
							rads = (power / 10) * sqrt( 1 / max(get_dist(l, src),1) )
		...
		power
			supermatter_shard
				proc
					get_integrity()
						var
							integrity = damage / explosion_point
...
obj
	machinery
		power
			supermatter_shard
				attackby(obj/item/W as obj, mob/living/user as mob, params)
				...
				proc
					Consume(atom/movable/AM)
						...
						for(var/mob/living/L in range(10))
							var
								rads = 500 * sqrt( 1 / (get_dist(L, src) + 1) )
							L.apply_effect(rads, IRRADIATE)
