...
/obj/machinery/atmospherics/supermatter_crystal/examine(mob/user)
	...
	. += "<span class='notice'>When actived by an item hitting this awe-inspiring feat of engineering, it emits radiation and heat. This is the basis of use the pseudo-perpetual energy source, the supermatter crystal.</span>"
	. +="<span class='notice'>Any object that touches [src] instantly turns to dust, be it complex as a human or simple as a metal rod. These bursts of energy can cause hallucinations if meson scanners are not worn near the crystal.</span>"
	if(isAntag(user))
