			...
			generator_type2
				process()
					...
					var
						datum
							gas_mixture
								air1 = input1.return_exchange_air()
					var
						datum
							gas_mixture
								air2 = input2.return_exchange_air()
					...
					if(air1 && air2)
						...
						var
							hot_air_heat_capacity = hot_air.heat_capacity()
						var
							cold_air_heat_capacity = cold_air.heat_capacity()

						var
							delta_temperature = hot_air.temperature - cold_air.temperature

						if(delta_temperature > 1 && cold_air_heat_capacity > 0.01 && hot_air_heat_capacity > 0.01)
							var
								efficiency = (1 - cold_air.temperature/hot_air.temperature)*0.65 //65% of Carnot efficiency

							var
								energy_transfer = delta_temperature*hot_air_heat_capacity*cold_air_heat_capacity/(hot_air_heat_capacity+cold_air_heat_capacity)
...
// update icon overlays only if displayed level has changed
	...
	var
		genlev = max(0, min( round(11*lastgen / 100000), 11))
