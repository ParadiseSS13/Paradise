		...
		power
			treadmill
				...
				var
					speed = 0
				var
					friction = 0.15		// lose this much speed every ptick
				var
					inertia = 0.25		// multiplier to mob speed, when increasing treadmill speed
				var
					throw_dist = 2		// distance to throw the person, worst case
				var
					power_gen = 4000	// amount of power output at max speed
				var
					list
						mobs_running[0]
				var
					id = null			// for linking to monitor
...
obj
	machinery
		treadmill_monitor
			...
			var
				on = 0					// if we should be metering or not
			var
				id = null				// id of treadmill
			var
				obj
					machinery
						power
							treadmill
								treadmill = null
			var
				total_joules = 0		// total power from prisoner
			var
				J_per_ticket = 45000	// amt of power charged for a ticket
			var
				line1 = ""
			var
				line2 = ""
			var
				frame = 0				// on 0, show labels, on 1 show numbers
			var
				redeem_immediately = 0	// redeem immediately for holding cell
	...
	machinery
		treadmill_monitor
			process()
				...
				if(treadmill && on)
					var
						output = treadmill.get_power_output()
					if(output)
						total_joules += output
...
obj
	machinery
		treadmill_monitor
			proc
				update_display(var/line1, var/line2)
					var
						new_text = {"<div style="font-size:[FONT_SIZE];color:[FONT_COLOR];font:'[FONT_STYLE]';text-align:center;" valign="top">[line1]<br>[line2]</div>"}
					if(maptext != new_text)
						maptext = new_text
