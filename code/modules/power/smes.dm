	...
	machinery
		power
			smes
				...
				var
					capacity = 5e6 // maximum charge
				var
					charge = 0 // actual charge

				var
					input_attempt = 0 			// 1 = attempting to charge, 0 = not attempting to charge
				var
					inputting = 0 				// 1 = actually inputting, 0 = not inputting
				var
					input_level = 50000 		// amount of power the SMES attempts to charge by
				var
					input_level_max = 200000 	// cap on input_level
				var
					input_available = 0 		// amount of charge available from input last tick

				var
					output_attempt = 1 			// 1 = attempting to output, 0 = not attempting to output
				var
					outputting = 1 				// 1 = actually outputting, 0 = not outputting
				var
					output_level = 50000		// amount of power the SMES attempts to output
				var
					output_level_max = 200000	// cap on output_level
				var
					output_used = 0				// amount of power actually outputted. may be less than output_level if the powernet returns excess power

				//Holders for powerout event.
				var
					last_output_attempt	= 0
				var
					last_input_attempt	= 0
				var
					last_charge			= 0
				...
				var
					name_tag = null
				var
					building_terminal = 0 //Suggestions about how to avoid clickspam building several terminals accepted!
				var
					obj
						machinery
							power
		...
		power
			smes
				process()
					...
					if(input_attempt)
						var
							target_load = min((capacity-charge)/SMESRATE, input_level)	// charge at set rate, limited to spare capacity
						var
							actual_load = draw_power(target_load)						// add the load to the terminal side network
			...
			smes
				proc
					restore()
						...
						var
							excess = powernet.netexcess		// this was how much wasn't used on the network last ptick, minus any removed by other SMESes
			...
			smes
				proc
					make_terminal(const/mob/user)
						...
						var
							turf
								tempLoc = get_step(src, reverse_direction(tempDir))
						...
						if(do_after(user, 50, target = src))
							...
							var
								obj
									structure
										cable
											N = T.get_cable_node() //get the connecting node cable, if there's one
...
obj
	machinery
		power
			smes
				attack_alien(mob/living/carbon/alien/humanoid/user)
					return
