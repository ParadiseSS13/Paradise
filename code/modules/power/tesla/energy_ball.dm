...
var
	list
		blacklisted_tesla_types = list(/obj/machinery/atmospherics,
												obj
													machinery
														power
															emitter,
...
obj
	singularity
		energy_ball
			proc
				move_the_basket_ball(var/move_amount)
					//we face the last thing we zapped, so this lets us favor that direction a bit
					var
						first_move = dir
					for(var/i in 0 to move_amount)
						var
							move_dir = pick(alldirs + first_move) //give the first move direction a bit of favoring.
						if(target && prob(60))
				...
				handle_energy()
					if(energy >= energy_to_raise)
						...
						spawn(100)
							...
							var
								orbitsize = (I.Width() + I.Height()) * pick(0.4, 0.5, 0.6, 0.7, 0.8)
						...
						var
							Orchiectomy_target = pick(orbiting_balls)
...
obj
	singularity
		energy_ball
			orbit(obj/singularity/energy_ball/target)
				if(istype(target))
...
proc
	tesla_zap(var/atom/source, zap_range = 3, power)
		. = source.dir
		if(power < 1000)
			return
		...
		if(closest_atom)
			...
			var
				zapdir = get_dir(source, closest_atom)
			if(zapdir)
				. = zapdir
		...
		if(closest_tesla_coil)
			...
			var
				shock_damage = Clamp(round(power/400), 10, 90) + rand(-5, 5)
