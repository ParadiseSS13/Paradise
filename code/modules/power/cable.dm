	...
	structure
		cable
			New()
				...
				var
					turf
						T = src.loc			// hide if turf is not intact
...
obj
	structure
		cable
			Destroy()					// called when a cable is deleted
				if(powernet)
...
obj
	structure
		cable
			proc
				get_powernet()			//TODO: remove this as it is obsolete
					return powernet
		...
		if(src.d1)	// 0-X cables are 1 unit, X-X cables are 2 units long
			new
				obj
					item
						stack
							cable_coil(T, 2, color)
		else
			new
				obj
					item
						stack
							cable_coil(T, 1, color)
...
///// Z-Level Stuff
		/*if(src.d1 == 11 || src.d2 == 11)
			var
				turf
					controllerlocation = locate(1, 1, z)
			for(var/obj/effect/landmark/zcontroller/controller in controllerlocation)
				if(controller.down)
					var
						turf
							below = locate(src.x, src.y, controller.down_target)
...
obj
	structure
		cable
			proc
				shock(mob/user, prb, var/siemens_coeff = 1.0)
					if(!prob(prb))
						return 0
						...
						if(prob(50))
							new
								obj
									item
										stack
											cable_coil(src.loc, src.d1 ? 2 : 1, color)
							qdel(src) // qdel
...
obj
	structure
		cable
			proc
				mergeDiagonalsNetworks(var/direction)

					//search for and merge diagonally matching cables from the first direction component (north/south)
					var
						turf
							T  = get_step(src, direction&3)//go north/south
...
obj
	structure
		cable
			proc
				mergeConnectedNetworks(var/direction)

					var
						fdir = (!direction)? 0 : turn(direction, 180) //flip the direction, to match with the source position on its turf
...
obj
	structure
		cable
			proc
				get_connections(var/powernetless_only = 0)
		...
		cable
			proc
				denode()
					...
					var
						list
							powerlist = power_list(T1,src,0,0) //find the other cables that ended in the centre of the turf, with or without a powernet
					if(powerlist.len>0)
						var
							datum
								powernet
...
obj
	item
		stack
			cable_coil
				New(loc, length = MAXCOIL, var/paramcolor = null)
					..()
					src.amount = length
					if(paramcolor)
						color = paramcolor
						...
						var
							obj
								item
									organ
										external
											S = H.organs_by_name[user.zone_sel.selecting]
									...
									item
										weapon
											restraints
												handcuffs
													cable
														B = new /obj/item/weapon/restraints/handcuffs/cable(usr.loc)
							B.color = color
...
obj
	item
		stack
			cable_coil
				proc
					cable_join(obj/structure/cable/C, mob/user)
						var
							turf
								U = user.loc
						if(!isturf(U))
						...
						if(C.d1 == dirn || C.d2 == dirn)
							if(U.intact)						// can't place a cable if the floor is complete
								...
								var
									fdirn = turn(dirn, 180)		// the opposite direction
						...
						var
							nd1 = C.d2	// these will be the new directions
								var
									nd2 = dirn
			...
			cable_coil
				cyborg
					attack_self(mob/user)
						var
							cablecolor = input(user,"Pick a cable color.","Cable Color") in list("red","yellow","green","blue","pink","orange","cyan","white")
						color = cablecolor
						update_icon()
