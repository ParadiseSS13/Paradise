...
obj
	machinery
		light
			...
			icon = 'icons/obj/lighting.dmi'
			var
				base_state = "tube"		// base description and icon_state
			icon_state = "tube1"
...
obj
	machinery
		light
			attack_alien(mob/living/carbon/alien/humanoid/user)//So larva don't go breaking light bulbs.
							...
							var
								obj
									item
										organ
											external
												affecting = H.get_organ("[user.hand ? "l" : "r" ]_hand")
...
obj
	machinery
		light
			proc
				broken(var/skip_sound_and_sparks = 0)
					if(status == LIGHT_EMPTY)
						return
		...
		weapon
			light
				...
				var
					base_state
				var
					switchcount = 0	// number of times switched
				materials = list(MAT_METAL=60)
				var
					rigged = 0		// true if rigged to explode
				var
					brightness_range = 2 //how much light it gives off
				var
					brightness_power = 1
				var
					brightness_color = null
