		...
		power
			solar
				...
				var
					sunfrac = 0
				var
					adir = SOUTH // actual dir
				var
					ndir = SOUTH // target dir
				var
					turn_angle = 0
				var
					obj
...
obj
	machinery
		power
			solar
				New(var/turf/loc, var/obj/item/solar_assembly/S)
					..(loc)
					Make(S)
					connect_to_network()
...
obj
	machinery
		power
			solar
				proc
					set_control(var/obj/machinery/power/solar_control/SC)
...
obj
	machinery
		power
			solar
				proc
					Make(var/obj/item/solar_assembly/S)
						if(!S)
			...
			solar
				proc
					update_solar_exposure()
						...
						var
							p_angle = min(abs(adir - sun.angle), 360 - abs(adir - sun.angle))
...
obj
	machinery
		power
			solar
				process()//TODO: remove/add this from machines to save on processing as needed ~Carn PRIORITY
					if(stat & BROKEN)
						return
		...
		power
			solar_control
				...
				var
					cdir = 0
				var
					targetdir = 0		// target angle in manual tracking (since it updates every game minute)
				var
					gen = 0
				var
					lastgen = 0
				var
					track = 0			// 0= off  1=timed  2=auto (tracker)
				var
					trackrate = 600		// 300-900 seconds
				var
					nexttime = 0		// time for a panel to rotate of 1? in manual tracking
				var
					autostart = 0 		// Automatically search for connected devices
				var
					obj
						machinery
							power
									...
									obj
										item
											weapon
												circuitboard
													solar_control
														M = new /obj/item/weapon/circuitboard/solar_control( A )
