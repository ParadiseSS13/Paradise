			...
			port_gen
				pacman
					...
					*/
					...
					var
						max_safe_output = 4		// For UI use, maximal output that won't cause overheat.
					var
						time_per_sheet = 96		//fuel efficiency - how long 1 sheet lasts at power level 1
					var
						max_sheets = 100 		//max capacity of the hopper
					var
						max_temperature = 300	//max temperature before overheating increases
					var
						temperature_gain = 50	//how much the temperature increases per power output level, in degrees per level

					var
						sheets = 0			//How many sheets of material are loaded in the generator
					var
						sheet_left = 0		//How much is left of the current sheet
					var
						temperature = 0		//The current temperature
					var
						overheating = 0		//if this gets high enough the generator explodes
		...
		power
			port_gen
				pacman
					HasFuel()
						var
							needed_sheets = power_output / time_per_sheet
								...
								obj
									item
										stack
											sheet
												mineral
													S = new sheet_path(loc)
							var
								amount = min(sheets, S.max_amount)
							S.amount = amount
							sheets -= amount
			...
			port_gen
				pacman
					UseFuel()
						...
						var
							lower_limit = 56 + power_output * temperature_gain
						var
							upper_limit = 76 + power_output * temperature_gain
						...
						*/
						...
						if(environment)
							var
								ratio = min(environment.return_pressure()/ONE_ATMOSPHERE, 1)
							var
								ambient = environment.temperature - T20C
							lower_limit += ambient*ratio
							upper_limit += ambient*ratio

						var
							average = (upper_limit + lower_limit)/2
						...
						var
							inc_limit = max(upper_limit - temperature, 0)
						var
							dec_limit = min(temperature - lower_limit, 0)
				...
				pacman
					handleInactive()
						...
						if(temperature > cooling_temperature)
							var
								temp_loss = (temperature - cooling_temperature)/TEMPERATURE_DIVISOR
								...
								obj
									item
										stack
											addstack = O
							var
								amount = min((max_sheets - sheets), addstack.amount)
							if(amount < 1)
