...
obj
	machinery
		power
			proc
				disconnect_terminal() // machines without a terminal will just return, no harm no fowl.
					return
...
obj
	machinery
		proc
			powered(var/chan = -1) // defaults to power_channel
...
obj
	machinery
		proc
			use_power(var/amount, var/chan = -1) // defaults to power_channel
				var
					area
						A = get_area(src)		// make sure it's in an area
				if(!A || !isarea(A))
					return
				if(chan == -1)
					chan = power_channel
...
obj
	machinery
		proc
			removeStaticPower(value, powerchannel)
				addStaticPower(-value, powerchannel)
...
proc
	power_list(var/turf/T, var/source, var/d, var/unmarked=0, var/cable_only = 0)
		. = list()
		var
			fdir = (!d)? 0 : turn(d, 180)			// the opposite direction to d (or 0 if d==0)
///// Z-Level Stuff
	var
		Zdir
	if(d==11)
...
proc
	makepowernet_for(var/obj/structure/cable/PC)
		if(!PC.powernet)
			var
				datum
					powernet
...
proc
	electrocute_mob(mob/living/carbon/M as mob, var/power_source, var/obj/source, var/siemens_coeff = 1.0)
		...
		var
			drained_hp = M.electrocute_act(shock_damage, source, siemens_coeff) //zzzzzzap!
		var
		...
		if(source_area)
			...
			var
				drained_power = drained_energy/CELLRATE //convert from "joules" to "watts"
