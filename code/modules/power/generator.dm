	...
	machinery
		power
			generator
				...
				var
					power_multiplier = 1 // This is so admins can tweak how good it is to find a good balance for effort -> power
				var
					powercap = 500000 //Not a hard cap, but outputs above this have a 10% chance to cause the TeG to lose half it's power.
			...
			generator
				process()
					...
					var
						datum
							gas_mixture
								air1 = circ1.return_transfer_air()
					var
						datum
							gas_mixture
								air2 = circ2.return_transfer_air()
					lastgen = 0

					if(air1 && air2)
						var
							air1_heat_capacity = air1.heat_capacity()
						var
							air2_heat_capacity = air2.heat_capacity()
						var
							delta_temperature = abs(air2.temperature - air1.temperature)
					...
					if(lastgen > powercap && prob(10))
						...
						lastgen *= 0.5
					var
						genlev = max(0, min( round(11*lastgen / powercap), 11))
		...
		power
			generator
				interact(mob/user)
					...
					var
						t = "<PRE><B>Thermo-Electric Generator</B><HR>"
