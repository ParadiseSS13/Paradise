...
/proc/set_security_level(var/level)
	...
	if(level >= SEC_LEVEL_GREEN && level <= SEC_LEVEL_DELTA && level != security_level)
		...
		switch(level)
			if(SEC_LEVEL_GREEN)
				security_announcement_down.Announce("Todas las amenazas a la estación han pasado. Todas las armas deben estar enfundadas y las leyes de privacidad se vuelven a aplicar por completo.","¡Atención! Nivel de seguridad bajado a Verde.")
			...
			if(SEC_LEVEL_EPSILON)
				security_announcement_up.Announce("El Comando Central ha ordenado el nivel de seguridad de Epsilon en la estación. Todos los contratos laborales rescindidos.","¡Atención! ¡Nivel de seguridad de Epsilon activado!", new_sound = sound('sound/effects/purge_siren.ogg'))
