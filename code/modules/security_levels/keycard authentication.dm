		...
		keycard_auth
			...
			var
				list
					ert_chosen = list()
			var
				confirmed = 0 //This variable is set by the device that confirms the request.
			var
				confirm_delay = 20 //(2 seconds)
			var
				busy = 0 //Busy when waiting for authentication or an event request has been sent from this device.
			var
				obj
					machinery
						keycard_auth
...
obj
	machinery
		keycard_auth
			proc
				receive_request(var/obj/machinery/keycard_auth/source)
