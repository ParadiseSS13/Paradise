...
/obj/machinery/keycard_auth/proc/trigger_event()
	switch(event)
		if("Red Alert")
			...
			feedback_report("alert_keycard_auth_red")
		if("Grant Emergency Maintenance Access")
			make_maint_all_access()
			feedback_report("alert_keycard_auth_maintGrant")
		if("Revoke Emergency Maintenance Access")
			revoke_maint_all_access()
			feedback_report("alert_keycard_auth_maintRevoke")
		if("Emergency Response Team")
			...
			if(fullmin_count)
				...
				feedback_report("alert_keycard_auth_ert")
				spawn(3000)
