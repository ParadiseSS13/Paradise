...
/obj/item/clothing/suit/space/space_ninja/proc/ninja_sword_recall()
	...
	if(!ninjacost(cost))
		...
		if(inview) //If we can see the katana, throw it towards ourselves, damaging people as we go.
			...
			H.visible_message("<span class='danger'>[energyKatana] flies towards [H]!</span>","<span class='warning'>You hold out your hand and [energyKatana] flies towards you!</span>")
