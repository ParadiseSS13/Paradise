										...
										living
											carbon
												human
													U = affecting
								var
									display_to = U//Who do we want to display certain messages to?
								...
								if(U.client) // Send the spider OS images to the client
									var
										datum
											asset
												simple
													S = new/datum/asset/simple/spider_os() //no longer exists ;)
									send_asset_list(U.client, S.assets)

								var
									dat = "<html><head><title>SpiderOS</title></head><body bgcolor=\"#3D5B43\" text=\"#DB2929\"><style>a, a:link, a:visited, a:active, a:hover { color: #DB2929; }img {border-style:none;}</style>"
					...
					space_ninja
						Topic(href, href_list)
							...
							switch(href_list["choice"])
									...
									var
										obj
											item
												device
													pda
														P = locate(href_list["target"])
									var
										t = input(U, "Please enter untraceable message.") as text
								...
								if("Trigger Ability")
									var
										ability_name = href_list["name"]+href_list["cost"]//Adds the name and cost to create the full proc name.
									var
										proc_arguments//What arguments to later pass to the proc, if any.
									var
										list
											targets = list()//To later check for.
									var
										safety = 0//To later make sure we're triggering the proc when needed.
								...
								if("Copy to Disk")
									var
										datum
											tech
												current_data = locate(href_list["target"])
