					...
					if(!used)
						...
						var
							datum
								martial_art
									ninja_martial_art
										N = new/datum/martial_art/ninja_martial_art(null)
						N.teach(user)
...
datum
	martial_art
		ninja_martial_art
			...
			var
				list
					attack_names = list("dragon", "eagle", "mantis", "tiger", "spider", "monkey", "snake", "crane", "xeno") // Fluff attack texts, used later in attack message generation.
			var
				has_choke_hold = 0 	// Are we current choking a bitch?
			var
				has_focus = 1		//Can we user our special moves?
...
datum
	martial_art
		ninja_martial_art
			proc
				wrist_wrench(var/mob/living/carbon/human/A, var/mob/living/carbon/human/D)
...
datum
	martial_art
		ninja_martial_art
			proc
				choke_hold(var/mob/living/carbon/human/A, var/mob/living/carbon/human/D)
					if(!D.stat && !D.weakened)
						A.face_atom(D)
						...
						var
							hold_name = "[pick(attack_names)] [pick("grip", "hold", "vise", "press")]"
...
datum
	martial_art
		ninja_martial_art
			proc
				palm_strike(var/mob/living/carbon/human/A, var/mob/living/carbon/human/D)
					if(!D.stat && !D.weakened)
						if(has_focus)
							has_focus = 0
							A.face_atom(D)
							...
							var
								strike_adjective = pick(attack_names)
							...
							var
								atom
									throw_target = get_ranged_target_turf(D, get_dir(D, get_step_away(D, A)), 3) // Get a turf 3 tiles away from the target relative to our direction from him.
...
datum
	martial_art
		ninja_martial_art
			grab_act(var/mob/living/carbon/human/A, var/mob/living/carbon/human/D) //Instant aggressive grab
				D.grabbedby(A)
				var
					obj
						item
...
datum
	martial_art
		ninja_martial_art
			harm_act(var/mob/living/carbon/human/A, var/mob/living/carbon/human/D) // 10 damage punches
				var
					strike_name = "[pick(attack_names)] [pick("punches", "kicks", "chops", "slams", "strikes")]"
...
datum
	middleClickOverride
		ninja_martial_art
			onClick(var/atom/A, var/mob/living/carbon/human/user)
				...
				var
					datum
						martial_art
							ninja_martial_art
								user_martial_art = user.martial_art
				var
					mob
						living
							carbon
