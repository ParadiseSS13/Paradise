datum
	nano_module
		appearance_changer
			...
			var
				list
					valid_head_accessories = list()
			var
				list
					valid_marking_styles = list()
			var
				list
					valid_body_accessories = list()
...
datum
	nano_module
		appearance_changer
			New(var/location, var/mob/living/carbon/human/H, var/check_species_whitelist = 1, var/list/species_whitelist = list(), var/list/species_blacklist = list())
				..()
				owner = H
...
datum
	nano_module
		appearance_changer
			Topic(ref, href_list, var/nowindow, var/datum/topic_state/state = default_state)
				if(..())
					return 1
						...
						if(owner.species.bodyflags & HAS_SKIN_TONE)
							...
							var
								const
									MAX_LINE_ENTRIES = 4
							var
								prompt = "Choose your character's skin tone: 1-[owner.species.icon_skin_tones.len]\n("
				...
				if(href_list["skin_color"])
					if(can_change_skin_color())
						var
							new_skin = input(usr, "Choose your character's skin colour: ", "Skin Color", rgb(owner.r_skin, owner.g_skin, owner.b_skin)) as color|null
				...
				if(href_list["hair_color"])
					if(can_change(APPEARANCE_HAIR_COLOR))
						var
							new_hair = input("Please select hair color.", "Hair Color", rgb(head_organ.r_hair, head_organ.g_hair, head_organ.b_hair)) as color|null
				...
				if(href_list["facial_hair_color"])
					if(can_change(APPEARANCE_FACIAL_HAIR_COLOR))
						var
							new_facial = input("Please select facial hair color.", "Facial Hair Color", rgb(head_organ.r_facial, head_organ.g_facial, head_organ.b_facial)) as color|null
				...
				if(href_list["eye_color"])
					if(can_change(APPEARANCE_EYE_COLOR))
						var
							new_eyes = input("Please select eye color.", "Eye Color", rgb(owner.r_eyes, owner.g_eyes, owner.b_eyes)) as color|null
				...
				if(href_list["head_accessory_color"])
					if(can_change_head_accessory())
						var
							new_head_accessory = input("Please select head accessory color.", "Head Accessory Color", rgb(head_organ.r_headacc, head_organ.g_headacc, head_organ.b_headacc)) as color|null
				...
				if(href_list["marking_color"])
					if(can_change_markings())
						var
							new_markings = input("Please select marking color.", "Marking Color", rgb(owner.r_markings, owner.g_markings, owner.b_markings)) as color|null
