	...
	nano_module
		law_manager
			...
			var
				ion_law	= "IonLaw"
			var
				zeroth_law = "ZerothLaw"
			var
				inherent_law = "InherentLaw"
			var
				supplied_law = "SuppliedLaw"
			var
				supplied_law_position = MIN_SUPPLIED_LAW_NUMBER
			...
			Topic(href, href_list)
				...
				if(href_list["state_law"])
					var
						datum
							ai_law
								AL = locate(href_list["ref"]) in owner.laws.all_laws()
					if(AL)
						var
							state_law = text2num(href_list["state_law"])
				...
				if(href_list["change_zeroth_law"])
					var
						new_law = sanitize(input("Enter new law Zero. Leaving the field blank will cancel the edit.", "Edit Law", zeroth_law))
				...
				if(href_list["change_ion_law"])
					var
						new_law = sanitize(input("Enter new ion law. Leaving the field blank will cancel the edit.", "Edit Law", ion_law))
				...
				if(href_list["change_inherent_law"])
					var
						new_law = sanitize(input("Enter new inherent law. Leaving the field blank will cancel the edit.", "Edit Law", inherent_law))
				...
				if(href_list["change_supplied_law"])
					var
						new_law = sanitize(input("Enter new supplied law. Leaving the field blank will cancel the edit.", "Edit Law", supplied_law))
				...
				if(href_list["change_supplied_law_position"])
					var
						new_position = input(usr, "Enter new supplied law position between 1 and [MAX_SUPPLIED_LAW_NUMBER], inclusive. Inherent laws at the same index as a supplied law will not be stated.", "Law Position", supplied_law_position) as num|null
				...
				if(href_list["edit_law"])
					if(is_malf(usr))
						var
							datum
								ai_law
									AL = locate(href_list["edit_law"]) in owner.laws.all_laws()
						if(AL)
							var
								new_law = sanitize(input(usr, "Enter new law. Leaving the field blank will cancel the edit.", "Edit Law", AL.law))
				...
				if(href_list["delete_law"])
					if(is_malf(usr))
						var
							datum
								ai_law
									AL = locate(href_list["delete_law"]) in owner.laws.all_laws()
						if(AL && is_malf(usr))
							owner.delete_law(AL)
					return 1
				...
				if(href_list["state_law_set"])
					var
						datum
							ai_laws
								ALs = locate(href_list["state_law_set"]) in (is_admin(usr) ? admin_laws : player_laws)
					if(ALs)
						owner.statelaws(ALs)
					return 1
				...
				if(href_list["transfer_laws"])
					if(is_malf(usr))
						var
							datum
								ai_laws
									ALs = locate(href_list["transfer_laws"]) in (is_admin(usr) ? admin_laws : player_laws)
						if(ALs)
...
datum
	nano_module
		law_manager
			proc
				package_laws(var/list/data, var/field, var/list/datum/ai_law/laws)
					var
						packaged_laws[0]
...
datum
	nano_module
		law_manager
			proc
				package_multiple_laws(var/list/datum/ai_laws/laws)
					var
						law_sets[0]
...
datum
	nano_module
		law_manager
			proc
				sync_laws(var/mob/living/silicon/ai/AI)
					if(!AI)
						return
					for(var/mob/living/silicon/robot/R in AI.connected_robots)
						R.sync()
