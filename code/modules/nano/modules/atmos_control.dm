...
datum
	nano_module
		atmos_control
			New(atmos_computer, req_access, req_one_access, monitored_alarm_ids)
				..()
					...
					if(ui_ref)
						var
							obj
								machinery
									alarm
										alarm = locate(href_list["alarm"]) in (monitored_alarms ? monitored_alarms : machines)
						if(alarm)
							var
								datum
									topic_state
										TS = generate_state(alarm)
							alarm.ui_interact(usr, master_ui = ui_ref, state = TS)

datum
	nano_module
		atmos_control
			ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1, var/master_ui = null, var/datum/topic_state/state = default_state)
				var
					data[0]
