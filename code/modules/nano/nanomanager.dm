...
datum
	nanomanager
		proc
			try_update_ui(var/mob/user, src_object, ui_key, var/datum/nanoui/ui, data, var/force_open = 0)
...
datum
	nanomanager
		proc
			get_open_ui(var/mob/user, src_object, ui_key)
				var
					src_object_key = "\ref[src_object]"
...
datum
	nanomanager
		proc
			update_user_uis(var/mob/user, src_object = null, ui_key = null)
...
datum
	nanomanager
		proc
			close_user_uis(var/mob/user, src_object = null, ui_key = null)
				if(isnull(user.open_uis) || !istype(user.open_uis, /list) || open_uis.len == 0)
...
datum
	nanomanager
		proc
			ui_opened(var/datum/nanoui/ui)
				var
					src_object_key = "\ref[ui.src_object]"
				if(isnull(open_uis[src_object_key]) || !istype(open_uis[src_object_key], /list))
				...
				var
					list
						uis = open_uis[src_object_key][ui.ui_key]
				uis.Add(ui)
...
datum
	nanomanager
		proc
			user_transferred(var/mob/oldMob, var/mob/newMob)
