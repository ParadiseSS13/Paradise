...
client
	proc
		nanomapgen_DumpTile(var/startX = 1, var/startY = 1, var/currentZ = 1, var/endX = -1, var/endY = -1)
			...
			var
				icon
					Tile = icon(file("nano/mapbase1024.png"))
			...
			for(var/WorldX = startX, WorldX <= endX, WorldX++)
				for(var/WorldY = startY, WorldY <= endY, WorldY++)
					...
					var
						atom
							Turf = locate(WorldX, WorldY, currentZ)

					var
						icon
							TurfIcon = new(Turf.icon, Turf.icon_state)
			...
			var
				mapFilename = "nanomap_z[currentZ]-new.png"
