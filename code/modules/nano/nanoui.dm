...
datum
	nanoui
		New(nuser, nsrc_object, nui_key, ntemplate_filename, ntitle = 0, nwidth = 0, nheight = 0, var/atom/nref = null, var/datum/nanoui/master_ui = null, var/datum/topic_state/state = default_state)
			user = nuser
...
datum
	nanoui
		proc
			update_status(var/push_update = 0)
				var
					new_status = src_object.CanUseTopic(user, state)
				if(master_ui)
	...
	nanoui
		proc
			get_send_data(var/list/data)
				...
				var
					list
						send_data = list("config" = config_data)
	...
	nanoui
		proc
			get_html()
				...
				var
					template_data_json = "{}" // An empty JSON object
				if(templates.len > 0)
					template_data_json = json_encode(templates)

				var
					list
						send_data = get_send_data(initial_data)
				var
					initial_data_json = replacetext(replacetext(json_encode(send_data), "&#34;", "&amp;#34;"), "'", "&#39;")

				var
					url_parameters_json = json_encode(list("src" = "\ref[src]"))
