...
/datum/nanoui/proc/get_html()
	...
	if(templates.len > 0)
		template_data_json = rjson_encode(templates)

	var/list/send_data = get_send_data(initial_data)
	var/initial_data_json = replacetext(replacetext(rjson_encode(send_data), "&#34;", "&amp;#34;"), "'", "&#39;")

	var/url_parameters_json = rjson_encode(list("src" = "\ref[src]"))
...
//	to_chat(user, list2json_usecache(send_data))// used for debugging //NANO DEBUG HOOK
	...
	user << output(list2params(list(rjson_encode(send_data))),"[window_id].browser:receiveUpdateData")
