	...
	item
		clothing
			suit
				cardborg
					proc
						disguise(mob/living/carbon/human/H, obj/item/clothing/head/cardborg/borghead)
							if(istype(H))
								if(!borghead)
									borghead = H.head
								if(istype(borghead, /obj/item/clothing/head/cardborg)) //why is this done this way? because equipped() is called BEFORE THE ITEM IS IN THE SLOT WHYYYY
									var
										image
											I = image(icon = 'icons/mob/robots.dmi' , icon_state = "robot", loc = H)
									I.override = 1
