	...
	item
		weapon
			rig
				...
				var
					interface_path = "hardsuit.tmpl"
				var
					ai_interface_path = "hardsuit.tmpl"
				var
					interface_title = "Hardsuit Controller"
				var
					wearer_move_delay //Used for AI moving.
				var
					ai_controlled_move_delay = 10
				...
				var
					helm_type =  /obj/item/clothing/head/helmet/space/new_rig
				var
					boot_type =  /obj/item/clothing/shoes/magboots/rig
				var
					glove_type = /obj/item/clothing/gloves/rig
				var
				...
				var
					obj
						item
							weapon
								tank
									air_supply                       // Air tank, if any.
				var
					obj
						item
							clothing
								shoes
									boots = null                  // Deployable boots, if any.
				var
					obj
						item
							clothing
								suit
									space
										new_rig
											chest                // Deployable chestpiece, if any.
				var
					obj
						item
							clothing
								head
									helmet
										space
											new_rig
												helmet = null // Deployable helmet, if any.
				var
					obj
						item
							clothing
								gloves
									rig
										gloves = null            // Deployable gauntlets, if any.
				var
					obj
						item
							weapon
								stock_parts
									cell
										cell                             // Power supply, if any.
				var
					obj
						item
							rig_module
								selected_module = null            // Primary system (used with middle-click)
				var
					obj
						item
							rig_module
								vision
									visor                      // Kinda shitty to have a var for a module, but saves time.
				var
					obj
						item
							rig_module
				...
				var
					mob
						living
							carbon
								human
									wearer                        // The person currently wearing the rig.
				var
					image
						mob_icon                                        // Holder for on-mob icon.
				var
					list
						installed_modules = list()                       // Power consumption/use bookkeeping.

				// Rig status vars.
				var
					open = 0                                              // Access panel status.
				var
				...
				var
					interface_locked = 0
				var
					control_overridden = 0
				var
					ai_override_enabled = 0
				var
				...
				var
					sealing                                               // Keeps track of seal status independantly of NODROP.
				var
					offline = 1                                           // Should we be applying suit maluses?
				var
					offline_slowdown = 3                                  // If the suit is deployed and unpowered, it sets slowdown to this.
				var
					vision_restriction
				var
					offline_vision_restriction = 1                        // 0 - none, 1 - welder vision, 2 - blind. Maybe move this to helmets.
				var
					airtight = 1 //If set, will adjust AIRTIGHT and STOPSPRESSUREDMAGE flags on components. Otherwise it should leave them untouched.
				...
				proc
					seal(mob/living/user)
						...
						if(!failed_to_seal)
							...
							var
								list
									pieces_data = list(list(wearer.shoes, boots, "boots", boot_type),
							...
							for(var/list/piece_data in pieces_data)
								var
									obj
										item
											user_piece = piece_data[1]
								var
									obj
										item
											correct_piece = piece_data[2]
								var
									msg_type = piece_data[3]
								var
									piece_type = piece_data[4]
...
obj
	item
		weapon
			rig
				proc
					check_power_cost(var/mob/living/user, var/cost, var/use_unconcious, var/obj/item/rig_module/mod, var/user_is_ai)
						if(!istype(user))
							return 0
...
obj
	item
		weapon
			rig
				ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1, var/nano_state = inventory_state)
					if(!user)
						return
					...
					var
						is_sealed = (flags & NODROP)	//1 if NODROP, 0 if no-nodrop
					...
					for(var/obj/item/rig_module/module in installed_modules)
						...
						if(module.charges && module.charges.len)
							...
							var
								datum
									rig_charge
										selected = module.charges[module.charge_selected]
							...
							for(var/chargetype in module.charges)
								var
									datum
										rig_charge
											charge = module.charges[chargetype]
...
obj
	item
		weapon
			rig
				update_icon(var/update_mob_icon)
...
obj
	item
		weapon
			rig
				proc
					check_suit_access(var/mob/living/carbon/human/user)
			...
			rig
				Topic(href,href_list)
					...
					if(href_list["toggle_piece"])
						...
						var
							module_index = text2num(href_list["interact_module"])
						...
						if(module_index > 0 && module_index <= installed_modules.len)
							var
								obj
									item
										rig_module
											module = installed_modules[module_index]
...
obj
	item
		weapon
			rig
				proc
					toggle_piece(var/piece, var/mob/living/user, var/deploy_mode, var/force)
...
obj
	item
		weapon
			rig
				proc
					take_hit(damage, source, is_emp=0)
...
obj
	item
		weapon
			rig
				proc
					malfunction_check(var/mob/living/carbon/human/user)
...
obj
	item
		weapon
			rig
				proc
					ai_can_move_suit(var/mob/user, var/check_user_module = 0, var/check_for_ai = 0)
...
obj
	item
		weapon
			rig
				proc
					forced_move(var/direction, var/mob/user)
