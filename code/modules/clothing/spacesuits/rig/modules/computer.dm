	...
	item
		rig_module
			ai_container
				...
				var
					mob
						integrated_ai // Direct reference to the actual mob held in the suit.
				var
					obj
						item
							ai_card  // Reference to the MMI, posibrain, intellicard or pAI card previously holding the AI.
				var
					obj
						item
							ai_verbs
...
obj
	item
		rig_module
			ai_container
				accepts_item(var/obj/item/input_device, var/mob/living/user)
...
obj
	item
		rig_module
			ai_container
				proc
					integrate_ai(var/obj/item/ai,var/mob/user)
						if(!ai) return
						...
						var
							mob
								living
									ai_mob = locate(/mob/living) in ai.contents
						if(ai_mob)
...
obj
	item
		rig_module
			power_sink // quite stupid anyways, iyam
				...
				var
					atom
						interfaced_with // Currently draining power from this device.
				var
	...
	item
		rig_module
			power_sink
				accepts_item(var/obj/item/input_device, var/mob/living/user)
					var
						can_drain = input_device.drain_power(1)
					if(can_drain > 0)
						engage(input_device)
						return 1
					return 0
		...
		rig_module
			power_sink
				process()
					...
					var
						to_drain = min(40000, ((holder.cell.maxcharge - holder.cell.charge) / CELLRATE))
					var
						target_drained = interfaced_with.drain_power(0,0,to_drain)
					if(target_drained <= 0)
...
obj
	item
		rig_module
			power_sink
				proc
					drain_complete(var/mob/living/M)
