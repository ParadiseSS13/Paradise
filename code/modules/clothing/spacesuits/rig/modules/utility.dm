	...
	item
		rig_module
			chem_dispenser
				...
				var
					max_reagent_volume = 80 //Used when refilling.
...
obj
	item
		rig_module
			chem_dispenser
				accepts_item(var/obj/item/input_item, var/mob/living/user)
					...
					for(var/datum/reagent/R in input_item.reagents.reagent_list)
						for(var/chargetype in charges)
							var
								datum
									rig_charge
										charge = charges[chargetype]
							if(charge.display_name == R.id)

								var
									chems_to_transfer = R.volume
			...
			chem_dispenser
				engage(atom/target)
					...
					var
						datum
							rig_charge
								charge = charges[charge_selected]
		...
		rig_module
			voice
				engage()
					...
					var
						choice= input("Would you like to toggle the synthesiser or set the name?") as null|anything in list("Enable","Disable","Set Name")
					...
					switch(choice)
						...
						if("Set Name")
							var
								raw_choice = sanitize(input(usr, "Please enter a new name.")  as text|null, MAX_NAME_LEN)
							if(!raw_choice)
								return 0
							...
							var
								obj
									item
										weapon
											weldingtool
												W = holder.wearer.get_active_hand()
							if(istype(W))
								fill_welder(W)
							else
...
obj
	item
		rig_module
			welding_tank
				proc
					fill_welder(var/obj/item/weapon/weldingtool/W)
						if(!istype(W))
							return 0
