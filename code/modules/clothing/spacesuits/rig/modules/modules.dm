...
obj
	item
		rig_module
			...
			var
				toggleable                      // Set to 1 for the device to show up as an active effect.
			var
				usable                          // Set to 1 for the device to have an on-use effect.
			var
				selectable                      // Set to 1 to be able to assign the device as primary system.
			var
				redundant                       // Set to 1 to ignore duplicate module checking when installing.
			var
				permanent                       // If set, the module can't be removed.
			var
				disruptive = 1                  // Can disrupt by other effects.
			var
				activates_on_touch              // If set, unarmed attacks will call engage() on the target.

			var
				active                          // Basic module status
			var
				disruptable                     // Will deactivate if some other powers are used.

			var
				use_power_cost = 0              // Power used when single-use ability called.
			var
				active_power_cost = 0           // Power used when turned on.
			var
				passive_power_cost = 0          // Power used when turned off.

			var
				list
					charges                    // Associative list of charge types and remaining numbers.
			var
				charge_selected                 // Currently selected option used for charge dispensing.

			// Icons.
			var
				suit_overlay
			var
				suit_overlay_active             // If set, drawn over icon and mob when effect is active.
			var
				suit_overlay_inactive           // As above, inactive.
			var
				suit_overlay_used               // As above, when engaged.

			//Display fluff
			var
				interface_name = "hardsuit upgrade"
			var
				interface_desc = "A generic hardsuit upgrade."
			var
				engage_string = "Engage"
			var
				activate_string = "Activate"
			var
				deactivate_string = "Deactivate"
...
obj
	item
		rig_module
			proc
				installed(var/obj/item/weapon/rig/new_holder)
					holder = new_holder
					return
...
obj
	item
		rig_module
			proc
				accepts_item(var/obj/item/input_device)
					return 0

mob
	proc
		SetupStat(var/obj/item/weapon/rig/R)
			if(R && (R.flags & NODROP) && R.installed_modules.len && statpanel("Hardsuit Modules"))
				var
					cell_status = R.cell ? "[R.cell.charge]/[R.cell.maxcharge]" : "ERROR"
...
stat_rig_module
	New(var/obj/item/rig_module/module)
		..()
		src.module = module

stat_rig_module
	proc
		AddHref(var/list/href_list)
			return
...
stat_rig_module
	charge
		AddHref(var/list/href_list)
			var
				charge_index = module.charges.Find(module.charge_selected)
			if(!charge_index)
				charge_index = 0
			else
		...
		CanUse()
			if(module.charges && module.charges.len)
				var
					datum
						rig_charge
							charge = module.charges[module.charge_selected]
