...
datum
	breach
		var
			class = 0                           // Size. Lower is smaller.
		var
			descriptor                          // 'gaping hole' etc.
		var
			damtype = BURN                      // Punctured or melted
		var
			obj
				item
					clothing
						suit
							space
								holder // Suit containing the list of breaches holding this instance.
		...
		clothing
			suit
				space
					...
					var
						can_breach = 0                      // Set to 0 to disregard all breaching.
					var
						list
							breaches = list()              // Breach datum container.
					var
						resilience = 0.2                    // Multiplier that turns damage into breach class. 1 is 100% of damage to breach, 0.1 is 10%.
					var
						breach_threshold = 3                // Min damage before a breach is possible.
					var
						damage = 0                          // Current total damage
					var
						brute_damage = 0                    // Specifically brute damage.
					var
						burn_damage = 0                     // Specifically burn damage.
					var
	...
	item
		clothing
			suit
				space
					proc
						repair_breaches(var/damtype, var/amount, var/mob/user)
	...
	item
		clothing
			suit
				space
					proc
						create_breaches(var/damtype, var/amount)
