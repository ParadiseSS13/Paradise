				...
				var
					obj
						item
							clothing
								under
									has_suit = null		//the suit the tie may be attached to
				var
					image
						inv_overlay = null	//overlay used when attached to clothing.
...
obj
	item
		clothing
			accessory
				proc
					on_attached(obj/item/clothing/under/S, mob/user as mob)
						if(!istype(S))
							return
						has_suit = S
						loc = has_suit
...
obj
	item
		clothing
			accessory
				attack(mob/living/carbon/human/H, mob/living/user)
					...
					attack(mob/living/carbon/human/M, mob/living/user)
						if(ishuman(M) && isliving(user))
							if(user.a_intent == I_HELP)
								var
									body_part = parse_zone(user.zone_sel.selecting)
								if(body_part)
									var
										their = "their"
									switch(M.gender)
		...
		clothing
			accessory
				holobadge
					...
					var
						emagged = 0 //Emagging removes Sec check.
					var
						stored_name = null
				...
				petcollar
					attack_self(mob/user as mob)
						...
						switch(option)
							if("Change Name")
								var
									t = input(user, "Would you like to change the name on the tag?", "Name your new pet", tagname ? tagname : "Spot") as null|text
								if(t)
...
obj
	item
		clothing
			accessory
				petcollar
					attackby(obj/item/weapon/card/id/W, mob/user, params)
						if(!istype(W))
							return ..()
						if(access_id)
...
obj
	item
		clothing
			accessory
				petcollar
					equipped(mob/living/simple_animal/user)
						if(istype(user))
							processing_objects |= src
...
obj
	item
		clothing
			accessory
				petcollar
					dropped(mob/living/simple_animal/user)
						processing_objects -= src
							...
							obj
								item
									device
										radio
											headset
												a = new /obj/item/device/radio/headset(null)
...
proc
	english_accessory_list(obj/item/clothing/under/U)
