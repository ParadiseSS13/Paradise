	...
	item
		clothing
			glasses
				sunglasses
					lasers
						equipped(mob/user, slot) //grant them laser eyes upon equipping it.
							if(slot == slot_glasses)
...
obj
	item
		clothing
			glasses
				thermal
					syndi	//These are now a traitor item, concealed as mesons.	-Pete
						name = "Optical Meson Scanner"
						desc = "Used for seeing walls, floors, and stuff through anything."
						icon_state = "meson"
...
obj
	item
		clothing
			glasses
				proc
					chameleon(var/mob/user)
						var
							input_glasses = input(user, "Choose a piece of eyewear to disguise as.", "Choose glasses style.") as null|anything in list("Sunglasses", "Medical HUD", "Mesons", "Science Goggles", "Glasses", "Security Sunglasses","Eyepatch","Welding","Gar")
