		...
		clothing
			mask
				muzzle
					...
					put_on_delay = 20
					var
						resist_time = 0 //deciseconds of how long you need to gnaw to get rid of the gag, 0 to make it impossible to remove
					var
						mute = 1 // 1 - completely mutes you, 0 - muffles everything you say "MHHPHHMMM!!!"
					...
					tapegag
						dropped(mob/living/carbon/human/user)
							var
								atom
									movable
										R = new /obj/item/trash/tapetrash
...
obj
	item
		clothing
			mask
				horsehead
					dropped() //this really shouldn't happen, but call it extreme caution
						if(flags & NODROP)
							goodbye_horses(loc)
						..()
	...
	item
		clothing
			mask
				horsehead
					proc
						goodbye_horses(mob/user) //I'm flying over you
							if(!ismob(user))
								return
