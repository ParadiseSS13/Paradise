obj
	item
		voice_changer
			...
			var
				voice //If set and item is present in mask/suit, this name will be used for the wearer's speech.
			var
				active
		...
		clothing
			mask
				gas
					voice
						verb
							Set_Voice(name as text)
								set category = "Object"
								set src in usr

								var
									voice = sanitize(copytext(name,1,MAX_MESSAGE_LEN))
