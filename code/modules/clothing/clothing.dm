...
/obj/item/clothing/under/proc/set_sensors(mob/user as mob)
	...
	if(src.loc == user)
		...
		switch(sensor_mode)
			...
			if(1)
				visible_message("[user] turns [loc]'s remote sensors to binary.", vision_distance = 1)
			if(2)
				visible_message("[user] sets [loc]'s sensors to track vitals.", vision_distance = 1)
			if(3)
				visible_message("[user] sets [loc]'s sensors to maximum.", vision_distance = 1)
