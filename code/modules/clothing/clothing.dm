obj
	item
		clothing
			...
			var
				list
					species_restricted = null //Only these species can wear this kit.
			var
				rig_restrict_helmet = 0 // Stops the user from equipping a rig helmet without attaching it to the suit first.
			var
				scan_reagents = 0 //Can the wearer see reagents while it's equipped?
			...
			*/
			...
			var
				flash_protect = 0		//What level of bright light protection item has. 1 = Flashers, Flashes, & Flashbangs | 2 = Welding | -1 = OH GOD WELDING BURNT OUT MY RETINAS
			var
				tint = 0				//Sets the item's level of visual impairment tint, normally set to the same as flash_protect
			var
				up = 0					//but seperated to allow items to protect but not impair vision, like space helmets
			var
				visor_flags = 0			//flags that are added/removed when an item is adjusted up/down
			var
				visor_flags_inv = 0		//same as visor_flags, but for flags_inv

			var
				toggle_message = null
			var
				alt_toggle_message = null
			var
				active_sound = null
			var
				toggle_cooldown = null
			var
				cooldown = 0
			var
				species_disguise = null
...
obj
	item
		clothing
			proc
				refit_for_species(var/target_species)
		...
		clothing
			glasses
				...
				var
					invisa_view = 0
				var
					color_view = null//overrides client.color while worn
	...
	item
		clothing
			gloves
				...
				var
					pickpocket = 0 //Master pickpocket?
				var
					clipped = 0
				strip_delay = 20
				put_on_delay = 40
			...
			under
				proc
					set_sensors(mob/user as mob)
						...
						var
							list
								modes = list("Off", "Binary sensors", "Vitals tracker", "Tracking beacon")
						var
							switchMode = input("Select a sensor mode:", "Suit Sensor Mode", modes[sensor_mode + 1]) in modes
...
obj
	item
		clothing
			mask
				proc
					adjustmask(var/mob/user)
						var
							mob
								living
									carbon
										human
											H = usr //Used to check if the mask is on the head, to check if the hands are full, and to turn off internals if they were on when the mask was pushed out of the way.
...
obj
	item
		clothing
			shoes
				proc
					step_action(var/mob/living/carbon/human/H) //squeek squeek
						if(shoe_sound)
							var
								turf
									T = get_turf(H)
...
obj
	item
		clothing
			suit
				equipped(var/mob/living/carbon/human/user, var/slot) //Handle tail-hiding on a by-species basis.
...
obj
	item
		clothing
			suit
				verb
					openjacket(var/mob/user) //The verb you can use to adjust jackets.
...
obj
	item
		clothing
			suit
				ui_action_click() //This is what happens when you click the HUD action button to adjust your suit.
					if(!ignore_suitadjust)
						adjustsuit(usr)
...
obj
	item
		clothing
			under
				proc
					can_attach_accessory(obj/item/clothing/accessory/A)
						if(istype(A))
							.=1
						else
							return 0
...
obj
	item
		clothing
			under
				proc
					remove_accessory(mob/user, obj/item/clothing/accessory/A)
						if(!(A in accessories))
							return
