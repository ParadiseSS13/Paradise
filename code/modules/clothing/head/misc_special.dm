		...
		clothing
			head
				cakehat
					...
					var
						processing = 0 //I dont think this is used anywhere.
							...
							obj
								item
									organ
										external
											head
												head_organ = user.get_organ("head")
...
//		mob2.Blend(rgb(user.r_hair, user.g_hair, user.b_hair), ICON_ADD) - Commented out because it seemingly does nothing.
	...
	var
		icon
			earbit = new/icon("icon" = 'icons/mob/head.dmi', "icon_state" = "kittyinner")
				...
				kitty
					mouse
						update_icon(var/mob/living/carbon/human/user)
							...
							var
								icon
									earbit = new/icon("icon" = 'icons/mob/head.dmi', "icon_state" = "mouseyinner")
							mob.Blend(earbit, ICON_OVERLAY)
