...
/datum/surgery_step/proxy/try_op(mob/living/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
	...
	for(var/datum/surgery/S in branches_init)
		...
		if((!tool ||(isrobot(user) && istype(tool, /obj/item/gripper/medical))) && first_step.accept_hand)
	...
	if(!isnull(next_surgery_step))
		...
		if((!tool || (isrobot(user) && istype(tool, /obj/item/gripper/medical))) && next_surgery_step.accept_hand && !(SURGERY_TOOL_HAND in starting_tools))
