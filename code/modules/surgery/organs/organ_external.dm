	...
	item
		organ
			external
				...
				var
					list
						s_col = null // If this is instantiated, it should be a list of length 3
				var
					list
						wounds = list()
				var
					list
						child_icons = list()
				var
					number_wounds = 0 // cache the number of wounds, which is NOT wounds.len!
				var
					perma_injury = 0
				...
				var
					damage_msg = "\red You feel an intense pain"
				var
					broken_description
				...
				var
					cavity = 0
				var
					sabotaged = 0 //If a prosthetic limb is emagged, it will detonate when it fails.
				var
					encased       // Needs to be opened with a saw to access the organs.
...
obj
	item
		organ
			external
				replaced(var/mob/living/carbon/human/target)
					owner = target
...
#define LIMB_DMG_PROB 5
	...
	var
		can_cut = (prob(brute*2) || sharp) && !(status & ORGAN_ROBOT)
	...
	if((brute_dam + burn_dam + brute + burn) < max_damage || !config.limbs_can_break)
		...
		var
			can_inflict = max_damage * config.organ_health_multiplier - (brute_dam + burn_dam)
		if(can_inflict)
			if(brute > 0)
	...
	var
		mob
			living
				carbon
					owner_old = owner //Need to update health, but need a reference in case the below check cuts off a limb.
...
obj
	item
		organ
			external
				proc
					heal_damage(brute, burn, internal = 0, robo_repair = 0)
...
obj
	item
		organ
			external
				proc
					createwound(var/type = CUT, var/damage)
						if(damage == 0) return
						...
						var
							local_damage = brute_dam + burn_dam + damage
						...
						var
							obj
								item
									organ
										internal
											target_organ = null	//make internal organs become infected one at a time instead of all at once
					...
					update_wounds()
						...
						for(var/datum/wound/W in wounds)
							...
							if(W.internal && !W.can_autoheal() && owner.bodytemperature >= 170)
								var
									bicardose = owner.reagents.get_reagent_amount("styptic_powder")
...
obj
	item
		organ
			external
				proc
					droplimb(var/clean, var/disintegrate, var/ignore_children, var/nodamage)
						...
						switch(disintegrate)
							if(DROPLIMB_EDGE)
								if(!clean)
									var
										gore_sound = "[(status & ORGAN_ROBOT) ? "tortured metal" : "ripping tendons and flesh"]"
							...
							if(DROPLIMB_BURN)
								var
									gore = "[(status & ORGAN_ROBOT) ? "": " of burning flesh"]"
								owner.visible_message(
							...
							if(DROPLIMB_BLUNT)
								var
									gore = "[(status & ORGAN_ROBOT) ? "": " in shower of gore"]"
								var
									gore_sound = "[(status & ORGAN_ROBOT) ? "rending sound of tortured metal" : "sickening splatter of gore"]"
								owner.visible_message(
...
obj
	item
		organ
			external
				proc
					release_restraints(var/mob/living/carbon/human/holder)
						if(!holder)
							holder = owner
						if(!holder)
							return
...
obj
	item
		organ
			external
				proc
					get_damage()	//returns total damage
...
obj
	item
		organ
			external
				proc
					embed(var/obj/item/weapon/W, var/silent = 0)
						if(!owner || loc != owner)
							return
						if(!silent)
...
obj
	item
		organ
			external
				remove(var/mob/living/user, var/ignore_children)

					if(!owner)
						return
					var
						is_robotic = status & ORGAN_ROBOT
					var
						mob
							living
								carbon
...
obj
	item
		organ
			external
				proc
					cleanup_wounds(var/list/slated_wounds)
						wound_cleanup_timer = null
