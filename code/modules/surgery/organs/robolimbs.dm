...
datum
	robolimb
		var
			company = "Unbranded"                            // Shown when selecting the limb.
		var
			desc = "A generic unbranded robotic prosthesis." // Seen when examining a limb.
		var
			icon = 'icons/mob/human_races/robotic.dmi'       // Icon base to draw from.
		var
			unavailable_at_chargen                           // If set, not available at chargen.
		var
			is_monitor										 // If set, limb is a monitor and should be getting monitor styles.
		var
			has_subtypes = 2								 // If null, object is a model. If 1, object is a brand (that serves as the default model) with child models. If 2, object is a brand that has no child models and thus also serves as the model..
		var
			parts = list("chest", "groin", "head", "r_arm", "r_hand", "r_leg", "r_foot", "l_leg", "l_foot", "l_arm", "l_hand")	// Defines what parts said brand can replace on a body.
