...
datum
	wound
		...
		var
			desc = "wound" //default in case something borks
		...
		proc
			merge_wound(var/datum/wound/other)
		...
		proc
			heal_damage(amount, heals_internal = 0)
				if(src.internal && !heals_internal)
					// heal nothing
					return amount

				var
					healed_damage = min(src.damage, amount)
		...
		proc
			can_worsen(damage_type, damage)
				...
				var
					max_wound_damage = 1.5*src.damage_list[1]
...
proc
	get_wound_type(var/type = CUT, var/damage)
		switch(type)
			if(CUT)
				switch(damage)
					if(70 to INFINITY)
...
datum
	wound
		lost_limb
			New(var/obj/item/organ/external/lost_limb, var/losstype, var/clean)
				var
					damage_amt = lost_limb.max_damage
