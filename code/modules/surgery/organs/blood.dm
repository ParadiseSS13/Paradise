...
var
	const
		BLOOD_VOLUME_SAFE = 501
var
	const
		BLOOD_VOLUME_OKAY = 336
var
	const
		BLOOD_VOLUME_BAD = 224
var
	const
		BLOOD_VOLUME_SURVIVE = 122
		...
		carbon
			human
				var
					datum
						reagents
							vessel	//Container for blood and BLOOD ONLY. Do not transfer other chems here.
mob
	living
		carbon
			human
				var
					var
						pale = 0			//Should affect how mob sprite is drawn, but currently doesn't.
					...
					handle_blood()
						...
						if(stat != DEAD && bodytemperature >= 170)	//Dead or cryosleep people do not pump the blood.
							if(species.exotic_blood)
								var
									blood_reagent = species.exotic_blood // This is a string of the name of the species' blood reagent
								...
								if(blood_volume < max_blood && blood_volume)
									var
										datum
											reagent
												blood
													B = locate() in vessel.reagent_list //Grab some blood
								...
								obj
									item
										organ
											internal
												heart
													heart = get_int_organ(/obj/item/organ/internal/heart)
					...
					drip(var/amt as num)
						...
						if(src.species.exotic_blood)
							...
							if(vessel.total_volume)
								var
									fraction = amm / vessel.total_volume
...
mob
	living
		carbon
			proc
				take_blood(obj/item/weapon/reagent_containers/container, var/amount)
					var
						datum
							reagent
...
mob
	living
		carbon
			proc
				inject_blood(obj/item/weapon/reagent_containers/container, var/amount)
					var
						datum
							reagent
								blood
...
mob
	living
		carbon
			proc
				get_blood(datum/reagents/container)
					var
						datum
							reagent
								blood
...
*/
proc
	blood_splatter(var/target,var/source,var/large = 0)
					...
					decal
						cleanable
							blood
								B
		var
			decal_type = /obj/effect/decal/cleanable/blood/splatter
		var
			turf
				T = get_turf(target)
		var
