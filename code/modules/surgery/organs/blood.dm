	...
	if(iscarbon(AM))
		...
		if(blood_id == C.get_blood_id())//both mobs have the same blood substance
			if(blood_id == "blood") //normal blood
				...
				if(!(blood_data["blood_type"] in get_safe_blood(C.dna.blood_type)) || C.dna.species.own_species_blood && C.dna.species.name != blood_data["species"] || blood_data["species_only"] && C.dna.species.name != blood_data["species"])
