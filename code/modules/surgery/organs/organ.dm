	...
	item
		organ
			...
			var
				status = 0
			var
				vital //Lose a vital limb, die immediately.
			var
				damage = 0 // amount of damage to the organ
			...
			var
				list
					trace_chemicals = list() // traces of chemicals in the organ,
			...
			var
				last_freezer_update_time = 0
			var
				freezer_update_period = 100
			var
				is_in_freezer = 0

			var
				sterile = 0 //can the organ be infected by germs?
			var
				tough = 0 //can organ be easily damaged?
		...
		organ
			proc
				handle_germ_effects()
					...
					if(germ_level >= INFECTION_LEVEL_ONE)
						var
							fever_temperature = (owner.species.heat_level_1 - owner.species.body_temperature - 5)* min(germ_level/INFECTION_LEVEL_TWO, 1) + owner.species.body_temperature
...
obj
	item
		organ
			proc
				add_autopsy_data(var/used_weapon, var/damage)
					var
						datum
							autopsy_data
...
obj
	item
		organ
			proc
				take_damage(amount, var/silent=0)
					if(tough)
						return
...
obj
	item
		organ
			proc
				robotize() //Being used to make robutt hearts, etc
					robotic = 2
...
obj
	item
		organ
			proc
				mechassist() //Used to add things like pacemakers, etc
					robotize()
					src.status &= ~ORGAN_ROBOT
					robotic = 1
...
obj
	item
		organ
			proc
				remove(var/mob/living/user,special = 0)
					if(!istype(owner))
						return
...
obj
	item
		organ
			proc
				replaced(var/mob/living/carbon/human/target,var/obj/item/organ/external/affected)
...
obj
	item
		organ
			proc
				is_primary_organ(var/mob/living/carbon/human/O = null)
					if(isnull(O))
						O = owner
