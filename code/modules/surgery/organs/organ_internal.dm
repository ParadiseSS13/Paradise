...
obj
	item
		organ
			internal
				proc
					insert(mob/living/carbon/M, special = 0, var/dont_remove_slot = 0)
						...
						var
							obj
								item
									organ
										internal
											replaced = M.get_organ_slot(slot)
						if(replaced)
							if(dont_remove_slot)
								non_primary = 1
							else
				...
				kidneys
					process()
						...
						var
							datum
								reagent
									coffee = locate(/datum/reagent/drink/coffee) in owner.reagents.reagent_list
						if(coffee)
							if(is_bruised())
