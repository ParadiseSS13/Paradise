...
/obj/item/organ/internal/proc/insert(mob/living/carbon/M, special = 0, dont_remove_slot = 0)
	...
	if(istype(M, /mob/living/carbon/human))
		...
		if(!istype(parent))
			log_runtime(EXCEPTION("[src] attempted to insert into a [parent_organ], but [parent_organ] wasn't an organ! "), src)
		else
...
/obj/item/organ/internal/remove(mob/living/carbon/M, special = 0)
	if(!owner)
		log_runtime(EXCEPTION("\'remove\' called on [src] without an owner! Mob: [M], "), src)
	owner = null
	if(M)
		M.internal_organs -= src
	...
	if(istype(M, /mob/living/carbon/human))
		...
		if(!istype(parent))
			log_runtime(EXCEPTION("[src] attempted to remove from a [parent_organ], but [parent_organ] didn't exist! "), src)
		else
			parent.internal_organs -= src
		H.update_int_organs()
