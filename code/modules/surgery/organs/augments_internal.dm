				...
				cyberimp
					New(var/mob/M = null)
						...
						if(implant_overlay)
							var
								image
									overlay = new /image(icon, implant_overlay)
		...
		organ
			internal
				cyberimp
					brain
						anti_drop
							remove(var/mob/living/carbon/M, special = 0)
								..()
								if(active)
									ui_action_click()
	...
	item
		organ
			internal
				cyberimp
					chest
						arm_mod//dummy parent item for making arm-mod implants. works best with nodrop items that are sent to nullspace upon being dropped.
							...
							var
								obj
									holder//is defined as the retractable item itself. ensure this is defined somewhere!
							var
								out = 0//determines if the item is in the owner's hand or not
							var
								overloaded = 0//is set to 1 when owner gets EMPed. if set to 1, implant doesn't work.
							var
								lasthand = null
		...
		organ
			internal
				cyberimp
					chest
						arm_mod
							emp_act(severity)//if the implant gets EMPed...
		...
		organ
			internal
				cyberimp
					chest
						arm_mod
							tase//mounted, self-charging taser!
			...
			internal
				cyberimp
					chest
						arm_mod
							tase
								New()//when the implant is created...
		...
		organ
			internal
				cyberimp
					chest
						arm_mod
							lase//mounted, self-charging laser!
				...
				box
					cyber_implants
						bundle
							...
							var
								list
									boxed = list(/obj/item/organ/internal/cyberimp/eyes/xray,/obj/item/organ/internal/cyberimp/eyes/thermals,
														obj
															item
																organ
																	internal
																		cyberimp
																			brain
																				anti_stun, /obj/item/organ/internal/cyberimp/chest/reviver)
							var
								amount = 5
