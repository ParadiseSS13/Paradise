...
/proc/can_operate(mob/living/carbon/M)
	if(locate(/obj/machinery/optable, M.loc) && (M.lying || M.resting))
		return TRUE
	if(locate(/obj/structure/bed, M.loc) && (M.buckled || M.lying || M.IsWeakened() || M.IsStunned() || M.paralysis || M.IsSleeping() || M.stat))
		return TRUE
	if(locate(/obj/structure/table, M.loc) && (M.lying || M.IsWeakened() || M.IsStunned() || M.paralysis || M.IsSleeping()  || M.stat))
		return TRUE
	return FALSE
