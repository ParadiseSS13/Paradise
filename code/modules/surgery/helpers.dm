proc
	attempt_initiate_surgery(obj/item/I, mob/living/M, mob/user, var/override )
		if(istype(M))
			var
				mob
					living
					...
					if(override)
						if(istype(I,/obj/item/robot_parts))
							var
								datum
									surgery
										S = available_surgeries["robotic limb attachment"]
							if(S)
								var
									datum
										surgery
											procedure = new S.type
								if(procedure)
						...
						var
							P = input("Begin which procedure?", "Surgery", null, null) as null|anything in available_surgeries
