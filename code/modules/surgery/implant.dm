	...
	surgery
		cavity_implant
			...
			datum
				surgery_step
					open_encased
						retract, /datum/surgery_step/cavity/make_space,/datum/surgery_step/cavity/place_item,/datum/surgery_step/cavity/close_space,/datum/surgery_step/open_encased/close,/datum/surgery_step/glue_bone, /datum/surgery_step/set_bone,/datum/surgery_step/finish_bone,/datum/surgery_step/generic/cauterize)
...
datum
	surgery_step
		cavity
			proc
				get_max_wclass(obj/item/organ/external/affected)
...
datum
	surgery_step
		cavity
			proc
				get_cavity(obj/item/organ/external/affected)
					switch(affected.limb_name)
						if("head")
							return "cranial"
						if("chest")
							...
							item
								organ
									external
										affected = target.get_organ(target_zone)
					var
						can_fit = affected && !affected.hidden && affected.cavity && tool.w_class <= get_max_wclass(affected)
					return ..() && can_fit
...
datum
	surgery_step
		remove_object
			begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool, datum/surgery/surgery)
...
datum
	surgery_step
		remove_object
			end_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool, datum/surgery/surgery)
				if(L)
					if(ishuman(target))
						var
							mob
