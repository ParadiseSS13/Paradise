	...
	surgery_step
		limb/
			...
			can_use(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
				...
				var
					list
						organ_data = target.species.has_limbs["[target_zone]"]
					...
					var
						obj
							item
								organ
									external
										E = target.get_organ(target_zone)
					...
					var
						obj
							item
								organ
									external
										stump = target.organs_by_name["limb stump"]
					if(stump)
						stump.remove(target)
					if(E.children)
						for(var/obj/item/organ/external/C in E.children)
				...
				end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
					...
					if(L.part)
						for(var/part_name in L.part)
							...
							var
								list
									organ_data = target.species.has_limbs["[part_name]"]
							if(!organ_data)
								continue
							var
								obj
							...
							if(stump)
								stump.remove(target)
							var
								new_limb_type = organ_data["path"]
							var
								obj
									item
										organ
											external
												new_limb = new new_limb_type(target)
