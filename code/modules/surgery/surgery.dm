...
datum
	surgery
		...
		var
			ribcage =	0
		var
			head_reattach = 0									//Steps in a surgery
		*/
		...
		var
			list
				in_progress = list()									//Actively performing a Surgery
		var
			location = "chest"										//Surgery location
		var
			requires_organic_bodypart = 1							//Prevents you from performing an operation on robotic limbs
		var
			list
				possible_locs = list() 							//Multiple locations -- c0
		var
			obj
				item
					organ
						organ_ref									//Operable body part
		var
			current_organ = "organ"
		var
			list
				allowed_mob = list(/mob/living/carbon/human)
...
datum
	surgery
		proc
			next_step(mob/user, mob/living/carbon/target)
...
datum
	surgery
		proc
			complete(mob/living/carbon/human/target)
...
datum
	surgery_step
		var
			priority = 0	//steps with higher priority would be attempted first
		...
		var
			name
		var
			accept_hand = 0				//does the surgery step require an open hand? If true, ignores implements. Compatible with accept_any_item.
		var
			accept_any_item = 0
...
datum
	surgery_step
		proc
			try_op(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
				var
					success = 0
				if(accept_hand)
					if(!tool)
...
datum
	surgery_step
		proc
			initiate(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery)
...
datum
	surgery_step
		proc
			is_valid_target(mob/living/carbon/human/target)
				if(!hasorgans(target))
					return 0
...
proc
	spread_germs_to_organ(obj/item/organ/E, mob/living/carbon/human/user)
		...
		while(gap > 1 || swapped)
			...
			for(var/i = 1; gap + i <= surgery_steps.len; i++)
				var
					datum
						surgery_step
							l = surgery_steps[i]		//Fucking hate
				var
					datum
						surgery_step
							r = surgery_steps[gap+i]	//how lists work here
