	...
	surgery
		organ_manipulation
			...
			datum
				surgery_step
					open_encased
						retract, /datum/surgery_step/internal/manipulate_organs, /datum/surgery_step/glue_bone, /datum/surgery_step/set_bone,/datum/surgery_step/finish_bone,/datum/surgery_step/generic/cauterize)
...
datum
	surgery
		organ_manipulation
			can_start(mob/user, mob/living/carbon/target)
	...
	surgery_step
		internal
			manipulate_organs
				...
				var
					implements_extract = list(/obj/item/weapon/hemostat = 100, /obj/item/weapon/kitchen/utensil/fork = 55)
				var
					implements_mend = list(/obj/item/stack/medical/advanced/bruise_pack = 100,/obj/item/stack/nanopaste = 100,/obj/item/stack/medical/bruise_pack = 20)
				//Finish is just so you can close up after you do other things.
				var
					implements_finsh = list(/obj/item/weapon/scalpel/manager = 120,/obj/item/weapon/retractor = 100 ,/obj/item/weapon/crowbar = 75)
				var
					current_type
				var
					obj
...
datum
	surgery_step
		internal
			manipulate_organs
				begin_step(mob/living/user, mob/living/carbon/target, target_zone, obj/item/tool,datum/surgery/surgery)
					...
					//same as surgery step /datum/surgery_step/open_encased/close/
						...
						if(affected && affected.encased)
							var
								msg = "[user] starts bending [target]'s [affected.encased] back into place with \the [tool]."
							var
								self_msg = "You start bending [target]'s [affected.encased] back into place with \the [tool]."
							user.visible_message(msg, self_msg)
						else
							var
								msg = "[user] starts pulling [target]'s skin back into place with \the [tool]."
							var
								self_msg = "You start pulling [target]'s skin back into place with \the [tool]."
							user.visible_message(msg, self_msg)
						if(affected)
			...
			manipulate_organs
				end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool,datum/surgery/surgery)
					if(current_type == "mend")
						...
						if(affected.encased)
							var
								msg = "<span class='notice'> [user] bends [target]'s [affected.encased] back into place with \the [tool].</span>"
							var
								self_msg = "<span class='notice'> You bend [target]'s [affected.encased] back into place with \the [tool].</span>"
							user.visible_message(msg, self_msg)
							affected.open = 2.5
						else
							var
								msg = "<span class='notice'>[user] pulls [target]'s flesh back into place with \the [tool].</span>"
							var
								self_msg = "<span class='notice'>You pull [target]'s flesh back into place with \the [tool].</span>"
							user.visible_message(msg, self_msg)
...
datum
	surgery_step
		saw_carapace
			end_step(mob/living/user, mob/living/carbon/target, target_zone, obj/item/tool,datum/surgery/surgery)
...
datum
	surgery_step
		saw_carapace
			fail_step(mob/living/user, mob/living/carbon/target, target_zone, obj/item/tool,datum/surgery/surgery)
...
datum
	surgery_step
		retract_carapace
			begin_step(mob/living/user, mob/living/carbon/target, target_zone, obj/item/tool,datum/surgery/surgery)
				var
					msg = "[user] starts to pry open the incision on [target]'s [target_zone] with \the [tool]."
				var
					self_msg = "You start to pry open the incision on [target]'s [target_zone] with \the [tool]."
...
datum
	surgery_step
		retract_carapace
			end_step(mob/living/user, mob/living/carbon/target, target_zone, obj/item/tool,datum/surgery/surgery)
				var
					msg = "<span class='notice'> [user] keeps the incision open on [target]'s [target_zone] with \the [tool]</span>."
				var
					self_msg = "<span class='notice'> You keep the incision open on [target]'s [target_zone] with \the [tool].</span>"
				if(target_zone == "chest")
	...
	surgery_step
		generic
			retract_carapace
				fail_step(mob/living/user, mob/living/carbon/target, target_zone, obj/item/tool,datum/surgery/surgery)
					var
						msg = "<span class='warning'> [user]'s hand slips, tearing the edges of incision on [target]'s [target_zone] with \the [tool]!</span>"
					var
						self_msg = "<span class='warning'> Your hand slips, tearing the edges of incision on [target]'s [target_zone] with \the [tool]!</span>"
					if(target_zone == "chest")
