									...
									item
										organ
											external
												affected = target.get_organ(target_zone)
							var
								limb_can_operate = (affected && affected.open == 2 && affected.burn_dam > 0 && target_zone != "mouth")
							if(limb_can_operate)
								if(istype(C))
									if(!C.get_amount() >= 3)
...
datum
	surgery_step
		robotics
			manipulate_robotic_organs

				name = "internal part mainpulation"
				allowed_tools = list(/obj/item/device/mmi = 100)
				var
					implements_extract = list(/obj/item/device/multitool = 100)
				var
					implements_mend = list(	/obj/item/stack/nanopaste = 100,/obj/item/weapon/bonegel = 30, /obj/item/weapon/screwdriver = 70)
				var
					implements_insert = list(/obj/item/organ/internal = 100)
				var
					implements_finish =list(/obj/item/weapon/retractor = 100,/obj/item/weapon/crowbar = 100,/obj/item/weapon/kitchen/utensil = 50)
				var
					current_type
				var
					obj
