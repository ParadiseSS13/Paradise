	...
	if(can_unwrench && istype(W, /obj/item/wrench))
		...
		if(internal_pressure > 2*ONE_ATMOSPHERE)
			...
			if(internal_pressure > 1750 && !safefromgusts) // 1750 is the pressure limit to do 60 damage when thrown
				...
				pipeslowdown = 3 // give them more time to decide this is a bad idea, since it might kill them
			else
		...
		if(do_after(user, 40 * W.toolspeed * pipeslowdown, target = src) && !QDELETED(src))
