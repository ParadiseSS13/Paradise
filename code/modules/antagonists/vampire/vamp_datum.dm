...
/datum/antagonist/vampire/proc/handle_bloodsucking(mob/living/carbon/human/H, suck_rate = 5 SECONDS)
	...
	while(do_mob(owner.current, H, suck_rate))
		...
		if((H.stat != DEAD || H.has_status_effect(STATUS_EFFECT_RECENTLY_SUCCUMBED)) && !HAS_MIND_TRAIT(H, TRAIT_ENLIGHTENED_HUMAN))
		...
		if(H.blood_volume)
			...
			break
		if((!H.ckey && !H.player_ghosted) || HAS_MIND_TRAIT(H, TRAIT_ENLIGHTENED_HUMAN)) //Only runs if there is no ckey and the body has not being ghosted while alive, also runs if the victim is an evolved caterpillar or diona nymph.
