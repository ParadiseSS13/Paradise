...
/datum/objective/eldergod/update_explanation_text()
	explanation_text = "Summon [ticker.cultdat.entity_name] on the Station via the use of the Tear Reality rune. The veil is weak enough in [english_list(summon_spots)] for the ritual to begin."
...
/datum/objective/convert/update_explanation_text()
	var/datum/game_mode/cult/cult_mode = ticker.mode
	explanation_text = "We must increase our influence before we can summon [ticker.cultdat.entity_name], Convert [cult_mode.convert_target] crew members. Take it slowly to avoid raising suspicions."
