...
/datum/spell/pointed/projectile/furious_steel/Grant(mob/grant_to)
	. = ..()
	if(!owner)
		return
...
/datum/spell/pointed/projectile/furious_steel/Remove(mob/remove_from)
...
/datum/spell/pointed/projectile/furious_steel/proc/on_focus_lost(mob/source)
	SIGNAL_HANDLER

	unset_click_ability(source, refund_cooldown = TRUE)

/datum/spell/pointed/projectile/furious_steel/InterceptClickOn(mob/living/clicker, params, atom/target)
...
/datum/spell/pointed/projectile/furious_steel/on_activation(mob/on_who)
	. = ..()
	if(!.)
		return
...
/datum/spell/pointed/projectile/furious_steel/on_deactivation(mob/on_who, refund_cooldown = TRUE)
	. = ..()
	QDEL_NULL(blade_effect)

/datum/spell/pointed/projectile/furious_steel/before_cast(atom/cast_on)
...
/datum/spell/pointed/projectile/furious_steel/fire_projectile(mob/living/user, atom/target)
	. = ..()
	qdel(blade_effect.blades[1])

/datum/spell/pointed/projectile/furious_steel/ready_projectile(obj/projectile/to_launch, atom/target, mob/user, iteration)
	. = ..()
	to_launch.def_zone = check_zone(user.zone_selected)

/// If our blade status effect is deleted, clear our refs and deactivate
/datum/spell/pointed/projectile/furious_steel/proc/on_status_effect_deleted(datum/status_effect/protective_blades/source)
	SIGNAL_HANDLER
...
/obj/projectile/floating_blade/prehit_pierce(atom/hit)
	if(isliving(hit) && isliving(firer))
		...
		if(victim.can_block_magic(MAGIC_RESISTANCE))
			visible_message("<span class='warning'>[src] drops to the ground and melts on contact [victim]!</span>")
...
/datum/spell/pointed/projectile/furious_steel/haunted
	name = "Cursed Steel"
