...
/datum/status_effect/heretic_curse/on_apply()
	...
	RegisterSignal(the_curser, COMSIG_PARENT_QDELETING, PROC_REF(on_curser_destroyed))
...
/datum/status_effect/heretic_curse/proc/experience_the_consequences()
	...
	if(carbon_owner.vomit(vomit_flags = VOMIT_CATEGORY_BLOOD))
		carbon_owner.visible_message("<span class='boldwarning'>[carbon_owner] vomits out [carbon_owner.p_their()] [removing_organ]</span>")
	else
		carbon_owner.visible_message("<span class='boldwarning'>[carbon_owner]'s [removing_organ] rips itself out of `[carbon_owner.p_their()] chest!</span>")
