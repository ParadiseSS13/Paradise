...
datum
	space_level
		var
			name = "Your config settings failed, you need to fix this for the datum space levels to work"
		var
			list
				neigbours
		var
			z_value = 1 //actual z placement
		var
			linked = 1
		var
			xi
		var
			yi   //imaginary placements on the grid
...
datum
	space_level
		New()
			...
			var
				list
					L = list(Z_LEVEL_NORTH,Z_LEVEL_SOUTH,Z_LEVEL_EAST,Z_LEVEL_WEST)
			for(var/A in L)
				neigbours[A] = src
...
proc
	setup_map_transitions() //listamania
		var
			list
				SLS = file2list("config/space_levels.txt", "\n\n")
		var
			datum
				space_level
					D
		var
			list
				config_settings[SLS.len][]
		for(var/A in SLS)
			config_settings[SLS.Find(A)] = splittext(A, ";")
		var
			conf_set_len = SLS.len
		SLS.Cut()
		...
		var
			list
				point_grid[conf_set_len*2+1][conf_set_len*2+1]
		var
			list
				grid = list()
		var
