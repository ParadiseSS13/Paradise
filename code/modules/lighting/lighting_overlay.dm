	...
	movable
		lighting_overlay
			New()
				...
				var
					turf
						T = loc //If this runtimes atleast we'll know what's creating overlays outside of turfs.
				T.luminosity = 0
...
atom
	movable
		lighting_overlay
			proc
				get_clamped_lum(var/minlum = 0, var/maxlum = 1)
					var
						lum = max(lum_r, lum_g, lum_b)
					if(lum <= minlum)
						return 0
					else if(lum >= maxlum)
						return 1
...
atom
	movable
		lighting_overlay
			proc
				update_lumcount(delta_r, delta_g, delta_b)
					...
					if(!needs_update) //If this isn't true, we're already updating anyways.
						if(max(lum_r, lum_g, lum_b) < 1) //Any change that could happen WILL change appearance.
							...
							var
								mx1 = max(lum_r, lum_g, lum_b)
							var
								mx2 = max(lum_r + delta_r, lum_g + delta_g, lum_b + delta_b)
