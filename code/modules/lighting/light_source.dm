datum
	light_source
		...
		var
			light_range
		var
			light_color // string, decomposed by parse_light_color()
		...
		var
			vis_update		//Whetever we should smartly recalculate visibility. and then only update tiles that became (in) visible to us
		var
			needs_update
		var
			destroyed
	...
	light_source
		proc
			smart_vis_update()
				...
				var
					list
						new_turfs = view - effect_turf //This will result with all the tiles that are added.
				...
				var
					list
						old_turfs = effect_turf - view
				for(var/turf/T in old_turfs)
					//Insert not-so-huge copy paste from remove_lum().
					var
						idx = effect_turf.Find(T) //Get the index, luckily Find() is cheap in small lists like this. (with small I mean under a couple thousand len)
					if(T.affecting_lights)
						T.affecting_lights -= src
