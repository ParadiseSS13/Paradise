	...
	machinery
		cooker
			...
			var
				thiscooktype = null
			var
				burns = 0				// whether a machine burns something - if it does, you probably want to add the cooktype to /snacks/badrecipe
			var
				firechance = 0
			var
				cooktime = 0
			var
				foodcolor = null
			var
				has_specials = 0		//Set to 1 if the machine has specials to check, otherwise leave it at 0
...
obj
	machinery
		cooker
			proc
				checkCooked(obj/item/weapon/reagent_containers/food/snacks/D)
...
obj
	machinery
		cooker
			proc
				setCooked(obj/item/weapon/reagent_containers/food/snacks/oldtypes, obj/item/weapon/reagent_containers/food/snacks/newtypes)
					var
						ct
...
obj
	machinery
		cooker
			proc
				setRegents(obj/item/weapon/reagent_containers/OldReg, obj/item/weapon/reagent_containers/NewReg)
...
obj
	machinery
		cooker
			proc
				checkValid(obj/item/check, mob/user)
					if(on)
...
obj
	machinery
		cooker
			proc
				setIcon(obj/item/copyme, obj/item/copyto)
...
obj
	machinery
		cooker
			proc
				burn(mob/user, obj/item/weapon/reagent_containers/props)
					var
						obj
							item
								weapon
					...
					var
						datum
							effect
								system
									bad_smoke_spread
										smoke = new /datum/effect/system/bad_smoke_spread()    // burning things makes smoke!
...
obj
	machinery
		cooker
			proc
				changename(obj/item/name, obj/item/setme)
...
obj
	machinery
		cooker
			proc
				putIn(obj/item/tocook, mob/chef)
					icon_state = onicon
			...
			attackby(obj/item/I, mob/user, params)
				...
				if(I && I.loc == src)
					...
					if(has_specials)						//Checks if the machine has any special recipes that should be checked
						var
							special = checkSpecials(I)		//Checks if the inserted item is one of the specials
