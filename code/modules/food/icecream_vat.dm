						...
						weapon
							reagent_containers
								glass
									beaker = null
			var
				useramount = 15	//Last used amount
...
obj
	machinery
		icemachine
			proc
				generate_name(reagent_name)
					var
						name_prefix = pick("Mr.","Mrs.","Super","Happy","Whippy")
					var
						name_suffix = pick(" Whippy "," Slappy "," Creamy "," Dippy "," Swirly "," Swirl ")
					var
						cone_name = null	//Heart failure prevention.
			...
			attackby(obj/item/I, mob/user, params)
				...
				if(istype(I, /obj/item/weapon/reagent_containers/food/snacks/icecream))
					if(!I.reagents.has_reagent("sprinkles"))
						...
						var
							image
								sprinkles = image('icons/obj/kitchen.dmi', src, "sprinkles")
		...
		icemachine
			Topic(href, href_list)
				...
				if(href_list["add"])
					...
					var
						name = generate_name(reagents.get_master_reagent_name())
					name += " Chocolate Cone"
					var
						obj
							item
		...
		icemachine
			attack_hand(mob/user)
				...
				var
					datum
						browser
							popup = new(user, "cream_master","Cream-Master Deluxe", 700, 400, src)
