	...
	machinery
		computer
			telescience
					...
					obj
						machinery
							telepad
								telepad = null
				var
					temp_msg = "Telescience control console initialized.<BR>Welcome."

				// VARIABLES //
				var
					teles_left	// How many teleports left until it becomes uncalibrated
				var
					datum
						projectile_data
							last_tele_data = null
				...
				var
					teleport_cooldown = 0 // every index requires a bluespace crystal
				var
					list
						power_options = list(5, 10, 20, 25, 30, 40, 50, 80)
				var
					teleporting = 0
				var
					starting_crystals = 0
			...
			telescience
				interact(mob/user)
					...
					var
						datum
							browser
								popup = new(user, "telesci", name, 300, 500)
					popup.set_content(t)
					popup.open()
					return
				...
				proc
					doteleport(mob/user)
						...
						if(telepad)
							...
							var
								truePower = Clamp(power + power_off, 1, 1000)
							var
								trueRotation = rotation + rotation_off
							var
								trueAngle = Clamp(angle, 1, 90)

							var
								datum
									projectile_data
										proj_data = projectile_trajectory(telepad.x, telepad.y, trueRotation, trueAngle, truePower)
							last_tele_data = proj_data

							var
								trueX = Clamp(round(proj_data.dest_x, 1), 1, world.maxx)
							var
								trueY = Clamp(round(proj_data.dest_y, 1), 1, world.maxy)
							var
								spawn_time = round(proj_data.time) * 10

							var
								turf
									target = locate(trueX, trueY, z_co)
							last_target = target
							var
								area
									A = get_area(target)
							...
							spawn(round(proj_data.time) * 10) // in seconds
								...
								var
									sparks = get_turf(target)
								var
									datum
										effect
											system
												spark_spread
													y = new /datum/effect/system/spark_spread
								y.set_up(5, 1, sparks)
								y.start()
			...
			telescience
				proc
					teleport(mob/user)
						...
						var
							turf
								target = locate(Clamp(round(proj_data.dest_x, 1), 1, world.maxx), Clamp(round(proj_data.dest_y, 1), 1, world.maxy), z_co)
						var
							area
								A = get_area(target)
			...
			telescience
				Topic(href, href_list)
					...
					if(href_list["setrotation"])
						var
							new_rot = input("Please input desired bearing in degrees.", name, rotation) as num
					...
					if(href_list["setangle"])
						var
							new_angle = input("Please input desired elevation in degrees.", name, angle) as num
						if(..())
							return
					...
					if(href_list["setz"])
						var
							new_z = input("Please input desired sector.", name, z_co) as num
						if(..())
							return
