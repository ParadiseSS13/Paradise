...
obj
	machinery
		logic_gate
			not
				handle_logic()		//Our output will always be a continuous signal, even with a FLICKER, it just will update the output when the FLICKER ends
...
obj
	machinery
		logic_gate
			status
				receive_signal(datum/signal/signal, receive_method, receive_params)
					..()
