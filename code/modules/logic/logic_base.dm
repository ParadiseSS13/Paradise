...
obj
	machinery
		logic_gate
			...
			var
				tamperproof = 0		//if set, will make the machine unable to be destroyed, adjusted, etc. via in-game interaction (USE ONLY FOR MAPPING STUFF)
			var
				mono_input = 0		//if set, will ignore input2
...
obj
	machinery
		logic_gate
			attack_alien(mob/user)		//No xeno logic, that's too silly.
...
obj
	machinery
		logic_gate
			attack_animal(mob/user)	//No animal logic either.
...
obj
	machinery
		logic_gate
			attack_slime(mob/user)		//No slime logic. Seriously.
