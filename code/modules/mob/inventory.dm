...
mob
	proc
		can_equip(obj/item/I, slot, disable_warning = 0)
			return 0

// Because there's several different places it's stored.
mob
	proc
		get_multitool(var/if_active=0)
			return null

//Puts the item into your l_hand if possible and calls all necessary triggers/updates. returns 1 on success.
mob
	proc
		put_in_l_hand(var/obj/item/W)
...
mob
	proc
		put_in_hand_check(var/obj/item/W)
...
mob
	proc
		put_in_active_hand(var/obj/item/W)
...
mob
	proc
		put_in_inactive_hand(var/obj/item/W)
...
mob
	proc
		drop_item() //THIS. DOES. NOT. NEED. AN. ARGUMENT.
			if(hand)
				return drop_l_hand()
			else
				return drop_r_hand()
...
mob
	proc
		unEquip(obj/item/I, force) //Force overrides NODROP for things like wizarditis and admin undress.
