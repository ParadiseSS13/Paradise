	...
	cult_viewpoint
		verb
			pray_to_mask()
				...
				var
					input = stripped_input(usr, "Please choose a message to say to your god.", "Pray to Nar'Sie", "")
				if(!input)
					return
	...
	cult_viewpoint
		proc
			become_mask()
				...
				if(!active_mask())
					var
						confirm = alert(owner.client, "You are about to become a Mask of Nar'sie.  This will destroy your body, and you will only be able to guide the other cultists through communication. Are you sure?", "Confirm Your Sacrifice", "Yes", "No")
					if(confirm == "Yes")
						var
							transformation_type = alert(owner.client, "You are about to become a Mask. Do you want it to be subtle or violent?", "Mask", "Subtle", "Violent")
						if(!active_mask())
