			...
			verb
				urge_cultist()
					...
					if(cultist)
						if(cultist.owner)
							var
								newUrge = stripped_input(usr, "", "Set Urge", "")
		...
		mask
			verb
				set_cult_name()
					...
					if(cultist)
						var
							newName = stripped_input(usr, "", "Set Cult Name", "")
						if(!newName)
							return
		...
		mask
			verb
				urge_cult()
					...
					var
						newUrge = stripped_input(usr, "Please choose an urge.", "Set Urge", "")
				...
				set_favor_for_cultist()
					...
					if(cultist)
						if(cultist.owner)
							var
								list
									favor = list("Pleased", "Displeased", "Indifference")
							var
								emotion = input("Pick your emotion", "Mask", null, null) in favor
							switch(emotion)
								if("Pleased")
									cultist.set_favor(1)
		...
		mask
			proc
				set_name()
					spawn(0)
						var
							newName = stripped_input(src, "Please pick a name.", "Pick Name for Mask", "")
		...
		mask
			proc
				pick_cultist()
					...
					var
						input = input("Please, select a cultist!", "Cult", null, null) as null|anything in cultists
					var
						obj
							cult_viewpoint
								result = cultists[input]
					return result
			...
			spell
				aoe_turf
					blood_speech
						cast(list/targets)
							var
								input = stripped_input(usr, "Please choose a message to tell your acolytes.", "Voice of Blood", "")
							if(!input)
								revert_cast(usr)
					...
					conjure
						create_talisman
							cast(list/targets)
								...
								var
									talisman = input("Pick a talisman type", "Talisman", null, null) as null|anything in talismans
								var
									imbue_value = talismans[talisman]
								if(!talisman)
								...
								switch(talisman)
									...
									if("Teleport")
										var
											target_rune = input("Pick a teleport target", "Teleport Rune", null, null) as null|anything in engwords
										if(!target_rune)
											to_chat(usr, "You choose not to create a talisman.")
											revert_cast(usr)
											return
