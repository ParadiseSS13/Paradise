				...
				var
					datum
						alternate_appearance
							AA = viewing_alternate_appearances[aakey]
				if(AA)
...
mob
	proc
		update_hotkey_mode()
			var
				hotkeyname = "hotkeymode"
			if(client)
				var
					hotkeys = client.hotkeylist[client.hotkeytype]
...
mob
	proc
		update_normal_mode()
			...
			if(client)
				var
					hotkeys = client.hotkeylist[client.hotkeytype]//get the list containing the hotkey names
