mob
	Destroy()//This makes sure that mobs with clients/keys are not just deleted from the game.
		mob_list -= src
		dead_mob_list -= src
		living_mob_list -= src
		qdel(hud_used)
...
mob
	proc
		show_message(msg, type, alt, alt_type)//Message, type of message (1 or 2), alternative message, alt message type (1 or 2)
...
mob
	visible_message(var/message, var/self_message, var/blind_message)
...
atom
	proc
		visible_message(var/message, var/blind_message)
			for(var/mob/M in viewers(src))
...
mob
	audible_message(var/message, var/deaf_message, var/hearing_distance, var/self_message)
		var
			range = 7
		if(hearing_distance)
		...
		// based on say code
		var
			omsg = replacetext(message, "<B>[src]</B> ", "")
		var
			list
				listening_obj = new
...
atom
	proc
		audible_message(var/message, var/deaf_message, var/hearing_distance)
			var
				range = 7
			if(hearing_distance)
				range = hearing_distance
...
mob
	proc
		put_in_any_hand_if_possible(obj/item/W as obj, del_on_fail = 0, disable_warning = 1, redraw_mob = 1)
...
mob
	proc
		equip_to_slot_if_possible(obj/item/W as obj, slot, del_on_fail = 0, disable_warning = 0, redraw_mob = 1)
...
mob
	proc
		equip_to_slot(obj/item/W as obj, slot)
			return

//This is just a commonly used configuration for the equip_to_slot_if_possible() proc, used to equip people when the rounds tarts and when events happen and such.
mob
	proc
		equip_to_slot_or_del(obj/item/W as obj, slot)
...
mob
	living
		carbon
			human
				proc
					equip_or_collect(var/obj/item/W, var/slot)
...
mob
	proc
		equip_to_appropriate_slot(obj/item/W)
			if(!istype(W)) return 0
...
mob
	proc
		check_for_open_slot(obj/item/W)
			if(!istype(W)) return 0
			var
				openslot = 0
			for(var/slot in slot_equipment_priority)
...
obj
	item
		proc
			mob_check_equip(M as mob, slot, disable_warning = 0)
				if(!M) return 0
				if(!slot) return 0
				if(ishuman(M))
					//START HUMAN
...
mob
	proc
		ret_grab(obj/effect/list_container/mobl/L as obj, flag)
...
mob
	proc
		store_memory(msg as message, popup, sane = 1)
			msg = copytext(msg, 1, MAX_MESSAGE_LEN)
	...
	proc
		update_flavor_text()
			...
			if(usr != src)
				to_chat(usr, "No.")
			var
				msg = input(usr,"Set the flavor text in your 'examine' verb. Can also be used for OOC notes about your character.","Flavor Text",html_decode(flavor_text)) as message|null
...
mob
	proc
		print_flavor_text(var/shrink = 1)
			if(flavor_text && flavor_text != "")
				var
					msg = replacetext(flavor_text, "\n", " ")
...
mob
	proc
		stripPanelUnequip(obj/item/what, mob/who)
			return
...
mob
	proc
		stripPanelEquip(obj/item/what, mob/who)
			return
...
mob
	proc
		show_stat_emergency_shuttle_eta()
			var
				ETA = shuttle_master.emergency.getModeStr()
			if(ETA)
				stat(null, "[ETA] [shuttle_master.emergency.getTimerStr()]")

mob
	proc
		add_stings_to_statpanel(var/list/stings)
			for(var/obj/effect/proc_holder/changeling/S in stings)
				if(S.chemical_cost >=0 && S.can_be_used_by(src))
					statpanel("[S.panel]",((S.chemical_cost > 0) ? "[S.chemical_cost]" : ""),S)
mob
	proc
		add_spell_to_statpanel(var/obj/effect/proc_holder/spell/S)
...
mob
	proc
		update_canmove()
			var
				ko = weakened || paralysis || stat || (status_flags & FAKEDEATH)
			var
				buckle_lying = !(buckled && !buckled.buckle_lying)
...
mob
	proc
		SetStunned(amount) //if you REALLY need to set stun to a set amount without the whole "can't go below current stunned"
		...
		yank_out_object()
			...
			var
				obj
					item
						weapon
							selection = input("What do you want to yank out?", "Embedded objects") in valid_objects
			...
			verb
				respawn()
					...
					if((usr in respawnable_list) && (stat==2 || istype(usr,/mob/dead/observer)))
						var
							list
								creatures = list("Mouse")
						for(var/mob/living/L in living_mob_list)
							if(safe_respawn(L.type) && L.stat!=2)
								if(!L.key)
									creatures += L
						var
							picked = input("Please select an NPC to respawn as", "Respawn as NPC")  as null|anything in creatures
						switch(picked)
							if("Mouse")
...
mob
	proc
		become_mouse()
			var
				timedifference = world.time - client.time_died_as_mouse
...
mob
	proc
		assess_threat() //For sec bot threat assessment
			return

mob
	proc
		get_ghost(even_if_they_cant_reenter = 0)
			if(mind)
...
mob
	proc
		notify_ghost_cloning(var/message = "Someone is trying to revive you. Re-enter your corpse if you want to be revived!", var/sound = 'sound/effects/genetics.ogg', var/atom/source = null)
			var
				mob
					dead
						observer
...
mob
	proc
		fakevomit(green = 0, no_text = 0) //for aesthetic vomits that need to be instant and do not stun. -Fox
			if(stat==DEAD)
				return
			var
				turf
