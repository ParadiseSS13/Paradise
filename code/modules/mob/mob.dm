...
/mob/proc/run_examinate(atom/A)
	if(!has_vision(information_only = TRUE) && !isobserver(src))
		to_chat(src, "<div class='boxed_message'><span class='notice'>Something is there but you can't see it.</span></div>")
		return 1

	face_atom(A)
	var/list/result = A.examine(src)
	to_chat(src, "<div class='boxed_message examine'>[result.Join("\n")]</div>")
