...
proc
	isloyal(A) //Checks to see if the person contains a mindshield implant, then checks that the implant is actually inside of them
...
proc
	ismindslave(A) //Checks to see if the person contains a mindslave implant, then checks that the implant is actually inside of them
			...
			mob
				living
					carbon
						C = A
		var
			special_role = C.mind.special_role
		var
			list
				crew_roles = list("traitor", "Changeling", "Vampire", "Cultist", "Head Revolutionary", "Revolutionary", "malfunctioning AI", "Shadowling", "loyalist", "mutineer", "Response Team")
...
proc
	ran_zone(zone, probability = 80)
		...
		var
			t = rand(1, 18) // randomly pick a different zone, or maybe the same one
		switch(t)
			if(1)		 return "head"
			if(2)		 return "chest"
...
proc
	slur(phrase, var/list/slurletters = ("'"))//use a different list as an input if you want to make robots slur with $#@%! characters
...
proc
	stutter(n)
		var
			te = html_decode(n)
		var
			t = ""//placed before the message. Not really sure what it's for.
		...
		while(p <= n)//while P, which starts at 1 is less or equal to N which is the length.
			var
				n_letter = copytext(te, p, p + 1)//copies text from a certain distance. In this case, only one letter at a time.
...
proc
	robostutter(n) //for robutts
		...
		while(p <= n)//while P, which starts at 1 is less or equal to N which is the length.
			var
				robotletter = pick("@", "!", "#", "$", "%", "&", "?") //for beep boop
			var
				n_letter = copytext(te, p, p + 1)//copies text from a certain distance. In this case, only one letter at a time.
			if(prob(80) && (ckey(n_letter) in list("b","c","d","f","g","h","j","k","l","m","n","p","q","r","s","t","v","w","x","y","z")))
				if(prob(10))
...
proc
	Gibberish(t, p)//t is the inputted message, and any value higher than 70 for p will cause letters to be replaced instead of added
...
var
	list
		intents = list(I_HELP,I_DISARM,I_GRAB,I_HARM)
proc
...
proc
	say_dead_direct(var/message, var/mob/subject = null)
		var
			name
		var
			keyname
...
proc
	notify_ghosts(var/message, var/ghost_sound = null, var/enter_link = null, var/atom/source = null, var/image/alert_overlay = null, var/attack_not_jump = 0) //Easy notification of ghosts.
					...
					var
						obj
							screen
								alert
									notify_jump
										A = O.throw_alert("\ref[source]_notify_jump", /obj/screen/alert/notify_jump)
					if(A)
	...
	proc
		rename_character(oldname, newname)
			...
			if(oldname)
				...
				var
					list
						searching = GetAllContents(searchDepth = 3)
				var
					search_id = 1
				var
					search_pda = 1
...
mob
	proc
		rename_self(var/role, var/allow_numbers=0)
			spawn(0)
				var
					oldname = real_name
