...
datum
	preferences
		proc
			random_character(gender_override)
				if(gender_override)
...
datum
	preferences
		proc
			randomize_hair_color(var/target = "hair")
				...
				var
					col = pick ("blonde", "black", "chestnut", "copper", "brown", "wheat", "old", "punk")
				switch(col)
					if("blonde")
						red = 255
						green = 255
	...
	preferences
		proc
			randomize_eyes_color()
				...
				var
					col = pick ("black", "grey", "brown", "chestnut", "blue", "lightblue", "green", "albino")
				switch(col)
					if("black")
						red = 0
						green = 0
...
datum
	preferences
		proc
			blend_backpack(var/icon/clothes_s,var/backbag,var/satchel,var/backpack="backpack")
				switch(backbag)
					if(2)
...
datum
	preferences
		proc
			update_preview_icon(var/for_observer=0)		//seriously. This is horrendous.
				...
				if(current_species && (current_species.bodyflags & HAS_TAIL))
					...
					if(body_accessory)
						var
							datum
								body_accessory
									accessory = body_accessory_by_name[body_accessory]
					...
					var
						icon
							temp = new /icon("icon" = tail_icon, "icon_state" = tail_icon_state)
				...
				if(!(current_species.bodyflags & NO_EYES))
					var
						icon
							eyes_s = new/icon("icon" = 'icons/mob/human_face.dmi', "icon_state" = current_species ? current_species.eyes : "eyes_s")
				...
				if(current_species && (current_species.bodyflags & HAS_HEAD_ACCESSORY))
					var
						datum
							sprite_accessory
								head_accessory_style = head_accessory_styles_list[ha_style]
				...
				var
					datum
						sprite_accessory
							facial_hair_style = facial_hair_styles_list[f_style]
				...
				var
					icon
						clothes_s = null
				var
					uniform_dmi='icons/mob/uniform.dmi'
