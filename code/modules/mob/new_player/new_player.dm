	...
	new_player
		proc
			new_player_panel_proc()
				...
				var
					output = "<center><p><a href='byond://?src=\ref[src];show_preferences=1'>Setup Character</A><br /><i>[real_name]</i></p>"
				...
				if(!IsGuestKey(src.key))
					...
					if(dbcon.IsConnected())
						...
						var
							DBQuery
								query = dbcon.NewQuery("SELECT id FROM [format_table_name("poll_question")] WHERE [(isadmin ? "" : "adminonly = false AND")] Now() BETWEEN starttime AND endtime AND id NOT IN (SELECT pollid FROM [format_table_name("poll_vote")] WHERE ckey = \"[ckey]\") AND id NOT IN (SELECT pollid FROM [format_table_name("poll_textreply")] WHERE ckey = \"[ckey]\")")
						query.Execute()
						var
							newpoll = 0
						while(query.NextRow())
				...
				var
					datum
						browser
							popup = new(src, "playersetup", "<div align='center'>New Player Options</div>", 220, 290)
		...
		Topic(href, href_list[])
			...
			if(href_list["observe"])
				...
				if(alert(src,"Are you sure you wish to observe? You cannot normally join the round after doing this!","Player Setup","Yes","No") == "Yes")
					...
					var
						obj
							O = locate("landmark*Observer-Start")
			...
			if(href_list["votepollid"] && href_list["votetype"])
				var
					pollid = text2num(href_list["votepollid"])
				var
					votetype = href_list["votetype"]
				switch(votetype)
					if("OPTION")
						var
							optionid = text2num(href_list["voteoptionid"])
					...
					if("NUMVAL")
						var
							id_min = text2num(href_list["minid"])
						var
							id_max = text2num(href_list["maxid"])
					...
					if("MULTICHOICE")
						var
							id_min = text2num(href_list["minoptionid"])
						var
							id_max = text2num(href_list["maxoptionid"])
				...
				if(config.assistantlimit)
					if(job.title == "Civilian")
						...
						var
							datum
								job
									officer = job_master.GetJob("Security Officer")
						var
							datum
								job
									warden = job_master.GetJob("Warden")
						var
							datum
								job
									hos = job_master.GetJob("Head of Security")
...
mob
	new_player
		proc
			AttemptLateSpawn(rank,var/spawning_at)
				if(src != usr)
					return 0
				if(!ticker || ticker.current_state != GAME_STATE_PLAYING)
					to_chat(usr, "\red The round is either not ready, or has already finished...")
...
mob
	new_player
		proc
			AnnounceArrival(var/mob/living/carbon/human/character, var/rank, var/join_message)
					...
					if(ailist.len)
						...
						if(character.mind)
							if((character.mind.assigned_role != "Cyborg") && (character.mind.special_role != "MODE"))
								...
								var
									arrivalmessage = announcer.arrivalmsg
...
mob
	new_player
		proc
			AnnounceCyborg(var/mob/living/character, var/rank, var/join_message)
				if(ticker.current_state == GAME_STATE_PLAYING)
					var
						ailist[] = list()
					for(var/mob/living/silicon/ai/A in living_mob_list)
					...
					if(ailist.len)
						...
						if(character.mind)
							if((character.mind.special_role != "MODE"))
								var
									arrivalmessage = "A new[rank ? " [rank]" : " visitor" ] [join_message ? join_message : "has arrived on the station"]."
								announcer.say(";[arrivalmessage]")
					else
						if(character.mind)
							if((character.mind.special_role != "MODE"))
...
mob
	new_player
		proc
			LateChoices()
				var
					mills = world.time // 1/10 of a second, not real milliseconds but whatever
				...
				for(var/datum/job/job in job_master.occupations)
					if(job && IsJobAvailable(job.title))
						...
						for(var/jobcat in categorizedJobs)
							var
								list
									jobs = categorizedJobs[jobcat]["jobs"]
				...
				for(var/jobcat in categorizedJobs)
					...
					if(length(categorizedJobs[jobcat]["jobs"]) < 1)
						continue
					var
						color = categorizedJobs[jobcat]["color"]
...
//		src << browse(dat, "window=latechoices;size=300x640;can_close=1")
	...
	var
		datum
			browser
				popup = new(src, "latechoices", "Choose Profession", 900, 600)
...
mob
	new_player
		proc
			is_species_whitelisted(datum/species/S)
				if(!S) return 1
