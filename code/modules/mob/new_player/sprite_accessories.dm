...
proc
	init_sprite_accessory_subtypes(var/prototype, var/list/L, var/list/male, var/list/female)
		if(!istype(L))	L = list()
...
datum
	sprite_accessory
		...
		var
			models_allowed = list() //Specifies which, if any, hairstyles can be accessed by which prosthetics. Should equal the manufacturing company name in robolimbs.dm.
		var
			marking_location //Specifies which bodypart a body marking is located on.
