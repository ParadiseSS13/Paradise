...
datum
	language
		var
			name = "an unknown language" // Fluff name of language if any.
		var
			desc = "A language."         // Short description for 'Check Languages'.
		var
			speech_verb = "says"         // 'says', 'hisses', 'farts'.
		var
			ask_verb = "asks"            // Used when sentence ends in a ?
		var
			exclaim_verb = "exclaims"    // Used when sentence ends in a !
		var
			whisper_verb                 // Optional. When not specified speech_verb + quietly/softly is used instead.
		var
			signlang_verb = list()       // list of emotes that might be displayed if this language has NONVERBAL or SIGNLANG flags
		var
			colour = "body"              // CSS style to use for strings in this language.
		var
			key = "x"                    // Character used to speak in language eg. :o for Unathi.
		var
			flags = 0                    // Various language flags.
		var
		...
		var
			follow = 0					 // Applies to HIVEMIND languages - should a follow link be included for dead mobs?
		var
			english_names = 0			 // Do we want English names by default, no matter what?
		var
			list
				scramble_cache = list()
...
datum
	language
		proc
			get_random_name(var/gender, name_count=2, syllable_count=4)
	...
	language
		proc
			scramble(var/input)
				...
				var
					ending = copytext(scrambled_text, length(scrambled_text))
				if(ending == ".")
					scrambled_text = copytext(scrambled_text,1,length(scrambled_text)-1)
				var
					input_ending = copytext(input, input_size)
...
datum
	language
		proc
			format_message(message, verb)
				return "[verb], <span class='message'><span class='[colour]'>\"[capitalize(message)]\"</span></span>"

datum
	language
		proc
			format_message_radio(message, verb)
				return "[verb], <span class='[colour]'>\"[capitalize(message)]\"</span>"

datum
	language
		proc
			get_talkinto_msg_range(message)
...
datum
	language
		proc
			broadcast(var/mob/living/speaker,var/message,var/speaker_mask)
				log_say("[key_name(speaker)]: ([name]) [message]")

				if(!speaker_mask) speaker_mask = speaker.name
				var
					msg = "<i><span class='game say'>[name], <span class='name'>[speaker_mask]</span> [format_message(message, get_spoken_verb(message))]</span></i>"

				for(var/mob/player in player_list)
					if(istype(player,/mob/dead) && follow)
						var
							msg_dead = "<i><span class='game say'>[name], <span class='name'>[speaker_mask]</span> ([ghost_follow_link(speaker, ghost=player)]) [format_message(message, get_spoken_verb(message))]</span></i>"
...
datum
	language
		proc
			check_special_condition(var/mob/other, var/mob/living/speaker)
				return 1

datum
	language
		proc
			get_spoken_verb(var/msg_end)
				switch(msg_end)
					if("!")
						return exclaim_verb
					if("?")
...
datum
	language
		diona
			get_random_name()
				var
					new_name = "[pick(list("To Sleep Beneath","Wind Over","Embrace of","Dreams of","Witnessing","To Walk Beneath","Approaching the"))]"
...
datum
	language
		wryn
			check_special_condition(var/mob/other)
...
datum
	language
		abductor
			check_special_condition(var/mob/living/carbon/human/other, var/mob/living/carbon/human/speaker)
	...
	language
		binary
			broadcast(var/mob/living/speaker,var/message,var/speaker_mask)
				...
				var
					message_start = "<i><span class='game say'>[name], <span class='name'>[speaker.name]</span>"
				var
					message_body = "<span class='message'>[speaker.say_quote(message)], \"[message]\"</span></span></i>"

				for(var/mob/M in dead_mob_list)
					if(!istype(M,/mob/new_player) && !istype(M,/mob/living/carbon/brain))
						var
							message_start_dead = "<i><span class='game say'>[name], <span class='name'>[speaker.name] ([ghost_follow_link(speaker, ghost=M)])</span>"
...
mob
	proc
		remove_language(var/rem_language)
			var
				datum
					language
						L = all_languages[rem_language]
			. = (L in languages)
			languages.Remove(L)

mob
	living
		remove_language(rem_language)
			var
				datum
					language
						L = all_languages[rem_language]
...
mob
	proc
		can_speak(datum/language/speaking)
			...
			if(href_list["default_lang"])
				if(href_list["default_lang"] == "reset")
					...
					var
						datum
							language
								L = all_languages[href_list["default_lang"]]
					if(L)
