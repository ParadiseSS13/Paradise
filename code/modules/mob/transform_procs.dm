...
mob
	living
		carbon
			human
				AIize(move=1) // 'move' argument needs defining here too because BYOND is dumb
					if(notransform)
						return
					for(var/t in organs)
						qdel(t)
			...
			var
				mob
					living
						silicon
							ai
								O = new (loc,,,1)//No MMI but safety is in effect.
			O.invisibility = 0
...
mob
	living
		carbon
			human
				make_into_mask(var/should_gib = 0)
					for(var/t in organs)
						qdel(t)
					return ..(should_gib)
...
mob
	proc
		make_into_mask(var/should_gib = 0, var/should_remove_items = 0)
			...
			human
				proc
					Alienize()
						...
						var
							alien_caste = pick("Hunter","Sentinel","Drone")
						var
							mob
								living
									carbon
...
mob
	living
		carbon
			human
				proc
					slimeize(adult as num, reproduce as num)
						if(notransform)
							return
						for(var/obj/item/W in src)
							unEquip(W)
						...
						if(reproduce)
							var
								number = pick(14;2,3,4)	//reproduce (has a small chance of producing 3 or 4 offspring)
							var
								list
									babies = list()
							...
							mob
								living
									simple_animal
										pet
											corgi
												new_corgi = new /mob/living/simple_animal/pet/corgi (loc)
		...
		carbon
			human
				Animalize()
					...
					var
						list
							mobtypes = typesof(/mob/living/simple_animal)
					var
						mobpath = input("Which type of mob should [src] turn into?", "Choose a type") in mobtypes
