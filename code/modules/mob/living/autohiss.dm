...
mob
	living
		proc
			handle_autohiss(message, datum/language/L)
...
datum
	species
		var
			list
				autohiss_basic_map = null
		var
			list
				autohiss_extra_map = null
		var
			list
				autohiss_exempt = null
...
datum
	species
		proc
			handle_autohiss(message, datum/language/lang, mode)
				...
				while(length(message))
					var
						min_index = 10000 // if the message is longer than this, the autohiss is the least of your problems
					var
						min_char = null
					for(var/char in map)
						var
							i = findtext(message, char)
