...
/mob/living/say(message, verb = "says", sanitize = TRUE, ignore_speech_problems = FALSE, ignore_atmospherics = FALSE, ignore_languages = FALSE, is_ai_auto = FALSE)
	if(client)
	...
	create_log(SAY_LOG, log_message, is_ai_auto = TRUE)
	...
	say_log += log_message
	log_say(log_message, src, is_ai_auto = TRUE)
