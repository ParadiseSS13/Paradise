	...
	machinery
		drone_fabricator
			process()
				...
				var
					elapsed = world.time - time_last_drone
	...
	dead
		verb
			join_as_drone()
				...
				var
					drone_age = 14 // 14 days to play as a drone
				var
					player_age_check = check_client_age(usr.client, drone_age)
				...
				var
					deathtime = world.time - src.timeofdeath
				var
					joinedasobserver = 0
				...
				var
					deathtimeminutes = round(deathtime / 600)
				var
					pluralcheck = "minute"
				...
				var
					deathtimeseconds = round((deathtime - deathtimeminutes * 600) / 10,1)
