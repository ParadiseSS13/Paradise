					...
					attackby(obj/item/weapon/W as obj, mob/user as mob, params)
						...
						if(istype(W, /obj/item/borg/upgrade/))
							...
							if(stat == 2)
								...
								var
									delta = (world.time / 10) - last_reboot
								if(reboot_cooldown > delta)
									var
										cooldown_time = round(reboot_cooldown - ((world.time / 10) - last_reboot), 1)
							...
							var
								obj
									item
										weapon
											gripper
												G = locate(/obj/item/weapon/gripper) in module
							if(G) G.drop_item()
	...
	living
		silicon
			robot
				drone
					proc
						question(var/client/C,var/mob/M)
							spawn(0)
								if(!C || !M || jobban_isbanned(M,"nonhumandept") || jobban_isbanned(M,"Drone"))	return
								var
									response = alert(C, "Someone is attempting to reboot a maintenance drone. Would you like to play as one?", "Maintenance drone reboot", "Yes", "No")
								if(!C || ckey)
									return
								if(response == "Yes")
	...
	living
		silicon
			robot
				drone
					proc
						transfer_personality(var/client/player)
...
mob
	living
		simple_animal
			drone
				flash_eyes(intensity = 1, override_blindness_check = 0, affect_silicon = 0)
					if(affect_silicon)
						return ..()
