					...
					use_power()
						if(stat == DEAD)
							...
							if(cell.charge <= 0)
								...
								var
									amt = Clamp((lamp_intensity - 2) * 2,1,cell.charge) //Always try to use at least one charge per tick, but allow it to completely drain the cell.
			...
			robot
				proc
					update_cell()
						if(cell)
							var
								cellcharge = cell.charge/cell.maxcharge
							switch(cellcharge)
								if(0.75 to INFINITY)
									clear_alert("charge")
								if(0.5 to 0.75)
