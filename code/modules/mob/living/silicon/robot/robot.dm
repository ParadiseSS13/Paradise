		...
		silicon
			robot
				...
				var
					custom_name = ""
				var
					custom_sprite = 0 //Due to all the sprites involved, a var for our custom borgs may be best
...
//Hud stuff
	...
	var
		shown_robot_modules = 0	//Used to determine whether they have the module menu shown or not
	var
		obj
			screen
...
//3 Modules can be activated at any one time.
	...
	var
		weaponlock_time = 120
	var
		lawupdate = 1 //Cyborgs will sync their laws with their AI by default
	var
		lockcharge //Used when locking down a borg to preserve cell charge
	var
		speed = 0 //Cause sec borgs gotta go fast //No they dont!
	var
		scrambledcodes = 0 // Used to determine if a borg shows up on the robotics console.  Setting to one hides them.
	var
		tracking_entities = 0 //The number of known entities currently accessing the internal camera
	var
		braintype = "Cyborg"
	var
		base_icon = ""
	...
	var
		lamp_max = 10 //Maximum brightness of a borg lamp. Set as a var for easy adjusting.
	var
		lamp_intensity = 0 //Luminosity of the headlamp. 0 is off. Higher settings than the minimum require power.
	var
		lamp_recharging = 0 //Flag for if the lamp is on cooldown after being forcibly disabled.

	var
		updating = 0 //portable camera camerachunk update
...
mob
	living
		silicon
			robot
				New(loc,var/syndie = 0,var/unfinished = 0, var/alien = 0)
			...
			robot
				proc
					init(var/alien=0)
						...
						var
							new_ai = select_active_ai_with_fewest_borgs()
						if(new_ai)
							lawupdate = 1
...
mob
	living
		silicon
			robot
				proc
					get_default_name(var/prefix as text)
						if(prefix)
							modtype = prefix
						if(mmi)
								...
								datum
									data
										pda
											app
												messenger
													M = rbPDA.find_program(/datum/data/pda/app/messenger)
							if(M)
								M.hidden = 1
			...
			robot
				Destroy()
					if(mmi && mind)//Safety for when a cyborg gets dust()ed. Or there is no MMI inside.
						var
							turf
								T = get_turf(loc)//To hopefully prevent run time errors.
						if(T)	mmi.loc = T
						if(mmi.brainmob)
			...
			robot
				proc
					pick_module()
						...
						var
							list
								modules = list("Standard", "Engineering", "Medical", "Miner", "Janitor", "Service", "Security")
						...
						designation = modtype
						var
							module_sprites[0] //Used to store the associations between sprite names and sprite index.
...
mob
	living
		silicon
			robot
				verb
					cmd_toggle_module(module as num)
			...
			robot
				proc
					self_diagnosis()
						...
						var
							dat = "<HEAD><TITLE>[src.name] Self-Diagnosis Report</TITLE></HEAD><BODY>\n"
			...
			robot
				verb
					toggle_component()
						...
						var
							toggle = input(src, "Which component do you want to toggle?", "Toggle Component") as null|anything in installed_components
						if(!toggle)
							return
							...
							obj
								item
									weapon
										tank
											jetpack
												current_jetpack = installed_jetpack()
						if(current_jetpack)
					...
					if(istype(W, /obj/item/weapon/weldingtool) && user.a_intent == I_HELP)
						...
						if(opened)
							if(cell)
									...
									var
										obj
											item
												robot_parts
													robot_suit
														C = new/obj/item/robot_parts/robot_suit(loc)
								...
								var
									remove = input(user, "Which component do you want to pry out?", "Remove Component") as null|anything in removable_components
								if(!remove)
									return
								var
									datum
		...
		silicon
			robot
				update_icons()
					...
					var
						combat = list("Combat","Peacekeeper")
...
*/
	var
		datum
			browser
				popup = new(src, "robotmod", "Modules")
...
mob
	living
		silicon
			robot
				proc
					update_headlamp(var/turn_off = 0, var/cooldown = 100)
						set_light(0)
...
mob
	living
		silicon
			robot
				proc
					choose_icon(var/triesleft, var/list/module_sprites)
						...
						if(triesleft >= 1)
							var
								choice = input("Look at your icon - is this what you want?") in list("Yes","No")
							if(choice=="No")
...
mob
	living
		silicon
			robot
				proc
					notify_ai(var/notifytype, var/oldname, var/newname)
						if(!connected_ai)
							return
						switch(notifytype)
							if(1) //New Cyborg
...
mob
	living
		silicon
			robot
				proc
					connect_to_ai(var/mob/living/silicon/ai/AI)
				...
				deathsquad
					attack_hand(mob/user)
						if(isnull(ckey) && !searching_for_ckey)
							...
							var
								list
									borg_candidates = pollCandidates("Do you want to play as a Nanotrasen Combat borg?")
							if(borg_candidates.len > 0 && isnull(ckey))
								searching_for_ckey = 0
								var
									mob
										M = pick(borg_candidates)
		...
		silicon
			robot
				syndicate
					...
					lawchannel = "State"
					var
						playstyle_string = "<span class='userdanger'>You are a Syndicate assault cyborg!</span><br>\
