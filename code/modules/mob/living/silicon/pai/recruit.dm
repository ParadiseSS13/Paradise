...
var
	datum
		paiController
			paiController			// Global handler for pAI candidates
...
datum
	paiController
		...
		var
			askDelay = 10 * 60 * 1	// One minute [ms * sec * min]
		...
		Topic(href, href_list[])
			...
			if(href_list["download"])
				var
					datum
						paiCandidate
							candidate = locate(href_list["candidate"])
				var
					obj
						item
							device
								paicard
									card = locate(href_list["device"])
				if(card.pai)
					return
		...
		proc
			recruitWindow(var/mob/M as mob)
				var
					datum
						paiCandidate
							candidate
		...
		proc
			findPAI(var/obj/item/device/paicard/p, var/mob/user)
		...
		proc
			requestRecruits(var/obj/item/device/paicard/P, mob/user)
		...
		proc
			check_recruit(var/mob/dead/observer/O)
		...
		proc
			question(var/client/C)
				spawn(0)
					if(!C)	return
					asked.Add(C.key)
					asked[C.key] = world.time
					var
						response = alert(C, "Someone is requesting a pAI personality. Would you like to play as a personal AI?", "pAI Request", "Yes", "No", "Never for this round")
					if(!C)	return		//handle logouts that happen whilst the alert is waiting for a response.
					if(response == "Yes")
						recruitWindow(C.mob)
					else if(response == "Never for this round")
						var
							warning = alert(C, "Are you sure? This action will be undoable and you will need to wait until next round.", "You sure?", "Yes", "No")
						if(warning == "Yes")
