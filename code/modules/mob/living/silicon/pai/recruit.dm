	...
	for(var/mob/dead/observer/O in GLOB.player_list)
		if(O.client && (ROLE_PAI in O.client.prefs.be_special))
			if(player_old_enough_antag(O.client,ROLE_PAI))
				if(check_recruit(O))
					to_chat(O, span_boldnotice("A pAI card activated by [user.real_name] is looking for personalities. (<a href='?src=[O.UID()];jump=\ref[P]'>Teleport</a> | <a href='?src=[UID()];signup=\ref[O]'>Sign Up</a>)"))
					//question(O.client)
