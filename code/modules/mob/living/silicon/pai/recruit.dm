		...
		if(href_list["new"])
			...
			switch(option)
				if("name")
					...
					if(t)
						candidate.name = sanitize_local(copytext(t,1,MAX_NAME_LEN))
				if("desc")
					t = input("Enter a description for your pAI", "pAI Description", candidate.description) as message
					if(t)
						candidate.description = sanitize_local(copytext(t,1,MAX_MESSAGE_LEN))
				if("role")
					t = input("Enter a role for your pAI", "pAI Role", candidate.role) as text
					if(t)
						candidate.role = sanitize_local(copytext(t,1,MAX_MESSAGE_LEN))
				if("ooc")
					t = input("Enter any OOC comments", "pAI OOC Comments", candidate.comments) as message
					if(t)
						candidate.comments = sanitize_local(copytext(t,1,MAX_MESSAGE_LEN))
				if("save")
				...
				if("load")
					...
					if(candidate.name)
						candidate.name = sanitize_local(copytext(candidate.name,1,MAX_NAME_LEN))
					if(candidate.description)
						candidate.description = sanitize_local(copytext(candidate.description,1,MAX_MESSAGE_LEN))
					if(candidate.role)
						candidate.role = sanitize_local(copytext(candidate.role,1,MAX_MESSAGE_LEN))
					if(candidate.comments)
						candidate.comments = sanitize_local(copytext(candidate.comments,1,MAX_MESSAGE_LEN))
