	...
	living
		silicon
			pai
				...
				var
					ram = 100	// Used as currency to purchase different abilities
				var
					list
						software = list()
				var
				...
				var
					obj
						item
							device
								radio
									radio		// Our primary radio

				var
					chassis = "repairbot"   // A record of your chosen chassis.
				var
					global
						list
				...
				var
					pai_law0 = "Serve your master."
				var
					pai_laws				// String for additional operating instructions our master might give us

				var
					silence_time			// Timestamp when we were silenced (normally via EMP burst), set to null after silence has faded

// Various software-specific vars

	var
		temp				// General error reporting text contained here will typically be shown once and cleared
	var
		screen				// Which screen our main window displays
	var
		subscreen			// Which specific function of the main screen is being displayed
	...
	var
		datum
			data
				record
					medicalActive1		// Datacore record declarations for record software
	var
		datum
			data
				record
	...
	var
		datum
			data
				record
					securityActive1		// Could probably just combine all these into one
	var
		datum
			data
				record
	...
	var
		hackprogress = 0				// Possible values: 0 - 100, >= 100 means the hack is complete and will be reset upon next check
	var
		hack_aborted = 0
		...
		obj
			item
				radio
					integrated
						signal
							sradio // AI's signaller

	var
		translator_on = 0 // keeps track of the translator module

	var
		current_pda_messaging = null

mob
	living
		silicon
			pai
				New(var/obj/item/device/paicard)
					src.loc = paicard
					card = paicard
					sradio = new(src)
					if(card)
							...
							datum
								data
									pda
										app
											messenger
												M = pda.find_program(/datum/data/pda/app/messenger)
						M.toff = 1
						var
							datum
								data
									pda
										app
											chatroom
												C = pda.find_program(/datum/data/pda/app/chatroom)
						C.toff = 1
					..()
			...
			pai
				proc
					show_silenced()
						if(src.silence_time)
							var
								timeleft = round((silence_time - world.timeofday)/10 ,1)
...
mob
	living
		silicon
			pai
				proc
					switchCamera(var/obj/machinery/camera/C)
			...
			pai
				proc
					choose_verbs()
						...
						var
							choice = input(usr,"What theme would you like to use for your speech verbs? This decision can only be made once.") as null|anything in possible_say_verbs
						if(!choice) return

						var
							list
								sayverbs = possible_say_verbs[choice]
					...
					var
						mob
							living
								carbon
									human
										H = over_object //changed to human to avoid stupid issues like xenos holding pAIs.
