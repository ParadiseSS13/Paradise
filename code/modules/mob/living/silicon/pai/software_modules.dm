		...
		directives
			...
			Topic(href, href_list)
				...
				if(href_list["getdna"])
					...
					// Check the carrier
					var
						answer = input(M, "[P] is requesting a DNA sample from you. Will you allow it to confirm your identity?", "[P] Check DNA", "No") in list("Yes", "No")
					if(answer == "Yes")
						var
							turf
								T = get_turf_or_move(P.loc)
		...
		radio_config
			...
			on_ui_interact(mob/living/silicon/pai/user, datum/nanoui/ui = null, force_open = 1)
				...
				for(var/ch_name in user.radio.channels)
					var
						ch_stat = user.radio.channels[ch_name]
					var
						ch_dat[0]
					ch_dat["name"] = ch_name
					...
					datum
						data
							pda
								app
									messenger
										M = user.pda.find_program(/datum/data/pda/app/messenger)
				if(!M)
					return
						...
						datum
							data
								pda
									app
										messenger
											M = P.pda.find_program(/datum/data/pda/app/messenger)
					if(!M)
						return
					...
					datum
						data
							pda
								app
									chatroom
										M = user.pda.find_program(/datum/data/pda/app/chatroom)
				if(!M)
					return
			...
			Topic(href, href_list)
				...
				if(!isnull(P.pda) && P.pda.can_use())
						...
						datum
							data
								pda
									app
										chatroom
											M = P.pda.find_program(/datum/data/pda/app/chatroom)
					if(!M)
						return
					...
					if(href_list["toggler"])
						...
						var
							t = input("Enter new topic:", M.current_room, M.current_room.topic) as text|null
						spawn()
			...
			on_ui_interact(mob/living/silicon/pai/user, datum/nanoui/ui=null, force_open=1)
				...
				var
					datum
						data
							record
								G = user.medicalActive1
				var
					datum
						data
							record
								M = user.medicalActive2
				...
				if(href_list["select"])
					var
						datum
							data
								record
									record = locate(href_list["select"])
					if(record)
						var
							datum
								data
			...
			on_ui_interact(mob/living/silicon/pai/user, datum/nanoui/ui=null, force_open=1)
				...
				var
					datum
						data
							record
								G = user.securityActive1
				var
					datum
						data
							record
								S = user.securityActive2
				data["general"] = G ? G.fields : null
		...
		signaller
			...
			Topic(href, href_list)
				...
				if(href_list["send"])
					...
					var
						new_frequency = (P.sradio.frequency + text2num(href_list["freq"]))
