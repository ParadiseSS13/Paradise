	...
	startup
		proc
			populate_pai_software_list()
				var
					r = 1 // I would use ., but it'd sacrifice runtime detection
		...
		silicon
			pai
				ui_interact(mob/user, ui_key = "main", datum/nanoui/ui = null, force_open = 1)
					...
					var
						not_bought_software[0]

					for(var/key in pai_software_by_key)
						var
							datum
								pai_software
									S = pai_software_by_key[key]
						var
							software_data[0]
			...
			pai
				Topic(href, href_list)
					...
					if(href_list["software"])
						...
						var
							soft = href_list["purchase"]
						var
							datum
								pai_software
									S = pai_software_by_key[soft]
						if(S && (ram >= S.ram_cost))
							ram -= S.ram_cost
							software[S.id] = S
						return 1
