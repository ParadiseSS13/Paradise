...
/mob/living/silicon/proc/run_armor(damage_amount, damage_type, damage_flag = 0, attack_dir, armor_penetration_flat, armor_penetration_percentage)
	...
	if(armor > 0)		//Only apply weak-against-armor/hollowpoint effects if there actually IS armor.
		armor = max(0, (armor - armor_penetration_flat) * (MAX_ARMOR_PERCENTAGE - armor_penetration_percentage) / MAX_ARMOR_PERCENTAGE)
	return round(damage_amount * (MAX_ARMOR_PERCENTAGE - armor) / MAX_ARMOR_PERCENTAGE, DAMAGE_PRECISION)
