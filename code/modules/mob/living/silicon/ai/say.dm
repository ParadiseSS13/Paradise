				...
				verb
					announcement_help()
						...
						var
							datum
								browser
									popup = new(src, "announce_help", "Announcement Help", 500, 400)
			...
			ai
				proc
					ai_announcement()
						...
						var
							message = input(src, "WARNING: Misuse of this verb can result in you being job banned. More help is available in 'Announcement Help'", "Announcement", last_announcement) as text|null
...
proc
	play_vox_word(var/word, var/z_level, var/mob/only_listener)
		...
		if(vox_sounds[word])
			...
			var
				sound
					voice = sound(sound_file, wait = 1, channel = VOX_CHANNEL)
