...
/mob/living/silicon/ai/proc/ai_announcement_text()
	...
	var/input = tgui_input_text(usr, "Please write a message to announce to the station crew.", "A.I. Announcement", multiline = TRUE, encode = FALSE, submit_on_enter = FALSE)
	if(!input)
		return
...
/mob/living/silicon/ai/proc/ai_call_shuttle()
	...
	var/input = tgui_input_text(src, "Please enter the reason for calling the shuttle.", "Shuttle Call Reason", multiline = TRUE, encode = FALSE, submit_on_enter = FALSE)
	if(!input || stat)
		return
