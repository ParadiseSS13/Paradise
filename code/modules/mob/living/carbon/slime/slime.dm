	...
	living
		carbon
			slime
				...
				var
					cores = 1 // the number of /obj/item/slime_extract's the slime has left inside
				var
					mutation_chance = 30 // Chance of mutating, should be between 25 and 35

				var
					powerlevel = 0 // 1-10 controls how much electricity they are generating
				var
					amount_grown = 0 // controls how long the slime has been overfed, if 10, grows or reproduces

				var
					number = 0 // Used to understand when someone is talking to it

				var
					mob
						living
							Victim = null // the person the slime is currently feeding on
				var
					mob
						living
							Target = null // AI variable - tells the slime to hunt this down
				var
					mob
						living
							Leader = null // AI variable - tells the slime to follow this person

				var
					attacked = 0 // Determines if it's been attacked recently. Can be any number, is a cooloff-ish variable
				var
					rabid = 0 // If set to 1, the slime will attack and eat anything it comes in contact with
				var
					holding_still = 0 // AI variable, cooloff-ish for how long it's going to stay in one place
				var
					target_patience = 0 // AI variable, cooloff-ish for how long it's going to follow its target

				var
					list
						Friends = list() // A list of friends; they are not considered targets for feeding; passed down after splitting

				var
					list
						speech_buffer = list() // Last phrase said near it and person who said it

				var
					mood = "" // To show its face
				var
					is_adult = 0

				var
					core_removal_stage = 0 //For removing cores.
				var
					mutator_used = FALSE //So you can't shove a dozen mutators into a single slime
			...
			slime
				New()
					...
					spawn (0)
						...
						var
							sanitizedcolour = replacetext(colour, " ", "")
