mob
	living
		carbon
			slime
				var
					AIproc = 0 // determines if the AI loop is activated
				var
					Atkcool = 0 // attack cooldown
				var
					Tempstun = 0 // temporary temperature stuns
				var
					Discipline = 0 // if a slime has been hit with a freeze gun, or wrestled/attacked off a human, they become disciplined and don't attack anymore for a while
				var
					SStun = 0 // stun variable
				...
				proc
					handle_targets()
						...
						if(!client)
							...
							var
								hungry = 0 // determines if the slime is hungry
				...
				proc
					handle_speech_and_mood()
						...
						if(speech_buffer.len > 0)
							var
								who = speech_buffer[1] // Who said it?
							var
								phrase = speech_buffer[2] // What did they say?
						...
						if(to_say)
							...
							var
								t = 10
							var
								slimes_near = -1 // Don't count myself
							var
								dead_slimes = 0
							var
								friends_near = list()
...
mob
	living
		carbon
			slime
				proc
					will_hunt(var/hunger = -1) // Check for being stopped from feeding and chasing
