...
/obj/effect/proc_holder/spell/recruit/cast(list/targets,mob/living/user = usr)
	...
	for(var/progress = 0, progress <= 3, progress++)
		switch(progress)
			...
			if(2)
				...
				if(ismindshielded(target))
					...
					to_chat(target, "<span class='danger'>Your mindshield microchip activates, protecting you from conversion.</span>")
					return
			if(3)
		...
		if(!do_mob(user, target, 100)) //around 30 seconds total for enthralling, 45 for someone with a mindshield microchip
