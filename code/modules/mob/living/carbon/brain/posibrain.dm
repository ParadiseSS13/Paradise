		...
		device
			mmi
				posibrain
					...
					var
						silenced = 0 //if set to 1, they can't talk.
					...
					proc
						question(var/client/C)
							spawn(0)
								if(!C)	return
								var
									response = alert(C, "Someone is requesting a personality for a positronic brain. Would you like to play as one?", "Positronic brain request", "Yes", "No", "Never for this round")
	...
	item
		device
			mmi
				posibrain
					proc
						transfer_personality(var/mob/candidate)
							src.searching = 0
	...
	item
		device
			mmi
				posibrain
					proc
						reset_search() //We give the players sixty seconds to decide, then reset the timer.
