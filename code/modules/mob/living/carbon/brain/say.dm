...
mob
	living
		carbon
			brain
				say(var/message, var/datum/language/speaking = null)
					if(silent)
						return
			...
			brain
				handle_message_mode(var/message_mode, var/message, var/verb, var/speaking, var/used_radios, var/alt_name)
					switch(message_mode)
						if("headset")
							var
								radio_worked = 0 // If any of the radios our brainmob could use functioned, this is set true so that we don't use any others
