		...
		carbon
			human
				examine(mob/user)
					...
					var
						t_He = "It" //capitalised for use at the start of each line.
					var
						t_his = "its"
					var
						t_him = "it"
					...
					var
						list
							nospecies = list("Abductor", "Shadowling", "Neara", "Monkey", "Stok", "Farwa", "Wolpin") //species that won't show their race no matter what

					var
						displayed_species = get_species()
					...
					for(var/organ_tag in species.has_limbs)
						...
						var
							list
								organ_data = species.has_limbs[organ_tag]
						var
							organ_descriptor = organ_data["descriptor"]
						...
						if(temp && !temp.is_stump())
							if(temp.status & ORGAN_ROBOT)
								...
								if(wound_descriptors.len)
									...
									var
										list
											no_exclude = list("gaping wound", "big gaping wound", "massive wound", "large bruise",\
						...
						organ
							internal
								cyberimp
									eyes
										hud
											CIH = H.get_int_organ(/obj/item/organ/internal/cyberimp/eyes/hud)
			switch(hudtype)
				if("security")
