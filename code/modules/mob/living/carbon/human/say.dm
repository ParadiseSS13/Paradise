...
mob
	living
		carbon
			human
				proc
					forcesay(list/append)
						if(stat == CONSCIOUS)
							if(client)
								var
									virgin = 1	//has the text been modified yet?
								var
...
mob
	living
		carbon
			human
				handle_speech_problems(var/message, var/verb)
					var
						list
							returns[3]
					var
...
mob
	living
		carbon
			human
				handle_message_mode(var/message_mode, var/message, var/verb, var/speaking, var/used_radios, var/alt_name)
					switch(message_mode)
						if("intercom")
