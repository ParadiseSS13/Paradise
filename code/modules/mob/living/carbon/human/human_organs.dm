							...
							obj
								item
									organ
										internal
											eyes
												eyes = get_int_organ(/obj/item/organ/internal/eyes)
						if(eyes)
							eyes.update_colour()
							regenerate_icons()
	...
	living
		carbon
			human
				var
					list
						organs_by_name = list() // map organ names to organs
				...
				proc
					handle_organs()
						...
						var
							force_process = 0
						var
							damage_this_tick = getBruteLoss() + getFireLoss() + getToxLoss()
					...
					handle_grasp()
						...
						for(var/obj/item/organ/external/E in organs)
							...
							if(E.is_broken())
								...
								var
									emote_scream = pick("screams in pain and ", "lets out a sharp cry and ", "cries out and ")
...
mob
	living
		carbon
			human
				proc
					sync_organ_dna(var/assimilate = 1, var/old_ue = null)
						var
							ue_to_compare = (old_ue) ? old_ue : dna.unique_enzymes
						var
							list
								all_bits = internal_organs|organs
...
mob
	living
		carbon
			human
				proc
					named_organ_parent(var/organ_name)
