...
var
	global
		list
			body_accessory_by_name = list("None" = null)
...
var
	global
		list
			body_accessory_by_species = list("None" = null)
...
proc
	__init_body_accessory(var/ba_path)
		if(ispath(ba_path))
			var
				_added_counter = 0
...
datum
	body_accessory
		proc
			try_restrictions(var/mob/living/carbon/human/H)
				return 1

datum
	body_accessory
		proc
			get_animated_icon() //return animated if it has it, return static if it does not.
				if(animated_icon)
