...
/mob/living/carbon/human/bullet_act(obj/item/projectile/P, def_zone)
	...
	if(mind?.martial_art?.deflection_chance) //Some martial arts users can deflect projectiles!
		if(!lying && !HAS_TRAIT(src, TRAIT_HULK) && !HasDisease(new /datum/disease/memetic_madness(0)) && prob(mind.martial_art.deflection_chance)) //But only if they're not lying down, and hulks can't do it
