...
/mob/living/carbon/human/proc/__getarmor_bodypart(obj/item/organ/external/def_zone, armor_type)
	...
	var/protection
	var/protection_to_add

	for(var/obj/item/thing as anything in covers_body)
		if(thing?.body_parts_covered & def_zone.body_part)
			protection_to_add = PERCENTAGE_TO_ARMOR_VALUE(thing.armor.getRating(armor_type))
			if(protection_to_add >= INFINITY)
				return MAX_ARMOR_VALUE
			protection += protection_to_add

	protection_to_add = PERCENTAGE_TO_ARMOR_VALUE(physiology.armor.getRating(armor_type))
	if(protection_to_add >= INFINITY)
		return MAX_ARMOR_VALUE
	protection += protection_to_add

	return ARMOR_VALUE_TO_PERCENTAGE(protection)
...
/mob/living/carbon/human/attacked_by(obj/item/I, mob/living/user, def_zone)
	...
	var/armor = run_armor_check(
		def_zone = affecting,
		armor_type = MELEE,
		absorb_text = "<span class='warning'>Your armour has protected your [hit_area].</span>",
		soften_text = "<span class='warning'>Your armour has softened hit to your [hit_area].</span>",
		armor_penetration_flat = I.armor_penetration_flat,
		armor_penetration_percentage = I.armor_penetration_percentage,
	)
	if(armor >= MAX_ARMOR_PERCENTAGE)
		return
