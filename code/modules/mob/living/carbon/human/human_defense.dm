...
mob
	living
		carbon
			human
				bullet_act(var/obj/item/projectile/P, var/def_zone)
					...
					if(istype(P, /obj/item/projectile/energy) || istype(P, /obj/item/projectile/beam))
						if(check_reflect(def_zone)) // Checks if you've passed a reflection% check
							...
							if(P.starting)
								var
									new_x = P.starting.x + pick(0, 0, 0, 0, 0, -1, 1, -2, 2)
								var
									new_y = P.starting.y + pick(0, 0, 0, 0, 0, -1, 1, -2, 2)
								var
									turf
										curloc = get_turf(src)
...
mob
	living
		carbon
			human
				getarmor(var/def_zone, var/type)
					var
						armorval = 0
					var
						organnum = 0
...
mob
	living
		carbon
			human
				proc
					getarmor_organ(var/obj/item/organ/external/def_zone, var/type)
						if(!type || !def_zone)	return 0
						var
							protection = 0
						var
							list
								body_parts = list(head, wear_mask, wear_suit, w_uniform, back, gloves, shoes, belt, s_store, glasses, l_ear, r_ear, wear_id) //Everything but pockets. Pockets are l_store and r_store. (if pockets were allowed, putting something armored, gloves or hats for example, would double up on the armor)
...
mob
	living
		carbon
			human
				proc
					get_siemens_coefficient_organ(var/obj/item/organ/external/def_zone)
						if(!def_zone)
							return 1.0
						...
						var
							list
								clothing_items = list(head, wear_mask, wear_suit, w_uniform, gloves, shoes) // What all are we checking?
			...
			human
				proc
					check_head_coverage()
						...
						var
							list
								body_parts = list(head, wear_mask, wear_suit, w_uniform)
						for(var/bp in body_parts)
							if(!bp)  continue
							if(bp && istype(bp ,/obj/item/clothing))
								var
...
mob
	living
		carbon
			human
				proc
					check_reflect(var/def_zone) //Reflection checks for anything in your l_hand, r_hand, or wear_suit based on the reflection chance var of the object
...
mob
	living
		carbon
			human
				proc
					check_shields(var/damage = 0, var/attack_text = "the attack", var/obj/item/O)
						if(O)
...
mob
	living
		carbon
			human
				emag_act(user as mob, var/obj/item/organ/external/affecting)
					if(!istype(affecting))
						return
...
mob
	living
		carbon
			human
				proc
					attacked_by(var/obj/item/I, var/mob/living/user, var/def_zone)
						if(!I || !user)	return 0
											...
											reagent_containers
												food
													snacks
														meat
															human
																newmeat = new /obj/item/weapon/reagent_containers/food/snacks/meat/human(get_turf(src.loc))
						...
						var
							armor = run_armor_check(affecting, "melee", "Your armor has protected your [hit_area].", "Your armor has softened hit to your [hit_area].", armour_penetration = I.armour_penetration)
						var
							weapon_sharp = is_sharp(I)
						var
							weapon_edge = has_edge(I)
						...
						if(!I.force)	return 0
						var
							Iforce = I.force //to avoid runtimes on the forcesay checks at the bottom. Some items might delete themselves if you drop them. (stunning yourself, ninja swords)
...
/*	//Melee weapon embedded object code. Commented out, as most people on the forums seem to find this annoying and think it does not contribute to general gameplay. - Dave
	if(I.damtype == BRUTE && !I.is_robot_module())
		...
		var
			embed_chance = weapon_sharp? damage/I.w_class : damage/(I.w_class*3)
		var
			embed_threshold = weapon_sharp? 5*I.w_class : 15*I.w_class
...
mob
	living
		carbon
			human
				hitby(atom/movable/AM as mob|obj,var/speed = 5)
					if(istype(AM,/obj/))
						var
							obj
								O = AM
						...
						if(istype(O,/obj/item/weapon))
							...
							dtype = W.damtype
						var
							throw_damage = O.throwforce*(speed/5)
						...
						*/
						...
						var
							obj
								item
									organ
										external
											affecting = get_organ(zone)
						if(!affecting)
							var
								missverb = (O.gender == PLURAL) ? "whizz" : "whizzes"
						...
						var
							armor = run_armor_check(affecting, "melee", "Your armor has protected your [hit_area].", "Your armor has softened hit to your [hit_area].") //I guess "melee" is the best fit here
						...
						if(dtype == BRUTE && istype(O,/obj/item))
							...
							if(!I.is_robot_module())
								...
								//blunt objects should really not be embedding in things unless a huge amount of force is involved
								var
									embed_chance = sharp? damage/I.w_class : damage/(I.w_class*3)
								var
									embed_threshold = sharp? 5*I.w_class : 15*I.w_class
						...
						if(O.throw_source && speed >= 15)
							...
							var
								momentum = speed/2
							var
								dir = get_dir(O.throw_source, src)
...
mob
	living
		carbon
			human
				proc
					bloody_hands(var/mob/living/source, var/amount = 2)
...
mob
	living
		carbon
			human
				proc
					bloody_body(var/mob/living/source)
						if(wear_suit)
...
mob
	living
		carbon
			human
				proc
					handle_suit_punctures(var/damtype, var/damage)
									...
									clothing
										suit
											space
												SS = wear_suit
						var
							penetrated_dam = max(0,(damage - max(0,(SS.breach_threshold - SS.damage))))
						...
						var
							obj
								item
									organ
										external
											affecting = get_organ(pick("chest", "chest", "chest", "head"))
						if(affecting)
							var
								update = 0
							switch(M.damtype)
