		...
		carbon
			human
				...
				var
					pressure_alert = 0
				var
					prev_gender = null // Debug for plural genders
				var
					temperature_alert = 0
				var
					in_stasis = 0
				...
				var
					heartbeat = 0
				var
					tinttotal = 0				// Total level of visually impairing items
		...
		carbon
			human
				calculate_affecting_pressure(var/pressure)
					..()
					var
						pressure_difference = abs( pressure - ONE_ATMOSPHERE )

					var
						pressure_adjustment_coefficient = 1	//Determins how much the clothing you are wearing protects you in percent.
				...
				handle_disabilities()
					...
					if(disabilities & TOURETTES)
						...
						if((prob(10) && paralysis <= 1))
							...
							switch(rand(1, 3))
								...
								if(2 to 3)
									var
										tourettes = pick("SHIT", "PISS", "FUCK", "CUNT", "COCKSUCKER", "MOTHERFUCKER", "TITS")
									say("[prob(50) ? ";" : ""][tourettes]")
							var
								x_offset = pixel_x + rand(-2,2) //Should probably be moved into the twitch emote at some point.
							var
								y_offset = pixel_y + rand(-1,1)
					...
					if(getBrainLoss() >= 60 && stat != 2)
						...
						if(prob(3))
							var
								list
									s1 = list("IM A PONY NEEEEEEIIIIIIIIIGH",
			...
			human
				handle_mutations_and_radiation()
					...
					if(gene_stability < 85)
						var
							instability = DEFAULT_GENE_STABILITY - gene_stability
...
mob
	living
		carbon
			human
				get_breath_from_internal(volume_needed) //making this call the parent would be far too complicated

					if(internal)
						var
							null_internals = 0      //internals are invalid, therefore turn them off
						var
							skip_contents_check = 0 //rigsuit snowflake, oxygen tanks aren't stored inside the mob, so the 'contents.Find' check has to be skipped.
							...
							var
								obj
									item
										weapon
											rig
												rig = back //needs to be typecasted because this doesn't use get_rig() for some reason
...
mob
	living
		carbon
			human
				check_breath(var/datum/gas_mixture/breath)
...
//	to_chat(world, "Loc temp: [loc_temp] - Body temp: [bodytemperature] - Fireloss: [getFireLoss()] - Thermal protection: [get_thermal_protection()] - Fire protection: [thermal_protection + add_fire_protection(loc_temp)] - Heat capacity: [environment_heat_capacity] - Location: [loc] - src: [src]")
	...
	if(!on_fire) //If you're on fire, you do not heat up or cool down based on surrounding gases
		if(loc_temp < bodytemperature)
			...
			var
				thermal_protection = get_cold_protection(loc_temp) //This returns a 0 - 1 value, which corresponds to the percentage of protection based on what you're wearing and what you're exposed to.
			if(thermal_protection < 1)
				bodytemperature += min((1-thermal_protection) * ((loc_temp - bodytemperature) / BODYTEMP_COLD_DIVISOR), BODYTEMP_COOLING_MAX)
		else
			//Place is hotter than we are
			var
				thermal_protection = get_heat_protection(loc_temp) //This returns a 0 - 1 value, which corresponds to the percentage of protection based on what you're wearing and what you're exposed to.
			if(thermal_protection < 1)
	...
	if(bodytemperature > species.heat_level_1)
		...
		var
			mult = species.hot_env_multiplier
		...
		if(!istype(loc, /obj/machinery/atmospherics/unary/cryo_cell))
			var
				mult = species.cold_env_multiplier
	...
	var
		adjusted_pressure = calculate_affecting_pressure(pressure) //Returns how much pressure actually affects the mob.
	if(status_flags & GODMODE)	return 1	//godmode
				...
				proc
					adjust_body_temperature(current, loc_temp, boost)
						var
							temperature = current
						var
							difference = abs(current-loc_temp)	//get difference
						var
							increments// = difference/10			//find how many increments apart they are
						if(difference > 50)
							increments = difference/5
						else
							increments = difference/10
						var
							change = increments*boost	// Get the amount to change by (x per increment)
						var
							temp_change
						if(current < loc_temp)
					...
					stabilize_temperature_from_calories()
						if(bodytemperature <= species.cold_level_1) //260.15 is 310.15 - 50, the temperature where you start to feel effects.
							if(nutrition >= 2) //If we are very, very cold we'll use up quite a bit of nutriment to heat us up.
								nutrition -= 2
							var
								body_temperature_difference = species.body_temperature - bodytemperature
...
mob
	living
		carbon
			human
				proc
					get_heat_protection_flags(temperature) //Temperature is the temperature you're being exposed to.
						var
			...
			human
				proc
					get_heat_protection(temperature) //Temperature is the temperature you're being exposed to.
						...
						var
							thermal_protection_flags = get_heat_protection_flags(temperature)
...
mob
	living
		carbon
			human
				proc
					get_cold_protection_flags(temperature)
						var
							thermal_protection_flags = 0
						//Handle normal clothing
			...
			human
				proc
					get_cold_protection(temperature)
						...
						var
							thermal_protection_flags = get_cold_protection_flags(temperature)
	...
	living
		carbon
			human
				handle_drunk()
					var
						slur_start = 30 //12u ethanol, 30u whiskey FOR HUMANS
					var
						confused_start = 40
					var
						brawl_start = 30
					...
					var
						pass_out = 90
					var
						spark_start = 50 //40u synthanol
					var
						collapse_start = 75
					var
						braindamage_start = 120
					var
						alcohol_strength = drunk
					var
						sober_str=!(SOBER in mutations)?1:2
...
mob
	living
		carbon
			human
				proc
					has_booze() //checks if the human has ethanol or its subtypes inside
				...
				handle_vision()
					if(machine)
						...
						if((REMOTE_VIEW in mutations) && remoteview_target)
							...
							var
								turf
									temp_turf = get_turf(remoteview_target)
					...
					if(isturf(loc) && rand(1,1000) == 1)
						...
						var
							atom
								movable
									lighting_overlay
										L = locate(/atom/movable/lighting_overlay) in currentTurf
		...
		carbon
			human
				proc
					handle_decay()
						var
							decaytime = world.time - timeofdeath
						...
						if(C && C.prefs.sound & SOUND_HEARTBEAT) //disable heartbeat by pref
								...
								obj
									item
										organ
											internal
												heart
													H = get_int_organ(/obj/item/organ/internal/heart)
							...
							if(H.status & ORGAN_ROBOT) //Handle robotic hearts specially with a wuuuubb. This also applies to machine-people.
								if(shock_stage >= 10 || istype(get_turf(src), /turf/space))
									...
									var
										rate = (PULSE_THREADY - 2)/2 //machine people (main target) have no pulse, manually subtract standard human pulse (2). Mechanic-heart humans probably have a pulse, but 'advanced neural systems' keep the heart rate steady, or something
