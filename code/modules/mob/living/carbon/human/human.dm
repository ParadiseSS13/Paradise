	...
	living
		carbon
			human
				...
				var
					embedded_flag	  //To check if we've need to roll for damage on movement while an item is imbedded in us.
				var
					obj
						item
							weapon
								rig
									wearing_rig // This is very not good, but it's much much better than calling get_rig() every update_canmove() call.

mob
	living
		carbon
			human
				New(var/new_loc, var/new_species = null, var/delay_ready_dna = 0)
					if(!dna)
					...
					switch(severity)
						if(1)
							...
							if(!prob(getarmor(null, "bomb")))
								...
								var
									atom
										target = get_edge_target_turf(src, get_dir(src, get_step_away(src, src)))
								...
								var
									obj
										item
											organ
												external
													processing_dismember
								var
									list
										valid_limbs = organs.Copy()
		...
		carbon
			human
				blob_act()
					...
					var
						dam_zone = pick("head", "chest", "groin", "l_arm", "l_hand", "r_arm", "r_hand", "l_leg", "l_foot", "r_leg", "r_foot")
					var
						obj
							item
								organ
								...
								item
									organ
										external
											affecting = get_organ(ran_zone(dam_zone))
						var
							armor = run_armor_check(affecting, "melee")
						...
						var
							obj
								item
									organ
										external
											affected = src.get_organ(dam_zone)
						if(affected)
							...
							var
								obj
									item
										organ
											external
												affecting = get_organ(ran_zone(L.zone_sel.selecting))
							var
		...
		carbon
			human
				show_inv(mob/user)
					...
					var
						has_breathable_mask = istype(wear_mask, /obj/item/clothing/mask)
					var
						list
					...
					var
						datum
							browser
								popup = new(user, "mob\ref[src]", "[src]", 440, 540)
...
mob
	living
		carbon
			human
				proc
					get_authentification_rank(var/if_no_id = "No id", var/if_no_job = "No job")
						var
							obj
								item
									device
...
mob
	living
		carbon
			human
				proc
					get_assignment(var/if_no_id = "No id", var/if_no_job = "No job")
						var
							obj
								item
									device
...
mob
	living
		carbon
			human
				proc
					get_authentification_name(var/if_no_id = "Unknown")
						var
							obj
								item
									device
...
mob
	living
		carbon
			human
				proc
					get_id_name(var/if_no_id = "Unknown")
						var
							obj
								item
									device
...
mob
	living
		carbon
			human
				electrocute_act(shock_damage, obj/source, siemens_coeff = 1, safety = 0, override = 0, tesla_shock = 0)
				...
				Topic(href, href_list)
					if(!usr.stat && usr.canmove && !usr.restrained() && in_range(src, usr))
						...
						if(href_list["pockets"])
							...
							var
								pocket_id = (pocket_side == "right" ? slot_r_store : slot_l_store)
							var
								obj
									item
										pocket_item = (pocket_id == slot_r_store ? r_store : l_store)
							var
								obj
									item
										place_item = usr.get_active_hand() // Item to place in the pocket, if it's empty
									...
									if(E.fields["name"] == perpname)
										for(var/datum/data/record/R in data_core.security)
											if(R.fields["id"] == E.fields["id"])
												...
												var
													setcriminal = input(usr, "Specify a new criminal status for this person.", "Security HUD", R.fields["criminal"]) in list("None", "*Arrest*", "Incarcerated", "Parolled", "Released", "Cancel")
								...
								if(E.fields["name"] == perpname)
									for(var/datum/data/record/R in data_core.security)
										if(R.fields["id"] == E.fields["id"])
											if(hasHUD(usr,"security"))
												var
													t1 = sanitize(copytext(input("Add Comment:", "Sec. records", null, null)  as message,1,MAX_MESSAGE_LEN))
							...
							for(var/datum/data/record/E in data_core.general)
								if(E.fields["name"] == perpname)
									for(var/datum/data/record/R in data_core.general)
										if(R.fields["id"] == E.fields["id"])
											var
												setmedical = input(usr, "Specify a new medical status for this person.", "Medical HUD", R.fields["p_stat"]) in list("*SSD*", "*Deceased*", "Physically Unfit", "Active", "Disabled", "Cancel")
					...
					if(href_list["lookitem"])
						var
							obj
								item
									I = locate(href_list["lookitem"])
						src.examinate(I)

					if(href_list["lookmob"])
						var
							mob
								M = locate(href_list["lookmob"])
						src.examinate(M)
					. = ..()
						...
						var
							obj
								item
									clothing
										head
											HFP = head			//if yes gets the flash protection value from that item
				...
				proc
					play_xylophone()
						if(!src.xylophone)
							...
							var
								song = pick('sound/effects/xylophone1.ogg','sound/effects/xylophone2.ogg','sound/effects/xylophone3.ogg')
...
mob
	living
		carbon
			human
				can_inject(var/mob/user, var/error_msg, var/target_zone, var/penetrate_thick = 0)
					. = 1
...
mob
	living
		carbon
			human
				proc
					check_and_regenerate_organs(var/mob/living/carbon/human/H) //Regenerates missing limbs/organs.
						var
							list
								types_of_int_organs = list() //This will hold all the types of organs in the mob before rejuvenation.
						...
						for(var/organ_name in H.organs_by_name)
							var
								obj
									item
										organ
											organ = H.organs_by_name[organ_name]
						...
						for(var/limb_type in H.species.has_limbs)
							if(!(limb_type in H.organs_by_name))
								var
									list
										organ_data = H.species.has_limbs[limb_type]
								var
									limb_path = organ_data["path"]
								var
									obj
										item
											organ
												external
													O = new limb_path(temp_holder)
						...
						for(var/index in H.species.has_organ)
							var
								organ = H.species.has_organ[index]
								...
								var
									obj
										item
											organ
												internal
													I = new organ(temp_holder) //Create the organ inside our holder so we can check it before implantation.
							...
							obj
								item
									organ
										internal
											lungs
												L = get_int_organ(/obj/item/organ/internal/lungs)
						if(!L)
							return 0
			...
			human
				verb
					simulate()
						...
						var
							damage = input("Wound damage","Wound damage") as num
...
mob
	living
		carbon
			human
				get_visible_implants(var/class = 0)

					var
						list
							visible_implants = list()
...
mob
	living
		carbon
			human
				proc
					set_species(var/new_species, var/default_colour, var/delay_icon_update = 0)
			...
			human
				proc
					bloody_doodle()
						...
						var
							direction = input(src,"Which way?","Tile selection") as anything in list("Here","North","South","East","West")
						if(direction != "Here")
						...
						var
							max_length = bloody_hands * 30 //tweeter style

						var
							message = stripped_input(src,"Write a message. It cannot be longer than [max_length] characters.","Blood writing", "")

						if(message)
							var
								used_blood_amount = round(length(message) / 30, 1)
					...
					change_monitor()
						...
						if(species.flags & ALL_RPARTS) //If they can have a fully cybernetic body...
							var
								datum
									robolimb
										robohead = all_robolimbs[head_organ.model]
							if(!head_organ)
								return
							if(!robohead.is_monitor) //If they've got a prosthetic head and it isn't a monitor, they've no screen to adjust. Instead, let them change the colour of their optics!
								var
									optic_colour = input(src, "Select optic colour", rgb(r_markings, g_markings, b_markings)) as color|null
								if(incapacitated())
								...
								var
									new_style = input(src, "Select a monitor display", "Monitor Display", head_organ.h_style)  as null|anything in hair
								if(incapacitated())
...
mob
	living
		carbon
			human
				assess_threat(var/mob/living/simple_animal/bot/secbot/judgebot, var/lasercolor)
					...
					if(judgebot.check_records)
						var
							perpname = get_face_name(get_id_name())
						var
							datum
								data
									record
		...
		carbon
			human
				get_permeability_protection()
					var
						list
							prot = list("hands"=0, "chest"=0, "groin"=0, "legs"=0, "feet"=0, "arms"=0, "head"=0)
...
mob
	living
		carbon
			human
				selfDrink(var/obj/item/weapon/reagent_containers/food/drinks/toDrink)
					if(!check_has_mouth())
...
mob
	living
		carbon
			human
				can_use_guns(var/obj/item/weapon/gun/G)
					. = ..()
