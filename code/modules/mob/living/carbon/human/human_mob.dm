...
/mob/living/carbon/human/narsie_act()
	if(iswizard(src) && src.mind.has_antag_datum(/datum/antagonist/cultist)) //Wizard cultists are immune to narsie because it would prematurely end the wiz round that's about to end by the automated shuttle call anyway
		return
	..()
