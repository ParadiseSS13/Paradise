...
mob
	living
		carbon
			human
				proc
					equip_in_one_of_slots(obj/item/W, list/slots, del_on_fail = 1)
					...
					var
						obj
							item
								organ
									internal
										O = I //Organs shouldn't be removed unless you call droplimb.
...
mob
	living
		carbon
			human
				equip_to_slot(obj/item/W as obj, slot, redraw_mob = 1)
					if(!slot) return
					if(!istype(W)) return
...
mob
	living
		carbon
			human
				stripPanelUnequip(obj/item/what, mob/who, where)
					var
						is_silent = 0
					var
						obj
...
mob
	living
		carbon
			human
				stripPanelEquip(obj/item/what, mob/who, where)
					var
						is_silent = 0
					var
						obj
