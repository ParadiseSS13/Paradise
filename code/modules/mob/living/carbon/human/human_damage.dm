							...
							obj
								item
									organ
										internal
											brain
												B = get_int_organ(/obj/item/organ/internal/brain)
						if(B)
							...
							obj
								item
									organ
										internal
											brain
												sponge = get_int_organ(/obj/item/organ/internal/brain)
						if(sponge)
...
mob
	living
		carbon
			human
				proc
					adjustBruteLossByPart(var/amount, var/organ_name, var/obj/damage_source = null)
						if(species && species.brute_mod)
							amount = amount*species.brute_mod
...
mob
	living
		carbon
			human
				proc
					adjustFireLossByPart(var/amount, var/organ_name, var/obj/damage_source = null)
						if(species && species.burn_mod)
							amount = amount*species.burn_mod
		...
		carbon
			human
				adjustCloneLoss(var/amount)
					...
					var
						heal_prob = max(0, 80 - getCloneLoss())
					var
						mut_prob = min(80, getCloneLoss() + 10)
								...
								list
									obj
										item
											organ
												external
													candidates = list() //TYPECASTED LISTS ARE NOT A FUCKING THING WHAT THE FUCK
							for(var/obj/item/organ/external/O in organs)
...
mob
	living
		carbon
			human
				proc
					get_damaged_organs(var/brute, var/burn, var/flags = AFFECT_ALL_ORGANS)
						var
							list
								obj
									item
...
mob
	living
		carbon
			human
				heal_organ_damage(var/brute, var/burn)
					var
						list
							obj
								item
...
mob
	living
		carbon
			human
				take_organ_damage(var/brute, var/burn, var/sharp = 0, var/edge = 0)
					var
						list
							obj
								item
...
mob
	living
		carbon
			human
				heal_overall_damage(var/brute, var/burn, var/internal=0, var/robotic=0)
					var
						list
							obj
								item
...
mob
	living
		carbon
			human
				take_overall_damage(var/brute, var/burn, var/sharp = 0, var/edge = 0, var/used_weapon = null)
					if(status_flags & GODMODE)
						return	//godmode
					var
						list
				...
				proc
					restore_blood()
						if(!(species.flags & NO_BLOOD))
							var
								blood_volume = vessel.get_reagent_amount("blood")
...
mob
	living
		carbon
			human
				apply_damage(var/damage = 0, var/damagetype = BRUTE, var/def_zone = null, var/blocked = 0, var/sharp = 0, var/edge = 0, var/obj/used_weapon = null)
							...
							if(LAssailant && ishuman(LAssailant)) //superheros still get the comical hit markers
								...
								if(H.mind && H.mind in (ticker.mode.superheroes || ticker.mode.supervillains || ticker.mode.greyshirts))
									...
									spawn(0)
										var
											image
												dmgIcon = image('icons/effects/hit_blips.dmi', src, "dmg[rand(1,2)]",MOB_LAYER+1)
