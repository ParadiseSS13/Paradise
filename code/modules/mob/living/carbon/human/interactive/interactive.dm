		...
		carbon
			human
				interactive
					...
					var
						list
							blacklistItems = list() // items we should be ignoring
					var
						maxStepsTick = 6 // step as many times as we can per frame
					//Job and mind data
					var
						obj
							item
								...
								weapon
									card
										id
											Path_ID
					var
						default_job = /datum/job/civilian	// the type for the default job
					var
						datum
							job
								myjob
					...
					var
						list
							favoured_types = list() // allow a mob to favour a type, and hold onto them
					var
						chattyness = SNPC_CHANCE_TALK
					var
						targetInterestShift = 5 // how much a good action should "reward" the npc
					//modules
					var
						list
							functions = list("nearbyscan","combat","shitcurity","chatter")
					var
						restrictedJob = 0
					var
						shouldUseDynamicProc = 0 // switch to make the AI control it's own proccessing
					var
						alternateProcessing = 0
					var
						forceProcess = 0
					var
						processTime = 10
					var
						speak_file = "npc_chatter.json"
					...
					var
						traitorTarget
					var
						traitorScale = 0 // our ability as a traitor
					var
						traitorType = 0
				...
				interactive
					proc
						loadVoice()
							var
								savefile
									S = new /savefile("data/npc_saves/snpc.sav")
	...
	living
		carbon
			human
				interactive
					proc
						takeDelegate(mob/living/carbon/human/interactive/from,doReset=TRUE)
...
client
	proc
		resetSNPC(mob/living/carbon/human/interactive/T in npc_master.botPool_l)
			set name = "Reset SNPC"
...
client
	proc
		toggleSNPC(mob/living/carbon/human/interactive/T in npc_master.botPool_l)
...
client
	proc
		customiseSNPC(mob/living/carbon/human/interactive/T in npc_master.botPool_l)
			...
			jobs = sortAssoc(jobs)
			var
				job_name = input("Choose Job") as null|anything in jobs
				...
				var
					datum
						job
							cjob = jobs[job_name]
				var
					alt_title = input("Choose Alt Title") in (list(job_name) + cjob.alt_titles)
			...
			var
				shouldDoppel = input("Do you want the SNPC to disguise themself as a crewmember?") as anything in list("Yes", "No")
			if(shouldDoppel == "Yes")
				var
					list
						validchoices = list()
				...
				var
					mob
						living
							carbon
								human
									chosen = input("Which crewmember?") as null|anything in validchoices
			...
			var
				doTrait = input("Do you want the SNPC to be a traitor?") as anything in list("Yes", "No")
			if(doTrait == "Yes")
				var
					list
						tType = list("Brute" = SNPC_BRUTE, "Stealth" = SNPC_STEALTH, "Martyr" = SNPC_MARTYR, "Psycho" = SNPC_PSYCHO)
				var
					cType = input("Choose the traitor personality.") as null|anything in tType
				if(cType)
					var
						value = tType[cType]
					T.makeTraitor(value)
			...
			var
				doTele = input("Place the SNPC in their department?") as anything in list("Yes", "No")
			if(doTele == "Yes")
						...
						makeTraitor(var/inPers)
							...
							switch(traitorType)
								...
								if(SNPC_MARTYR) // MY LIFE FOR SPESZUL
									var
										targetType = pick(/obj/structure/particle_accelerator, /obj/machinery/gravity_generator/main, /obj/machinery/power/smes)
									traitorTarget = locate(targetType) in world
									var
										datum
											objective
												O = new("Destroy \the [traitorTarget].")
									O.owner = mind
									mind.objectives += O
...
mob
	living
		carbon
			human
				interactive
					attacked_by(obj/item/I, mob/living/user, def_zone)
						..()
						retalTarget(user)
...
mob
	living
		carbon
			human
				interactive
					hitby(atom/movable/AM)
						..()
						var
							mob
								living
									carbon
										C = locate(/mob/living/carbon) in view(SNPC_MIN_RANGE_FIND, src)
						if(C)
							retalTarget(C)
...
mob
	living
		carbon
			human
				interactive
					can_inject(mob/user, error_msg, target_zone, var/penetrate_thick = 0)
						..()
						retalTarget(user)
	...
	living
		carbon
			human
				interactive
					proc
						take_to_slot(obj/item/G, var/hands=0)
							var
								list
									slots = list("left pocket" = slot_l_store, "right pocket" = slot_r_store, "left hand" = slot_l_hand, "right hand" = slot_r_hand)
							if(hands)
				...
				interactive
					proc
						insert_into_backpack()
							var
								list
									slots = list(slot_l_store, slot_r_store, slot_l_hand, slot_r_hand)
							var
								obj
									item
										I = get_item_by_slot(pick(slots))
							var
								obj
									item
										weapon
											storage
												BP = get_item_by_slot(slot_back)
							if(back && BP && I)
...
mob
	living
		carbon
			human
				interactive
					hear_say(message, verb = "says", datum/language/language = null, alt_name = "", italics = 0, mob/speaker = null, sound/speech_sound, sound_vol)
...
mob
	living
		carbon
			human
				interactive
					hear_radio(message, verb = "says", datum/language/language=null, part_a, part_b, mob/speaker = null, hard_to_hear = 0, vname = "", atom/follow_target)
						if(!istype(speaker, /mob/living/carbon/human/interactive))
							knownStrings |= html_decode(message)
						..()
				...
				interactive
					proc
						doProcess()
							...
							var
								mistake_chance = (100 - smartness) / 2
							spawn(0)
							...
							if(TARGET && Adjacent(TARGET))
								...
								if(istype(TARGET, /obj/machinery/door))
									...
									if(D.check_access(MYID) && !istype(D,/obj/machinery/door/poddoor))
										...
										var
											turf
												T = get_step(get_step(D.loc, dir), dir) //recursion yo
										tryWalk(T)
								//THIEVING SKILLS
	...
	living
		carbon
			human
				interactive
					proc
						getGoodPath(target, maxtries=512)
							set background = 1
							var
								turf
									end = get_turf(target)
				...
				interactive
					proc
						target_filter(target)
							var
								list
									filtered_targets = list(/area, /turf, /obj/machinery/door, /atom/movable/lighting_overlay, /obj/structure/cable, /obj/machinery/atmospherics, /obj/item/device/radio/intercom)
							var
								list
									L = target
	...
	living
		carbon
			human
				interactive
					proc
						shouldModulePass() // returns 1 if the npc is in anything "primary"
				...
				interactive
					proc
						enforceHome()
							var
								list
									validHome = get_area_turfs(job2area(myjob))
	...
	living
		carbon
			human
				interactive
					proc
						npcDrop(obj/item/A, blacklist = 0)
							if(blacklist)
								blacklistItems += A
							unEquip(A)
							enforce_hands()
	...
	living
		carbon
			human
				interactive
					proc
						compareFaction(list/targetFactions)
							var
								hasSame = 0
