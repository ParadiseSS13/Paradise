				...
				interactive
					proc
						chatter(obj)
							...
							var
								nouns_insult = pick_list(speak_file, "nouns_insult")
							var
								nouns_generic = pick_list(speak_file, "nouns_generic")
							var
								nouns_objects = pick_list(speak_file, "nouns_objects")
							var
								nouns_body = pick_list(speak_file, "nouns_body")
							var
							...
							if(prob(traitorScale))
								if(!Adjacent(traitorTarget) && !(traitorType == SNPC_PSYCHO))
									...
									switch(traitorType)
										...
										if(SNPC_PSYCHO)
											var
												choice = pick(typesof(/obj/item/weapon/grenade/chem_grenade) - /obj/item/weapon/grenade/chem_grenade)
										...
										weapon
											reagent_containers
												glass
													beaker
														bluespace
															internalBeaker = locate(/obj/item/weapon/reagent_containers/glass/beaker/bluespace) in allContents
							var
								obj
									item
										weapon
											storage
												bag
													plants
														internalBag = locate(/obj/item/weapon/storage/bag/plants) in allContents
										...
										if(HP.harvest || HP.dead)
											...
											var
												obj
													item
														seeds
															SEED = new /obj/item/seeds/random(src)
										...
										weapon
											reagent_containers
												food
													snacks
														grown
															GF = locate(/obj/item/weapon/reagent_containers/food/snacks/grown) in view(12,src)
							if(GF)
								if(!Adjacent(GF))
									tryWalk(get_turf(GF))
								else
							...
							if(internalBag.contents.len)
								var
									obj
										machinery
											smartfridge
												SF = locate(/obj/machinery/smartfridge) in range(12, src)
								if(!Adjacent(SF))
									tryWalk(get_turf(SF))
								else
						...
						bartend(obj)
							...
							if(RT && serveTarget)
								if(!Adjacent(RT))
									...
									var
										drinkChoice = pick(typesof(/obj/item/weapon/reagent_containers/food/drinks) - /obj/item/weapon/reagent_containers/food/drinks)
									if(drinkChoice)
										var
											obj
												item
													weapon
														reagent_containers
															food
																drinks
																	D = new drinkChoice(get_turf(src))
										RT.attackby(D,src)
	...
	living
		carbon
			human
				interactive
					proc
						makerecipe(datum/recipe/R, allContents, rangeCheck)
							set background = 1
							...
							ingredients.Cut()
							var
								result = new R.result(get_turf(src))
						...
						souschef(obj)
							...
							try
								...
								for(var/customType in customableTypes)
									var
										A = locate(customType) in rangeCheck
									var
										obj
											item
												weapon
								...
								var
									obj
										machinery
											smartfridge
												SF = locate(/obj/machinery/smartfridge) in rangeCheck
								if(SF)
								...
								var
									ingredientZone = rangeCheck + allContents
								var
									highest_count = 0
								var
									datum
								...
								for(var/datum/recipe/R in available_recipes)
									if(R.check_items(ingredientZone) >= 0 && R.check_fruit(ingredientZone) >= 0)
										var
											count = (R.items ? R.items.len : 0) + (R.fruit ? R.fruit.len : 0)
											...
											item
												weapon
													reagent_containers
														food
															snacks
																newSnack = new chosenType(get_turf(src))
									TARGET = newSnack
								...
								if((graytide || (TRAITS & TRAIT_MEAN)) || retal)
									...
									if(retal)
										...
										var
											mob
												living
													M = locate(/mob/living) in oview(7, src)
										if(!M)
					...
					proc
						paperwork_sentence()
							...
							var
								curse_words = pick_list(speak_file, "curse_words")
							var
								result = pick("fired", "promoted", "demoted", "terminated", "incinerated", "sent to CentCom", "borged")
							...
							var
								obj
									item
										weapon
											paper
												P = locate() in allContents
							var
								obj
									item
										weapon
											stamp
												S = pickStamp(allContents)
							var
								mob
									living
										carbon
