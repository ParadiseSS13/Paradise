		...
		carbon
			human
				movement_delay()
					...
					var
						health_deficiency = (maxHealth - health + staminaloss)
					if(reagents)
					...
					var
						hungry = (500 - nutrition)/5 // So overeat would be 100 and default level would be 80
					if(hungry >= 70)
						tally += hungry/50
				...
				handle_footstep(turf/T)
					if(..())
						if(T.footstep_sounds["human"])
							var
								S = pick(T.footstep_sounds["human"])
							if(S)
								if(m_intent == "run")
