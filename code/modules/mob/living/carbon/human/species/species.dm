...
datum
	species
		...
		var
			blurb = "A completely nondescript species."      // A brief lore summary for use in the chargen screen.
		var
			butt_sprite = "human"
		...
		var
			silent_steps = 0          // Stops step noises
		...
		var
			cold_env_multiplier = 1 // Damage multiplier for being in a cold environment
		...
		var
			heat_level_3 = 460 // Heat damage level 3 above this point; used for body temperature
		var
			hot_env_multiplier = 1 // Damage multiplier for being in a hot environment
		var
			heat_level_3_breathe = 1000 // Heat damage level 3 above this point; used for breathed air temperature
		...
		var
			passive_temp_gain = 0			//IS_SYNTHETIC species will gain this much temperature every second
		var
		...
		var
			total_health = 100
		var
			punchdamagelow = 0       //lowest possible punch damage
		var
			punchdamagehigh = 9      //highest possible punch damage
		var
			punchstunthreshold = 9	 //damage at which punches from this race will stun //yes it should be to the attacked race but it's not useful that way even if it's logical
		var
			list
				default_genes = list()

		var
			ventcrawler = 0 //Determines if the mob can go through the vents.
		var
			has_fine_manipulation = 1 // Can use small items.

		var
			list
				allowed_consumed_mobs = list() //If a species can consume mobs, put the type of mobs it can consume here.

		var
			flags = 0       // Various specific features.
		var
			clothing_flags = 0 // Underwear and socks.
		var
			exotic_blood
		var
			bodyflags = 0
		var
			dietflags  = 0	// Make sure you set this, otherwise it won't be able to digest a lot of foods

		var
			list
				abilities = list()	// For species-derived or admin-given powers
		...
		var
			virus_immune
		var
			can_revive_by_healing				// Determines whether or not this species can be revived by simply healing them
		...
		var
			language = "Galactic Common"         // Default racial language, if any.
		var
		...
		var
			scream_verb = "screams"
		var
			male_scream_sound = 'sound/goonstation/voice/male_scream.ogg'
		var
			female_scream_sound = 'sound/goonstation/voice/female_scream.ogg'

		//Default hair/headacc style vars.
		var
			default_hair = "Bald" 		//Default hair style for newly created humans unless otherwise set.
		var
			default_hair_colour
		var
			default_fhair = "Shaved"	//Default facial hair style for newly created humans unless otherwise set.
		var
			default_fhair_colour
		var
			default_headacc = "None"	//Default head accessory style for newly created humans unless otherwise set.
		var
			default_headacc_colour
		...
		proc
			get_random_name(var/gender)
				var
					datum
						language
							species_language = all_languages[language]
				return species_language.get_random_name(gender)

datum
	species
		proc
			create_organs(var/mob/living/carbon/human/H) //Handles creation of mob organs.
				...
				for(var/limb_type in has_limbs)
					var
						list
							organ_data = has_limbs[limb_type]
					var
...
datum
	species
		proc
			handle_breath(var/datum/gas_mixture/breath, var/mob/living/carbon/human/H)
				var
				...
				var
					O2_pp = (breath.oxygen/breath.total_moles()) * breath_pressure
				// Partial pressure of Nitrogen
				var
					N2_pp = (breath.nitrogen/breath.total_moles()) * breath_pressure
				// Partial pressure of plasma
				var
					Tox_pp = (breath.toxins/breath.total_moles()) * breath_pressure
				// Partial pressure of CO2
				var
					CO2_pp = (breath.carbon_dioxide/breath.total_moles()) * breath_pressure
				...
				if(O2_pp < atmos_requirements["min_oxy"])
					...
					if(O2_pp > 0)
						var
							ratio = atmos_requirements["min_oxy"] / O2_pp
					...
					var
						ratio = (breath.oxygen / atmos_requirements["max_oxy"]) * 1000
				...
				if(N2_pp < atmos_requirements["min_nitro"])
					...
					if(N2_pp > 0)
						var
							ratio = atmos_requirements["min_nitro"] / N2_pp
						H.adjustOxyLoss(min(5 * ratio, HUMAN_MAX_OXYLOSS))
					else
						H.adjustOxyLoss(HUMAN_MAX_OXYLOSS)
					...
					var
						ratio = (breath.nitrogen / atmos_requirements["max_nitro"]) * 1000
					H.adjustToxLoss(Clamp(ratio, MIN_PLASMA_DAMAGE, MAX_PLASMA_DAMAGE))
				...
				if(Tox_pp < atmos_requirements["min_tox"])
					...
					if(Tox_pp > 0)
						var
							ratio = atmos_requirements["min_tox"] / Tox_pp
						H.adjustOxyLoss(min(5 * ratio, HUMAN_MAX_OXYLOSS))
					else
						H.adjustOxyLoss(HUMAN_MAX_OXYLOSS)
					...
					var
						ratio = (breath.toxins / atmos_requirements["max_tox"]) * 10
					if(H.reagents)
				...
				if(CO2_pp < atmos_requirements["min_co2"])
					...
					if(CO2_pp)
						var
							ratio = atmos_requirements["min_co2"] / CO2_pp
						H.adjustOxyLoss(min(5 * ratio, HUMAN_MAX_OXYLOSS))
					else
						H.adjustOxyLoss(HUMAN_MAX_OXYLOSS)
...
datum
	species
		proc
			handle_temperature(datum/gas_mixture/breath, var/mob/living/carbon/human/H) // called by human/life, handles temperatures
...
datum
	species
		proc
			handle_post_spawn(var/mob/living/carbon/C) //Handles anything not already covered by basic species assignment.
				grant_abilities(C)
				return
...
datum
	species
		proc
			updatespeciescolor(var/mob/living/carbon/human/H) //Handles changing icobase for species that have multiple skin colors.
				return

datum
	species
		proc
			grant_abilities(var/mob/living/carbon/human/H)
...
datum
	species
		proc
			remove_abilities(var/mob/living/carbon/human/H)
				for(var/proc/ability in species_abilities)
					H.verbs -= ability
				return
...
datum
	species
		proc
			handle_dna(var/mob/living/carbon/C, var/remove) //Handles DNA mutations, as that doesn't work at init. Make sure you call genemutcheck on any blocks changed here
				return
...
datum
	species
		proc
			handle_death(var/mob/living/carbon/human/H) //Handles any species-specific death events (such as dionaea nymph spawns).
				return

datum
	species
		proc
			handle_attack_hand(var/mob/living/carbon/human/H, var/mob/living/carbon/human/M) //Handles any species-specific attackhand events.
				return

datum
	species
		proc
			say_filter(mob/M, message, datum/language/speaking)
				return message
...
datum
	species
		proc
			handle_npc(var/mob/living/carbon/human/H)
				return
...
datum
	unarmed_attack
		...
		var
			damage = 0						// How much flat bonus damage an attack will do. This is a *bonus* guaranteed damage amount on top of the random damage attacks do.
		var
			attack_sound = "punch"
		var
...
datum
	species
		proc
			handle_can_equip(obj/item/I, slot, disable_warning = 0, mob/living/carbon/human/user)
				return 0

datum
	species
		proc
			handle_vision(mob/living/carbon/human/H)
				if(H.stat == DEAD)
				...
				var
					minimum_darkness_view = INFINITY
				if(H.glasses)
...
datum
	species
		proc
			handle_hud_icons(mob/living/carbon/human/H)
				if(H.healths)
					if(H.analgesic)
				...
				if(H.healthdoll)
					...
					if(H.stat == DEAD)
						...
						for(var/obj/item/organ/external/O in H.organs)
							var
								damage = O.burn_dam + O.brute_dam
							var
								comparison = (O.max_damage/5)
							var
								icon_num = 0
							if(damage)
								icon_num = 1
