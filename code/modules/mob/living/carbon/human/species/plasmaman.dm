	...
	species
		plasmaman
			equip(var/mob/living/carbon/human/H)
				...
				var
					helm=/obj/item/clothing/head/helmet/space/eva/plasmaman
				var
					tank_slot = slot_s_store
				var
	...
	species
		plasmaman
			handle_breath(var/datum/gas_mixture/breath, var/mob/living/carbon/human/H)
				var
					safe_plasma_min = 16 // Minimum safe partial pressure of PLASMA, in kPa
				...
				var
					Toxins_pp = (breath.toxins/breath.total_moles())*breath_pressure
				// And CO2, lets say a PP of more than 10 will be bad (It's a little less really, but eh, being passed out all round aint no fun)
				var
					CO2_pp = (breath.carbon_dioxide/breath.total_moles())*breath_pressure // Tweaking to fit the hacky bullshit I've done with atmo -- TLE
				...
				if(Toxins_pp < safe_plasma_min)
					...
					if(Toxins_pp > 0)
						var
							ratio = safe_plasma_min/Toxins_pp
