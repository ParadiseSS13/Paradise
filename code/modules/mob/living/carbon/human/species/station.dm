		...
		unathi
			...
			allowed_consumed_mobs = list(/mob/living/simple_animal/mouse, /mob/living/simple_animal/lizard, /mob/living/simple_animal/chick, /mob/living/simple_animal/chicken,
											mob
												living
													simple_animal
														crab, /mob/living/simple_animal/butterfly, /mob/living/simple_animal/parrot, /mob/living/simple_animal/tribble)
	...
	species
		vox
			equip(var/mob/living/carbon/human/H)
				...
				var
					tank_pref = H.client.prefs.speciesprefs
...
datum
	species
		vox
			updatespeciescolor(var/mob/living/carbon/human/H) //Handling species-specific skin-tones for the Vox race.
...
datum
	species
		slime
			handle_life(var/mob/living/carbon/human/H)
...
#define SLIMEPERSON_BLOOD_SCALING_FACTOR 5 // Used to adjust how much of an effect the blood has on the rate of color change. Higher is slower.
	...
	if((H in recolor_list) && H.reagents.total_volume > SLIMEPERSON_COLOR_SHIFT_TRIGGER)
		var
			blood_amount = H.vessel.total_volume
		var
			r_color = mix_color_from_reagents(H.reagents.reagent_list)
		var
			new_body_color = BlendRGB(r_color, rgb(H.r_skin, H.g_skin, H.b_skin), (blood_amount*SLIMEPERSON_BLOOD_SCALING_FACTOR)/((blood_amount*SLIMEPERSON_BLOOD_SCALING_FACTOR)+(H.reagents.total_volume)))
		var
			list
				new_color_list = ReadRGB(new_body_color)
...
mob
	living
		carbon
			human
				proc
					toggle_recolor(var/silent = 0)
						var
							datum
								species
									slime
										S = all_species[get_species()]
						if(!istype(S))
							if(!silent)
...
#define SLIMEPERSON_REGROWTHDELAY 450 // 45 seconds
					...
					item
						organ
							external
								limb = limblist["path"]
			var
				parent_organ = initial(limb.parent_organ)
			var
				obj
					item
						organ
							external
								parentLimb = organs_by_name[parent_organ]
	...
	var
		limb_select = input(src, "Choose a limb to regrow", "Limb Regrowth") as null|anything in missing_limbs
	var
		chosen_limb = missing_limbs[limb_select]
	...
	if(do_after(src, SLIMEPERSON_REGROWTHDELAY, needhand=0, target = src))
		...
		var
			limb_list = species.has_limbs[chosen_limb]
		var
			obj
				item
					organ
						external
							limb_path = limb_list["path"]
		// Parent check
		var
			obj
				item
					organ
						external
							potential_parent = organs_by_name[initial(limb_path.parent_organ)]
...
datum
	species
		slime
			handle_pre_change(var/mob/living/carbon/human/H)
				..()
...
datum
	species
		grey
			handle_dna(var/mob/living/carbon/C, var/remove)
				if(!remove)
