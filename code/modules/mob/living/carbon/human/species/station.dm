...
/datum/species/machine
	...
	species_traits = list(IS_WHITELISTED, NO_BREATHE, NO_SCAN, NO_BLOOD, NO_PAIN, NO_DNA, RADIMMUNE, VIRUSIMMUNE, NOTRANSSTING, NO_HUNGER, NO_INTORGANS)
	clothing_flags = HAS_UNDERWEAR | HAS_UNDERSHIRT | HAS_SOCKS
	...
	has_organ = list(
		...
		"cell mount" = /obj/item/organ/internal/cell_mount/full,
...
/datum/species/machine/handle_life(mob/living/carbon/human/H)
	..()
	var/obj/item/organ/internal/cell_mount/cell_mount = H.get_int_organ(/obj/item/organ/internal/cell_mount) //IPCs are dependent on batteries
	if(istype(cell_mount))
		if(cell_mount.cell && cell_mount.cell.charge && !(cell_mount.status & ORGAN_DEAD))
			return
	H.adjustFireLoss(5) // No cell or cell charge? Catastrophic system failure
	H.Paralyse(4)
	H.adjustBruteLoss(2)
