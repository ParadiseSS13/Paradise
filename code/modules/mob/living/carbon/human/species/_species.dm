/datum/species
	...
	/// Additional armor percentage for the species. Ranges from -INFINITY to 100
	var/armor = 0
...
/datum/species/proc/apply_damage(damage = 0, damagetype = BRUTE, def_zone, blocked = 0, mob/living/carbon/human/H, sharp = FALSE, obj/used_weapon, spread_damage = FALSE)
	...
	var/armor_value
	var/armor_value_to_add = PERCENTAGE_TO_ARMOR_VALUE(blocked)
	...
	switch(damagetype)
		if(BRUTE)
			...
			if(organ?.receive_damage(brute = damage, sharp = sharp, used_weapon = used_weapon))
				H.UpdateDamageIcon()
			else // no bodypart, we deal damage with a more general method.
				H.adjustBruteLoss(damage)
		if(BURN)
			damage = ARMOR_EQUATION(damage, armor_value) * burn_mod * H.physiology.burn_mod
			if(damage)
				H.damageoverlaytemp = 20

			if(organ?.receive_damage(burn = damage, sharp = sharp, used_weapon = used_weapon))
				H.UpdateDamageIcon()
			else
				H.adjustFireLoss(damage)
		if(TOX)
			damage = ARMOR_EQUATION(damage, armor_value) * H.physiology.tox_mod
			H.adjustToxLoss(damage)
		if(OXY)
			damage = ARMOR_EQUATION(damage, armor_value) * H.physiology.oxy_mod
			H.adjustOxyLoss(damage)
		if(CLONE)
			damage = ARMOR_EQUATION(damage, armor_value) * H.physiology.clone_mod
			H.adjustCloneLoss(damage)
		if(STAMINA)
			damage = ARMOR_EQUATION(damage, armor_value) * H.physiology.stamina_mod
			H.adjustStaminaLoss(damage)
		if(BRAIN)
			damage = ARMOR_EQUATION(damage, armor_value) * H.physiology.brain_mod
