					...
					if(hud_used)
						var
							obj
								screen
									inventory
										R = hud_used.inv_slots[slot_r_hand]
						var
							obj
								screen
									inventory
										L = hud_used.inv_slots[slot_l_hand]
						if(R && L)
							R.update_icon()
							L.update_icon()
...
mob
	living
		carbon
			update_inv_r_hand(ignore_cuffs)
...
mob
	living
		carbon
			update_inv_l_hand(ignore_cuffs)
				if(handcuffed && !ignore_cuffs)
					drop_l_hand()
					return
				if(l_hand)
				...
				if(client && hud_used && hud_used.inv_slots[slot_back])
					var
						obj
							screen
								inventory
									inv = hud_used.inv_slots[slot_back]
					inv.update_icon()
