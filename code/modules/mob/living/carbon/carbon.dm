	...
	if(has_status_effect(STATUS_EFFECT_CHARGING))
		...
		if(isturf(hit_atom))
			...
			if(iswallturf(T))
				...
				hit_something = TRUE
		if(hit_something)
			visible_message("<span class='danger'>[src] slams into [hit_atom]!</span>", "<span class='userdanger'>[hit_atom] slams into you!</span>")
			playsound(get_turf(src), 'sound/effects/meteorimpact.ogg', 100, 1)
		return
