mob
	living
		var
			canEnterVentWith = "/obj/item/weapon/implant=0&/obj/item/clothing/mask/facehugger=0&/obj/item/device/radio/borg=0&/obj/machinery/camera=0"
		var
			datum
				middleClickOverride
					middleClickOverride = null
...
mob
	living
		carbon
			var
				last_stomach_attack //defining this here because no one would look in carbon_defines for it
		...
		carbon
			relaymove(var/mob/user, direction)
				if(user in src.stomach_contents)
					...
					if(I && I.force)
						var
							d = rand(round(I.force / 4), I.force)
...
mob
	living
		carbon
			proc
				vomit(var/lost_nutrition = 10, var/blood = 0, var/stun = 1, var/distance = 0, var/message = 1)
					if(src.is_muzzled())
						if(message)
...
mob
	living
		carbon
			electrocute_act(shock_damage, obj/source, siemens_coeff = 1, override = 0, tesla_shock = 0)
		...
		carbon
			swap_hand()
				var
					obj
						item
							item_in_hand = src.get_active_hand()
...
mob
	living
		carbon
			activate_hand(var/selhand) //0 or "r" or "right" for right hand; 1 or "l" or "left" for left hand.
...
mob
	living
		carbon
			proc
				help_shake_act(mob/living/carbon/M)
...
mob
	living
		carbon
			flash_eyes(intensity = 1, override_blindness_check = 0, affect_silicon = 0, visual = 0)
				. = ..()
				var
					damage = intensity - check_eye_prot()
				if(.)
					if(visual)
						return
					if(weakeyes)
						...
						obj
							item
								organ
									internal
										eyes
											E = get_int_organ(/obj/item/organ/internal/eyes)
					if(!E)
						return
...
mob
	living
		carbon
			proc
				setDNA(var/datum/dna/newDNA)
					dna = newDNA


var
	list
		ventcrawl_machinery = list(/obj/machinery/atmospherics/unary/vent_pump, /obj/machinery/atmospherics/unary/vent_scrubber)

mob
	living
		handle_ventcrawl(var/atom/clicked_on) // -- TLE -- Merged by Carn
...
mob
	living
		proc
			add_ventcrawl(obj/machinery/atmospherics/starting_machine)
				if(!istype(starting_machine) || !starting_machine.returnPipenet())
					return
				var
					datum
						pipeline
							pipeline = starting_machine.returnPipenet()
				var
					list
						totalMembers = list()
		...
		carbon
			throw_item(atom/target)
				...
				if(istype(I, /obj/item/weapon/grab))
					...
					var
						mob
							throwable_mob = G.get_mob_if_throwable() //throw the person instead of the grab
					qdel(G)	//We delete the grab.
					if(throwable_mob)
						thrown_thing = throwable_mob
						var
							turf
								start_T = get_turf(loc) //Get the start and target tile for the descriptors
						var
							turf
								end_T = get_turf(target)
						if(start_T && end_T)
							var
								start_T_descriptor = "<font color='#6b5d00'>tile at [start_T.x], [start_T.y], [start_T.z] in area [get_area(start_T)]</font>"
							var
								end_T_descriptor = "<font color='#6b4400'>tile at [end_T.x], [end_T.y], [end_T.z] in area [get_area(end_T)]</font>"
...
mob
	living
		carbon
			unEquip(obj/item/I) //THIS PROC DID NOT CALL ..()
		...
		carbon
			show_inv(mob/user)
				...
				var
					datum
						browser
							popup = new(user, "mob\ref[src]", "[src]", 440, 500)
			...
			Topic(href, href_list)
				...
				if(!usr.stat && usr.canmove && !usr.restrained() && in_range(src, usr))
					if(href_list["internal"])
						var
							slot = text2num(href_list["internal"])
						var
							obj
								item
...
mob
	living
		carbon
			proc
				get_pulse(var/method)	//method 0 is for hands, 1 is for machines, more accurate
					var
						temp = 0								//see setup.dm:694
					switch(src.pulse)
						if(PULSE_NONE)
							return "0"
						if(PULSE_SLOW)
...
mob
	living
		carbon
			proc
				cuff_resist(obj/item/I, breakouttime = 600, cuff_break = 0)
...
mob
	living
		carbon
			get_standard_pixel_y_offset(lying = 0)
				if(lying)
							...
							item
								organ
									internal
										xenos
											plasmavessel
												vessel = get_int_organ(/obj/item/organ/internal/xenos/plasmavessel)
					if(vessel)
...
mob
	living
		carbon
			proc
				slip(var/description, var/stun, var/weaken, var/tilesSlipped, var/walkSafely, var/slipAny)
...
mob
	living
		carbon
			proc
				eat(var/obj/item/weapon/reagent_containers/food/toEat, mob/user)
					if(!istype(toEat))
						return 0
					var
						fullness = 0
...
mob
	living
		carbon
			proc
				selfFeed(var/obj/item/weapon/reagent_containers/food/toEat, fullness)
...
mob
	living
		carbon
			proc
				selfDrink(var/obj/item/weapon/reagent_containers/food/drinks/toDrink, mob/user)
					return 1
...
mob
	living
		carbon
			proc
				forceFed(var/obj/item/weapon/reagent_containers/food/toEat, mob/user, fullness)
...
mob
	living
		carbon
			proc
				forceFedAttackLog(var/obj/item/weapon/reagent_containers/food/toEat, mob/user)
...
mob
	living
		carbon
			proc
				consume(var/obj/item/weapon/reagent_containers/food/toEat)
					if(!toEat.reagents)
						return
