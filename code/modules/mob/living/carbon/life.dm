...
mob
	living
		carbon
			proc
				check_breath(datum/gas_mixture/breath)
					...
					var
						oxygen_used = 0
					var
						breath_pressure = (breath.total_moles()*R_IDEAL_GAS_EQUATION*breath.temperature)/BREATH_VOLUME

					var
						O2_partialpressure = (breath.oxygen/breath.total_moles())*breath_pressure
					var
						Toxins_partialpressure = (breath.toxins/breath.total_moles())*breath_pressure
					var
					...
					if(Toxins_partialpressure > safe_tox_max)
						var
							ratio = (breath.toxins/safe_tox_max) * 10
						if(reagents)
...
mob
	living
		carbon
			proc
				handle_breath_temperature(datum/gas_mixture/breath)
					return
		...
		carbon
			proc
				CheckStamina()
					if(staminaloss)
						var
							total_health = (health - staminaloss)
			...
			handle_status_effects()
				...
				if(dizziness)
					...
					var
						temp
					var
						saved_dizz = dizziness
					if(C)
						var
							oldsrc = src
						var
							amplitude = dizziness*(sin(dizziness * 0.044 * world.time) + 1) / 70 // This shit is annoying at high strength
						src = null
						spawn(0)
							if(C)
