					...
					obj
						item
							weapon
								card
									id
										wear_id = null // Fix for station bounced radios -- Skie
				var
...
mob
	living
		carbon
			alien
				adjustFireLoss(amount) // Weak to Fire
					if(amount > 0)
						..(amount * 2)
					else
						..(amount)
...
mob
	living
		carbon
			alien
				handle_environment(var/datum/gas_mixture/environment)
...
//	to_chat(world, "Loc temp: [loc_temp] - Body temp: [bodytemperature] - Fireloss: [getFireLoss()] - Fire protection: [heat_protection] - Location: [loc] - src: [src]")
	...
	if(!on_fire) // If you're on fire, ignore local air temperature
		if(loc_temp > bodytemperature)
			...
			var
				thermal_protection = heat_protection //This returns a 0 - 1 value, which corresponds to the percentage of protection based on what you're wearing and what you're exposed to.
...
mob
	living
		carbon
			alien
				handle_fire()//Aliens on fire code
					if(..())
						return
											...
											item
												organ
													internal
														body_egg
															alien_embryo
																A = C.get_int_organ(/obj/item/organ/internal/body_egg/alien_embryo)
									if(A)
										var
											I = image('icons/mob/alien.dmi', loc = C, icon_state = "infected[A.stage]")
										client.images += I
						return
