...
var
	const
		MIN_ACTIVE_TIME = 200 //time between being dropped and going idle
var
	const
		MAX_ACTIVE_TIME = 400
		...
		clothing
			mask
				facehugger
					...
					var
						stat = CONSCIOUS //UNCONSCIOUS is the idle state in this case

					var
						sterile = 0
					var
						real = 1 //0 for the toy, 1 for real. Sure I could istype, but fuck that.
					var
						strength = 5
...
obj
	item
		clothing
			mask
				facehugger
					attack_alien(mob/user) //can be picked up by aliens
						attack_hand(user)
						return
...
obj
	item
		clothing
			mask
				facehugger
					attackby(obj/item/O,mob/m, params)
						if(O.force)
							Die()
						return
	...
	item
		clothing
			mask
				facehugger
					proc
						Impregnate(mob/living/target as mob)
	...
	item
		clothing
			mask
				facehugger
					lamarr
						New()//to prevent deleting it if aliums are disabled
							return
