...
/// Used for the IPC version
/obj/item/organ/internal/body_egg/alien_embryo/machine/on_life()
	switch(stage)
		if(2)
			if(prob(2))
				owner.emote("Buzz")
				owner.visible_message("<span class='danger'>[owner] has a BIOS warning briefly flash on their monitor!</span>", \
						"<span class='userdanger'>Your monitor quickly reconfigures after a short error!</span>")
			if(prob(2))
				owner.emote("Beep")
			if(prob(2))
				to_chat(owner, "<span class='danger'>ERROR: Internal damage detected in chest cavity.</span>")
			if(prob(2))
				to_chat(owner, "<span class='danger'>Runtime error in microcell.ts: Cannot read `plasma`.charge.</span>")
		if(3)
			if(prob(2))
				owner.emote("Ping")
				to_chat(owner, "<span class='danger'>DANGER: Infinite loop detected. Throttling...</span>")
			if(prob(2))
				owner.emote("Beep")
				to_chat(owner, "<span class='danger'>ALERT: disconnect from lower control systems. Automatic reconnect sucessful.</span>")
			if(prob(4))
				to_chat(owner, "<span class='danger'>ALERT: Heating system override engaged, overclocking activated in AREA ASSIGNED: Chest.</span>")
				if(prob(20))
					owner.take_organ_damage(0, 1)
			if(prob(4))
				to_chat(owner, "<span class='danger'>ALERT: Damage detected in hydraulic systems.</span>")
				if(prob(20))
					owner.adjustBruteLoss(1)
		if(4)
			to_chat(owner, "<span class='danger'>FATAL ERROR: PRIMARY POWER WIRE CUT. ALERT: SEVERE DAMAGE DETECTED IN cannot read null.build_structure. ALER-bzzz...</span>")
