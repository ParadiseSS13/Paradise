		...
		organ
			internal
				body_egg
					alien_embryo
						proc
							AttemptGrow(var/gib_on_success = 1)
								if(!owner || polling)
									return
								polling = 1
								spawn()
									var
										list
											candidates = pollCandidates("Do you want to play as an alien?", ROLE_ALIEN, 0)
									var
										mob
											C = null
									...
									var
										overlay = image('icons/mob/alien.dmi', loc = owner, icon_state = "burst_lie")
					...
					alien_embryo
						AddInfectionImages()
							for(var/mob/living/carbon/alien/alien in player_list)
								if(alien.client)
									var
										I = image('icons/mob/alien.dmi', loc = owner, icon_state = "infected[stage]")
