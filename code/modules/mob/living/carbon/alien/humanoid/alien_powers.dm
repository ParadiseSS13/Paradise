...
mob
	living
		carbon
			proc
				powerc(X, Y)//Y is optional, checks for weed planting. X can be null.
					if(stat)
	...
	living
		carbon
			alien
				humanoid
					verb
						transfer_plasma(mob/living/carbon/alien/M as mob in oview())
							...
							if(isalien(M))
								var
									amount = input("Amount:", "Transfer Plasma to [M]") as num
								if(amount)
	...
	living
		carbon
			alien
				humanoid
					proc
						corrosive_acid(O as obj|turf in oview(1)) //If they right click to corrode, an error will flash if its an invalid target./N
					...
					proc
						neurotoxin() // ok
							...
							if(powerc(50))
								...
								var
									turf
										U = get_step(src, dir) // Get the tile infront of the move, based on their direction
									...
									obj
										item
											projectile
												bullet
													neurotoxin
														A = new /obj/item/projectile/bullet/neurotoxin(usr.loc)
								A.current = U
								A.firer = src
								A.yo = U.y - T.y
								A.xo = U.x - T.x
				...
				humanoid
					proc
						resin() // -- TLE
							...
							if(powerc(55))
								var
									choice = input("Choose what you wish to shape.","Resin building") as null|anything in list("resin wall","resin membrane","resin nest") //would do it through typesof but then the player choice would have the type path and we don't want the internal workings to be exposed ICly - Urist
