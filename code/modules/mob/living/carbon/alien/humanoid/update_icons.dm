		...
		carbon
			alien
				humanoid
					var
						list
							overlays_standing[X_TOTAL_LAYERS]
...
mob
	living
		carbon
			alien
				humanoid
					update_transform() //The old method of updating lying/standing was update_icons(). Aliens still expect that.
						if(lying > 0)
...
mob
	living
		carbon
			alien
				humanoid
					update_inv_wear_suit(var/update_icons=1)
						if(client && hud_used)
							var
								obj
									screen
										inventory
											inv = hud_used.inv_slots[slot_wear_suit]
							inv.update_icon()
						...
						if(wear_suit)
							...
							var
								image
									standing	= image("icon" = 'icons/mob/mob.dmi', "icon_state" = "[t_state]")
...
mob
	living
		carbon
			alien
				humanoid
					update_inv_head(var/update_icons=1)
						if(head)
							var
...
mob
	living
		carbon
			alien
				humanoid
					update_inv_pockets(var/update_icons=1)
...
mob
	living
		carbon
			alien
				humanoid
					update_inv_r_hand(var/update_icons=1)
						..(1)
						if(r_hand)
							var
...
mob
	living
		carbon
			alien
				humanoid
					update_inv_l_hand(var/update_icons=1)
						..(1)
						if(l_hand)
							var
