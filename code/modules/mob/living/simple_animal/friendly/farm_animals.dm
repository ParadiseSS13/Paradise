							...
							if(locate(/obj/effect/plant) in loc)
								var
									obj
										effect
											plant
												SV = locate(/obj/effect/plant) in loc
								qdel(SV)
								if(prob(10))
									say("Nom")
											...
											weapon
												reagent_containers
													glass
														G = O
								var
									transfered = min(milk_content, rand(5,10), (G.volume - G.reagents.total_volume))
								if(transfered > 0)
...
mob
	living
		simple_animal
			cow
				attackby(var/obj/item/O as obj, var/mob/user as mob, params, params)
					if(stat == CONSCIOUS && istype(O, /obj/item/weapon/reagent_containers/glass))
						user.changeNext_move(CLICK_CD_MELEE)
						var
							obj
					...
					if(!stat && M.a_intent == I_DISARM && icon_state != icon_dead)
						...
						spawn(rand(20,50))
							if(!stat && M)
								...
								var
									list
										responses = list(	"[src] looks at you imploringly.",
							...
							var
								mob
									living
										simple_animal
											chicken
												C = new /mob/living/simple_animal/chicken(loc)
							if(mind)
								mind.transfer_to(C)
							qdel(src)
