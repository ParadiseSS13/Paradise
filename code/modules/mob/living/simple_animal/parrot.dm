	...
	living
		simple_animal
			parrot
				...
				var
					parrot_state = PARROT_WANDER //Hunt for a perch when created
				var
					parrot_sleep_max = 25 //The time the parrot sits while perched before looking around. Mosly a way to avoid the parrot's AI in process_ai() being run every single tick.
				var
					parrot_sleep_dur = 25 //Same as above, this is the var that physically counts down
				var
					parrot_dam_zone = list("chest", "head", "l_arm", "l_leg", "r_arm", "r_leg") //For humans, select a bodypart to attack

				var
					parrot_speed = 5 //"Delay in world ticks between movement." according to byond. Yeah, that's BS but it does directly affect movement. Higher number = slower.
				var
					parrot_been_shot = 0 //Parrots get a speed bonus after being shot. This will deincrement every process_ai() and at 0 the parrot will return to regular speed.
				...
				var
					obj
						desired_perches = list(/obj/structure/computerframe, 		/obj/structure/displaycase, \
														obj
															structure
																filingcabinet,		/obj/machinery/teleport, \
														obj
															machinery
																suit_storage_unit,			/obj/machinery/clonepod, \
														obj
															machinery
																dna_scannernew,		/obj/machinery/telecomms, \
														obj
															machinery
																nuclearbomb,			/obj/machinery/particle_accelerator, \
														obj
															machinery
																recharge_station,	/obj/machinery/smartfridge, \
														obj
															machinery
																computer)
		...
		simple_animal
			parrot
				New()
					...
					if(!ears)
						var
							headset = pick(/obj/item/device/radio/headset/headset_sec, \
											obj
												item
													device
														radio
...
mob
	living
		simple_animal
			parrot
				attack_hand(mob/living/carbon/M as mob)
					..()
					if(client) return
...
//-----WANDERING - This is basically a 'I dont know what to do yet' state
		...
		if(!held_item && !parrot_perch) //If we've got nothing to do.. look for something to do.
			var
				atom
					movable
						AM = search_for_perch_and_item() //This handles checking through lists so we know it's either a perch or stealable item
			if(AM)
				...
				var
					obj
						item
							organ
								external
									affecting = H.get_organ(ran_zone(pick(parrot_dam_zone)))
...
mob
	living
		simple_animal
			parrot
				proc
					drop_held_item(var/drop_gently = 1)
						set name = "Drop held item"
						set category = "Parrot"
						set desc = "Drop the item you're holding."
...
mob
	living
		simple_animal
			parrot
				hear_say(var/message, var/verb = "says", var/datum/language/language = null, var/alt_name = "",var/italics = 0, var/mob/speaker = null)
...
mob
	living
		simple_animal
			parrot
				hear_radio(var/message, var/verb="says", var/datum/language/language=null, var/part_a, var/part_b, var/mob/speaker = null, var/hard_to_hear = 0, var/atom/follow_target)
					if(speaker != src && prob(50))
						parrot_hear(html_decode(message))
					..()
