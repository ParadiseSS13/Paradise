...
/mob/living/simple_animal/parrot/death(gibbed)
	if(can_die())
		if(held_item)
			custom_emote(MSG_VISIBLE, "lets go of [held_item]!")
			drop_held_item()
		walk(src, 0)
	return ..()
...
//-----SLEEPING
	if(parrot_state == PARROT_PERCH)
		...
		if(--parrot_sleep_dur) //Zzz
			...
			if(parrot_interest)
				custom_emote(MSG_VISIBLE, "looks in [parrot_interest]'s direction and takes flight.")
		...
		if(!held_item && !parrot_perch) //If we've got nothing to do.. look for something to do.
			...
			if(AM)
				if(istype(AM, /obj/item) || isliving(AM))	//If stealable item
					...
					parrot_state = PARROT_SWOOP|PARROT_STEAL
					face_atom(AM)
					custom_emote(MSG_VISIBLE, "turns and flies towards [parrot_interest].")
					return
...
//-----ATTACKING
		...
		if(Adjacent(parrot_interest))
			...
			if(ishuman(parrot_interest))
				...
				custom_emote(MSG_VISIBLE, pick("pecks [H]'s [affecting].", "cuts [H]'s [affecting] with its talons."))
			else
				L.adjustBruteLoss(damage)
				custom_emote(MSG_VISIBLE, pick("pecks at [L].", "claws [L]."))
			return
