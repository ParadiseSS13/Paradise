		...
		simple_animal
			hostile
				mushroom
					...
					var
						powerlevel = 0 //Tracks our general strength level gained from eating other shrooms
					var
						bruised = 0 //If someone tries to cheat the system by attacking a shroom to lower its health, punish them so that it wont award levels to shrooms that eat it
					var
						recovery_cooldown = 0 //So you can't repeatedly revive it during a fight
					var
						faint_ticker = 0 //If we hit three, another mushroom's gonna eat us
					var
						image
							cap_living = null //Where we store our cap icons so we dont generate them constantly to update our icon
					var
						image
							cap_dead = null
...
mob
	living
		simple_animal
			hostile
				mushroom
					New()//Makes every shroom a little unique
...
mob
	living
		simple_animal
			hostile
				mushroom
					adjustHealth(damage)//Possibility to flee from a fight just to make it more visually interesting
...
mob
	living
		simple_animal
			hostile
				mushroom
					attack_animal(var/mob/living/L)
						if(istype(L, /mob/living/simple_animal/hostile/mushroom) && stat == DEAD)
							...
							var
								level_gain = (powerlevel - M.powerlevel)
										...
										weapon
											reagent_containers
												food
													snacks
														hugemushroomslice
															S = new /obj/item/weapon/reagent_containers/food/snacks/hugemushroomslice(src.loc)
