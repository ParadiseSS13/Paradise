	...
	living
		simple_animal
			hostile
				poison
					bees
						Process_Spacemove(movement_dir = 0)
							return 1
		...
		simple_animal
			hostile
				poison
					bees
						proc
							assign_reagent(datum/reagent/R)
								if(istype(R))
									beegent = R
		...
		simple_animal
			hostile
				poison
					bees
						proc
							reagent_incompatible(mob/living/simple_animal/hostile/poison/bees/B)
								if(!B)
									return 0
			...
			hostile
				poison
					bees
						worker
							proc
								pollinate(obj/machinery/portable_atmospherics/hydroponics/Hydro)
									...
									var
										growth = health //Health also means how many bees are in the swarm, roughly.
									...
									if(prob(BEE_POLLINATE_YIELD_CHANCE)) //Yield mod is HELLA powerful, but quite rare
										...
										var
											seed_yield = Hydro.seed.get_trait(TRAIT_YIELD)
									...
									if(prob(BEE_POLLINATE_POTENTCY_CHANCE))
										if(!isnull(plant_controller.seeds[Hydro.seed.name]))
											...
											Hydro.seed.update_name_prefixes()
										var
											seed_potency = Hydro.seed.get_trait(TRAIT_POTENCY)
				...
				if(istype(I,/obj/item/weapon/reagent_containers/syringe))
					...
					if(S.reagents.has_reagent("royal_bee_jelly")) //checked twice, because I really don't want royal bee jelly to be duped
						...
						var
							datum
								reagent
									R = chemical_reagents_list[S.reagents.get_master_reagent_id()]
