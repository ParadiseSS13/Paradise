		...
		simple_animal
			hostile
				asteroid/
					...
					var
						icon_aggro = null // for swapping to when we get aggressive
					see_in_dark = 8
...
mob
	living
		simple_animal
			hostile
				asteroid
					bullet_act(var/obj/item/projectile/P)//Reduces damage from most projectiles to curb off-screen kills
						if(!stat)
							Aggro()
...
mob
	living
		simple_animal
			hostile
				asteroid
					hitby(atom/movable/AM)//No floor tiling them to death, wiseguy
		...
		simple_animal
			hostile
				asteroid
					goldgrub
						proc
							Burrow()//Begin the chase to kill the goldgrub in time
								if(!alerted)
									alerted = 1
									spawn(chase_time)
									if(alerted)
									...
									living
										simple_animal
											hostile
												asteroid
													hivelordbrood
														A = new /mob/living/simple_animal/hostile/asteroid/hivelordbrood(src.loc)
							A.GiveTarget(target)
							A.friends = friends
							A.faction = faction
							return
						...
						if(ishuman(owner))
							...
							var
								datum
									reagent
										blood
											B = locate() in H.vessel.reagent_list //Grab some blood
							var
			...
			original
				New()
					...
					for(counter = 1, counter <= 3, counter++)
						var
							spawndir = pick(directions)
						directions -= spawndir
						var
							turf
								T = get_step(src,spawndir)
