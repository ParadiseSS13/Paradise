						...
						mob
							living
								simple_animal
									hostile
										spaceWorm
											previousWorm //next/previous segments, correspondingly
					var
						mob
							living
								simple_animal
									hostile
										spaceWorm
											nextWorm     //head is the nextest segment
							...
							living
								simple_animal
									hostile
										spaceWorm
											wormHead
												myHead		//Can be the same as next, just a general reference to the main worm.

					var
						stomachProcessProbability = 50
					var
						digestionProbability = 20

					var
						atom
							currentlyEating //what the worm is currently eating
					var
						plasmaPoopPotential = 5 //this mainly exists for the name
			...
			hostile
				spaceWorm
					wormHead
						...
						var
							list
								totalWormSegments = list() //doesn't contain src
						var
							catastrophicDeathProb = 15 //15% chance for the death of the head to kill the whole thing
	...
	living
		simple_animal
			hostile
				spaceWorm
					wormHead
						New(var/location, var/segments = spawnWithSegments)
							..()
									...
									mob
										living
											simple_animal
												hostile
													spaceWorm
														newSegment = new /mob/living/simple_animal/hostile/spaceWorm(loc)
									...
									if(noms && Adjacent(noms) && (currentlyEating == noms))//It exists, were next to it, and it's still the thing were eating
										if(W)
											...
											if(ismob(noms))
												var
													mob
														M = noms //typecast because noms isn't movable
	...
	living
		simple_animal
			hostile
				spaceWorm
					proc
						Attach(var/mob/living/simple_animal/hostile/spaceWorm/toAttach)
							if(!toAttach)
								return
									...
									living
										simple_animal
											hostile
												spaceWorm
													wormHead
														newHead = new /mob/living/simple_animal/hostile/spaceWorm/wormHead(loc,0)
							var
								mob
									living
										simple_animal
