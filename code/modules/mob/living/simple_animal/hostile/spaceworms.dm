...
/mob/living/simple_animal/hostile/spaceWorm/Process_Spacemove(check_drift = 0)
...
/mob/living/simple_animal/hostile/spaceWorm/wormHead/proc/attemptToEat(atom/noms)
	...
	src.visible_message("<span class='userdanger'>\the [src] starts to eat \the [noms]!</span>",span_notice("You start to eat \the [noms]. (This will take about [ufnomDelay] seconds.)"),"<span class='userdanger'>You hear gnashing.</span>") //inform everyone what the fucking worm is doing.
	...
	if(do_after(src, nomDelay, 0, target = noms))
		if(noms && Adjacent(noms) && (currentlyEating == noms))//It exists, were next to it, and it's still the thing were eating
			if(W)
				...
				src.visible_message("<span class='userdanger'>\the [src] eats \the [noms]!</span>",span_notice("You eat \the [noms]!"),"<span class='userdanger'>You hear gnashing.</span>") //inform everyone what the fucking worm is doing.
			else
