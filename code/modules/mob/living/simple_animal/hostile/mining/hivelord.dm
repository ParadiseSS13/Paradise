...
/obj/item/organ/internal/hivelord_core/proc/preserved(implanted = 0)
	...
	if(implanted)
		feedback_report("hivelord_core", "[type]|implanted")
	else
		feedback_report("hivelord_core", "[type]|stabilizer")


/obj/item/organ/internal/hivelord_core/proc/go_inert()
	inert = TRUE
	desc = "The remains of a hivelord that have become useless, having been left alone too long after being harvested."
	feedback_report("hivelord_core", "[src.type]|inert")
	update_icon()
	...
	if(proximity_flag && ishuman(target))
		...
		if(inert)
			...
			if(H != user)
				...
				feedback_report("hivelord_core","[src.type]|used|other")
			else
				to_chat(user, "<span class='notice'>You start to smear [src] on yourself. It feels and smells disgusting, but you feel amazingly refreshed in mere moments.</span>")
				feedback_report("hivelord_core","[src.type]|used|self")
