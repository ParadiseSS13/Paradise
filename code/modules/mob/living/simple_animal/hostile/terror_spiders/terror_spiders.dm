...
/mob/living/simple_animal/hostile/poison/terror_spider/get_status_tab_items()
	...
	if(ckey && stat == CONSCIOUS)
		if(degenerate)
			. += "<font color='#eb4034'>Hivemind Connection Severed! Dying...</font>" // color=red
			return
		if(health != maxHealth)
			...
			if(hp_points_per_second > 0)
				...
				. += "Regeneration: [ltext]: <font color='[lcolor]'>[num2text(pc_of_max_per_second)]% of health per second</font>"
