	...
	if(charging)
		...
		if(isliving(A))
			...
			if(!istype(A, /mob/living/simple_animal/hostile/ancient_robot_leg))
				...
				L.apply_damage(25, BRUTE, limb_to_hit, L.run_armor_check(limb_to_hit, MELEE, null, null, armour_penetration_flat))
...
/mob/living/simple_animal/hostile/ancient_robot_leg/Bump(atom/A)
	...
	if(isliving(A))
		if(!istype(A, /mob/living/simple_animal/hostile/megafauna/ancient_robot))
			...
			forceMove(get_turf(L))
			var/limb_to_hit = L.get_organ(pick(BODY_ZONE_HEAD, BODY_ZONE_CHEST, BODY_ZONE_R_ARM, BODY_ZONE_L_ARM, BODY_ZONE_R_LEG, BODY_ZONE_L_LEG))
			L.apply_damage(12.5, BRUTE, limb_to_hit, L.run_armor_check(limb_to_hit, MELEE, null, null, armour_penetration_flat))
			playsound(get_turf(L), 'sound/effects/meteorimpact.ogg', 100, TRUE)
			shake_camera(L, 4, 3)
			shake_camera(src, 2, 3)
	..()
