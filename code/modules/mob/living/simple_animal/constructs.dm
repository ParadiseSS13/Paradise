				...
				armoured
					bullet_act(var/obj/item/projectile/P)
						if(istype(P, /obj/item/projectile/energy) || istype(P, /obj/item/projectile/beam))
							var
								reflectchance = 80 - round(P.damage/3)
							if(prob(reflectchance))
								...
								if(P.starting)
									var
										new_x = P.starting.x + pick(0, 0, -1, 1, -2, 2, -2, 2, -2, 2, -3, 3, -3, 3)
									var
										new_y = P.starting.y + pick(0, 0, -1, 1, -2, 2, -2, 2, -2, 2, -3, 3, -3, 3)
									var
										turf
											curloc = get_turf(src)
			...
			construct
				proc
					updateglow()
						overlays = 0
						var
							overlay_layer = LIGHTING_LAYER + 1
						if(layer != MOB_LAYER)
		...
		summon_cultist()
			...
			for(var/datum/mind/H in ticker.mode.cult)
				if(istype(H.current,/mob/living))
					...
					var
						mob
							cultist = input("Choose the one who you want to summon", "Followers of Geometer") as null|anything in (cultists - usr)
					if(!cultist)
						return
