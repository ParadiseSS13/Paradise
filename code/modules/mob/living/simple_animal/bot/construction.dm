...
obj
	item
		weapon
			bucket_sensor
				attackby(obj/item/W, mob/user as mob, params)
					..()
					if(istype(W, /obj/item/robot_parts/l_arm) || istype(W, /obj/item/robot_parts/r_arm))
							...
							mob
								living
									simple_animal
										bot
											cleanbot
												A = new /mob/living/simple_animal/bot/cleanbot(T)
						A.name = created_name
						...
						var
							t = stripped_input(user, "Enter new robot name", name, created_name,MAX_NAME_LEN)
						if(!t)
							return
	...
	item
		weapon
			storage
				toolbox
					mechanical
						attackby(obj/item/stack/tile/plasteel/T, mob/user, params)
								...
								var
									obj
										item
											weapon
												toolbox_tiles
													B = new /obj/item/weapon/toolbox_tiles
								user.put_in_hands(B)
						...
						var
							obj
								item
									weapon
										toolbox_tiles_sensor
											B = new /obj/item/weapon/toolbox_tiles_sensor()
							...
							mob
								living
									simple_animal
										bot
											floorbot
												A = new /mob/living/simple_animal/bot/floorbot(T)
						A.name = created_name
	...
	item
		weapon
			firstaid_arm_assembly
				...
				var
					skin = null //Same as medbot, set to tox or ointment for the respective kits.
				w_class = 3
				var
...
obj
	item
		weapon
			storage
				firstaid
					attackby(obj/item/robot_parts/S, mob/user, params)
						...
						var
							obj
								item
									weapon
										firstaid_arm_assembly
											A = new /obj/item/weapon/firstaid_arm_assembly
										...
										mob
											living
												simple_animal
													bot
														medbot
															S = new /mob/living/simple_animal/bot/medbot(T)
									S.skin = skin
									S.name = created_name
...
obj
	item
		clothing
			head
				helmet
					attackby(obj/item/device/assembly/signaler/S, mob/user, params)
						..()
						if(!issignaler(S))
							..()
							return
							...
							var
								obj
									item
										weapon
											secbot_assembly
												A = new /obj/item/weapon/secbot_assembly
							user.put_in_hands(A)
