	...
	living
		simple_animal
			bot
					...
					obj
						machinery
							bot_core
								bot_core = null
				var
					bot_core_type = /obj/machinery/bot_core
				var
					list
						users = list() //for dialog updates
				var
					window_id = "bot_control"
				var
					window_name = "Protobot 1.0" //Popup title
				var
					window_width = 0 //0 for default size
				var
					window_height = 0
				var
					obj
				...
				var
					allow_pai = 1 // Are we even allowed to insert a pai card.
				var
					bot_name
				...
				var
					locked = 1
				var
					hacked = 0 //Used to differentiate between being hacked by silicons and emagged by humans.
				var
				...
				var
					frustration = 0 //Used by some bots for tracking failures to reach their target.
				var
					base_speed = 2 //The speed at which the bot moves, or the number of times it moves per process() tick.
				var
					turf
						ai_waypoint //The end point of a bot's path, or the target location.
				var
					list
						path = list() //List of turfs through which a bot 'steps' to reach the waypoint.
				var
					pathset = 0
				var
					list
						ignore_list = list() //List of unreachable targets for an ignore-list enabled bot to ignore.
				var
					mode = BOT_IDLE //Standardizes the vars that indicate the bot is busy with its function.
				var
					tries = 0 //Number of times the bot tried and failed to move.
				var
					remote_disabled = 0 //If enabled, the AI cannot *Remotely* control a bot. It can still control it through cameras.
				var
					mob
						living
							silicon
				...
				var
					list
						radio_config = null //which channels can the bot listen to
				var
					radio_channel = "Common" //The bot's default radio channel
				var
				...
				var
					bot_purpose = "improve the station to the best of your ability"
				var
					control_freq = BOT_FREQ		// bot control frequency
				var
					bot_filter 				// The radio filter the bot uses to identify itself on the network.
				var
					bot_type = 0 //The type of bot it is, for radio control.
				//This holds text for what the bot is mode doing, reported on the remote bot control interface.
				var
					list
						mode_name = list("In Pursuit","Preparing to Arrest", "Arresting", \
...
mob
	living
		simple_animal
			bot
				proc
					set_custom_texts() //Superclass for setting hack texts. Appears only if a set is not given to a bot locally.
...
mob
	living
		simple_animal
			bot
				proc
					speak(message, channel) //Pass a message to have the bot say() it. Pass a frequency to say it on the radio.
						if((!on) || (!message))
							return
						if(channel)
...
mob
	living
		simple_animal
			bot
				proc
					scan(scan_type, old_target, scan_range = DEFAULT_SCAN_RANGE)
						var
							final_result
						for(var/scan in shuffle(view(scan_range, src))) //Search for something in range!
							...
							if( (scan in ignore_list) || (scan == old_target) ) //Filter for blacklisted elements, usually unreachable or previously processed oness
								continue
							var
								scan_result = process_scan(scan) //Some bots may require additional processing when a result is selected.
							if(scan_result)
				...
				proc
					bot_move(dest, move_speed)
						...
						if(get_turf(src) == dest) //We have arrived, no need to move again.
							...
							return 0
						var
							step_count = move_speed ? move_speed : base_speed //If a value is passed into move_speed, use that instead of the default speed var.
...
mob
	living
		simple_animal
			bot
				proc
					bot_step(dest) //Step,increase tries if failed
						if(!path)
							return 0
						if(path.len > 1)
...
mob
	living
		simple_animal
			bot
				proc
					call_bot(caller, turf/waypoint, message=TRUE)
						bot_reset() //Reset a bot before setting it to call mode.
						var
							area
								end_area = get_area(waypoint)
							...
							obj
								item
									weapon
										card
											id
												all_access = new /obj/item/weapon/card/id
						var
							datum
								job
									captain
										All = new/datum/job/captain
...
mob
	living
		simple_animal
			bot
				proc
					call_mode() //Handles preparing a bot for a call, as well as calling the move proc.
//Handles the bot's movement during a call.
	var
		success = bot_move(ai_waypoint, 3)
	if(!success)
		if(calling_ai)
				...
				proc
					patrol_step()
						...
						if(loc == patrol_target)		// reached target
							...
							var
								moved = bot_move(patrol_target)//step_towards(src, next)	// attempt to move
...
mob
	living
		simple_animal
			bot
				proc
					bot_control_message(command, mob/user, user_turf)
						switch(command)
							if("stop")
			...
			bot
				proc
					receive_signal(datum/signal/signal)
						...
						var
							recv = signal.data["command"]
						var
							user = signal.data["user"]
						...
						if(signal.data["active"] == src)
							...
							switch(recv)
								...
								if("summon")
									...
									var
										list
											user_access = signal.data["useraccess"]
				...
				proc
					summon_step()
						...
						if(loc == summon_target)		// Arrived to summon location.
							...
							var
								moved = bot_move(summon_target, 3)	// Move attempt
							if(!moved)
								spawn(2)
									calc_summon_path()
									tries = 0
...
mob
	living
		simple_animal
			bot
				proc
					openedDoor(obj/machinery/door/D)
						frustration = 0
				...
				proc
					show_controls(mob/M)
						...
						var
							datum
								browser
									popup = new(M,window_id,window_name,350,600)
...
mob
	living
		simple_animal
			bot
				proc
					topic_denied(mob/user) //Access check proc for bot topics! Remember to place in a bot's individual Topic if desired.
...
mob
	living
		simple_animal
			bot
				proc
					ejectpai(mob/user = null, announce = 1)
						if(paicard)
							if(mind && paicard.pai)
...
mob
	living
		simple_animal
			bot
				revive(full_heal = 0, admin_revive = 0)
					if(..())
						update_icon()
						. = 1
...
mob
	living
		simple_animal
			bot
				proc
					door_opened(obj/machinery/door/D)
						frustration = 0
...
mob
	living
		simple_animal
			bot
				handle_message_mode(var/message_mode, var/message, var/verb, var/speaking, var/used_radios)
					switch(message_mode)
						if("intercom")
