		...
		simple_animal
			bot
				mulebot
					...
					var
						loaddir = 0				// this the direction to unload onto/load from
					var
						home_destination = "" 	// tag of home beacon

					var
						reached_target = 1 	//true if already reached the target

					var
						auto_return = 1		// true if auto return to home beacon after unload
					var
						auto_pickup = 1 	// true if auto-pickup at beacon
					var
						report_delivery = 1 // true if bot will announce an arrival to a location.
				...
				mulebot
					Topic(href, list/href_list)
						...
						switch(href_list["op"])
							...
							if("destination")
								var
									new_dest = input(usr, "Enter Destination:", name, destination) as null|anything in deliverybeacontags
								if(new_dest)
									set_destination(new_dest)
							if("setid")
								var
									new_id = stripped_input(usr, "Enter ID:", name, suffix, MAX_NAME_LEN)
								if(new_id)
									set_suffix(new_id)
							if("sethome")
								var
									new_home = input(usr, "Enter Home:", name, home_destination) as null|anything in deliverybeacontags
								if(new_home)
...
mob
	living
		simple_animal
			bot
				mulebot
					MouseDrop_T(atom/movable/AM, mob/user)
			...
			bot
				mulebot
					handle_automated_action()
						...
						if(on)
							var
								speed = (wires.Motor1() ? 1 : 0) + (wires.Motor2() ? 2 : 0)
...
//					to_chat(world, "at ([x],[y]) moving to ([next.x],[next.y])")
					...
					var
						oldloc = loc
					var
						moved = step_towards(src, next)	// attempt to move
					if(cell) cell.use(1)
	...
	living
		simple_animal
			bot
				mulebot
					proc
						RunOver(mob/living/carbon/human/H)
						...
						get_nav()
							...
							for(var/obj/machinery/navbeacon/NB in deliverybeacons)
								if(NB.location == new_destination)	// if the beacon location matches the set destination
									...
									target = NB.loc
									var
										direction = NB.dir	// this will be the load/unload dir
									loaddir = direction
									update_icon()
									calc_path()
									return
...
mob
	living
		simple_animal
			bot
				mulebot
					remove_air(amount) //To prevent riders suffocating
						if(loc)
