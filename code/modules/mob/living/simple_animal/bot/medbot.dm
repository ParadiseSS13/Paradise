			...
			bot
				medbot
						...
						obj
							item
								weapon
									reagent_containers
										glass
											reagent_glass = null //Can be set to draw from this for reagents.
					var
						skin = null //Set to "tox", "ointment" or "o2" for the other two firstaid kits.
					var
						mob
							living
								carbon
					...
					var
						last_found = 0
					var
						last_newpatient_speak = 0 //Don't spam the "HEY I'M COMING" messages
					var
						injection_amount = 15 //How much reagent do we inject at a time?
					var
						heal_threshold = 10 //Start healing when they have this much damage in a category
					var
						use_beaker = 0 //Use reagents in beaker instead of default treatment agents.
					var
						declare_crit = 1 //If active, the bot will transmit a critical patient alert to MedHUD users.
					var
						declare_cooldown = 0 //Prevents spam of critical patient alerts.
					var
						stationary_mode = 0 //If enabled, the Medibot will not move automatically.
					...
					var
						treat_virus = 1 //If on, the bot will attempt to treat viral infections, curing them if possible.
					var
						shut_up = 0 //self explanatory :)
				...
				medbot
					New()
						...
						var
							datum
								atom_hud
									medsensor = huds[DATA_HUD_MEDICAL_ADVANCED]
	...
	living
		simple_animal
			bot
				medbot
					proc
						soft_reset() //Allows the medibot to still actively perform its medical duties without being completely halted as a hard reset does.
							path = list()
							patient = null
							mode = BOT_IDLE
							last_found = world.time
				...
				medbot
					Topic(href, href_list)
						...
						if(href_list["adj_threshold"])
							var
								adjust_num = text2num(href_list["adj_threshold"])
							...
							var
								adjust_num = text2num(href_list["adj_inject"])
...
mob
	living
		simple_animal
			bot
				medbot
					process_scan(mob/living/carbon/human/H)
						if(H.stat == 2)
							return
						...
						if(assess_patient(H))
							...
							if((last_newpatient_speak + 300) < world.time) //Don't spam these messages!
								var
									message = pick("Hey, [H.name]! Hold on, I'm coming.","Wait [H.name]! I want to help!","[H.name], you appear to be injured!")
								speak(message)
				...
				medbot
					handle_automated_action()
						...
						if(!patient)
							if(!shut_up && prob(1))
								var
									message = pick("Radar, put a mask on!","There's always a catch, and it's the best there is.","I knew it, I should've been a plastic surgeon.","What kind of medbay is this? Everyone's dropping like dead flies.","Delicious!")
								speak(message)
							var
								scan_range = (stationary_mode ? 1 : DEFAULT_SCAN_RANGE) //If in stationary mode, scan range is limited to adjacent patients.
	...
	living
		simple_animal
			bot
				medbot
					proc
						assess_patient(mob/living/carbon/C)
	...
	living
		simple_animal
			bot
				medbot
					proc
						medicate_patient(mob/living/carbon/C)
							if(!on)
								return
							...
							if(C.stat == 2)
								var
									death_message = pick("No! NO!","Live, damnit! LIVE!","I...I've never lost a patient before. Not today, I mean.")
								speak(death_message)
								oldpatient = patient
								soft_reset()
								return
							...
							if(!reagent_id) //If they don't need any of that they're probably cured!
								var
									message = pick("All patched up!","An apple a day keeps me away.","Feel better soon!")
								speak(message)
								bot_reset()
								return
							else
									...
									if((get_dist(src, patient) <= 1) && on && assess_patient(patient))
										if(reagent_id == "internal_beaker")
											if(use_beaker && reagent_glass && reagent_glass.reagents.total_volume)
												var
													fraction = min(injection_amount/reagent_glass.reagents.total_volume, 1)
	...
	living
		simple_animal
			bot
				medbot
					proc
						check_overdose(mob/living/carbon/patient,reagent_id,injection_amount)
							var
								datum
									reagent
										R  = chemical_reagents_list[reagent_id]
							if(!R.overdose_threshold)
								return 0
							var
								current_volume = patient.reagents.get_reagent_amount(reagent_id)
