			...
			bot
				ed209
					...
					var
						lastfired = 0
					var
						shot_delay = 3 //.3 seconds between shots
					var
						lasercolor = ""
					var
						disabled = 0//A holder for if it needs to be disabled, if true it will not seach for targets, shoot at targets, or move, currently only used for lasertag
					...
					var
						last_found //There's a delay
					var
						declare_arrests = 1 //When making an arrest, should it notify everyone wearing sechuds?
					var
						idcheck = 1 //If true, arrest people with no IDs
					var
						weaponscheck = 1 //If true, arrest people for weapons if they don't have access
					var
						check_records = 1 //Does it check security records?
					var
						arrest_type = 0 //If true, don't handcuff
					var
						projectile = /obj/item/projectile/energy/electrode //Holder for projectile type
					var
...
mob
	living
		simple_animal
			bot
				ed209
					New(loc,created_name,created_lasercolor)
						..()
						if(created_name)
							name = created_name
						...
						var
							datum
								atom_hud
									secsensor = huds[DATA_HUD_SECURITY_ADVANCED]
	...
	living
		simple_animal
			bot
				ed209
					proc
						retaliate(mob/living/carbon/human/H)
...
mob
	living
		simple_animal
			bot
				ed209
					attack_hand(mob/living/carbon/human/H)
						if(H.a_intent == "harm")
							retaliate(H)
						return ..()
	...
	living
		simple_animal
			bot
				ed209
					proc
						check_for_weapons(var/obj/item/slot_item)
						...
						var
							obj
								item
									weapon
										ed209_assembly
											Sa = new /obj/item/weapon/ed209_assembly(Tsec)
						Sa.build_step = 1
										...
										weapon
											gun
												energy
													gun
														advtaser
															G = new /obj/item/weapon/gun/energy/gun/advtaser(Tsec)
										...
										weapon
											gun
												energy
													laser
														bluetag
															G = new /obj/item/weapon/gun/energy/laser/bluetag(Tsec)
							G.power_supply.charge = 0
							G.update_icon()
										...
										weapon
											gun
												energy
													laser
														redtag
															G = new /obj/item/weapon/gun/energy/laser/redtag(Tsec)
							G.power_supply.charge = 0
							G.update_icon()
	...
	living
		simple_animal
			bot
				ed209
					proc
						set_weapon()  //used to update the projectile type and firing sound
				...
				ed209
					proc
						shootAt(mob/target)
							...
							var
								atom
									U = (istype(target, /atom/movable) ? target.loc : target)
							if((!( U ) || !( T )))
								return
							while(!(istype(U, /turf)))
								U = U.loc
...
mob
	living
		simple_animal
			bot
				ed209
					attack_alien(mob/living/carbon/alien/user)
						..()
						if(!isalien(target))
							target = user
							mode = BOT_HUNT
						...
						if(severity==2 && prob(70))
							...
							var
								obj
									effect
										overlay
											pulse2 = new/obj/effect/overlay ( loc )
