		...
		simple_animal
			bot
				secbot
					...
					var
						declare_arrests = 1 //When making an arrest, should it notify everyone on the security channel?
					var
						idcheck = 0 //If true, arrest people with no IDs
					var
						weaponscheck = 0 //If true, arrest people for weapons if they lack access
					var
						check_records = 1 //Does it check security records?
					var
						arrest_type = 0 //If true, don't handcuff
					var
						harmbaton = 0 //If true, beat instead of stun
					var
						flashing_lights = 0 //If true, flash lights
					var
						prev_flashing_lights = 0
					allow_pai = 0
...
mob
	living
		simple_animal
			bot
				secbot
					hitby(atom/movable/AM, skipcatch = 0, hitpush = 1, blocked = 0)
						...
						var
							obj
								item
									weapon
										secbot_assembly
											Sa = new /obj/item/weapon/secbot_assembly(Tsec)
						Sa.build_step = 1
						Sa.overlays += "hs_hole"
...
mob
	living
		simple_animal
			bot
				secbot
					attack_alien(var/mob/living/carbon/alien/user as mob)
						..()
						if(!isalien(target))
							target = user
							mode = BOT_HUNT
