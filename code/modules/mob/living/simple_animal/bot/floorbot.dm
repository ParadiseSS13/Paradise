			...
			bot
				floorbot
					...
					var
						nag_on_empty = 1
					var
						nagged = 0 //Prevents the Floorbot nagging more than once per refill.
					var
						max_targets = 50
					var
						turf
				...
				floorbot
					Topic(href, href_list)
						...
						switch(href_list["operation"])
							...
							if("bridgemode")
								var
									setdir = input("Select construction direction:") as null|anything in list("north","east","south","west","disable")
								switch(setdir)
									if("north")
										targetdirection = 1
									if("south")
					...
					handle_automated_action()
						...
						if(!target && emagged < 2 && amount > 0)
							if(targetdirection != null) //The bot is in bridge mode.
								...
								var
									turf
										T = get_step(src, targetdirection)
	...
	living
		simple_animal
			bot
				floorbot
					proc
						nag() //Annoy everyone on the channel to refill us!
							if(!nagged)
	...
	living
		simple_animal
			bot
				floorbot
					proc
						is_hull_breach(turf/t) //Ignore space tiles not considered part of a structure, also ignores shuttle docking areas.
							var
								area
									t_area = get_area(t)
	...
	living
		simple_animal
			bot
				floorbot
					proc
						eattile(obj/item/stack/tile/plasteel/T)
	...
	living
		simple_animal
			bot
				floorbot
					proc
						maketile(obj/item/stack/sheet/metal/M)
									...
									obj
										item
											stack
												tile
													plasteel
														T = new /obj/item/stack/tile/plasteel
								T.amount = 4
								T.forceMove(M.loc)
								if(M.amount > 1)
									M.amount--
								...
								item
									weapon
										storage
											toolbox
												mechanical
													N = new /obj/item/weapon/storage/toolbox/mechanical(Tsec)
						N.contents = list()
