	...
	living
		simple_animal
			possessed_object
				...
				var
					spirit_name = "mysterious force" // What we call ourselves in attack messages.
				health = 50
				maxHealth = 50
...
mob
	living
		simple_animal
			possessed_object
				start_pulling(var/atom/movable/AM) // Silly motherfuckers think they can pull things.
...
mob
	living
		simple_animal
			possessed_object
				ghost() // Ghosting will return the object to normal, and will not disqualify the ghoster from various mid-round antag positions.
					var
						response = alert(src, "End your possession of this object? (It will not stop you from respawning later)","Are you sure you want to ghost?","Ghost","Stay in body")
					if(response != "Ghost")
						return
					resting = 1
					var
...
mob
	living
		simple_animal
			possessed_object
				get_active_hand() // So that our attacks count as attacking with the item we've possessed.
...
mob
	living
		simple_animal
			possessed_object
				IsAdvancedToolUser() // So we can shoot guns (Mostly ourselves), among other things.
					return 1
...
mob
	living
		simple_animal
			possessed_object
				get_access() // If we've possessed an ID card we've got access to lots of fun things!
