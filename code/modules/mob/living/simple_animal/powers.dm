...
/datum/action/innate/hide/Activate()
	...
	if(owner.layer != layer_to_change_to)
		...
		owner.visible_message("<span class=notice'><b>[owner] scurries to the ground!</b></span>", "<span class='alertalien'>You are now hiding.</span>")
	...
	owner.visible_message("<span class=notice'>[owner] slowly peeks up from the ground...</span>", "<span class='alertalien'>You have stopped hiding.</span>")
	if(istype(simplemob) && simplemob.pass_door_while_hidden || isdrone(simplemob))
		simplemob.pass_flags &= ~PASSDOOR
