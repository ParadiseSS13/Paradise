...
/datum/ai_planning_subtree/treat_wounded_target/select_behaviors(datum/ai_controller/basic_controller/bot/controller, seconds_per_tick)
	var/mob/living/basic/bot/medbot/bot_pawn = controller.pawn
...
/datum/ai_behavior/tend_to_patient/perform(seconds_per_tick, datum/ai_controller/basic_controller/bot/controller, target_key, threshold, access_flags, is_stationary)
	. = ..()
	var/mob/living/carbon/human/patient = controller.blackboard[target_key]
	if(QDELETED(patient) || patient.stat == DEAD)
		return AI_BEHAVIOR_DELAY | AI_BEHAVIOR_FAILED
	if(check_if_healed(patient, threshold, access_flags))
		return AI_BEHAVIOR_DELAY | AI_BEHAVIOR_SUCCEEDED
...
/datum/ai_behavior/tend_to_patient/finish_action(datum/ai_controller/basic_controller/bot/controller, succeeded, target_key, threshold, access_flags, is_stationary)
	. = ..()
	var/atom/target = controller.blackboard[target_key]
	if(!succeeded)
...
/datum/ai_planning_subtree/find_and_hunt_target/patients_in_crit/select_behaviors(datum/ai_controller/basic_controller/bot/controller, seconds_per_tick)
	var/mob/living/basic/bot/medbot/bot_pawn = controller.pawn
...
/datum/ai_planning_subtree/find_patrol_beacon/medbot/select_behaviors(datum/ai_controller/controller, seconds_per_tick)
	var/mob/living/basic/bot/medbot/bot_pawn = controller.pawn
	if(bot_pawn.medical_mode_flags & MEDBOT_STATIONARY_MODE)
		return
	return ..()
