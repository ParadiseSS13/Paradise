...
/datum/ai_planning_subtree/pet_planning/cleanbot/select_behaviors(datum/ai_controller/basic_controller/bot/controller, seconds_per_tick)
...
/datum/ai_planning_subtree/cleaning_subtree/select_behaviors(datum/ai_controller/basic_controller/bot/cleanbot/controller, seconds_per_tick)
...
/datum/ai_behavior/find_and_set/in_list/clean_targets/search_tactic(datum/ai_controller/basic_controller/bot/controller, locate_paths, search_range = 7)
...
/datum/ai_planning_subtree/acid_spray/select_behaviors(datum/ai_controller/basic_controller/bot/controller, seconds_per_tick)
	var/mob/living/basic/bot/cleanbot/bot_pawn = controller.pawn
	if(!(bot_pawn.bot_access_flags & BOT_COVER_EMAGGED))
		return
...
/datum/ai_behavior/find_and_set/spray_target/search_tactic(datum/ai_controller/controller, locate_path, search_range = 7)
	var/list/ignore_list = controller.blackboard[BB_TEMPORARY_IGNORE_LIST]
...
/datum/ai_behavior/execute_clean/finish_action(datum/ai_controller/basic_controller/bot/controller, succeeded, target_key, targeting_strategy_key, hiding_location_key)
	...
	if(length(speech_list))
		...
		if(!QDELETED(living_pawn)) // pawn can be null at this point
			living_pawn.say(pick(speech_list))
	controller.clear_blackboard_key(target_key)
...
/datum/ai_planning_subtree/befriend_janitors/select_behaviors(datum/ai_controller/basic_controller/bot/controller, seconds_per_tick)
	var/mob/living/basic/bot/bot_pawn = controller.pawn
...
/datum/ai_behavior/find_and_set/friendly_janitor/search_tactic(datum/ai_controller/controller, locate_path, search_range = 7)
	var/mob/living/living_pawn = controller.pawn
...
/datum/ai_planning_subtree/find_patrol_beacon/cleanbot/select_behaviors(datum/ai_controller/basic_controller/bot/controller, seconds_per_tick)
