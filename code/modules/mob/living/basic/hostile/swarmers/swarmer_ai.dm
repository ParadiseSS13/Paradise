...
/datum/ai_behavior/find_hunt_target/swarmer_objects/valid_dinner(mob/living/source, atom/dinner, radius)
	...
	for(var/turf/T in range(1, dinner))
		...
		if(isspaceturf(T) || istype(A, /area/shuttle) || istype(A, /area/space) || istype(A, /area/station/engineering/engine/supermatter) || istype(A, /area/station/engineering/engine/reactor) || istype(A, /area/station/hallway/secondary/entry) || pressure_dangerlevel)
			return FALSE
	return TRUE
