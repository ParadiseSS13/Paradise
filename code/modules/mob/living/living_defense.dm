...
mob
	living
		proc
			run_armor_check(var/def_zone = null, var/attack_flag = "melee", var/absorb_text = null, var/soften_text = null, armour_penetration, penetrated_text)
				var
...
mob
	living
		bullet_act(var/obj/item/projectile/P, var/def_zone)
			//Armor
			var
				armor = run_armor_check(def_zone, P.flag, armour_penetration = P.armour_penetration)
			var
				proj_sharp = is_sharp(P)
			var
				proj_edge = has_edge(P)
...
mob
	living
		proc
			electrocute_act(shock_damage, obj/source, siemens_coeff = 1, safety = 0, tesla_shock = 0)
...
mob
	living
		hitby(atom/movable/AM as mob|obj,var/speed = 5)//Standardization and logging -Sieve
			if(istype(AM,/obj/))
				var
					obj
						O = AM
				var
					zone = ran_zone("chest", 65)//Hits a random part of the body, geared towards the chest
				var
					dtype = BRUTE
				...
				*/
				...
				var
					armor = run_armor_check(zone, "melee")
...
mob
	living
		proc
			adjust_fire_stacks(add_fire_stacks) //Adjusting the amount of fire_stacks we have on person
		...
		proc
			handle_fire()
				...
				var
					datum
						gas_mixture
							G = loc.return_air() // Check if we're standing in an oxygenless environment
				if(G.oxygen < 1)
...
mob
	living
		proc
			turf_collision(var/turf/T, var/speed)
				src.take_organ_damage(speed*5)

mob
	living
		proc
			near_wall(var/direction,var/distance=1)
				var
					turf
						T = get_step(get_turf(src),direction)
				var
					turf
						last_turf = src.loc
				var
...
mob
	living
		proc
			grabbedby(mob/living/carbon/user,var/supress_message = 0)
				...
				var
					obj
						item
							weapon
								grab
									G = new /obj/item/weapon/grab(user, src)
				if(buckled)
