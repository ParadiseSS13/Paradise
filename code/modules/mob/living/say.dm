...
var
	list
		channel_to_radio_key = new
proc
	get_radio_key_from_channel(var/channel)
		var
			key = channel_to_radio_key[channel]
		if(!key)
...
mob
	living
		proc
			handle_message_mode(message_mode, message, verb, speaking, used_radios, alt_name)
				switch(message_mode)
...
mob
	living
		say(var/message, var/datum/language/speaking = null, var/verb = "says", var/alt_name="")
			if(client)
			...
			if(speaking && !(speaking.flags & NO_STUTTER))
				var
					list
						handle_s = handle_speech_problems(message, verb)
				message = handle_s[1]
				verb = handle_s[2]
			...
			var
				speech_bubble_test = say_test(message)
			...
			spawn(0)
				if(loc && !isturf(loc))
					var
						atom
							A = loc //Non-turf, let it handle the speech bubble
...
mob
	living
		proc
			say_signlang(var/message, var/verb="gestures", var/datum/language/language)
...
mob
	living
		emote(var/act, var/type, var/message) //emote code is terrible, this is so that anything that isn't
...
mob
	living
		proc
			whisper_say(var/message, var/datum/language/speaking = null, var/alt_name="", var/verb="whispers")
				if(client)
					if(client.prefs.muted & MUTE_IC)
				...
				var
					not_heard //the message displayed to people who could not hear the whispering
				if(speaking)
				...
				var
					atom
						whisper_loc = get_whisper_loc()
				var
					list
						listening = hearers(message_range, whisper_loc)
				listening |= src
				...
				var
					list
						eavesdropping = hearers(eavesdropping_range, whisper_loc)
				eavesdropping -= src
				eavesdropping -= listening

				var
					list
						watching = hearers(watching_range, whisper_loc)
				watching  -= src
				watching  -= listening
				...
				if(eavesdropping.len)
					var
						new_message = stars(message)	//hopefully passing the message twice through stars() won't hurt... I guess if you already don't understand the language, when they speak it too quietly to hear normally you would be able to catch even less.
				...
				spawn(0)
					var
						image
							I = image('icons/mob/talk.dmi', src, "h[speech_bubble_test]", MOB_LAYER + 1)
					I.appearance_flags = APPEARANCE_UI_IGNORE_ALPHA
					flick_overlay(I, speech_bubble_recipients, 30)

				if(watching.len)
					var
						rendered = "<span class='game say'><span class='name'>[src.name]</span> [not_heard].</span>"
...
mob
	living
		speech_bubble(var/bubble_state = "",var/bubble_loc = src, var/list/bubble_recipients = list())
			var
				image
					I = image('icons/mob/talk.dmi', bubble_loc, bubble_state, MOB_LAYER + 1)
			I.appearance_flags = APPEARANCE_UI_IGNORE_ALPHA
