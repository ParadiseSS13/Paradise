...
mob
	verb
		say_verb(message as text)
			...
			/*
			var
				first_letter = copytext(message,1,2)
...
mob
	proc
		say_understands(var/mob/other,var/datum/language/speaking = null)
...
mob
	proc
		say_quote(var/message, var/datum/language/speaking = null)
			var
				verb = "says"
			var
				ending = copytext(message, length(message))
...
mob
	proc
		emote(var/act, var/type, var/message)
			if(act == "me")
...
mob
	proc
		say_test(var/text)
			var
				ending = copytext(text, length(text))
			if(ending == "?")
				return "1"
			else if(ending == "!")
				return "2"
...
mob
	proc
		parse_message_mode(var/message, var/standard_mode="headset")
...
mob
	proc
		parse_language(var/message)
			var
				prefix = copytext(message,1,2)
			if(length(message) >= 1 && prefix == "!")
				return all_languages["Noise"]

			if(length(message) >= 2)
				var
					language_prefix = trim_right(lowertext(copytext(message, 1 ,4)))
				var
					datum
						language
