...
var
	list
		image
			ghost_darkness_images = list() //this is a list of images for things ghosts should still be able to see when they toggle darkness
	...
	dead
		observer
			...
			var
				bootime = 0
			var
				started_as_observer //This variable is set to 1 when you enter the game as an observer.
			...
			var
				image
					ghostimage = null //this mobs ghost image, for deleting and stuff
			var
				ghostvision = 1 //is the ghost able to see things humans can't?
			var
				seedarkness = 1
			var
				data_hud_seen = 0 //this should one of the defines in __DEFINES/hud.dm

mob
	dead
		observer
			New(var/mob/body=null, var/flags=1)
...
mob
	dead
		proc
			assess_targets(list/target_list, mob/dead/observer/U)
				var
					client
						C = U.client
...
mob
	proc
		ghostize(var/flags = GHOST_CAN_REENTER)
			if(key)
				var
					mob
						dead
							observer
								ghost = new(src, flags)	//Transfer safety to observer spawning proc.
				...
				if(stat == DEAD)
					...
					var
						mob
							dead
								observer
									ghost = ghostize(0)            //0 parameter is so we can never re-enter our body, "Charlie, you can never come baaaack~" :3
				...
				reenter_corpse()
					...
					var
						obj
							structure
								morgue
									Morgue = locate() in mind.current.loc
...
mob
	dead
		observer
			proc
				notify_cloning(var/message, var/sound, var/atom/source)
					if(message)
						...
						if(source)
							var
								obj
									screen
										alert
											A = throw_alert("\ref[source]_notify_cloning", /obj/screen/alert/notify_cloning)
							if(A)
			...
			verb
				toggle_antagHUD()
					...
					if(config.antag_hud_restricted && !M.has_enabled_antagHUD && !check_rights(R_ADMIN|R_MOD,0))
						var
							response = alert(src, "If you turn this on, you will not be able to take any part in the round.","Are you sure you want to turn this feature on?","Yes","No")
...
mob
	dead
		observer
			proc
				dead_tele(A in ghostteleportlocs)
					set category = "Ghost"
					set name = "Teleport"
...
mob
	dead
		observer
			verb
				follow(input in getmobs())
					set category = "Ghost"
					set name = "Follow" // "Haunt"
					set desc = "Follow and haunt a mob."

					var
						target = getmobs()[input]
					if(!target) return
					ManualFollow(target)
...
mob
	dead
		observer
			proc
				ManualFollow(var/atom/movable/target)
					if(!target)
						return
...
mob
	dead
		observer
			verb
				jumptomob(target in getmobs()) //Moves the ghost instead of just changing the ghosts's eye -Nodrak
					set category = "Ghost"
					...
					if(istype(usr, /mob/dead/observer)) //Make sure they're an observer!
						...
						if(!target)//Make sure we actually have a target
							...
							var
								mob
									M = getmobs()[target] //Destination mob
							var
								turf
									T = get_turf(M) //Turf of the destination mob
				...
				boo()
					...
					var
						obj
							machinery
								light
									L = locate(/obj/machinery/light) in view(1, src)
					if(L)
						L.flicker()
			...
			verb
				analyze_air()
					...
					var
						datum
							gas_mixture
								environment = usr.loc.return_air()
...
proc
	ghost_follow_link(var/atom/target, var/atom/ghost)
		...
		observer
			Topic(href, href_list)
				...
				if(href_list["follow"])
					var
						atom
							target = locate(href_list["follow"])
					if(target)
						ManualFollow(target)
