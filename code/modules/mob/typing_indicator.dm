...
mob
	var
		hud_typing = 0 //set when typing in an input window instead of chatline
mob
	var
		typing
mob
...
mob
	proc
		set_typing_indicator(var/state)
...
mob
	verb
		say_wrapper()
			...
			hud_typing = 1
			var
				message = input("","say (text)") as null|text
			hud_typing = 0
...
mob
	verb
		me_wrapper()
			...
			hud_typing = 1
			var
				message = input("","me (text)") as null|text
			hud_typing = 0
			set_typing_indicator(0)
			if(message)
				me_verb(message)
