...
//	var/force_down //determines if the affecting mob will be pinned to the ground //disabled due to balance, kept for an example for any new things.
	var
		dancing //determines if assailant and affecting keep looking at each other. Basically a wrestling position
...
obj
	item
		weapon
			grab
				New(var/mob/user, var/mob/victim)
					..()
...
obj
	item
		weapon
			grab
				proc
					clean_grabbed_by(var/mob/user, var/mob/victim) //Cleans up any nulls in the grabbed_by list.
						if(istype(user))
				...
				proc
					adjust_position()
						...
						var
							shift = 0
						var
							adir = get_dir(assailant, affecting)
						affecting.layer = 4
						switch(state)
							if(GRAB_PASSIVE)
								shift = 8
...
obj
	item
		weapon
			grab
				attack(mob/living/M, mob/living/carbon/user)
					if(!affecting)
						return
											...
											obj
												item
													organ
														internal
															eyes
																eyes = affected.get_int_organ(/obj/item/organ/internal/eyes)
...
obj
	item
		weapon
			grab
				proc
					checkvalid(var/mob/attacker, var/mob/prey) //does all the checking for the attack proc to see if a mob can eat another with the grab
...
obj
	item
		weapon
			grab
				proc
					checktime(var/mob/attacker, var/mob/prey) //Returns the time the attacker has to wait before they eat the prey
						if(isalien(attacker))
