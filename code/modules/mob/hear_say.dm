...
mob
	proc
		hear_say(var/message, var/verb = "says", var/datum/language/language = null, var/alt_name = "",var/italics = 0, var/mob/speaker = null, var/sound/speech_sound, var/sound_vol)
			if(!client)
				return
			...
			if((T) && (!(istype(src, /mob/dead/observer)))) //Ghosts can hear even in vacuum.
				...
				var
					pressure = (environment)? environment.return_pressure() : 0
			...
			if(sdisabilities & DEAF || ear_deaf)
				...
				if(speech_sound && (get_dist(speaker, src) <= world.view && src.z == speaker.z))
					var
						turf
							source = speaker? get_turf(speaker) : get_turf(src)
					src.playsound_local(source, speech_sound, sound_vol, 1)


mob
	proc
		hear_radio(var/message, var/verb="says", var/datum/language/language=null, var/part_a, var/part_b, var/mob/speaker = null, var/hard_to_hear = 0, var/vname ="", var/atom/follow_target)
			...
			if(isAI(src) && !hard_to_hear)
				var
					jobname // the mob's "job"
				var
					mob
						living
							carbon
								human
									impersonating //The crewmember being impersonated, if any.
...
mob
	proc
		hear_signlang(var/message, var/verb = "gestures", var/datum/language/language, var/mob/speaker = null)
			if(!client)
				return
	...
	proc
		hear_sleep(var/message)
			...
			if(prob(15))
				...
				var
					R = rand(1, messages.len)
				var
					heardword = messages[R]
