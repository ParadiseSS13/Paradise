obj
	tram
		tram_controller
			...
			var
				list
					blacklist = list(/obj/tram/rail,/atom/movable/lighting_overlay)
			var
				list
					ancwhitelist = list(/obj/tram, /obj/vehicle, /obj/structure/stool/bed/chair, /obj/structure/grille, /obj/structure/window)
				...
				init_floors()
					...
					var
						obj
							tram
								floor
									TTF = locate(/obj/tram/floor) in T
				...
				init_walls()
					...
					var
						obj
							tram
								floor
									TTW = locate(/obj/tram/wall) in T //Find and link wall on controller turf
						...
						for(var/cdir in cardinal)
							var
								obj
									tram
										wall
											TW = locate(/obj/tram/wall) in get_step(TF,cdir)
							if(istype(TW))
...
obj
	tram
		tram_controller
			proc
				check_validity(var/atom/movable/AM)
					if(!AM)	return 0
...
obj
	tram
		tram_controller
			proc
				add_floor(var/obj/tram/floor/TF)
					if(!istype(TF))	return
...
obj
	tram
		tram_controller
			proc
				remove_floor(var/obj/tram/floor/TF)
					if(!istype(TF))	return
...
obj
	tram
		tram_controller
			proc
				remove_wall(var/obj/tram/wall/TW)
					if(!istype(TW))	return
					if(TW in tram_walls)
						tram_walls -= TW
					TW.controller = null
...
obj
	tram
		tram_controller
			proc
				add_controller(var/obj/tram/controlpad/CCT)
...
obj
	tram
		tram_controller
			proc
				remove_controller(var/obj/tram/controlpad/CCT)
					if(!istype(CCT))	return
