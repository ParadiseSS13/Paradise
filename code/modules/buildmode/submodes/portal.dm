...
/datum/buildmode_mode/portal/proc/process_portals(user)
	if(two_way)
		...
		message_admins("[key_name_admin(user)] Created a two-way portal from [get_area(origin)] to [get_area(destination)] for [lifetime / 10] seconds")
	else
		spawn_portal(origin, destination, user)
		message_admins("[key_name_admin(user)] Created a portal from [get_area(origin)] to [get_area(destination)] for [lifetime / 10] seconds")
