		...
		mineral
			ore_redemption
				...
				var
					stk_amt   = list()
				var
					stack_list[0] //Key: Type.  Value: Instance of type.
				var
					obj
						item
							weapon
				...
				var
					list
						ore_values = list(("sand" = 1), ("iron" = 1), ("plasma" = 15), ("silver" = 16), ("gold" = 18), ("uranium" = 30), ("diamond" = 50), ("bananium" = 60), ("tranquillite" = 60))
				var
					list
						supply_consoles = list("Science", "Robotics", "Research Director's Desk", "Mechanic", "Engineering" = list("metal", "glass", "plasma"), "Chief Engineer's Desk" = list("metal", "glass", "plasma"), "Atmospherics" = list("metal", "glass", "plasma"), "Bar" = list("uranium", "plasma"), "Virology" = list("plasma", "uranium", "gold"))
...
obj
	machinery
		mineral
			ore_redemption
				proc
					process_sheet(obj/item/weapon/ore/O)
						var
							obj
								item
									stack
										sheet
											processed_sheet = SmeltMineral(O)
						if(processed_sheet)
							if(!(processed_sheet in stack_list)) //It's the first of this sheet added
								...
								var
									msg = "\[[worldtime2text()]\]: [capitalize(s.name)] sheets have been stocked in the ore reclaimer."
							...
							var
								obj
									item
										stack
											sheet
												storage = stack_list[processed_sheet]
...
obj
	machinery
		mineral
			ore_redemption
				attackby(var/obj/item/weapon/W, var/mob/user, params)
					if(!powered())
						return
...
obj
	machinery
		mineral
			ore_redemption
				proc
					SmeltMineral(var/obj/item/weapon/ore/O)
						if(O.refined_type)
							var
								obj
									item
			...
			ore_redemption
				interact(mob/user)
					...
					var
						datum
							browser
								popup = new(user, "console_stacking_machine", "Ore Redemption Machine", 400, 500)
				...
				Topic(href, href_list)
							...
							var
								obj
									item
										stack
											sheet
												inp = stack_list[text2path(href_list["release"])]
							var
								obj
									item
										stack
											sheet
												out = new inp.type()
							var
								desired = input("How much?", "How much to eject?", 1) as num
					...
					if(href_list["plasteel"])
						if(check_access(inserted_id) || allowed(usr))
							...
							var
								desired = input("How much?", "How much would you like to smelt?", 1) as num
							var
								obj
									item
										stack
			...
			equipment_vendor
				interact(mob/user)
					...
					var
						datum
							browser
								popup = new(user, "miningvendor", "Mining Equipment Vendor", 400, 350)
					popup.set_content(dat)
					popup.open()
					return
						...
						if(istype(inserted_id))
							var
								datum
									data
										mining_equipment
											prize = locate(href_list["purchase"])
...
obj
	machinery
		mineral
			equipment_vendor
				proc
					RedeemVoucher(obj/item/weapon/mining_voucher/voucher, mob/redeemer)
						var
							selection = input(redeemer, "Pick your equipment", "Mining Voucher Redemption") as null|anything in list("Kinetic Accelerator", "Resonator", "Mining Drone", "Advanced Scanner")
							...
							obj
								effect
									portal
										wormhole
											jaunt_tunnel
												J = new /obj/effect/portal/wormhole/jaunt_tunnel(get_turf(src), chosen_beacon)
						try_move_adjacent(J)
...
obj
	item
		weapon
			resonator
				proc
					CreateResonance(var/target, var/creator)
						var
							turf
								T = get_turf(target)
...
obj
	item
		weapon
			resonator
				afterattack(atom/target, mob/user, proximity_flag)
					if(proximity_flag)
...
obj
	effect
		resonance
			New(loc, var/creator = null, var/timetoburst)
				var
					turf
				...
				if(istype(proj_turf, /turf/simulated/mineral))
					...
					var
						datum
							gas_mixture
								environment = proj_turf.return_air()
					var
					...
					wanted_objects = list(/obj/item/weapon/ore/diamond, /obj/item/weapon/ore/gold, /obj/item/weapon/ore/silver,
												obj
													item
														weapon
															ore
																plasma,  /obj/item/weapon/ore/uranium,    /obj/item/weapon/ore/iron,
												obj
													item
														weapon
															ore
																bananium, /obj/item/weapon/ore/tranquillite, /obj/item/weapon/ore/glass)
...
obj
	item
		device
			mine_bot_ugprade
				afterattack(mob/living/simple_animal/hostile/mining_drone/M, mob/user)
					if(!istype(M))
						return
					upgrade_bot(M, user)
...
obj
	item
		device
			mine_bot_ugprade
				proc
					upgrade_bot(mob/living/simple_animal/hostile/mining_drone/M, mob/user)
				...
				adv_mining_scanner
					scan()
						if(!cooldown)
							...
							var
								list
									mobs = recursive_mob_check(t, client_check = 1, sight_check = 0, include_radio = 0)
							if(!mobs.len)
								return
			...
			for(var/mob/user in mobs)
				if(user.client)
					...
					for(var/turf/simulated/mineral/M in minerals)
						...
						var
							image
								I = image('icons/turf/mining.dmi', loc = F, icon_state = M.scan_state, layer = 18)
						C.images += I
						spawn(30)
							if(C)
								C.images -= I
...
obj
	item
		weapon
			hivelordstabilizer
				afterattack(obj/item/organ/internal/M, mob/user)
					var
						obj
							item
								organ
