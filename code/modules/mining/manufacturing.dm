		...
		manufacturer
			...
			Topic(href, href_list)
				...
				if((usr.contents.Find(src) || ((get_dist(src, usr) <= 1) && istype(src.loc, /turf))))
					...
					if(href_list["delete"])
						var
							operation = text2num(href_list["delete"])
					...
					if(href_list["eject"])
						var
							operation = text2num(href_list["eject"])
						var
							ejectamt = 0
						var
							ejecting = null
					...
					if(href_list["disp"])
						...
						var
							worktime = I.time * 10
						var
							powconsume = round(1500*worktime/3)
						/*for(var/i = robotics_research.starting_tier, i <= robotics_research.max_tiers, i++)
