...
var
	global
		list
			mining_surprises = subtypesof(/mining_surprise)
...
surprise_room
	...
	proc
		UpdateTurf(var/turf/T, var/no_adjacent=0)
	...
	proc
		AddTypeToTurf(var/turf/T, var/newtype)
			var
				surprise_turf_info
					sti = GetTurfInfo(T)
			sti.types |= newtype
	...
	proc
		UpdateAdjacentsOfTurf(var/turf/T)
			var
				surprise_turf_info
					Ti = turf_info[T]
...
layout_rule
	...
	var
		min_to_place=5 // Force placement at ALL candidates.
	var
		max_to_place=10 // 0 = max amount of ALL candidates.
	...
	var
		list
			decorations=list() // types, empty for no decorations
	...
	proc
		Plop(var/turf/T)
			...
			if(decorations.len)
				var
					decoration = pickweight(decorations)
				new decoration(T)
...
layout_rule
	place_adjacent
		...
		proc
			IsTurfCandidate(var/turf/T,var/list/opt_nt,var/list/opt_nnt)
				var
					surprise_turf_info
						sti = room.GetTurfInfo(T)
				for(var/dir in cardinal)
					var
						di = "[dir]"
...
mining_surprise
	...
	proc
		spawn_complex(var/atom/start_loc)
			...
			while(nrooms && maxtries)
				var
					sx=rand(3,room_size_max)
				var
					sy=rand(3,room_size_max)
				var
					o_x=l_size_x?rand(0,l_size_x):0
				var
					o_y=l_size_y?rand(0,l_size_y):0
				var
					atom
						npos
	...
	proc
		postProcessRoom(var/surprise_room/room)
			...
			for(var/i=0;i<=rand(5,10);i++)
				...
				var
					turf
						T = pick(candidates)
				var
					thing = pickweight(fluffitems)
				if(thing==null)
					continue
				new thing(T)

	proc
		spawn_room(var/atom/start_loc, var/x_size, var/y_size, var/clean=0)
