								...
								if(!alone_in_area(get_area(src), usr))
									...
									switch(shuttle_master.moveShuttle("laborcamp","laborcamp_home"))
										...
										if(3)
											...
											var
												message = "[inserted_id.registered_name] has returned to the station. Minerals and Prisoner ID card ready for retrieval."
								...
								if(alone_in_area(get_area(loc), usr))
									var
										obj
											docking_port
												stationary
													S = shuttle_master.getDock("laborcamp_home")
									if(S && S.get_docked())
		...
		mineral
			stacking_machine
				laborstacker
					...
					var
						list
							ore_values = list(("glass" = 1), ("metal" = 2), ("solid plasma" = 20), ("plasteel" = 23), ("reinforced glass" = 4), ("gold" = 20), ("silver" = 20), ("uranium" = 20), ("diamond" = 25), ("bananium" = 50), ("tranquillite" = 50))
...
obj
	machinery
		mineral
			stacking_machine
				laborstacker
					process_sheet(obj/item/stack/sheet/inp)
						if(istype(inp))
							var
								n = inp.name
							var
