/*********************Kinetic Crusher****************/
/obj/item/twohanded/required/kinetic_crusher //Seriously who called it both hammer and crusher, that was confusing as shit
...
/obj/item/twohanded/required/kinetic_crusher/attackby(obj/item/A, mob/user)
...
/obj/item/twohanded/required/kinetic_crusher/attack(mob/living/target, mob/living/carbon/user)
	var/datum/status_effect/crusher_damage/C = target.apply_status_effect(STATUS_EFFECT_CRUSHERDAMAGETRACKING)
	...
	if(istype(target, /mob/living/simple_animal/hostile/megafauna))
		var/mob/living/simple_animal/hostile/megafauna/M = target
		M.crusher_damage += target_health - target.health // Increase amount of damage
...
/obj/item/projectile/destabilizer/on_hit(atom/target, blocked = FALSE, hit_zone)
	if(isliving(target))
		var/mob/living/L = target
...
/obj/effect/temp_visual/hierophant/wall/crusher/New()
	. = ..()
	queue_smooth_neighbors(src)
	queue_smooth(src)