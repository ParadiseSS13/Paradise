...
/obj/item/wormhole_jaunter/contractor/activate(mob/user)
	if(!turf_check(user))
		return
	if(istype(get_area(src), /area/ruin/space/telecomns)) //It should work in the depot, because it's syndicate, but I don't want someone lighting the flare in the middle of telecomns and calling it a day.
