...
datum
	materials
		proc
			addAmount(var/mat_id,var/amount)
...
datum
	materials
		proc
			removeAmount(var/mat_id,var/amount)
				if(!(mat_id in storage))
		...
		proc
			getAmount(var/mat_id)
				...
				var
					datum
						material
							mat=getMaterial(mat_id)
				return mat.stored

datum
	materials
		proc
			getMaterial(var/mat_id)
				if(!(mat_id in storage))
...
datum
	material
		...
		var
			stored=0
		var
			cc_per_sheet=CC_PER_SHEET_MISC
		var
			oretype=null
		var
			sheettype=null
