	...
	simulated
		mineral //wall piece
			...
			var
				mineralName = null
			var
				spread = 0 //will the seam spread?
			var
				spreadChance = 0 //the percentual chance of an ore spreading to the neighbouring tiles
			var
				last_act = 0
			var
				scan_state = null //Holder for the image we display when we're pinged by a mining scanner
			var
				hidden = 1
			...
			random
				New()
					...
					if(prob(mineralChance))
						var
							mName = pickweight(mineralSpawnChanceList) //temp mineral name
	...
	simulated
		mineral
			gibtonite
				...
				var
					det_time = 8 //Countdown till explosion, but also rewards the player for how close you were to detonation when you defuse it
				var
				...
				var
					activated_ckey = null //These are to track who triggered the gibtonite deposit for logging purposes
				var
					activated_name = null
...
turf
	simulated
		mineral
			gibtonite
				proc
					explosive_reaction(var/mob/user = null, triggered_by_explosion = 0)
						if(stage == 0)
...
turf
	simulated
		mineral
			gibtonite
				gets_drilled(var/mob/user, triggered_by_explosion = 0)
								...
								item
									weapon
										twohanded
											required
												gibtonite
													G = new /obj/item/weapon/twohanded/required/gibtonite/(src)
						if(det_time <= 0)
							G.quality = 3
								...
								floor
									plating
										airless
											asteroid
												gibtonite_remains
													G = ChangeTurf(/turf/simulated/floor/plating/airless/asteroid/gibtonite_remains)
...
turf
	simulated
		mineral
			attackby(var/obj/item/weapon/pickaxe/P as obj, mob/user as mob, params)
							...
							simulated
								floor
									plating
										airless
											asteroid
												N = ChangeTurf(/turf/simulated/floor/plating/airless/asteroid)
...
turf
	simulated
		mineral
			attack_alien(var/mob/living/carbon/alien/M)
					...
					asteroid
						cave
							var
								length = 100
							var
								mob_spawn_list = list("Goldgrub" = 1, "Goliath" = 5, "Basilisk" = 4, "Hivelord" = 3)
							var
								sanity = 1
		...
		floor
			plating
				airless
					asteroid
						cave
							New(loc, var/length, var/go_backwards = 1, var/exclude_dir = -1)
										...
										for(var/edge_angle in L)
											var
												turf
													simulated
														mineral
															edge = get_step(tunnel, angle2dir(dir2angle(dir) + edge_angle))
											if(istype(edge))
												SpawnFloor(edge)
								...
								SpawnFloor(var/turf/T)
									...
									var
										turf
											simulated
												floor
													t = new /turf/simulated/floor/plating/airless/asteroid(T)
									spawn(2)
