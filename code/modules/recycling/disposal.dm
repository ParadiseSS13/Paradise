...
/obj/machinery/disposal/MouseDrop_T(mob/target, mob/user)
	...
	var/msg
	for(var/mob/V in viewers(usr))
		if(target == user && !user.stat && !user.IsKnockdown() && !user.IsStun() && !user.IsUnconscious())
			V.show_message("[usr] starts climbing into the disposal.", 3)
		if(target != user && !user.restrained() && !user.stat && !user.IsKnockdown() && !user.IsStun() && !user.IsUnconscious())
	...
	if(target_loc != target.loc)
		return
	if(target == user && !user.stat && !user.IsKnockdown() && !user.IsStun() && !user.IsUnconscious())	// if drop self, then climbed in
											// must be awake, not stun or whatever
