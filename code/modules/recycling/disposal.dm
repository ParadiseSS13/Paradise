	...
	machinery
		disposal
			...
			var
				datum
					gas_mixture
						air_contents	// internal reservoir
			var
				mode = 1	// item mode 0=off 1=charging 2=charged
			var
				flush = 0	// true if flush handle is pulled
			var
				obj
					structure
						disposalpipe
							trunk
								trunk = null // the attached pipe trunk
			var
				flushing = 0	// true if flushing in progress
			var
				flush_every_ticks = 30 //Every 30 ticks it will look whether it is ready to flush
			var
				flush_count = 0 //this var adds 1 once per tick. When it reaches flush_every_ticks it resets and tries to flush.
			var
				last_sound = 0
		...
		disposal
			initialize()
				...
				var
					datum
						gas_mixture
							removed = env.remove(SEND_PRESSURE + 1)
	...
	machinery
		disposal
			ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1)
				...
				var
					pressure_round = round(pressure,1)
	...
	machinery
		disposal
			process()
				...
				var
					atom
						L = loc						// recharging from loc turf
				...
				var
					datum
						gas_mixture
							env = L.return_air()
				var
					pressure_delta = (SEND_PRESSURE*1.01) - air_contents.return_pressure()
	...
	structure
		disposalholder
			...
			var
				datum
					gas_mixture
						gas = null	// gas used to flush, will appear at exit point
			var
				active = 0	// true if the holder is moving, otherwise inactive
			dir = 0
			var
				count = 1000	//*** can travel 1000 steps before going inactive (in case of loops)
			var
				has_fat_guy = 0	// true if contains a fat person
			var
				destinationTag = 0 // changes if contains a delivery container
			var
				tomail = 0 //changes if contains wrapped package
			var
				hasmob = 0 //If it contains a mob
			...
			proc
				init(var/obj/machinery/disposal/D)
			...
			proc
				start(var/obj/machinery/disposal/D)
					if(!D.trunk)
			...
			proc
				findpipe(var/turf/T)
					...
					var
						fdir = turn(dir, 180)	// flip the movement direction
			...
			proc
				merge(var/obj/structure/disposalholder/other)
	...
	structure
		disposalpipe
			sortjunction
				...
				icon_state = "pipe-j1s"
				var
					sortType = 0	//Look at the list called TAGGERLOCATIONS in setup.dm
				var
					posdir = 0
				var
					negdir = 0
				...
				proc
					updatedesc()
						...
						if(sortType>0)
							var
								tag = uppertext(TAGGERLOCATIONS[sortType])
				...
				transfer(var/obj/structure/disposalholder/H)
					var
						nextdir = nextdir(H.dir, H.destinationTag)
					H.dir = nextdir
					var
						turf
							T = H.nextloc()
	...
	structure
		disposalpipe
			wrapsortjunction
				...
				transfer(var/obj/structure/disposalholder/H)
					var
						nextdir = nextdir(H.dir, H.tomail)
					H.dir = nextdir
					var
						turf
							T = H.nextloc()
	...
	structure
		disposalpipe
			trunk
				...
				var
					obj
						linked 	// the linked obj/machinery/disposal or obj/disposaloutlet
...
obj
	structure
		disposaloutlet
			...
			var
				turf
					target	// this will be where the output objects are 'thrown' to.
			var
				mode = 0
