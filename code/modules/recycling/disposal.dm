...
/obj/machinery/disposal/MouseDrop_T(mob/living/target, mob/living/user)
	...
	var/msg
	for(var/mob/V in viewers(usr))
		if(target == user && !user.stat && !user.IsWeakened() && !user.IsStunned() && !user.paralysis)
			V.show_message("[usr] starts climbing into the disposal.", 3)
		if(target != user && !user.restrained() && !user.stat && !user.IsWeakened() && !user.IsStunned() && !user.paralysis)
	...
	if(QDELETED(src) || target_loc != target.loc)
		return
	if(target == user && !user.stat && !user.IsWeakened() && !user.IsStunned() && !user.paralysis)	// if drop self, then climbed in
