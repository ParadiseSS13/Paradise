			...
			attackby(obj/item/W as obj, mob/user as mob, params)
				if(istype(W, /obj/item/device/destTagger))
					...
					if(sortTag != O.currTag)
						var
							tag = uppertext(TAGGERLOCATIONS[O.currTag])
					...
					var
						str = copytext(sanitize(input(user,"Label text?","Set label","")),1,MAX_NAME_LEN)
					if(!str || !length(str))
						...
						if(use(1))
							var
								obj
									item
										smallDelivery
											P = new /obj/item/smallDelivery(get_turf(O.loc))	//Aaannd wrap it up!
						...
						if(use(3))
							var
								obj
									structure
										bigDelivery
											P = new /obj/structure/bigDelivery(get_turf(O.loc))
				...
				flush()
					...
					var
						obj
							structure
								disposalholder
									H = new()	// virtual holder object which actually
