...
/obj/machinery/conveyor/proc/handle_rotation(mob/user, params) //Rotates the belt depending on where you clicked it
	var/list/click_params = params2list(params)
	var/word = "rotate"
	if(!click_params)
		to_chat("<span class='notice'>You fail to [word] [src].</span>")
		return
	var/x = click_params["icon-x"] ? text2num(click_params["icon-x"]) : null
	var/y = click_params["icon-y"] ? text2num(click_params["icon-y"]) : null
	switch(x)
		if(1 to 11)
			. = WEST
		if(21 to 32)
	...
	switch(y)
		...
		if(21 to 32)
			. |= NORTH
	if(!.)
		dir = turn(dir, clockwise ? -45 : 45)
	if(dir == . && dir in cardinal)
		return
	else if(dir == . && dir in diagonals)
		clockwise = !clockwise
		word = "flip"
	else
		dir = .
	to_chat(user, "<span class='notice'>You [word] [src].</span>")
	update_move_direction()
