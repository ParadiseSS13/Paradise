...
obj
	effect
		portal_sensor
			...
			var
				datum
					owner			// owner that receive signals
			var
				list
					params[0]		// what to send to the main object to indicate which sensor
			var
				trigger_limit = 5	// number of time we're allowed to trigger per ptick
					...
					if(istype(T) && T.lighting_overlay && !T.lighting_overlay.needs_update)
						var
							atom
								movable
									lighting_overlay
										O = T.lighting_overlay
						var
							hash = O.lum_r + O.lum_g + O.lum_b
						if(hash != light_hash)
							light_hash = hash
							trigger()
					else
...
turf
	unsimulated
		floor
			upperlevel
				proc
					init(var/obj/effect/levelref/R)
...
turf
	unsimulated
		floor
			upperlevel
				Entered(atom/movable/AM, atom/OL, ignoreRest = 0)
					...
					trigger()
						...
						for(var/X in list(lower_turf) + lower_turf.contents)
							...
							if(A && A.invisibility <= SEE_INVISIBLE_LIVING)
								var
									image
										I = image(A, layer = AREA_LAYER + A.layer * 0.01, dir = A.dir)
								I.pixel_x = A.pixel_x
								I.pixel_y = A.pixel_y
								underlays += I
...
obj
	effect
		view_portal
			...
			var
				id = null				// id of other portal turf we connect to
...
obj
	effect
		view_portal
			proc
				can_link(obj/effect/view_portal/P)
	...
	effect
		view_portal
			visual
				...
				appearance_flags = TILE_BOUND | KEEP_TOGETHER
				var
					dist = 6				// dist that we render out
				var
					radius = 3				// dist we render on other axis, in each direction
				var
					frustrum = 0			// if 1, get wider and wider at each step outward
				var
					teleport = 1			// should teleport?
		...
		view_portal
			visual
				linkup()
					...
					// setup references
					var
						crossdir = angle2dir((dir2angle(dir) + 90) % 360)
		...
		view_portal
			visual
				proc
					setup_near()
						var
							nvs = dir & (EAST|WEST) ? near_viewpoint.x - x : near_viewpoint.y - y
						if(nvs)
							nvs = SIGN(nvs)
						// need a mob for view() to work correctly
						var
							mob
								M = new(near_viewpoint)
					...
					trigger(near, turf/T)
						...
						for(var/AX in list(T) + T.contents)
							...
							if(A && A.invisibility <= SEE_INVISIBLE_LIVING)
								var
									image
										I = image(A, layer = D.layer + A.layer * 0.01, dir = A.dir)
								I.pixel_x = A.pixel_x
								I.pixel_y = A.pixel_y
								D.overlays += I
...
obj
	effect
		view_portal_dummy
			New(obj/effect/view_portal/visual/V, near, turf/T)
				..()
				if(!near)
					loc = V.loc
				owner = V
