...
/obj/effect/mob_spawn/human/alive/golem/Initialize(mapload, datum/species/golem/species = null, mob/creator = null)
	...
	if(!mapload && A)
		if(COOLDOWN_FINISHED(src, ghost_flash_cooldown))
			notify_ghosts("\A [initial(species.prefix)] golem shell has been completed in [A.name].", source = src, role = ROLE_SENTIENT)
			COOLDOWN_START(src, ghost_flash_cooldown, 20 MINUTES)
		else
			notify_ghosts("\A [initial(species.prefix)] golem shell has been completed in [A.name].", source = src, role = ROLE_SENTIENT, flashwindow = FALSE)
