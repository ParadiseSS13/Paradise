			...
			map_loader
				hotel_room
					New()
						...
						var
							static
								path = "_maps/map_files/templates/spacehotel/"
						var
							global
								loaded = 0
						if(!loaded)
							...
							for(var/map in flist(path))
								var
									datum
										map_template
											T = new(path = "[path][map]", rename = "[map]")
						...
						var
							datum
								map_template
									M = safepick(dir == NORTH ? north_room_templates : south_room_templates)
						if(M)
							template = M
							load(M)
							if(dir == NORTH)
						...
						obj
							item
								weapon
									card
										hotel_card
											card// room's key card
					var
						mob
							living
								occupant = null			// the current room occupant
					var
						datum
							money_account
								account			// Account we're pulling from
					var
						roomtimer							// timer PS handle for updating room
...
obj
	machinery
		door
			unpowered
				hotel_door
					allowed(mob/living/carbon/user)
...
obj
	effect
		hotel_controller
			...
			var
				list
					room_doors[0]			// assoc list of [room id]=hotel_door
			var
				list
					vacant_rooms[0]		// list of vacant room doors
			var
				list
					guests[0]				// assoc list of [guest mob]=room id
			...
			var
				obj
					item
						device
							radio
								radio	// for shouting at deadbeats
...
obj
	effect
		hotel_controller
			proc
				add_room(obj/machinery/door/unpowered/hotel_door/D)
					room_doors["[D.id]"] = D
					vacant_rooms += D
...
obj
	effect
		hotel_controller
			proc
				checkin(roomid, mob/living/carbon/occupant, obj/item/weapon/card/id/id)
					if(!istype(occupant))
						return null
					var
						obj
