	...
	effect
		landmark
			corpse
				name = "Unknown"
				var
					mobname = "Unknown"  //Unused now but it'd fuck up maps to remove it now
				var
				...
				var
					corpseidicon = null //For setting it to be a gold, silver, centcomm etc ID
				var
					timeofdeath = null
				var
					coffin = 0
...
obj
	effect
		landmark
			corpse
				proc
					createCorpse() //Creates a mob and checks for gear in each slot before attempting to equip it.
						var
							mob
								living
									carbon
										human
											human
												M = new /mob/living/carbon/human/human (src.loc)
								...
								obj
									structure
										closet
											coffin
												sarcophagus
													sarc = locate(/obj/structure/closet/coffin/sarcophagus) in loc
							if(sarc) M.loc = sarc
						qdel(src)
