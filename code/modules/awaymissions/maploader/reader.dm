...
/**
	...
	*/
dmm_suite
	load_map(dmm_file as file, x_offset = 0 as num, y_offset = 0 as num, z_offset as num, do_sleep = 1 as num)
		...
		var
			quote = ascii2text(34)
		var
			tfile = file2text(dmm_file)//the map file we're creating
		var
			tfile_len = length(tfile)
		var
			lpos = 1 // the models definition index
		...
		var
			list
				grid_models = list()
		var
			key_len = length(copytext(tfile,2,findtext(tfile,quote,2,0)))//the length of the model key (e.g "aa" or "aba")

		//proceed line by line
		for(lpos=1; lpos<tfile_len; lpos=findtext(tfile,"\n",lpos,0)+1)
			var
				tline = copytext(tfile,lpos,findtext(tfile,"\n",lpos,0))
			if(copytext(tline,1,2) != quote)//we reached the map "layout"
				break
			var
				model_key = copytext(tline,2,2+key_len)
			var
				model_contents = copytext(tline,findtext(tfile,"=")+3,length(tline))
		...
		for(var/zpos=findtext(tfile,"\n(1,1,",lpos,0);zpos!=0;zpos=findtext(tfile,"\n(1,1,",zpos+1,0))	//in case there's several maps to load
			...
			var
				zgrid = copytext(tfile,findtext(tfile,quote+"\n",zpos,0)+2,findtext(tfile,"\n"+quote,zpos,0)+1) //copy the whole map grid
			var
				z_depth = length(zgrid)

			//if exceeding the world max x or y, increase it
			var
				x_depth = length(copytext(zgrid,1,findtext(zgrid,"\n",2,0)))
			var
			...
			var
				y_depth = z_depth / (x_depth+1)//x_depth + 1 because we're counting the '\n' characters in z_depth
			...
			for(var/gpos=1;gpos!=0;gpos=findtext(zgrid,"\n",gpos,0)+1)
				var
					grid_line = copytext(zgrid,gpos,findtext(zgrid,"\n",gpos,0))
				...
				for(var/mpos in 1 to x_depth step key_len)
					xcrd++
					var
						model_key = copytext(grid_line,mpos,mpos+key_len)
...
dmm_suite
	proc
		parse_grid(model as text, xcrd as num, ycrd as num, zcrd as num, do_sleep = 1)
			/*Method parse_grid()
			...
			*/
			...
			var
				list
					members = list()//will contain all members (paths) in model (in our example : /turf/unsimulated/wall and /area/mine/explored)
			var
				list
					members_attributes = list()//will contain lists filled with corresponding variables, if any (in our example : list(icon_state = "rock") and list())
			...
			do
				...
				var
					full_def = copytext(model,old_position,dpos)//full definition, e.g : /obj/foo/bar{variables=derp}
				var
					atom_def = text2path(copytext(full_def,1,findtext(full_def,"{")))//path definition, e.g /obj/foo/bar
				...
				var
					variables_start = findtext(full_def,"{")
			...
			if(T)
				...
				while(index <= members.len)
					...
					var
						turf
							UT = instance_atom(members[index],members_attributes[index],xcrd,ycrd,zcrd)//instance new turf
...
dmm_suite
	proc
		instance_atom(path,list/attributes, x, y, z)
			var
				atom
					instance
...
dmm_suite
	proc
		trim_text(what as text,trim_quotes=0)
...
dmm_suite
	proc
		find_next_delimiter_position(text as text,initial_position as num, delimiter=",",opening_escape=quote,closing_escape=quote)
			var
...
dmm_suite
	proc
		readlist(text as text, delimiter=",")
			...
			do
				...
				var
					equal_position = findtext(text,"=",old_position, position)

				var
					trim_left = trim_text(copytext(text,old_position,(equal_position ? equal_position : position)),1)//the name of the variable, must trim quotes to build a BYOND compliant associatives list
...
dmm_suite
	proc
		add_underlying_turf(turf/placed,turf/underturf, list/turfs_underlays)
...
dmm_suite
	preloader
		proc
			setup(list/the_attributes, path)
