...
swapmap
	var
		id		// a string identifying this map uniquely
	var
		x1		// minimum x,y,z coords
	var
		y1
	var
		z1
	var
		x2		// maximum x,y,z coords (also used as width,height,depth until positioned)
	var
		y2
	var
		z2
	var
		tmp
			locked	// don't move anyone to this map; it's saving or loading
	var
		tmp
			mode	// save as text-mode
	var
		ischunk		// tells the load routine to load to the specified location
		...
		AllocateSwapMap()
			...
			if(!ischunk)
				if(world.maxz<=swapmaps_compiled_maxz)
					...
					var
						list
							l=ConsiderRegion(1,1,world.maxx,world.maxy,swapmaps_compiled_maxz+1)
					x1=l[1]
					y1=l[2]
					z1=l[3]
					qdel(l)
	...
	proc
		ConsiderRegion(X1,Y1,X2,Y2,Z1,Z2)
			while(1)
				var
					nextz=0
				var
	...
	proc
		CutXYZ()
			var
				mx=swapmaps_compiled_maxx
			var
				my=swapmaps_compiled_maxy
			var
				mz=swapmaps_compiled_maxz
