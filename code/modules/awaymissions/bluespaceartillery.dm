		...
		computer
			artillerycontrol
				...
				var
					last_fire = 0
				var
					reload_cooldown = 180 // 3 minute cooldown
				var
					area
						targetarea
			...
			artillerycontrol
				ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1)
					var
						data[0]
					var
						time_to_wait = round(reload_cooldown - ((world.time / 10) - last_fire), 1)
					var
					...
					if(targetarea)
						var
							areaname = sanitize(targetarea.name)
