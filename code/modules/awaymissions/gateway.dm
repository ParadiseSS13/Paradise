...
/obj/machinery/gateway/centeraway/Bumped(atom/movable/AM)
	...
	if(isliving(AM))
		...
		for(var/mob/living/L in AM.contents)
			if(exilecheck(L))
				atom_say("Rejecting [AM]: Exile microchip detected in contained lifeform.")
				return
	if(AM.has_buckled_mobs())
		for(var/mob/living/L in AM.buckled_mobs)
			if(exilecheck(L))
				atom_say("Rejecting [AM]: Exile microchip detected in close proximity lifeform.")
				return
...
/obj/machinery/gateway/centeraway/proc/exilecheck(mob/living/carbon/M)
	for(var/obj/item/implant/exile/E in M)//Checking that there is an exile microchip in the contents
		if(E.imp_in == M)//Checking that it's actually implanted vs just in their pocket
			to_chat(M, "<span class='notice'>The station gate has detected your exile microchip and is blocking your entry.</span>")
			return 1
	return 0
