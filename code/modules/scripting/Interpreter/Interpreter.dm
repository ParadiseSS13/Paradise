...
datum
	n_Interpreter
		...
		var
			datum
				container // associated container for interpeter
/*
	Var: status
...
datum
	n_Interpreter
		proc
			RaiseError(datum/runtimeError/e)
...
datum
	n_Interpreter
		proc
			CreateScope(datum/node/BlockDefinition/B)
				var
					datum
						scope
							S = new(B, curScope)
...
datum
	n_Interpreter
		proc
			RunBlock(var/datum/node/BlockDefinition/Block, var/datum/scope/scope = null)
				var
					is_global = istype(Block, /datum/node/BlockDefinition/GlobalBlock)
				if(!is_global)
					if(scope)
						curScope = scope
					else
					...
					for(var/datum/node/statement/S in Block.statements)
						...
						if(istype(S, /datum/node/statement/VariableAssignment))
							...
							if(!stmt.object)
								...
								var
									datum
										D = Eval(GetVariable(stmt.object.id_name))
								if(!D)
									return
							...
							if(!dec.object)
								...
								var
									datum
										D = Eval(GetVariable(dec.object.id_name))
								if(!D)
									return
...
datum
	n_Interpreter
		proc
			RunFunction(var/datum/node/statement/FunctionCall/stmt)
				...
				if(!stmt.object)							//A scope's function is being called, stmt.object is null
					...
					var
						O = GetVariable(stmt.object.id_name)	//Gets a reference to the object which is the target of the function call.
...
datum
	n_Interpreter
		proc
			RunIf(var/datum/node/statement/IfStatement/stmt)
				if(!stmt.skip)
					if(Eval(stmt.cond))
						RunBlock(stmt.block)
...
datum
	n_Interpreter
		proc
			RunWhile(var/datum/node/statement/WhileLoop/stmt)
				var
					i = 1
...
datum
	n_Interpreter
		proc
			Iterate(var/datum/node/BlockDefinition/block, count)
				RunBlock(block)
...
datum
	n_Interpreter
		proc
			AssignVariable(name, datum/node/expression/value, var/datum/scope/S = null)
