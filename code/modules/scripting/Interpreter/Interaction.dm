...
datum
	n_Interpreter
		proc
			Load(var/datum/node/BlockDefinition/GlobalBlock/program)
				ASSERT(program)
				src.program 	= program
				CreateGlobalScope()
				...
				var
					datum
						node
							statement
								FunctionDefinition
									func = globalScope.functions[name]
...
datum
	n_Interpreter
		proc
			HandleError(var/datum/runtimeError/e)
