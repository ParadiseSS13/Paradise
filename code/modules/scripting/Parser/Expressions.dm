...
datum
	n_Parser
		nS_Parser
			proc
				Precedence(var/datum/node/expression/operator/top, var/datum/node/expression/operator/input)
					if(istype(top))
						top = top.precedence
...
datum
	n_Parser
		nS_Parser
			proc
				GetExpression(var/datum/token/T)
					if(!T)
						return
...
datum
	n_Parser
		nS_Parser
			proc
				GetOperator(O, type = /datum/node/expression/operator, L[])
					if(istype(O, type))
...
datum
	n_Parser
		nS_Parser
			proc
				Reduce(var/datum/stack/opr, var/datum/stack/val)
					var
						datum
							node
								expression
...
datum
	n_Parser
		nS_Parser
			proc
				ParseExpression(var/list/end = list(/datum/token/end), list/ErrChars = list("{", "}"), check_functions = 0)
					var
						datum
							stack
								opr = new
					...
					for()
							...
							var
								datum
									node
										expression
											operator
												curOperator											//Figure out whether it is unary or binary and get a new instance.
						...
						if(!check_functions)
								...
								var
									datum
										token
											preToken = curToken
								var
									old_expect = src.expecting
								var
									fex = ParseFunctionExpression()
								if(old_expect != VALUE)
							...
							var
								datum
									n_Keyword
										kw = options.keywords[curToken.value]
							kw = new kw(inline=1)
							if(kw)
								if(!kw.Parse(src))
									return
