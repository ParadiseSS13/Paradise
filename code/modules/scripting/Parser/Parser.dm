...
datum
	n_Parser
		nS_Parser
			New(tokens[], var/datum/n_scriptOptions/options)
				src.tokens = tokens
				src.options = options
...
datum
	n_Parser
		nS_Parser
			proc
				AddBlock(var/datum/node/BlockDefinition/B)
		...
		nS_Parser
			proc
				ParseAssignment()
					...
					NextToken()
					var
						t = options.binary_operators[options.assign_operators[curToken.value]]
					var
						datum
							node
								statement
					...
					for()
						...
						var
							datum
								node
									expression
										P = ParseParamExpression(check_functions = 1)
						stmt.parameters += P
						if(istype(curToken, /datum/token/symbol) && curToken.value == ",")
							NextToken()
