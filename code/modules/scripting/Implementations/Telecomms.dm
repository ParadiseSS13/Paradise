		...
		var
			obj
				machinery
					telecomms
						server
							Holder	// the server that is running the code
		var
			ready = 1 // 1 if ready to run code
		...
		proc
			Run(var/datum/signal/signal)
				...
				*/
				...
				var
					message = interpreter.GetVar("$content")
				var
					regex
						bannedTags = new ("(<script|<iframe|<video|<audio)")
				...
				var
					setname = interpreter.GetVar("$source", signal.data["name"])
			...
			delay(var/time)
				...
				var
					datum
						n_Interpreter
							TCS_Interpreter
								interpreter = S.Compiler.interpreter
				// Backup the scope
				var
					datum
						scope
							globalScope = interpreter.globalScope
				var
					datum
						scope
							curScope = interpreter.curScope
				var
					list
						scopes = interpreter.scopes.stack.Copy()
				var
					list
						functions = interpreter.functions.stack.Copy()
				var
					datum
						node
							statement
								FunctionDefinition
									curFunction = interpreter.curFunction
				var
					status = interpreter.status
				var
					returnVal = interpreter.returnVal
				// Sleep
				sleep(time)
				// Restore the scope
...
datum
	signal
		proc
			tcombroadcast(var/message, var/freq, var/source, var/job)
				var
					datum
						signal
							newsign = new
				...
				var
					datum
						radio_frequency
							connection = radio_controller.return_frequency(freq)
