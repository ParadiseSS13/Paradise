...
// Clone of rand()
proc
	rand_chance(var/low = 0, var/high)
...
proc
	timestamp(var/format = "hh:mm:ss") // Get the game time in text
...
proc
	list_implode(var/list/li, var/separator)
...
proc
	n_clamp(var/num, var/min = 0, var/max = 1)
...
proc
	n_isInRange(var/const/x, var/const/min, var/const/max)
		if(isnum(x) && isnum(min) && isnum(max))
			return IsInRange(x, min, max)

proc
	n_lcm(var/const/a, var/const/b)
		if(isnum(a) && isnum(b))
			return Lcm(a, b)

proc
	n_root(var/const/n, var/const/x)
		if(isnum(n) && isnum(x))
			return Root(n, x)

proc
	n_toDegrees(var/const/x)
		if(isnum(x))
			return ToDegrees(x)

proc
	n_toRadians(var/const/x)
		if(isnum(x))
			return ToRadians(x)
...
proc
	n_replacetext(text, r, with)
		return replacetext(text, r, with)

proc
	n_replacetextEx(text, r, with)
		return replacetextEx(text, r, with)

proc
	n_jointext(list, glue)
		return jointext(list, glue)

proc
	n_splittext(text, delim)
