...
/datum/computer_file/program/card_mod/Topic(href, href_list)
	...
	switch(href_list["action"])
		if("PRG_modify")
			if(modify)
				SSrecords.manifest_modify(modify.registered_name, modify.assignment)
			...
			if(!printing && computer)
				...
				spawn(50)
					...
					if(mode == 2)
						...
						content = "<h4>Crew Manifest</h4><br>[SSrecords ? SSrecords.get_manifest(0) : ""]"
					else if(modify && !mode)
						title = "access report"
...
/datum/computer_file/program/card_mod/ui_data(mob/user)
	...
	data["manifest"] = SSrecords ? SSrecords.get_manifest(0) : null
