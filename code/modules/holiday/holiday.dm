datum
	holiday
		...
		var
			begin_month = 0
		var
			end_day = 0 // Default of 0 means the holiday lasts a single day
		var
			end_month = 0
		var
			eventChance = 0
...
datum
	holiday
		proc
			handle_event() //used for special holiday events
				return
...
datum
	holiday
		easter
			...
			var
				const
					days_early = 1 //to make editing the holiday easier
			var
				const
					days_extra = 6
...
client
	proc
		Set_Holiday(T as text|null)
			...
			var
				selected = input("What holiday would you like to force?","Holiday Forcing","--CANCEL--") in choice

			if(selected == "--CANCEL--")
				return

			var
				selected2path = text2path(selected)
