...
var
	global
		list
			library_section_names = list("Any", "Fiction", "Non-Fiction", "Adult", "Reference", "Religion")
...
datum
	cachedbook // Datum used to cache the SQL DB books locally in order to achieve a performance gain.
		...
		var
			author
		var
			ckey // ADDED 24/2/2015 - N3X
		var
			category
		var
			content
		var
			programmatic=0                // Is the book programmatically added to the catalog?
		var
			forbidden=0
		var
			path = /obj/item/weapon/book // Type path of the book to generate
		var
			flagged = 0
...
datum
	library_catalog
		proc
			flag_book_by_id(mob/user, id)
				var
					global
						books_flagged_this_round[0]
				...
				var
					DBQuery
						query = dbcon.NewQuery("UPDATE [format_table_name("library")] SET flagged = flagged + 1 WHERE id=[sqlid]")
				query.Execute()

datum
	library_catalog
		proc
			rmBookByID(mob/user, id)
				if("[id]" in cached_books)
					var
						datum
							cachedbook
				...
				var
					DBQuery
						query = dbcon.NewQuery("DELETE FROM [format_table_name("library")] WHERE id=[sqlid]")
				query.Execute()
	...
	library_catalog
		proc
			getBookByID(id)
				...
				var
					DBQuery
						query = dbcon.NewQuery("SELECT id, author, title, category, content, ckey, flagged FROM [format_table_name("library")] WHERE id=[sqlid]")
				query.Execute()
