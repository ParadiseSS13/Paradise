client
	proc
		delbook()
			...
			var
				isbn = input("ISBN number?", "Delete Book") as num | null
			if(!isbn)
				return

			var
				DBQuery
					query_delbook = dbcon.NewQuery("DELETE FROM [format_table_name("library")] WHERE id=[isbn]")
			if(!query_delbook.Execute())
				var
					err = query_delbook.ErrorMsg()
...
client
	proc
		view_flagged_books()
			...
			var
				DBQuery
					query = dbcon.NewQuery("SELECT id, title, flagged FROM [format_table_name("library")] WHERE flagged > 0 ORDER BY flagged DESC")
			if(!query.Execute())
				var
					err = query.ErrorMsg()
