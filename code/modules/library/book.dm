...
/obj/item/book/proc/edit_book(mob/user)
	...
	switch(choice)
		...
		if("Author")
			...
			author = newauthor
		if("Summary")
			var/newsummary = tgui_input_text(user, "Write the new summary:", "Summary", summary, MAX_SUMMARY_LEN, multiline = TRUE, submit_on_enter = FALSE)
		...
		if("Edit Current Page")
			...
			if(character_space_remaining <= 0)
				...
				return
			var/content = tgui_input_text(user, "Add Text to this page, you have [character_space_remaining] characters of space left:", "Edit Current Page", max_length = MAX_CHARACTERS_PER_BOOKPAGE, multiline = TRUE, submit_on_enter = FALSE)
			if(isnull(content))
				to_chat(user, "<span class='notice'>You change your mind.</span>")
				return
