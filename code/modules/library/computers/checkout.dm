				...
				checkout
					interact(var/mob/user)
						...
						switch(screenstate)
							...
							if(2)
								...
								for(var/datum/borrowbook/b in checkouts)
									var
										timetaken = world.time - b.getdate
									//timetaken *= 10
									timetaken /= 600
									timetaken = round(timetaken)
									var
										timedue = b.duedate - world.time
									//timedue *= 10
									timedue /= 600
									if(timedue <= 0)
								...
								if(!dbcon.IsConnected())
									...
									for(var/datum/cachedbook/CB in get_page(page_num))
										var
											author = CB.author
										var
											controls =  "<A href='?src=\ref[src];id=[CB.id]'>\[Order\]</A>"
						...
						var
							datum
								browser
									B = new /datum/browser(user, "library", "Book Inventory Management")
						B.set_content(dat)
						B.open()
				...
				checkout
					Topic(href, href_list)
						...
						if(href_list["settitle"])
							var
								newtitle = input("Enter a title to search for:") as text|null
							if(newtitle)
						...
						if(href_list["setcategory"])
							var
								newcategory = input("Choose a category to search for:") in (list("Any") + library_section_names)
							if(newcategory == "Any")
								query.category = null
							else if(newcategory)
						...
						if(href_list["setauthor"])
							var
								newauthor = input("Enter an author to search for:") as text|null
							if(newauthor)
						...
						if(href_list["del"])
							...
							var
								datum
									cachedbook
										target = getBookByID(href_list["del"]) // Sanitized in getBookByID
							var
								ans = alert(usr, "Are you sure you wish to delete \"[target.title]\", by [target.author]? This cannot be undone.", "Library System", "Yes", "No")
							if(ans=="Yes")
								var
									DBQuery
										query = dbcon.NewQuery("DELETE FROM [format_table_name("library")] WHERE id=[target.id]")
								var
						...
						if(href_list["delbyckey"])
							if(!check_rights(R_ADMIN))
								...
								return
							var
								tckey = ckey(href_list["delbyckey"])
							var
								ans = alert(usr,"Are you sure you wish to delete all books by [tckey]? This cannot be undone.", "Library System", "Yes", "No")
							if(ans=="Yes")
								var
									DBQuery
										query = dbcon.NewQuery("DELETE FROM [format_table_name("library")] WHERE ckey='[sanitizeSQL(tckey)]'")
								var
									response = query.Execute()
								if(!response)
									to_chat(usr, query.ErrorMsg())
											...
											obj
												item
													weapon
														storage
															bible
																B = new /obj/item/weapon/storage/bible(src.loc)
							...
							var
								obj
									item
										weapon
											book
												b = locate(href_list["delbook"])
							inventory.Remove(b)
						if(href_list["uploadauthor"])
							var
								newauthor = copytext(sanitize(input("Enter the author's name: ") as text|null),1,MAX_MESSAGE_LEN)
							if(newauthor && scanner)
								scanner.cache.author = newauthor
						if(href_list["uploadcategory"])
							var
								newcategory = input("Choose a category: ") in list("Fiction", "Non-Fiction", "Adult", "Reference", "Religion")
							if(newcategory)
						...
						if(href_list["upload"])
							if(scanner)
								if(scanner.cache)
									var
										choice = input("Are you certain you wish to upload this title to the Archive?") in list("Confirm", "Abort")
									if(choice == "Confirm")
										...
										if(!dbcon.IsConnected())
											...
											var
												sqltitle = sanitizeSQL(scanner.cache.name)
											var
												sqlauthor = sanitizeSQL(scanner.cache.author)
											var
												sqlcontent = sanitizeSQL(scanner.cache.dat)
											var
												sqlcategory = sanitizeSQL(upload_category)
											var
												DBQuery
													query = dbcon.NewQuery("INSERT INTO [format_table_name("library")] (author, title, content, category, ckey, flagged) VALUES ('[sqlauthor]', '[sqltitle]', '[sqlcontent]', '[sqlcategory]', '[ckey(usr.key)]', 0)")
											var
												response = query.Execute()
											if(!response)
												to_chat(usr, query.ErrorMsg())
						...
						if(href_list["id"])
							...
							var
								datum
									cachedbook
										newbook = getBookByID(href_list["id"]) // Sanitized in getBookByID
							if(!newbook)
								alert("No book found")
								return
	...
	machinery
		computer
			library
				checkout
					proc
						make_external_book(var/datum/cachedbook/newbook)
