			...
			library
				proc
					get_page(var/page_num)
						...
						var
							sql = "SELECT id, author, title, category, ckey, flagged FROM [format_table_name("library")] [searchquery] LIMIT [(page_num - 1) * LIBRARY_BOOKS_PER_PAGE], [LIBRARY_BOOKS_PER_PAGE]"

						// Pagination
						var
							DBQuery
								_query = dbcon.NewQuery(sql)
						_query.Execute()
						if(_query.ErrorMsg())
		...
		computer
			library
				proc
					get_num_results()
						var
							sql = "SELECT COUNT(*) FROM [format_table_name("library")]"
			...
			library
				proc
					get_pagelist()
						...
						var
							end = min(num_pages, page_num + 3)
						for(var/i = start to end)
							var
								dat = "<a href='?src=\ref[src];page=[i]'>[i]</a>"
							if(i == page_num)
