...
obj
	structure
		bookcase
			...
			var
				list
					valid_types = list(/obj/item/weapon/book, \
												obj
													item
														weapon
															tome, \
		...
		bookcase
			attackby(obj/O as obj, mob/user as mob, params)
				...
				if(is_type_in_list(O, valid_types))
					...
					var
						newname = stripped_input(user, "What would you like to title this [name]?")
					if(newname)
					...
					var
						obj
							item
								weapon
									book
										choice = input("Which book would you like to remove from \the [src]?") in contents as obj|null
					if(choice)
	...
	item
		weapon
			book
				...
				var
					carved = 0	 // Has the book been hollowed out for use as a secret storage item?
				var
					forbidden = 0     // Prevent ordering of this book. (0=no, 1=yes, 2=emag only)
				var
					obj
						item
				...
				attackby(obj/item/weapon/W as obj, mob/user as mob, params)
					...
					if(istype(W, /obj/item/weapon/pen))
						if(unique)
							...
							return 1
						var
							choice = input("What would you like to change?") in list("Title", "Contents", "Author", "Cancel")
						switch(choice)
							if("Title")
								var
									newtitle = reject_bad_text(stripped_input(usr, "Write a new title:"))
								if(!newtitle)
							...
							if("Contents")
								var
									content = strip_html(input(usr, "Write your book's contents (HTML NOT allowed):"), MAX_BOOK_MESSAGE_LEN) as message|null
								if(!content)
							...
							if("Author")
								var
									newauthor = stripped_input(usr, "Write the author's name:")
								if(!newauthor)
					...
					obj
						machinery
							computer
								library
									checkout
										computer // Associated computer - Modes 1 to 3 use this
				var
					obj
						item
							weapon
								book
									book	 //  Currently scanned book
				var
