...
obj
	machinery
		r_n_d
			protolathe
				proc
					check_mat(datum/design/being_built, var/M)	// now returns how many times the item can be built with the material
						var
							A = materials.amount(M)
						if(!A)
							...
							item
								stack
									sheet
										stack = O
					var
						amount = round(input("How many sheets do you want to add?") as num)//No decimals
					if(!in_range(src, stack) || !user.Adjacent(src))
						return
					var
						amount_inserted = materials.insert_stack(O,amount)
					if(!amount_inserted)
						return 1
					else
						busy = 1
