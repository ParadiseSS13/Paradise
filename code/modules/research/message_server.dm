...
datum
	data_pda_msg
		var
			recipient = "Unspecified" //name of the person
		var
			sender = "Unspecified" //name of the sender
		var
			message = "Blank" //transferred message

datum
	data_pda_msg
		New(var/param_rec = "",var/param_sender = "",var/param_message = "")
...
datum
	data_rc_msg
		var
			rec_dpt = "Unspecified" //name of the person
		var
			send_dpt = "Unspecified" //name of the sender
		var
			message = "Blank" //transferred message
		var
			stamp = "Unstamped"
...
datum
	data_rc_msg
		New(var/param_rec = "",var/param_sender = "",var/param_message = "",var/param_stamp = "",var/param_id_auth = "",var/param_priority)
			if(param_rec)
				rec_dpt = param_rec
			if(param_sender)
...
obj
	machinery
		message_server
			proc
				send_pda_message(var/recipient = "",var/sender = "",var/message = "")
...
obj
	machinery
		message_server
			proc
				send_rc_message(var/recipient = "",var/sender = "",var/message = "",var/stamp = "", var/id_auth = "", var/priority = 1)
...
obj
	machinery
		blackbox_recorder
			...
			var
				list
					messages = list()		//Stores messages of non-standard frequencies
			var
				list
					messages_admin = list()
				...
				if(T)
					...
					var
						obj
							machinery
								blackbox_recorder
									BR = new/obj/machinery/blackbox_recorder(T)
...
obj
	machinery
		blackbox_recorder
			proc
				find_feedback_datum(var/variable)
		...
		blackbox_recorder
			proc
				save_all_data_to_sql()
					...
					var
						DBQuery
							query = dbcon.NewQuery("SELECT MAX(round_id) AS round_id FROM [format_table_name("feedback")]")
					query.Execute()
					while(query.NextRow())
					...
					for(var/datum/feedback_variable/FV in feedback)
						var
							sql = "INSERT INTO [format_table_name("feedback")] VALUES (null, Now(), [round_id], \"[FV.get_variable()]\", [FV.get_value()], \"[FV.get_details()]\")"
						var
							DBQuery
								query_insert = dbcon.NewQuery(sql)
