		...
		computer
			rdconsole
				...
				var
					datum
						research
							files							//Stores all the collected research data.
				var
					obj
						item
							weapon
								disk
									tech_disk
										t_disk = null	//Stores the technology disk.
				var
					obj
						item
							weapon
								disk
									design_disk
										d_disk = null	//Stores the design disk.
						...
						machinery
							r_n_d
								destructive_analyzer
									linked_destroy = null	//Linked Destructive Analyzer
				var
					obj
						machinery
							r_n_d
								protolathe
									linked_lathe = null				//Linked Protolathe
				var
					obj
						machinery
							r_n_d
								circuit_imprinter
									linked_imprinter = null	//Linked Circuit Imprinter
				...
				var
					list
						datum
							design
								matching_designs = list() //for the search function

proc
	CallTechName(ID) //A simple helper proc to find the name of a tech with a given ID.
		var
			datum
				tech
					check_tech
					...
					if(href_list["category"])
						...
						if(linked_destroy)
							if(linked_destroy.busy)
								...
								var
									choice = input("Proceeding will destroy loaded item.") in list("Proceed", "Cancel")
						...
						if(linked_lathe)
							...
							if(being_built)
								...
								if(linked_lathe.busy)
									g2g = 0
								var
									key = usr.key	//so we don't lose the info during the spawn delay
								...
								if(g2g) //If input is incorrect, nothing happens
									...
									var
										P = being_built.build_path //lets save these values before the spawn() just in case. Nobody likes runtimes.
									var
										O = being_built.locked
							...
							var
								obj
									item
										stack
											sheet
												sheet = new type(linked_imprinter.loc)
							var
								available_num_sheets = round(linked_imprinter.vars[res_amount]/sheet.perunit)
						...
						griefProtection()
						var
							choice = alert("Are you sure you want to reset the R&D console's database? Data lost cannot be recovered.", "R&D Console Database Reset", "Continue", "Cancel")
						if(choice == "Continue")
							wait_message = "Updating Database...."
							qdel(files)
				...
				ui_interact(mob/user, ui_key="main", var/datum/nanoui/ui = null, var/force_open = 1)
					...
					if(menu == 2)
						...
						if(d_disk != null && d_disk.blueprint != null && submenu == 0)
							...
							var
								b_type = d_disk.blueprint.build_type
							var
								list
									lathe_types = list()
...
obj
	machinery
		computer
			rdconsole
				proc
					list_categories(var/list/categories, var/menu_num as num)
						if(!categories)
							return
