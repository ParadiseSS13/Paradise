	...
	machinery
		r_n_d
			experimentor
				...
				var
					list
						valid_items = list() //valid items for special reactions like transforming
				var
					list
						critical_items = list() //items that can cause critical reactions
...
obj
	machinery
		r_n_d
			experimentor
				proc
					ConvertReqString2List(list/source_list)
						var
							list
			...
			experimentor
				verb
					forceReaction()
						...
						set src in oview(1)
						var
							reaction = input(usr,"What reaction?") in list(SCANTYPE_POKE,SCANTYPE_IRRADIATE,SCANTYPE_GAS,SCANTYPE_HEAT,SCANTYPE_COLD,SCANTYPE_OBLITERATE)
						var
							oldReaction = item_reactions["[loaded_item.type]"]
...
obj
	machinery
		r_n_d
			experimentor
				default_deconstruction_crowbar(obj/item/O)
					ejectItem()
					..(O)
			...
			experimentor
				attack_hand(mob/user)
					...
					if(loaded_item)
						...
						var
							list
								D = ConvertReqString2List(loaded_item.origin_tech)
						for(var/T in D)
							dat += "[T]<br>"
					...
					var
						datum
							browser
								popup = new(user, "experimentor","Experimentor", 700, 400, src)
...
obj
	machinery
		r_n_d
			experimentor
				proc
					matchReaction(matching,reaction)
						var
							obj
								item
									D = matching
				...
				proc
					ejectItem(delete=FALSE)
						if(loaded_item)
							...
							var
								turf
									dropturf = get_turf(pick(view(1,src)))
...
obj
	machinery
		r_n_d
			experimentor
				proc
					experiment(exp,obj/item/exp_on)
						recentlyExperimented = 1
						icon_state = "h_lathe_wloop"
						var
							chosenchem
						var
							criticalReaction = (exp_on.type in critical_items) ? TRUE : FALSE
						////////////////////////////////////////////////////////////////////////////////////////////////
						if(exp == SCANTYPE_POKE)
							...
							if(prob(EFFECT_PROB_MEDIUM-badThingCoeff))
								...
								var
									mob
										living
											target = locate(/mob/living) in oview(7,src)
								if(target)
									var
										obj
											item
												throwing = loaded_item
						...
						if(exp == SCANTYPE_IRRADIATE)
											...
											var
												obj
													effect
														decal
															cleanable
																reagentdecal = new/obj/effect/decal/cleanable/greenglow(T)
							...
							if(prob(EFFECT_PROB_MEDIUM-badThingCoeff))
								...
								ejectItem(TRUE)
								var
									newPath = pickWeighted(valid_items)
						...
						if(exp == SCANTYPE_HEAT)
											...
											weapon
												reagent_containers
													food
														drinks
															coffee
																C = new /obj/item/weapon/reagent_containers/food/drinks/coffee(get_turf(pick(oview(1,src))))
							...
							if(prob(EFFECT_PROB_VERYLOW-badThingCoeff))
								...
								var
									mob
										M = locate(/mob/living) in view(src, 3)
								var
									turf
										MT = get_turf(M)
								if(MT)
										...
										obj
											item
												projectile
													magic
														fireball
															FB = new /obj/item/projectile/magic/fireball(start)
									FB.original = MT
									FB.current = start
									FB.yo = MT.y - start.y
									FB.xo = MT.x - start.x
						...
						if(exp == FAIL)
							var
								a = pick("rumbles","shakes","vibrates","shudders")
							var
								b = pick("crushes","spins","viscerates","smashes","insults")
						...
						if(prob(EFFECT_PROB_VERYLOW) && prob(13))
							...
							var
								list
									reqs = ConvertReqString2List(exp_on.origin_tech)
							for(var/T in reqs)
								reqs[T] = reqs[T] + 1
			...
			experimentor
				Topic(href, href_list)
					...
					var
						obj
							item
								process = locate(href_list["item"]) in src
					...
					if(href_list["close"])
						...
						var
							obj
								machinery
									computer
										rdconsole
											D = locate(/obj/machinery/computer/rdconsole) in oview(3,src)
						if(D)
							linked_console = D
						...
						if(dotype != FAIL)
							if(process && process.origin_tech)
								var
									list
										temp_tech = ConvertReqString2List(process.origin_tech)
...
obj
	machinery
		r_n_d
			experimentor
				proc
					warn_admins(mob/user, ReactionName)
						var
							turf
								T = get_turf(src)
...
obj
	item
		weapon
			relic
				proc
					floofcannon(mob/user)
						playsound(src.loc, "sparks", rand(25,50), 1)
						var
							mob
								living
									C = new floof(get_turf(user))
								...
								item
									weapon
										grenade
											chem_grenade
												cleaner
													CL = new/obj/item/weapon/grenade/chem_grenade/cleaner(get_turf(user))
						CL.prime()
							...
							obj
								item
									weapon
										grenade
											flashbang
												CB = new/obj/item/weapon/grenade/flashbang(get_turf(user))
						CB.prime()
		...
		weapon
			relic
				proc
					petSpray(mob/user)
						var
							message = "<span class='danger'>[src] begans to shake, and in the distance the sound of rampaging animals arises!</span>"
						...
						var
							list
								valid_animals = list(/mob/living/simple_animal/parrot,/mob/living/simple_animal/butterfly,/mob/living/simple_animal/pet/cat,/mob/living/simple_animal/pet/corgi,/mob/living/simple_animal/crab,/mob/living/simple_animal/pet/fox,/mob/living/simple_animal/lizard,/mob/living/simple_animal/mouse,/mob/living/simple_animal/pet/pug,/mob/living/simple_animal/hostile/bear,/mob/living/simple_animal/hostile/poison/bees,/mob/living/simple_animal/hostile/carp)
						for(counter = 1; counter < animals; counter++)
							var
								mobType = pick(valid_animals)
...
obj
	item
		weapon
			relic
				proc
					warn_admins(mob/user, RelicType, priority = 1)
						var
							turf
								T = get_turf(src)
						var
							log_msg = "[RelicType] relic used by [key_name(user)] in ([T.x],[T.y],[T.z])"
