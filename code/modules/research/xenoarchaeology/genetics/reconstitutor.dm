	...
	machinery
		computer
			reconstitutor
				...
				var
					list
						undiscovered_genesequences = null
				var
					list
						discovered_genesequences = list()
				var
					list
						completed_genesequences = list()
				var
					list
						undiscovered_genomes = list()
				var
					list
						manually_placed_genomes = list()
				var
					list
						discovered_genomes = list("! Clear !")
				var
					list
						accepted_fossil_types = list(/obj/item/weapon/fossil/plant)
						...
						for(var/obj/item/weapon/fossil/F in S.contents)
							switch(scan_fossil(F))
								...
								if(SCANFOSSIL_RETVAL_SUCCESS)
									...
									updateDialog()
						var
							outmsg = "\blue You empty all the fossils from [S] into [src]."
						if(numaccepted)
		...
		computer
			reconstitutor
				interact(mob/user)
					...
					var
						dat = "<B>Garland Corp genetic reconstitutor</B><BR>"
					dat += "<HR>"
					if(!pod1)
					...
					for(var/sequence_num = 1, sequence_num <= discovered_genesequences.len, sequence_num += 1)
						var
							datum
								genesequence
									cur_genesequence = discovered_genesequences[sequence_num]
						dat += "<tr>"
						var
							num_correct = 0
						for(var/curindex = 1, curindex <= 7, curindex++)
							var
								bgcolour = "#ffffff"//white ffffff, red ff0000
							...
							var
								this_genome_slot = manually_placed_genomes[sequence_num][curindex]
							if(!this_genome_slot)
								this_genome_slot = "- - - - -"
					...
					for(var/sequence_num = 1, sequence_num <= completed_genesequences.len, sequence_num += 1)
						var
							datum
								genesequence
									cur_genesequence = completed_genesequences[sequence_num]
						dat += "<tr>"
						for(var/curindex = 1, curindex <= 7, curindex++)
							var
								this_genome_slot = cur_genesequence.full_genome_sequence[curindex]
			...
			reconstitutor
				animal
					Topic(href, href_list)
						if(href_list["clone"])
							var
								sequence_num = text2num(href_list["sequence_num"])
							var
								datum
									genesequence
										cloned_genesequence = completed_genesequences[sequence_num]
							if(pod1)
								if(pod1.occupant)
...
//		to_chat(world, "inserting gene for genesequence [href_list["insertgenome"]] at pos [text2num(href_list["insertpos"])]")
		var
			sequence_num = text2num(href_list["sequence_num"])
		var
			insertpos = text2num(href_list["insertpos"])

		var
			old_genome = manually_placed_genomes[sequence_num][insertpos]
		discovered_genomes = sortList(discovered_genomes)
		var
			new_genome = input(usr, "Which genome do you want to insert here?") as null|anything in discovered_genomes
		if(new_genome == "! Clear !")
		...
		for(var/curindex = 1, curindex <= 7, curindex++)
			var
				old_genome = manually_placed_genomes[sequence_num][curindex]
		...
		var
			datum
				genesequence
					wiped_genesequence = completed_genesequences[sequence_num]
...
obj
	machinery
		computer
			reconstitutor
				proc
					scan_fossil(var/obj/item/weapon/fossil/scan_fossil)
						...
						var
							new_genome_prob = discovered_genesequences.len * 50
						...
						if( (new_genome_prob >= 100 || prob(new_genome_prob)) && undiscovered_genomes.len)
							...
							var
								datum
									genesequence
										newly_discovered_genesequence = pick(undiscovered_genesequences)
