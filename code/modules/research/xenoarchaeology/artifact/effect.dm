...
datum
	artifact_effect
		var
			effecttype = "unknown"		//purely used for admin checks ingame, not needed any more
		var
			effect = EFFECT_TOUCH
		var
			effectrange = 4
...
datum
	artifact_effect
		proc
			ToggleActivate(var/reveal_toggle = 1)
...
datum
	artifact_effect
		proc
			DoEffectAura(var/atom/holder)
datum
	artifact_effect
		proc
			DoEffectPulse(var/atom/holder)
datum
	artifact_effect
		proc
			UpdateMove()
