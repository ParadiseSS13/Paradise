		...
		replicator
			New()
				...
				for(var/i=0, i<quantity, i++)
					var
						button_desc = "a [pick("yellow","purple","green","blue","red","orange","white")], "
			...
			process()
				if(spawning_types.len && powered())
					...
					if(spawn_progress_time > max_spawn_time)
						...
						var
							obj
								source_material = pop(stored_materials)
						var
							spawn_type = pop(spawning_types)
						var
							obj
								spawned_obj = new spawn_type(src.loc)
						if(source_material)
...
obj
	machinery
		replicator
			attackby(obj/item/weapon/W as obj, mob/living/user as mob, params)
				user.drop_item()
				W.loc = src
		...
		replicator
			Topic(href, href_list)
				...
				if(href_list["activate"])
					var
						index = text2num(href_list["activate"])
