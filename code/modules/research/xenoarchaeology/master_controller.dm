...
datum
	controller
		game_controller
			var
				list
					all_animal_genesequences = list()
			var
				list
					all_plant_genesequences = list()
			var
				list
					genome_prefixes = null
			var
				list
					artifact_spawning_turfs = list()
			var
				list
					digsite_spawning_turfs = list()

			var
				list
					spawn_types_animal = list("/mob/living/carbon/slime",\
			...
			var
				list
					spawn_types_plant = list("/obj/item/seeds/walkingmushroommycelium",\
			...
			proc
				SetupXenoarch()
					...
					for(var/turf/simulated/mineral/M in block(locate(1,1,1), locate(world.maxx, world.maxy, world.maxz)))
						...
						var
							digsite = get_random_digsite_type()
						var
							target_digsite_size = rand(DIGSITESIZE_LOWER, DIGSITESIZE_UPPER)
						var
							list
								processed_turfs = list()
						var
							list
								turfs_to_process = list(M)
						while(turfs_to_process.len)
							var
								turf
									simulated
										mineral
											archeo_turf = pop(turfs_to_process)

							if(target_digsite_size > 1)
								var
									list
										viable_adjacent_turfs = orange(1, archeo_turf)
					...
					var
						num_artifacts_spawn = rand(ARTIFACTSPAWNNUM_LOWER, ARTIFACTSPAWNNUM_UPPER)
					while(artifact_spawning_turfs.len > num_artifacts_spawn)
						pick_n_take(artifact_spawning_turfs)

					var
						list
							artifacts_spawnturf_temp = artifact_spawning_turfs.Copy()
					while(artifacts_spawnturf_temp.len > 0)
						var
							turf
								simulated
									mineral
										artifact_turf = pop(artifacts_spawnturf_temp)
					...
					while(spawn_types_animal.len && genome_prefixes.len)
						var
							datum
								genesequence
									new_sequence = new/datum/genesequence()
						...
						var
							prefixletter = pick(genome_prefixes)
