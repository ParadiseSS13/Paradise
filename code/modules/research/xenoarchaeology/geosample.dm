	...
	item
		weapon
			rocksliver
				...
				var
					source_rock = "/turf/simulated/mineral/"
				var
					datum
						geosample
							geological_data
...
datum
	geosample
		var
			age = 0								//age can correspond to different archaeological finds
		var
			age_thousand = 0
		var
			age_million = 0
		var
			age_billion = 0
		var
			artifact_id = ""					//id of a nearby artifact, if there is one
		var
			artifact_distance = -1				//proportional to distance
		var
			source_mineral = "chlorine"			//machines will pop up a warning telling players that the sample may be confused
		//
		//var/source_mineral
...
datum
	geosample
		New(var/turf/simulated/mineral/container)
...
datum
	geosample
		proc
			UpdateTurf(var/turf/simulated/mineral/container)
				set background = 1
				...
				for(var/datum/find/F in container.finds)
					var
						responsive_reagent = get_responsive_reagent(F.find_type)
...
datum
	geosample
		proc
			UpdateNearbyArtifactInfo(var/turf/simulated/mineral/container)
				if(!container || !istype(container))
					return
				...
				if(container.artifact_find)
					...
					for(var/turf/simulated/mineral/T in master_controller.artifact_spawning_turfs)
						if(T.artifact_find)
							var
								cur_dist = get_dist(container, T) * 2
