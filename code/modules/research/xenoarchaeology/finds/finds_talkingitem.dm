...
datum
	talking_atom
		proc
			catchMessage(var/msg, var/mob/source)
				if(!holder_atom)
					return
				...
				for(var/Xa = 1,Xa<seperate.len,Xa++)
					...
					var
						list
							w = heard_words["[lowertext(seperate[Xa])]"]
					if(w)
...
//		to_chat(world, "Adding [lowertext(seperate[next])] to [lowertext(seperate[Xa])]")
	...
	if(prob(30))
		var
			list
				options = list("[holder_atom] seems to be listening intently to [source]...",\
		...
		proc
			SaySomething(var/word = null)
				...
				var
					msg
				var
					limit = rand(max(5,heard_words.len/2))+3
				var
					text
				if(!word)
