...
datum
	find
		var
			find_type = 0				//random according to the digsite type
		var
			excavation_required = 0		//random 5-95%
		var
			view_range = 20				//how close excavation has to come to show an overlay on the turf
		var
			clearance_range = 3			//how close excavation has to come to extract the item
											//if excavation hits var/excavation_required exactly, it's contained find is extracted cleanly without the ore
		var
			prob_delicate = 90			//probability it requires an active suspension field to not insta-crumble
		var
			dissonance_spread = 1		//proportion of the tile that is affected by this find
											//used in conjunction with analysis machines to determine correct suspension field type

datum
	find
		New(var/digsite, var/exc_req)
						...
						obj
							item
								weapon
									inside
					var
						method = 0// 0 = fire, 1 = brush, 2 = pick
...
obj
	item
		weapon
			ore
				strangerock
					New(loc, var/inside_item_type = 0)
						..(loc)
				...
				New(loc, var/new_item_type)
					...
					switch(find_type)
						...
						if(20)
							...
							var
								list
									possible_spawns = list(/obj/item/clothing/head/culthood,
							obj
								item
									clothing
										head
						...
						if(24)
							var
								list
									possible_spawns = subtypesof(/obj/item/weapon/stock_parts)
									...
									obj
										item
											weapon
												gun
													energy
														new_gun = new spawn_type(src.loc)
								new_item = new_gun
...
/*		if(27)
					...
					item
						weapon
							gun
								projectile
									revolver
										new_gun = new /obj/item/weapon/gun/projectile/revolver(src.loc)
			new_item = new_gun
			...
			if(prob(33))
				var
					num_bullets = rand(1,new_gun.max_shells)
				if(num_bullets < new_gun.loaded.len)
					new_gun.loaded.Cut()
					for(var/i = 1, i <= num_bullets, i++)
						var
							A = text2path(new_gun.ammo_type)
...
if(29)
			...
			var
				list
					candidates = list("/obj/item/weapon/fossil/bone"=9,"/obj/item/weapon/fossil/skull"=3,
			"/obj/item/weapon/fossil/skull/horned"=2)
			var
				spawn_type = pickweight(candidates)
