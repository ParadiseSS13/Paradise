				...
				process()
					...
					if(charges >= 3)
						if(prob(5))
							charges -= 1
							var
								spawn_type = pick(/mob/living/simple_animal/hostile/creature)
					...
					if(shadow_wights.len)
						...
						var
							obj
								effect
									shadow_wight
										W = shadow_wights[wight_check_index]
						if(isnull(W))
...
obj
	item
		weapon
			vampiric
				proc
					bloodcall(var/mob/living/carbon/human/M)
						...
						if(istype(M))
							...
							var
								target = pick("chest","groin","head","l_arm","r_arm","r_leg","l_leg","l_hand","r_hand","l_foot","r_foot")
