...
obj
	machinery
		radiocarbon_spectrometer
			...
			var
				obj
					item
						scanned_item
			var
				last_scan_data = "No scans on record."
			//
			var
				last_process_worldtime = 0
			//
			var
				scanner_progress = 0
			var
				scanner_rate = 1.25			//80 seconds per scan
			var
				scanner_rpm = 0
			var
				scanner_rpm_dir = 1
			var
				scanner_temperature = 0
			var
				scanner_seal_integrity = 100
			//
			var
				coolant_usage_rate = 0		//measured in u/microsec
			var
				fresh_coolant = 0
			var
				coolant_purity = 0
			...
			var
				optimal_wavelength_target = 0
			var
				tleft_retarget_optimal_wavelength = 0
			var
				maser_efficiency = 0
			//
			var
				radiation = 0				//0-100 mSv
			var
				t_left_radspike = 0
			var
				rad_shield = 0
			...
			attackby(var/obj/I as obj, var/mob/user as mob, params)
				if(scanning)
					...
					if(istype(I, /obj/item/stack/nanopaste))
						var
							choice = alert("What do you want to do with the nanopaste?","Radiometric Scanner","Scan nanopaste","Fix seal integrity")
									...
									item
										stack
											nanopaste
												N = I
							var
								amount_used = min(N.amount, 10 - scanner_seal_integrity / 10)
							N.use(amount_used)
							scanner_seal_integrity = round(scanner_seal_integrity + amount_used * 10)
							return
					if(istype(I, /obj/item/weapon/reagent_containers/glass))
						var
							choice = alert("What do you want to do with the container?","Radiometric Scanner","Add coolant","Empty coolant","Scan container")
										...
										weapon
											reagent_containers
												glass
													G = I
							var
								amount_transferred = min(src.reagents.maximum_volume - src.reagents.total_volume, G.reagents.total_volume)
										...
										weapon
											reagent_containers
												glass
													G = I
							var
								amount_transferred = min(G.reagents.maximum_volume - G.reagents.total_volume, src.reagents.total_volume)
				...
				update_coolant()
					...
					for(var/datum/reagent/current_reagent in src.reagents.reagent_list)
						if(!current_reagent)
							continue
						var
							cur_purity = coolant_reagents_purity[current_reagent.id]
						if(!cur_purity)
							cur_purity = 0.1
			...
			process()
				if(scanning)
					if(!scanned_item || scanned_item.loc != src)
						...
						var
							deltaT = (world.time - last_process_worldtime) * 0.1
						...
						if(coolant_usage_rate > 0)
							var
								coolant_used = min(fresh_coolant, coolant_usage_rate * deltaT)
							if(coolant_used > 0)
						...
						if(G)
							...
							for(var/carrier in G.find_presence)
								if(G.find_presence[carrier])
									var
										index = responsive_carriers.Find(carrier)
