			...
			Topic(href, href_list)
				...
				if(href_list["harvest"])
					if(!inserted_battery)
						...
						if(articount <= 0)
							var
								message = "<b>[src]</b> states, \"Cannot harvest. No noteworthy energy signature isolated.\""
							...
							if(articount > 1)
								...
								if(cur_artifact.my_effect && cur_artifact.my_effect.activated && cur_artifact.secondary_effect.activated)
									...
									if(inserted_battery.stored_charge > 0)
										...
										var
											battery_matches_secondary_id = 0
									...
									if(source_effect)
										...
										var
											message = "<b>[src]</b> states, \"Beginning energy harvesting.\""
										src.visible_message(message)
										last_process = world.time
										...
										if(!inserted_battery.battery_effect)
											var
												effecttype = source_effect.type
											var
												datum
													artifact_effect
														E = new effecttype(inserted_battery)
					...
					if(inserted_battery)
						if(inserted_battery.battery_effect && inserted_battery.stored_charge > 0)
							if(alert("This action will dump all charge, safety gear is recommended before proceeding","Warning","Continue","Cancel"))
								...
								icon_state = "incubator_on"
								var
									message = "<b>[src]</b> states, \"Warning, battery charge dump commencing.\""
								src.visible_message(message)
						else
							var
								message = "<b>[src]</b> states, \"Cannot dump energy. Battery is drained of charge already.\""
							src.visible_message(message)
					else
						var
							message = "<b>[src]</b> states, \"Cannot dump energy. No battery inserted.\""
						src.visible_message(message)
