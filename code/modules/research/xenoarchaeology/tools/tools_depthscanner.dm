...
obj
	item
		device
			depth_scanner
				proc
					scan_atom(var/mob/user, var/atom/A)
			...
			depth_scanner
				interact(var/mob/user as mob)
					...
					if(current)
						...
						var
							index = responsive_carriers.Find(current.material)
