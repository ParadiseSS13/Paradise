...
obj
	item
		weapon
			anodevice
				attackby(var/obj/I as obj, var/mob/user as mob, params)
		...
		weapon
			anodevice
				interact(var/mob/user)
					var
						dat = "<b>Anomalous Materials Energy Utiliser</b><br>"
					if(inserted_battery)
						if(activated)
			...
			anodevice
				Topic(href, href_list)
					...
					if(href_list["changetime"])
						var
							timedif = text2num(href_list["changetime"])
				...
				proc
					UpdateSprite()
						if(!inserted_battery)
							...
							return
						var
							p = (inserted_battery.stored_charge/inserted_battery.capacity)*100
						p = min(p, 100)
