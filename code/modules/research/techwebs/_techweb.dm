...
/datum/techweb
	var/list/researched_nodes = list()		//Already unlocked and all designs are now available. Assoc list, id = datum
	var/list/visible_nodes = list()			//Visible nodes, doesn't mean it can be researched. Assoc list, id = datum
	var/list/available_nodes = list()		//Nodes that can immediately be researched, all reqs met. assoc list, id = datum
	var/list/researched_designs = list()			//Designs that are available for use. Assoc list, id = datum
	var/list/boosted_nodes = list()			//Already boosted nodes that can't be boosted again. node datum = path of boost object.
	var/list/hidden_nodes = list()			//Hidden nodes. id = datum. Used for unhiding nodes when requirements are met by removing the entry of the node.
	...
	var/spent_points = 0										//Points put into the system
...
/datum/techweb/proc/add_design_by_id(id)
	return add_design(SSresearch.get_techweb_design_by_id(id))
...
/datum/techweb/proc/remove_design_by_id(id)
	return remove_design(SSresearch.get_techweb_design_by_id(id))
...
/datum/techweb/proc/research_node_id(id, force, auto_update_points)
	return research_node(SSresearch.get_techweb_node_by_id(id), force, auto_update_points)
...
/datum/techweb/proc/unresearch_node_id(id)
	return unresearch_node(SSresearch.get_techweb_node_by_id(id))
...
/datum/techweb/specialized/autounlocking/exofab
	node_autounlock_ids = list("robotics", "mmi", "cyborg", "mecha_odysseus", "mech_gygax", "mech_durand", "mecha_phazon", "mecha", "mech_tools", "clown", "spacepod_tech")
	allowed_buildtypes = MECHFAB | PODFAB // I hate this too, dont worry
