...
obj
	item
		slime_extract
			...
			var
				Uses = 1 // uses before it goes inert
...
obj
	item
		slimepotion
			afterattack(obj/item/weapon/reagent_containers/target, mob/user , proximity)
				if(istype(target))
			...
			docility
				attack(mob/living/carbon/slime/M, mob/user)
					...
					if(M.is_adult)
						var
							mob
								living
									simple_animal
										adultslime
											pet = new /mob/living/simple_animal/adultslime(M.loc)
						...
						qdel(M)
						var
							newname = sanitize(copytext(input(user, "Would you like to give the slime a name?", "Name your new pet", "pet slime") as null|text,1,MAX_NAME_LEN))
...
obj
	item
		slimepotion
			sentience
				afterattack(mob/living/M, mob/user)
					...
					var
						ghostmsg = "Play as [SM.name], pet of [user.name]?"
					var
						list
							candidates = pollCandidates(ghostmsg, ROLE_SENTIENT, 0, 100)
...
obj
	item
		slimepotion
			speed
				afterattack(obj/item/C, mob/user)
					..()
					if(!istype(C))
...
obj
	item
		slimepotion
			fireproof
				afterattack(obj/item/clothing/C, mob/user)
					..()
					if(!uses)
						qdel(src)
						return
					...
					mob
						living
							carbon
								human
									golem
										G = new /mob/living/carbon/human/golem
...
obj
	effect
		golemrune
			proc
				check_observer(var/mob/dead/observer/O)
					if(!O)
						return 0
					if(!O.client)
						return 0
...
obj
	effect
		golemrune
			proc
				volunteer(var/mob/dead/observer/O)
					if(O in ghosts)
						ghosts.Remove(O)
	...
	effect
		timestop
			...
			var
				mob
					living
						immune = list() // the one who creates the timestop is immune
			var
				list
					stopped_atoms = list()
			var
