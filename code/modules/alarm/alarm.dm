...
datum
	alarm_source
		var
			source		= null	// The source trigger
		var
			source_name = ""	// The name of the source should it be lost (for example a destroyed camera)
		var
			duration	= 0		// How long this source will be alarming, 0 for indefinetely.
		var
			severity 	= 1		// How severe the alarm from this source is.
		var
			start_time	= 0		// When this source began alarming.
		var
			end_time	= 0		// Use to set when this trigger should clear, in case the source is lost.
...
datum
	alarm
		var
			atom
				origin					//Used to identify the alarm area.
		var
			list
				sources = new()		//List of sources triggering the alarm. Used to determine when the alarm should be cleared.
		var
			list
				sources_assoc = new()	//Associative list of source triggers. Used to efficiently acquire the alarm source.
		var
			list
				cameras				//List of cameras that can be switched to, if the player has that capability.
		var
			area
				last_area				//The last acquired area, used should origin be lost (for example a destroyed borg containing an alarming camera).
		var
			area
				last_name				//The last acquired name, used should origin be lost
		var
			area
				last_camera_area		//The last area in which cameras where fetched, used to see if the camera list should be updated.
		var
			end_time					//Used to set when this alarm should clear, in case the origin is lost.

datum
	alarm
		New(var/atom/origin, var/atom/source, var/duration, var/severity)
			src.origin = origin
...
datum
	alarm
		proc
			set_source_data(var/atom/source, var/duration, var/severity)
				var
					datum
						alarm_source
							AS = sources_assoc[source]
				if(!AS)
