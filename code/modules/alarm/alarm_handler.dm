	...
	alarm_handler
		...
		var
			list
				datum
					alarm
						alarms = new		// All alarms, to handle cases when an origin has been deleted with one or more active alarms
		var
			list
				datum
					alarm
						alarms_assoc = new	// Associative list of alarms, to efficiently acquire them based on origin.
		var
			list
				listeners = new				// A list of all objects interested in alarm changes.
...
datum
	alarm_handler
		proc
			triggerAlarm(var/atom/origin, var/atom/source, var/duration = 0, var/severity = 1)
				var
					new_alarm
...
datum
	alarm_handler
		proc
			clearAlarm(var/atom/origin, var/source)
				//Proper origin and source mandatory
				if(!(origin && source))
					return
				origin = origin.get_alarm_origin()
...
datum
	alarm_handler
		proc
			check_alarm_cleared(var/datum/alarm/alarm)
...
datum
	alarm_handler
		proc
			on_alarm_change(var/datum/alarm/alarm, var/was_raised)
...
datum
	alarm_handler
		proc
			get_alarm_severity_for_origin(var/atom/origin)
				if(!origin)
					return
...
datum
	alarm_handler
		proc
			register(var/object, var/procName)
				listeners[object] = procName

datum
	alarm_handler
		proc
			unregister(var/object)
				listeners -= object

datum
	alarm_handler
		proc
			notify_listeners(var/alarm, var/was_raised)
