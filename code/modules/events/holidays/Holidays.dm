...
proc
	Get_Holiday()
		...
		var
			YY	=	text2num(time2text(world.timeofday, "YY")) 	// get the current year
		var
			MM	=	text2num(time2text(world.timeofday, "MM")) 	// get the current month
		var
			DD	=	text2num(time2text(world.timeofday, "DD")) 	// get the current day
...
/*			var/list/turf/simulated/floor/Floorlist = list()
			...
			var
				turf
					simulated
						floor
							F = Floorlist[rand(1,Floorlist.len)]
			Floorlist = null
			var
				obj
					structure
						closet
							C = locate(/obj/structure/closet) in F
			var
				obj
					item
						weapon
