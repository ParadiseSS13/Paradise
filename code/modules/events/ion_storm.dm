	...
	event
		ion_storm
			var
				botEmagChance = 10
			var
				announceEvent = ION_RANDOM // -1 means don't announce, 0 means have it randomly announce, 1 means
			var
				ionMessage = null
			var
...
datum
	event
		ion_storm
			New(var/botEmagChance = 10, var/announceEvent = ION_RANDOM, var/ionMessage = null, var/ionAnnounceChance = 33)
			...
			start()
				...
				for(var/mob/living/silicon/ai/M in living_mob_list)
					if(M.stat != 2 && M.see_in_dark != 0)
						var
							message = generate_ion_law(ionMessage)
						if(message)
...
proc
	generate_ion_law(ionMessage)
		...
		var
			ionthreats = pick_list("ion_laws.json", "ionthreats")
		//Objects are anything that can be found on the station or elsewhere, plural.
		var
			ionobjects = pick_list("ion_laws.json", "ionobjects")
		...
		var
			ioncrew1 = pick_list("ion_laws.json", "ioncrew")
		var
			ioncrew2 = pick_list("ion_laws.json", "ioncrew")
		...
		var
			ionadjectives = pick_list("ion_laws.json", "ionadjectives")
		var
			ionadjectiveshalf = pick("", 400;(pick_list("ion_laws.json", "ionadjectives") + " "))
		//Verbs are verbs
		var
			ionverb = pick_list("ion_laws.json", "ionverb")
		...
		var
			ionnumberbase = pick_list("ion_laws.json", "ionnumberbase")
		//var/ionnumbermod = pick_list("ion_laws.json", "ionnumbermod")
		var
			ionnumbermodhalf = pick(900;"",(pick_list("ion_laws.json", "ionnumbermod") + " "))
		//Areas are specific places, on the station or otherwise.
		var
			ionarea = pick_list("ion_laws.json", "ionarea")
		//Thinksof is a bit weird, but generally means what X feels towards Y.
		var
			ionthinksof = pick_list("ion_laws.json", "ionthinksof")
		//Musts are funny things the AI or crew has to do.
		var
			ionmust = pick_list("ion_laws.json", "ionmust")
		//Require are basically all dumb internet memes.
		var
			ionrequire = pick_list("ion_laws.json", "ionrequire")
		...
		var
			ionthings = pick_list("ion_laws.json", "ionthings")
		//Allergies should be broad and appear somewhere on the station for maximum fun. Severity
		//is how bad the allergy is.
		var
			ionallergy = pick_list("ion_laws.json", "ionallergy")
		var
			ionallergysev = pick_list("ion_laws.json", "ionallergysev")
		//Species, for when the AI has to commit genocide. Plural.
		var
			ionspecies = pick_list("ion_laws.json", "ionspecies")
		//Abstract concepts for the AI to decide on it's own definition of.
		var
			ionabstract = pick_list("ion_laws.json", "ionabstract")
		...
		var
			ionfood = pick_list("ion_laws.json", "ionfood")
...
proc
	generate_static_ion_law()
		...
		var
			random_player = "The Captain"
		if(players.len)
			random_player = pick(players)		//Random player's name, to be used in laws.
		var
			list
				laws = list(	"You are a mouse.",
										"You must always lie.",
