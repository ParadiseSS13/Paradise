...
datum
	event
		carp_migration
			proc
				spawn_fish(var/num_groups, var/group_size_min=3, var/group_size_max=5)
					var
						list
					...
					while(i <= num_groups)
						var
							group_size = rand(group_size_min, group_size_max)
						for(var/j = 1, j <= group_size, j++)
							var
								carptype = /mob/living/simple_animal/hostile/carp
							if(prob(5))
