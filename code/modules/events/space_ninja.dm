...
/datum/event/space_ninja/proc/poll_ninja()
	var/list/candidates = SSghost_spawns.poll_candidates("Do you want to play as a space ninja?", ROLE_NINJA, TRUE)
	...
	addtimer(CALLBACK(src, PROC_REF(get_species), new_ninja, gender_pref), 10 SECONDS)

/datum/event/space_ninja/proc/get_species(mob/new_ninja, datum/async_input/gender_pref)
	gender_pref.close()
	...
	addtimer(CALLBACK(src, PROC_REF(create_ninja), new_ninja, sel_gender, species_pref), 10 SECONDS)

/datum/event/space_ninja/proc/create_ninja(mob/new_ninja, sel_gender, datum/async_input/species_pref)
	species_pref.close()
