...
datum
	event
		money_hacker
			announce()
				var
					message = "A brute force hack has been detected (in progress since [worldtime2text()]). The target of the attack is: Financial account #[affected_account.account_number], \
	...
	event
		money_hacker
				...
				if(affected_account && !affected_account)
					...
					//subtract the money
					var
						lost = affected_account.money * 0.8 + (rand(2,4) - 2) / 10
					...
					var
						date2 = "[num2text(rand(1,31))] [pick("January","February","March","April","May","June","July","August","September","October","November","December")], [rand(1000,3000)]"
					T.date = pick("", current_date_string, date1, date2)
					var
						time1 = rand(0, 99999999)
					var
						time2 = "[round(time1 / 36000)+12]:[(time1 / 600 % 60) < 10 ? add_zero(time1 / 600 % 60, 1) : time1 / 600 % 60]"
