datum
	event_meta
		var
			name 		= ""
		var
			enabled 	= 1	// Whether or not the event is available for random selection at all
		var
			weight 		= 0 // The base weight of this event. A zero means it may never fire, but see get_weight()
		var
			min_weight	= 0 // The minimum weight that this event will have. Only used if non-zero.
		var
			max_weight	= 0 // The maximum weight that this event will have. Only use if non-zero.
		var
			severity 	= 0 // The current severity of this event
		var
			one_shot	= 0	//If true, then the event will not be re-added to the list of available events
		var
			list
				role_weights = list()
		var
...
datum
	event_meta
		New(var/event_severity, var/event_name, var/datum/event/type, var/event_weight, var/list/job_weights, var/is_one_shot = 0, var/min_event_weight = 0, var/max_event_weight = 0)
			name = event_name
...
datum
	event_meta
		proc
			get_weight(var/list/active_with_role)
				if(!enabled)
					return 0
				...
				var
					total_weight = weight + job_weight
...
datum
	event	//NOTE: Times are measured in master controller ticks!
		var
			processing = 1
		var
			startWhen		= 0	//When in the lifetime to call start().
		var
			announceWhen	= 0	//When in the lifetime to call announce().
		var
			endWhen			= 0	//When in the lifetime the event should end.

		var
			severity		= 0 //Severity. Lower means less severe, higher means more severe. Does not have to be supported. Is set on New().
		var
			activeFor		= 0	//How long the event has existed. You don't need to change this.
		var
			isRunning		= 1 //If this event is currently running. You should not change this.
		var
			startedAt		= 0 //When this event started.
		var
			endedAt			= 0 //When this event ended.
		var
			noAutoEnd       = 0 //Does the event end automatically after endWhen passes?
		var
			area
