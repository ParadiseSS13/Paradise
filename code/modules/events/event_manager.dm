datum
	event_manager
		...
		var
			window_y = 600
		var
			report_at_round_end = 0
		var
			table_options = " align='center'"
		var
			head_options = " style='font-weight:bold;'"
		var
			row_options1 = " width='85px'"
		var
			row_options2 = " width='260px'"
		var
			row_options3 = " width='150px'"
		var
			datum
				event_container
				...
				for(var/i = EVENT_LEVEL_MUNDANE to EVENT_LEVEL_MAJOR)
					var
						list
							datum
								event_container
									EC = event_containers[i]
					EC.process()

datum
	event_manager
		proc
			event_complete(var/datum/event/E)
				...
				var
					datum
						event_container
							EC = event_containers[theseverity]
				var
					datum
						event_meta
							EM = E.event_meta
...
datum
	event_manager
		proc
			delay_events(var/severity, var/delay)
				var
					list
						datum
							event_container
								EC = event_containers[severity]
		...
		proc
			Interact(var/mob/living/user)
				...
				var
					datum
						browser
							popup = new(user, "event_manager", "Event Manager", window_x, window_y)
			...
			RoundEnd()
				...
				for(var/datum/event/E in active_events|finished_events)
					...
					if(EM.name == "Nothing")
						continue
					var
						message = "'[EM.name]' began at [worldtime2text(E.startedAt)] "
					if(E.isRunning)
...
datum
	event_manager
		proc
			GetInteractWindow()
				var
					html = "<A align='right' href='?src=\ref[src];refresh=1'>Refresh</A>"

				if(selected_event_container)
					var
						event_time = max(0, selected_event_container.next_event_time - world.time)
						...
						var
							datum
								event_container
									EC = event_containers[severity]
						var
							next_event_at = max(0, EC.next_event_time - world.time)
						html += "<tr>"
						...
						var
							datum
								event_meta
									EM = E.event_meta
						var
							ends_at = E.startedAt + (E.lastProcessAt() * 20)	// A best estimate, based on how often the manager processes
						var
							ends_in = max(0, round((ends_at - world.time) / 600, 0.1))
						var
							no_end = E.noAutoEnd
						html += "<tr>"
		...
		Topic(href, href_list)
			...
			if(href_list["toggle_report"])
				...
				var
					datum
						event_container
							EC = locate(href_list["event"])
				var
					decrease = (60 * RaiseToPower(10, text2num(href_list["dec_timer"])))
				...
				var
					datum
						event_container
							EC = locate(href_list["event"])
				var
					increase = (60 * RaiseToPower(10, text2num(href_list["inc_timer"])))
				...
				var
					datum
						event_container
							EC = locate(href_list["select_event"])
				var
					datum
						event_meta
							EM = EC.SelectEvent()
				...
				var
					datum
						event_container
							EC = locate(href_list["pause"])
				EC.delayed = !EC.delayed
				admin_log_and_message_admins("has [EC.delayed ? "paused" : "resumed"] countdown for [severity_to_string[EC.severity]] events.")
			else if(href_list["interval"])
				var
					delay = input("Enter delay modifier. A value less than one means events fire more often, higher than one less often.", "Set Interval Modifier") as num|null
				if(delay && delay > 0)
					var
						datum
							event_container
								EC = locate(href_list["interval"])
				...
				var
					name = input("Enter event name.", "Set Name") as text|null
				if(name)
					var
						datum
							event_meta
								EM = locate(href_list["set_name"])
					EM.name = name
			else if(href_list["set_type"])
				var
					type = input("Select event type.", "Select") as null|anything in allEvents
				if(type)
					var
						datum
							event_meta
								EM = locate(href_list["set_type"])
					EM.event_type = type
			else if(href_list["set_weight"])
				var
					weight = input("Enter weight. A higher value means higher chance for the event of being selected.", "Set Weight") as num|null
				if(weight && weight > 0)
					var
						datum
							event_meta
								EM = locate(href_list["set_weight"])
					EM.weight = weight
					if(EM != new_event)
				...
				var
					datum
						event_meta
							EM = locate(href_list["toggle_oneshot"])
				...
				var
					datum
						event_meta
							EM = locate(href_list["toggle_enabled"])
				...
				var
					datum
						event_meta
							EM = locate(href_list["remove"])
				var
					datum
						event_container
							EC = locate(href_list["EC"])
				EC.available_events -= EM
...
client
	proc
		forceEvent(var/type in event_manager.allEvents)
...
proc
	findEventArea() //Here's a nice proc to use to find an area for your event to land in!
		var
			area
				candidate = null
...
proc
	number_active_with_role()
		var
			list
				active_with_role = list()
