var
	global
		list
			unused_trade_stations = list("sol")
		...
		traders
			start()
				...
				spawn()
					var
						list
							candidates = pollCandidates("Do you want to play as a trader?", ROLE_TRADER, 1)
					var
						index = 1
...
datum
	event
		traders
			proc
				create_trader(var/turf/picked_loc)
					var
						mob
							living
								carbon
...
datum
	event
		traders
			proc
				greet_trader(var/mob/living/carbon/human/M)
...
datum
	event
		traders
			proc
				forge_trader_objectives()
					var
						i = 1
					var
						max_objectives = pick(2, 2, 2, 2, 3, 3, 3, 4)
					var
						list
							objs = list()
					var
						list
							goals = list("stockparts")
