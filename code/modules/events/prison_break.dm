	...
	event
		prison_break
			...
			var
				list
					area
						areas = list()		//List of areas to affect. Filled by start()

			var
				eventDept = "Security"			//Department name in announcement
			var
				list
					areaName = list("Brig")	//Names of areas mentioned in AI and Engineering announcements
			var
				list
					areaType = list(/area/security/prison, /area/security/brig, /area/security/permabrig)	//Area types to include.
			var
				list
					areaNotType = list()		//Area types to specifically exclude.
	...
	event
		prison_break
			start()
				...
				if(areas && areas.len > 0)
					var
						my_department = "[station_name()] firewall subroutines"
					var
						rc_message = "An unknown malicious program has been detected in the [english_list(areaName)] lighting and airlock control systems at [worldtime2text()]. Systems will be fully compromised within approximately three minutes. Direct intervention is required immediately.<br>"
