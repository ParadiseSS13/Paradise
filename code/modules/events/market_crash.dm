...
/datum/event/market_crash/start()
	for(var/obj/machinery/economy/vending/vendor in SSmachines.get_by_type(/obj/machinery/economy/vending))
		...
		for(var/datum/data/vending_product/product in vendor.product_records + vendor.physical_product_records + vendor.hidden_records + vendor.physical_hidden_records)
			product.price *= price_mult
