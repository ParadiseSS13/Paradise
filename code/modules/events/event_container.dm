...
var
	global
		list
			severity_to_string = list(EVENT_LEVEL_MUNDANE = "Mundane", EVENT_LEVEL_MODERATE = "Moderate", EVENT_LEVEL_MAJOR = "Major")
var
	list
		event_last_fired = list()
		...
		proc
			acquire_event()
				if(available_events.len == 0)
					return
				var
					active_with_role = number_active_with_role()
				...
				for(var/datum/event_meta/EM in available_events)
					var
						event_weight = EM.get_weight(active_with_role)
				...
				for(var/event_meta in last_event_time) if(possible_events[event_meta])
					var
						time_passed = world.time - event_last_fired[event_meta]
					var
						weight_modifier = max(0, (config.expected_round_length - time_passed) / 300)
					var
						new_weight = max(possible_events[event_meta] - weight_modifier, 0)
				...
				var
					picked_event = pickweight(possible_events)
		...
		proc
			set_event_delay()
				...
				if(next_event_time == 0 && config.event_first_run[severity])
					var
						lower = config.event_first_run[severity]["lower"]
					var
						upper = config.event_first_run[severity]["upper"]
					var
						event_delay = rand(lower, upper)
		...
		proc
			SelectEvent()
				var
					datum
						event_meta
							EM = input("Select an event to queue up.", "Event Selection", null) as null|anything in available_events
				if(!EM)
					return
				if(next_event)
