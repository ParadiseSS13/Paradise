datum
	event
		grid_check	//NOTE: Times are measured in master controller ticks!
			announceWhen		= 5
...
proc
	power_failure(var/announce = 1)
		...
		var
			list
				skipped_areas = list(/area/turret_protected/ai)
		var
			list
				skipped_areas_apc = list(/area/engine/engineering)

		for(var/obj/machinery/power/smes/S in machines)
			var
				area
					current_area = get_area(S)
