...
/datum/event/disease_outbreak/event_resource_cost()
	return list("[ASSIGNMENT_CREW]" = (severity / EVENT_LEVEL_MAJOR) * (length(infected_players)), "[ASSIGNMENT_MEDICAL]" = (severity / nominal_severity) * (length(infected_players) / 3), "[ASSIGNMENT_VIROLOGIST]" = 1)
