...
datum
	event
		spawn_slaughter
			proc
				get_slaughter(var/end_if_fail = 0)
					spawn()
						var
							list
								candidates = pollCandidates("Do you want to play as a slaughter demon?", ROLE_DEMON, 1)
						if(!candidates.len)
						...
						var
							datum
								mind
									player_mind = new /datum/mind(key_of_slaughter)
						...
						var
							mob
								living
									simple_animal
										slaughter
											S = new /mob/living/simple_animal/slaughter/(holder)
						S.holder = holder
