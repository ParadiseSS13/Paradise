	...
	machinery
		arcade
			...
			var
				tokens = 0
			var
				freeplay = 0				//for debugging and admin kindness
			var
				token_price = 0
			var
				last_winner = null			//for letting people who to hunt down and steal prizes from
			var
				window_name = "arcade"		//in case you want to change the window name for certain machines
	...
	machinery
		arcade
			New()
				..()
				var
					choice = pick(subtypesof(/obj/machinery/arcade))
				new choice(loc)
				qdel(src)
...
obj
	machinery
		arcade
			proc
				pay_with_cash(var/obj/item/weapon/spacecash/cashmoney, var/mob/user)
					...
					var
						left = cashmoney.get_total() - token_price
...
obj
	machinery
		arcade
			proc
				pay_with_card(var/obj/item/weapon/card/id/I, var/mob/user)
					visible_message("<span class='info'>[usr] swipes a card through [src].</span>")
					var
						datum
							money_account
								customer_account = attempt_account_access_nosec(I.associated_account_number)
					if(!customer_account)
