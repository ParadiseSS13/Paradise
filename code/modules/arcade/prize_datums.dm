...
datum
	prizes
		proc
			PlaceOrder(var/obj/machinery/prize_counter/prize_counter, var/itemID)
				if(!prize_counter)
					return 0
				var
					datum
						prize_item
							item = global_prizes.prizes[itemID]
				if(!item)
					return 0
...
datum
	prize_item
		var
			name = "Prize"
		var
			desc = "This shouldn't show up..."
		var
			typepath = /obj/item/toy/prizeball
		var
			cost = 0
