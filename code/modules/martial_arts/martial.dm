...
datum
	martial_art
		proc
			disarm_act(var/mob/living/carbon/human/A, var/mob/living/carbon/human/D)
				return 0

datum
	martial_art
		proc
			harm_act(var/mob/living/carbon/human/A, var/mob/living/carbon/human/D)
				return 0

datum
	martial_art
		proc
			grab_act(var/mob/living/carbon/human/A, var/mob/living/carbon/human/D)
				return 0

datum
	martial_art
		proc
			help_act(var/mob/living/carbon/human/A, var/mob/living/carbon/human/D)
				return 0

datum
	martial_art
		proc
			add_to_streak(var/element,var/mob/living/carbon/human/D)
				if(D != current_target)
					current_target = D
					streak = ""
...
datum
	martial_art
		proc
			basic_hit(var/mob/living/carbon/human/A,var/mob/living/carbon/human/D)

				A.do_attack_animation(D)
				var
					damage = rand(A.species.punchdamagelow, A.species.punchdamagehigh)
				var
					datum
						unarmed_attack
							attack = A.species.unarmed

				var
					atk_verb = "[pick(attack.attack_verb)]"
				if(D.lying)
					atk_verb = "kick"
				...
				var
					obj
						item
							organ
								external
									affecting = D.get_organ(ran_zone(A.zone_sel.selecting))
				var
...
datum
	martial_art
		proc
			teach(var/mob/living/carbon/human/H,var/make_temporary=0)
				if(help_verb)
					H.verbs += help_verb
				if(make_temporary)
					temporary = 1
...
datum
	martial_art
		proc
			remove(var/mob/living/carbon/human/H)
					...
					if(!used)
						...
						var
							datum
								martial_art
									plasma_fist
										F = new/datum/martial_art/plasma_fist(null)
						F.teach(H)
...
obj
	item
		weapon
			twohanded
				bostaff
					attack(mob/target, mob/living/user)
						...
						switch(user.a_intent)
							if("disarm")
								...
								var
									list
										fluffmessages = list("[user] clubs [H] with [src]!", \
								...
								if(H.staminaloss && !H.sleeping)
									var
										total_health = (H.health - H.staminaloss)
