		...
		weapon
			paper
				...
				var
					ico[0]      //Icons and
				var
					offset_x[0] //offsets stored for later
				var
					offset_y[0] //usage by the photocopier
				var
					rigged = 0
				var
					spam_flag = 0
...
obj
	item
		weapon
			paper
				proc
					show_content(var/mob/user, var/forceshow = 0, var/forcestars = 0, var/infolinks = 0, var/view = 1)
						var
							datum
								asset
				...
				verb
					rename()
						...
						if((CLUMSY in usr.mutations) && prob(50))
							...
							return
						var
							n_name = sanitize(copytext(input(usr, "What would you like to label the paper?", "Paper Labelling", name) as text, 1, MAX_MESSAGE_LEN))
			...
			paper
				proc
					addtofield(var/id, var/text, var/links = 0)
						...
						if(links)
							var
								before = copytext(info_links, 1, textindex)
							var
								after = copytext(info_links, textindex)
							info_links = before + text + after
						else
							var
								before = copytext(info, 1, textindex)
							var
								after = copytext(info, textindex)
...
//Count the fields
	...
	while(fields < MAX_PAPER_FIELDS)
		var
			i = findtext(t, "<span class=\"paper_field\">", laststart)
		if(i==0)
			break
		laststart = i+1
		fields++
...
obj
	item
		weapon
			paper
				proc
					burnpaper(obj/item/weapon/lighter/P, mob/user)
						var
			...
			paper
				Topic(href, href_list)
					...
					if(href_list["write"])
						...
						var
							t =  input("Enter what you want to write:", "Write", null, null)  as message
						var
							obj
								item
									i = usr.get_active_hand() // Check to see if he still got that darn pen, also check if he's using a crayon or pen.
						var
							iscrayon = 0
...
obj
	item
		weapon
			paper
				proc
					stamp(var/obj/item/weapon/stamp/S)
						stamps += (!stamps || stamps == "" ? "<HR>" : "") + "<img src=large_[S.icon_state].png>"

						var
							image
								stampoverlay = image('icons/obj/bureaucracy.dmi')
						var/{x; y;}
	...
	item
		weapon
			paper
				evilfax
					proc
						evilpaper_specialaction(var/mob/living/carbon/target)
							spawn(30)
											...
											var
												obj
													item
														organ
															internal
																organ = new /obj/item/organ/internal/honktumor
											organ.insert(target)
