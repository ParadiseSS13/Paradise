...
var
	list
		admin_departments = list("Central Command")
var
	list
		hidden_admin_departments = list("Syndicate")
var
	list
		alldepartments = list()
					...
					obj
						item
							weapon
								card
									id
										scan = null // identification
				var
					authenticated = 0
				var
					sendcooldown = 0 // to avoid spamming fax messages

				var
					department = "Unknown" // our department

				var
					destination = "Central Command" // the department we're sending to
...
obj
	machinery
		photocopier
			faxmachine
				attackby(obj/item/weapon/item, mob/user, params)
				...
				Topic(href, href_list)
					...
					if(href_list["dept"])
						if(authenticated)
							...
							var
								list
									combineddepartments = alldepartments + admin_departments
							if(emagged)
					...
					if(href_list["rename"])
						if(copyitem)
							var
								n_name = sanitize(copytext(input(usr, "What would you like to label the fax?", "Fax Labelling", copyitem.name)  as text, 1, MAX_MESSAGE_LEN))
...
obj
	machinery
		photocopier
			faxmachine
				proc
					scan(var/obj/item/weapon/card/id/card = null)
...
obj
	machinery
		photocopier
			faxmachine
				proc
					sendfax(var/destination,var/mob/sender)
...
obj
	machinery
		photocopier
			faxmachine
				proc
					receivefax(var/obj/item/incoming)
						if(stat & (BROKEN|NOPOWER))
							return 0
...
obj
	machinery
		photocopier
			faxmachine
				proc
					send_admin_fax(var/mob/sender, var/destination)
						if(stat & (BROKEN|NOPOWER))
							return
