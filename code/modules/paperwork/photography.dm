...
obj
	item
		weapon
			photo
				attackby(obj/item/weapon/P as obj, mob/user as mob, params)
					if(istype(P, /obj/item/weapon/pen) || istype(P, /obj/item/toy/crayon))
						var
							txt = sanitize(input(user, "What would you like to write on the back?", "Photo Writing", null)  as text)
...
obj
	item
		weapon
			photo
				proc
					burnphoto(obj/item/weapon/lighter/P, mob/user)
						var
			...
			photo
				verb
					rename()
						...
						var
							n_name = sanitize(copytext(input(usr, "What would you like to label the photo?", "Photo Labelling", name) as text, 1, MAX_MESSAGE_LEN))
		...
		device
			camera
				...
				var
					size = 3
				var
					see_ghosts = 0 //for the spoop of it
						...
						var
							obj
								item
									device
										camera
											C = locate(/obj/item/device/camera) in contents
						if(C)
...
var
	list
		SpookyGhosts = list("ghost","shade","shade2","ghost-narsie","horror","shadow","ghostian2")
			...
			camera
				verb
					change_size()
						...
						set category = "Object"
						var
							nsize = input("Photo Size","Pick a size of resulting photo.") as null|anything in list(1,3,5,7)
						if(nsize)
							size = nsize
...
obj
	item
		device
			camera
				proc
					get_icon(list/turfs, turf/center,mob/user)
						...
						var
							list
								sorted = sort_atoms_by_layer(atoms)
						var
							center_offset = (size-1)/2 * 32 + 1
						for(var/i; i <= sorted.len; i++)
							...
							if(A)
								var
									icon
										img = getFlatIcon(A)//build_composite_icon(A)
								...
								if(istype(img, /icon))
									...
									var
										xoff = (A.x - center.x) * 32 + center_offset
									var
										yoff = (A.y - center.y) * 32 + center_offset
						...
						for(var/turf/the_turf in turfs)
							// Calculate where we are relative to the center of the photo
							var
								xoff = (the_turf.x - center.x) * 32 + center_offset
							var
								yoff = (the_turf.y - center.y) * 32 + center_offset
...
obj
	item
		device
			camera
				proc
					get_mobs(turf/the_turf as turf)
						var
							mob_detail
...
obj
	item
		device
			camera
				proc
					can_capture_turf(turf/T, mob/user)
						var
							mob
								dummy = new(T)	//Go go visibility check dummy
						var
							viewer = user
						if(user.client)		//To make shooting through security cameras possible
							viewer = user.client.eye
						var
							can_see = (dummy in viewers(world.view, viewer)) != null
...
obj
	item
		device
			camera
				proc
					captureimage(atom/target, mob/user, flag)
						var
							x_c = target.x - (size-1)/2
						var
							y_c = target.y + (size-1)/2
						var
							z_c	= target.z
						var
							list
						...
						var
							datum
								picture
									P = createpicture(target, user, turfs, mobs, flag)
						printpicture(user, P)
...
obj
	item
		device
			camera
				proc
					createpicture(atom/target, mob/user, list/turfs, mobs, flag)
						var
							icon
								photoimage = get_icon(turfs, target,user)

						var
							icon
								small_img = icon(photoimage)
						var
							icon
								tiny_img = icon(photoimage)
						var
							icon
								ic = icon('icons/obj/items.dmi',"photo")
						var
							icon
								pc = icon('icons/obj/bureaucracy.dmi', "photo")
						small_img.Scale(8, 8)
						tiny_img.Scale(4, 4)
...
obj
	item
		device
			camera
				proc
					printpicture(mob/user, var/datum/picture/P)
						var
							obj
								item
									weapon
										photo
											Photo = new/obj/item/weapon/photo()
						Photo.loc = user.loc
...
obj
	item
		device
			camera
				proc
					handle_haunt(mob/user as mob)
								var
									list
										creepyasssounds = list('sound/effects/ghost.ogg', 'sound/effects/ghost2.ogg', 'sound/effects/Heart Beat.ogg', 'sound/effects/screech.ogg',\
