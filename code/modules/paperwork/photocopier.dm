	...
	machinery
		photocopier
			...
			var
				obj
					item
						copyitem = null	//what's in the copier!
			var
				...
				if(href_list["copy"])
					...
					if(toner >= 5)
						...
						var
							datum
								picture
									selection = camera.selectpicture()
						if(!selection)
							return
						...
						var
							obj
								item
									weapon
										photo
											p = new /obj/item/weapon/photo (src.loc)
...
obj
	machinery
		photocopier
			proc
				copy(var/obj/item/weapon/paper/copy)
					var
						obj
							item
								weapon
					...
					var
						list
							temp_overlays = copy.overlays       //Iterates through stamps
					var
						image
							img                                //and puts a matching
...
obj
	machinery
		photocopier
			proc
				photocopy(var/obj/item/weapon/photo/photocopy)
					var
						obj
							item
								weapon
		...
		photocopier
			proc
				copyass()
					...
					var
						icon
							small_img = icon(temp_img) //Icon() is needed or else temp_img will be rescaled too >.>
					var
						icon
...
obj
	machinery
		photocopier
			proc
				bundlecopy(var/obj/item/weapon/paper_bundle/bundle, var/need_toner=1)
					var
						obj
							item
								weapon
									paper_bundle
										p = new /obj/item/weapon/paper_bundle (src)
...
obj
	machinery
		photocopier
			proc
				check_ass() //I'm not sure wether I made this proc because it's good form or because of the name.
					if(!ass)
						return 0
					if(ass.loc != src.loc)
						ass = null
