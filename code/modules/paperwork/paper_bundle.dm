	...
	item
		weapon
			paper_bundle
				...
				var
					amount = 0 //Amount of items clipped to the paper
				var
					page = 1
				var
					screen = 0
			...
			paper_bundle
				verb
					rename()
						...
						var
							n_name = sanitize(copytext(input(usr, "What would you like to label the bundle?", "Bundle Labelling", name) as text, 1, MAX_MESSAGE_LEN))
			...
			paper_bundle
				update_icon()
					...
					for(var/obj/O in src)
						var
							image
								img = image('icons/obj/bureaucracy.dmi')
