	...
	item
		device
			onetankbomb
				...
				var
					status = 0   //0 - not readied //1 - bomb finished with welder
				var
					obj
						item
							device
								assembly_holder
									bombassembly = null   //The first part of the bomb is an assembly holder, holding an igniter+some device
				var
					obj
						item
							weapon
								tank
									bombtank = null //the second part of the bomb is a plasma tank
...
obj
	item
		device
			onetankbomb
				attack_self(mob/user as mob) //pressing the bomb accesses its assembly
...
obj
	item
		device
			onetankbomb
				Crossed(atom/movable/AM as mob|obj) //for mousetraps
					if(bombassembly)
...
obj
	item
		device
			onetankbomb
				on_found(mob/finder as mob) //for mousetraps
					if(bombassembly)
...
obj
	item
		weapon
			tank
				proc
					bomb_assemble(W,user)	//Bomb assembly proc. This turns assembly+tank into a bomb
						var
							obj
								item
									device
						...
						var
							obj
								item
									device
										onetankbomb
											R = new /obj/item/device/onetankbomb(loc)
...
obj
	item
		weapon
			tank
				proc
					detonate()	//This happens when a bomb is told to explode
						var
							fuel_moles = air_contents.toxins + air_contents.oxygen/6
						var
							strength = 1

						var
							turf
								ground_zero = get_turf(loc)
						loc = null
...
obj
	item
		weapon
			tank
				proc
					release()	//This happens when the bomb is not welded. Tank contents are just spat out.
						var
							datum
								gas_mixture
									removed = air_contents.remove(air_contents.total_moles())
						var
							turf
								simulated
									T = get_turf(src)
