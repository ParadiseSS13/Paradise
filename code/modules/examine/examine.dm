...
atom/
	var
		description_info = null //Helpful blue text.
	var
		description_fluff = null //Green text about the atom's fluff, if any exists.
	var
		description_antag = null //Malicious red text, for the antags.
...
client
	var
		description_holders[0]

client
	proc
		update_description_holders(atom/A, update_antag_info=0)
...
mob
	examinate(atom/A as mob|obj|turf in view())
		if(..())
			return 1

		var
			is_antag = (isAntag(src) || isobserver(src)) //ghosts don't have minds
		if(client)
