	...
	mapGenerator
		proc
			defineCircularRegion(var/turf/Start, var/turf/End, var/replace = 0)
				...
				var
					centerX = max(abs((End.x+Start.x)/2),1)
				var
					centerY = max(abs((End.y+Start.y)/2),1)

				var
					lilZ = min(Start.z,End.z)
				var
					bigZ = max(Start.z,End.z)

				var
					sphereMagic = max(abs(bigZ-(lilZ/2)),1) //Spherical maps! woo!

				var
					radius = abs(max(centerX,centerY)) //take the biggest displacement as the radius
...
client
	proc
		debugNatureMapGenerator()
				...
				datum
					mapGenerator
						nature
							N = new()
			var
				startInput = input(usr,"Start turf of Map, (X;Y;Z)", "Map Gen Settings", "1;1;1") as text
			var
				endInput = input(usr,"End turf of Map (X;Y;Z)", "Map Gen Settings", "[world.maxx];[world.maxy];[mob ? mob.z : 1]") as text
			...
			var
				list
					startCoords = splittext(startInput, ";")
			var
				list
					endCoords = splittext(endInput, ";")
			...
			var
				turf
					Start = locate(text2num(startCoords[1]),text2num(startCoords[2]),text2num(startCoords[3]))
			var
				turf
					End = locate(text2num(endCoords[1]),text2num(endCoords[2]),text2num(endCoords[3]))
			if(!Start || !End)
			...
			var
				list
					clusters = list("None"=CLUSTER_CHECK_NONE,"All"=CLUSTER_CHECK_ALL,"Sames"=CLUSTER_CHECK_SAMES,"Differents"=CLUSTER_CHECK_DIFFERENTS, \
			...
			var
				moduleClusters = input("Cluster Flags (Cancel to leave unchanged from defaults)","Map Gen Settings") as null|anything in clusters
			//null for default
