...
mob
	living
		carbon
			human/ instead. And that gives a run-time error.
	...
	proc
		callproc()
			...
			spawn(0)
				...
				var
					procname = input("Proc path, eg: /proc/fake_blood","Path:", null) as text|null
				if(!procname)	return
...
client
	proc
		callproc_datum(var/A as null|area|mob|obj|turf)
			set category = "Debug"
			...
			var
				procname = input("Proc name, eg: fake_blood","Proc:", null) as text|null
			if(!procname)
				return
			...
			while(argnum--)
				...
				switch(class)
					...
					if("mob's area")
						var
							mob
								temp = input("Select mob", "Selection", usr) as mob in world
						lst += temp.loc
...
client
	proc
		cmd_admin_robotize(var/mob/M in mob_list)
			set category = "Event"
			set name = "Make Robot"
...
client
	proc
		cmd_admin_animalize(var/mob/M in mob_list)
			set category = "Event"
...
client
	proc
		makepAI(var/turf/T in mob_list)
			set category = "Event"
			set name = "Make pAI"
			...
			var
				mob
					choice = input("Choose a player to play the pAI", "Spawn pAI") in available
			if(!choice)
				return 0
			if(!istype(choice, /mob/dead/observer))
				var
					confirm = input("[choice.key] isn't ghosting right now. Are you sure you want to yank him out of them out of their body and place them in this pAI?", "Spawn pAI Confirmation", "No") in list("Yes", "No")
				if(confirm != "Yes")
					return 0
			var
				obj
...
client
	proc
		cmd_admin_alienize(var/mob/M in mob_list)
			set category = "Event"
			set name = "Make Alien"
...
client
	proc
		cmd_admin_slimeize(var/mob/M in mob_list)
			set category = "Event"
			set name = "Make slime"
...
client
	proc
		cmd_admin_super(var/mob/M in mob_list)
			set category = "Event"
			...
			if(ishuman(M))
				var
					type = input("Pick the Superhero","Superhero") as null|anything in all_superheroes
				var
					datum
						superheroes
							S = all_superheroes[type]
				if(S)
					S.create(M)
...
client
	proc
		cmd_debug_del_all()
			...
			var
				blocked = list(/mob/living, /mob/living/carbon, /mob/living/carbon/human, /mob/dead, /mob/dead/observer, /mob/living/silicon, /mob/living/silicon/robot, /mob/living/silicon/ai)
			var
				hsbitem = input(usr, "Choose an object to delete.", "Delete:") as null|anything in subtypesof(/obj) + subtypesof(/mob) - blocked
			if(hsbitem)
...
client
	proc
		cmd_debug_del_sing()
			...
			var
				confirm = alert("This will delete ALL Singularities and Tesla orbs except for any that are on away mission z-levels or the centcomm z-level. Are you sure you want to delete them?", "Confirm Panic Button", "Yes", "No")
			if(confirm != "Yes")
				return
...
client
	proc
		cmd_admin_grantfullaccess(var/mob/M in mob_list)
			set category = "Admin"
...
client
	proc
		cmd_assume_direct_control(var/mob/M in mob_list)
			set category = "Admin"
				...
				if(alert("This mob is being controlled by [M.ckey]. Are you sure you wish to assume control of it? [M.ckey] will be made a ghost.",,"Yes","No") != "Yes")
					...
					var
						mob
							dead
								observer
									ghost = new/mob/dead/observer(M,1)
					ghost.ckey = M.ckey
...
client
	proc
		cmd_admin_areatest()
			...
			var
				list
					areas_with_air_alarm = list()
			var
				list
					areas_with_RC = list()
			var
				list
					areas_with_light = list()
			var
				list
					areas_with_LS = list()
			var
				list
					areas_with_intercom = list()
			var
				list
					areas_with_camera = list()

			var
				list
					areas_with_multiple_APCs = list()
			var
				list
					areas_with_multiple_air_alarms = list()
...
client
	proc
		cmd_admin_dress(var/mob/living/carbon/human/M in mob_list)
			set category = "Event"
			...
			var
				dostrip = input("Do you want to strip [M] before equipping them? (0=no, 1=yes)", "STRIPTEASE") as null|anything in list(0,1)
			if(isnull(dostrip))
				return
			var
				dresscode = input("Select dress for [M]", "Robust quick dress shop") as null|anything in dresspacks
			if(isnull(dresscode))
				return
			...
			if(dresscode == "as job...")
				var
					jobname = input("Select job", "Robust quick dress shop") as null|anything in get_all_jobs()
				...
				for(var/obj/machinery/the_singularitygen/G in world)
					if(G.anchored)
						var
							obj
								singularity
									S = new /obj/singularity(get_turf(G), 50)
						spawn(0)
							qdel(G)
						S.energy = 1750
						S.current_size = 7
							...
							obj
								item
									weapon
										tank
											plasma
												Plasma = new/obj/item/weapon/tank/plasma(Rad)
...
client
	proc
		cmd_admin_toggle_block(var/mob/M,var/block)
			if(!check_rights(R_SPAWN))
				return
			...
			if(istype(M, /mob/living/carbon))
				...
				M.update_mutations()
				var
					state="[M.dna.GetSEState(block)?"on":"off"]"
				var
					blockname=assigned_blocks[block]
	...
	proc
		reload_nanoui_resources()
			...
			var
				datum
					asset
						assets = get_asset_datum(/datum/asset/nanoui)
			assets.register()
