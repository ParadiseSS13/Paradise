...
ADMIN_VERB(debug_check_piping, R_ADMIN, "Check Piping", "Check Piping", VERB_CATEGORY_MAPPING)
	...
	to_chat(user, "Checking for disconnected pipes...")

	//Manifolds
	for(var/obj/machinery/atmospherics/pipe/manifold/pipe in SSair.atmos_machinery)
		if(!pipe.node1 || !pipe.node2 || !pipe.node3)
			to_chat(user, "Unconnected [pipe.name] located at [pipe.x],[pipe.y],[pipe.z] ([get_area(pipe.loc)])")
	...
	for(var/datum/regional_powernet/PN in SSmachines.powernets)
		if(!PN.nodes || !length(PN.nodes))
			if(PN.cables && (length(PN.cables) > 1))
				...
				to_chat(user, "Powernet with no nodes! (number [PN.number]) - example cable at [C.x], [C.y], [C.z] in area [get_area(C.loc)]")

		if(!PN.cables || (length(PN.cables) < 10))
			if(PN.cables && (length(PN.cables) > 1))
				var/obj/structure/cable/C = PN.cables[1]
				to_chat(user, "Powernet with fewer than 10 cables! (number [PN.number]) - example cable at [C.x], [C.y], [C.z] in area [get_area(C.loc)]")
