...
/mob/living/carbon/human/proc/equip_syndicate_commando(is_leader = FALSE, full_gear = FALSE)
	...
	equip_to_slot_or_del(new /obj/item/clothing/under/syndicate(src), ITEM_SLOT_ICLOTHING)
	if(!full_gear)
		equip_to_slot_or_del(new /obj/item/clothing/shoes/combat(src), ITEM_SLOT_FEET)
	...
	if(full_gear)
		...
		equip_to_slot_or_del(new /obj/item/clothing/suit/space/hardsuit/syndi/elite/sst(src), ITEM_SLOT_OCLOTHING)
		...
		equip_to_slot_or_del(new /obj/item/clothing/shoes/magboots/elite(src), ITEM_SLOT_FEET)
