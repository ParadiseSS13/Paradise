...
var
	const
		syndicate_commandos_possible = 6 //if more Commandos are needed in the future
var
	global
		sent_syndicate_strike_team = 0
client
	proc
		syndicate_strike_team()
			set category = "Event"
			...
			var
				syndicate_commando_number = syndicate_commandos_possible //for selecting a leader
			var
...
//Generates a list of commandos from active ghosts. Then the user picks which characters to respawn as the commandos.
	...
	for(var/i=commandos_possible,(i>0&&candidates.len),i--)//Decrease with every commando selected.
		var
			candidate = input("Pick characters to spawn as the commandos. This will go on until there either no more ghosts to pick from or the slots are full.", "Active Players") as null|anything in candidates	//It will auto-pick a person when there is only one candidate.
...
mob
	living
		carbon
			human
				proc
					equip_syndicate_commando(syndicate_leader_selected = 0)
						...
						var
							obj
								item
									device
										radio
											R = new /obj/item/device/radio/headset/syndicate/alt(src)
