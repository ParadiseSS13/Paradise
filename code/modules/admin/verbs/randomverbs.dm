client
	proc
		cmd_admin_drop_everything(mob/M as mob in mob_list)
			set category = null
			...
			var
				confirm = alert(src, "Make [M] drop everything?", "Message", "Yes", "No")
			if(confirm != "Yes")
				return
...
client
	proc
		cmd_admin_prison(mob/M as mob in mob_list)
			set category = "Admin"
			set name = "Prison"
...
client
	proc
		cmd_admin_subtle_message(mob/M as mob in mob_list)
			set category = "Event"
			...
			var
				msg = input("Message:", text("Subtle PM to [M.key]")) as text
...
client
	proc
		cmd_mentor_check_new_players()	//Allows mentors / admins to determine who the newer players are.
			set category = "Admin"
			...
			var
				age = alert(src, "Age check", "Show accounts yonger then _____ days","7", "30" , "All")
...
client
	proc
		cmd_admin_world_narrate() // Allows administrators to fluff events a little easier -- TLE
			set category = "Event"
			...
			var
				msg = input("Message:", text("Enter the text you wish to appear to everyone:")) as text
...
client
	proc
		cmd_admin_direct_narrate(var/mob/M)	// Targetted narrate -- TLE
			set category = "Event"
			...
			var
				msg = input("Message:", text("Enter the text you wish to appear to your target:")) as text
...
client
	proc
		cmd_admin_godmode(mob/M as mob in mob_list)
...
client
	proc
		cmd_admin_add_random_ai_law()
			...
			var
				confirm = alert(src, "You sure?", "Confirm", "Yes", "No")
			if(confirm != "Yes") return
			log_admin("[key_name(src)] has added a random AI law.")
			message_admins("[key_name_admin(src)] has added a random AI law.")

			var
				show_log = alert(src, "Show ion message?", "Message", "Yes", "No")
			var
...
client
	proc
		respawn_character()
			...
			var
				input = ckey(input(src, "Please specify which key will be respawned.", "Key", ""))
			if(!input)
				return
			...
			var
				mob
					living
						carbon
							human
								new_character = new(pick(latejoin))//The mob being spawned.

			var
				datum
					data
						record
							record_found			//Referenced to later to either randomize or not randomize the character.
			if(G_found.mind && !G_found.mind.active)	//mind isn't currently in use by someone/something
				/*Try and locate a record for the person being respawned through data_core.
				This isn't an exact science but it does the trick more often than not.*/
				var
					id = md5("[G_found.real_name][G_found.mind.assigned_role]")
				...
				if("Syndicate")
					var
						obj
							effect
								landmark
									synd_spawn = locate("landmark*Syndicate-Spawn")
					if(synd_spawn)
...
proc
	create_xeno(ckey)
		...
		var
			alien_caste = input(usr, "Please choose which caste to spawn.","Pick a caste",null) as null|anything in list("Queen","Hunter","Sentinel","Drone","Larva")
		var
			obj
				effect
					landmark
						spawn_here = xeno_spawn.len ? pick(xeno_spawn) : pick(latejoin)
		var
			mob
				living
					carbon
...
client
	proc
		get_ghosts(var/notify = 0,var/what = 2)
			...
			var
				list
					sortmob = sortAtom(mob_list)                           // get the mob list.
			var
				any=0
...
client
	proc
		cmd_admin_add_freeform_ai_law()
			...
			var
				input = input(usr, "Please enter anything you want the AI to do. Anything. Serious.", "What?", "") as text|null
			if(!input)
				return
...
client
	proc
		cmd_admin_rejuvenate(mob/living/M as mob in mob_list)
			set category = "Special Verbs"
			set name = "Rejuvenate"
...
client
	proc
		cmd_admin_create_centcom_report()
			...
			var
				list
					MsgType = list("Centcom Report","Enemy Communications")
			if(!check_rights(R_SERVER|R_EVENT))
				return

			var
				type = input(usr, "Pick a type of report to send", "Report Type", "") as anything in MsgType
			var
				input = input(usr, "Please enter anything you want. Anything. Serious.", "What?", "") as message|null
			var
				customname = input(usr, "Pick a title for the report.", "Title") as text|null
			if(!input)
				return
			...
			if(type == "Enemy Communications")
				...
				var
					from = input(usr, "What kind of report? Example: Syndicate Communique", "From") as text|null
				if(!from)
...
client
	proc
		cmd_admin_delete(atom/O as obj|mob|turf in view())
			set category = "Admin"
			set name = "Delete"
...
client
	proc
		cmd_admin_explosion(atom/O as obj|mob|turf in view())
			set category = "Event"
			set name = "Explosion"
...
client
	proc
		cmd_admin_emp(atom/O as obj|mob|turf in view())
			set category = "Special Verbs"
			set name = "EM Pulse"
			...
			var
				heavy = input("Range of heavy pulse.", text("Input"))  as num|null
			if(heavy == null) return
			var
				light = input("Range of light pulse.", text("Input"))  as num|null
			if(light == null) return
...
client
	proc
		cmd_admin_gib(mob/M as mob in mob_list)
			set category = "Special Verbs"
			set name = "Gib"
...
client
	proc
		cmd_admin_check_contents(mob/living/M as mob in mob_list)
			set category = "Special Verbs"
...
client
	proc
		cmd_admin_attack_log(mob/M as mob in mob_list)
			set category = "Special Verbs"
			set name = "Attack Log"
...
client
	proc
		everyone_random()
			...
			var
				notifyplayers = alert(src, "Do you want to notify the players?", "Options", "Yes", "No", "Cancel")
...
client
	proc
		reset_all_tcs()
			...
			var
				confirm = alert(src, "You sure you want to blank all NTSL scripts?", "Confirm", "Yes", "No")
			if(confirm != "Yes")
				return
