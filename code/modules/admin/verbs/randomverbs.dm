...
/client/proc/cmd_admin_subtle_message(mob/M as mob in GLOB.mob_list)
	...
	message_admins("<span class='boldnotice'>SubtleMessage: [key_name_admin(usr)] -> [key_name_admin(M)] : [msg]</span>")
...
/client/proc/cmd_admin_world_narrate() // Allows administrators to fluff events a little easier -- TLE
	...
	message_admins("<span class='boldnotice'>GlobalNarrate: [key_name_admin(usr)]: [msg]<BR></span>")
...
/client/proc/cmd_admin_direct_narrate(var/mob/M)	// Targetted narrate -- TLE
	...
	message_admins("<span class='boldnotice'>DirectNarrate: [key_name_admin(usr)] to ([key_name_admin(M)]): [msg]<BR></span>")
...
/client/proc/respawn_character()
	...
	*/
	...
	message_admins("<span class='notice'>[key_name_admin(usr)] has respawned [key_name_admin(G_found)] as [new_character.real_name].</span>")
...
/client/proc/toggle_view_range()
	...
	//message_admins("<span class='notice'>[key_name_admin(usr)] changed their view range to [view].</span>")	//why? removed by order of XSI
...
/client/proc/toggle_ert_calling()
	...
	if(ticker.mode.ert_disabled)
		...
		message_admins("Admin [key_name_admin(usr)] has enabled ERT calling.")
	else
		ticker.mode.ert_disabled = 1
		to_chat(usr, "<span class='warning'>ERT has been <b>Disabled</b>.</span>")
		log_admin("Admin [key_name(src)] has disabled ERT calling.")
		message_admins("Admin [key_name_admin(usr)] has disabled ERT calling.")
