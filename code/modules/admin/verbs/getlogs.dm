...
client
	proc
		giveruntimelog()
			...
			var
				client
					target = input(src,"Choose somebody to grant access to the server's runtime logs (permissions expire at the end of each round):","Grant Permissions",null) as null|anything in clients
...
client
	proc
		getruntimelog()
			...
			var
				path = browse_files("data/logs/runtime/")
			if(!path)
				return
	...
	admins
		proc
			view_txt_log()
				...
				var
					path = "data/logs/[time2text(world.realtime,"YYYY/MM-Month/DD-Day")].log"
				if( fexists(path) )
					src << ftp(file(path))
	...
	admins
		proc
			view_atk_log()
				...
				var
					path = "data/logs/[time2text(world.realtime,"YYYY/MM-Month/DD-Day")] Attack.log"
				if( fexists(path) )
					src << ftp(file(path))
