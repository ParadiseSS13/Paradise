...
var
	const
		commandos_possible = 6 //if more Commandos are needed in the future
var
	global
		sent_strike_team = 0
...
client
	proc
		strike_team()
			...
			var
				commando_number = commandos_possible //for selecting a leader
			var
				leader_selected = 0 //when the leader is chosen. The last person spawned.
			...
			var
				mob
					living
						carbon
							human
								new_commando = create_death_commando(L, leader_selected)
...
client
	proc
		create_death_commando(obj/spawn_location, leader_selected = 0)
			var
				mob
					living
						carbon
							human
								new_commando = new(spawn_location.loc)
			var
				commando_leader_rank = pick("Lieutenant", "Captain", "Major")
			var
				commando_rank = pick("Corporal", "Sergeant", "Staff Sergeant", "Sergeant 1st Class", "Master Sergeant", "Sergeant Major")
			var
...
mob
	living
		carbon
			human
				proc
					equip_death_commando(leader_selected = 0)
						...
						var
							obj
								item
									device
										radio
											R = new /obj/item/device/radio/headset/alt(src)
							...
							obj
								item
									weapon
										implant
											loyalty
												L = new/obj/item/weapon/implant/loyalty(src)//Here you go Deuryn
						L.imp_in = src
						L.implanted = 1
