...
/client/proc/strike_team()
	...
	message_admins(span_notice("[key_name_admin(usr)] has started to spawn a CentComm DeathSquad."), 1)
	...
	for(var/obj/effect/landmark/spawner/ds/L in GLOB.landmarks_list)
		...
		if(use_ds_borg)
			...
			R.mind.store_memory("<B>Mission:</B> [span_warning("[input].")]")
			...
			new_commando.mind.store_memory("<B>Mission:</B> [span_warning("[input].")]")
	...
	message_admins(span_notice("[key_name_admin(usr)] has spawned a CentComm DeathSquad."), 1)
