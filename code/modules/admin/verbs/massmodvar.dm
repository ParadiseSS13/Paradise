client
	proc
		cmd_mass_modify_object_variables(atom/A, var/var_name)
			set category = "Debug"
			set name = "Mass Edit Variables"
			set desc="(target) Edit all instances of a target item's variables"

			var
				method = 0	//0 means strict type detection while 1 means this type and all subtypes (IE: /obj/item with this set to 1 will set it to ALL itms)
...
client
	proc
		massmodify_variables(var/atom/O, var/var_name = "", var/method = 0)
			if(!check_rights(R_VAREDIT))	return

			var
				list
					locked = list("vars", "key", "ckey", "client")
			...
			switch(class)
				...
				if("text")
					var
						new_value = input("Enter new text:","Text",O.vars[variable]) as message|null
				...
				if("file")
					var
						new_value = input("Pick file:","File",O.vars[variable]) as null|file
					if(new_value == null) return
					O.vars[variable] = new_value
				...
				if("icon")
					var
						new_value = input("Pick icon:","Icon",O.vars[variable]) as null|icon
					if(new_value == null) return
					O.vars[variable] = new_value
					if(method)
						if(istype(O, /mob))
