...
client
	proc
		cmd_admin_pm_context(mob/M as mob in mob_list)
			set category = null
...
client
	proc
		cmd_admin_pm_panel()
			...
			var
				list
					sorted = sortList(targets)
			var
				target = input(src,"To whom shall we send a message?","Admin PM",null) in sorted|null
				...
				if(config.popup_admin_pm)
					spawn(0)	//so we don't hold the caller proc up
						...
						var
							sendername = key
						var
							reply = input(C, msg,"[recieve_pm_type] [type] from-[sendername]", "") as text|null		//show message and await a reply
						if(C && reply)
							if(sender)
...
client
	proc
		cmd_admin_irc_pm()
			...
			var
				msg = input(src,"Message:", "Private message to admins on IRC / 400 character limit") as text|null
