...
/client/proc/cmd_admin_pm(whom, msg, type = "PM", ticket_id = -1)
	...
	if(message_type == MESSAGE_TYPE_MENTORPM && check_rights(R_ADMIN|R_MENTOR, 0, C.mob))
		...
		recieve_window_link = ticket_link
	recieve_message = "<span class='[recieve_span]'>[type] from-<b>[recieve_pm_type] [C.holder ? key_name(src, TRUE, type, ticket_id = ticket_id) : key_name_hidden(src, TRUE, type, ticket_id = ticket_id)]</b>:<br><br>[emoji_msg][C.holder ? "<br>[ping_link] [recieve_window_link] [alert_link] [observe_link]" : ""]</span>"
	if(message_type == MESSAGE_TYPE_MENTORPM)
