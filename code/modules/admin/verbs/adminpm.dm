...
/client/proc/cmd_admin_pm(whom, msg, type = "PM")
	...
	recieve_message = "<span class='[recieve_span]'>[type] from-<b>[recieve_pm_type][key_name(src, C, C.holder ? 1 : 0, type)]</b>: [sanitize_local(msg)]</span>"
	to_chat(C, recieve_message)
	to_chat(src, "<font color='blue'>[send_pm_type][type] to-<b>[key_name(C, src, holder ? 1 : 0, type)]</b>: [sanitize_local(msg)]</font>")
			...
			switch(type)
				if("Mentorhelp")
					if(check_rights(R_ADMIN|R_MOD|R_MENTOR, 0, X.mob))
						to_chat(X, "<span class='mentorhelp'>[type]: [key_name(src, X, 0, type)]-&gt;[key_name(C, X, 0, type)]: [sanitize_local(msg)]</span>")
				if("Adminhelp")
					if(check_rights(R_ADMIN|R_MOD, 0, X.mob))
						to_chat(X, "<span class='adminhelp'>[type]: [key_name(src, X, 0, type)]-&gt;[key_name(C, X, 0, type)]: [sanitize_local(msg)]</span>")
				else
					if(check_rights(R_ADMIN|R_MOD, 0, X.mob))
						to_chat(X, "<span class='boldnotice'>[type]: [key_name(src, X, 0, type)]-&gt;[key_name(C, X, 0, type)]: [sanitize_local(msg)]</span>")
...
/client/proc/cmd_admin_irc_pm()
	...
	to_chat(src, "<font color='blue'>IRC PM to-<b>IRC-Admins</b>: [sanitize_local(msg)]</font>")
	...
	for(var/client/X in admins)
		if(X == src)
			continue
		if(check_rights(R_ADMIN|R_MOD|R_MENTOR, 0, X.mob))
			to_chat(X, "<B><font color='blue'>PM: [key_name(src, X, 0)]-&gt;IRC-Admins:</B> \blue [sanitize_local(msg)]</font>")
