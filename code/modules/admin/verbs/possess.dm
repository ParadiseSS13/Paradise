/proc/possess(obj/O as obj in world)
	...
	if(T)
		...
		message_admins("[key_name_admin(usr)] has possessed [O] ([O.type]) at ([T.x], [T.y], [T.z])")
	else
		log_admin("[key_name(usr)] has possessed [O] ([O.type]) at an unknown location")
		message_admins("[key_name_admin(usr)] has possessed [O] ([O.type]) at an unknown location")
