...
client
	proc
		SDQL2_query(query_text as message)
			set category = "Debug"
...
//	to_chat(world, query_text)
	...
	var
		list
			query_list = SDQL2_tokenize(query_text)
...
proc
	SDQL_var(datum/object, list/expression, start = 1, source)
		var
			v
			...
			if(expression[start + 1] == ".")
				...
				var
					list
						L = v
				var
					index = SDQL_expression(source, expression[start + 2])
...
proc
	SDQL_function(var/datum/object, var/procname, var/list/arguments, source)
		var
			list
				new_args = list()
