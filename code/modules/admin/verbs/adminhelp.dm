...
var
	list
		adminhelp_ignored_words = list("unknown","the","a","an","of","monkey","alien","as")
...
client
	verb
		adminhelp()
			...
			var
				list
					type = list("Mentorhelp","Adminhelp")
			var
				selected_type = input("Pick a category.", "Admin Help", null, null) as null|anything in type
			if(selected_type)
			...
			var
				admin_number_present = adminholders.len - admin_number_afk
...
proc
	send2irc_adminless_only(source, msg, requiredflags = R_BAN)
		var
			admin_number_total = 0		//Total number of admins
		var
			admin_number_afk = 0		//Holds the number of admins who are afk
		var
			admin_number_ignored = 0	//Holds the number of admins without +BAN (so admins who are not really admins)
		var
			admin_number_decrease = 0	//Holds the number of admins with are afk, ignored or both
		for(var/client/X in admins)
			admin_number_total++;
			var
				invalid = 0
		...
		var
			admin_number_present = admin_number_total - admin_number_decrease	//Number of admins who are neither afk nor invalid
		if(admin_number_present <= 0)
