...
var
	const
		honksquad_possible = 6 //if more Commandos are needed in the future
var
	global
		sent_honksquad = 0
...
client
	proc
		honksquad()
			...
			var
				honksquad_number = honksquad_possible //for selecting a leader
			var
				honk_leader_selected = 0 //when the leader is chosen. The last person spawned.


//Generates a list of HONKsquad from active ghosts. Then the user picks which characters to respawn as the commandos.
	var
		list
			candidates = list()	//candidates for being a commando out of all the active ghosts in world.
	var
		list
			commandos = list()			//actual commando ghosts as picked by the user.
	...
	for(var/i=honksquad_possible,(i>0&&candidates.len),i--)//Decrease with every commando selected.
		var
			candidate = input("Pick characters to spawn as the HONKsquad. This will go on until there either no more ghosts to pick from or the slots are full.", "Active Players") as null|anything in candidates	//It will auto-pick a person when there is only one candidate.
			...
			var
				mob
					living
						carbon
							human
								new_honksquad = create_honksquad(L, honk_leader_selected)
...
client
	proc
		create_honksquad(obj/spawn_location, honk_leader_selected = 0)
			var
				mob
					living
						carbon
							human
								new_honksquad = new(spawn_location.loc)
			var
				honksquad_leader_rank = pick("Lieutenant", "Captain", "Major")
			var
				honksquad_rank = pick("Corporal", "Sergeant", "Staff Sergeant", "Sergeant 1st Class", "Master Sergeant", "Sergeant Major")
			var
				honksquad_name = pick(clown_names)

			var
				datum
					preferences
						A = new()//Randomize appearance for the commando.
...
mob
	living
		carbon
			human
				proc
					equip_honksquad(honk_leader_selected = 0)
						...
						var
							obj
								item
									device
										radio
											R = new /obj/item/device/radio/headset(src)
