ADMIN_VERB(set_ticklag, R_MAINTAINER, "Set Ticklag", \
	...
	if(newtick && newtick <= 2 && newtick > 0)
		log_admin("[key_name(user)] has modified world.tick_lag to [newtick]", 0)
		message_admins("[key_name_admin(user)] has modified world.tick_lag to [newtick]", 0)
		...
		to_chat(user, "<span class='warning'>Error: ticklag(): Invalid world.ticklag value. No changes made.</span>")
