...
var
	global
		list
			frozen_mob_list = list()
client
	proc
		freeze(var/mob/living/M as mob in mob_list)
...
mob
	living
		var
			frozen = 0 //used for preventing attacks on admin-frozen mobs
mob
	living
		var
			admin_prev_sleeping = 0 //used for keeping track of previous sleeping value with admin freeze

mob
	living
		proc
			admin_Freeze(var/client/admin)
				if(istype(admin))
...
mob
	living
		proc
			admin_unFreeze(var/client/admin)
				if(istype(admin))
...
client
	proc
		freezemecha(var/obj/mecha/O as obj in mechas_list)
			set category = "Special Verbs"
				...
				if(usr)
					if(usr.client)
						if(usr.client.holder)
							var
								adminomaly = new/obj/effect/overlay/adminoverlay
							if(M.can_move == 1)
								M.can_move = 0
