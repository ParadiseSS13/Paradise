						...
						var
							obj
								effect
									debugging
										marker
											F = new/obj/effect/debugging/marker(T)
...
var
	list
		admin_verbs_show_debug_verbs = list(
			client
				proc
					camera_view,
			client
			...
			client
				proc
					Cell, //More air things
			client
				proc
					atmosscan, //check plumbing
			client
				proc
					powerdebug, //check power
			client
				proc
			...
			client
				proc
					cmd_admin_grantfullaccess,
			client
				proc
					cmd_admin_areatest,
			client
				proc
					cmd_admin_rejuvenate,
			datum
				admins
					proc
						show_traitor_panel,
			...
			client
				proc
					print_jobban_old_filter,
			client
				proc
					forceEvent,
			client
				proc
					nanomapgen_DumpImage,
			client
				proc
					reload_nanoui_resources
)
...
client
	proc
		count_objects_on_z_level()
			...
			var
				level = input("Which z-level?","Level?") as text
			if(!level) return
			var
				num_level = text2num(level)
			if(!num_level) return
			if(!isnum(num_level)) return

			var
				type_text = input("Which type path?","Path?") as text
			if(!type_text) return
			var
				type_path = text2path(type_text)
			if(!type_path) return
...
client
	proc
		count_objects_all()
			...
			var
				type_text = input("Which type path?","") as text
			if(!type_text) return
			var
				type_path = text2path(type_text)
			if(!type_path) return
