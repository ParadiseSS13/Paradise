client
	proc
		only_one()
			...
			var
				list
					incompatible_species = list("Plasmaman", "Vox")
			...
			for(var/mob/living/carbon/human/H in player_list)
				...
				var
					datum
						objective
							hijackclone
								hijack_objective = new /datum/objective/hijackclone
				hijack_objective.owner = H.mind
				H.mind.objectives += hijack_objective
				...
				var
					obj
						item
							slot_item_ID = H.get_item_by_slot(slot_wear_id)
				qdel(slot_item_ID)
				var
					obj
						item
							slot_item_hand = H.get_item_by_slot(slot_r_hand)
