	...
	admins
		proc
			one_click_antag()
				...
				var
					dat = {"<B>One-click Antagonist</B><br>
	...
	admins
		proc
			makeTraitors()
				...
				if(candidates.len)
					var
						numTratiors = min(candidates.len, 3)
	...
	admins
		proc
			makeChanglings()
				...
				if(candidates.len)
					var
						numChanglings = min(candidates.len, 3)
					...
					var
						mob
							living
								carbon
									human
										new_character=makeBody(theghost)
	...
	admins
		proc
			makeCult()
				...
				if(candidates.len)
					var
						numCultists = min(candidates.len, 4)
				...
				if(candidates.len)
					...
					var
						obj
							effect
								landmark
									closet_spawn = locate("landmark*Nuclear-Closet")
							...
							if(synd_mind.current.client)
								for(var/datum/mind/synd_mind_1 in ticker.mode.syndicates)
									if(synd_mind_1.current)
										var
											I = image('icons/mob/mob.dmi', loc = synd_mind_1.current, icon_state = "synd")
	...
	admins
		proc
			makeDeathsquad()
				...
				var
					syndicate_leader_selected = 0 //when the leader is chosen. The last person spawned.
							...
							var
								mob
									living
										carbon
											human
												new_syndicate_commando = create_syndicate_death_commando(L, syndicate_leader_selected)
...
proc
	makeBody(var/mob/dead/observer/G_found) // Uses stripped down and bastardized code from respawn character
...
datum
	admins
		proc
			create_syndicate_death_commando(obj/spawn_location, syndicate_leader_selected = 0)
				var
					mob
						living
							carbon
								human
									new_syndicate_commando = new(spawn_location.loc)
				var
					syndicate_commando_leader_rank = pick("Lieutenant", "Captain", "Major")
				var
					syndicate_commando_rank = pick("Corporal", "Sergeant", "Staff Sergeant", "Sergeant 1st Class", "Master Sergeant", "Sergeant Major")
				var
					syndicate_commando_name = pick(last_names)
	...
	admins
		proc
			makeVoxRaiders()
				...
				var
					leader_chosen = 0 //when the leader is chosen. The last person spawned.
							...
							var
								mob
									living
										carbon
											human
												new_vox = create_vox_raider(L, leader_chosen)
...
datum
	admins
		proc
			create_vox_raider(obj/spawn_location, leader_chosen = 0)
				...
				var
					mob
						living
							carbon
								human
									new_vox = new /mob/living/carbon/human/vox(spawn_location.loc)
	...
	admins
		proc
			makeVampires()
				...
				if(candidates.len)
					var
						numVampires = min(candidates.len, 3)
...
datum
	admins
		proc
			makeThunderdomeTeams() // Not strictly an antag, but this seemed to be the best place to put it.
