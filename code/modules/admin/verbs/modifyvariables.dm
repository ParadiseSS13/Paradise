var
	list
		forbidden_varedit_object_types = list(
												datum
													admins,						//Admins editing their own admin-power object? Yup, sounds like a good idea.
												obj
													machinery
														blackbox_recorder,	//Prevents people messing with feedback gathering
												datum
													feedback_variable			//Prevents people messing with feedback gathering
			)

/*
client
	proc
		cmd_modify_object_variables(obj/O as obj|mob|turf|area in world)
			set category = "Debug"
...
client
	proc
		mod_list_add_ass() //haha

			var
				class = "text"
			var
				list
					allowed_types = list("text", "num","type", "type from text","reference","mob reference", "icon","file","list","edit referenced object","restore to default")
			...
			switch(class)
				...
				if("type from text")
					var
						type_text = input("Enter type:", "Type") as null|message
...
client
	proc
		mod_list(var/list/L)
			...
			if(L.len > 1000)
				var
					confirm = alert(src, "The list you're trying to edit is very long, continuing may crash the server.", "Warning", "Continue", "Abort")
				if(confirm != "Continue")
					return

			var
				list
					locked = list("vars", "key", "ckey", "client", "firemut", "ishulk", "telekinesis", "xray", "virus", "viruses", "cuffed", "ka", "last_eaten", "urine", "poo", "icon", "icon_state", "step_x", "step_y")
			...
			var
				list
					allowed_types = list("text", "num","type", "type from text", "reference","mob reference", "icon","file","list","edit referenced object","restore to default","DELETE FROM LIST")
			if(src.holder && src.holder.marked_datum)
				allowed_types += "marked datum ([holder.marked_datum.type])"
...
client
	proc
		modify_variables(var/atom/O, var/param_var_name = null, var/autodetect_class = 0)
			if(!check_rights(R_VAREDIT))	return

			var
				list
					locked = list("vars", "key", "ckey", "client", "firemut", "ishulk", "telekinesis", "xray", "virus", "cuffed", "ka", "last_eaten", "icon", "icon_state")
			...
			if(!autodetect_class)
				...
				var
					list
						allowed_types = list("text", "num","type","reference","mob reference", "path", "matrix", "icon","file","list","edit referenced object","restore to default")
				if(src.holder && src.holder.marked_datum)
					allowed_types += "marked datum ([holder.marked_datum.type])"
			...
			switch(class)
				...
				if("text")
					var
						var_new = input("Enter new text:","Text",O.vars[variable]) as null|message
				...
				if("num")
					if(variable=="light_range")
						var
							var_new = input("Enter new number:","Num",O.vars[variable]) as null|num
						if(var_new == null) return
						O.set_light(var_new)
				...
				if("type")
					var
						var_new = input("Enter type:","Type",O.vars[variable]) as null|anything in typesof(/obj,/mob,/area,/turf)
					if(var_new==null) return
					O.vars[variable] = var_new

				if("path")
					var
						path_text = input("Enter path:", "Path",O.vars[variable]) as null|text
					var
						var_new = text2path(path_text)
				...
				if("matrix")
					var
						matrix_text = input("Enter a, b, c, d, e, and f, separated by a space.", "Matrix", "1 0 0 0 1 0") as null|text
					var
						var_new = text2matrix(matrix_text)
				...
				if("reference")
					var
						var_new = input("Select reference:","Reference",O.vars[variable]) as null|mob|obj|turf|area in world
					if(var_new==null) return
					O.vars[variable] = var_new

				if("mob reference")
					var
						var_new = input("Select reference:","Reference",O.vars[variable]) as null|mob in world
					if(var_new==null) return
					O.vars[variable] = var_new

				if("file")
					var
						var_new = input("Pick file:","File",O.vars[variable]) as null|file
					if(var_new==null) return
					O.vars[variable] = var_new

				if("icon")
					var
						var_new = input("Pick icon:","Icon",O.vars[variable]) as null|icon
					if(var_new==null) return
					O.vars[variable] = var_new
