...
/client/proc/cmd_mod_say(msg as text)
	...
	for(var/client/C in admins)
		if(R_MOD & C.holder.rights)
			C << "<span class='[spanclass]'>" + create_text_tag("mod", "MOD:", C) + "<span class='name'>[key_name(usr, 1)]</span> (<A HREF='?src=\ref[C.holder];adminplayerobservejump=\ref[mob]'>JMP</A>): <span class='message'>[msg]</span></span>"
