/client/proc/cmd_admin_say(msg as text)
	...
	for(var/client/C in GLOB.admins)
		...
		if(R_ADMIN & C.holder.rights)
			...
			to_chat(C, SPAN_ADMIN_CHANNEL("ADMIN: <font color='[display_color]'>[key_name(usr, 1)]</font> ([admin_jump_link(mob)]): [SPAN_MESSAGE("[temp_message]")]"), MESSAGE_TYPE_ADMINCHAT, confidential = TRUE)
...
/client/proc/cmd_dev_say(msg as text)
	...
	for(var/client/C in GLOB.admins)
		if(check_rights(R_ADMIN|R_MOD|R_DEV_TEAM, 0, C.mob))
			...
			to_chat(C, "<span class='[check_rights(R_ADMIN, 0) ? "dev_channel_admin" : "dev_channel"]'>DEV: <font color='[display_color]'>[display_name]</font> ([admin_jump_link(mob)]): [SPAN_MESSAGE("[msg]")]</span>", MESSAGE_TYPE_DEVCHAT, confidential = TRUE)
		...
		if(check_rights(0, 0, C.mob))
			...
			if(holder.fakekey)
				if(C.holder && C.holder.rights & R_ADMIN)
					...
					display_name = holder.fakekey
			msg = SPAN_EMOJI_ENABLED("[msg]")
			to_chat(C, "<span class='[check_rights(R_ADMIN, 0) ? "staff_channel_admin" : "staff_channel"]'>STAFF: <font color='[display_color]'>[display_name]</font> ([admin_jump_link(mob)]): [SPAN_MESSAGE("[msg]")]</span>", MESSAGE_TYPE_STAFFCHAT, confidential = TRUE)
		...
		if(check_rights(R_ADMIN|R_MOD|R_MENTOR, 0, C.mob))
			...
			if(holder.fakekey)
				if(C.holder && C.holder.rights & R_ADMIN)
					...
					display_name = holder.fakekey
			msg = SPAN_EMOJI_ENABLED("[msg]")
			to_chat(C, "<span class='[check_rights(R_ADMIN, 0) ? "mentor_channel_admin" : "mentor_channel"]'>MENTOR: <font color='[display_color]'>[display_name]</font> ([admin_jump_link(mob)]): [SPAN_MESSAGE("[msg]")]</span>", MESSAGE_TYPE_MENTORCHAT, confidential = TRUE)
