	...
	if(check_rights(R_ADMIN,0))
		...
		for(var/client/C in GLOB.admins)
			...
			if(R_ADMIN & C.holder.rights)
				...
				to_chat(C, span_admin_channel("ADMIN: [span_name("[key_name(usr, 1)]")] ([admin_jump_link(mob)]): [span_message("[temp_message]")]"), MESSAGE_TYPE_ADMINCHAT, confidential = TRUE)
...
/client/proc/cmd_dev_say(msg as text)
	...
	for(var/client/C in GLOB.admins)
		if(check_rights(R_ADMIN|R_MOD|R_DEV_TEAM, 0, C.mob))
			...
			to_chat(C, "<span class='[check_rights(R_ADMIN, 0) ? "dev_channel_admin" : "dev_channel"]'>DEV: [span_name("[display_name]")] ([admin_jump_link(mob)]): [span_message("[msg]")]</span>", MESSAGE_TYPE_DEVCHAT, confidential = TRUE)
		...
		if(check_rights(0, 0, C.mob))
			...
			if(holder.fakekey)
				if(C.holder && C.holder.rights & R_ADMIN)
					...
					display_name = holder.fakekey
			msg = span_emoji_enabled("[msg]")
			to_chat(C, "<span class='[check_rights(R_ADMIN, 0) ? "staff_channel_admin" : "staff_channel"]'>STAFF: [span_name("[display_name]")] ([admin_jump_link(mob)]): [span_message("[msg]")]</span>", MESSAGE_TYPE_STAFFCHAT, confidential = TRUE)
