	...
	switch(action)
		...
		if("show_player_panel")
			...
			if(QDELETED(mind.current))
				...
				return
			SSadmin_verbs.invoke_verb(ui.user, /datum/admin_verb/show_player_panel, mind.current)
		if("pm")
		...
		if("obs")
			...
			if(!ismob(mind.current))
				...
				return
			SSadmin_verbs.invoke_verb(C, /datum/admin_verb/admin_observe_target, mind.current)
		if("tp")
			var/datum/mind/mind = locateUID(params["mind_uid"])
			if(QDELETED(mind))
