...
datum
	admins
		proc
			DB_ban_record(var/bantype, var/mob/banned_mob, var/duration = -1, var/reason, var/job = "", var/rounds = 0, var/banckey = null, var/banip = null, var/bancid = null)
				...
				var
					bantype_str
				var
					maxadminbancheck	//Used to limit the number of active bans of a certein type that each admin can give. Used to protect against abuse or mutiny.
				var
					announceinirc		//When set, it announces the ban in irc. Intended to be a way to raise an alarm, so to speak.
				var
					blockselfban		//Used to prevent the banning of yourself.
				var
					kickbannedckey		//Defines whether this proc should kick the banned person, if they are connected (if banned_mob is defined).
				...
				var
					DBQuery
						query = dbcon.NewQuery("SELECT id FROM [format_table_name("player")] WHERE ckey = '[ckey]'")
				query.Execute()
				var
					validckey = 0
				if(query.NextRow())
				...
				if(maxadminbancheck)
					var
						DBQuery
							adm_query = dbcon.NewQuery("SELECT count(id) AS num FROM [format_table_name("ban")] WHERE (a_ckey = '[a_ckey]') AND (bantype = 'ADMIN_PERMABAN'  OR (bantype = 'ADMIN_TEMPBAN' AND expiration_time > Now())) AND isnull(unbanned)")
					adm_query.Execute()
					if(adm_query.NextRow())
						var
							adm_bans = text2num(adm_query.item[1])
...
datum
	admins
		proc
			DB_ban_unban(var/ckey, var/bantype, var/job = "")
				...
				var
					sql = "SELECT id FROM [format_table_name("ban")] WHERE ckey = '[ckey]' AND [bantype_sql] AND (unbanned is null OR unbanned = false)"
				if(job)
					sql += " AND job = '[job]'"
...
datum
	admins
		proc
			DB_ban_edit(var/banid = null, var/param = null)
				...
				var
					DBQuery
						query = dbcon.NewQuery("SELECT ckey, duration, reason FROM [format_table_name("ban")] WHERE id = [banid]")
				query.Execute()

				var
					eckey = usr.ckey	//Editing admin ckey
				var
					pckey				//(banned) Player ckey
				var
				...
				switch(param)
					if("reason")
						...
						var
							DBQuery
								update_query = dbcon.NewQuery("UPDATE [format_table_name("ban")] SET reason = '[value]', edits = CONCAT(edits,'- [eckey] changed ban reason from <cite><b>\\\"[reason]\\\"</b></cite> to <cite><b>\\\"[value]\\\"</b></cite><BR>') WHERE id = [banid]")
					...
					if("duration")
						...
						var
							DBQuery
								update_query = dbcon.NewQuery("UPDATE [format_table_name("ban")] SET duration = [value], edits = CONCAT(edits,'- [eckey] changed ban duration from [duration] to [value]<br>'), expiration_time = DATE_ADD(bantime, INTERVAL [value] MINUTE) WHERE id = [banid]")
...
datum
	admins
		proc
			DB_ban_unban_by_id(var/id)

				if(!check_rights(R_BAN))	return

				var
					sql = "SELECT ckey FROM [format_table_name("ban")] WHERE id = [id]"
				...
				var
					unban_ckey = src.owner:ckey
				var
					unban_computerid = src.owner:computer_id
				var
					unban_ip = src.owner:address

				var
					sql_update = "UPDATE [format_table_name("ban")] SET unbanned = 1, unbanned_datetime = Now(), unbanned_ckey = '[unban_ckey]', unbanned_computerid = '[unban_computerid]', unbanned_ip = '[unban_ip]' WHERE id = [id]"
				message_admins("[key_name_admin(usr)] has lifted [pckey]'s ban.",1)

				var
					DBQuery
						query_update = dbcon.NewQuery(sql_update)
...
datum
	admins
		proc
			DB_ban_panel(var/playerckey = null, var/adminckey = null, var/playerip = null, var/playercid = null, var/dbbantype = null, var/match = null)
				...
				if(adminckey || playerckey || playerip || playercid || dbbantype)
					...
					if(adminckey || playerckey || playerip || playercid || dbbantype)
						...
						var
							blcolor = "#ffeeee" //banned light
						var
							bdcolor = "#ffdddd" //banned dark
						var
							ulcolor = "#eeffee" //unbanned light
						var
							udcolor = "#ddffdd" //unbanned dark
						...
						var
							DBQuery
								select_query = dbcon.NewQuery("SELECT id, bantime, bantype, reason, job, duration, expiration_time, ckey, a_ckey, unbanned, unbanned_ckey, unbanned_datetime, edits, ip, computerid FROM [format_table_name("ban")] WHERE 1 [playersearch] [adminsearch] [ipsearch] [cidsearch] [bantypesearch] ORDER BY bantime DESC LIMIT 100")
						...
						while(select_query.NextRow())
							...
							var
								ip = select_query.item[14]
							var
								cid = select_query.item[15]
