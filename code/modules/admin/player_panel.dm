...
datum
	admins
		proc
			check_antagonists_line(mob/M, caption = "", close = 1)
				var
					logout_status
				...
				return {"<tr><td><a href='?src=\ref[src];adminplayeropts=\ref[M]'>[M.real_name]</a><b>[caption]</b>[logout_status][M.stat == 2 ? " <b><font color=red>(DEAD)</font></b>" : ""]</td>
...
datum
	admins
		proc
			check_role_table(name, list/members, show_objectives=1)
				var
					txt = "<br><table cellspacing=5><tr><td><b>[name]</b></td><td></td></tr>"
...
datum
	admins
		proc
			check_role_table_row(mob/M, show_objectives)
				if(!istype(M))
					return "<tr><td><i>Not found!</i></td></tr>"

				var
					txt = check_antagonists_line(M, close = 0)
