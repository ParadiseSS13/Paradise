...
obj
	effect
		buildmode_line
			New(var/client/c, var/atom/atom_a, var/atom/atom_b, var/linename)
				name = linename
				loc = get_turf(atom_a)
				I = image('icons/misc/mark.dmi', src, "line", 19.0)
				var
					x_offset = ((atom_b.x * 32) + atom_b.pixel_x) - ((atom_a.x * 32) + atom_a.pixel_x)
				var
					y_offset = ((atom_b.y * 32) + atom_b.pixel_y) - ((atom_a.y * 32) + atom_a.pixel_y)
...
datum
	click_intercept
		proc
			InterceptClickOn(user,params,atom/object)
				return
			...
			proc
				change_settings(mob/user)
					switch(mode)
						...
						if(ADV_BUILDMODE)
							var
								target_path = input(user,"Enter typepath:" ,"Typepath","/obj/structure/closet")
						...
						if(VAR_BUILDMODE)
							var
								list
									locked = list("vars", "key", "ckey", "client", "firemut", "ishulk", "telekinesis", "xray", "virus", "viruses", "cuffed", "ka", "last_eaten", "urine")
							...
							if(varholder in locked && !check_rights(R_DEBUG,0))
								...
								return 1
							var
								thetype = input(user,"Select variable type:" ,"Type") in list("text","number","mob-reference","obj-reference","turf-reference")
							if(!thetype) return 1
							switch(thetype)
								if("text")
						...
						if(AREA_BUILDMODE)
							var
								list
									gen_paths = subtypesof(/datum/mapGenerator)

							var
								type = input(user,"Select Generator Type","Type") as null|anything in gen_paths
							if(!type) return
						...
						if(AREAEDIT_BUILDMODE)
							var
								target_path = input(user,"Enter typepath:", "Typepath", "/area")
							var
								areatype = text2path(target_path)
							if(ispath(areatype,/area))
								var
									areaname = input(user,"Enter area name:", "Area name", "Area")
...
datum
	click_intercept
		buildmode
			proc
				Reset()//Reset temporary variables
					deselect_region()
...
datum
	click_intercept
		buildmode
			InterceptClickOn(user,params,atom/object) //Click Intercept
				var
					list
						pa = params2list(params)
				var
									...
									var
										obj
											structure
												window
													reinforced
														WIN = new/obj/structure/window/reinforced(get_turf(object))
									WIN.dir = NORTH
								if(SOUTH)
									var
										obj
					...
					if(ADV_BUILDMODE)
						if(left_click)
							if(ispath(objholder,/turf))
								...
								var
									obj
										A = new objholder (get_turf(object))
								A.dir = build_dir
								...
								if(P.id_tag == M.id)
									var
										obj
											effect
												buildmode_line
													L = new(holder, M, P, "[M.name] to [P.name]")
									...
									var
										obj
											effect
												buildmode_line
													L2 = new(holder, P, M, "[M.name] to [P.name]") // Yes, reversed one so that you can see it from both sides.
									L2.color = L.color
									link_lines += L2
