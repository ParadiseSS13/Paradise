var
	list
		admin_ranks = list()								//list of all ranks with associated rights
...
proc
	load_admin_ranks()
		...
		var
			list
				Lines = file2list("config/admin_ranks.txt")
...
proc
	load_admins()
		...
		if(config.admin_legacy_system)
			...
			var
				DBQuery
					query = dbcon.NewQuery("SELECT ckey, rank, level, flags FROM [format_table_name("admin")]")
			query.Execute()
			while(query.NextRow())
				var
					ckey = query.item[1]
...
client
	verb
		changerank(newrank in admin_ranks)
			if(holder)
				holder.rank = newrank
