...
var
	appearanceban_runonce	//Updates legacy bans with new info
var
	appearance_keylist[0]	//to store the keys
...
proc
	appearance_loadbanfile()
		if(config.ban_legacy_system)
			var
				savefile
					S=new("data/appearance_full.ban")
			...
			var
				DBQuery
					query = dbcon.NewQuery("SELECT ckey FROM [format_table_name("ban")] WHERE bantype = 'APPEARANCE_BAN' AND NOT unbanned = 1")
			query.Execute()
...
/*
proc
	DB_ban_isappearancebanned(var/playerckey)
		establish_db_connection()
		if(!dbcon.IsConnected())
			return

		var
			sqlplayerckey = sql_sanitize_text(ckey(playerckey))

		var
			DBQuery
				query = dbcon.NewQuery("SELECT id FROM [format_table_name("ban")] WHERE CKEY = '[sqlplayerckey]' AND ((bantype = 'APPEARANCE_BAN') OR (bantype = 'APPEARANCE_TEMPBAN' AND expiration_time > Now())) AND unbanned != 1")
		query.Execute()
		while(query.NextRow())
			return 1
		return 0
