...
/datum/admins/proc/DB_ban_record(bantype, mob/banned_mob, duration = -1, reason, job = "", rounds = 0, banckey = null, banip = null, bancid = null)
	...
	if(!SSdbcore.IsConnected())
		to_chat(usr, span_boldannounce("Database connection failure when attempting to make DB ban. Please freeze them and write their ckey in notepad, so they can be banned when the DB returns."))
		return
...
/datum/admins/proc/DB_ban_unban(ckey, bantype, job = "")
	...
	if(!SSdbcore.IsConnected())
		to_chat(usr, span_boldannounce("Database connection failure when attempting to remove DB ban. Please remember to unban them at a later date!."))
		return
