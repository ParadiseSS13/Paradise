/obj/machinery/proc/machine_upgrade()
	set name = "\[Admin\] Tweak Component Ratings"
	set category = null

	if(!check_rights(R_DEBUG))
		return

	var/new_rating = input("Enter new rating:","Num") as num
	if(!isnull(new_rating) && component_parts)
		for(var/obj/item/stock_parts/P in component_parts)
			P.rating = new_rating
		RefreshParts()

		message_admins("[key_name_admin(usr)] has set the component rating of [src] to [new_rating]")
		log_admin("[key_name(usr)] has set the component rating of [src] to [new_rating]")

	SSblackbox.record_feedback("tally", "admin_verb", 1, "Machine Upgrade") //If you are copy-pasting this, ensure the 2nd parameter is unique to the new proc!
