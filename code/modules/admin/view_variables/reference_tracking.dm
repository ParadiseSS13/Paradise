...
/client/proc/view_refs(atom/D) //it actually supports datums as well but byond no likey
	...
	var/list/dat = list()
	dat += "<h1>References of [refid] - [D]</h1><br><a href='?_src_=vars;viewreferences=[refid]'>\[Refresh\]</a><hr>"
	...
	for(var/ref in backrefs)
		var/datum/R = ref
		if(istype(R))
			dat += "<tr><td><a href='?_src_=vars;RefVars=[R.UID()]'>[R.UID()]</td><td>[R.type]</td><td>[backrefs[R]]</td><td><a href='?_src_=vars;viewreferences=[R.UID()]'>\[Follow\]</a></td></tr>"
		else if(islist(R))
			dat += "<tr><td><a href='?_src_=vars;RefVars=\ref[R]'>\ref[R]</td><td>[R.type]</td><td>[backrefs[R]]</td><td><a href='?_src_=vars;viewreferences=\ref[R]'>\[Follow\]</a></td></tr>"
	dat += "</table><hr>"
	...
	for(var/ref in frontrefs)
		...
		if(istype(R))
			dat += "<tr><td>[ref]</td><td><a href='?_src_=vars;RefVars=[R.UID()]'>[R.UID()]</a></td><td>[R.type]</td><td><a href='?_src_=vars;viewreferences=[R.UID()]'>\[Follow\]</a></td></tr>"
		else if(islist(R))
			dat += "<tr><td>[ref]</td><td><a href='?_src_=vars;RefVars=\ref[R]'>\ref[R]</a></td><td>[R.type]</td><td><a href='?_src_=vars;viewreferences=\ref[R]'>\[Follow\]</a></td></tr>"
	dat += "</table><hr>"
	dat = dat.Join()
