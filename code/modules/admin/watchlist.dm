/client/proc/watchlist_add(target_ckey, browse = 0)
	...
	if(!target_ckey)
		...
		var/datum/DBQuery/query_watchfind = SSdbcore.NewQuery("SELECT ckey FROM [format_table_name("player")] WHERE ckey = '[new_ckey]'")
	...
	var/datum/DBQuery/query_watchadd = SSdbcore.NewQuery("INSERT INTO [format_table_name("watch")] (ckey, reason, adminckey, timestamp) VALUES ('[target_sql_ckey]', '[reason]', '[admin_sql_ckey]', '[timestamp]')")
...
/client/proc/watchlist_remove(target_ckey, browse = 0)
	if(!check_rights(R_ADMIN))
		return
	var/target_sql_ckey = sanitizeSQL(target_ckey)
	var/datum/DBQuery/query_watchdel = SSdbcore.NewQuery("DELETE FROM [format_table_name("watch")] WHERE ckey = '[target_sql_ckey]'")
...
/client/proc/watchlist_edit(target_ckey, browse = 0)
	if(!check_rights(R_ADMIN))
		return
	var/target_sql_ckey = sanitizeSQL(target_ckey)
	var/datum/DBQuery/query_watchreason = SSdbcore.NewQuery("SELECT reason FROM [format_table_name("watch")] WHERE ckey = '[target_sql_ckey]'")
	...
	if(query_watchreason.NextRow())
		...
		var/datum/DBQuery/query_watchupdate = SSdbcore.NewQuery("UPDATE [format_table_name("watch")] SET reason = '[new_reason]', last_editor = '[sql_ckey]', edits = CONCAT(IFNULL(edits,''),'[edit_text]') WHERE ckey = '[target_sql_ckey]'")
...
/client/proc/watchlist_show(search)
	...
	var/datum/DBQuery/query_watchlist = SSdbcore.NewQuery("SELECT ckey, reason, adminckey, timestamp, last_editor FROM [format_table_name("watch")] WHERE ckey REGEXP '[search]' ORDER BY ckey")
...
/client/proc/check_watchlist(target_ckey)
	if(!check_rights(R_ADMIN,0))
		return
	var/target_sql_ckey = sanitizeSQL(target_ckey)
	var/datum/DBQuery/query_watch = SSdbcore.NewQuery("SELECT reason FROM [format_table_name("watch")] WHERE ckey = '[target_sql_ckey]'")
