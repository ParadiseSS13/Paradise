client
	proc
		watchlist_add(target_ckey, browse = 0)
			if(!check_rights(R_ADMIN))
				return
			if(!target_ckey)
				var
					new_ckey = ckey(input(usr,"Who would you like to add to the watchlist?","Enter a ckey",null) as text)
				if(!new_ckey)
					return
				new_ckey = sanitizeSQL(new_ckey)
				var
					DBQuery
						query_watchfind = dbcon.NewQuery("SELECT ckey FROM [format_table_name("player")] WHERE ckey = '[new_ckey]'")
			...
			var
				DBQuery
					query_watchadd = dbcon.NewQuery("INSERT INTO [format_table_name("watch")] (ckey, reason, adminckey, timestamp) VALUES ('[target_sql_ckey]', '[reason]', '[admin_sql_ckey]', '[timestamp]')")
			if(!query_watchadd.Execute())
				var
					err = query_watchadd.ErrorMsg()
...
client
	proc
		watchlist_remove(target_ckey, browse = 0)
			if(!check_rights(R_ADMIN))
				return
			var
				target_sql_ckey = sanitizeSQL(target_ckey)
			var
				DBQuery
					query_watchdel = dbcon.NewQuery("DELETE FROM [format_table_name("watch")] WHERE ckey = '[target_sql_ckey]'")
...
client
	proc
		watchlist_edit(target_ckey, browse = 0)
			if(!check_rights(R_ADMIN))
				return
			var
				target_sql_ckey = sanitizeSQL(target_ckey)
			var
				DBQuery
					query_watchreason = dbcon.NewQuery("SELECT reason FROM [format_table_name("watch")] WHERE ckey = '[target_sql_ckey]'")
			...
			if(query_watchreason.NextRow())
				...
				var
					new_reason = input("Input the new reason", "New Reason", "[watch_reason]") as message|null
				...
				var
					edit_text = "Edited by [sql_ckey] on [SQLtime()] from \"[watch_reason]\" to \"[new_reason]\""
				edit_text = sanitizeSQL(edit_text)
				var
					DBQuery
						query_watchupdate = dbcon.NewQuery("UPDATE [format_table_name("watch")] SET reason = '[new_reason]', last_editor = '[sql_ckey]', edits = CONCAT(IFNULL(edits,''),'[edit_text]') WHERE ckey = '[target_sql_ckey]'")
				if(!query_watchupdate.Execute())
					var
						err = query_watchupdate.ErrorMsg()
...
client
	proc
		watchlist_show(search)
			...
			var
				DBQuery
					query_watchlist = dbcon.NewQuery("SELECT ckey, reason, adminckey, timestamp, last_editor FROM [format_table_name("watch")] WHERE ckey REGEXP '[search]' ORDER BY ckey")
...
client
	proc
		check_watchlist(target_ckey)
			...
			var
				DBQuery
					query_watch = dbcon.NewQuery("SELECT reason FROM [format_table_name("watch")] WHERE ckey = '[target_sql_ckey]'")
