				...
				switch(action)
					...
					if("add")
						...
						if(data["reason"])
							...
							var
								reason = input(usr,"Reason","Reason","Ban Evasion") as text|null
							if(!reason)
								return
							ban["message"] = "[reason]"
					...
					if("edit")
						...
						if(!ban)
							...
							return
						var
							oldreason = ban["message"]
						var
							reason = input(usr,"Reason","Reason","[ban["message"]]") as text|null
	...
	admins
		proc
			stickyban_show()
				...
				var
					list
						bans = sortList(world.GetConfig("ban"))
				var
					banhtml = ""
				for(var/key in bans)
					var
						ckey = ckey(key)
					var
						ban = stickyban2list(world.GetConfig("ban",key))
					banhtml += "<br /><hr />\n"
...
proc
	get_stickyban_from_ckey(var/ckey)
		if(!ckey)
			return null
		ckey = ckey(ckey)
		. = null
