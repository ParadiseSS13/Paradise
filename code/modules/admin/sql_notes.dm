proc
	add_note(target_ckey, notetext, timestamp, adminckey, logged = 1, server, checkrights = 1)
		...
		if(!target_ckey)
			var
				new_ckey = ckey(input(usr,"Who would you like to add a note for?","Enter a ckey",null) as text|null)
			if(!new_ckey)
				return
			new_ckey = sanitizeSQL(new_ckey)
			var
				DBQuery
					query_find_ckey = dbcon.NewQuery("SELECT ckey FROM [format_table_name("player")] WHERE ckey = '[new_ckey]'")
		...
		if(!adminckey)
			...
			if(!adminckey)
				return
		var
			admin_sql_ckey = sanitizeSQL(adminckey)
		if(!server)
		...
		var
			DBQuery
				query_noteadd = dbcon.NewQuery("INSERT INTO [format_table_name("notes")] (ckey, timestamp, notetext, adminckey, server) VALUES ('[target_sql_ckey]', '[timestamp]', '[notetext]', '[admin_sql_ckey]', '[server]')")
...
proc
	remove_note(note_id)
		...
		var
			DBQuery
				query_find_note_del = dbcon.NewQuery("SELECT ckey, notetext, adminckey FROM [format_table_name("notes")] WHERE id = [note_id]")
		...
		var
			DBQuery
				query_del_note = dbcon.NewQuery("DELETE FROM [format_table_name("notes")] WHERE id = [note_id]")
...
proc
	edit_note(note_id)
		...
		var
			target_ckey
		var
			sql_ckey = sanitizeSQL(usr.ckey)
		var
			DBQuery
				query_find_note_edit = dbcon.NewQuery("SELECT ckey, notetext, adminckey FROM [format_table_name("notes")] WHERE id = [note_id]")
		...
		if(query_find_note_edit.NextRow())
			...
			var
				new_note = input("Input new note", "New Note", "[old_note]") as message|null
			if(!new_note)
				return
			new_note = sanitizeSQL(new_note)
			var
				edit_text = "Edited by [sql_ckey] on [SQLtime()] from \"[old_note]\" to \"[new_note]\"<hr>"
			edit_text = sanitizeSQL(edit_text)
			var
				DBQuery
					query_update_note = dbcon.NewQuery("UPDATE [format_table_name("notes")] SET notetext = '[new_note]', last_editor = '[sql_ckey]', edits = CONCAT(IFNULL(edits,''),'[edit_text]') WHERE id = [note_id]")
...
proc
	show_note(target_ckey, index, linkless = 0)
		if(!check_rights(R_ADMIN|R_MOD))
			return
		var
			output
...
proc
	show_player_info_irc(var/key as text)
		var
			target_sql_ckey = sanitizeSQL(key)
		var
			DBQuery
				query_get_notes = dbcon.NewQuery("SELECT timestamp, notetext, adminckey, server FROM [format_table_name("notes")] WHERE ckey = '[target_sql_ckey]' ORDER BY timestamp")
		if(!query_get_notes.Execute())
			var
				err = query_get_notes.ErrorMsg()
			...
			return
		var
			output = " Info on [key]%0D%0A"
		while(query_get_notes.NextRow())
			var
				timestamp = query_get_notes.item[1]
			var
				notetext = query_get_notes.item[2]
			var
				adminckey = query_get_notes.item[3]
			var
				server = query_get_notes.item[4]
