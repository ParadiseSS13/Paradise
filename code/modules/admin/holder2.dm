...
/datum/admins/proc/associate(client/C, delay_2fa_complaint = FALSE)
	...
	if(needs_2fa() && istype(owner) && !owner.has_2fa())
		...
		if(!delay_2fa_complaint)
			...
			to_chat(owner,span_boldannounceooc("<big>You do not have 2FA enabled. Admin verbs will be unavailable until you have enabled 2FA.</big>")) // Very fucking obvious
