...
datum
	admins
		...
		var
			admincaster_screen = 0	//See newscaster.dm under machinery for a full description
		var
			datum
				feed_message
					admincaster_feed_message = new /datum/feed_message   //These two will act as holders.
		var
			datum
				feed_channel
					admincaster_feed_channel = new /datum/feed_channel
		var
			admincaster_signature	//What you'll sign the newsfeeds as

datum
	admins
		New(initial_rank = "Temporary Admin", initial_rights = 0, ckey)
			if(!ckey)
...
*/
proc
	check_rights(rights_required, show_msg=1, var/mob/user = usr)
		if(user && user.client)
			if(rights_required)
