...
proc
	jobban_loadbanfile()
		if(config.ban_legacy_system)
			...
			var
				DBQuery
					query = dbcon.NewQuery("SELECT ckey, job FROM [format_table_name("ban")] WHERE bantype = 'JOB_PERMABAN' AND isnull(unbanned)")
			query.Execute()
			...
			var
				DBQuery
					query1 = dbcon.NewQuery("SELECT ckey, job FROM [format_table_name("ban")] WHERE bantype = 'JOB_TEMPBAN' AND isnull(unbanned) AND expiration_time > Now()")
			query1.Execute()
