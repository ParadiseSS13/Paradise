	...
	proc
		admin_memo()
			...
			if(!dbcon.IsConnected())
				...
				return
			var
				memotask = input(usr,"Choose task.","Memo") in list("Show","Write","Edit","Remove")
			if(!memotask)
				return
...
client
	proc
		admin_memo_output(task, checkrights = 1, silent = 0)
			...
			if(!dbcon.IsConnected())
				...
				return
			var
				sql_ckey = sanitizeSQL(src.ckey)
			switch(task)
				if("Write")
					var
						DBQuery
							query_memocheck = dbcon.NewQuery("SELECT ckey FROM [format_table_name("memo")] WHERE ckey = '[sql_ckey]'")
					...
					if(query_memocheck.NextRow())
						...
						return
					var
						memotext = input(src,"Write your Memo","Memo") as message
					if(!memotext)
						return
					...
					var
						DBQuery
							query_memoadd = dbcon.NewQuery("INSERT INTO [format_table_name("memo")] (ckey, memotext, timestamp) VALUES ('[sql_ckey]', '[memotext]', '[timestamp]')")
					if(!query_memoadd.Execute())
						var
							err = query_memoadd.ErrorMsg()
				...
				if("Edit")
					var
						DBQuery
							query_memolist = dbcon.NewQuery("SELECT ckey FROM [format_table_name("memo")]")
					if(!query_memolist.Execute())
						var
							err = query_memolist.ErrorMsg()
						log_game("SQL ERROR obtaining ckey from memo table. Error : \[[err]\]\n")
						return
					var
						list
					...
					if(!memolist.len)
						...
						return
					var
						target_ckey = input(src, "Select whose memo to edit", "Select memo") as null|anything in memolist
					if(!target_ckey)
						return
					var
					...
					var
						DBQuery
							query_memofind = dbcon.NewQuery("SELECT memotext FROM [format_table_name("memo")] WHERE ckey = '[target_sql_ckey]'")
					if(!query_memofind.Execute())
						var
							err = query_memofind.ErrorMsg()
					...
					if(query_memofind.NextRow())
						...
						var
							new_memo = input("Input new memo", "New Memo", "[old_memo]", null) as message
						if(!new_memo)
							return
						new_memo = sanitizeSQL(new_memo)
						var
							edit_text = "Edited by [sql_ckey] on [SQLtime()] from<br>[old_memo]<br>to<br>[new_memo]<hr>"
						edit_text = sanitizeSQL(edit_text)
						var
							DBQuery
								update_query = dbcon.NewQuery("UPDATE [format_table_name("memo")] SET memotext = '[new_memo]', last_editor = '[sql_ckey]', edits = CONCAT(IFNULL(edits,''),'[edit_text]') WHERE ckey = '[target_sql_ckey]'")
						if(!update_query.Execute())
							var
								err = update_query.ErrorMsg()
				...
				if("Show")
					var
						DBQuery
							query_memoshow = dbcon.NewQuery("SELECT ckey, memotext, timestamp, last_editor FROM [format_table_name("memo")]")
					if(!query_memoshow.Execute())
						var
							err = query_memoshow.ErrorMsg()
					...
					while(query_memoshow.NextRow())
						...
						var
							memotext = query_memoshow.item[2]
						var
							timestamp = query_memoshow.item[3]
						var
				...
				if("Remove")
					var
						DBQuery
							query_memodellist = dbcon.NewQuery("SELECT ckey FROM [format_table_name("memo")]")
					if(!query_memodellist.Execute())
						var
							err = query_memodellist.ErrorMsg()
						log_game("SQL ERROR obtaining ckey from memo table. Error : \[[err]\]\n")
						return
					var
						list
					...
					while(query_memodellist.NextRow())
						var
							ckey = query_memodellist.item[1]
						memolist += "[ckey]"
					if(!memolist.len)
						to_chat(src, "No memos found in database.")
						return
					var
						target_ckey = input(src, "Select whose memo to delete", "Select memo") as null|anything in memolist
					if(!target_ckey)
						return
					var
						target_sql_ckey = sanitizeSQL(target_ckey)
					var
						DBQuery
							query_memodel = dbcon.NewQuery("DELETE FROM [format_table_name("memo")] WHERE ckey = '[target_sql_ckey]'")
					if(!query_memodel.Execute())
						var
							err = query_memodel.ErrorMsg()
