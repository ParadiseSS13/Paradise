...
//Regular jobs
	...
	//Player Notes
		...
		var/datum/DBQuery/query_noteedits = SSdbcore.NewQuery("SELECT edits FROM [format_table_name("notes")] WHERE id = '[note_id]'")
	...
	//Watchlist
		...
		var/datum/DBQuery/query_watchedits = SSdbcore.NewQuery("SELECT edits FROM [format_table_name("watch")] WHERE ckey = '[target_ckey]'")
		...
		var/datum/DBQuery/query_memoedits = SSdbcore.NewQuery("SELECT edits FROM [format_table_name("memo")] WHERE (ckey = '[sql_key]')")
		...
		switch(href_list["secretsfun"])
			if("sec_clothes")
				...
				SSblackbox.add_details("admin_secrets_fun_used","SC")
				for(var/obj/item/clothing/under/O in world)
					qdel(O)
				ok = 1
			if("sec_all_clothes")
				SSblackbox.inc("admin_secrets_fun_used",1)
				SSblackbox.add_details("admin_secrets_fun_used","SAC")
				for(var/obj/item/clothing/O in world)
					qdel(O)
				ok = 1
			if("sec_classic1")
				SSblackbox.inc("admin_secrets_fun_used",1)
				SSblackbox.add_details("admin_secrets_fun_used","SC1")
			...
			if("honksquad")
				if(usr.client.honksquad())
					SSblackbox.inc("admin_secrets_fun_used",1)
					SSblackbox.add_details("admin_secrets_fun_used","HONK")
			if("striketeam")
				if(usr.client.strike_team())
					SSblackbox.inc("admin_secrets_fun_used",1)
					SSblackbox.add_details("admin_secrets_fun_used","Strike")
			...
			if("infiltrators_syndicate")
				if(usr.client.syndicate_infiltration_team())
					SSblackbox.inc("admin_secrets_fun_used",1)
					SSblackbox.add_details("admin_secrets_fun_used","SyndieInfiltrationTeam")
			if("gimmickteam")
				if(usr.client.gimmick_team())
					SSblackbox.inc("admin_secrets_fun_used",1)
					SSblackbox.add_details("admin_secrets_fun_used","GimmickTeam")
			if("tripleAI")
			...
			if("gravity")
				...
				SSblackbox.inc("admin_secrets_fun_used",1)
				SSblackbox.add_details("admin_secrets_fun_used","Grav")
				if(gravity_is_on)
			...
			if("prisonwarp")
				if(!ticker)
					...
					return
				SSblackbox.inc("admin_secrets_fun_used",1)
				SSblackbox.add_details("admin_secrets_fun_used","PW")
...
//don't warp them if they aren't ready or are already there
			...
			if("flicklights")
				SSblackbox.inc("admin_secrets_fun_used",1)
				SSblackbox.add_details("admin_secrets_fun_used","FL")
				while(!usr.stat)
//knock yourself out to stop the ghosts
			...
			if("floorlava")
				SSblackbox.inc("admin_secrets_fun_used", 1)
				SSblackbox.add_details("admin_secrets_fun_used", "LF")
			...
			if("fakelava")
				SSblackbox.inc("admin_secrets_fun_used", 1)
				SSblackbox.add_details("admin_secrets_fun_used", "LZ")
				var/sure = alert(usr, "Are you sure you want to do this?", "Confirmation", "Yes", "No")
				if(sure == "No")
					return
			...
			if("weatherashstorm")
				SSblackbox.inc("admin_secrets_fun_used", 1)
				SSblackbox.add_details("admin_secrets_fun_used", "WA")
				var/sure = alert(usr, "Are you sure you want to do this?", "Confirmation", "Yes", "No")
				if(sure == "No")
					return
			...
			if("fakeguns")
				SSblackbox.inc("admin_secrets_fun_used",1)
				SSblackbox.add_details("admin_secrets_fun_used","FG")
...
//				message_admins("[key_name_admin(usr)] has triggered HIGHLANDER")
			...
			if("onlyoneteam")
				SSblackbox.inc("admin_secrets_fun_used",1)
				SSblackbox.add_details("admin_secrets_fun_used","OOT")
				usr.client.only_one_team()
//				message_admins("[key_name_admin(usr)] has triggered ")
			if("rolldice")
				SSblackbox.inc("admin_secrets_fun_used",1)
				SSblackbox.add_details("admin_secrets_fun_used","ROL")
	...
	// Library stuff
		...
		if(href_list["view_library_book"])
			var/datum/DBQuery/query_view_book = SSdbcore.NewQuery("SELECT content, title FROM [format_table_name("library")] WHERE id=[isbn]")
			...
			var/datum/DBQuery/query_unflag_book = SSdbcore.NewQuery("UPDATE [format_table_name("library")] SET flagged = 0 WHERE id=[isbn]")
