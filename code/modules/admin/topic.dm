...
//knock yourself out to stop the ghosts
					for(var/mob/M in GLOB.player_list)
						if(M.stat != 2 && prob(25))
							...
							if(AffectedArea.name != "Space" && AffectedArea.name != "Engine Walls" && AffectedArea.name != "Chemical Lab Test Chamber" && AffectedArea.name != "Escape Shuttle" && AffectedArea.name != "Arrival Area" && AffectedArea.name != "Arrival Shuttle" && AffectedArea.name != "start area" && AffectedArea.name != "Engine Combustion Chamber")
								...
								switch(Message)
									if(1)
										to_chat(M, "<span class='notice'>You shudder as if cold...</span>")
									if(2)
										to_chat(M, "<span class='notice'>You feel something gliding across your back...</span>")
									if(3)
										to_chat(M, "<span class='notice'>Your eyes twitch, you feel like something you can't see is here...</span>")
									if(4)
										to_chat(M, "<span class='notice'>You notice something moving out of the corner of your eye, but nothing is there...</span>")
				...
				for(var/mob/M in GLOB.player_list)
					if(M.stat != 2)
						to_chat(M, "<span class='notice'>The chilling wind suddenly stops...</span>")
			if("lightout")
