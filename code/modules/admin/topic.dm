	...
	admins
		Topic(href, href_list)
			...
			if(href_list["rejectadminhelp"])
				...
				var
					client
						C = locate(href_list["rejectadminhelp"])
				if(!C)
					return
			...
			if(href_list["makeAntag"])
				...
				var
					playerip = href_list["dbsearchip"]
				var
					playercid = href_list["dbsearchcid"]
				var
					dbbantype = text2num(href_list["dbsearchbantype"])
				var
					match = 0
				...
				var
					banid = text2num(href_list["dbbanid"])
				if(!banedit || !banid)
					return
				...
				var
					bantype = text2num(href_list["dbbanaddtype"])
				var
				...
				var
					banduration = text2num(href_list["dbbaddduration"])
				var
				...
				if(task == "add")
					var
						new_ckey = ckey(input(usr,"New admin's ckey","Admin ckey", null) as text|null)
					if(!new_ckey)	return
				...
				if(task == "remove")
					...
					var
						client
							C = directory[adm_ckey]						//find the client with the specified ckey (if they are logged in)
				...
				var
					timer = input("Enter new shuttle duration (seconds):","Edit Shuttle Timeleft", shuttle_master.emergency.timeLeft() ) as num
			...
			/////////////////////////////////////new ban stuff
				...
				var
					banfolder = href_list["unbane"]
				Banlist.cd = "/base/[banfolder]"
				var
					reason2 = Banlist["reason"]
				var
					temp = Banlist["temp"]

				var
					minutes = Banlist["minutes"]

				var
					banned_key = Banlist["key"]
				Banlist.cd = "/base"
			...
			/////////////////////////////////////new ban stuff
				...
				var
					mob
						M = locate(href_list["appearanceban"])
				if(!ismob(M))
					to_chat(usr, "This can only be used on instances of type /mob")
					return
				if(!M.ckey)	//sanity
				...
				var
					banreason = appearance_isbanned(M)
				if(banreason)
			...
			switch(alert("Reason: '[banreason]' Remove appearance ban?","Please Confirm","Yes","No"))
					...
					if("Yes")
						var
							reason = input(usr,"Please state the reason","Reason") as message|null
						if(!reason)
							return
...
//		if(!check_rights(R_BAN))	return
		...
		var
			dat = ""
		var
			header = "<head><title>Job-Ban Panel: [M.name]</title></head>"
		var
			body
		var
			jobs = ""
...
//Regular jobs
	...
	//Antagonist (Orange)
		var
			isbanned_dept = jobban_isbanned(M, "Syndicate")
		jobs += "<table cellpadding='1' cellspacing='0' width='100%'>"
	...
	//Player Notes
		...
		var
			target = href_list["nonalpha"]
		target = text2num(target)
		show_note(index = target)

	else if(href_list["shownoteckey"])
		var
			target_ckey = href_list["shownoteckey"]
		show_note(target_ckey)

	else if(href_list["notessearch"])
		var
			target = href_list["notessearch"]
		show_note(index = target)

	else if(href_list["noteedits"])
		var
			note_id = sanitizeSQL("[href_list["noteedits"]]")
		var
			DBQuery
				query_noteedits = dbcon.NewQuery("SELECT edits FROM [format_table_name("notes")] WHERE id = '[note_id]'")
	...
	//Watchlist
		...
		var
			target_ckey = locate(href_list["watchadd"])
		usr.client.watchlist_add(target_ckey)

	else if(href_list["watchremove"])
		var
			target_ckey = href_list["watchremove"]
		var
			confirm = alert("Are you sure you want to remove [target_ckey] from the watchlist?", "Confirm Watchlist Removal", "Yes", "No")
		if(confirm == "Yes")
			usr.client.watchlist_remove(target_ckey)

	else if(href_list["watchedit"])
		var
			target_ckey = href_list["watchedit"]
		...
		var
			target_ckey = sanitizeSQL("[href_list["watcheditlog"]]")
		var
			DBQuery
				query_watchedits = dbcon.NewQuery("SELECT edits FROM [format_table_name("watch")] WHERE ckey = '[target_ckey]'")
		if(!query_watchedits.Execute())
			var
				err = query_watchedits.ErrorMsg()
			log_game("SQL ERROR obtaining edits from watch table. Error : \[[err]\]\n")
			return
		if(query_watchedits.NextRow())
			var
				edit_log = query_watchedits.item[1]
		...
		var
			dat = {"<B>What mode do you wish to play?</B><HR>"}
		...
		var
			dat = {"<B>What game mode do you want to force secret to be? Use this if you want to change the game mode, but want the players to believe it's secret. This will only work if the current game mode is secret.</B><HR>"}
		for(var/mode in config.modes)
		...
		var
			mob
				M = locate(href_list["forcespeech"])
		if(!ismob(M))
			to_chat(usr, "this can only be used on instances of type /mob")

		var
			speech = input("What will [key_name(M)] say?.", "Force speech", "")// Don't need to sanitize, since it does that in say(), we also trust our admins.
		if(!speech)	return
		M.say(speech)
		...
		var
			mob
				M = locate(href_list["sendtoprison"])
		if(!ismob(M))
			to_chat(usr, "This can only be used on instances of type /mob")
			return
			...
			obj
				structure
					closet
						secure_closet
							brig
								locker = new /obj/structure/closet/secure_closet/brig(prison_cell)
		locker.opened = 0
		locker.locked = 1
		...
		var
			mob
				M = locate(href_list["sendbacktolobby"])
		...
		var
			mob
				M = locate(href_list["tdomeobserve"])
		if(!ismob(M))
			to_chat(usr, "This can only be used on instances of type /mob")
			return
		if(istype(M, /mob/living/silicon/ai))
		...
		var
			mob
				currentMob = locate(href_list["makemask"])
		...
		var
			mob
				dead
					observer
						G = locate(href_list["incarn_ghost"])
		if(!istype(G))
		...
		var
			mob
				living
					carbon
						human
							H = locate(href_list["togmutate"])
		if(!istype(H))
			to_chat(usr, "This can only be used on instances of type /mob/living/carbon/human")
			return
		var
			block=text2num(href_list["block"])
		...
		var
			mob
				M = locate(href_list["adminplayeropts"])
		show_player_panel(M)
		...
		var
			mob
				M = locate(href_list["adminplayerobservejump"])
		...
		var
			mob
				M = locate(href_list["adminplayerobservefollow"])
		...
		var
			x = text2num(href_list["X"])
		var
			y = text2num(href_list["Y"])
		var
			z = text2num(href_list["Z"])
		...
		var
			mob
				M = locate(href_list["adminmoreinfo"])
		if(!ismob(M))
			to_chat(usr, "This can only be used on instances of type /mob")
			return
		...
		var
			mob
				living
					carbon
						human
							H = locate(href_list["adminspawncookie"])
		if(!ishuman(H))
			to_chat(usr, "This can only be used on instances of type /mob/living/carbon/human")
			return
		...
		var
			mob
				living
					M = locate(href_list["BlueSpaceArtillery"])
		if(!isliving(M))
			to_chat(usr, "This can only be used on instances of type /mob/living")
			return
		...
		var
			mob
				living
					carbon
						human
							H = locate(href_list["CentcommReply"])
		if(!istype(H))
			to_chat(usr, "This can only be used on instances of type /mob/living/carbon/human")
			return
		...
		var
			input = input(src.owner, "Please enter a message to reply to [key_name(H)] via their headset.","Outgoing message from Centcomm", "")
		if(!input)	return
		...
		var
			mob
				living
					carbon
						human
							H = locate(href_list["EvilFax"])
		if(!istype(H))
			to_chat(usr, "This can only be used on instances of type /mob/living/carbon/human")
			return
		var
			etypes = list("borging","corgifying","firedeath","braindeath","honktumor","demotion")
		var
			eviltype = input(src.owner, "Which type of evil fax do you wish to send [H]?","Its good to be baaaad...", "") as null|anything in etypes
		if(!(eviltype in etypes))
			return
		var
			customname = input(src.owner, "Pick a title for the evil fax.", "Fax Title") as text|null
		if(!customname)
			customname = "paper"
		var
			obj
				item
					weapon
						paper
							evilfax
								P = new /obj/item/weapon/paper/evilfax(null)
		var
			obj
				machinery
					photocopier
						faxmachine
							fax = locate(href_list["originfax"])
		...
		var
			mob
				living
					carbon
						human
							H = locate(href_list["FaxReplyTemplate"])
		if(!istype(H))
			to_chat(usr, "This can only be used on instances of type /mob/living/carbon/human")
			return
		var
			obj
				item
					weapon
						paper
							P = new /obj/item/weapon/paper(null)
		var
			obj
				machinery
					photocopier
		...
		var
			stypes = list("handle it yourself","illegible","not signed","sorry, not right now","stop wasting our time")
		var
			stype = input(src.owner, "Which type of standard reply do you wish to send to [H]?","Choose your paperwork", "") as null|anything in stypes
		var
			tmsg = "<font face='Verdana' color='black'><center><img src = 'ntlogo.png'><BR><BR><BR><font size='4'><B>NanoTrasen Science Station Cyberiad</B></font><BR><BR><BR><font size='4'>NAS Trurl Communications Department Report</font></center><BR><BR>"
		...
		var
			mob
				living
					carbon
						human
							H = locate(href_list["SyndicateReply"])
		if(!istype(H))
			to_chat(usr, "This can only be used on instances of type /mob/living/carbon/human")
			return
		if(H.stat != 0)
		...
		if(!istype(H.l_ear, /obj/item/device/radio/headset) && !istype(H.r_ear, /obj/item/device/radio/headset))
			...
			return
		var
			input = input(src.owner, "Please enter a message to reply to [key_name(H)] via their headset.","Outgoing message from The Syndicate", "")
		if(!input)
			return
		to_chat(src.owner, "You sent [input] to [H] via a secure channel.")
		...
		var
			input = input(src.owner, "Please enter a message to reply to [key_name(H)] via their headset.","Outgoing message from HONKplanet", "")
		if(!input)	return
		...
		if(alert(src.owner, "Accept or Deny ERT request?", "CentComm Response", "Accept", "Deny") == "Deny")
			...
			var
				input = input(src.owner, "Please enter a reason for denying [key_name(H)]'s ERT request.","Outgoing message from CentComm", "")
			if(!input)	return
		...
		var
			obj
				item
					fax = locate(href_list["AdminFaxView"])
		...
		var
			page = text2num(href_list["AdminFaxViewPage"])
		var
			obj
				item
					weapon
						paper_bundle
							bundle = locate(href_list["paper_bundle"])
		...
		var
			mob
				sender = locate(href_list["AdminFaxCreate"])
		var
			obj
				machinery
					photocopier
						faxmachine
							fax = locate(href_list["originfax"])
		var
			faxtype = href_list["faxtype"]
		var
			reply_to = locate(href_list["replyto"])
		var
			destination
		var
			notify
		...
		var
			obj
				item
					weapon
						paper
							P = new /obj/item/weapon/paper(null) //hopefully the null loc won't cause trouble for us

		if(!fax)
			var
				list
					departmentoptions = alldepartments + "All Departments"
		...
		var
			input = input(src.owner, "Please enter a message to send a fax via secure connection. Use <br> for line breaks. Both pencode and HTML work.", "Outgoing message from Centcomm", "") as message|null
		if(!input)
			qdel(P)
			return
		...
		var
			customname = input(src.owner, "Pick a title for the fax.", "Fax Title") as text|null
		if(!customname)
			customname = "paper"
		...
		var
			mob
				M = locate(href_list["subtlemessage"])
		...
		var
			list
				offset = splittext(href_list["offset"],",")
		var
			number = dd_range(1, 100, text2num(href_list["object_count"]))
		var
			X = offset.len > 0 ? text2num(offset[1]) : 0
		var
			Y = offset.len > 1 ? text2num(offset[2]) : 0
		var
			Z = offset.len > 2 ? text2num(offset[3]) : 0
		var
			tmp_dir = href_list["object_dir"]
		var
			obj_dir = tmp_dir ? text2num(tmp_dir) : 2
		if(!obj_dir || !(obj_dir in list(1,2,4,8,5,6,9,10)))
			obj_dir = 2
		var
			obj_name = sanitize(href_list["object_name"])


		var
			atom
				target //Where the object will be spawned
		var
		...
		if(ticker && ticker.current_state == GAME_STATE_PLAYING)
			var
				afkonly = text2num(href_list["afkonly"])
			if(alert("Are you sure you want to kick all [afkonly ? "AFK" : ""] clients from the lobby?","Confirmation","Yes","Cancel") != "Yes")
				return
			var
				list
					listkicked = kick_clients_in_lobby("<span class='danger'>You were kicked from the lobby by an Administrator.</span>", afkonly)
		...
		if(!check_rights(R_SERVER)) return
		var
			sql_key = sanitizeSQL("[href_list["memoeditlist"]]")
		var
			DBQuery
				query_memoedits = dbcon.NewQuery("SELECT edits FROM [format_table_name("memo")] WHERE (ckey = '[sql_key]')")
		if(!query_memoedits.Execute())
			var
				err = query_memoedits.ErrorMsg()
			log_game("SQL ERROR obtaining edits from memo table. Error : \[[err]\]\n")
			return
		if(query_memoedits.NextRow())
			var
				edit_log = query_memoedits.item[1]
...
if("traitor_all")
				if(!ticker)
					...
					return
				var
					objective = sanitize(copytext(input("Enter an objective"),1,MAX_MESSAGE_LEN))
				if(!objective)
					return
				feedback_inc("admin_secrets_fun_used",1)
...
if("togglebombcap")
				...
				var
					newBombCap = input(usr,"What would you like the new bomb cap to be. (entered as the light damage range (the 3rd number in common (1,2,3) notation)) Must be between 4 and 128)", "New Bomb Cap", MAX_EX_LIGHT_RANGE) as num|null
				if(newBombCap < 4)
					return
				if(newBombCap > 128)
					newBombCap = 128
...
if("floorlava")
				...
				//Options
				var
					length = input(usr, "How long will the lava last? (in seconds)", "Length", 180) as num
				length = min(abs(length), 1200)

				var
					damage = input(usr, "How deadly will the lava be?", "Damage", 2) as num
				damage = min(abs(damage), 100)

				var
					sure = alert(usr, "Are you sure you want to do this?", "Confirmation", "YES!", "Nah")
				if(sure == "Nah")
					return
				floorIsLava = 1
...
//				message_admins("[key_name_admin(usr)] has triggered ")
			...
			if("tdomereset")
				var
					delete_mobs = alert("Clear all mobs?","Confirm","Yes","No","Cancel")
				if(delete_mobs == "Cancel")
					return

				var
					area
						thunderdome = locate(/area/tdome/arena)
				if(delete_mobs == "Yes")
				...
				var
					area
						template = locate(/area/tdome/arena_source)
			...
			if("tdomestart")
				var
					confirmation = alert("Start a Thunderdome match?","Confirm","Yes","No")
				if(confirmation == "No")
					return
...
if(usr)
		...
		switch(href_list["secretscoder"])
			...
			if("infinite_sec")
				var
					datum
						job
							J = job_master.GetJob("Security Officer")
				if(!J) return
				J.total_positions = -1
				J.spawn_positions = -1
		...
		if(src.admincaster_feed_channel.channel_name == "" || src.admincaster_feed_channel.channel_name == "\[REDACTED\]" || check )
			...
			var
				choice = alert("Please confirm Feed channel creation","Network Channel Handler","Confirm","Cancel")
			if(choice=="Confirm")
				var
					datum
						feed_channel
		...
		var
			input_param = text2num(href_list["ac_submit_wanted"])
		if(src.admincaster_feed_message.author == "" || src.admincaster_feed_message.body == "")
			src.admincaster_screen = 16
		else
			var
				choice = alert("Please confirm Wanted Issue [(input_param==1) ? ("creation.") : ("edit.")]","Network Security Handler","Confirm","Cancel")
			if(choice=="Confirm")
				if(input_param==1)          //If input_param == 1 we're submitting a new wanted issue. At 2 we're just editing an existing one. See the else below
					var
						datum
							feed_message
								WANTED = new /datum/feed_message
		...
		var
			choice = alert("Please confirm Wanted Issue removal","Network Security Handler","Confirm","Cancel")
		if(choice=="Confirm")
		...
		var
			datum
				feed_channel
					FC = locate(href_list["ac_censor_channel_author"])
		...
		var
			datum
				feed_message
					MSG = locate(href_list["ac_censor_channel_story_author"])
		...
		var
			datum
				feed_message
					MSG = locate(href_list["ac_censor_channel_story_body"])
		...
		var
			datum
				feed_channel
					FC = locate(href_list["ac_pick_d_notice"])
		...
		var
			datum
				feed_channel
					FC = locate(href_list["ac_toggle_d_notice"])
		...
		var
			datum
				feed_channel
					FC = locate(href_list["ac_show_channel"])
		src.admincaster_feed_channel = FC
		...
		var
			datum
				feed_channel
					FC = locate(href_list["ac_pick_censor_channel"])
		src.admincaster_feed_channel = FC
...
if(href_list["secretsmenu"])
		...
		var
			datum
				ErrorViewer
					error_viewer = locate(href_list["viewruntime"])
...
proc
	admin_jump_link(var/atom/target, var/source)
		if(!target) return
