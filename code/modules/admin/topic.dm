...
//Regular jobs
	...
	if(href_list["convert_ticket"])
		...
		var/stypes = list("Handle it yourselves!","Illegible fax","Fax not signed","Fax not stamped","Not Right Now","You are wasting our time", "Keep up the good work", "ERT Instructions")
		...
		else if(stype == "Fax not stamped")
			tmsg += "Greetings, esteemed crewmember. Your fax has been <b><I>DECLINED</I></b> automatically by NAS Trurl Fax Registration.<BR><BR>Your fax has not been stamped and, as such, it cannot be deemed official.<BR><BR>Please stamp your faxes before sending them so that we may verify their legitimacy.<BR><BR><i><small>This is an automatic message.</small>"
