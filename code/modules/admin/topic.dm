	...
	/*		if(!config.ban_legacy_system)
			...
			switch(alert("Reason: '[banreason]' Remove appearance ban?","Please Confirm","Yes","No"))
				if("Yes")
					...
					feedback_report("ban_appearance_unban")
			...
			if("Yes")
				...
				feedback_report("ban_appearance")
...
//Regular jobs
			...
			switch(alert("Temporary Ban?",,"Yes","No", "Cancel"))
				if("Yes")
					...
					for(var/job in notbannedlist)
						...
						feedback_report("ban_job_tmp")
						DB_ban_record(BANTYPE_JOB_TEMP, M, mins, reason, job)
						feedback_report("ban_job_tmp","- [job]")
				...
				if("No")
					...
					if(reason)
						...
						for(var/job in notbannedlist)
							...
							feedback_report("ban_job")
							DB_ban_record(BANTYPE_JOB_PERMA, M, -1, reason, job)
							feedback_report("ban_job","- [job]")
			...
			for(var/job in joblist)
				...
				switch(alert("Job: '[job]' Reason: '[reason]' Un-jobban?","Please Confirm","Yes","No"))
					if("Yes")
						...
						feedback_report("ban_job_unban")
						feedback_report("ban_job_unban","- [job]")
						jobban_unban(M, job)
						if(!msg)	msg = job
						else		msg += ", [job]"
					else
	...
	//Player Notes
		...
		switch(alert("Temporary Ban?",,"Yes","No", "Cancel"))
			if("Yes")
				...
				feedback_report("ban_tmp")
				DB_ban_record(BANTYPE_TEMP, M, mins, reason)
				feedback_report("ban_tmp_mins", amount = mins)
				if(config.banappeals)
					to_chat(M, "<span class='warning'>To try to resolve this matter head to [config.banappeals]</span>")
				else
					to_chat(M, "<span class='warning'>No ban appeals URL has been set.</span>")
	...
	//Watchlist
		...
		feedback_report("admin_cookies_spawned")
		...
		switch(href_list["secretsfun"])
			if("sec_clothes")
				feedback_report("admin_secrets_fun_used")
				feedback_report("admin_secrets_fun_used","SC")
				for(var/obj/item/clothing/under/O in world)
					qdel(O)
				ok = 1
			if("sec_all_clothes")
				feedback_report("admin_secrets_fun_used")
				feedback_report("admin_secrets_fun_used","SAC")
				for(var/obj/item/clothing/O in world)
					qdel(O)
				ok = 1
			if("sec_classic1")
				feedback_report("admin_secrets_fun_used")
				feedback_report("admin_secrets_fun_used","SC1")
			...
			if("monkey")
				feedback_report("admin_secrets_fun_used")
				feedback_report("admin_secrets_fun_used","M")
			...
			if("honksquad")
				if(usr.client.honksquad())
					feedback_report("admin_secrets_fun_used")
					feedback_report("admin_secrets_fun_used","HONK")
			if("striketeam")
				if(usr.client.strike_team())
					feedback_report("admin_secrets_fun_used")
					feedback_report("admin_secrets_fun_used","Strike")
			...
			if("infiltrators_syndicate")
				if(usr.client.syndicate_infiltration_team())
					feedback_report("admin_secrets_fun_used")
					feedback_report("admin_secrets_fun_used","SyndieInfiltrationTeam")
			if("gimmickteam")
				if(usr.client.gimmick_team())
					feedback_report("admin_secrets_fun_used")
					feedback_report("admin_secrets_fun_used","GimmickTeam")
			if("tripleAI")
				usr.client.triple_ai()
				feedback_report("admin_secrets_fun_used")
				feedback_report("admin_secrets_fun_used","TriAI")
			if("gravity")
				if(!(ticker && ticker.mode))
				...
				feedback_report("admin_secrets_fun_used")
				feedback_report("admin_secrets_fun_used","Grav")
				if(gravity_is_on)
			...
			if("prisonwarp")
				if(!ticker)
					...
					return
				feedback_report("admin_secrets_fun_used")
				feedback_report("admin_secrets_fun_used","PW")
...
//don't warp them if they aren't ready or are already there
			...
			if("traitor_all")
				...
				if(!objective)
					return
				feedback_report("admin_secrets_fun_used")
				feedback_report("admin_secrets_fun_used","TA([objective])")
			...
			if("togglebombcap")
				feedback_report("admin_secrets_fun_used")
				feedback_report("admin_secrets_fun_used","BC")
			...
			if("flicklights")
				feedback_report("admin_secrets_fun_used")
				feedback_report("admin_secrets_fun_used","FL")
				while(!usr.stat)
...
/*				if("shockwave")
			...
			if("lightout")
				feedback_report("admin_secrets_fun_used")
				feedback_report("admin_secrets_fun_used","LO")
			...
			if("blackout")
				feedback_report("admin_secrets_fun_used")
				feedback_report("admin_secrets_fun_used","BO")
				message_admins("[key_name_admin(usr)] broke all lights", 1)
				for(var/obj/machinery/light/L in machines)
					L.broken()
			if("whiteout")
				feedback_report("admin_secrets_fun_used")
				feedback_report("admin_secrets_fun_used","WO")
				message_admins("[key_name_admin(usr)] fixed all lights", 1)
				for(var/obj/machinery/light/L in machines)
					L.fix()
			if("floorlava")
				feedback_report("admin_secrets_fun_used")
				feedback_report("admin_secrets_fun_used", "LF")
			...
			if("fakelava")
				feedback_report("admin_secrets_fun_used")
				feedback_report("admin_secrets_fun_used", "LZ")
				var/sure = alert(usr, "Are you sure you want to do this?", "Confirmation", "Yes", "No")
				if(sure == "No")
					return
			...
			if("weatherashstorm")
				feedback_report("admin_secrets_fun_used")
				feedback_report("admin_secrets_fun_used", "WA")
				var/sure = alert(usr, "Are you sure you want to do this?", "Confirmation", "Yes", "No")
				if(sure == "No")
					return
			...
			if("weatherdarkness")
				feedback_report("admin_secrets_fun_used")
				feedback_report("admin_secrets_fun_used", "WD")
				var/sure = alert(usr, "Are you sure you want to do this?", "Confirmation", "Yes", "No")
				if(sure == "No")
					return
			...
			if("retardify")
				feedback_report("admin_secrets_fun_used")
				feedback_report("admin_secrets_fun_used","RET")
				for(var/mob/living/carbon/human/H in player_list)
			...
			if("fakeguns")
				feedback_report("admin_secrets_fun_used")
				feedback_report("admin_secrets_fun_used","FG")
			...
			if("schoolgirl")
				feedback_report("admin_secrets_fun_used")
				feedback_report("admin_secrets_fun_used","SG")
			...
			if("eagles")//SCRAW
				feedback_report("admin_secrets_fun_used")
				feedback_report("admin_secrets_fun_used","EgL")
			...
			if("onlyone")
				feedback_report("admin_secrets_fun_used")
				feedback_report("admin_secrets_fun_used","OO")
				usr.client.only_one()
//				message_admins("[key_name_admin(usr)] has triggered HIGHLANDER")
			if("onlyme")
				feedback_report("admin_secrets_fun_used")
				feedback_report("admin_secrets_fun_used","OM")
				usr.client.only_me()
			if("onlyoneteam")
				feedback_report("admin_secrets_fun_used")
				feedback_report("admin_secrets_fun_used","OOT")
				usr.client.only_one_team()
//				message_admins("[key_name_admin(usr)] has triggered ")
			if("rolldice")
				feedback_report("admin_secrets_fun_used")
				feedback_report("admin_secrets_fun_used","ROL")
			...
			if("magic")
				feedback_report("admin_secrets_fun_used")
				feedback_report("admin_secrets_fun_used","SM")
				usr.rightandwrong(1)
			if("tdomereset")
			...
			if("moveminingshuttle")
				feedback_report("admin_secrets_fun_used")
				feedback_report("admin_secrets_fun_used","ShM")
			...
			if("movelaborshuttle")
				feedback_report("admin_secrets_fun_used")
				feedback_report("admin_secrets_fun_used","ShL")
			...
			if("moveferry")
				feedback_report("admin_secrets_fun_used")
				feedback_report("admin_secrets_fun_used","ShF")
