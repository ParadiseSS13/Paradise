...
//				message_admins("[key_name_admin(usr)] has triggered ")
			...
			if("revolver")
				SSblackbox.record_feedback("tally", "admin_secrets_fun_used", 1, "Revovlers")
				usr.rightandwrong(FALSE, revolver_fight = TRUE)
			if("fakerevolver")
				SSblackbox.record_feedback("tally", "admin_secrets_fun_used", 1, "Fake Revovlers")
		...
		if(src.admincaster_feed_channel.channel_name == "" || src.admincaster_feed_channel.channel_name == "\[REDACTED\]" || check )
			...
			if(choice=="Confirm")
				...
				SSblackbox.record_feedback("text", "newscaster_channels", 1, newChannel.channel_name)
