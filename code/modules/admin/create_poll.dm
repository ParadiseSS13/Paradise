	...
	proc
		create_poll()
			...
			if(!dbcon.IsConnected())
				...
				return
			var
				returned = create_poll_function()
			if(returned)
				var
					DBQuery
						query_check_option = dbcon.NewQuery("SELECT id FROM [format_table_name("poll_option")] WHERE pollid = [returned]")
				...
				if(query_check_option.NextRow())
					var
						DBQuery
							query_log_get = dbcon.NewQuery("SELECT polltype, question, adminonly FROM [format_table_name("poll_question")] WHERE id = [returned]")
					if(!query_log_get.Execute())
						var
							err = query_log_get.ErrorMsg()
					...
					if(query_log_get.NextRow())
						...
						var
							adminonly = query_log_get.item[3]
					...
					var
						DBQuery
							query_del_poll = dbcon.NewQuery("DELETE FROM [format_table_name("poll_question")] WHERE id = [returned]")
					if(!query_del_poll.Execute())
						var
							err = query_del_poll.ErrorMsg()
	...
	proc
		create_poll_function()
			if(!check_rights(R_PERMISSIONS))	
				return
			var
				polltype = input("Choose poll type.","Poll Type") in list("Single Option","Text Reply","Rating","Multiple Choice")
			var
				choice_amount = 0
			switch(polltype)
				if("Single Option")
			...
			var
				starttime = SQLtime()
			var
				endtime = input("Set end time for poll as format YYYY-MM-DD HH:MM:SS. All times in server time. HH:MM:SS is optional and 24-hour. Must be later than starting time for obvious reasons.", "Set end time", SQLtime()) as text
			if(!endtime)
				return
			endtime = sanitizeSQL(endtime)
			var
				DBQuery
					query_validate_time = dbcon.NewQuery("SELECT STR_TO_DATE('[endtime]','%Y-%c-%d %T')")
			...
			var
				DBQuery
					query_time_later = dbcon.NewQuery("SELECT TIMESTAMP('[endtime]') < NOW()")
			if(!query_time_later.Execute())
				var
					err = query_time_later.ErrorMsg()
				log_game("SQL ERROR comparing endtime to NOW(). Error : \[[err]\]\n")
				return
			if(query_time_later.NextRow())
				var
					checklate = text2num(query_time_later.item[1])
				if(checklate)
			...
			var
				question = input("Write your question","Question") as message|null
			if(!question)
				return
			question = sanitizeSQL(question)
			var
				DBQuery
					query_polladd_question = dbcon.NewQuery("INSERT INTO [format_table_name("poll_question")] (polltype, starttime, endtime, question, adminonly, multiplechoiceoptions, createdby_ckey, createdby_ip) VALUES ('[polltype]', '[starttime]', '[endtime]', '[question]', '[adminonly]', '[choice_amount]', '[sql_ckey]', '[address]')")
			if(!query_polladd_question.Execute())
				var
					err = query_polladd_question.ErrorMsg()
			...
			var
				DBQuery
					query_get_id = dbcon.NewQuery("SELECT id FROM [format_table_name("poll_question")] WHERE question = '[question]' AND starttime = '[starttime]' AND endtime = '[endtime]' AND createdby_ckey = '[sql_ckey]' AND createdby_ip = '[address]'")
			if(!query_get_id.Execute())
				var
					err = query_get_id.ErrorMsg()
			...
			while(add_option)
				var
					option = input("Write your option",POLLTYPE_OPTION) as message|null
				if(!option)
					return pollid
				...
				var
					DBQuery
						query_polladd_option = dbcon.NewQuery("INSERT INTO [format_table_name("poll_option")] (pollid, text, percentagecalc, minval, maxval, descmin, descmid, descmax) VALUES ('[pollid]', '[option]', '[percentagecalc]', '[minval]', '[maxval]', '[descmin]', '[descmid]', '[descmax]')")
				if(!query_polladd_option.Execute())
					var
						err = query_polladd_option.ErrorMsg()
