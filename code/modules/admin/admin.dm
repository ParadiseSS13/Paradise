...
proc
	msg_admin_attack(var/text) //Toggleable Attack Messages
		log_attack(text)
		if(!nologevent)
			var
				rendered = "<span class=\"admin\"><span class=\"prefix\">ATTACK:</span> <span class=\"message\">[text]</span></span>"
			for(var/client/C in admins)
...
datum
	admins
		proc
			show_player_panel(var/mob/M in mob_list)
				set category = "Admin"
...
datum
	player_info
		var
			rank //rank of admin who made the notes
datum
	player_info
		var
...
datum
	admins
		proc
			show_player_notes(var/key as text)
				set category = "Admin"
	...
	admins
		proc
			announce()
				...
				var
					message = input("Global message to send:", "Admin Announce", null, null) as message|null
				if(message)
...
proc
	is_special_character(mob/M as mob) // returns 1 for specail characters and 2 for heroes of gamemode
...
datum
	admins
		proc
			show_traitor_panel(var/mob/M in mob_list)
				set category = "Admin"
...
client
	proc
		update_mob_sprite(mob/living/carbon/human/H as mob)
			set category = "Admin"
...
var
	gamma_ship_location = 1 // 0 = station , 1 = space
...
proc
	kick_clients_in_lobby(message, kick_only_afk = 0)
		var
			list
				kicked_client_names = list()
...
datum
	admins
		proc
			cmd_ghost_drag(var/mob/dead/observer/frommob, var/tothing)
				if(!istype(frommob))
				...
				if(istype(tothing, /obj/item))
					...
					var
						ask = alert("Are you sure you want to allow [frommob.name]([frommob.key]) to possess [toitem.name]?", "Place ghost in control of item?", "Yes", "No")
					if(ask != "Yes")
						return 1
				...
				if(isliving(tothing))
					...
					var
						ask = alert(question, "Place ghost in control of mob?", "Yes", "No")
					if(ask != "Yes")
						return 1
