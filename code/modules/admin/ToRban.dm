...
proc
	ToRban_update()
		spawn(0)
			...
			var
				http[] = world.Export("http://exitlist.torproject.org/exit-addresses")

			var
				list
					rawlist = file2list(http["CONTENT"])
			if(rawlist.len)
				fdel(TORFILE)
				var
					savefile
				...
				for( var/line in rawlist )
					...
					if( copytext(line,1,12) == "ExitAddress" )
						var
							cleaned = copytext(line,13,length(line)-19)
						if(!cleaned)	continue
						F[cleaned] << 1
...
client
	proc
		ToRban(task in list("update","toggle","show","remove","remove all","find"))
			set name = "ToRban"
			set category = "Server"
			if(!holder)	return
			switch(task)
				...
				if("remove")
					var
						savefile
							F = new(TORFILE)
					var
						choice = input(src,"Please select an IP address to remove from the ToR banlist:","Remove ToR ban",null) as null|anything in F.dir
					if(choice)
						F.dir.Remove(choice)
				...
				if("find")
					var
						input = input(src,"Please input an IP address to search for:","Find ToR ban",null) as null|text
					if(input)
