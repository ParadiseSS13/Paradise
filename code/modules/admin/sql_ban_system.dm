...
/proc/is_banned_from(player_ckey, list/roles)
	...
	if(player_client)
		...
		var/datum/db_query/query_check_ban = SSdbcore.NewQuery({"
			SELECT 1
			FROM [sqlfdbkdbutil].[format_table_name("ban")]
			WHERE
...
/proc/is_banned_from_with_details(player_ckey, player_ip, player_cid, role)
	...
	var/datum/db_query/query_check_ban = SSdbcore.NewQuery({"
		SELECT
			...
			applies_to_admins,
			reason,
			IFNULL((SELECT byond_key FROM [format_table_name("player")] WHERE [format_table_name("player")].ckey = [sqlfdbkdbutil].[format_table_name("ban")].ckey), ckey),
			ip,
			computerid,
			IFNULL((SELECT byond_key FROM [format_table_name("player")] WHERE [format_table_name("player")].ckey = [sqlfdbkdbutil].[format_table_name("ban")].a_ckey), a_ckey)
		FROM [sqlfdbkdbutil].[format_table_name("ban")]
		WHERE role = :role
...
/proc/build_ban_cache(client/player_client)
	...
	var/datum/db_query/query_build_ban_cache = SSdbcore.NewQuery(
		"SELECT role, applies_to_admins FROM [sqlfdbkdbutil].[format_table_name("ban")] WHERE ckey = :ckey AND unbanned_datetime IS NULL AND (expiration_time IS NULL OR expiration_time > NOW())",
		list("ckey" = ckey)
	)
		...
		var/datum/db_query/query_unban_search_bans = SSdbcore.NewQuery({"
			SELECT
				...
				IFNULL((
					...
					WHERE [format_table_name("player")].ckey = [sqlfdbkdbutil].[format_table_name("ban")].ckey
				), ckey),
				ip,
				computerid,
				IFNULL((
					SELECT byond_key
					FROM [format_table_name("player")]
					WHERE [format_table_name("player")].ckey = [sqlfdbkdbutil].[format_table_name("ban")].a_ckey
				), a_ckey),
				...
				IFNULL((
					SELECT byond_key
					FROM [format_table_name("player")]
					WHERE [format_table_name("player")].ckey = [sqlfdbkdbutil].[format_table_name("ban")].unbanned_ckey
				), unbanned_ckey),
				unbanned_round_id
			FROM [sqlfdbkdbutil].[format_table_name("ban")]
			WHERE
...
/datum/admins/proc/unban(ban_id, player_key, player_ip, player_cid, role, page, admin_key)
	...
	var/datum/db_query/query_unban = SSdbcore.NewQuery({"
		UPDATE [sqlfdbkdbutil].[format_table_name("ban")] SET
...
/datum/admins/proc/edit_ban(ban_id, player_key, ip_check, player_ip, cid_check, player_cid, use_last_connection, applies_to_admins, duration, interval, reason, mirror_edit, old_key, old_ip, old_cid, old_applies, admin_key, page, list/changes, is_server_ban)
	...
	if(player_ckey)
		var/datum/db_query/query_edit_ban_get_player = SSdbcore.NewQuery({"
			SELECT
				byond_key,
				(SELECT bantime FROM [sqlfdbkdbutil].[format_table_name("ban")] WHERE id = :ban_id),
				ip,
				computerid
			FROM [format_table_name("player")]
	...
	var/datum/db_query/query_edit_ban = SSdbcore.NewQuery({"
		UPDATE [sqlfdbkdbutil].[format_table_name("ban")]
		SET
...
/datum/admins/proc/ban_log(ban_id)
	...
	var/datum/db_query/query_get_ban_edits = SSdbcore.NewQuery({"
		SELECT edits FROM [sqlfdbkdbutil].[format_table_name("ban")] WHERE id = :ban_id
	"}, list("ban_id" = ban_id))
