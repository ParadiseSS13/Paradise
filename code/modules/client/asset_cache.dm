...
client
	var
		list
			cache = list() // List of all assets sent to this client by the asset cache.
	var
		list
			completed_asset_jobs = list() // List of all completed jobs, awaiting acknowledgement.
	var
		list
			sending = list()
	var
		last_asset_job = 0 // Last job done.

//This proc sends the asset to the client, but only if it needs it.
//This proc blocks(sleeps) unless verify is set to false
proc
	send_asset(var/client/client, var/asset_name, var/verify = TRUE)
		if(!istype(client))
			if(ismob(client))
				var
					mob
		...
		var
			t = 0
		var
			timeout_time = (ASSET_CACHE_SEND_TIMEOUT * client.sending.len) + ASSET_CACHE_SEND_TIMEOUT
...
//This proc blocks(sleeps) unless verify is set to false
proc
	send_asset_list(var/client/client, var/list/asset_list, var/verify = TRUE)
		if(!istype(client))
			if(ismob(client))
				var
					mob
		...
		var
			t = 0
		var
			timeout_time = ASSET_CACHE_SEND_TIMEOUT * client.sending.len
		while(client && !client.completed_asset_jobs.Find(job) && t < timeout_time) // Reception is handled in Topic()
			sleep(1) // Lock up the caller until this is received.
			t++
...
proc
	getFilesSlow(var/client/client, var/list/files, var/register_asset = TRUE)
...
proc
	register_asset(var/asset_name, var/asset)
