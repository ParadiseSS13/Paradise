...
//		to_chat(src, "ASSET JOB [href_list["asset_cache_confirm_arrival"]] ARRIVED.")
		var
			job = text2num(href_list["asset_cache_confirm_arrival"])
	...
	switch(href_list["karmashop"])
		...
		if("shop")
			...
			if(href_list["KarmaRefund"])
				var
					type = href_list["KarmaRefundType"]
				var
					job = href_list["KarmaRefund"]
				var
					cost = href_list["KarmaRefundCost"]
...
client
	proc
		handle_spam_prevention(var/message, var/mute_type)
...
/*	//Don't need this at the moment. But it's here if it's needed later.
	...
	var
		time_to_wait = fileaccess_timer - world.time
	if(time_to_wait > 0)
...
client
	proc
		log_client_to_db()
			...
			var
				sql_ckey = sql_sanitize_text(src.ckey)

			var
				DBQuery
					query = dbcon.NewQuery("SELECT id, datediff(Now(),firstseen) as age FROM [format_table_name("player")] WHERE ckey = '[sql_ckey]'")
			query.Execute()
			var
				sql_id = 0
			...
			var
				DBQuery
					query_ip = dbcon.NewQuery("SELECT ckey FROM [format_table_name("player")] WHERE ip = '[address]'")
			query_ip.Execute()
			...
			var
				DBQuery
					query_cid = dbcon.NewQuery("SELECT ckey FROM [format_table_name("player")] WHERE computerid = '[computer_id]'")
			query_cid.Execute()
			...
			var
				watchreason = check_watchlist(sql_ckey)
			if(watchreason)
			...
			var
				sql_ip = sql_sanitize_text(src.address)
			var
				sql_computerid = sql_sanitize_text(src.computer_id)
			var
				sql_admin_rank = sql_sanitize_text(admin_rank)
			...
			if(sql_id)
				...
				var
					DBQuery
						query_update = dbcon.NewQuery("UPDATE [format_table_name("player")] SET lastseen = Now(), ip = '[sql_ip]', computerid = '[sql_computerid]', lastadminrank = '[sql_admin_rank]' WHERE id = [sql_id]")
				query_update.Execute()
			else
				//New player!! Need to insert all the stuff
				var
					DBQuery
						query_insert = dbcon.NewQuery("INSERT INTO [format_table_name("player")] (id, ckey, firstseen, lastseen, ip, computerid, lastadminrank) VALUES (null, '[sql_ckey]', Now(), Now(), '[sql_ip]', '[sql_computerid]', '[sql_admin_rank]')")
			...
			var
				DBQuery
					query_accesslog = dbcon.NewQuery("INSERT INTO `[format_table_name("connection_log")]`(`id`,`datetime`,`serverip`,`ckey`,`ip`,`computerid`) VALUES(null,Now(),'[serverip]','[sql_ckey]','[sql_ip]','[sql_computerid]');")
