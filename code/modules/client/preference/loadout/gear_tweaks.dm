datum
	gear_tweak
		proc
			get_contents(var/metadata)
				return

datum
	gear_tweak
		proc
			get_metadata(var/user, var/metadata)
				return
...
datum
	gear_tweak
		proc
			tweak_gear_data(var/metadata, var/datum/gear_data)
				return

datum
	gear_tweak
		proc
			tweak_item(var/obj/item/I, var/metadata)
				return
...
datum
	gear_tweak
		path
			tweak_gear_data(var/metadata, var/datum/gear_data/gear_data)
...
datum
	gear_tweak
		contents
			get_metadata(var/user, var/list/metadata)
				. = list()
				for(var/i = metadata.len to valid_contents.len)
					metadata += "Random"
				for(var/i = 1 to valid_contents.len)
					var
						entry = input(user, "Choose an entry.", "Character Preference", metadata[i]) as null|anything in (valid_contents[i] + list("Random", "None"))
					if(entry)
						. += entry
					else
						return metadata
...
datum
	gear_tweak
		contents
			tweak_item(var/obj/item/I, var/list/metadata)
