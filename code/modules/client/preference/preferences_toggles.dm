...
client
	verb
		deadchat() // Deadchat toggle is usable by anyone.
	...
	verb
		togglemidis()
			...
			if(prefs.sound & SOUND_MIDI)
				...
				var
					sound
						break_sound = sound(null, repeat = 0, wait = 0, channel = 777)
...
client
	verb
		Toggle_Soundscape() //All new ambience should be added here so it works with this verb until someone better at things comes up with a fix that isn't awful
...
client
	verb
		Toggle_Buzz() //No more headaches because headphones bump up shipambience.ogg to insanity levels.
...
client
	verb
		Toggle_Heartbeat() //to toggle off heartbeat sounds, in case they get too annoying
