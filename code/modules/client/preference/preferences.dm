...
var
	list
		preferences_datums = list()

var
	global
		list
			special_role_times = list( //minimum age (in days) for accounts to play these roles
				ROLE_PAI = 0,
				ROLE_POSIBRAIN = 0,
				ROLE_GUARDIAN = 0,
				ROLE_TRAITOR = 7,
...
proc
	player_old_enough_antag(client/C, role)
		if(available_in_days_antag(C, role) == 0)
			return 1	//Available in 0 days = available right now = player is old enough to play.
		return 0

proc
	available_in_days_antag(client/C, role)
		if(!C)
			return 0
		if(!role)
			return 0
		...
		var
			minimal_player_age_antag = special_role_times[num2text(role)]
...
proc
	check_client_age(client/C, var/days) // If days isn't provided, returns the age of the client. If it is provided, it returns the days until the player_age is equal to or greater than the days variable
		if(!days)
			return C.player_age
		else
...
//	var/path
	var
		default_slot = 1				//Holder so it doesn't default to slot 1, rather the last one used
//	var/savefile_version = 0
	...
	//game-preferences
	var
		lastchangelog = ""				//Saved changlog filesize to detect if there was a change
	var
		ooccolor = "#b82e00"
	var
		be_special = list()				//Special role selection
	var
		UI_style = "Midnight"
	var
		nanoui_fancy = TRUE
	...
	var
		sound = SOUND_DEFAULT
	var
		show_ghostitem_attack = TRUE
	var
		UI_style_color = "#ffffff"
	var
		UI_style_alpha = 255
	...
	var
		real_name						//our character's name
	var
		be_random_name = 0				//whether we are a random name every round
	var
		gender = MALE					//gender of character (well duh)
	var
		age = 30						//age of character
	var
		spawnpoint = "Arrivals Shuttle" //where this character will spawn (0-2).
	var
		b_type = "A+"					//blood type (not-chooseable)
	var
		underwear = "Nude"					//underwear type
	var
		undershirt = "Nude"					//undershirt type
	var
		socks = "Nude"					//socks type
	var
		backbag = 2						//backpack type
	var
		ha_style = "None"				//Head accessory style
	var
		r_headacc = 0					//Head accessory colour
	var
		g_headacc = 0					//Head accessory colour
	var
		b_headacc = 0					//Head accessory colour
	var
		m_style = "None"				//Marking style
	var
		r_markings = 0					//Marking colour
	var
		g_markings = 0					//Marking colour
	var
		b_markings = 0					//Marking colour
	var
		h_style = "Bald"				//Hair type
	var
		r_hair = 0						//Hair color
	var
		g_hair = 0						//Hair color
	var
		b_hair = 0						//Hair color
	var
		f_style = "Shaved"				//Face hair type
	var
		r_facial = 0					//Face hair color
	var
		g_facial = 0					//Face hair color
	var
		b_facial = 0					//Face hair color
	var
		s_tone = 0						//Skin tone
	var
		r_skin = 0						//Skin color
	...
	var
		species = "Human"
	var
		language = "None"				//Secondary language

	var
		body_accessory = null

	var
		speciesprefs = 0//I hate having to do this, I really do (Using this for oldvox code, making names universal I guess
	...
	var
		list
			player_alt_titles = new()		// the default name of a job like "Medical Doctor"
//	var/accent = "en-us"
//	var/voice = "m1"
//	var/pitch = 50
//	var/talkspeed = 175
...
datum
	preferences
		New(client/C)
			...
			var
				loaded_preferences_successfully = load_preferences(C)
		...
		proc
			ShowChoices(mob/user)
				...
				switch(current_tab)
					if(TAB_CHAR) // Character Settings
						...
						if(species in list("Unathi", "Vulpkanin", "Tajaran", "Machine")) //Species that have head accessories.
							var
								headaccessoryname = "Head Accessory: "
							if(species == "Unathi")
						...
						if(jobban_isbanned(user, "Syndicate"))
							...
							for(var/i in special_roles)
								if(jobban_isbanned(user, i))
									...
									var
										available_in_days_antag = available_in_days_antag(user.client,i)
					...
					if(TAB_GEAR)
						...
						var
							datum
								loadout_category
									LC = loadout_categories[gear_tab]
				...
				var
					datum
						browser
							popup = new(user, "preferences", "<div align='center'>Character Setup</div>", 820, 640)
...
datum
	preferences
		proc
			get_gear_metadata(var/datum/gear/G)
				. = gear[G.display_name]
				if(!.)
					. = list()
					gear[G.display_name] = .
...
datum
	preferences
		proc
			get_tweak_metadata(var/datum/gear/G, var/datum/gear_tweak/tweak)
				var
					list
						metadata = get_gear_metadata(G)
				. = metadata["[tweak]"]
				if(!.)
					. = tweak.get_default()
					metadata["[tweak]"] = .
...
datum
	preferences
		proc
			set_tweak_metadata(var/datum/gear/G, var/datum/gear_tweak/tweak, var/new_metadata)
				var
					list
						metadata = get_gear_metadata(G)
...
datum
	preferences
		proc
			SetChoices(mob/user, limit = 12, list/splitJobs = list("Civilian","Research Director","AI","Bartender"), width = 760, height = 790)
				if(!job_master)
					return
				...
				for(var/datum/job/job in job_master.occupations)
					...
					var
						prefLevelColor = "pink"
					var
						prefUpperLevel = -1 // level to assign on left click
					var
						prefLowerLevel = -1 // level to assign on right click
...
//		user << browse(HTML, "window=mob_occupation;size=[width]x[height]")
	var
		datum
			browser
				popup = new(user, "mob_occupation", "<div align='center'>Occupation Preferences</div>", width, height)
...
datum
	preferences
		proc
			SetJobPreferenceLevel(var/datum/job/job, var/level)
				if(!job)
					return 0
...
datum
	preferences
		proc
			UpdateJobPreference(mob/user, role, desiredLvl)
				var
					datum
						job
							job = job_master.GetJob(role)
...
datum
	preferences
		proc
			ShowDisabilityState(mob/user,flag,label)
...
datum
	preferences
		proc
			GetPlayerAltTitle(datum/job/job)
...
datum
	preferences
		proc
			SetPlayerAltTitle(datum/job/job, new_title)
...
datum
	preferences
		proc
			GetJobDepartment(var/datum/job/job, var/level)
...
datum
	preferences
		proc
			SetJobDepartment(var/datum/job/job, var/level)
				if(!job || !level)	return 0
				switch(level)
...
datum
	preferences
		proc
			process_link(mob/user, list/href_list)
				if(!user)	return
				...
				if(href_list["preference"] == "job")
					switch(href_list["task"])
						...
						if("alt_title")
							var
								datum
									job
										job = locate(href_list["job"])
							if(job)
								var
									choices = list(job.title) + job.alt_titles
								var
									choice = input("Pick a title for [job.title].", "Character Generation", GetPlayerAltTitle(job)) as anything in choices | null
								if(choice)
					...
					switch(href_list["task"])
						...
						if("input")
							var
								dflag=text2num(href_list["disability"])
							if(dflag >= 0)
					...
					if(href_list["task"] == "med_record")
						var
							medmsg = input(usr,"Set your medical notes here.","Medical Records",html_decode(med_record)) as message
					...
					if(href_list["task"] == "sec_record")
						var
							secmsg = input(usr,"Set your security notes here.","Security Records",html_decode(sec_record)) as message
					...
					if(href_list["task"] == "gen_record")
						var
							genmsg = input(usr,"Set your employment notes here.","Employment Records",html_decode(gen_record)) as message
				...
				if(href_list["preference"] == "gear")
					if(href_list["toggle_gear"])
						var
							datum
								gear
									TG = gear_datums[href_list["toggle_gear"]]
						...
						var
							datum
								gear
									gear = gear_datums[href_list["gear"]]
						var
							datum
								gear_tweak
									tweak = locate(href_list["tweak"])
				...
				switch(href_list["task"])
					...
					if("input")
						switch(href_list["preference"])
							if("name")
								var
									raw_name = input(user, "Choose your character's name:", "Character Preference") as text|null
							...
							if("species")
								...
								var
									list
										new_species = list("Human", "Tajaran", "Skrell", "Unathi", "Diona", "Vulpkanin")
								var
...
//								alert(user, "You cannot change your species as you need to be whitelisted. If you wish to be whitelisted contact an admin in-game, on the forums, or on IRC.")
					...
					if(prev_species != species)
						...
						var
							list
								valid_facialhairstyles = list()
						for(var/facialhairstyle in facial_hair_styles_list)
							var
								datum
									sprite_accessory
										S = facial_hair_styles_list[facialhairstyle]
							if(gender == MALE && S.gender == FEMALE)
								continue
							if(gender == FEMALE && S.gender == MALE)
								continue
...
if("metadata")
					var
						new_metadata = input(user, "Enter any information you'd like others to see, such as Roleplay-preferences:", "Game Preference" , metadata)  as message|null
					if(new_metadata)
						metadata = sanitize(copytext(new_metadata,1,MAX_MESSAGE_LEN))

if("b_type")
					var
						new_b_type = input(user, "Choose your character's blood-type:", "Character Preference") as null|anything in list( "A+", "A-", "B+", "B-", "AB+", "AB-", "O+", "O-" )
					if(new_b_type)
						b_type = new_b_type
...
if("hair")
					if(species in list("Human", "Unathi", "Tajaran", "Skrell", "Machine", "Vulpkanin", "Vox"))
						var
							input = "Choose your character's hair colour:"
						var
							new_hair = input(user, input, "Character Preference", rgb(r_hair, g_hair, b_hair)) as color|null
						if(new_hair)
...
if("h_style")
					...
					for(var/hairstyle in hair_styles_list)
						...
						if(species == "Machine") //Species that can use prosthetic heads.
							...
							var
								datum
									robolimb
										robohead = all_robolimbs[H.model]
							if(species in S.species_allowed)
					...
					var
						new_h_style = input(user, "Choose your character's hair style:", "Character Preference") as null|anything in valid_hairstyles
					if(new_h_style)
						h_style = new_h_style
...
if("headaccessory")
					if(species in list("Unathi", "Vulpkanin", "Tajaran", "Machine")) //Species with head accessories.
						var
							input = "Choose the colour of your your character's head accessory:"
						var
							new_head_accessory = input(user, input, "Character Preference", rgb(r_headacc, g_headacc, b_headacc)) as color|null
...
if("ha_style")
					if(species in list("Unathi", "Vulpkanin", "Tajaran", "Machine")) //Species with head accessories.
						var
							list
								valid_head_accessory_styles = list()
						for(var/head_accessory_style in head_accessory_styles_list)
							var
								datum
									sprite_accessory
										H = head_accessory_styles_list[head_accessory_style]
						...
						var
							new_head_accessory_style = input(user, "Choose the style of your character's head accessory:", "Character Preference") as null|anything in valid_head_accessory_styles
...
if("markings")
					if(species in list("Unathi", "Vulpkanin", "Tajaran", "Machine")) //Species with markings.
						var
							input = "Choose the colour of your your character's markings:"
						var
							new_markings = input(user, input, "Character Preference", rgb(r_markings, g_markings, b_markings)) as color|null
						if(new_markings)
					...
					if(species in list("Unathi", "Vulpkanin", "Tajaran", "Machine")) //Species with markings.
						...
						for(var/markingstyle in marking_styles_list)
							var
								datum
									sprite_accessory
										M = marking_styles_list[markingstyle]
						...
						var
							new_marking_style = input(user, "Choose the style of your character's markings:", "Character Preference", m_style) as null|anything in valid_markings
...
if("body_accessory")
					var
						list
							possible_body_accessories = list()
					if(check_rights(R_ADMIN, 1, user))
						...
						for(var/B in body_accessory_by_name)
							var
								datum
									body_accessory
										accessory = body_accessory_by_name[B]
							if(!istype(accessory))
					...
					var
						new_body_accessory = input(user, "Choose your body accessory:", "Character Preference") as null|anything in possible_body_accessories
...
if("facial")
					if(species in list("Human", "Unathi", "Tajaran", "Skrell", "Machine", "Vulpkanin", "Vox"))
						var
							new_facial = input(user, "Choose your character's facial-hair colour:", "Character Preference", rgb(r_facial, g_facial, b_facial)) as color|null
						if(new_facial)
...
if("f_style")
					...
					var
						new_f_style = input(user, "Choose your character's facial-hair style:", "Character Preference")  as null|anything in valid_facialhairstyles
					if(new_f_style)
						f_style = new_f_style
...
if("underwear")
					...
					var
						new_underwear = input(user, "Choose your character's underwear:", "Character Preference")  as null|anything in underwear_options
					if(new_underwear)
...
if("socks")
					var
						list
							valid_sockstyles = list()
					for(var/sockstyle in socks_list)
						var
							datum
								sprite_accessory
									S = socks_list[sockstyle]
						if(gender == MALE && S.gender == FEMALE)
							continue
						if(gender == FEMALE && S.gender == MALE)
							continue
					...
					var
						new_socks = input(user, "Choose your character's socks:", "Character Preference")  as null|anything in valid_sockstyles
					ShowChoices(user)
					if(new_socks)
						socks = new_socks
...
if("eyes")
					var
						new_eyes = input(user, "Choose your character's eye colour:", "Character Preference", rgb(r_eyes, g_eyes, b_eyes)) as color|null
					if(new_eyes)
...
if("skin")
					if((species in list("Unathi", "Tajaran", "Skrell", "Slime People", "Vulpkanin", "Machine")) || body_accessory_by_species[species] || check_rights(R_ADMIN, 0, user))
						var
							new_skin = input(user, "Choose your character's skin colour: ", "Character Preference", rgb(r_skin, g_skin, b_skin)) as color|null
						if(new_skin)
...
if("ooccolor")
					var
						new_ooccolor = input(user, "Choose your OOC colour:", "Game Preference", ooccolor) as color|null
					if(new_ooccolor)
						ooccolor = new_ooccolor

if("bag")
					var
						new_backbag = input(user, "Choose your character's style of bag:", "Character Preference")  as null|anything in backbaglist
					if(new_backbag)
						backbag = backbaglist.Find(new_backbag)

if("nt_relation")
					var
						new_relation = input(user, "Choose your relation to NT. Note that this represents what others can find out about your character by researching your background, not what your character actually thinks.", "Character Preference")  as null|anything in list("Loyal", "Supportive", "Neutral", "Skeptical", "Opposed")
					if(new_relation)
						nanotrasen_relation = new_relation

if("flavor_text")
					var
						msg = input(usr,"Set the flavor text in your 'examine' verb. This can also be used for OOC notes and preferences!","Flavor Text",html_decode(flavor_text)) as message
...
if("limbs")
					var
						valid_limbs = list("Left Leg", "Right Leg", "Left Arm", "Right Arm", "Left Foot", "Right Foot", "Left Hand", "Right Hand")
					if(species == "Machine")
						valid_limbs = list("Torso", "Lower Body", "Head", "Left Leg", "Right Leg", "Left Arm", "Right Arm", "Left Foot", "Right Foot", "Left Hand", "Right Hand")
					var
						limb_name = input(user, "Which limb do you want to change?") as null|anything in valid_limbs
					if(!limb_name) return

					var
						limb = null
					var
						second_limb = null // if you try to change the arm, the hand should also change
					var
						third_limb = null  // if you try to unchange the hand, the arm should also change
					var
						valid_limb_states = list("Normal", "Amputated", "Prosthesis")
					var
						no_amputate = 0
					...
					var
						new_state = input(user, "What state do you wish the limb to be in?") as null|anything in valid_limb_states
					if(!new_state) return
...
if("organs")
					var
						organ_name = input(user, "Which internal function do you want to change?") as null|anything in list("Heart", "Eyes")
					...
					var
						new_state = input(user, "What state do you wish the organ to be in?") as null|anything in list("Normal","Assisted","Mechanical")
					if(!new_state) return
...
/*
				if("skin_style")
					var
						skin_style_name = input(user, "Select a new skin style") as null|anything in list("default1", "default2", "default3")
...
/*					if("spawnpoint")
					...
					for(var/S in spawntypes)
						spawnkeys += S
					var
						choice = input(user, "Where would you like to spawn when latejoining?") as null|anything in spawnkeys
			...
			switch(href_list["preference"])
				...
				if("UIcolor")
					var
						UI_style_color_new = input(user, "Choose your UI color, dark colors are not recommended!", UI_style_color) as color|null
				...
				if("UIalpha")
					var
						UI_style_alpha_new = input(user, "Select a new alpha(transparence) parameter for UI, between 50 and 255", UI_style_alpha) as num
...
datum
	preferences
		proc
			copy_to(mob/living/carbon/human/character)
				var
					datum
						species
				...
				if(config.humans_need_surnames)
					var
						firstspace = findtext(real_name, " ")
					var
						name_length = length(real_name)
						...
						var
							obj
								item
									organ
										internal
											I = character.get_int_organ_tag(name)
						if(I)
							if(status == "assisted")
								I.mechassist()
				...
				var
					obj
						item
							organ
								external
									l_foot = character.get_organ("l_foot")
				var
					obj
						item
							organ
								external
									r_foot = character.get_organ("r_foot")
							...
							structure
								stool
									bed
										chair
											wheelchair
												W = new /obj/structure/stool/bed/chair/wheelchair (character.loc)
					character.buckled = W
...
datum
	preferences
		proc
			open_load_dialog(mob/user)

				var
					DBQuery
						query = dbcon.NewQuery("SELECT slot,real_name FROM [format_table_name("characters")] WHERE ckey='[user.ckey]' ORDER BY slot")
...
//		user << browse(dat, "window=saves;size=300x390")
	var
		datum
			browser
				popup = new(user, "saves", "<div align='center'>Character Saves</div>", 300, 390)
	popup.set_content(dat)
	popup.open(0)
