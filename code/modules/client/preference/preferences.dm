	...
	switch(current_tab)
		...
		if(TAB_GEAR)
			...
			for(var/gear_name in LC.gear)
				...
				var/ticked = (G.type in loadout_gear)
				...
				if(ticked)
					...
					for(var/datum/gear_tweak/tweak in G.gear_tweaks)
						. += " <a href='?_src_=prefs;preference=gear;gear=[G.type];tweak=\ref[tweak]'>[tweak.get_contents(get_tweak_metadata(G, tweak))]</a>"
					. += "</td></tr>"
			dat += "</table>"
...
/**
  * Rebuilds the `loadout_gear` list, and returns the total end cost.
  *
  * Caches and cuts the existing [/datum/preferences/var/loadout_gear] list and remakes it, checking the `subtype_selection_cost` and overall cost validity of each item.
  *
  ...
  */
/datum/preferences/proc/setup_loadout(datum/gear/new_item)
	var/total_cost = 0
...
/datum/preferences/proc/process_link(mob/user, list/href_list)
	...
	if(href_list["preference"] == "gear")
		...
		else if(href_list["gear"] && href_list["tweak"]) // Unused
