datum
	preferences
		proc
			load_preferences(client/C)

				var
					DBQuery
						query = dbcon.NewQuery({"SELECT
										ooccolor,
										UI_style,
										UI_style_color,
										UI_style_alpha,
	...
	preferences
		proc
			save_preferences(client/C)
				...
				var
					DBQuery
						query = dbcon.NewQuery({"UPDATE [format_table_name("player")]
									SET
										ooccolor='[ooccolor]',
										UI_style='[UI_style]',
		...
		proc
			load_character(client/C,slot)
				...
				if(slot != default_slot)
					...
					var
						DBQuery
							firstquery = dbcon.NewQuery("UPDATE [format_table_name("player")] SET default_slot=[slot] WHERE ckey='[C.ckey]'")
					firstquery.Execute()
	...
	preferences
		proc
			save_character(client/C)
				...
				var
					DBQuery
						firstquery = dbcon.NewQuery("SELECT slot FROM [format_table_name("characters")] WHERE ckey='[C.ckey]' ORDER BY slot")
				firstquery.Execute()
				while(firstquery.NextRow())
					if(text2num(firstquery.item[1]) == default_slot)
						var
							DBQuery
								query = dbcon.NewQuery({"UPDATE [format_table_name("characters")] SET OOC_Notes='[sql_sanitize_text(metadata)]',
...
datum
	preferences
		proc
			random_character(client/C)
				var
					DBQuery
						query = dbcon.NewQuery("SELECT slot FROM [format_table_name("characters")] WHERE ckey='[C.ckey]' ORDER BY slot")
...
datum
	preferences
		proc
			SetChangelog(client/C,hash)
				lastchangelog=hash
				winset(C, "rpane.changelog", "background-color=none;font-style=")
				var
					DBQuery
						query = dbcon.NewQuery("UPDATE [format_table_name("player")] SET lastchangelog='[lastchangelog]' WHERE ckey='[C.ckey]'")
				if(!query.Execute())
					var
						err = query.ErrorMsg()
