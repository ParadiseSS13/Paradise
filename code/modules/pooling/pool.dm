...
/*
tg
	station13 /datum Pool:
---------------------------------
By RemieRichards

Creation
	Deletion is laggy, so let's reduce reuse and recycle!
...
proc
	PlaceInPool(datum/diver, destroy = 1)
		if(!istype(diver))
			return
...
var
	list
		exclude = list("animate_movement", "contents", "loc", "locs", "parent_type", "vars", "verbs", "type")
var
	list
		pooledvariables = list()
