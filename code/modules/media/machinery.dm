						...
						machinery
							media
								transmitter
									hooked = list()
			var
				exclusive_hook=null // Disables output to the room
...
obj
	machinery
		media
			proc
				hookMediaOutput(var/obj/machinery/media/transmitter/T, exclusive=0)
					if(exclusive)
						exclusive_hook=T
					hooked.Add(T)
					return 1
...
obj
	machinery
		media
			proc
				unhookMediaOutput(var/obj/machinery/media/transmitter/T)
					if(exclusive_hook==T)
						exclusive_hook=null
					hooked.Remove(T)
					return 1
...
obj
	machinery
		media
			setLoc(var/turf/T, var/teleported=0)
				disconnect_media_source()
				..(T)
				if(anchored)
					update_music()
