		...
		media
			jukebox
				process()
					if(!playlist)
						var
							url="[config.media_base_url]/index.php?playlist=[playlist_id]"
						...
						if(response)
							var
								json = file2text(response["CONTENT"])
							if("/>" in json)
