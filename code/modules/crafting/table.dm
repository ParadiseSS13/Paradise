...
obj
	structure
		table
			proc
				check_contents(datum/table_recipe/R)
					check_table()
					main_loop:
						if(R.fruit)
...
obj
	structure
		table
			proc
				check_tools(mob/user, datum/table_recipe/R)
					if(!R.tools.len)
						return 1
					var
						list
...
obj
	structure
		table
			proc
				construct_item(mob/user, datum/table_recipe/R)
					check_table()
...
obj
	structure
		table
			proc
				del_reqs(datum/table_recipe/R, atom/movable/resultobject)
					var
						list
							Deletion = list()
					var
		...
		table
			interact(mob/user)
				...
				var
					datum
						browser
							popup = new(user, "table", "Table", 300, 300)
