var
	global
		automation_types = subtypesof(/datum/automation)
...
datum
	automation
		proc
			unpackChildren(var/list/childList)
				. = list()
				if(childList.len>0)
...
datum
	automation
		proc
			packChildren(var/list/childList)
				. = list()
				if(childList.len>0)
...
datum
	automation
		Topic(href,href_list)
			...
			if(href_list["add"])
				var
					new_child=selectValidChildFor(usr)
			...
			if(href_list["remove"])
				if(href_list["remove"]=="*")
					...
					if(!A) return 1
					var
						confirm=alert("Are you sure you want to remove this automation?","Automations","Yes","No")
					if(confirm == "No") return 0
					A.OnRemove()
					children.Remove(A)
				parent.updateUsrDialog()
...
datum
	automation
		proc
			selectValidChildFor(var/mob/user, var/list/returntypes=valid_child_returntypes)
...
datum
	automation
		and
			...
			GetText()
				var
					out="AND (<a href=\"?src=\ref[src];add=1\">Add</a>)"
				if(children.len>0)
					out += "<ul>"
					for(var/datum/automation/stmt in children)
						out += {"<li>
...
datum
	automation
		or
			...
			GetText()
				var
					out="OR (<a href=\"?src=\ref[src];add=1\">Add</a>)"
				if(children.len>0)
					out += "<ul>"
					for(var/datum/automation/stmt in children)
						out += {"<li>
...
datum
	automation
		if_statement
			...
			var
				list
					valid_conditions=list(AUTOM_RT_NUM)
			...
			GetText()
				var
					out="<b>IF</b> (<a href=\"?src=\ref[src];set_condition=1\">SET</a>):<blockquote>"
				if(condition)
			...
			Topic(href,href_list)
				...
				if(href_list["remove"])
					if(href_list["remove"]=="*")
						var
							confirm=input("Are you sure you want to remove ALL automations?","Automations","No") in list("Yes","No")
						if(confirm == "No") return 0
						...
						if(!A) return 1
						var
							confirm=input("Are you sure you want to remove this automation?","Automations","No") in list("Yes","No")
						if(confirm == "No") return 0
						A.OnRemove()
				...
				if(href_list["set_condition"])
					var
						new_condition = selectValidChildFor(usr,valid_conditions)
	...
	automation
		compare
			...
			GetText()
				...
				var
					out = "<a href=\"?src=\ref[src];set_field=1\">(Set Left)</a> ("
				if(left==null)
					out += "-----"
				else
					out += left.GetText()
			...
			Topic(href,href_list)
				...
				if(href_list["set_field"])
					var
						idx = text2num(href_list["set_field"])
					var
						new_child = selectValidChildFor(usr)
					if(!new_child)
						return 1
