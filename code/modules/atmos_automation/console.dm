		...
		computer
			general_air_control
				atmos_automation
					...
					proc
						request_device_refresh(var/device)
							send_signal(list("tag"=device, "status"))

					proc
						send_signal(var/list/data, var/filter = RADIO_ATMOSIA)//filter's here so the AAC can cross communicate to things like vents, which have a different filter
							var
								datum
									signal
										signal = new
					...
					proc
						selectValidChildFor(var/datum/automation/parent, var/mob/user, var/list/valid_returntypes)
							var
								list
									choices=list()
							...
							if(choices.len==0)
								...
								return 0
							var
								label=input(user, "Select new automation:", "Automations", "Cancel") as null|anything in choices
							if(!label)
								return 0
					...
					Topic(href,href_list)
						...
						if(href_list["add"])
							var
								new_child=selectValidChildFor(null,usr,list(0))
							if(!new_child)
								return 1
						...
						if(href_list["label"])
							var
								datum
									automation
										A=locate(href_list["label"])
							if(!A) return 1
							var
								nl=input(usr, "Please enter a label for this automation task.") as text|null
							if(!nl) return 1
						...
						if(href_list["remove"])
							if(href_list["remove"]=="*")
								var
									confirm=alert("Are you sure you want to remove ALL automations?","Automations","Yes","No")
						...
						if(href_list["read"])
							var
								code = input("Input exported AAC code.","Automations","") as message|null
							if(!code) return 0
							ReadCode(code)
							updateUsrDialog()
							return 1
					...
					proc
						MakeCompare(var/datum/automation/a, var/datum/automation/b, var/comparetype)
							var
								datum
									automation
										compare
					...
					proc
						MakeGetSensorData(var/sns_tag,var/field)
							var
								datum
									automation
										get_sensor_data
		...
		computer
			general_air_control
				atmos_automation
					air_mixing
						var
							n2_injector_tag="air_n2_in"
						var
							o2_injector_tag="air_o2_in"
						var
							output_tag="air_out"
						var
