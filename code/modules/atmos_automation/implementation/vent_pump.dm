	...
	automation
		set_vent_pump_mode
			...
			var
				mode = "stabilize"
			var
				vent_type = 0//0 for unary vents, 1 for DP vents

			var
				list
					modes = list("stabilize","purge")
			...
			process()
				if(vent_pump)
					var
						dirvalue = (mode == "stabilize" ? 1 : mode == "purge" ? 0 : 1)
	...
	automation
		set_vent_pump_power
			...
			var
				state = 0
			var
				mode = 0//0 for unary vents, 1 for DP vents.
...
datum
	automation
		set_vent_pump_pressure//controls the internal/external pressure bounds of a vent pump.
			name = "Vent Pump: Pressure Settings"

			var
				vent_pump = null
			var
				intpressureout = 0//these 2 are for DP vents, if it's a unary vent you're sending to it will take intpressureout as var
			var
				intpressurein = 0
			var
				extpressure = 0
			...
			Topic(href, href_list)
				...
				if(href_list["set_intpressure_out"])
					var
						response = input("Set new pressure, in kPa. \[0-[50*ONE_ATMOSPHERE]\]") as num
				...
				if(href_list["set_external"])
					var
						response = input(usr,"Set new pressure, in kPa. \[0-[50*ONE_ATMOSPHERE]\]") as num
	...
	automation
		set_vent_pressure_checks
			...
			var
				checks = 1
			var
				mode = 0//1 for DP vent, 0 for unary vent
/*
checks bitflags
1 = external
...
*/
	...
	Topic(href, href_list)
		...
		if(href_list["togglecheck"])
			var
				bitflagvalue = text2num(href_list["togglecheck"])
			if(mode)
