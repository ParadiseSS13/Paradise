...
Gunshots
	explosions
		opening doors/less rare audio (done)
		...
		carbon
			proc
				handle_hallucinations()
					...
					var
						list
							minor = list("sounds"=20,"bolts_minor"=10,"whispers"=15,"message"=5)
					//Something's wrong here
					var
						list
							medium = list("hudscrew"=15,"items"=15,"dangerflash"=15,"bolts"=10,"flood"=10,"husks"=10,"battle"=10)
					//AAAAH
					var
						list
							major = list("fake"=10,"death"=5,"xeno"=10,"singulo"=10,"delusion"=10)
...
obj
	effect
		hallucination
			simple
				New(loc,var/mob/living/carbon/T)
					target = T
			...
			simple
				proc
					GetImage()
						var
							image
								I = image(image_icon,loc,image_state,image_layer,dir=src.dir)
						I.pixel_x = px
						I.pixel_y = py
						if(col_mod)
							I.color = col_mod
...
obj
	effect
		hallucination
			simple
				update_icon(var/new_state,var/new_icon,var/new_px=0,var/new_py=0)
				...
				var
					list
						turf
							flood_turfs = list()
				var
					image_icon = 'icons/effects/tile_effects.dmi'
				var
					image_state = "plasma"
				var
					radius = 0
...
obj
	effect
		hallucination
			simple
				xeno
					throw_at(atom/target, range, speed) // TODO : Make diagonal trhow into proc/property
						...
						while(target && (((((dist_x > dist_y) && ((src.x < target.x) || (src.x > target.x))) || ((dist_x <= dist_y) && ((src.y < target.y) || (src.y > target.y))) || (src.x > target.x)) && dist_travelled < range) || !has_gravity(src)))
							...
							var
								atom
									step = get_step(src, get_dir(src,target))
							if(!step)
								break
				...
				New(loc,var/mob/living/carbon/T)
					...
					var
						turf
							start = T.loc
					var
						screen_border = pick(SOUTH,EAST,WEST,NORTH)
					for(var/i = 0,i<11,i++)
...
obj
	effect
		hallucination
			delusion
				New(loc, mob/living/carbon/T, force_kind = null, duration = 300,skip_nearby = 1, custom_icon = null, custom_icon_file = null)
					target = T
					var
						image
							A = null
					var
						kind = force_kind ? force_kind : pick("clown", "corgi", "carp", "skeleton", "demon")
				...
				New(loc,var/mob/living/carbon/T)
					...
					var
						obj
							effect
								fake_attacker
									F = new/obj/effect/fake_attacker(get_turf(target),target)
					if(clone.l_hand)
...
obj
	effect
		fake_attacker
			attackby(var/obj/item/weapon/P as obj, mob/living/user as mob, params)
				...
				fake_blood(var/mob/target)
					var
						obj
							effect
								overlay
									O = new/obj/effect/overlay(target.loc)
					O.name = "blood"
					var
						image
							I = image('icons/effects/blood.dmi',O,"floor[rand(1,7)]",O.dir,1)
					target << I
					spawn(300)
						qdel(O)
					return
...
var
	list
		non_fakeattack_weapons = list(/obj/item/weapon/gun/projectile, /obj/item/ammo_box/a357,\
			obj
				item
					weapon
						gun
			...
			obj
				item
					weapon
						storage
							box
								syndicate, /obj/item/weapon/storage/box/emps,\
			obj
				item
					weapon
						cartridge
							syndicate, /obj/item/clothing/under/chameleon,\
			obj
				item
					clothing
						shoes
							syndigaloshes, /obj/item/weapon/card/id/syndicate,\
			obj
				item
					clothing
						mask
							gas
								voice, /obj/item/clothing/glasses/thermal,\
			obj
				item
					device
						chameleon, /obj/item/weapon/card/emag,\
			obj
				item
					weapon
						storage
							toolbox
								syndicate, /obj/item/weapon/aiModule,\
			obj
				item
					device
						radio
							headset
								syndicate,	/obj/item/weapon/c4,\
			obj
				item
					device
						powersink, /obj/item/weapon/storage/box/syndie_kit,\
			obj
				item
					toy
						syndicateballoon, /obj/item/weapon/gun/energy/laser/captain,\
			obj
				item
					weapon
						hand_tele, /obj/item/weapon/rcd, /obj/item/weapon/tank/jetpack,\
			obj
				item
					clothing
						under
							rank
								captain, /obj/item/device/aicard,\
			obj
				item
					clothing
						shoes
							magboots, /obj/item/areaeditor/blueprints, /obj/item/weapon/disk/nuclear,\
			obj
				item
					clothing
						suit
							space
								nasavoid, /obj/item/weapon/tank)
...
obj
	effect
		hallucination
			bolts
				New(loc,var/mob/living/carbon/T,var/door_number=-1) //-1 for sever 1-2 for subtle
					target = T
					var
						image
							I = null
		...
		hallucination
			whispers
				New(loc,var/mob/living/carbon/T)
					target = T
					var
						speak_messages = list("I'm watching you...","[target.name]!","Go away!","Kchck-Chkck? Kchchck!","Did you hear that?","What did you do ?","Why?","Give me that!","Honk!","HELP!!")
					var
						radio_messages = list("Xenos!","Singularity loose!","They are arming the nuke!","They butchered Ian!","H-help!","[pick(teleportlocs)]!!","Where's [target.name]?","Call the shuttle!")
		...
		hallucination
			message
				New(loc,var/mob/living/carbon/T)
					target = T
					var
						chosen = pick("<span class='userdanger'>The light burns you!</span>", \
...
mob
	living
		carbon
			proc
				hallucinate(var/hal_type) // Todo -> proc / defines
					switch(hal_type)
						if("xeno")
...
//			to_chat(src, "Traitor Items")
			if(!halitem)
				...
				var
					list
						slots_free = list(ui_lhand,ui_rhand)
				...
				if(possible_points.len)
					...
					switch(rand(1,4))
						if(1)
							var
								image
									body = image('icons/mob/human.dmi',target,"husk_s",TURF_LAYER)
							var
								matrix
									M = matrix()
							M.Turn(90)
