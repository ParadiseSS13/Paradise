...
obj
	proc
		get_card_account(var/obj/item/weapon/card/I, var/mob/user=null, var/terminal_name="", var/transaction_purpose="", var/require_pin=0)
			if(terminal_name=="")
			...
			if(istype(I, /obj/item/weapon/card/id))
				...
				var
					datum
						money_account
							D = get_money_account(C.associated_account_number)
				if(require_pin && user)
...
mob
	proc
		get_worn_id_account(var/require_pin=0, var/mob/user=null)
			if(ishuman(src))
				var
					mob
						living
				...
				var
					datum
						money_account
							D = get_money_account(I.associated_account_number)
				if(require_pin && user)
					attempt_pin = input(user,"Enter pin code", "Transaction") as num
					if(D.remote_access_pin != attempt_pin)
						return null
...
datum
	money_account
		proc
			charge(var/transaction_amount,var/datum/money_account/dest,var/transaction_purpose, var/terminal_name="", var/terminal_id=0, var/dest_name = "UNKNOWN")
				if(suspended)
