...
obj
	machinery
		computer
			account_database
				proc
					create_transation(target, reason, amount)
						var
							datum
								transaction
									T = new()
...
obj
	machinery
		computer
			account_database
				proc
					accounting_letterhead(report_name)
						return {"
			...
			account_database
				Topic(href, href_list)
					...
					var
						datum
							nanoui
								ui = nanomanager.get_open_ui(usr, src, "main")
					...
					if(href_list["choice"])
						switch(href_list["choice"])
							...
							if("add_funds")
								var
									amount = input("Enter the amount you wish to add", "Silently add funds") as num
								if(detailed_account_view)
									detailed_account_view.money += amount

							if("remove_funds")
								var
									amount = input("Enter the amount you wish to remove", "Silently remove funds") as num
								if(detailed_account_view)
									detailed_account_view.money -= amount
							...
							if("finalise_create_account")
								...
								if(starting_funds > 0)
									...
									var
										trx = create_transation(account_name, "New account activation", "([starting_funds])")
							...
							if("revoke_payroll")
								var
									funds = detailed_account_view.money
								var
									account_trx = create_transation(station_account.owner_name, "Revoke payroll", "([funds])")
								var
									station_trx = create_transation(detailed_account_view.owner_name, "Revoke payroll", funds)
							...
							if("print")
								...
								if(detailed_account_view)
									...
									var
										title = "Account #[detailed_account_view.account_number] Details"
									text = {"
