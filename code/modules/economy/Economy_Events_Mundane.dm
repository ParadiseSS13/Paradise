		...
		mundane_news
			announce()
				var
					datum
						trade_destination
							affected_dest = pickweight(weighted_mundaneevent_locations)
				var
					event_type = 0
				...
				if(!newMsg.body)
					...
					switch(event_type)
						...
						if(RESIGNATION)
							...
							if(prob(25))
								var
									locstring = pick("Segunda","Salusa","Cepheus","Andromeda","Gruis","Corona","Aquila","Asellus") + " " + pick("I","II","III","IV","V","VI","VII","VIII")
		...
		trivial_news
			announce()
				...
				var
					datum
						trade_destination
							affected_dest = pick(weighted_mundaneevent_locations)
