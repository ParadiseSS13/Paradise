		...
		device
			eftpos
				...
				var
					machine_id = ""
				var
					eftpos_name = "Default EFTPOS scanner"
				var
				...
				var
					transaction_purpose = "Default charge"
				var
					access_code = 0
				var
					obj
...
obj
	item
		device
			eftpos
				attackby(O as obj, user as mob, params)
				...
				Topic(var/href, var/href_list)
					if(href_list["choice"])
						switch(href_list["choice"])
							if("change_code")
								var
									attempt_code = input("Re-enter the current EFTPOS access code", "Confirm old EFTPOS code") as num
								if(attempt_code == access_code)
									var
										trycode = input("Enter a new access code for this device (4-6 digits, numbers only)", "Enter new EFTPOS code") as num
							...
							if("change_id")
								var
									attempt_code = text2num(input("Re-enter the current EFTPOS access code", "Confirm EFTPOS code"))
								if(attempt_code == access_code)
							...
							if("link_account")
								...
								if(linked_db)
									var
										attempt_account_num = input("Enter account number to pay EFTPOS charges into", "New account number") as num
									var
										attempt_pin = input("Enter pin code", "Account pin") as num
									linked_account = attempt_account_access(attempt_account_num, attempt_pin, 1)
								else
									to_chat(usr, "[bicon(src)]<span class='warning'>Unable to connect to accounts database.</span>")
							if("trans_purpose")
								var
									purpose = input("Enter reason for EFTPOS transaction", "Transaction purpose") as text|null
								if(purpose)
									transaction_purpose = purpose
							if("trans_value")
								var
									try_num = input("Enter amount for EFTPOS transaction", "Transaction amount") as num
								if(try_num < 0)
							...
							if("toggle_lock")
								if(transaction_locked)
									var
										attempt_code = input("Enter EFTPOS access code", "Reset Transaction") as num
									if(attempt_code == access_code)
										transaction_locked = 0
										transaction_paid = 0
...
obj
	item
		device
			eftpos
				proc
					scan_card(var/obj/item/weapon/card/I)
