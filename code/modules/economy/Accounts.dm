...
proc
	create_account(var/new_owner_name = "Default user", var/starting_funds = 0, var/obj/machinery/computer/account_database/source_db)
		...
		if(!source_db)
			...
			var
				obj
					item
						smallDelivery
							P = new /obj/item/smallDelivery(source_db.loc)
...
datum
	money_account
		...
		var
			security_level = 0	//0 - auto-identify from worn ID, require only account number
...
obj
	machinery
		account_database
			attackby(O as obj, user as mob)//TODO:SANITY
				...
				if(href_list["choice"])
					switch(href_list["choice"])
						...
						if("finalise_create_account")
							...
							var
								starting_funds = max(text2num(href_list["starting_funds"]), 0)
						...
						if("view_account_detail")
							var
								index = text2num(href_list["account_index"])
...
obj
	machinery
		computer
			account_database
				proc
					charge_to_account(var/attempt_account_number, var/source_name, var/purpose, var/terminal_id, var/amount)
						if(!activated)
							return 0
...
proc
	attempt_account_access(var/attempt_account_number, var/attempt_pin_number, var/security_level_passed = 0,var/pin_needed=1)
		for(var/datum/money_account/D in all_money_accounts)
...
obj
	machinery
		computer
			account_database
				proc
					get_account(var/account_number)
						for(var/datum/money_account/D in all_money_accounts)
