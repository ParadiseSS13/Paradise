...
obj
	machinery
		pos
			proc
				PrintReceipt(var/order_id)
					var
						receipt = {"[RECEIPT_HEADER]<div>POINT OF SALE #[id]<br />
	...
	machinery
		pos
			proc
				ProductsScreen()
					var
						dat={"<fieldset><legend>Product List</legend>
							<form action="?src=\ref[src]" method="get">
							<input type="hidden" name="src" value="\ref[src]" />
							<table>
								<tr class=\"first\">
		...
		pos
			Topic(var/href, var/list/href_list)
				...
				if("act" in href_list)
					switch(href_list["act"])
						...
						if("Save Settings")
							var
								datum
									money_account
										new_linked_account = get_money_account(text2num(href_list["payableto"]),z)
					...
					var
						lid = text2num(href_list["setunits"])
					var
						newunits = input(usr,"Enter the units sold.") as num
					if(!newunits) return
					var
						line_item
							LI = line_items[lid]
					...
					var
						newtext = sanitize(input(usr,"Enter the product's name."))
					if(!newtext) return
					var
					...
					var
						newprice = input(usr,"Enter the product's price.") as num
					if(!newprice) return
					var
						pid = href_list["setprice"]
					var
						line_item
							LI = products[pid]
					LI.price = newprice
...
obj
	machinery
		pos
			attackby(var/atom/movable/A, var/mob/user, params)
