...
obj
	machinery
		atm
			...
			var
				previous_account_number = 0
			var
				max_pin_attempts = 3
			var
				ticks_left_locked_down = 0
			var
				ticks_left_timeout = 0
			var
				machine_id = ""
			var
				obj
					item
						weapon
							card
								held_card
			var
				editing_security_level = 0
			var
				view_screen = NO_SCREEN
			var
				lastprint = 0 // Printer needs time to cooldown
		...
		atm
			attack_hand(mob/user as mob)
				...
				if(get_dist(src,user) <= 1)
					...
					var
						dat = {"<h1>Nanotrasen Automatic Teller Machine</h1>
					...
					if(ticks_left_locked_down > 0)
						...
						switch(view_screen)
							if(CHANGE_SECURITY_LEVEL)
								...
								var
									text = "Zero - Either the account number or card is required to access this account. EFTPOS transactions will require a card and ask for a pin, but not verify the pin is correct."
					...
					switch(href_list["choice"])
						if("transfer")
							if(authenticated_account && linked_db)
								var
									transfer_amount = text2num(href_list["funds_amount"])
								if(transfer_amount <= 0)
									alert("That is not a valid amount.")
								else if(transfer_amount <= authenticated_account.money)
									var
										target_account_number = text2num(href_list["target_acc_number"])
									var
						...
						if("change_security_level")
							if(authenticated_account)
								var
									new_sec_level = max( min(text2num(href_list["new_security_level"]), 2), 0)
									...
									if(previous_account_number == tried_account_num)
										if(number_incorrect_tries > max_pin_attempts)
											...
											var
												datum
													money_account
														failed_account = linked_db.get_account(tried_account_num)
											if(failed_account)
												var
													datum
														transaction
						...
						if("withdrawal")
							var
								amount = max(text2num(href_list["funds_amount"]),0)
							if(amount <= 0)
								alert("That is not a valid amount.")
...
obj
	machinery
		atm
			proc
				withdraw_arbitrary_sum(var/arbitrary_sum)
...
obj
	machinery
		atm
			proc
				scan_user(mob/living/carbon/human/human_user as mob)
					if(!authenticated_account && linked_db)
