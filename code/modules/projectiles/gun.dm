		...
		weapon
			gun
				...
				var
					fire_sound = "gunshot"
				var
					fire_sound_text = "gunshot" //the fire sound that shows in chat messages: laser blast, gunshot, etc.
				var
					suppressed = 0					//whether or not a message is displayed when fired
				var
					can_suppress = 0
				var
					can_unsuppress = 1
				var
					recoil = 0						//boom boom shake the room
				var
					clumsy_check = 1
				var
					obj
				...
				var
					sawn_desc = null				//description change if weapon is sawn-off
				var
					sawn_state = SAWN_INTACT
				var
					burst_size = 1					//how large a burst is
				var
				...
				var
					unique_rename = 0 //allows renaming with a pen
				var
					unique_reskin = 0 //allows one-time reskinning
				var
					current_skin = null //the skin choice if we had a reskin
				var
					list
						options = list()
				...
				var
					ammo_x_offset = 0 //used for positioning ammo count overlay on sprite
				var
					ammo_y_offset = 0
				var
					flight_x_offset = 0
				...
				//Zooming
				var
					zoomable = FALSE //whether the gun generates a Zoom action on creation
				var
					zoomed = FALSE //Zoom toggle
				var
					zoom_amt = 3 //Distance in TURFs to move the user's screen forward (the "zoom" effect)
				var
					datum
						action
							toggle_scope_zoom
...
obj
	item
		weapon
			gun
				proc
					can_trigger_gun(var/mob/living/user)
		...
		weapon
			gun
				proc
					reskin_gun(mob/M)
						var
							choice = input(M,"Warning, you can only reskin your weapon once!","Reskin Gun") in options
		...
		weapon
			gun
				proc
					rename_gun(mob/M)
						var
							input = stripped_input(M,"What do you want to name the gun?", ,"", MAX_NAME_LEN)
...
obj
	item
		weapon
			gun
				proc
					handle_suicide(mob/living/carbon/human/user, mob/living/carbon/human/target, params)
...
obj
	item
		weapon
			gun
				proc
					zoom(mob/living/user, forced_zoom)
						if(!user || !user.client)
							return
