...
obj
	item
		ammo_casing
			proc
				newshot(params) //For energy weapons, shotgun shells and wands (!).
					if(!BB)
						...
						if(BB)
							if(initial(BB.name) == "bullet")
								var
									tmp_label = ""
								var
									label_text = sanitize(input(user, "Inscribe some text into \the [initial(BB.name)]","Inscription",tmp_label))
...
obj
	item
		ammo_box
			proc
				give_round(obj/item/ammo_casing/R, replace_spent = 0)
...
obj
	item
		ammo_box
			attackby(obj/item/A, mob/user, params, silent = 0, replace_spent = 0)
				var
					num_loaded = 0
				if(!can_load(user))
					return
				if(istype(A, /obj/item/ammo_box))
					...
					for(var/obj/item/ammo_casing/AC in AM.stored_ammo)
						var
							did_load = give_round(AC, replace_spent)
						if(did_load)
							AM.stored_ammo -= AC
							num_loaded++
