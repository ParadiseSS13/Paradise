		...
		weapon
			gun
				energy
						...
						obj
							item
								weapon
									stock_parts
										cell
											power_supply //What type of power cell this uses
					var
					...
					var
						select = 1 //The state of the select fire switch. Determines from the ammo_type list what kind of shot is fired next.
					var
						can_charge = 1
					var
						charge_sections = 4
					ammo_x_offset = 2
					var
						shaded_charge = 0 //if this gun uses a stateful charge bar for more detail
					var
						selfcharge = 0
					var
						use_external_power = 0 //if set, the weapon will look for an external power source to draw from, otherwise it recharges magically
					var
						charge_tick = 0
					var
						charge_delay = 4
									...
									obj
										item
											weapon
												stock_parts
													cell
														external = get_external_power_supply()
								...
								var
									mob
										living
											carbon
												human
													H = user //Otherwise the mob icon doesn't update, blame shitty human update_icons() code
								H.update_inv_l_hand()
								H.update_inv_r_hand()
...
obj
	item
		weapon
			gun
				energy
					afterattack(atom/target as mob|obj|turf, mob/living/user as mob|obj, params)
									...
									var
										obj
											item
												ammo_casing
													energy
														shot = ammo_type[select] //Necessary to find cost of shot
