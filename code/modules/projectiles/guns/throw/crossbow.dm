			...
			gun
				throw
					crossbow
						...
						var
							list
								possible_tensions = list(XBOW_TENSION_20, XBOW_TENSION_40, XBOW_TENSION_60, XBOW_TENSION_80, XBOW_TENSION_FULL)
	...
	item
		weapon
			gun
				throw
					crossbow
						modify_projectile(obj/item/I, on_chamber = 0)
						...
						verb
							set_tension()
								...
								if(user.incapacitated())
									return
								var
									choice = input("Select tension to draw to:", "[src]", XBOW_TENSION_FULL) as null|anything in possible_tensions
								if(!choice)
									return
	...
	item
		weapon
			gun
				throw
					crossbow
						process_fire(atom/target as mob|obj|turf, mob/living/user as mob|obj, message = 1, params, zone_override)
							..()
							tension = 0
							update_icon()
obj
...
obj
	item
		weapon
			arrow
				proc
					removed() //Helper for metal rods falling apart.
						return
								...
								obj
									item
										weapon
											shard
												shrapnel
													S = new /obj/item/weapon/shard/shrapnel
							S.loc = get_turf(src)
							qdel(src)
