					...
					var
						obj
							item
								weapon
									dart_cartridge
										cartridge = null //Container of darts.
					var
						max_beakers = 3
					var
				...
				dartgun
					proc
						get_mixed_syringe()
							...
							if(mixing.len)
								var
									mix_amount = dart_reagent_amount/mixing.len
	...
	item
		weapon
			gun
				dartgun
					proc
						fire_dart(atom/target, mob/user)
							if(locate (/obj/structure/table, src.loc))
								...
								var
									obj
										effect
											syringe_gun_dummy
												D = new/obj/effect/syringe_gun_dummy(get_turf(src))
								var
									obj
										item
											weapon
			...
			gun
				dartgun
					attack_self(mob/user)
						...
						var
							dat = "<b>[src] mixing control:</b><br><br>"
	...
	item
		weapon
			gun
				dartgun
					proc
						check_beaker_mixing(var/obj/item/B)
							if(!mixing || !beakers)
								return 0
				...
				dartgun
					Topic(href, href_list)
						...
						if(href_list["stop_mix"])
							var
								index = text2num(href_list["stop_mix"])
							...
							var
								index = text2num(href_list["mix"])
							if(index <= beakers.len)
								mixing += beakers[index]
						else if(href_list["eject"])
							var
								index = text2num(href_list["eject"])
							if(index <= beakers.len)
								if(beakers[index])
									var
										obj
...
obj
	effect
		syringe_gun_dummy //moved this shitty thing here
			name = ""
			desc = ""
