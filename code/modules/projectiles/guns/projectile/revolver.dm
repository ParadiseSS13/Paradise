	...
	item
		weapon
			gun
				projectile
					revolver
						get_ammo(countchambered = 0, countempties = 1)
							var
						...
						russian
							proc
								Spin()
									chambered = null
									var
										random = rand(1, magazine.max_ammo)
								...
								if(ishuman(user))
									...
									if(chambered)
										...
										if(AC.fire(user, user))
											playsound(user, fire_sound, 50, 1)
											var
												zone = check_zone(user.zone_sel.selecting)
			...
			gun
				projectile
					revolver
						russian
							proc
								shoot_self(mob/living/carbon/human/user, affecting = "head")
		...
		weapon
			gun
				projectile
					revolver
						doublebarrel
							isHandgun() //contrary to popular opinion, double barrels are not, shockingly, handguns
								return 0
				...
				projectile
					revolver
						doublebarrel
							improvised
								cane
									examine(mob/user) // HAD TO REPEAT EXAMINE CODE BECAUSE GUN CODE DOESNT STEALTH
										var
											f_name = "\a [src]."
