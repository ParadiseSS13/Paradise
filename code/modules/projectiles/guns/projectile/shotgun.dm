	...
	item
		weapon
			gun
				projectile
					shotgun
						attackby(obj/item/A, mob/user, params)
							. = ..()
							if(.)
								return
							var
							...
							pump_reload(mob/M)
								...
								var
									obj
										item
											ammo_casing
												AC = magazine.get_round() //load next casing.
								chambered = AC
	...
	item
		weapon
			gun
				projectile
					shotgun
						isHandgun() //You cannot, in fact, holster a shotgun.
							return 0
			...
			gun
				projectile
					shotgun
						boltaction
							enchanted
								shoot_live_shot(mob/living/user as mob|obj, pointblank = 0, mob/pbtarget = null, message = 1)
									..()
									if(guns_left)
										var
											obj
		...
		weapon
			gun
				projectile
					shotgun
						automatic
							shoot_live_shot(mob/living/user as mob|obj)
								..()
								pump(user)
				...
				projectile
					shotgun
						automatic
							dual_tube
								proc
									toggle_tube(mob/living/user)
										var
											current_mag = magazine
										var
											alt_mag = alternate_magazine
										magazine = alt_mag
