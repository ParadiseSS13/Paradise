...
/obj/item/gun/projectile/automatic/toy/pistol/enforcer/update_overlays()
	. = ..()
	if(suppressed)
		. += image(icon = 'icons/obj/guns/projectile.dmi', icon_state = "suppressor_attached", pixel_x = 15, pixel_y = 5)
	if(gun_light)
		var/flashlight = "uflashlight_attached"
		if(gun_light.on)
			flashlight = "uflashlight_attached-on"
		. += image(icon = 'icons/obj/guns/projectile.dmi', icon_state = flashlight, pixel_x = 5, pixel_y = -2)
