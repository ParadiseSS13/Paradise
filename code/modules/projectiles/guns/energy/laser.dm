...
/obj/item/gun/energy/laser/examine_more(mob/user)
	...
	The early models would be significant at carving out Shellguard's niche in the arms market and later weapon philosophy around lasers. The LG series was the first mass-produced and iconic laser brand throughout the Orion Sector, thanks to low cost and excessive overproduction in units. \
	...
	The use of lightweight alloys and polymers in the body and furniture makes it surprisingly light for its size. Heat generated during firing is dissipated by a fan-assisted heatsink, protecting the internal power cell and optical assembly from overheating. \
	The LG-5 is prevalent among corporate security forces and PMCs. Thanks to the popularity of the design - along with its simplicity and robust construction - a wide variety of aftermarket modifications have been created for it both by laser enthusiasts and corporate interests, giving end-users many options to customise it to fit their needs. \
...
/obj/item/gun/energy/xray/examine_more(mob/user)
	...
	When a target is hit by the X-ray beam, only a portion of the beam's energy is transferred into the target, allowing it to continue flying. The very high energy used to generate the beam ensures that the damage dealt is roughly equivalent to a traditional laser rifle (at close range). \
