			...
			magic
				fireball
					Range()
						var
							turf
								T1 = get_step(src,turn(dir, -45))
						var
							turf
								T2 = get_step(src,turn(dir, 45))
						var
							mob
								living
									L = locate(/mob/living) in T1 //if there's a mob alive in our front right diagonal, we hit it.
						if(L && L.stat != DEAD)
...
obj
	item
		projectile
			magic
				resurrection
					on_hit(var/mob/living/carbon/target)
						. = ..()
						if(ismob(target))
							var
		...
		projectile
			magic
				door
					...
					var
						list
							door_types = list(/obj/structure/mineral_door/wood,/obj/structure/mineral_door/iron,/obj/structure/mineral_door/silver,\
								obj
									structure
										mineral_door
											gold,/obj/structure/mineral_door/uranium,/obj/structure/mineral_door/sandstone,/obj/structure/mineral_door/transparent/plasma,\
								obj
									structure
										mineral_door
											transparent
	...
	item
		projectile
			magic
				door
					proc
						OpenDoor(var/obj/machinery/door/D)
	...
	wabbajack(mob/living/M)
		if(istype(M))
			if(istype(M, /mob/living) && M.stat != DEAD)
				...
				var
					randomize = pick("robot","slime","xeno","human","animal")
				switch(randomize)
					if("robot")
						if(prob(30))
					...
					if("animal")
						if(prob(50))
							var
								beast = pick("carp","bear","mushroom","statue", "bat", "goat", "tomato")
							switch(beast)
							...
							var
								animal = pick("parrot","corgi","crab","pug","cat","mouse","chicken","cow","lizard","chick","fox")
							switch(animal)
					...
					if("human")
						...
						var
							picked_species = pick(new_species)
						var
							mob
								living
									carbon
						...
						var
							datum
								preferences
									A = new()	//Randomize appearance for the human
						A.copy_to(new_mob)
					else
						return
										...
										mob
											living
												simple_animal
													hostile
														statue
															S = new /mob/living/simple_animal/hostile/statue(change.loc, firer)
