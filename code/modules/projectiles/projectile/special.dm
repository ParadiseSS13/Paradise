...
obj
	item
		projectile
			beam
				wormhole
					New(var/obj/item/ammo_casing/energy/wormhole/casing)
						if(casing)
							gun = casing.gun
				...
				wormhole
					on_hit(atom/target)
						if(ismob(target))
							var
								turf
									portal_destination = pick(orange(6, src))
								...
								simple_animal
									hostile
										mimic
											copy
												ranged
													R = new /mob/living/simple_animal/hostile/mimic/copy/ranged(T, G, firer)
					if(ismob(target))
						R.target = target
