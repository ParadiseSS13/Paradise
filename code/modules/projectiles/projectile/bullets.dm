...
obj
	item
		projectile
			bullet
				weakbullet
					rubber //beanbag that shells that don't embed
						embed = 0
						sharp = 0
...
obj
	item
		projectile
			bullet
				weakbullet2  //detective revolver instastuns, but multiple shots are better for keeping punks down
					damage = 5
					weaken = 3
					stamina = 60
...
obj
	item
		projectile
			bullet
				weakbullet2
					rubber //detective's bullets that don't embed
						embed = 0
						sharp = 0
				...
				meteorshot
					on_hit(var/atom/target, var/blocked = 0)
						...
						if(istype(target, /atom/movable))
							...
							var
								atom
									throw_target = get_edge_target_turf(M, get_dir(src, get_step_away(M, src)))
