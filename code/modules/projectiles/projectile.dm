obj
	item
		projectile
			...
			var
				atom
					original = null // the original target clicked
			var
				turf
					starting = null // the projectile's starting turf
			var
				list
					permutated = list() // we've passed through these atoms, don't try to hit them again
			var
				paused = FALSE //for suspending the projectile midair
			var
				p_x = 16
			var
				p_y = 16 // the pixel location of the tile that the player clicked. Default is the center
			var
				speed = 1			//Amount of deciseconds it takes for projectile to travel
			var
				Angle = 0
			var
				spread = 0			//amount (in degrees) of projectile spread
			var
				legacy = 0			//legacy projectile system
			animate_movement = 0
			...
			var
				flag = "bullet" //Defines what armor to use when it hits things.  Must be set to bullet, laser, energy,or bomb	//Cael - bio and rad are also valid
			var
				projectile_type = "/obj/item/projectile"
			var
				range = 50 //This will de-increment every step. When 0, it will delete the projectile.
			//Effects
			var
				stun = 0
			var
			...
			var
				forcedodge = 0 //to pass through everything
	...
	item
		projectile
			Bump(atom/A, yes)
				...
				var
					distance = get_dist(get_turf(A), starting) // Get the distance between the turf shot from and the mob we hit and use that for the calculations.
				def_zone = ran_zone(def_zone, max(100-(7*distance), 5)) //Lower accurancy/longer range tradeoff. 7 is a balanced number to use.

				if(isturf(A) && hitsound_wall)
					var
						volume = Clamp(vol_by_damage() + 20, 0, 100)
					if(suppressed)
						volume = 5
					...
					if(!legacy) //new projectiles
						...
						while(loc)
							if(!paused)
								...
								var
									Pixel_x=round(sin(Angle)+16*sin(Angle)*2)
								var
									Pixel_y=round(cos(Angle)+16*cos(Angle)*2)
								var
									pixel_x_offset = pixel_x + Pixel_x
								var
									pixel_y_offset = pixel_y + Pixel_y
								var
									new_x = x
								var
									new_y = y
...
obj
	item
		projectile
			Crossed(atom/movable/AM) //A mob moving on a tile with a projectile is hit by it.
				..()
