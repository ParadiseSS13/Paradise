		...
		device
			pda
				...
				var
					owner = null
				var
					default_cartridge = 0 // Access level defined by cartridge
				var
					obj
						item
							weapon
								cartridge
									cartridge = null //current cartridge
				var
					datum
						data
							pda
				...
				var
					lock_code = "" // Lockcode to unlock uplink
				var
					honkamt = 0 //How many honks left when infected with honk.exe
				var
					mimeamt = 0 //How many silence left when infected with mime.exe
				var
					detonate = 1 // Can the PDA be blown up?
				var
					ttone = "beep" //The ringtone!
				var
					list
						ttone_sound = list("beep" = 'sound/machines/twobeep.ogg',
					...
					obj
						item
							weapon
								card
									id
										id = null //Making it possible to slot an ID card into the PDA so it can function as both.
				var
					ownjob = null //related to above
				var
					ownrank = null // this one is rank, never alt title
				...
				var
					obj
						item
							device
								paicard
									pai = null	// A slot for a personal AI device
				var
					retro_mode = 0
			...
			pda
				ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1)
					...
					var
						datum
							nanoui
								old_ui = nanomanager.get_open_ui(user, src, "main")
					var
						auto_update = 1
					if(!current_app)
						return
					...
					var
						data[0]  // This is the data that will be sent to the PDA
...
obj
	item
		device
			pda
				proc
					start_program(datum/data/pda/P)
								...
								datum
									data
										pda
											app
												main_menu
													A = find_program(/datum/data/pda/app/main_menu)
							if(A)
								start_program(A)
						if("StartProgram")
...
obj
	item
		device
			pda
				proc
					id_check(mob/user as mob, choice as num)//To check for IDs; 1 for in-pda use, 2 for out of pda use.
						if(choice == 1)
							if(id)
								remove_id()
							else
...
obj
	item
		device
			pda
				attack(mob/living/C as mob, mob/living/user as mob)
		...
		device
			pda
				proc
					set_ringtone()
						var
							t = input("Please enter new ringtone", name, ttone) as text
