		...
		pda
			app
				status_display
					...
					var
						message1	// used for status_displays
					var
						message2
...
datum
	data
		pda
			app
				status_display
					update_ui(mob/user as mob, list/data)
						data["records"] = list(
	...
	data
		pda
			app
				status_display
					proc
						post_status(var/command, var/data1, var/data2)
							var
								datum
									radio_frequency
					...
					Topic(href, list/href_list)
						if(pda.cartridge && istype(pda.cartridge.radio, /obj/item/radio/integrated/signal))
							...
							switch(href_list["choice"])
								...
								if("Signal Frequency")
									var
										new_frequency = sanitize_frequency(R.frequency + text2num(href_list["sfreq"]))
	...
	data
		pda
			app
				crew_records
					proc
						load_records(datum/data/record/R)
							general_records = R
							has_back = 1
