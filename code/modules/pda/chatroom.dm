var
	list
		chatrooms = list(new /datum/chatroom("General Discussion"))

datum
	chatroom
		var
			name = "Generic Chatroom"
		var
			list
				users = list()
		var
		...
		var
			list
				logs = list() // chat logs
		var
			topic = "Discussion" // topic message for the chatroom
		var
			is_public = 1
		var
			announcer = "CyberiadAI"
...
datum
	chatroom
		proc
			login(datum/data/pda/app/chatroom/user)
...
datum
	chatroom
		proc
			logout(datum/data/pda/app/chatroom/user)
										...
										datum
											data
												pda
													app
														chatroom
															C = P.find_program(/datum/data/pda/app/chatroom)
									var
										datum
											data
												pda
					...
					Topic(href, list/href_list)
						...
						switch(href_list["choice"])
							...
							if("Post")
								...
								var
									t = input("Please enter message", target) as text|null
								spawn()
							...
							if("Topic")
								...
								var
									t = input("Enter new topic:", current_room, current_room.topic) as text|null
								spawn()
							...
							if("New Room")
								...
								var
									t = input("Enter room name:", name) as text|null
								if(!t)
									return
								...
								var
									access = input("Room access?", current_room) as null|anything in list("Public", "Private")
								if(!access)
									return
