		...
		pda
			utility
				flashlight
					...
					var
						fon = 0 //Is the flashlight function on?
					var
						f_lum = 2 //Luminosity for the flashlight function
		...
		pda
			utility
				honk
					...
					var
						last_honk //Also no honk spamming that's bad too
	...
	data
		pda
			utility
				scanmode
					medical
						scan_mob(mob/living/C as mob, mob/living/user as mob)
	...
	data
		pda
			utility
				scanmode
					dna
						scan_atom(atom/A as mob|obj|turf|area, mob/user as mob)
							scan_blood(A, user)
						...
						scan_atom(atom/A as mob|obj|turf|area, mob/user as mob)
							if(!isnull(A.reagents))
								if(A.reagents.reagent_list.len > 0)
									var
										reagents_length = A.reagents.reagent_list.len
					...
					notes
						scan_atom(atom/A as mob|obj|turf|area, mob/user as mob)
							if(notes && istype(A, /obj/item/weapon/paper))
								...
								var
									list
										brlist = list("p", "/p", "br", "hr", "h1", "h2", "h3", "h4", "/h1", "/h2", "/h3", "/h4")
								...
								var
									raw_scan = sanitize_simple(P.info, list("\t" = "", "ÿ" = ""))
								var
									formatted_scan = ""
								...
								while(tag_start && tag_stop)
									var
										pre = copytext(raw_scan, 1, tag_start) // Get the stuff that comes before the tag
									var
										tag = lowertext(copytext(raw_scan, tag_start + 1, tag_stop)) // Get the tag so we can do intellegent replacement
									var
										tagend = findtext(tag, " ") // Find the first space in the tag if there is one.
