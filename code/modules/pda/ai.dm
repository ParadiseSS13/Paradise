	...
	item
		device
			pda
				silicon
					proc
						set_name_and_job(newname as text, newjob as text, newrank as null|text)
							owner = newname
							ownjob = newjob
							if(newrank)
								ownrank = newrank
								...
								datum
									data
										pda
											app
												messenger
													M = find_program(/datum/data/pda/app/messenger)
							if(!M)
								to_chat(usr, "<span class='warning'>Cannot use messenger!</span>")
							var
								list
									plist = M.available_pdas()
							if(plist)
								var
									c = input(usr, "Please select a PDA") as null|anything in sortList(plist)
					...
					verb
						cmd_show_message_log()
							...
							if(!M)
								to_chat(usr, "<span class='warning'>Cannot use messenger!</span>")
							var
								HTML = "<html><head><title>AI PDA Message Log</title></head><body>"
