		...
		pda
			app
				messenger
					...
					var
						toff = 0 //If 1, messenger disabled
					var
						list
							tnote[0]  //Current Texts
					var
						last_text //No text spamming

					var
						m_hidden = 0 // Is the PDA hidden from the PDA list?
					var
						active_conversation = null // New variable that allows us to only view a single conversation.
					var
						list
							conversations = list()    // For keeping up with who we have PDA messsages from.
					var
						latest_post = 0
					var
						auto_scroll = 1
								...
								var
									datum
										data
											pda
												messenger_plugin
													plugin = locate(href_list["plugin"])
	...
	data
		pda
			app
				messenger
					proc
						create_message(var/mob/living/U, var/obj/item/device/pda/P)
							var
								t = input(U, "Please enter message", name, null) as text|null
							if(!t)
								return
