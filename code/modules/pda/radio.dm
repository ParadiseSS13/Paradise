	...
	item
		radio
			integrated
				...
				var
					list
						botlist = null		// list of bots
				var
					mob
						living
							simple_animal
								bot
									active 	// the active bot; if null, show bot list
				var
					list
						botstatus			// the status signal sent by the bot
				var
					bot_type				//The type of bot it is.
				var
					bot_filter				//Determines which radio filter to use.
...
obj
	item
		radio
			integrated
				proc
					post_signal(var/freq, var/key, var/value, var/key2, var/value2, var/key3, var/value3,var/key4, var/value4, s_filter)

//	to_chat(world, "Post: [freq]: [key]=[value], [key2]=[value2]")
	var
		datum
			radio_frequency
				frequency = radio_controller.return_frequency(freq)
...
obj
	item
		radio
			integrated
				proc
					add_to_radio(bot_filter) //Master filter control for bots. Must be placed in the bot's local New() to support map spawned bots.
						if(radio_controller)
					...
					Topic(href, href_list)
						...
						switch(href_list["op"])
							...
							if("setdest")
								if(deliverybeacons)
									var
										dest = input("Select Bot Destination", "Mulebot [active.suffix] Interlink", active.destination) as null|anything in deliverybeacontags
									if(dest)
										spawn(0)
