	...
	machinery
		lapvend
			attack_hand(mob/user as mob)
				...
				var
					vendorname = (src.name)  //import the machine's name
				var
					dat = "<TT><center><b>[vendorname]</b></center><hr /><br>" //display the name, and added a horizontal rule
				if(vendmode == 0)
					...
					if(istype(I, /obj/item/weapon/card/id))
						...
						if(vendor_account)
							var
								datum
									money_account
										D = attempt_account_access_nosec(C.associated_account_number)
...
obj
	machinery
		lapvend
			proc
				choose_progs(var/obj/item/weapon/card/id/C)
...
obj
	machinery
		lapvend
			proc
				calc_reimburse(var/obj/item/device/laptop/L)
...
obj
	machinery
		lapvend
			proc
				reimburse(var/obj/item/weapon/card/I)
					if(istype(I, /obj/item/weapon/card/id))
						var
							obj
								item
