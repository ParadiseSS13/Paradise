...
var
	prison_shuttle_can_send = 1
var
	prison_shuttle_time = 0
var
	prison_shuttle_timeleft = 0
			...
			prison_shuttle
									...
									item
										part
											board
												circuit
													prison_shuttle
														M = new /obj/item/part/board/circuit/prison_shuttle( A )
				...
				attack_hand(var/mob/user as mob)
					...
					var
						datum
							browser
								popup = new(user, "computer", name, 575, 450)
	...
	proc
		post_signal(var/command)
			var
				datum
					radio_frequency
						frequency = radio_controller.return_frequency(1311)
			if(!frequency) return
			var
				datum
					signal
	...
	proc
		prison_process()
			while(prison_shuttle_time - world.timeofday > 0)
				var
					ticksleft = prison_shuttle_time - world.timeofday
			...
			switch(prison_shuttle_at_station)
				...
				if(0)
					...
					var
						area
							start_location = locate(/area/shuttle/prison/prison)
					var
						area
				...
				if(1)
					...
					var
						area
							end_location = locate(/area/shuttle/prison/prison)
