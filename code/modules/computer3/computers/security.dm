...
/datum/file/program/secure_data
	...
	interact()
		...
		if(temp)
			...
			dat = text("Confirm Identity (R): <A href='?src=[UID()];choice=Confirm Identity R'>[]</A><HR>", (scan ? text("[]", scan.name) : "----------"))
			if(computer.cardslot.dualslot)
				dat += text("Check Identity (W): <A href='?src=[UID()];choice=Confirm Identity W'>[]</A><BR>", (scan2 ? text("[]", scan2.name) : "----------"))
				if(scan2 && !scan)
							...
							for(var/datum/data/record/R in sortRecord(data_core.general, sortBy, order))
								...
								switch(crimstat)
									...
									if("")
										...
										crimstat = "No Record."
								dat += text("<tr style=[]><td><A href='?src=[UID()];choice=Browse Record;d_rec=\ref[]'>[]</a></td>", background, R, R.fields["name"])
...
//RECORD DELETE
			...
			if("Add Entry")
				if(!( istype(active2, /datum/data/record) ))
					return
				var/a2 = active2
				var/t1 = sanitize_local(copytext(input("Add Comment:", "Secure. records", null, null)  as message,1,MAX_MESSAGE_LEN))
...
//FIELD FUNCTIONS
			if("Edit Field")
				...
				switch(href_list["field"])
					...
					if("id")
						if(istype(active2, /datum/data/record))
							var/t1 = sanitize_local(copytext(input("Please input id:", "Secure. records", active1.fields["id"], null)  as text,1,MAX_MESSAGE_LEN))
							if((!( t1 ) || !( authenticated ) || usr.stat || usr.restrained() || (!interactable() && (!istype(usr, /mob/living/silicon))) || active1 != a1))
								return
							active1.fields["id"] = t1
					if("fingerprint")
						if(istype(active1, /datum/data/record))
							var/t1 = sanitize_local(copytext(input("Please input fingerprint hash:", "Secure. records", active1.fields["fingerprint"], null)  as text,1,MAX_MESSAGE_LEN))
							if((!( t1 ) || !( authenticated ) || usr.stat || usr.restrained() || (!interactable() && (!istype(usr, /mob/living/silicon))) || active1 != a1))
								return
					...
					if("mi_crim")
						if(istype(active2, /datum/data/record))
							var/t1 = sanitize_local(copytext(input("Please input minor crimes list:", "Secure. records", active2.fields["mi_crim"], null)  as text,1,MAX_MESSAGE_LEN))
							if((!( t1 ) || !( authenticated ) || usr.stat || usr.restrained() || (!interactable() && (!istype(usr, /mob/living/silicon))) || active2 != a2))
								return
							active2.fields["mi_crim"] = t1
					if("mi_crim_d")
						if(istype(active2, /datum/data/record))
							var/t1 = sanitize_local(copytext(input("Please summarize minor crimes:", "Secure. records", active2.fields["mi_crim_d"], null)  as message,1,MAX_MESSAGE_LEN))
							if((!( t1 ) || !( authenticated ) || usr.stat || usr.restrained() || (!interactable() && (!istype(usr, /mob/living/silicon))) || active2 != a2))
								return
							active2.fields["mi_crim_d"] = t1
					if("ma_crim")
						if(istype(active2, /datum/data/record))
							var/t1 = sanitize_local(copytext(input("Please input major crimes list:", "Secure. records", active2.fields["ma_crim"], null)  as text,1,MAX_MESSAGE_LEN))
							if((!( t1 ) || !( authenticated ) || usr.stat || usr.restrained() || (!interactable() && (!istype(usr, /mob/living/silicon))) || active2 != a2))
								return
							active2.fields["ma_crim"] = t1
					if("ma_crim_d")
						if(istype(active2, /datum/data/record))
							var/t1 = sanitize_local(copytext(input("Please summarize major crimes:", "Secure. records", active2.fields["ma_crim_d"], null)  as message,1,MAX_MESSAGE_LEN))
							if((!( t1 ) || !( authenticated ) || usr.stat || usr.restrained() || (!interactable() && (!istype(usr, /mob/living/silicon))) || active2 != a2))
								return
							active2.fields["ma_crim_d"] = t1
					if("notes")
						if(istype(active2, /datum/data/record))
							var/t1 = trim(sanitize_local(copytext(input("Please summarize minor crimes:", "Secure. records", active2.fields["mi_crim_d"], null)  as message,1,MAX_MESSAGE_LEN)))
							if((!( t1 ) || !( authenticated ) || usr.stat || usr.restrained() || (!interactable() && (!istype(usr, /mob/living/silicon))) || active2 != a2))
								return
					...
					if("species")
						if(istype(active1, /datum/data/record))
							var/t1 = sanitize_local(copytext(input("Please enter race:", "General records", active1.fields["species"], null)  as message,1,MAX_MESSAGE_LEN))
							if((!( t1 ) || !( authenticated ) || usr.stat || usr.restrained() || (!interactable() && (!istype(usr, /mob/living/silicon))) || active1 != a1))
								return
