...
var
	syndicate_elite_shuttle_can_send = 1
var
	syndicate_elite_shuttle_time = 0
var
	syndicate_elite_shuttle_timeleft = 0

obj
	machinery
		computer3
			syndicate_elite_shuttle
...
proc
	syndicate_elite_process()
		var
			area
				syndicate_mothership
					control
						syndicate_ship = locate()//To find announcer. This area should exist for this proc to work.
		var
			area
				syndicate_mothership
					elite_squad
						elite_squad = locate()//Where is the specops area located?
		var
			mob
				living
					silicon
						decoy
							announcer = locate() in syndicate_ship//We need a fake AI to announce some stuff below. Otherwise it will be wonky.

		var
			message_tracker[] = list(0,1,2,3,5,10,30,45)//Create a a list with potential time values.
		var
			message = "THE SYNDICATE ELITE SHUTTLE IS PREPARING FOR LAUNCH"//Initial message shown.
		if(announcer)
		...
		while(syndicate_elite_shuttle_time - world.timeofday > 0)
			var
				ticksleft = syndicate_elite_shuttle_time - world.timeofday
			...
			if(announcer)
				var
					rounded_time_left = round(syndicate_elite_shuttle_timeleft)//Round time so that it will report only once, not in fractions.
...
/*
	var
		obj
			explosionmarker = locate("Syndicate Breach Area")
	if(explosionmarker)
		var
			turf
				simulated
					T = explosionmarker.loc
		if(T)
...
*/
	var
		area
			start_location = locate(/area/shuttle/syndicate_elite/mothership)
	var
		area
			end_location = locate(/area/shuttle/syndicate_elite/station)
...
//		return
	...
	var
		datum
			browser
				popup = new(user, "computer", "Special Operations Shuttle", 575, 450)
