	...
	file
		program
			dnascanner
				...
				var
					injectorready = 0	//Quick fix for issue 286 (screwdriver the screen twice to restore injector)	-Pete
				...
				Topic(var/href, var/list/href_list)
					...
					if("duration" in href_list)
						var
							modifier = text2num(href_list["duration"])
						radduration	+= modifier
					if("strength" in href_list)
						var
							modifier = text2num(href_list["strength"])
					...
					if("save" in href_list)
						...
						if(G && dest)
							var
								label = input(usr, "Enter a filename:", "Save file", buffer.real_name) as text
							G.name = G.name + "([label])"
							dest:addfile(G)
					...
					if("inject" in href_list)
						...
						var
							which = href_list["inject"]
						var
							datum
								dna
									target = scanner.occupant.dna
						switch(which)
							if("all",null)
								if(buffer.struc_enzymes)
					...
					if("generate" in href_list)
						...
						if(inj)
							...
							spawn(300)
								injectorready = 1
							var
								label = input(usr, "Enter a label", "Label [inj.name] Injector", buffer.real_name) as null|text
				...
				proc
					status_display(var/dna_summary = 0)
						var
							mob
								living
									occupant = scanner.occupant
						var
							status_html
						if(viable)
							...
							if(ishuman(occupant))
								var
									rejuvenators = round(occupant.reagents.get_reagent_amount("inaprovaline") / REJUVENATORS_MAX * 100)
						...
						var
							occupant_status = "Scanner Unoccupied"
						if(present)
							if(!viable)
						...
						var
							dat = "<h3>Scanner Status</h3>[topic_link(src,"","Scan")]<div class='statusDisplay'>[occupant_status][status_html]</div>"
						if(present)
				...
				proc
					main_menu()
						var
							dat = status_display(dna_summary = 1)
				...
				proc
					block_plus_minus(var/topicstr, var/blockval,var/min,var/max)
						var
							left = blockval - 1
						var
							right = blockval + 1
				...
				proc
					getblockstring(var/ui = 0)
						var
							mainblock = "seblock"
						var
							block = se_block
						var
							input = scanner.occupant.dna.struc_enzymes
						if(ui)
							mainblock = "uiblock"
							block = ui_block
						...
						var
							string = "<div class='getblockstring'><div class='blockString'>"
						var
							subpos = 1 // keeps track of the current sub block
						var
							blockpos = 1
				...
				proc
					pulse(var/target as null|anything in list("ui","se"))
						...
						if(!viable)
							...
							switch(target)
								if("ui")
									var
										ui = scanner.occupant.dna.uni_identity
									var
										block = getblock(ui,ui_block,3)
									var
										result = ""
									for(var/sb = 1; sb <=3; sb++)
										var
											temp = copytext(block,sb,sb+1)
										if(sb == subblock)
								...
								if("se")
									var
										se = scanner.occupant.dna.struc_enzymes
									var
										targetblock = se_block
									...
									if(!(se_block in list(2,8,10,12)) && prob (20)) // shifts the target slightly
										if(se_block <= 5)
											...
											targetblock--
									var
										block = getblock(scanner.occupant.dna.struc_enzymes,targetblock,3)
				...
				proc
					rejuv()
						...
						if(istype(H))
							var
								inap = H.reagents.get_reagent_amount("inaprovaline") // oh my *god* this section was ugly before I shortened it
		...
		computer
			scan_consolenew
				Topic(href, href_list)
					...
					temp_html = null
					var
						temp_header_html = null
					var
						temp_footer_html = null
					...
					var
						mob
							living
								carbon
									human
										human_occupant = scanner.occupant
					...
					if(!viable_occupant) // If there is no viable occupant only allow certain screens
						var
							allowed_no_occupant_screens = list("mainmenu", "radsetmenu", "buffermenu") //These are the screens which will be allowed if there's no occupant
					...
					if(href_list["uimenuset"] && href_list["uimenusubset"]) // This chunk of code updates selected block / sub-block based on click
						var
							menuset = text2num(href_list["uimenuset"])
						var
							menusubset = text2num(href_list["uimenusubset"])
...
//				to_chat(usr, text("Occupant now has [] units of rejuvenation in his/her bloodstream.", human_occupant.reagents.get_reagent_amount("inaprovaline")))
	...
	if(href_list["semenuset"] && href_list["semenusubset"]) // This chunk of code updates selected block / sub-block based on click (se stands for strutural enzymes)
		var
			menuset = text2num(href_list["semenuset"])
		var
			menusubset = text2num(href_list["semenusubset"])
				...
				obj
					item
						tool
							medical
								dnainjector
									I = new /obj/item/tool/medical/dnainjector
			I.dna = buffer1
	...
	if(href_list["load_disk"])
		var
			buffernum = text2num(href_list["load_disk"])
	...
	if(href_list["save_disk"])
		var
			buffernum = text2num(href_list["save_disk"])
		if((buffernum > 3) || (buffernum < 1))
			return
	...
	if(viable_occupant && !scanner_status_html && occupant) //is there REALLY someone in there?
		...
		if(human_occupant)
			var
				rejuvenators = round(human_occupant.reagents.get_reagent_amount("inaprovaline") / REJUVENATORS_MAX * 100)
	...
	var
		scanner_access_text = "Lock Scanner"
	if(scanner.locked)
