	...
	file
		program
			cloning
				...
				proc
					updatemodules()
						...
						if(!scanner || !pod1)
							var
								list
									nearby = computer.net.get_machines()
								...
								obj
									item
										weapon
											implant
												health
													H = locate(active_record.fields["imp"])
						...
						var
							datum
								file
									data
										genome
											G = locate(href_list["loadfile"]) in computer.floppy.files
						if(!istype(G))
				...
				proc
					scan_mob(mob/living/carbon/human/subject as mob)
