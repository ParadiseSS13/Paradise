			...
			card_comp
				...
				proc
					login_menu()
						//assume peripherals and cards, do checks for them in interact
						var
							dat = "<br><i>Please insert the cards into the slots</i><br>"
				...
				proc
					set_default_access(var/jobname)
						var
							datum
								job
									jobdatum
				...
				Topic(href, list/href_list)
						...
						var
							obj
								item
									weapon
										card
											card = usr.get_active_hand()
						if(!istype(card)) return

						var
							which = href_list["insert"]
						if(which == "writer")
					...
					if("access" in href_list)
						if(auth)
							var
								access_type = text2num(href_list["access"])
