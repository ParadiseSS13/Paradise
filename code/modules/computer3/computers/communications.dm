			...
			communications
				captain
					...
					spawn_files			= list(/datum/file/program/card_comp, /datum/file/program/security, /datum/file/program/crew, /datum/file/program/arcade,
											datum
												file
													camnet_key, /datum/file/camnet_key/entertainment, /datum/file/camnet_key/singularity)
			...
			communications
				...
				Topic(var/href, var/list/href_list)
					...
					if("callshuttle2" in href_list)
						...
						if(authenticated)
							var
								input = stripped_input(usr, "Please enter the reason for calling the shuttle.", "Shuttle Call Reason.","") as text|null
					...
					if("MessageCentcomm" in href_list)
						...
						if(authenticated==2)
							if(centcomm_message_cooldown)
								...
								return
							var
								input = stripped_input(usr, "Please choose a message to transmit to Centcomm via quantum entanglement.  Please be aware that this process is very expensive, and abuse will lead to... termination.  Transmission does not guarantee a response.", "To abort, send an empty message.", "")
							if(!input || !interactable())
								return
					...
					if("MessageSyndicate" in href_list)
						if((authenticated==2) && (computer.emagged))
							if(centcomm_message_cooldown)
								...
								return
							var
								input = stripped_input(usr, "Please choose a message to transmit to \[ABNORMAL ROUTING CORDINATES\] via quantum entanglement.  Please be aware that this process is very expensive, and abuse will lead to... termination. Transmission does not guarantee a response.", "To abort, send an empty message.", "")
							if(!input || !interactable())
								return
					...
					if("ai-delmessage2" in href_list)
						if(aicurrmsg)
							var
								title = messagetitle[aicurrmsg]
							var
								text  = messagetext[aicurrmsg]
							messagetitle.Remove(title)
							messagetext.Remove(text)
							if(currmsg == aicurrmsg)
								currmsg = 0
				...
				proc
					confirm_menu(var/prompt,var/yes_option)
