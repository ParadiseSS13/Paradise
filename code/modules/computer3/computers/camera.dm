	...
	file
		camnet_key
			...
			var
				title = "Station"
			var
				desc = "Connects to station security cameras."
			var
				list
					networks = list("SS13")
			var
			...
			security
				...
				execute(var/datum/file/program/caller)
					...
					if(computer && !key)
						var
							list
								fkeys = computer.list_files(/datum/file/camnet_key)
						if(fkeys && fkeys.len)
							key = fkeys[1]
						update_icon()
				...
				interact()
					...
					if(world.time - last_camera_refresh > 50 || !camera_list)
						...
						var
							list
								temp_list = computer.camnet.get_machines(key)
