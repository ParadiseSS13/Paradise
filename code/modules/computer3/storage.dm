			...
			computer
				storage
					...
					var
						volume		= 0
					var
						max_volume	= 64		// should be enough for anyone

					var
						driveletter	= null		// drive letter according to the computer

					var
						list
							files	= list()	// a list of files in the memory (ALL files)
					var
						removeable	= 0			// determinse if the storage device is a removable hard drive (ie floppy)


					var
						writeprotect = 0		// determines if the drive forbids writing.
													// note that write-protect is hardware and does not respect emagging.

					var
						list
							spawnfiles = list()// For mappers, special drives, and data disks
					...
					proc
						addfile(var/datum/file/F,var/forced = 0)
							...
							return 1
					proc
						removefile(var/datum/file/F,var/forced = 0)
							if(!F || !(F in files))
								return 1
							if(writeprotect && !forced)
								return 0
