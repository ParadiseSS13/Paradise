...
obj
	machinery
		computer3
			...
			*/
			proc
				format_atomlist(var/list/atoms)
					var
						list
							output = list()
					for(var/atom/A in atoms)
						var
							title = "[A] (Range [get_dist(A,src)] meters, [dir2text(get_dir(src,A))])"
						output[title] = A
					return output
			...
			*/
			proc
				Crash(var/errorcode = PROG_CRASH)
					if(!src)
						return null
			...
			proc
				writefile(var/datum/file/F, var/where = ANY_DRIVE, var/required_location = 0)
					if(where != ANY_DRIVE)
