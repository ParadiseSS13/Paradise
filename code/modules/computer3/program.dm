...
datum
	file
		program
			...
			var
				desc				= "An unidentifiable program."

			var
				image
					overlay		= null							// Icon to be put on top of the computer frame.

			var
				active_state		= "generic"						// the icon_state that the computer goes to when the program is active

			drm						= 0								// prevents a program from being copied
			var
				refresh				= 0								// if true, computer does screen updates during process().
			var
				error				= 0								// set by background programs so an error pops up when used

			var
				human_controls		= 0								// if true, non-human animals cannot interact with this program (monkeys, xenos, etc)
			var
				ai_allowed			= 1								// if true, silicon mobs (AI/cyborg) are allowed to use this program.
			...
			var
				list
					req_access 	= list()						// requires all of these UNLESS below succeeds
			var
				list
					req_one_access = list()						// requires one of these
...
datum
	file
		program
			proc
				list_all_files_by_drive(var/typekey,var/linkop = "runfile")
					var
						dat = ""
