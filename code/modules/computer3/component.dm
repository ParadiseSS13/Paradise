			...
			computer
				...
				var
					list
						attackby_types	= list()
				proc
					allow_attackby(var/obj/item/I as obj,var/mob/user as mob, params)
		...
		part
			computer
				cardslot
					...
					var
						obj
							item
								weapon
									card
										writer	= null	// so that you don't need to typecast dual cardslots, but pretend it's not here
																				// alternately pretend they did it to save money on manufacturing somehow
					var
						dualslot = 0 // faster than typechecking
					...
					proc
						insert(var/obj/item/weapon/card/card, var/slot = 3)
							if(!computer)
								return 0
					...
					proc
						equip_to_reader(var/obj/item/weapon/card/card, var/mob/living/L)
							if(!reader)
								L.drop_item()
								card.loc = src
								reader = card
					...
					proc
						equip_to_writer(var/obj/item/weapon/card/card, var/mob/living/L)
							if(!writer && dualslot)
								L.drop_item()
								card.loc = src
								writer = card
					...
					proc
						remove_reader(var/mob/living/L)
							if(reader)
								reader.loc = loc
					...
					proc
						remove_writer(var/mob/living/L)
							if(writer && dualslot)
								writer.loc = loc
								if(istype(L) && !L.get_active_hand())
