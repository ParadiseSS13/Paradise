...
datum
	file
		...
		var
			volume = 10 // in KB
		var
			image = 'icons/ntos/file.png' // determines the icon to use, found in icons/ntos
		var
			obj
				machinery
					computer3
						computer // the parent computer, if fixed
		var
			obj
				item
					part
						computer
							storage
								device // the device that is containing this file
		var
			hidden_file = 0 // Prevents file from showing up on NTOS program list.
		var
			drm	= 0			// Copy protection, called by copy() and move()
		var
			readonly = 0	// Edit protection, called by edit(), which is just a failcheck proc
		...
		//
		proc
			copy(var/obj/item/part/computer/storage/dest)
		...
		//
		proc
			move(var/obj/item/part/computer/storage/dest)
				if(!computer || computer.crit_fail) return null
				if(drm)
					if(!computer.emagged)
						return null
	...
	file
		data
			...
			var
				file_increment	= 1
			var
				binary			= 0 // determines if the file can't be opened by editor

			// Set the content to a specific amount, increase filesize appropriately.
			proc
				set_content(var/text)
					content = text
					if(file_increment > 1)
		...
		data
			research
				...
				var
					datum
						tech
							stored // the actual tech contents
				volume = 1440
