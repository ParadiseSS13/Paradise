proc
	sql_report_karma(var/mob/spender, var/mob/receiver)
		var
			sqlspendername = sanitizeSQL(spender.name)
		var
			sqlspenderkey = spender.key
		var
			sqlreceivername = sanitizeSQL(receiver.name)
		var
			sqlreceiverkey = receiver.key
		var
			sqlreceiverrole = "None"
		var
			sqlreceiverspecial = "None"

		var
			sqlspenderip = spender.client.address
		...
		if(!dbcon.IsConnected())
			...
			var
				sqltime = time2text(world.realtime, "YYYY-MM-DD hh:mm:ss")
			var
				DBQuery
					query = dbcon.NewQuery("INSERT INTO [format_table_name("karma")] (spendername, spenderkey, receivername, receiverkey, receiverrole, receiverspecial, spenderip, time) VALUES ('[sqlspendername]', '[sqlspenderkey]', '[sqlreceivername]', '[sqlreceiverkey]', '[sqlreceiverrole]', '[sqlreceiverspecial]', '[sqlspenderip]', '[sqltime]')")
			if(!query.Execute())
				var
					err = query.ErrorMsg()
...
mob
	verb
		spend_karma_list()
			...
			var
				pickedmob = input("Who would you like to award Karma to?", "Award Karma", "Cancel") as null|mob in karma_list
...
mob
	verb
		spend_karma(var/mob/M)
			...
			var
				choice = input("Give [M.name] good karma?", "Karma") in list("Good", "Cancel")
			...
			if(M.client.karma <= -2 || M.client.karma >= 2)
				...
				var
					assigned_role = "None"
				var
					karma_diary = file("data/logs/karma_[time2text(world.realtime, "YYYY/MM-Month/DD-Day")].log")
				if(M.mind)
	...
	proc
		verify_karma()
			...
			if(!dbcon.IsConnected())
				...
				var
					DBQuery
						query = dbcon.NewQuery("SELECT karma, karmaspent FROM [format_table_name("karmatotals")] WHERE byondkey='[src.key]'")
				query.Execute()
	...
	proc
		karmashopmenu()
			...
			var
				datum
					browser
						popup = new(usr, "karmashop", "<div align='center'>Karma Shop</div>", 400, 400)
...
client
	proc
		DB_job_unlock(var/job,var/cost)
			var
				DBQuery
					query = dbcon.NewQuery("SELECT * FROM [format_table_name("whitelist")] WHERE ckey='[usr.key]'")
			query.Execute()
...
client
	proc
		DB_species_unlock(var/species,var/cost)
			var
				DBQuery
					query = dbcon.NewQuery("SELECT * FROM [format_table_name("whitelist")] WHERE ckey='[usr.key]'")
			query.Execute()
			...
			if(dbckey)
				...
				if(!(species in specieslist))
					specieslist += species
					var
						newspecieslist = jointext(specieslist,",")
...
client
	proc
		karmacharge(var/cost,var/refund = 0)
			var
				DBQuery
					query = dbcon.NewQuery("SELECT * FROM [format_table_name("karmatotals")] WHERE byondkey='[usr.key]'")
			query.Execute()

			while(query.NextRow())
				var
					spent = text2num(query.item[4])
				if(refund)
					spent -= cost
				else
					spent += cost
...
client
	proc
		karmarefund(var/type,var/name,var/cost)
...
client
	proc
		checkpurchased(var/name = null) // If the first parameter is null, return a full list of purchases
			var
				DBQuery
					query = dbcon.NewQuery("SELECT * FROM [format_table_name("whitelist")] WHERE ckey='[usr.key]'")
			query.Execute()
			...
			if(dbckey)
				...
				var
					list
						combinedlist = joblist + specieslist
				if(name)
