...
datum
	module
		var
			status				// bits set if working, 0 if broken
		var
			installed			// bits set if installed, 0 if missing
...
datum
	moduletypes
		var
			list
				modcount = list()	// assoc list of the count of modules for a type


var
	list
		modules = list(			// global associative list
...
datum
	module
		New(var/obj/O)
			...
			var
				type = O.type		// the type of the creating object

			var
				mneed = mods.inmodlist(type)		// find if this type has modules defined
			...
			var
				needed = mods.getbitmask(type)		// get a bitmask for the number of modules in this object
			status = needed
			installed = needed
...
datum
	moduletypes
		proc
			addmod(var/type, var/modtextlist)
