...
obj
	effect
		datacore
			proc
				manifest_modify(var/name, var/assignment)
					if(PDA_Manifest.len)
						PDA_Manifest.Cut()
					var
						datum
					...
					var
						list
							all_jobs = get_job_datums()

					for(var/datum/job/J in all_jobs)
						var
							list
								alttitles = get_alternate_titles(J.title)
						if(!J)	continue
			...
			proc
				manifest_inject(var/mob/living/carbon/human/H)
					...
					if(H.mind && (H.mind.assigned_role != "MODE"))
						...
						var
							id = add_zero(num2hex(rand(1, 1.6777215E7)), 6)	//this was the best they could come up with? A large random number? *sigh*
...
*/
	...
	var
		icon
			face_s = new/icon("icon" = 'icons/mob/human_face.dmi', "icon_state" = "bald_s")
	if(!(H.species.bodyflags & NO_EYES))
		var
			icon
				eyes_s = new/icon("icon" = 'icons/mob/human_face.dmi', "icon_state" = H.species ? H.species.eyes : "eyes_s")
	...
	if(H.species.bodyflags & HAS_MARKINGS)
		var
			datum
				sprite_accessory
					marking_style = marking_styles_list[H.m_style]
