	...
	effect
		proc_holder
			spell
				targeted
					lichdom
						cast(list/targets,mob/user = usr)	
							...
							for(var/mob/M in targets)
								...
								if(stat_allowed) //Death is not my end!
									...
									var
										turf
											user_turf = get_turf(M)
									var
										turf
											item_turf = get_turf(marked_item)
									...
									var
										mob
											living
												carbon
													human
														lich = new /mob/living/carbon/human(item_turf)
									...
									if(old_body && old_body.loc)
										if(iscarbon(old_body))
											...
											for(var/obj/item/W in C)
												C.unEquip(W)
										var
											wheres_wizdo = dir2text(get_dir(body_turf, item_turf))
										if(wheres_wizdo)
