...
/obj/effect/proc_holder/spell/targeted/magnet/cast(list/targets, mob/user = usr)
	...
	switch(energy)
		if(0 to 25)
			var/target_hand = rand(1, target.held_items.len)
			for(var/obj/item/I in target.get_item_for_held_index(target_hand))
				if(I.flags & CONDUCT)
					I.throw_at(user, I.throw_range, 4, target)
			playsound(get_turf(target), 'sound/machines/defib_zap.ogg', 50, 1, -1)

		if(25 to 75)
			for(var/hand_index in 1 to target.held_items.len)
				for(var/obj/item/I in target.get_item_for_held_index(hand_index))
					if(I.flags & CONDUCT)
						I.throw_at(user, I.throw_range, 4, target)
			playsound(get_turf(target), 'sound/machines/defib_zap.ogg', 50, 1, -1)
