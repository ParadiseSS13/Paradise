...
/mob/living/carbon/human/proc/mimetouched()
	...
	if(iswizard(src) || (mind && mind.special_role == SPECIAL_ROLE_WIZARD_APPRENTICE)) //Wizards get non-cursed mime outfit. Replace with mime robes if we add those.
		...
		if(src.dna.species.name == "Plasmaman")
			unEquip(head, TRUE)
			equip_to_slot_if_possible(new /obj/item/clothing/head/helmet/space/plasmaman/mime, slot_head, TRUE, TRUE)
			equip_to_slot_if_possible(new /obj/item/clothing/under/plasmaman/mime, slot_w_uniform, TRUE, TRUE)
		else
		...
		if(src.dna.species.name == "Plasmaman")
			qdel(head, TRUE)
			equip_to_slot_if_possible(new /obj/item/clothing/head/helmet/space/plasmaman/mime/nodrop, slot_head, TRUE, TRUE)
			equip_to_slot_if_possible(new /obj/item/clothing/under/plasmaman/mime/nodrop, slot_w_uniform, TRUE, TRUE)
		else
