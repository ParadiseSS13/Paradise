...
/datum/admin_ai_template/proc/apply(mob/living/target, client/user)
	if(QDELETED(target) || !isliving(target))
		to_chat(user, SPAN_NOTICE("Invalid target for AI controller."))
		return
...
/datum/admin_ai_template/hostile_ranged/apply_controller(mob/living/target, client/user)
	...
	// TODO: /tg/ adds /datum/component/ranged_mob_full_auto here, and we probably should too at some point
...
/datum/admin_ai_template/ability/gather_information(mob/living/target, client/user)
	...
	var/static/list/all_mob_actions = sortTim(subtypesof(/datum/action/cooldown/mob_cooldown), GLOBAL_PROC_REF(cmp_typepaths_asc))
