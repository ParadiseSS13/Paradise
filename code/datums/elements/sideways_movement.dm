...
/datum/element/sideways_movement

/datum/element/sideways_movement/Attach(atom/movable/target)
	. = ..()
	if(!ismovable(target))
...
/datum/element/sideways_movement/proc/on_client_move(atom/movable/source, direction, old_dir)
	SIGNAL_HANDLER
	on_move(source, direction, old_dir)

/datum/element/sideways_movement/proc/on_moved_from_loop(atom/movable/source, datum/move_loop/loop, old_dir, direction)
	SIGNAL_HANDLER
	if(!CHECK_MOVE_LOOP_FLAGS(source, MOVEMENT_LOOP_NO_DIR_UPDATE))
		on_move(source, direction, old_dir)

/// retain the old dir unless walking straight ahead or backward, in which case rotate the dir left or right by a 90°
/datum/element/sideways_movement/proc/on_move(atom/movable/source, direction, old_dir)
