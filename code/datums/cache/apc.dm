...
datum
	repository
		apc
			proc
				apc_data(var/obj/machinery/computer/monitor/powermonitor)
					var
						apcData[0]
					...
					if(powermonitor && !isnull(powermonitor.powernet))
						...
						var
							list
								Status = list("Off","AOff","On","AOn") // Status:  off, auto-off, on, auto-on
						var
							list
								chg = list("N","C","F") // Charging: no, charging, full
