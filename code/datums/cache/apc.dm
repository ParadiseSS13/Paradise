...
/datum/repository/apc/proc/apc_data(var/obj/machinery/computer/monitor/powermonitor)
	...
	if (powermonitor && !isnull(powermonitor.powernet))
		...
		for(var/obj/machinery/power/apc/A in L)
			apcData[++apcData.len] = list("Name" = lhtml_encode(A.area.name), "Equipment" = Status[A.equipment+1], "Lights" = Status[A.lighting+1], "Environment" = Status[A.environ+1], "CellPct" = A.cell ? round(A.cell.percent(),1) : "M", "CellStatus" = A.cell ? chg[A.charging+1] : "M", "Load" = round(A.lastused_total,1))
