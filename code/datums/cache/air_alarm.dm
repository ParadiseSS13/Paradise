var
	global
		datum
			repository
				air_alarm
					air_alarm_repository = new()

datum
	repository
		air_alarm
			proc
				air_alarm_data(var/list/monitored_alarms, var/refresh = 0, var/obj/machinery/alarm/passed_alarm)
					var
						alarms[0]
...
datum
	repository
		air_alarm
			proc
				update_cache(var/obj/machinery/alarm/alarm)
