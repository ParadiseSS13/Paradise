...
var
	const
		CAMERA_WIRE_NOTHING1 = 16
var
	const
		CAMERA_WIRE_NOTHING2 = 32
			...
			UpdateCut(var/index, var/mended)
				...
				switch(index)
					if(CAMERA_WIRE_FOCUS)
						var
							range = (mended ? initial(C.view_range) : C.short_range)
		...
		camera
			UpdatePulsed(var/index)
				...
				switch(index)
					if(CAMERA_WIRE_FOCUS)
						var
							new_range = (C.view_range == initial(C.view_range) ? C.short_range : initial(C.view_range))
