...
var
	list
		wireColours = list("red", "blue", "green", "black", "orange", "brown", "gold", "gray", "cyan", "navy", "purple", "pink")

datum
	wires

		var
			random = 0 // Will the wires be different for every single instance.
		var
			atom
				holder = null // The holder
		var
			holder_type = null // The holder type; used to make sure that the holder is the correct type.
		var
			wire_count = 0 // Max is 16
		var
			wires_status = 0 // BITFLAG OF WIRES
	...
	wires
		proc
			GenerateWires()
				var
					list
						colours_to_pick = wireColours.Copy() // Get a copy, not a reference.
				var
					list
						indexes_to_pick = list()
				...
				while(colours_to_pick.len && indexes_to_pick.len)
					...
					var
						colour = pick_n_take(colours_to_pick)

					// Pick and remove an index
					var
						index = pick_n_take(indexes_to_pick)
		...
		proc
			Interact(var/mob/living/user)
				...
				var
					datum
						browser
							popup = new(user, "wires", holder.name, window_x, window_y)
...
datum
	wires
		proc
			PulseColour(var/colour)
				PulseIndex(GetIndex(colour))

datum
	wires
		proc
			PulseIndex(var/index)
				if(IsIndexCut(index))
					return
				UpdatePulsed(index)
...
datum
	wires
		proc
			IsColourCut(var/colour)
				var
					index = GetIndex(colour)
				return IsIndexCut(index)

datum
	wires
		proc
			IsIndexCut(var/index)
...
datum
	wires
		proc
			Attach(var/colour, var/obj/item/device/assembly/signaler/S)
				if(colour && S)
					if(!IsAttached(colour))
						signallers[colour] = S
						S.loc = holder
...
datum
	wires
		proc
			Pulse(var/obj/item/device/assembly/signaler/S)
