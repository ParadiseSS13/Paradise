...
/datum/holocall/New(mob/living/caller, obj/machinery/hologram/holopad/calling_pad, list/callees)
	...
	for(var/I in callees)
		...
		if(!QDELETED(H) && !(H.stat & NOPOWER) && H.anchored)
			...
			H.atom_say("[area] pad beeps: Llamada entrante de [caller]!")

	if(!dialed_holopads.len)
		calling_holopad.atom_say("Fallo de coneccion.")
		qdel(src)
		return
...
/datum/holocall/proc/Disconnect(obj/machinery/hologram/holopad/H)
	if(H == connected_holopad)
		...
		calling_holopad.atom_say("[A] holopad desconectado.")
	else if(H == calling_holopad && connected_holopad)
		connected_holopad.atom_say("[user] desconectado.")
...
/datum/holocall/proc/ConnectionFailure(obj/machinery/hologram/holopad/H, graceful = FALSE)
	...
	if(!dialed_holopads.len)
		if(graceful)
			calling_holopad.atom_say("Llamada rechazada.")
		qdel(src)
...
/datum/holocall/proc/Check()
	...
	if(.)
		if(!connected_holopad)
			...
			if(!.)
				calling_holopad.atom_say("Sin respuesta recibida.")
