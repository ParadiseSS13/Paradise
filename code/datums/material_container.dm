...
datum
	material_container
		New(obj/O, list/mat_list, max_amt = 0)
			owner = O
...
datum
	material_container
		proc
			insert_amount(amt, material_type = null)
...
datum
	material_container
		proc
			insert_stack(obj/item/stack/S, amt = 0)
				if(amt <= 0)
					return 0
				if(amt > S.amount)
					amt = S.amount
				var
					material_amt = get_item_material_amount(S)
				if(!material_amt)
					return 0
...
datum
	material_container
		proc
			insert_item(obj/item/I, multiplier = 1)
				if(!I)
					return 0
				...
				var
					material_amount = get_item_material_amount(I)
...
datum
	material_container
		proc
			insert_materials(obj/item/I, multiplier = 1) //for internal usage only
				var
					datum
						material
							M
...
datum
	material_container
		proc
			use_amount(list/mats, multiplier=1)
				if(!mats || !mats.len)
					return 0
...
datum
	material_container
		proc
			use_amount_type(amt, material_type)
				var
					datum
						material
							M
...
datum
	material_container
		proc
			retrieve(sheet_amt, datum/material/M)
				if(sheet_amt > 0)
...
datum
	material_container
		proc
			retrieve_sheets(sheet_amt, material_type)
...
datum
	material_container
		proc
			retrieve_amount(amt, material_type)
...
datum
	material_container
		proc
			has_materials(list/mats, multiplier=1)
				if(!mats || !mats.len)
					return 0
...
datum
	material_container
		proc
			get_item_material_amount(obj/item/I)
				if(!istype(I))
					return 0
				var
					material_amount = 0
