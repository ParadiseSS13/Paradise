var
	global
		const
			base_law_type = /datum/ai_laws/nanotrasen
...
datum
	ai_laws
		proc
			sync(var/mob/living/silicon/S, var/full_sync = 1)
...
mob
	living
		silicon
			proc
				sync_zeroth(var/datum/ai_law/zeroth_law, var/datum/ai_law/zeroth_law_borg)
...
datum
	ai_laws
		proc
			set_zeroth_law(var/law, var/law_borg = null)
				if(!law)
					return
	...
	ai_laws
		proc
			add_ion_law(var/law)
				...
				var
					new_law = new/datum/ai_law/ion(law)
				ion_laws += new_law
	...
	ai_laws
		proc
			add_inherent_law(var/law)
				...
				var
					new_law = new/datum/ai_law/inherent(law)
...
datum
	ai_laws
		proc
			add_supplied_law(var/number, var/law)
				if(!law)
					return
				...
				if(supplied_laws.len >= number)
					var
						datum
							ai_law
								existing_law = supplied_laws[number]
				...
				var
					new_law = new/datum/ai_law/supplied(law, number)
...
datum
	ai_laws
		proc
			delete_law(var/datum/ai_law/law)
				if(istype(law))
					law.delete_law(src)
...
datum
	ai_law
		proc
			delete_law(var/datum/ai_laws/laws)
...
datum
	ai_law
		supplied
			delete_law(var/datum/ai_laws/laws)
				var
					index = laws.supplied_laws.Find(src)
				if(index)
...
datum
	ai_laws
		proc
			internal_delete_law(var/list/datum/ai_law/laws, var/list/state, var/list/datum/ai_law/law)
				var
					index = laws.Find(law)
				if(index)
					laws -= law
...
datum
	ai_laws
		proc
			get_state_law(var/datum/ai_law/law)
				return law.get_state_law(src)

datum
	ai_law
		proc
			get_state_law(var/datum/ai_laws/laws)
...
datum
	ai_laws
		proc
			get_state_internal(var/list/datum/ai_law/laws, var/list/state, var/list/datum/ai_law/law)
				var
					index = laws.Find(law)
				if(index)
					return state[index]
...
datum
	ai_laws
		proc
			set_state_law(var/datum/ai_law/law, var/state)
...
datum
	ai_law
		zero
			set_state_law(var/datum/ai_laws/laws, var/state)
				if(src == laws.zeroth_law)
...
datum
	ai_laws
		proc
			set_state_law_internal(var/list/datum/ai_law/laws, var/list/state, var/list/datum/ai_law/law, var/do_state)
				var
					index = laws.Find(law)
				if(index)
