...
/datum/component/defib
	...
	var/actual_unit_uid
	///How long the defib will take before speed multipliers
	var/defib_time = 7 SECONDS
	...
	///Sound when the defib shocks the victim
	var/zap_sound = 'sound/machines/defib_release.ogg'
	///Sound when the defibs safety is enabled
	var/safety_on_sound = 'sound/machines/defib_safety_on.ogg'
	///Sound when the defibs safety is disabled
	var/safety_off_sound = 'sound/machines/defib_safety_off.ogg'
...
/datum/component/defib/proc/on_emp(obj/item/unit)
	...
	if(safety)
		...
		playsound(get_turf(unit), safety_off_sound, 50, 0)
	else
		safety = TRUE
		unit.visible_message("<span class='notice'>[unit] beeps: Safety protocols enabled!</span>")
		playsound(get_turf(unit), safety_on_sound, 50, 0)
...
/datum/component/defib/proc/defibrillate(mob/living/user, mob/living/carbon/human/target)
	...
	if(application_result & COMPONENT_BLOCK_DEFIB_DEAD)
		...
		playsound(get_turf(defib_ref), fail_sound, 50, 0)
		return
	...
	playsound(get_turf(defib_ref), charge_sound, 50, 0)
	...
	if(target.undergoing_cardiac_arrest()) // Can have a heart attack and heart is either missing, necrotic, or not beating
		...
		playsound(get_turf(defib_ref), success_sound, 50, 0)
		busy = FALSE
		return
	...
	if(!defib_success)
		...
		if(target.getBrainLoss() >= 100)
			...
			playsound(get_turf(defib_ref), safety_off_sound, 50, 0)
