...
/atom/movable/screen/alert/status_effect/internal_pinpointer
	name = "Target Integrated Pinpointer"
	desc = "A completely undetectable implant, giving you  a sixth sense to aid you in locating your target."
	icon = 'icons/obj/device.dmi'
	icon_state = "pinon"

/datum/status_effect/internal_pinpointer
	id = "internal_pinpointer"
	duration = -1
	tick_interval = PINPOINTER_PING_TIME
	alert_type = /atom/movable/screen/alert/status_effect/internal_pinpointer
...
/datum/status_effect/internal_pinpointer/tick()
	if(!owner)
		qdel(src)
		return
	scan_for_target()
...
/datum/status_effect/internal_pinpointer/proc/point_to_target()
	if(!scan_target)
...
/datum/status_effect/internal_pinpointer/proc/scan_for_target()
	scan_target = null
	if(!owner && !owner.mind)
		return
...
/datum/status_effect/internal_pinpointer/paradox_stalking/tick()
	scan_target = target_brain.owner
	if(!owner || !target_brain)
		qdel(src)
		owner.remove_status_effect(/datum/status_effect/internal_pinpointer/paradox_stalking)
		return
	point_to_target()

/datum/status_effect/internal_pinpointer/paradox_stalking/scan_for_target()
	return
