...
datum
	mind
		...
		var
			mob
				living
					original	//TODO: remove.not used in any meaningful way ~Carn. First I'll need to tweak the way silicon-mobs handle minds.
		var
			active = 0
		...
		var
			list
				spell_list = list() // Wizard mode & "Give Spell" badmin button.
		...
		var
			has_been_rev = 0//Tracks if this mind has been a rev or not

		var
			miming = 0 // Mime's vow of silence
		var
			datum
				faction
		...
		var
			datum
				changeling
					changeling		//changeling holder
		var
			datum
				vampire
					vampire			//vampire holder
		var
			datum
				nations
					nation			//nation holder
		var
			datum
				abductor
					abductor			//abductor holder

		var
			antag_hud_icon_state = null //this mind's ANTAG_HUD should have this icon_state
		var
			datum
				atom_hud
					antag
						antag_hud = null //this mind's antag HUD
		var
			datum
				mindslaves
					som //stands for slave or master...hush..
...
datum
	mind
		proc
			transfer_to(mob/living/new_character)
				...
				var
					datum
						atom_hud
							antag
								hud_to_transfer = antag_hud//we need this because leave_hud() will clear this list
...
datum
	mind
		proc
			show_memory(mob/recipient)
				var
					output = "<B>[current.real_name]'s Memory</B><HR>"
				output += memory
	...
	mind
		proc
			edit_memory()
				...
				var
					out = "<B>[name]</B>[(current&&(current.real_name!=name))?" (as [current.real_name])":""]<br>"
	...
	mind
		Topic(href, href_list)
			...
			if(href_list["role_edit"])
				var
					new_role = input("Select new role", "Assigned role", assigned_role) as null|anything in joblist
				if(!new_role) return
				...
				var
					new_memo = copytext(input("Write new memory", "Memory", memory) as null|message,1,MAX_MESSAGE_LEN)
				...
				if(href_list["obj_edit"])
					...
					var
						temp_obj_type = "[objective.type]"//Convert path into a text string.
					def_value = copytext(temp_obj_type, 19)//Convert last part of path into an objective keyword.
					if(!def_value)//If it's a custom objective, it will be an empty string.
						def_value = "custom"

				var
					new_obj_type = input("Select objective type:", "Objective type", def_value) as null|anything in list("assassinate", "blood", "debrain", "protect", "prevent", "brig", "hijack", "escape", "survive", "steal", "download", "nuclear", "capture", "absorb", "destroy", "maroon", "identity theft", "custom")
				...
				switch(new_obj_type)
					if("assassinate","protect","debrain", "brig", "maroon")
						...
						var
							objective_type_capital = uppertext(copytext(new_obj_type, 1,2))//Capitalize first letter.
						var
							objective_type_text = copytext(new_obj_type, 2)//Leave the rest of the text.
						var
							objective_type = "[objective_type_capital][objective_type_text]"//Add them together into a text string.

						var
							list
								possible_targets = list("Free objective")
						...
						var
							mob
								def_target = null
						var
							objective_list[] = list(/datum/objective/assassinate, /datum/objective/protect, /datum/objective/debrain)
						...
						var
							objective_path = text2path("/datum/objective/[new_obj_type]")
					...
					if("download","capture","absorb", "blood")
						...
						var
							target_number = input("Input target number:", "Objective", def_num) as num|null
					...
					if("custom")
						var
							expl = sanitize(copytext(input("Custom objective:", "Objective", objective ? objective.explanation_text : "") as text|null,1,MAX_MESSAGE_LEN))
						if(!expl) return
				...
				var
					datum
						objective
							objective = locate(href_list["obj_delete"])
				...
				var
					datum
						objective
							objective = locate(href_list["obj_completed"])
				if(!istype(objective))	return
							...
							obj
								item
									weapon
										implant
											loyalty
												L = new/obj/item/weapon/implant/loyalty(H)
						L.imp_in = H
						L.implanted = 1
					...
					if("headrev")
						...
						if(ticker.mode.head_revolutionaries.len>0)
							...
							var
								datum
									mind
										valid_head = locate() in ticker.mode.head_revolutionaries
							if(valid_head)
				...
				switch(href_list["cult"])
					...
					if("tome")
						...
						if(istype(H))
							...
							var
								where = H.equip_in_one_of_slots(T, slots)
							if(!where)
				...
				switch(href_list["abductor"])
					...
					if("equip")
						var
							gear = alert("Agent or Scientist Gear","Gear","Agent","Scientist")
						if(gear)
							var
								datum
									game_mode
...
datum
	mind
		proc
			make_Abductor()
				var
					role = alert("Abductor Role ?","Role","Agent","Scientist")
				var
					team = input("Abductor Team ?","Team ?") in list(1,2,3,4)
				var
					teleport = alert("Teleport to ship ?","Teleport","Yes","No")
	...
	mind
		proc
			is_brigged(duration)
				...
				var
					is_currently_brigged = current.is_in_brig()
...
datum
	mind
		proc
			AddSpell(var/obj/effect/proc_holder/spell/spell)
				spell_list += spell
				if(!spell.action)
...
datum
	mind
		proc
			transfer_actions(var/mob/living/new_character)
...
datum
	mind
		proc
			transfer_mindbound_actions(var/mob/living/new_character)
...
datum
	mind
		proc
			get_ghost(even_if_they_cant_reenter)
			...
			grab_ghost(force)
				var
					mob
						dead
							observer
								G = get_ghost(even_if_they_cant_reenter = force)
				. = G
				if(G)
					G.reenter_corpse()
