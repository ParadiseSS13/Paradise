...
datum
	gas_mixture
		proc
			heat_capacity()
				var
					heat_capacity = HEAT_CAPACITY_CALCULATION(oxygen,carbon_dioxide,nitrogen,toxins)
	...
	gas_mixture
		proc
			heat_capacity_archived()
				var
					heat_capacity_archived = HEAT_CAPACITY_CALCULATION(oxygen_archived,carbon_dioxide_archived,nitrogen_archived,toxins_archived)
...
datum
	gas_mixture
		proc
			total_moles()
				var
					moles = oxygen + carbon_dioxide + nitrogen + toxins
	...
	gas_mixture
		proc
			react(atom/dump_location)
				var
					reacting = 0 //set to 1 if a notable reaction occured (used by pipe_network)
						...
						if(toxins > MINIMUM_HEAT_CAPACITY && carbon_dioxide > MINIMUM_HEAT_CAPACITY)
							var
								datum
									gas
										oxygen_agent_b
											trace_gas = locate(/datum/gas/oxygen_agent_b/) in trace_gases
							if(trace_gas)
								var
									reaction_rate = min(carbon_dioxide*0.75, toxins*0.25, trace_gas.moles*0.05)
			...
			fire()
				...
				var
					datum
						gas
							volatile_fuel
								fuel_store = locate(/datum/gas/volatile_fuel/) in trace_gases
				...
				if(energy_released > 0)
					var
						new_heat_capacity = heat_capacity()
...
datum
	gas_mixture
		proc
			mimic(turf/model) //I want this proc to die a painful death
...
datum
	gas_mixture
		proc
			check_turf(turf/model) //I want this proc to die a painful death
				//Returns: 0 if self-check failed or 1 if check passes

datum
	gas_mixture
		proc
			temperature_mimic(turf/model, conduction_coefficient) //I want this proc to die a painful death
	...
	gas_mixture
		merge(datum/gas_mixture/giver)
			...
			if(abs(temperature-giver.temperature)>MINIMUM_TEMPERATURE_DELTA_TO_CONSIDER)
				var
					self_heat_capacity = heat_capacity()
				var
					giver_heat_capacity = giver.heat_capacity()
				var
...
datum
	gas_mixture
		check_turf(turf/model, atmos_adjacent_turfs = 4)
			var
				delta_oxygen = (oxygen_archived - model.oxygen)/(atmos_adjacent_turfs+1)
			var
				delta_carbon_dioxide = (carbon_dioxide_archived - model.carbon_dioxide)/(atmos_adjacent_turfs+1)
			var
				delta_nitrogen = (nitrogen_archived - model.nitrogen)/(atmos_adjacent_turfs+1)
			var
				delta_toxins = (toxins_archived - model.toxins)/(atmos_adjacent_turfs+1)

			var
				delta_temperature = (temperature_archived - model.temperature)
...
datum
	gas_mixture
		proc
			check_turf_total(turf/model) //I want this proc to die a painful death
				var
					delta_oxygen = (oxygen - model.oxygen)
				var
					delta_carbon_dioxide = (carbon_dioxide - model.carbon_dioxide)
				var
					delta_nitrogen = (nitrogen - model.nitrogen)
				var
					delta_toxins = (toxins - model.toxins)

				var
					delta_temperature = (temperature - model.temperature)
...
datum
	gas_mixture
		share(datum/gas_mixture/sharer, atmos_adjacent_turfs = 4)
			if(!sharer)	return 0
			var
				delta_oxygen = QUANTIZE(oxygen_archived - sharer.oxygen_archived)/(atmos_adjacent_turfs+1)
			var
				delta_carbon_dioxide = QUANTIZE(carbon_dioxide_archived - sharer.carbon_dioxide_archived)/(atmos_adjacent_turfs+1)
			var
				delta_nitrogen = QUANTIZE(nitrogen_archived - sharer.nitrogen_archived)/(atmos_adjacent_turfs+1)
			var
				delta_toxins = QUANTIZE(toxins_archived - sharer.toxins_archived)/(atmos_adjacent_turfs+1)

			var
				delta_temperature = (temperature_archived - sharer.temperature_archived)
			...
			if(abs(delta_temperature) > MINIMUM_TEMPERATURE_DELTA_TO_CONSIDER)
				...
				if(delta_air)
					var
						air_heat_capacity = SPECIFIC_HEAT_AIR*delta_air
					if(delta_air > 0)
				...
				if(delta_carbon_dioxide)
					var
						carbon_dioxide_heat_capacity = SPECIFIC_HEAT_CDO*delta_carbon_dioxide
				...
				if(delta_toxins)
					var
						toxins_heat_capacity = SPECIFIC_HEAT_TOXIN*delta_toxins
					if(delta_toxins > 0)
			...
			if((delta_temperature > MINIMUM_TEMPERATURE_TO_MOVE) || abs(moved_moles) > MINIMUM_MOLES_DELTA_TO_MOVE)
				var
					delta_pressure = temperature_archived*(total_moles() + moved_moles) - sharer.temperature_archived*(sharer.total_moles() - moved_moles)
				return delta_pressure*R_IDEAL_GAS_EQUATION/volume

datum
	gas_mixture
		mimic(turf/model, atmos_adjacent_turfs = 4)
			var
				delta_oxygen = QUANTIZE(oxygen_archived - model.oxygen)/(atmos_adjacent_turfs+1)
			var
				delta_carbon_dioxide = QUANTIZE(carbon_dioxide_archived - model.carbon_dioxide)/(atmos_adjacent_turfs+1)
			var
				delta_nitrogen = QUANTIZE(nitrogen_archived - model.nitrogen)/(atmos_adjacent_turfs+1)
			var
				delta_toxins = QUANTIZE(toxins_archived - model.toxins)/(atmos_adjacent_turfs+1)
			...
			if((delta_temperature > MINIMUM_TEMPERATURE_TO_MOVE) || abs(moved_moles) > MINIMUM_MOLES_DELTA_TO_MOVE)
				var
					delta_pressure = temperature_archived*(total_moles() + moved_moles) - model.temperature*(model.oxygen+model.carbon_dioxide+model.nitrogen+model.toxins)
				return delta_pressure*R_IDEAL_GAS_EQUATION/volume
			else
				return 0
...
datum
	gas_mixture
		temperature_share(datum/gas_mixture/sharer, conduction_coefficient)
			...
			if(abs(delta_temperature) > MINIMUM_TEMPERATURE_DELTA_TO_CONSIDER)
				var
					self_heat_capacity = heat_capacity_archived()
				var
...
datum
	gas_mixture
		temperature_turf_share(turf/simulated/sharer, conduction_coefficient)
			var
				delta_temperature = (temperature_archived - sharer.temperature)
			if(abs(delta_temperature) > MINIMUM_TEMPERATURE_DELTA_TO_CONSIDER)
				var
					self_heat_capacity = heat_capacity()
...
datum
	gas_mixture
		proc
			get_true_breath_pressure(breath_pp)
