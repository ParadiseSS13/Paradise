	...
	construction
		...
		var
			list
				steps_desc
		var
			taskpath = null // Path of job objective completed.
		...
		proc
			check_step(atom/used_atom,mob/user as mob) //check last step only
				var
					valid_step = is_right_key(used_atom)
				if(valid_step)
		...
		proc
			is_right_key(atom/used_atom) // returns current step num if used_atom is of the right type.
				var
					list
						L = steps[steps.len]
		...
		proc
			custom_action(step, used_atom, user)
		...
		proc
			check_all_steps(atom/used_atom,mob/user as mob) //check all steps, remove matching one.
				...
				if(result)
					if(taskpath)
						var
							datum
								job_objective
									task = user.mind.findJobTask(taskpath)
						if(istype(task))
							task.unit_completed()
		...
		proc
			try_consume(mob/user as mob, atom/used_atom, amount)
				if(amount>0)
					// STACKS
...
datum
	construction
		reversible
			...
			proc
				update_index(diff as num, mob/user as mob)
					index+=diff
					if(index==0)
						spawn_result(user)
					else
...
datum
	construction
		reversible2
			...
			check_step(atom/used_atom,mob/user as mob)
				var
					diff = is_right_key(user,used_atom)
				if(diff)
					if(custom_action(index, diff, used_atom, user))
						update_index(diff,user)
						update_icon()
			...
			custom_action(index, diff, used_atom, var/mob/user)
				...
				var
					list
						state = step[diff==FORWARD ? state_next : state_prev]
				...
				if("delete" in state)
					...
					var
						spawntype=state["spawn"]
					var
						atom
							A = new spawntype(holder.loc)
					if("amount" in state)
