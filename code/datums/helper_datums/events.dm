...
datum
	events
		...
		proc
			addEventType(event_type as text)
		...
		proc
			addEvent(event_type as text, proc_holder, proc_name as text)
				...
				var
					datum
						events_event
							E = new /datum/events_event(proc_holder,proc_name)
				event += E
				return E
...
//		to_chat(world, "Events in [args[1]] called")
		var
			list
				event = listgetindex(events,args[1])
		if(istype(event))
			spawn(-1)
...
// Arguments: event_type as text, E as /datum/events_event
	...
	proc
		clearEvent(event_type as text, datum/events_event/E)
			if(!event_type || !E)
				return
			var
				list
					event = listgetindex(events,event_type)
			event -= E
			return 1
