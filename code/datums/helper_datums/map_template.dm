...
/datum/map_template/proc/load(turf/T, centered = 0)
	...
	try
		...
		SSair.synchronize(CALLBACK(src, PROC_REF(late_setup_level), block(bot_left, top_right), block(ST_bot_left, ST_top_right)))
		...
		throw e
	GLOB.space_manager.remove_dirt(placement.z)
	SSair.synchronize(CALLBACK(src, GLOBAL_PROC_REF(late_setup_level),
