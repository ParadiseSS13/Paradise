...
/datum/award/achievement
	desc = "An Achievement for Excellent People!"
...
/datum/award/achievement/get_ui_data(list/award_data, datum/achievement_data/holder)
	. = ..()
	.["achieve_info"] = "[times_achieved] player[times_achieved == 1 ? "" : "s"] earned this."

	if(!SSachievements.most_unlocked_achievement)
		.["achieve_tooltip"] = "No one has unlocked this achievement yet. Be the first!"
		return

	if(SSachievements.most_unlocked_achievement == src)
		.["achieve_tooltip"] = "This achievement was unlocked the most."
		return

	var/percent = FLOOR(times_achieved / SSachievements.most_unlocked_achievement.times_achieved * 100, 0.01)
	.["achieve_tooltip"] = "[(times_achieved && !percent) ? "Less than 0.01" : percent]% of players unlocked: \"[SSachievements.most_unlocked_achievement.name])\""
...
/datum/award/achievement/on_unlock(mob/user)
	. = ..()
	to_chat(user, SPAN_GREENANNOUNCE(SPAN_BOLD("Achievement Unlocked: [name]!")))
...
/datum/award/score
	desc = "You've done this waaaaaay too much."
