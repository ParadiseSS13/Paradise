...
datum
	recipe
		var
			list
				reagents	// example: = list("berryjuice" = 5) // do not list same reagent twice
		var
			list
				items		// example: = list(/obj/item/weapon/crowbar, /obj/item/weapon/welder) // place /foo/bar before /foo
		var
			list
				fruit		// example: = list("fruit" = 3)
		var
			result			// example: = /obj/item/weapon/reagent_containers/food/snacks/donut/normal
		var
			time = 100		// 1/10 part of second
		var
			byproduct		// example: = /obj/item/weapon/kitchen/mould		// byproduct to return, such as a mould or trash


datum
	recipe
		proc
			check_reagents(var/datum/reagents/avail_reagents) //1=precisely, 0=insufficiently, -1=superfluous
				. = 1
	...
	select_recipe(var/list/datum/recipe/avaiable_recipes, var/obj/obj as obj, var/exact = 1 as num)
		...
		if(possible_recipes.len==0)
			...
			for(var/datum/recipe/recipe in possible_recipes)
				var
					count = ((recipe.items)?(recipe.items.len):0) + ((recipe.reagents)?(recipe.reagents.len):0) + ((recipe.fruit)?(recipe.fruit.len):0)
