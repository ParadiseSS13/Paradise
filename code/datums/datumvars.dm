...
client
	proc
		debug_variables(datum/D in world)
			set category = "Debug"
			...
			var
				formatted_type = text("[D.type]")
			if(length(formatted_type) > 25)
				var
					middle_point = length(formatted_type) / 2
				var
					splitpoint = findtext(formatted_type,"/",middle_point)
				if(splitpoint)
...
client
	proc
		debug_variable(name, value, level, var/datum/DA = null)
			var
				html = ""
	...
	proc
		view_var_Topic(href, href_list, hsrc)
			...
			//~CARN: for renaming mobs (updates their name, real_name, mind.name, their ID/PDA and datacore records).
				...
				var
					mob
						M = locate(href_list["rename"])
				if(!istype(M))
					to_chat(usr, "This can only be used on instances of type /mob")
					return

				var
					new_name = sanitize(copytext(input(usr,"What would you like to name this mob?","Input a name",M.real_name) as text|null,1,MAX_NAME_LEN))
				...
				var
					D = locate(href_list["datumchange"])
				if(!istype(D,/datum) && !istype(D,/client))
					to_chat(usr, "This can only be used on instances of types /client or /datum")
					return
				...
				var
					mob
						M = locate(href_list["mob_player_panel"])
				if(!istype(M))
					to_chat(usr, "This can only be used on instances of type /mob")
					return
				...
				var
					mob
						M = locate(href_list["give_disease"])
				if(!istype(M))
					to_chat(usr, "This can only be used on instances of type /mob")
					return
				...
				var
					mob
						M = locate(href_list["drop_everything"])
				if(!istype(M))
					to_chat(usr, "This can only be used on instances of type /mob")
					return
				...
				var
					mob
						M = locate(href_list["direct_control"])
				if(!istype(M))
					to_chat(usr, "This can only be used on instances of type /mob")
					return
				...
				var
					mob
						living
							carbon
								human
									H = locate(href_list["make_skeleton"])
				if(!istype(H))
					to_chat(usr, "This can only be used on instances of type /mob/living/carbon/human")
					return

				var
					confirm = alert("Are you sure you want to turn this mob into a skeleton?","Confirm Skeleton Transformation","Yes","No")
				if(confirm != "Yes")
					return
				...
				var
					mob
						M = locate(href_list["offer_control"])
				if(!istype(M))
					to_chat(usr, "This can only be used on instances of type /mob")
					return
				...
				var
					list
						mob
							dead
								observer
									candidates = pollCandidates("Do you want to play as [M.real_name]?", poll_time = 100)
				var
					mob
						dead
							observer
				...
				var
					obj
						O = locate(href_list["delall"])
				if(!isobj(O))
					to_chat(usr, "This can only be used on instances of type /obj")
					return

				var
					action_type = alert("Strict type ([O.type]) or type and all subtypes?",,"Strict type","Type and subtypes","Cancel")
				...
				var
					atom
						A = locate(href_list["addreagent"])

				if(!A.reagents)
					var
						amount = input(usr, "Specify the reagent size of [A]", "Set Reagent Size", 50) as num
					if(amount)
				...
				if(A.reagents)
					var
						list
							reagent_options = list()
					for(var/r_id in chemical_reagents_list)
						var
							datum
								reagent
									R = chemical_reagents_list[r_id]
					...
					if(reagent_options.len)
						...
						var
							chosen = input(usr, "Choose a reagent to add.", "Choose a reagent.") in reagent_options
						var
							chosen_id = reagent_options[chosen]

						if(chosen_id)
							var
								amount = input(usr, "Choose the amount to add.", "Choose the amount.", A.reagents.maximum_volume) as num
							if(amount)
				...
				var
					datum
						D = locate(href_list["mark_object"])
				if(!istype(D))
				...
				var
					atom
						A = locate(href_list["rotatedatum"])
				if(!istype(A))
				...
				var
					mob
						living
							carbon
								human
									H = locate(href_list["setspecies"])
				if(!istype(H))
					to_chat(usr, "This can only be done to instances of type /mob/living/carbon/human")
					return

				var
					new_species = input("Please choose a new species.","Species",null) as null|anything in all_species
				...
				var
					mob
						H = locate(href_list["addlanguage"])
				if(!istype(H))
					to_chat(usr, "This can only be done to instances of type /mob")
					return

				var
					new_language = input("Please choose a language to add.","Language",null) as null|anything in all_languages
				...
				var
					mob
						H = locate(href_list["remlanguage"])
				if(!istype(H))
					to_chat(usr, "This can only be done to instances of type /mob")
					return
				...
				var
					datum
						language
							rem_language = input("Please choose a language to remove.","Language",null) as null|anything in H.languages
				...
				var
					verb = input("Select a verb!", "Verbs",null) as anything in possibleverbs
				if(!H)
					to_chat(usr, "Mob doesn't exist anymore")
					return
				if(!verb || verb == "Cancel")
				...
				var
					mob
						H = locate(href_list["remverb"])

				if(!istype(H))
					to_chat(usr, "This can only be done to instances of type /mob")
					return
				var
					verb = input("Please choose a verb to remove.","Verbs",null) as null|anything in H.verbs
				if(!H)
					to_chat(usr, "Mob doesn't exist anymore")
					return
				if(!verb)
				...
				var
					mob
						living
							carbon
								M = locate(href_list["addorgan"])
				if(!istype(M))
					to_chat(usr, "This can only be done to instances of type /mob/living/carbon")
					return

				var
					new_organ = input("Please choose an organ to add.","Organ",null) as null|anything in subtypesof(/obj/item/organ)-/obj/item/organ
				if(!new_organ) return
				...
				var
					obj
						item
							organ
								internal
									rem_organ = input("Please choose an organ to remove.","Organ",null) as null|anything in M.internal_organs
				...
				var
					mob
						M = locate(href_list["regenerateicons"])
				if(!ismob(M))
					to_chat(usr, "This can only be done to instances of type /mob")
					return
				M.regenerate_icons()
				...
				var
					mob
						living
							L = locate(href_list["mobToDamage"])
				if(!istype(L)) return
				...
				var
					amount =  input("Deal how much damage to mob? (Negative values here heal)","Adjust [Text]loss",0) as num
			...
			if(href_list["datumrefresh"])
				var
					datum
						DAT = locate(href_list["datumrefresh"])
				if(!istype(DAT, /datum))
					return
