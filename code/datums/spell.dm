...
obj
	effect
		proc_holder
			var
				panel = "Debug"//What panel the proc holder needs to go on.

var
	list
		spells = typesof(/obj/effect/proc_holder/spell) //needed for the badmin verb for now
	...
	effect
		proc_holder
			spell
				...
				var
					school = "evocation" //not relevant at now, but may be important later if there are changes to how spells work. the ones I used for now will probably be changed... maybe spell presets? lacking flexibility but with some other benefit?
				...
				var
					charge_max = 100 //recharge time in deciseconds if charge_type = "recharge" or starting charges if charge_type = "charges"
				var
				...
				var
					holder_var_type = "bruteloss" //only used if charge_type equals to "holder_var"
				var
					holder_var_amount = 20 //same. The amount adjusted with the mob's var when the spell is used

				var
					ghost = 0 // Skip life check.
				var
					clothes_req = 1 //see if it requires clothes
				var
					stat_allowed = 0 //see if it requires being conscious/alive, need to set to 1 for ghostpells
				var
					invocation = "HURP DURP" //what is uttered when the wizard casts the spell
				var
					invocation_emote_self = null
				var
					invocation_type = "none" //can be none, whisper and shout
				var
				...
				var
					level_max = 4 //The max possible level_max is 4
				var
					cooldown_min = 0 //This defines what spell quickened four timeshas as a cooldown. Make sure to set this for every spell
				...
				var
					critfailchance = 0
				var
					centcom_cancast = 1 //Whether or not the spell should be allowed on z2
...
obj
	effect
		proc_holder
			spell
				proc
					cast_check(skipcharge = 0, mob/living/user = usr) //checks if the spell can be cast based on its settings; skipcharge is used when an additional cast_check is called inside the spell
							...
							obj
								effect
									proc_holder
										spell
											noclothes
												spell = locate() in (user.spell_list | (user.mind ? user.mind.spell_list : list()))
...
obj
	effect
		proc_holder
			spell
				proc
					invocation(mob/user = usr) //spelling the spell out and setting it on recharge/reducing charges amount
...
obj
	effect
		proc_holder
			spell
				proc
					choose_targets(mob/user = usr) //depends on subtype - /targeted or /aoe_turf
						return
...
obj
	effect
		proc_holder
			spell
				proc
					perform(list/targets, recharge = 1, mob/user = usr) //if recharge is started is important for the trigger spells
						before_cast(targets)
						invocation()
							...
							for(var/atom/target in targets)
								...
								var
									obj
										effect
											overlay
												spell = new /obj/effect/overlay(location)
									...
									var
										datum
											effect
												system
													sleep_smoke_spread
														smoke = new /datum/effect/system/sleep_smoke_spread()
...
obj
	effect
		proc_holder
			spell
				proc
					revert_cast(mob/user = usr) //resets recharge or readds a charge
						switch(charge_type)
							if("recharge")
								charge_counter = charge_max
							if("charges")
...
obj
	effect
		proc_holder
			spell
				proc
					adjust_var(mob/living/target = usr, type, amount) //handles the adjustment of the var when the spell is used. has some hardcoded types
						switch(type)
							if("bruteloss")
...
obj
	effect
		proc_holder
			spell
				targeted //can mean aoe for mobs (limited/unlimited number) or one target mob
					var
...
obj
	effect
		proc_holder
			spell
				aoe_turf //affects all turfs in view or range (depends)
					var
						inner_radius = -1 //for all your ring spell needs
								...
								obj
									effect
										proc_holder
											spell
												noclothes
													clothcheck = locate() in user.spell_list
							var
								obj
									effect
										proc_holder
											spell
												noclothes
													clothcheck2 = locate() in user.mind.spell_list
