...
datum
	beam
		...
		var
			static_beam = 0
		var
			beam_type = /obj/effect/ebeam //must be subtype


datum
	beam
		New(beam_origin,beam_target,beam_icon='icons/effects/beam.dmi',beam_icon_state="b_beam",time=50,maxdistance=10,btype = /obj/effect/ebeam)
...
datum
	beam
		proc
			Draw()
				var
					Angle = round(Get_Angle(origin,target))
				...
				var
					N = 0
				var
					length = round(sqrt((DX)**2+(DY)**2)) //hypotenuse of the triangle formed by target and origin's displacement
...
atom
	proc
		Beam(atom/BeamTarget,icon_state="b_beam",icon='icons/effects/beam.dmi',time=50, maxdistance=10,beam_type=/obj/effect/ebeam)
			var
				datum
					beam
						newbeam = new(src,BeamTarget,icon,icon_state,time,maxdistance,beam_type)
			spawn(0)
				newbeam.Start()
			return newbeam
