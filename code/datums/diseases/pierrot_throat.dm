...
/datum/disease/pierrot_throat/stage_act()
	...
	if(!H.has_organ_for_slot(SLOT_HUD_MASK) || !H.canUnEquip(H.get_item_by_slot(SLOT_HUD_MASK)))
		return

	var/saved_internals = H.internal

	H.unEquip(H.get_item_by_slot(SLOT_HUD_MASK))
	var/obj/item/clothing/mask/gas/clown_hat/peak_comedy = new
	peak_comedy.flags |= DROPDEL
	H.equip_to_slot_or_del(peak_comedy, SLOT_HUD_MASK)
