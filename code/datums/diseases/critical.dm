...
/datum/disease/critical/powerfail
	name = "Power Failure"
	form = "Medical Emergency"
	max_stages = 3
	spread_flags = SPECIAL
	spread_text = "The patient is running on reserve power."
	cure_text = "Recharging internal microbattery."
	viable_mobtypes = list(/mob/living/carbon/human)
	stage_prob = 1
	severity = DANGEROUS
	disease_flags = CURABLE
...
/datum/disease/critical/hypoglycemia/stage_act()
	if(..())
		if(isLivingSSD(affected_mob)) // We don't want AFK people dying from this.
			return
		if(affected_mob.nutrition > NUTRITION_LEVEL_HYPOGLYCEMIA)
			to_chat(affected_mob, "<span class='notice'>Power restored.</span>")
			cure()
			return
		switch(stage)
			if(1)
				if(prob(4))
					to_chat(affected_mob, "<span class='warning'>Power failure imminent.</span>")
			if(2)
				if(prob(4))
					to_chat(affected_mob, "<span class='warning'>You feel like everything is wrong with your life!</span>")
				if(prob(5))
					affected_mob.Slowed(rand(8 SECONDS, 32 SECONDS))
					to_chat(affected_mob, "<span class='warning'>Power insufficient, redirecting power from servos.</span>")
				if(prob(5))
					affected_mob.Weaken(12 SECONDS)
					affected_mob.Stuttering(20 SECONDS)
					to_chat(affected_mob, "<span class='warning'>Power insufficient, redirecting all power to positronic brain.</span>")
					affected_mob.emote("collapse")
			if(3)
				if(prob(8))
