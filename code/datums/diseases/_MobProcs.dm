...
mob
	proc
		CanContractDisease(datum/disease/D)
			if(stat == DEAD)
				return 0
...
mob
	proc
		ContractDisease(datum/disease/D)
...
mob
	proc
		AddDisease(datum/disease/D)
			...
			var
				list
					skipped = list("affected_mob","holder","carrier","stage","type","parent_type","vars","transformed")
...
var
	target_zone = pick(head_ch;1,body_ch;2,hands_ch;3,feet_ch;4)
...
mob
	proc
		ForceContractDisease(datum/disease/D)
			if(!CanContractDisease(D))
				return 0
			AddDisease(D)
