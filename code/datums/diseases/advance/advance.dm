...
datum
	disease
		advance
			...
			var
				list
					symptoms = list() // The symptoms of the disease.
			var
				id = ""
			var
				processing = 0
...
datum
	disease
		advance
			New(var/process = 1, var/datum/disease/advance/D)
...
datum
	disease
		advance
			IsSame(datum/disease/advance/D)
...
datum
	disease
		advance
			proc
				GenerateSymptoms(level_min, level_max, amount_get = 0)

					var
						list
							generated = list() // Symptoms we generated.
...
//	to_chat(world, "[src.name] \ref[src] - REFRESH!")
	...
	var
		datum
			disease
				advance
					A = archive_diseases[GetDiseaseID()]
	AssignName(A.name)
		...
		advance
			proc
				GenerateProperties()
					...
					var
						list
							properties = list("resistance" = 1, "stealth" = 0, "stage_rate" = 1, "transmittable" = 1, "severity" = 0)
...
datum
	disease
		advance
			proc
				AssignProperties(list/properties = list())
...
datum
	disease
		advance
			proc
				GenerateCure(list/properties = list())
					if(properties && properties.len)
						var
							res = Clamp(properties["resistance"] - (symptoms.len / 2), 1, advance_cures.len)
//		to_chat(world, "Res = [res]")
		...
		var
			datum
				reagent
					D = chemical_reagents_list[cures[1]]
		cure_text = D.name
...
datum
	disease
		advance
			proc
				Evolve(min_level, max_level)
					var
						s = safepick(GenerateSymptoms(min_level, max_level, 1))
					if(s)
						AddSymptom(s)
						Refresh(1)
					return
...
//	to_chat(world, "END MIXING!!!!!")
	var
		datum
			disease
				advance
					to_return = pick(diseases)
	to_return.Refresh(1)
	return to_return

proc
	SetViruses(datum/reagent/R, list/data)
		if(data)
			var
				list
					preserve = list()
...
proc
	AdminCreateVirus(client/user)
		...
		do
			if(user)
				var
					symptom = input(user, "Choose a symptom to add ([i] remaining)", "Choose a Symptom") in symptoms
				if(isnull(symptom))
					return
		...
		if(D.symptoms.len > 0)
			...
			var
				new_name = stripped_input(user, "Name your new disease.", "New Name")
			if(!new_name)
				return
