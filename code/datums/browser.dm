	...
	browser
		...
		var
			atom
				ref = null
		var
			window_options = "focus=0;can_close=1;can_minimize=1;can_maximize=0;can_resize=1;titlebar=1;" // window option is set using window_id
		var
			stylesheets[0]
		var
			scripts[0]
...
datum
	browser
		New(nuser, nwindow_id, ntitle = 0, nwidth = 0, nheight = 0, var/atom/nref = null)
...
datum
	browser
		proc
			add_head_content(nhead_content)
				head_content = nhead_content

datum
	browser
		proc
			set_title_buttons(ntitle_buttons)
...
mob
	proc
		browse_rsc_icon(icon, icon_state, dir = -1)
			/*
			var
				icon
					I
...
client
	verb
		windowclose(var/atomref as text)
...
//	to_chat(world, "windowclose: [atomref]")
	if(atomref!="null")				// if passed a real atomref
		var
			hsrc = locate(atomref)	// find the reffed atom
		if(hsrc)
