...
/proc/get_uplink_items(uplink_type)
	...
	if(!uplink_items.len)
		...
		var/list/last = list()
		is_allowed_on_uplink: //Yes this is not normal but it's how we continue the loop neatly from nested loops
			for(var/path in GLOB.uplink_items)

				var/datum/uplink_item/I = new path
				if(!I.item)
					continue
				if(I.on_uplink_type.len)
					for(var/position in 1 to I.on_uplink_type.len)
						if(!istype(uplink_type, I.on_uplink_type[position]))
							continue is_allowed_on_uplink

				if(I.exclude_from_uplink_type.len)
					for(var/position in 1 to I.exclude_from_uplink_type.len)
						if(istype(uplink_type, I.exclude_from_uplink_type[position]))
							continue is_allowed_on_uplink
				if(I.last)
					last += I
					continue
...
/datum/uplink_item/proc/spawn_item(turf/loc, datum/uplink/U)
...
/datum/uplink_item/proc/buy(datum/uplink/U, mob/user)
...
/datum/uplink_item/jobspecific
	...
	cant_discount = TRUE
	exclude_from_uplink_type = list(UPLINK_OPERATIVE) // Stops the job specific category appearing for nukies
...
/datum/uplink_item/jobspecific/poison_pen
	...
	cost = 2
	exclude_from_uplink_type = list(UPLINK_OPERATIVE)
...
/datum/uplink_item/dangerous/smg
	...
	cost = 14
	on_uplink_type = list(UPLINK_OPERATIVE)
	surplus = 40
...
/datum/uplink_item/bundles_TC/contractor/spawn_item(turf/loc, datum/uplink/U)
...
/datum/uplink_item/bundles_TC/surplus_crate/spawn_item(turf/loc, datum/uplink/U)
	var/obj/structure/closet/crate/C = new(loc)
	var/list/temp_uplink_list = get_uplink_items(UPLINK_TRAITOR)
