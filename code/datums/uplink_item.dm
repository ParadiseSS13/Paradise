...
datum
	uplink_item
		...
		var
			list
				gamemodes = list() // Empty list means it is in all the gamemodes. Otherwise place the gamemode name here.
		var
			list
				excludefrom = list() //Empty list does nothing. Place the name of gamemode you don't want this item to be available in here. This is so you dont have to list EVERY mode to exclude something.
		var
			list
				job = null
		var
			surplus = 100 //Chance of being included in the surplus crate (when pick() selects it)
		var
			hijack_only = 0 //can this item be purchased only during hijackings?

datum
	uplink_item
		proc
			spawn_item(var/turf/loc, var/obj/item/device/uplink/U)
				if(hijack_only)
...
datum
	uplink_item
		proc
			buy(var/obj/item/device/uplink/hidden/U, var/mob/user)
				..()
...
datum
	uplink_item
		cyber_implants
			spawn_item(turf/loc, obj/item/device/uplink/U)
				if(item)
		...
		badass
			random
				spawn_item(var/turf/loc, var/obj/item/device/uplink/U)
					...
					var
						list
							buyable_items = get_uplink_items()
					var
						list
							possible_items = list()
		...
		badass
			surplus_crate
				spawn_item(turf/loc, obj/item/device/uplink/U)
					...
					var
						list
							temp_uplink_list = get_uplink_items()
					var
						list
							buyable_items = list()
