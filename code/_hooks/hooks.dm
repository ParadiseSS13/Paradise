...
hook
	var
		name = "DefaultHookName"
	var
		list
			handlers = list()

	proc
		Called(var/list/args) // When the hook is called
			return 0

	proc
		Setup() // Called when the setup things is ran for the hook, objs contain all objects with that is hooking
