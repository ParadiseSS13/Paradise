/obj/item/taperoll/police/Initialize()
	. = ..()
	if(apply_tape)
...
/obj/item/taper/police/update_icon()
...
/obj/item/taperoll/police
	...
	var/turf/start
	var/turf/end
	var/tape_type = /obj/item/taper/police
	var/icon_base = "tape"
	var/maxlengths = 7
...
/obj/item/taperoll/police/engineering
	name = "engineering tape"
	desc = "A roll of engineering tape used to block off working areas from the public."
	tape_type = /obj/item/taper/police/engineering
	color = COLOR_ORANGE

/obj/item/taper/police/engineering
	name = "engineering tape"
...
/obj/item/taperoll/police/update_icon()
	overlays.Cut()
...
/obj/item/taperoll/police/dropped(mob/user)
	update_icon()
	return ..()

/obj/item/taperoll/police/pickup(mob/user)
	update_icon()
	return ..()

/obj/item/taperoll/police/attack_hand()
	update_icon()
	return ..()

/obj/item/taperoll/police/attack_self(mob/user as mob)
	if(!start)
		start = get_turf(src)
		...
		if(start == end)
			...
			if(possible_dirs & (NORTH|SOUTH))
				var/obj/item/taper/police/TP = new tape_type(start)
				...
				if(T && !T.density)
					...
					for(var/obj/structure/window/W in T)
						if(W.fulltile || W.dir == reverse_direction(orientation))
							tape_dir = dir
			for(var/obj/item/taper/police/T in cur)
				if((T.tape_dir == tape_dir) && (T.icon_base == icon_base))
					tapetest = 1
					break
			if(!tapetest)
				var/obj/item/taper/police/T = new tape_type(cur)
				T.tape_dir = tape_dir
				T.update_icon()
				if(tape_dir & SOUTH)
...
/obj/item/taperoll/police/afterattack(var/atom/A, mob/user as mob, proximity)
	if(!proximity)
		return

	if(istype(A, /obj/machinery/door/airlock))
		var/turf/T = get_turf(A)
		var/obj/item/taper/police/P = new tape_type(T)
		P.update_icon()
...
/obj/item/taper/police/proc/crumple()
	playsound(src,'sound/effects/pageturn1.ogg', 100, 1)
	crumpled = 1
	update_icon()

/obj/item/taper/police/CanPass(atom/movable/mover, turf/target, height=0, air_group=0)
...
/obj/item/taper/police/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(user.a_intent == INTENT_HARM)
		breaktape(user)

/obj/item/taper/police/attack_hand(mob/user as mob)
	if(user.a_intent == INTENT_HELP)
		user.visible_message("<span class='notice'>\The [user] lifts \the [src], allowing passage.</span>")
		for(var/obj/item/taper/police/T in gettapeline())
			crumple()
			T.lift(20) //~2 seconds
	else
		breaktape(user)

/obj/item/taper/police/proc/lift(time)
	lifted = 1
...
/obj/item/taper/police/proc/gettapeline()
	var/list/dirs = list()
	if(tape_dir & NORTH)
		dirs += NORTH
	if(tape_dir & SOUTH)
	...
	var/list/obj/item/taper/police/tapeline = list()
	for (var/obj/item/taper/police/T in get_turf(src))
		tapeline += T
	for(var/dir in dirs)
...
/obj/item/taper/police/proc/breaktape(mob/user)
	if(user.a_intent == INTENT_HELP)
