...
/datum/event/temperature_change/announce()
	event_announcement.Announce("Major failure detected within the ventilation system. Strange temperature changes expected within the next minutes.")

/datum/event/temperature_change/end()
	alarm_controller(T20C)
	event_announcement.Announce("Temperature failure cleared, report to medbay if you feel unwell.")
