/obj/item/paper_bundle
	...
	var/page = 1
	var/screen = 0
	var/book = 0 /// 1 when it's turned into a book
	var/dat
...
/obj/item/paper_bundle/attackby(obj/item/W as obj, mob/user as mob, params)
	...
	if(istype(W, /obj/item/paper))
		...
		if (book)
			return
		else if(istype(W, /obj/item/pen) || istype(W, /obj/item/toy/crayon))
	...
	if((src in usr.contents) || (istype(src.loc, /obj/item/folder) && (src.loc in usr.contents)))
		...
		if(href_list["remove"])
			...
			if (book)
				...
				..()
				to_chat(usr, "<span class='notice'>You rip the [W.name] off the book!.</span>")
			else
