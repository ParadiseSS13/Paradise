...
export const dragStartHandler = (event) => {
  ...
  dragging = true;
  dragPointOffset = vecSubtract([event.screenX * pixelRatio, event.screenY * pixelRatio], getWindowPosition()) as [
    number,
    number,
  ];
  // Focus click target
...
const dragMoveHandler = (event: MouseEvent) => {
  ...
  setWindowPosition(
    vecSubtract([event.screenX * pixelRatio, event.screenY * pixelRatio], dragPointOffset) as [number, number]
  );
};
...
const resizeMoveHandler = (event: MouseEvent) => {
  ...
  event.preventDefault();
  const currentOffset = vecSubtract([event.screenX * pixelRatio, event.screenY * pixelRatio], getWindowPosition());
  const delta = vecSubtract(currentOffset, dragPointOffset);
  // Extra 1x1 area is added to ensure the browser can see the cursor
  size = vecAdd(initialSize, vecMultiply(resizeMatrix, delta), [1, 1]) as [number, number];
