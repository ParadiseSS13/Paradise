...
const RodDisplay = ({ rods, title }) => {
  ...
  if (!rods || rods.length === 0) {
    return (
      <Section title={title}>
        <NoticeBox type="info">
          No **{title.toLowerCase()}** designs found.
        </NoticeBox>
      </Section>
    );
  }
                ...
                <Stack vertical>
                  <Stack.Item>
                    <Box bold fontSize="1.1em">
                      {rod.name || <Box inline color="bad">[Unnamed Design]</Box>}
                    </Box>
                  </Stack.Item>

                  <Stack.Item>
                    <Box fontSize="0.9em" color="label" mb={0.5}>
                      {rod.desc || <Box inline color="average">[No description available]</Box>}
                    </Box>
                  </Stack.Item>

                  <Stack.Item>
                    <Box fontSize="0.85em">
                      <Box inline color="label">Heat Mod:</Box>{' '}
                      <Box inline bold color={rod.heat_amp_mod ? "good" : "bad"}>
                        {rod.heat_amp_mod ?? "N/A"}
                      </Box>
                      {' | '}
                      <Box inline color="label">Power Mod:</Box>{' '}
                    ...
                    <Tooltip
                      content={
                        <Box p={1} style={{ whiteSpace: "pre-wrap", maxWidth: "400px" }}>
                          <Box bold mb={0.5}>Full Object Data:</Box>
    ...
    <Window width={600} height={800}>
      <Window.Content scrollable>
        <Stack vertical fill>
          ...
          <RodDisplay
            rods={data.fuel_rods}
            title="Fuel Rod Designs"
          />

          <RodDisplay
            rods={data.moderator_rods}
            title="Moderator Rod Designs"
          />

          <RodDisplay
            rods={data.coolant_rods}
            title="Coolant Rod Designs"
          />
