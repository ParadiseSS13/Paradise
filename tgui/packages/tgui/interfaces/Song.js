...
export const Song = (properties, context) => {
...
const SongHelp = (properties, context) => {
  const { act, data } = useBackend(context);
  const {
    help,
  } = data;
...
const SongInstrument = (properties, context) => {
  const { act, data } = useBackend(context);
  const {
    lines,
    tempo,
      ...
      <LabeledList>
        ...
        <LabeledList.Item label="Repeat">
          <Box>
            ...
            {repeat}
            <Button
              disabled={playing || repeat >= maxRepeat}
              content="+"
              as="span"
              ml="0.5rem"
              onClick={() => act('repeat', {
        ...
        <LabeledList.Item label="Tempo">
          <Box>
            <Button
              disabled={playing || tempo >= maxTempo}
              content="-"
              as="span"
              mr="0.5rem"
              onClick={() => act('tempo', {
            ...
            <Button
              disabled={playing || tempo <= minTempo}
              content="+"
              as="span"
              ml="0.5rem"
              onClick={() => act('tempo', {
...
const SongEditor = (properties, context) => {
  const { act, data } = useBackend(context);
  const {
    playing,
    lines,
