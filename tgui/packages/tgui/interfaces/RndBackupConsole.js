...
import { useBackend, act } from "../backend";
          ...
          <Box mb={2}>
            <LabeledList>
              <LabeledList.Item label="Current Network">
                {linked ? <Button content={network_name} icon="unlink" selected={1} onClick={() => act('unlink')} /> : "None" }
              </LabeledList.Item>
              <LabeledList.Item label="Loaded Disk">
                {has_disk ? <Fragment>
                  <Button content={disk_name + " (Last backup: " + (last_timestamp) + ")"} icon="save" selected={1} onClick={() => act('eject_disk')} />
              ...
              {Object.keys(techs).map(t => (
                (!((techs[t]["network_level"] > 0) || (techs[t]["disk_level"] > 0)) || (
                  <Table.Row key={t}>
                    ...
                    <Table.Cell>
                      <Button icon="sign-in-alt" content="Load to network" disabled={!has_disk || !linked} onClick={() => act('savetech2network', { tech: t })} />
                      <Button icon="sign-out-alt" content="Load to disk" disabled={!has_disk || !linked} onClick={() => act('savetech2disk', { tech: t })} />
                    </Table.Cell>
                  </Table.Row>
                ))
              ))}
