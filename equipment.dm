...
/datum/spacepod/equipment
	...
	var/obj/item/device/spacepod_equipment/lock/lock_system // door locks
...
// ID card System
/obj/item/device/spacepod_equipment/lock/card
	name = "\improper spacepod card lock system"
	desc = "A locking system to stop podjacking. This version uses a swipe of an ID card."
	icon_state = "pod_locator"

/obj/item/device/spacepod_equipment/lock/card/attackby(obj/item/I as obj, mob/user as mob, params)
	if(isscrewdriver(I))
		if(mode)
			mode = 0
			to_chat(user, "<span class='notice'>You change \the [src]'s setting to personal")
			return
		mode = 1
		to_chat(user, "<span class='notice'>You change \the [src]'s setting to access.</span>")
	else
		..()